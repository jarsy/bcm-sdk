...
/*
 * $Id: linux-uk-proxy.c,v 1.19 Broadcom SDK $
...
#define LDBG(l, str) gprintk("%d:%s:%s - %s\n", current->pid, l->service, l->name, str)
    ...
    if (flags & _LIST_OP_F_PUT) {
        ...
        if (flags & _LIST_OP_F_USER) {
            ...
            if (raw_copy_from_user((void *)(unsigned long)dp->data, data, *len)) {
                return -EFAULT;
            }
	} else {
	...
	if (flags & _LIST_OP_F_USER) {
            ...
            if (raw_copy_to_user(data, (void *)(unsigned long)dp->data, dp->len)) {
                return -EFAULT;
            }
        } else {
...
_ioctl(unsigned int cmd, unsigned long arg)
{
    ...
    if (raw_copy_from_user(&io, (void*)arg, sizeof(io))) {
        return -EFAULT;
    }
    ...
    if (raw_copy_to_user((void*)arg, &io, sizeof(io))) {
        return -EFAULT;
    }
