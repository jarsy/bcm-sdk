...
bkn_dnx_packet_parse_ftmh(uint8_t hdr_buff[], bkn_dnx_packet_info *packet_info)
{
    ...
    DBG_DUNE(("FTMH(%d) Packet-size %d Action-type %d Source-system-port 0x%x Traffic-class %d\n",
              packet_info->ntwrk_header_ptr, packet_info->ftmh.packet_size, packet_info->ftmh.action_type, packet_info->ftmh.src_sys_port, packet_info->ftmh.prio));

    
    /* FTMH load balancing extension disable */
    /* Relevant only if ftmh_mode_is_petra_B is TRUE. */
    ...
    /* ext_msb = BKN_DNX_FTMH_FIRST_EXT_MSB; */
    if (dsp_ext_exist)
    {
        ...
        /*
        packet_info->ftmh.dest_ext.valid = 1;
        bkn_dnx_bitstream_get_field(
             &hdr_buff[header_ptr],
             ext_msb,
        ...
        packet_info->ftmh.dest_ext.dst_sysport=fld_val;
        */
    }
    
    /* disable stacking headers by default */
    /* disable tdm headers by default */
...
bkn_dnx_packet_parse_internal(uint8_t hdr_buff[], bkn_dnx_packet_info *packet_info)
{
    ...
    bkn_dnx_bitstream_get_field(
            &hdr_buff[header_ptr],
            4 /* ARAD_PP_INTERNAL_7 */,
            4 /* ARAD_PP_INTERNAL_8 */,
            &forward_code
         );
    is_trapped = (uint8_t)(forward_code == 7 /* ARAD_PP_TRAP_MGMT_IS_TRAP_VAL */);
    bkn_dnx_bitstream_get_field(
        &hdr_buff[header_ptr],
        1, /* ARAD_PP_INTERNAL_15 */
        1, /* ARAD_PP_INTERNAL_16 */
        &fld_val
    );
    l_ext = fld_val;
    ...
    bkn_dnx_bitstream_get_field(
         &hdr_buff[header_ptr],
         0, /* ARAD_PP_INTERNAL_13 */
         1, /* ARAD_PP_INTERNAL_14 */
         &e_ext
    );
    bkn_dnx_bitstream_get_field(
         &hdr_buff[header_ptr],
         22, /* ARAD_PP_INTERNAL_13 */
         16, /* ARAD_PP_INTERNAL_14 */
         &fld_val
    );
    packet_info->internal.vsi = fld_val;
     ...
     bkn_dnx_bitstream_get_field(
         &hdr_buff[header_ptr],
         2, /* ARAD_PP_INTERNAL_5 */
         2, /* ARAD_PP_INTERNAL_6 */
         &app_ext
     );

     /* Advance header ptr to end of internal */
     packet_info->ntwrk_header_ptr +=  7; /* ARAD_PP_INTERNAL_1 */
     header_ptr = packet_info->ntwrk_header_ptr;

    /* trap info */
    if (is_trapped && app_ext)
    {
        /* CPU trap code qualifier */
        bkn_dnx_bitstream_get_field(
                &hdr_buff[header_ptr],
                0, /* ARAD_PP_INTERNAL_E_1 */
                16, /* ARAD_PP_INTERNAL_E_2 */
                &fld_val
        );
       ...
       /* CPU trap code */
        bkn_dnx_bitstream_get_field(
          &hdr_buff[header_ptr],
          16, /* ARAD_PP_INTERNAL_E_3 */
          8, /* ARAD_PP_INTERNAL_E_4 */
          &fld_val
          );
   ...
   switch(app_ext) {
    ...
    if (e_ext)
    {
        packet_info->ntwrk_header_ptr += 3; /* ARAD_PP_INTERNAL_3 */
    }

    if (l_ext) {
        packet_info->ntwrk_header_ptr += 5; /* ARAD_PP_INTERNAL_2 */
    }

    DBG_DUNE(("PPH(%d) Forward-Code %d EEI-Extension %d Learn-Extension %d VSI %d FHEI %d\n", packet_info->ntwrk_header_ptr,
        forward_code, e_ext, l_ext, packet_info->internal.vsi, app_ext));
    DBG_DUNE(("FHEI(3B) trap_qualifier 0x%x trap_id 0x%x\n", packet_info->internal.trap_qualifier, packet_info->internal.trap_id));
    return rc;
...
bkn_dnx_packet_header_parse(uint8 *buff, uint32_t buff_len, bkn_dnx_packet_info *packet_info)
{
    ...
    /* FTMH */
    rc = bkn_dnx_packet_parse_ftmh(hdr_buff, packet_info);

    /* internal type */
    fld_val = 0;
    bkn_dnx_bitstream_get_field(
        &hdr_buff[header_ptr],
        BKN_DNX_FTMH_EXT_MSB,
        BKN_DNX_FTMH_EXT_NOF_BITS,
        &fld_val
    );
    internal_type = fld_val;
    DBG_DUNE(("FTMH(%d) PPH-type %d\n", packet_info->ntwrk_header_ptr, internal_type));
    ...
    case 3: /*Base + OAM-TS*/
       ...
       /* no parse */
       packet_info->ntwrk_header_ptr +=  6; /* ARAD_PP_INTERNAL_4 */
    ...
    if (has_internal) {
      ...
      /* internal */
      rc = bkn_dnx_packet_parse_internal(&hdr_buff[0], packet_info);
      /* fwd_hdr_offset */
      bkn_dnx_bitstream_get_field(
        &hdr_buff[header_ptr],
        8, /* ARAD_PP_INTERNAL_9 */
        7, /* ARAD_PP_INTERNAL_10 */
        &fld_val
        );
    }
...
bkn_do_api_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcb_chain->dcb_cur < dcb_chain->dcb_cnt) {
        ...
        if (device_is_dune(sinfo)) {
            ...
            bkn_dnx_packet_header_parse(pkt, (uint32_t)pktlen, &packet_info);
            ...
            bkn_dnx_bitstream_set_field((uint32_t *)dcb, 281, 5, packet_info.ftmh.src_sys_port);
                ...
                if (priv) {
                    ...
                    if ((priv->flags & KCOM_NETIF_F_KEEP_RX_TAG) == 0) {
                        if (filter->kf.flags & KCOM_FILTER_F_STRIP_TAG) {
                            /* Strip the VLAN tag */
                            /* XGS: alway with vlan tag */
                            /* DUNE: depends on packets */
...
bkn_do_skb_rx(bkn_switch_info_t *sinfo, int chan, int budget)
{
    ...
    while (dcbs_done < budget) {
        ...
        if (device_is_dune(sinfo)) {
            memset(&packet_info, 0, sizeof(bkn_dnx_packet_info));
            bkn_dnx_packet_header_parse(skb->data, (uint32_t)pktlen, &packet_info);
            if (packet_info.ftmh.action_type == 0x2) {
                bkn_dnx_bitstream_set_field((uint32_t *)dcb, 192, 32, 0x8);
            } else if (packet_info.ftmh.action_type == 0x1) {
                bkn_dnx_bitstream_set_field((uint32_t *)dcb, 231, 25, 0x20);
            ...
            case KCOM_DEST_T_NETIF:
                ...
                if (priv) {
                    ...
                    if (device_is_dune(sinfo)) {
                        ...
                        /* Strip Dune headers */
                        skb->len = pktlen;
                        skb_pull(skb, packet_info.ntwrk_header_ptr);
                        pktlen -= packet_info.ntwrk_header_ptr;
                        bkn_dump_pkt(skb->data, 32);
                    ...
                    if ((filter->kf.mirror_type == KCOM_DEST_T_API) && (!device_is_dune(sinfo))) {
                        sinfo->rx[chan].pkts_m_api++;
                        bkn_api_rx_copy_from_skb(sinfo, chan, desc);
                    }
