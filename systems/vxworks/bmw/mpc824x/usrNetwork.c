...
/* $Id: usrNetwork.c,v 1.6 2011/07/21 16:14:08 yshtil Exp $
...
STATUS usrNetInit 
	...
	if (usrNetIfAttach (params.bootDev, params.unitNum, params.bad) != OK) 
            return (ERROR);

        if (usrNetIfConfig (params.bootDev, params.unitNum, params.bad, 
	...
	netmask = 0;
		...
		{
		printf("Attaching network interface %s%d... ",
		       params.other, params.unitNum);
#ifdef INCLUDE_END
		/* Find the END_OBJ associated with it. */

		pEnd = endFindByName(params.other, params.unitNum);
		if (pEnd == NULL)
		    return (ERROR);
#if VX_VERSION == 55
#else
		if (muxIoctl(pEnd, EIOCGMIB2, (caddr_t) &endM2Tbl)
		    ...
		    return (ERROR);
#endif
		if (ipAttach(params.unitNum, params.other) != OK)
		    {
		    logMsg("Failed to attach to device %s",
			   (int)params.bootDev, 2, 3, 4, 5, 6);
		    return (ERROR);
		    }
#if defined(BROADCOM_BSP)
        if  (sysIsCFM()) {
		if (usrNetIfConfig (params.other, params.unitNum,
                    (sysSlotIdGet() ? "192.168.1.1":"192.168.0.1"),
				    params.targetName,
				    0xffffff00) !=OK)
		    return (ERROR);
        } else {
		if (usrNetIfConfig (params.other, params.unitNum, params.ead,
				    params.targetName,
				    netmask) !=OK)
		    return (ERROR);
        }
...
#endif
#else
		if (usrNetIfAttach (params.other, params.unitNum, params.ead) 
                    != OK)
		    return (ERROR);
	    ...
	    printf("Attaching network interface %s%d... ",
		   params.other, params.unitNum);
#ifdef INCLUDE_END
            /* Find the END_OBJ associated with it. */
            pEnd = endFindByName(params.bootDev, params.unitNum);
            if (pEnd == NULL)
                return (ERROR);
#if VX_VERSION == 55
#else
            if (muxIoctl(pEnd, EIOCGMIB2, (caddr_t) &endM2Tbl)
                ...
                return (ERROR);
#endif
            if (ipAttach(params.unitNum, params.bootDev) != OK)
                {
                logMsg("Failed to attach to device %s",
                       (int)params.bootDev, 2, 3, 4, 5, 6);
                return (ERROR);
                }
            if (usrNetIfConfig (params.bootDev, params.unitNum, params.ead,
                                params.targetName,
                                netmask) !=OK)
                return (ERROR);
#else
            if ((usrNetIfAttach (params.bootDev, params.unitNum,
                                 params.ead) !=OK) ||
                (usrNetIfConfig (params.bootDev, params.unitNum,
                                 params.ead, params.targetName,
                                 netmask) !=OK))
                return (ERROR);
#endif /* INCLUDE_END */
	    printf("done.\n");
...
#define BP_ADDR_NOT_GIVEN	(params.bad [0] == EOS)
	...
	useEtherAddr = TRUE;
	    ...
	    if (usrBpInit (bpDev, params.unitNum, 0) == ERROR)
	        return (ERROR);
    ...
    if (dhcpcConfigSet (&params, &netmask, dhcpBoot) == ERROR)
        return (ERROR);
