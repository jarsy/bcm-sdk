...
sb_corereg(si_t *sih, uint coreidx, uint regoff, uint mask, uint val)
{
	...
	if (regoff >= SBCONFIGOFF)
		...
		if ((CHIPID(sii->pub.chip) == BCM5354_CHIP_ID) &&
			...
			/*
			 * PR42921: The readback of chipcommon watchdog
			 * register hangs for 5354 after its write.
			 */
			w = val;
		} else
			w = R_REG(sii->osh, r);
...
#if 0
	else if (BUSTYPE(sii->bustype) == JTAG_BUS) {
...
sb_core_tofixup(si_t *sih)
{
	...
	/* just return if no need for workaround */
