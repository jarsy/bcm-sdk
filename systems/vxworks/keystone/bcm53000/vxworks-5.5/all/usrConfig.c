...
/* $Id: usrConfig.c,v 1.5 2011/07/21 16:14:27 yshtil Exp $
...
#if 0
/* JJA DEBUG */
volatile unsigned int *plog = (volatile unsigned int *)0xa6000000;

/*extern volatile unsigned int *plog;*/
#define JTRACE() do { *plog++ = __LINE__; } while(0)
#define JLOG(x)  do { *plog++ = (unsigned int)(x); } while(0)

LOCAL int usr_clk_cnt=0;
int sys_clk_bp0 = 0; 
int sys_clk_bp1 = 0;
int sys_clk_bp2 = 0;
int sys_clk_bp3 = 0;
int sys_sr_0 = 0xffffffff;
int sys_sr_1 = 0xffffffff; 
 ...
 */
  cacheCreateInternalDataRAM((UINT32 *)FD_ORIGIN, FD_NUMLINES);
    ...
    /*
     * For Keystone BSP VxWorks 5.5, higher memory is default disabled.
     * To enable the feature, CACHE_MEM_POOL_RESERVED has to be defined.
     * The cache pool size is determined by CACHEMEM_POOL_SIZE defined
     * in the config.h.
     */
    {
...
#error	Define only one of INCLUDE_CPLUS or INCLUDE_CPLUS_MIN, not both
	...
	extern unsigned int Bcm53000HiMemAddr;
        if (Bcm53000HiMemSize > 0) {
