...
/* $Id: usrConfig.c,v 1.6 2011/07/21 16:14:31 yshtil Exp $
...
#ifdef _MIPS_SDA_SUPPORTED /* SDA support is not ready yet. Do not enable it. */
IMPORT void vxSdaInit (void);
#endif  /* _MIPS_SDA_SUPPORTED */
...
#if (CPU_FAMILY == PPC) /* || (CPU_FAMILY == MIPS)) MIPS SDA not ready yet */
    /* 
...
# endif	/* defined(_WRS_OSM_INIT) */
    ...
    /* Activate all idle tasks */

    kernelIdleTaskActivate ();
#endif /* _WRS_VX_SMP  */
...
/* Initialize the Cross-processor Call library */

#if (CPU_FAMILY == PPC) || (CPU_FAMILY == MIPS) || (CPU_FAMILY == I80X86) \
...
# endif	/* INCLUDE_LOG_STARTUP */
    ...
    usrHrfsInit (HRFS_DEFAULT_MAX_BUFFERS, HRFS_DEFAULT_MAX_FILES,
                 HRFS_DEFAULT_COMMIT_POLICY, HRFS_DEFAULT_COMMIT_PERIOD);
#endif
    ...
    dosFsCacheLibInit (DOSFS_DEFAULT_DATA_CACHE_SIZE,
...
#if 0
    /*
     * For Keystone BSP VxWorks 6.6, higher memory is enabled and adds to
     * system pool. But the cache pool is default disabled.
     * The following code section is the sample code to create cache pool.
     * The pool size is determined by CACHEMEM_POOL_SIZE defined in the
     * config.h.
     */
    {
	extern int cachePoolCreate(), Bcm53000HiMemSize;
        extern int cacheBcm53000LibUpdate();

	if(Bcm53000HiMemSize > 0) {
	    printf("Detected %dMB Highmem (%dMB total memory)\n",
		    Bcm53000HiMemSize>>20,
		    (Bcm53000HiMemSize+BCM53000_SDRAM_SZ)>>20);
	    printf("Reserving %dMB Lowmem for DMA: ", CACHEMEM_POOL_SIZE>>20);
	    if(cachePoolCreate() != OK) {
		printf("Failed!  Giving up on Highmem.\n");
		Bcm53000HiMemSize = 0;
	    } else {
               /* Update DMA alloc/free routines */
              cacheBcm53000LibUpdate();
		printf("OK.\n");
	    }
	}
    }
...
/*  usrMmuOptimize needs to be called before the additional CPU's are enabled.*/
    ...
    /* enable all CPUs for SMP if ENABLE_ALL_CPUS is set to TRUE */
