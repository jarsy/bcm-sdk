 ...
 * $Id: osl_vxworks.c,v 1.1 2004/02/24 07:47:00 csm Exp $
 * 
...
extern struct semaphore robo_sema;
#endif /* user mode */
#else
#error "This OSL is for Linux only."
#endif /* !__linux__ */
...
#if 0 /* remove for vxworks */
#ifdef __KERNEL__
    int time_wait = 1, usec = BCM_SEM_TIMEOUT_VAL;
    if (!in_interrupt()) {
        down(&robo_sema);
    }
    else {
        for (;;) {
            if (down_trylock(&robo_sema) == 0)
                break;

            time_wait = (time_wait > usec) ? usec : time_wait;
	    udelay(time_wait);
	    usec -= time_wait;
	    if (usec == 0) {
		retval = BCM_RET_SEMA_UNAVAIL;
		break;
	    }
	    time_wait *= 2;
        }
...
#else
    if ((retval = ioctl(robo_fd, UBSEC_ROBOGETSEM, NULL))) {
        printf("getsem ioctl failed %x\n",retval);
    }
#endif
#endif /* remove for vxworks */
    return retval;
...
bcm_rel_sema(void)
{
    ...
    up(&robo_sema);
#else
    if ((retval = ioctl(robo_fd, UBSEC_ROBORELSEM, NULL))) {
        printf("relsem ioctl failed %x\n",retval);
    }
#endif
#endif /* remove for vxworks */
    return retval;
