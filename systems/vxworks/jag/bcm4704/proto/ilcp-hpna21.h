 ...
 *
 * $Id: ilcp-hpna21.h,v 1.1 2004/02/24 07:47:01 csm Exp $
 */
...
#if BCMINTERNAL
#define ILCP_SUBTYPE_SRS		6
#endif /* BCMINTERNAL */

#define ILCP_SUBTYPE_VENDOR_LONG	32769
#define ILCP_SUBTYPE_CERT		32770
#if BCMINTERNAL
#define ILCP_SUBTYPE_LRS		32771

#define ILCP_EXT_PAD 0
#define ILCP_EXT_VENDOR 1
#define ILCP_EXT_VENDOR_LONG 32769
#define ILCP_EXT_CSA_STLIST 2
#define ILCP_EXT_CSA_STPARAMS 3
#define ILCP_EXT_CSA_RSPARAMS 4
#endif /* BCMINTERNAL */
...
#define CSA_CUR_TX_OFF (CSA_MIN_LENGTH - sizeof(uint16) - 3*sizeof(uint32))
#define CSA_OLD_TX_OFF (CSA_MIN_LENGTH - sizeof(uint16) - 2*sizeof(uint32))
#define CSA_CUR_RX_OFF (CSA_MIN_LENGTH - sizeof(uint16) - sizeof(uint32))
...
#ifdef BCMINTERNAL
#ifndef IL_RS
#define IL_RS 1 /* for now, compile Reed Solomon header stuff */
#endif

#if IL_RS
/* The basic RS info, repeated 3 times in a short RS header */
typedef union rs_hdr_fields
{
	uint16		rs_fields;
	struct 
	{
		uint8		rs_vers_len;/* 4 bits length mod 16, plus version(0) */
		uint8		rs_params; /* rs params, R/2=low 4 bits, Dlog2=high 4 bits*/
	} PACKED rsfu;
} rs_hdr_fields_t;

typedef struct srs_hdrseg
{
	ilcp_t8hdr_t	h;
	rs_hdr_fields_t	rsf;
} PACKED srs_hdrseg_t;

/* The basic RS info, repeated 3 times in a long RS header */
typedef struct lrs_hdrseg
{
	ilcp_t16hdr_t	h;
	rs_hdr_fields_t	rsf;
} PACKED lrs_hdrseg_t;

typedef  struct srs_hdr
{
	srs_hdrseg_t	rshs[3];		/* three identical copies */
	union
	{
		uint8		checkbytes[1]; /* padded to 2mod4 */
		uint16		next_ethertype;	/* placeholder, macro should be used */
	} us;
} PACKED srs_hdr_t;

typedef  struct lrs_hdr
{
	lrs_hdrseg_t	rshl[3];		/* three identical copies */
	union
	{
		uint8		rscheck[1];		/* padded to 0mod4 */
		uint16		next_ethertype;	/* placeholder, macro should be used */
	} ul;
} PACKED lrs_hdr_t;

#define RS_VERSION		0
#define RS_VERS_VMASK (0xf0)
#define RS_VERS_VSHIFT (4)
#define RS_VERS_LMASK (0x0f)

#define RS_VERS_GETVERS(v) (((v)&RS_VERS_VMASK)>>RS_VERS_VSHIFT)
#define RS_VERS_GETLEN_MOD16(v) ((v)&RS_VERS_LMASK)

#define RS_ISVALID_PKTLEN(lmod16, plen) ((lmod16) == ((plen) & RS_VERS_LMASK))
/* when attempting to fix, assume we can only make the length shorter */
/* CHECKME: this should be in the RS spec? */
#define RS_FIX_PKTLEN(lmod16, plen) \
((((plen) & ~RS_VERS_LMASK) | (lmod16)) <= (plen) \
     ? (((plen) & ~RS_VERS_LMASK) | (lmod16)) \
     : ((((plen) & ~RS_VERS_LMASK) | (lmod16)) - 16))

#define RS_PARAMS_RMASK	(0x0f)
#define RS_PARAMS_DMASK	(0xf0)
#define RS_PARAMS_DSHIFT (4)

#define RS_ISVALID_PARAM_R(r) ((unsigned)(r) <= 10)
#define RS_ISVALID_R(r) (((unsigned)(r) <= 20) && !((r) & 1))
#define RS_ISVALID_PARAM_D(d) ((unsigned)(d) <= 6)
#define RS_ISVALID_D(d) (((unsigned)(d) <= 64) && (((d)&((d)-1))==0))

/* convert encoded R[0,1,...10] to actual R[0,2,...20] */
#define RS_N2H_R(r) ((r)<<1)
/* convert encoded D[0,1,...6] to actual D[1,2,4,8,...64] */
#define RS_N2H_D(d) (1<<(d))
/* convert actual R[0,2,...20] to encoded R[0,1,...10] */
#define RS_H2N_R(r) ((r)>>1)
/* convert actual D[1,2,4,8,...64] to encoded D[0,1,...6] */
#define RS_H2N_D(d) (((d)==1)?0:(((d)==2)?1:(((d)==4)?2:(((d)==8)?3:(((d)==16)?4:(((d)==32)?5:(((d)==64)?6:255)))))))

/* generate the 8-bit version/length value from pktlength and RS_VERSION */
#define RS_VL_2_VERSLEN(v,l) ((((v)<<RS_VERS_VSHIFT)&RS_VERS_VMASK) \
| ((l)&RS_VERS_LMASK))
/* generate the 8-bit params value from R,D values (includes encoding) */
#define RS_RD_2_PARAMS(r,d) (((RS_H2N_D(d)<<RS_PARAMS_DSHIFT)&RS_PARAMS_DMASK) \
| (RS_H2N_R(r)&RS_PARAMS_RMASK))

/* get raw(encoded) R,D from 8-bit params field */
#define RS_PARAMS_R(p) ((params)&RS_PARAMS_RMASK)
#define RS_PARAMS_D(p) (((params)&RS_PARAMS_DMASK)>>RS_PARAMS_DSHIFT)

/* get decoded R,D values from 8-bit params field */
#define RS_PARAMS_GETR(p) RS_N2H_R(RS_PARAMS_R(p))
#define RS_PARAMS_GETD(p) RS_N2H_D(RS_PARAMS_D(p))

/* actual number of RS checkbytes */
#define RS_CHECKBYTES_LEN(r, d) ((r)*(d))
/* checkbytes count derived from the 8-bit params field */
#define RS_PARAMS_GET_CHECKLEN(p) \
RS_CHECKBYTES_LEN(RS_PARAMS_GETR(p),RS_PARAMS_GETD(p))

/* short format has checkbytes padded to multiple of 2 octets */
#define RS_SHORT_PAD_CHECKLEN(len) (((len)&(~(0x3)))+2)
/* long format has checkbytes padded to multiple of 4 octets */
#define RS_LONG_PAD_CHECKLEN(len) (((len)+2)&(~(0x3)))

/* size of fixed fields in short RS header */
#define RS_SHORT_FIXED_HDR_TOTLEN sizeof(srs_hdr_t)

/* total length of long format header based on padded checkbytes length */
#define RS_SHORT_TOTLEN(checklen) (RS_SHORT_FIXED_HDR_TOTLEN+(checklen))
/* value to put in ILCP length field based on padded checkbytes length */
#define RS_SHORT_ILCPLEN(checklen) \
(RS_SHORT_TOTLEN(checklen) - sizeof(ilcp_t8hdr_t))
/* ILCP length field value as function of R,D */
#define RS_SHORT_RD_2_ILCPLEN(r,d) \
RS_SHORT_ILCPLEN(RS_SHORT_PAD_CHECKLEN(RS_CHECKBYTES_LEN((r),(d))))
/* total rs header length as function of R,D */
#define RS_SHORT_RD_2_TOTLEN(r,d) \
RS_SHORT_TOTLEN(RS_SHORT_PAD_CHECKLEN(RS_CHECKBYTES_LEN((r),(d))))

/* size of fixed fields in long RS header */
#define RS_LONG_FIXED_HDR_TOTLEN sizeof(lrs_hdr_t)

/* total length of long format header based on padded checkbytes length */
#define RS_LONG_TOTLEN(checklen) (RS_LONG_FIXED_HDR_TOTLEN+(checklen))
/* value to put in ILCP length field based on padded checkbytes length */
#define RS_LONG_ILCPLEN(checklen) \
(RS_LONG_TOTLEN(checklen) - sizeof(ilcp_t16hdr_t))
/* ILCP length field value as function of R,D */
#define RS_LONG_RD_2_ILCPLEN(r,d) \
RS_LONG_ILCPLEN(RS_LONG_PAD_CHECKLEN(RS_CHECKBYTES_LEN((r),(d))))
/* total rs header length as function of R,D */
#define RS_LONG_RD_2_TOTLEN(r,d) \
RS_LONG_TOTLEN(RS_LONG_PAD_CHECKLEN(RS_CHECKBYTES_LEN((r),(d))))

#define RS_MAX_PKTLEN(r,d) (d * (255 - r))
#define RS_USELONG_HDR(r,d) (RS_SHORT_RD_2_ILCPLEN(r,d) > 255)

#define RS_ISVALID_PARAMS(params) (RS_ISVALID_PARAM_R(RS_PARAMS_R(params)) && \
								   RS_ISVALID_PARAM_D(RS_PARAMS_D(params)))
#define RS_ISVALID_VERSION(vers_len) (RS_VERS_GETVERS(vers_len) == RS_VERSION)

#define RS_ISVALID_SHORT_FIELDS(sstype, sslen, r, d) \
(((sstype) == ILCP_SUBTYPE_SRS) && RS_ISVALID_R(r) && RS_ISVALID_D(d) && \
 (RS_SHORT_RD_2_ILCPLEN(r,d) == (sslen)))
#define RS_ISVALID_LONG_FIELDS(lstype, lslen, r, d) \
(((lstype) == ILCP_SUBTYPE_LRS) && RS_ISVALID_R(r) && RS_ISVALID_D(d) && \
 (RS_LONG_RD_2_ILCPLEN(r,d) == (lslen)))


/*
 * Define tags for LCP TLV Extensions
 *
 * The Extension headers look EXACTLY like the ilcp t8 and t16 headers.
 * The proposal (hopefully to be accepted) requires natural alignment for
 * all fields within TLV extensions, so packed structure overlays should
 * work.
 */
/* All subtypes */
#define EXT_PAD_TAG 0		/* one octet(no length field), pad for alignment */
#define VS_SHORT_TAG 1      /* Vendor-specific extension, short format */
#define VS_LONG_TAG 32769   /* Vendor-specific extension, long format */

/* Type specific */
#define CSA_SUBTYPES_TAG 2  /* List of supported optional subtypes, CSA only */
#define CSA_PARAMS_TAG 3    /* Parameters for a subtype, CSA only */
#define CSA_CSSMGT_TAG 4	/* Pri 7 CSS assignment info, for CSA */
#define CSA_BWMGT_TAG 5		/* Bandwidth mgt extension for CSA */

#define RATE_RSREQ_TAG 2    /* Request for rs encoding in rate requests */

typedef  struct tlv_t8hdr
{
	uint8	tag8;
	uint8	length;
} PACKED tlv_t8hdr_t;

typedef  struct tlv_t16hdr
{
	uint16	tag16;
	uint16	length;
} PACKED tlv_t16hdr_t;

typedef struct rs_pref
{
	uint8	epr;	/* Enhanced payload rate when requested RS encoding used */
	uint8	rsr;	/* Number of redundant bytes per code word requested */
	uint8	kmax;	/* Maximum payload size per codeword */
	uint8	pad;
} PACKED rs_pref_t;

typedef struct tlv_rate_rsreq
{
	tlv_t8hdr_t	hdr;
	rs_pref_t rspref[1];
} PACKED tlv_rate_rsreq_t;

typedef struct tlv_csa_stlist
{
	tlv_t8hdr_t	hdr;
	uint16 stlist[1];	/* list of supported subtypes */
} PACKED tlv_csa_stlist_t;

/* standard format for a subtype-parameters extension */
/* may be copied to streamline implementation for specific subtypes */
typedef struct tlv_csa_stparams
{
	tlv_t8hdr_t	hdr;
	uint16 subtype;		/* a subtype */
	uint8 params[1];	/* subtype-dependent params */
} PACKED tlv_csa_stparams_t;

typedef struct tlv_csa_rsparams
{
	tlv_t8hdr_t	hdr;
	uint16 subtype;		/* RS subtype */
	uint16 encode_mask;	/* bit mask for supported R encodings */
	uint16 decode_mask;	/* bit mask for supported R decodings */
} PACKED tlv_csa_rsparams_t;

#if 1 /* hopeful definition of bit mask values */
#define RS_DO_R_2	0x1
#define RS_DO_R_4	0x2
#define RS_DO_R_6	0x4
#define RS_DO_R_8	0x8
#define RS_DO_R_10	0x10
#define RS_DO_R_12	0x20
#define RS_DO_R_14	0x40
#define RS_DO_R_16	0x80

#define RS_DO_R_18	0x100
#define RS_DO_R_20	0x200

#define RS_DO_ALL 0x3ff

#else /* little endian */
#define RS_DO_R_2	0x100
#define RS_DO_R_4	0x200
#define RS_DO_R_6	0x400
#define RS_DO_R_8	0x800
#define RS_DO_R_10	0x1000
#define RS_DO_R_12	0x2000
#define RS_DO_R_14	0x4000
#define RS_DO_R_16	0x8000

#define RS_DO_R_18	0x1
#define RS_DO_R_20	0x2

#define RS_DO_ALL 0xff03
