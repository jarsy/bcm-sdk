 ...
 * $Id: bcmsrom.c,v 1.1 2004/02/24 07:47:00 csm Exp $
 */
...
initvars_srom_pci(void *curmap, char **vars, int *count)
{
	...
	if (sprom_read_pci((void *)((uint)curmap + PCI_BAR0_SPROM_OFFSET), 0, b, sizeof (b), TRUE)) {
		printf("initvars_srom_pci: sprom_read_pci failed \n");
		return (-1);
	}
...
#if 0  /* Fix Me */
	if (spromversion != 1) {
		printf("initvars_srom_pci: spromversion %d failed \n",spromversion);
		return (-2);
	}
#else
	printf("spromversion Fix\n");
#endif
