...
/* $Id: ns16550Sio.c,v 1.4 2011/07/21 16:14:21 yshtil Exp $
...
#ifndef  MIPSEL /* define MIPSEL to change to LE */
#warning "Compiling UART driver for Big Endian"
#define REG(reg, pchan) \
 (*(volatile UINT8 *)(((UINT32)pchan->regs + (reg * pchan->regDelta))^3))
#define REGPTR(reg, pchan) \
 ((volatile UINT8 *)(((UINT32)pchan->regs + (reg * pchan->regDelta))^3))
#else
#warning "Compiling UART driver for Little Endian"
#define REG(reg, pchan) \
