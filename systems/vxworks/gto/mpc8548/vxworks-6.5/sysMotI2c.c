...
/* $Id: sysMotI2c.c,v 1.5 2012/01/26 02:18:47 bhanup Exp $
...
IMPORT void sysMpc85xxUsDelay (UINT uSeconds);
...
#define IOCTL_DELAY(__fast_delay_us, __slow_delay_ms)       \
    do                                                      \
        {                                                   \
        if (I2C_IS_FAST_ACCESS_SET)                         \
            sysMpc85xxUsDelay(__fast_delay_us);             \
        else                                                \
            sysMpc85xxMsDelay(__slow_delay_ms);             \
        }                                                   \
    while (0)
 ...
 */
UINT8 i2c_freq_divider = 0x31; /* 3072 */
    ...
    if ( pI2cCmdPacket->command == I2C_WRITOP )
    {
	    ...
	    /* write data */
            ...
            if ( I2C_CYCLE_WRITE(unit,
               *((unsigned char *)pI2cCmdPacket->memoryAddress+byteCount) ) )
            {
                pI2cCmdPacket->status = I2C_ERROR_CYCLE_WRITE;
                return ERROR ;
            }
...
#define I2C_DO_OP_GENERIC_EXIT(__status, __rv)  \
    do                                          \
        {                                       \
        pI2cCmdPacket->status = __status;       \
        return (__rv);                          \
        }                                       \
    while (0)                           

/* 
 * i2cDoOpGeneric - Only applicable for SAND reference board. Can be used 
 *                  to access various devices on the i2c bus. Devices supported
 *                  are EEPROM, FPGA, Synthesizers-via-FPGA-indirect-access 
 */
int i2cDoOpGeneric
    (
    int             unit,
...
{
    int             byteCount, blockCount;
    int             ack;
    unsigned char   *pWriteData;
    unsigned char   data;

    /* 0x06 --> 832
     * On SAND reference board, this is the freq divider to use. Override 
     * the old value.
     */
    i2c_freq_divider = 0x06;

    /* read operation */
    if ( pI2cCmdPacket->command == I2C_READOP )
    {
        if (I2C_KNOWN_STATE(unit)) {
            I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_KNOWN_STATE, ERROR);
        }
        if (I2C_CYCLE_START(unit)) {
            I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_START, ERROR);
        }
        /* device address - write, R/W bit = 0  */
        if (I2C_CYCLE_WRITE(unit,i2cAddressMunge(deviceAddress))) {
            I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_WRITE, ERROR);
        }
        if (I2C_CYCLE_ACKIN(unit)) {
            I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_ACKIN, ERROR);
        }

        for (blockCount = 0; blockCount < pI2cCmdPacket->blockSize; 
              blockCount++) {
            data = (pI2cCmdPacket->blockNumber >> 
                    (8 * (pI2cCmdPacket->blockSize - blockCount - 1))) & 0xff;
            /* write all address bytes */
            if (I2C_CYCLE_WRITE( unit, data)) {
                I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_WRITE, ERROR);
            }
            if (I2C_CYCLE_ACKIN(unit)) {
                I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_ACKIN, ERROR);
        ...
        /* generate ack on next to last byte; generate stop on last byte */
        for (byteCount = 0; byteCount < pI2cCmdPacket->nBlocks; byteCount++) {
            if ((deviceAddress == 0x52 /* EEPROM */) || 
                (byteCount == (pI2cCmdPacket->nBlocks-1))) {
                ack = 0;
            } else {
                ack = 1;
            }
            if (byteCount != 0) {
                /* set no-latch bit on all reads except the first */
                ack |= I2C_READ_NO_LATCH;
            }
            if (byteCount == (pI2cCmdPacket->nBlocks-1))
            {       
                /* No ACK on last byte */
                if (I2C_CYCLE_READ(unit,
                   (unsigned char *)pI2cCmdPacket->memoryAddress+byteCount, 
                                   ack)) {
                    I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_READ, ERROR);
                }
                /* send a STOP on last transfer */
                if (I2C_CYCLE_STOP(unit)) {
                    I2C_DO_OP_GENERIC_EXIT(I2C_ERROR_CYCLE_STOP, ERROR);
                }
            }
            else {
                /* Send an ACK on next to last transfer */
        ...
        I2C_KNOWN_STATE(unit);
        I2C_DO_OP_GENERIC_EXIT(0, ERROR);
    }

    /* Leave the I2C bus in a known state. */
    I2C_KNOWN_STATE(unit);

    I2C_DO_OP_GENERIC_EXIT(0, OK);
}
...
int i2cDoOp
    ...
    switch(pI2cCmdPacket->deviceType)
		...
		return ERROR ;
            break;
        case I2C_DEVICE_TYPE_GENERIC:
        if (i2cDoOpGeneric(unit,deviceAddress,pI2cCmdPacket) != OK) {
            return ERROR;
        }
        break;
        default :
