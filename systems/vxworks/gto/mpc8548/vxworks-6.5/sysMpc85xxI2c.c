...
/* $Id: sysMpc85xxI2c.c,v 1.4 2012/01/26 02:18:47 bhanup Exp $
...
int I2CDoFastAccess = 0;    /* indicates if a "Fast access" is requested */

#define I2C_CYCLE_DELAY(__fast_delay_us, __slow_delay_ms)   \
    do                                                      \
        {                                                   \
        if (I2CDoFastAccess == 1)                           \
...
int i2cCycleMpc85xxRead
    ...
    int ack    /* 0 = don't ack, 1 = ack . Also no-latch signal */
    )
    {
    unsigned int    readData = 0;
    int             latch_read = 1; /* do latch read unless no latch is set */

    if (ack & I2C_READ_NO_LATCH) 
        {
        /* check for explicit no latch signal */
        latch_read = 0;
        }
    ack &= I2C_READ_ACK_MASK; /* mask off latch signal bits */
...
void sysMpc85xxUsDelay
    (
    UINT        delay            /* length of time in Micro-Seconds to delay */
    )
    {
    sysUsDelay(delay);
    }
