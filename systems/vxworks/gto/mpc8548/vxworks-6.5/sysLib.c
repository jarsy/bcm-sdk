...
/* $Id: sysLib.c,v 1.25 2011/12/21 02:15:46 bhanup Exp $
...
void
pcieShow(int min_bus, int max_bus)
{
#define P_MIN_BUS       min_bus
#define P_MAX_BUS       max_bus
#define P_MAX_DEV       31
#define P_MAX_FUNC      0
#define P_MAX_OFFSET    (0x80 -1)
...
static char * io_sel[] = {"reserved", "reserved", "reserved",
                          ...
                          "x4 RIO 1.25Gbps", 
                          "x8 PCI-E"};
static char * if_mode[] = {"reserved", "10/100 Mbps MII/RMII",
                           "1000 Mbps GMII/TBI", "resvered"};
void sysPorStatShow()
{
    UINT reg;
    int  field; 
...
void sysTsecShow(int dev)
{
    UINT offset; 
    UINT maccfg1, maccfg2, ecntrl;
    int  field;

        offset = dev * 0x1000;
        maccfg1 = *M85XX_MACCFG1(CCSBAR + offset);
        maccfg2 = *M85XX_MACCFG2(CCSBAR + offset);
        ecntrl  = *M85XX_ECNTRL(CCSBAR + offset);
        ...
        field = (maccfg1 >> 19) & 1;
        printf("Reset Rx MC  = %d\n", field);

        field = (maccfg1 >> 18) & 1;
        printf("Reset Tx MC  = %d\n", field);

        field = (maccfg1 >> 17) & 1;
        printf("Reset Rx Fun = %d\n", field);
        ...
        field = (maccfg1 >> 3) & 1;
        printf("Sync'd Rx EN = %d\n", field);

        field = (maccfg1 >> 2) & 1;
        printf("Rx_EN        = %d\n", field);

        field = (maccfg1 >> 1) & 1;
        printf("Sync'd Tx EN = %d\n", field);
...
void sysTsecSetup(int speed)
{
    ...
    if (speed == 10 || speed == 100) {
        for (dev = 0; dev < 4; dev++) {
            ...
            maccfg2 &= ~0x300;
            maccfg2 |= 0x100;
            *M85XX_MACCFG2(CCSBAR + offset) = maccfg2;

            ecntrl = *M85XX_ECNTRL(CCSBAR + offset);
            ecntrl &= ~0x38;
            ecntrl |= (speed == 10) ? 0x10 : 0x18;
            *M85XX_ECNTRL(CCSBAR + offset) = ecntrl; 
        }
    }
...
void sysBacktracePpc(char *pfx, char *sfx, int direct)
{
    ...
    if (direct)
	logMsg("%s", (int)buf, 2, 3, 4, 5, 6);  /* Atomically print giant string */
    else {
