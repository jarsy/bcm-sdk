...
/* $Id: usrNetwork.c,v 1.2 2011/07/21 16:14:13 yshtil Exp $
...
--------------------
08x,07jun06,jon  Added ND6_MAX_PREFIXES_CFG (SPR 120097)
08w,12may06,dlk  Added UDP(6)_STRICT_MCAST_MSHIP_CFG (WIND00038260).
08v,26mar06,kch  Added ALTQ_USE_MACH_CLOCK to allow user to override the
                 machine dependent clock init and read routines (SPR#119511).
08v,12jun06,wap  Modified usrNetEndLibInit() to initialize function pointer
                 _func_m2PollStatsIfPoll with END_POLL_STATS_ROUTINE_HOOK
                 configuration.
08u,09mar06,kch  Renamed ALTQ configuration parameters. Also renamed
                 INCLUDE_ALTQ_PRIQ to INCLUDE_ALTQ_PRIORITYQ (SPR#118834).
08t,02mar06,dlk  Remove ipflow related items (SPR #104107).
08s,10feb06,kch  Added ALTQ init routines.
08r,23jan06,dlk  Scalable DNS resolver debug output.
08v,03feb06,pcm  fixed typo in NFS macro
08u,03jan06,wap  Add support for VxBus network drivers
08t,20dec05,pcm  added usrNfs2CacheInit()
08s,07dec05,pcm  added usrNfs3CacheInit()
08r,02nov05,tlu  Add start up code for MIP6 HA and remove setting of 
		 ipv6_forwarding from MIP6 MN start up
08q,24oct05,md   changed PATH_MAX count to include null terminator
08p,13oct05,ebh  Eliminate TIPC initialization debug statement
08o,28sep05,tlu  add checking for ipv6_forwarding state, exit MN start up if true
		 and change MIP6 init before RTSOL
08n,28sep05,dlk  Add initialization code for unsupported INCLUDE_HOSTCACHE
                 component.
08m,13sep05,dlk  Added ICMPV4_TIMESTAMP_REQ_SUPPORT_CFG support.
...
#ifdef INCLUDE_DNS_RESOLVER_DEBUG
#define INCLUDE_DNS_RESOLVER
extern void dnsDebug (char * msg);
#endif
...
#ifdef INCLUDE_ALTQ
#include <altq/altq_config_var.h>
#endif /* INCLUDE_ALTQ */
...
#ifdef INCLUDE_ALTQ_CONFIG
#include <net/utils/altqConfig.h>
#endif /* INCLUDE_ALTQ_CONFIG */
...
#endif     /* INCLUDE_NFS2_CLIENT || INCLUDE_NFS3_CLIENT */
...
#if ALTQ_USE_MACH_CLOCK == TRUE

/* 
 * forward declarations for the machine dependent clock init and read 
 * routines.
 */
void ALTQ_MACH_CLOCK_INIT_HOOK (void);
u_int64_t ALTQ_MACH_CLOCK_READ_HOOK (void);

#else

/*
 * ALTQ_MACH_CLOCK_INIT_HOOK and ALTQ_MACH_CLOCK_READ_HOOK are not applicable,
 * explicitly re-define these hooks to NULL so that the internal ALTQ routines
 * will be used.
 */
#undef ALTQ_MACH_CLOCK_INIT_HOOK
#define ALTQ_MACH_CLOCK_INIT_HOOK NULL

#undef ALTQ_MACH_CLOCK_READ_HOOK
#define ALTQ_MACH_CLOCK_READ_HOOK  NULL

#endif /* ALTQ_USE_MACH_CLOCK */

LOCAL void usrNetAltqCommonInit (void)
    {
    bzero ((char *)&altqConfigParams, sizeof(ALTQ_CONFIG_PARAMS));
    altqConfigParams.cfgh.len = sizeof(ALTQ_CONFIG_PARAMS);
    altqConfigParams.defaultQlimit = ALTQ_DEFAULT_QUEUE_LIMIT;
    altqConfigParams.defaultPriority = ALTQ_DEFAULT_QUEUE_PRIORITY;
    altqConfigParams.defaultIfqDrvMaxLen = ALTQ_DISPATCH_QUEUE_MAX_LEN;
    altqConfigParams.maxQueueAllowed = ALTQ_MAX_QUEUE_ALLOWED;
    altqConfigParams.machClkFreq = ALTQ_MACH_CLOCK_FREQ;
    altqConfigParams.machClkInit = ALTQ_MACH_CLOCK_INIT_HOOK;
    altqConfigParams.readMachClk = ALTQ_MACH_CLOCK_READ_HOOK;

    (void)altq_subr_init((void *)&altqConfigParams);
}

STATUS usrNetAltqInit (void)
    {
    usrNetAltqCommonInit();   /* always required! */

#ifdef INCLUDE_ALTQ_RED
    (void)altq_red_init();
#endif /* INCLUDE_ALTQ_RED */

#ifdef INCLUDE_ALTQ_RIO
    (void)altq_rio_init();
#endif /* INCLUDE_ALTQ_RIO */

#ifdef INCLUDE_ALTQ_CBQ
    (void)altq_cbq_init();
#endif /* INCLUDE_ALTQ_CBQ */

#ifdef INCLUDE_ALTQ_PRIORITYQ
    (void)altq_priq_init();
#endif /* INCLUDE_ALTQ_PRIORITYQ */
...
#endif
    udpDefaultConfigParams.cfg_udp_strict_mcast_mship =
	UDP_STRICT_MCAST_MSHIP_CFG;
...
void usrNetNdInit (void)
    ...
    nd6DefaultConfigParams.cfg_nd6_max_prefixes = ND6_MAX_PREFIXES_CFG;
...
void usrNetUdpv6Init (void)
    ...
    udp6DefaultConfigParams.cfg_udp6_strict_mcast_mship =
	UDP6_STRICT_MCAST_MSHIP_CFG;
...
extern STATUS brcmUserVerify(char *, char *);
...
char * muxDevConnect_desc = "MUX Connection Supported";
#endif
...
#ifdef INCLUDE_END_POLLED_STATS
    /* initialize the _func_m2PollStatsIfPoll function pointer */
...
#ifdef INCLUDE_DNS_RESOLVER_DEBUG
    /* To ensure the reference isn't dropped when DNS_DEBUG is FALSE */
    dbgRtn = (FUNCPTR)dnsDebug;
    dnsDefaultConfigParams.cfg_debugRtn = DNS_DEBUG ? (FUNCPTR)dnsDebug : NULL;
#else
    dnsDefaultConfigParams.cfg_debugRtn = NULL;
#endif
...
/***************************************************************************
*
* usrNfsInit - initialize NFS for the supplied version
*
* This routine initializes NFS for the supplied version.
*
...
STATUS usrNfsInit
    ...
    nfsAuthUnixSet (sysBootParams.hostName, NFS_USER_ID, NFS_GROUP_ID,
                    0, (int *) 0);
