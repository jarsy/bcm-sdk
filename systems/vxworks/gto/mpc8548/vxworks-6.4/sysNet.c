...
/* $Id: sysNet.c,v 1.6 2011/07/21 16:14:13 yshtil Exp $
...
int (*funcUsrBootLineCrack)(char *bootString, BOOT_PARAMS *pParams) = NULL;
/*
...
LOCAL STATUS sysNetBootLineMacAddrGet
    ...
    )
    {
        ...
        if (funcUsrBootLineCrack != NULL) {
            if (funcUsrBootLineCrack(bootLine, &params) != OK) {
                return (ERROR);
            }
        }
