...
/* $Id: sysLib.c,v 1.4 2011/07/21 16:14:33 yshtil Exp $
...
/* following assignments are to force inclusion of modules which are required
 * by romInit.s and which would be ignored by the partial link otherwise. If
 * they are not forced here, the vxWorks_rom image would not link
LOCAL void * sysForceLink1 = (FUNCPTR) bcm1250_dram_init;
LOCAL void * sysForceLink2 = (FUNCPTR) bcm1250_l2cache_init;
 */
...
STATUS sysFPGAIntConnect(VOIDFUNCPTR isr, int arg)
{
    STATUS status;
    status = bcm1250IntConnect(
        ILVL_BCM1250_FPGA_INT, IV_INT2_VEC, isr, arg);
    if (status != OK)
        return status;
    status = bcm1250IntEnable(ILVL_BCM1250_FPGA_INT);
    return status;
}
...
void sysHwInit2 (void)
{
    ...
    /* sysPCIBridgeProbe(0, 0, 1, 1); */  /* rely on auto config */
...
/******************************************************************************
*
* bcm1250ExcCacheVecInit - Initialize the cache exception vector for this CPU.
*
* This routine initializes the cache exception vector to reference the customized
* routine for the SB1 core.
*
* RETURNS: N/A
*/

void bcm1250ExcCacheVecInit(void * vec)
    {
    bcopy((const char *)sb1CacheExcVec, (char *)vec, 0x80);
    cacheTextUpdate(vec, 0x80);
    }

/******************************************************************************
*
* sysExcVecInit - Initialize exception vectors for this CPU.
*
* This routine initializes the exception vectors for this instance
* of VxWorks.  The vectors are placed so that the vectors for the
* separate instances of VxWorks running on different core CPU's do
* not conflict with one another.
*
* RETURNS: N/A
*/

void sysExcVecInit (void)
    {
    UINT32 cpuNum;
    UINT32 config;
    ...
    /* place exception vectors for this CPU at CPU-specific location */

    excNormVecInit  ((void *) (E_VEC + cpuNum * 0x10000));
    excTlbVecInit   ((void *) (T_VEC + cpuNum * 0x10000));
    excXtlbVecInit  ((void *) (X_VEC + cpuNum * 0x10000));
    bcm1250ExcCacheVecInit ((void *) (C_VEC + cpuNum * 0x10000));

    /* select the new exception vectors */

    config = sysConfigGet ();
    config &= ~0xf0000;
    config |= cpuNum * 0x10000;
    sysConfigSet (config);
    }

/******************************************************************************
*
* sysSetExcVecInitHook - Provide the exception vector hook to VxWorks.
*
* This routine sets the global exception-vector-loading-hook to point
* to the BSP-specific vector loading function.  This function is called
* _very_ early in the boot process, using the SYS_HW_INIT_0 hook.  When
* VxWorks is ready to initialize the exception vectors, it will call our
* attached hook, rather than doing the work itself.
*
*
* RETURNS: N/A
*/

void sysSetExcVecInitHook (void)
    {
    extern FUNCPTR sysExcVecInitHook;
    UINT32 config;

    sysExcVecInitHook = (FUNCPTR) sysExcVecInit;

    /*
     * set the cache mode to COHERENT
     * this cannot be set in romInit because COHERENT caches are not valid
...
#endif	/* (defined (INCLUDE_SM_OBJ) || defined (INCLUDE_SM_NET)) */
...
*
* This function is called wfrom sysHwInit2, this function sets up the
...
* and second, we must register as a receiver forinterrupts from the 
* other CPU.
*
* RETURNS: NA
*/
...
#define SEM_NAME1 "mySharedSem1"
#define SEM_NAME2 "mySharedSem2"
#define DELAY_TICKS 200
 
LOCAL SEM_ID semSmId1, semSmId2;
 
STATUS semTaskSmCreate (void)
    {
    /* created shared semaphore */
    if ((semSmId1 = semBSmCreate (SEM_Q_FIFO, SEM_EMPTY)) == NULL)
        {
        printf( "semTask: can't create shared semaphore\n");
        return ERROR;
        }
 
    if ((semSmId2 = semCSmCreate (SEM_Q_FIFO, 0)) == NULL)
        {
        printf( "semTask: can't create shared semaphore\n");
        return ERROR;
        }
 
    /* add object to name databsae */
 
    if (smNameAdd (SEM_NAME1, semSmId1, T_SM_SEM_B) == ERROR)
        {
        printf ("semTask: can't name the semaphore\n");
        return ERROR;
        }
 
    if (smNameAdd (SEM_NAME2, semSmId2, T_SM_SEM_C) == ERROR)
        {
        printf ("semTask: can't name the semaphore\n");
        return ERROR;
        }
...
/********************************************************************************
* semTask1 - User-callable test function for shared semphores.
*
...
STATUS semTask1 (int loop)
    {
    int thisTick, count;
 
    semTake (semSmId1, WAIT_FOREVER);
 
    thisTick = tickGet();
    printf("Task1 start tick = %d\n", thisTick);
 
    /* grab shared semaphore and hold it for a while */
    for(count = 1; count < loop; count++)
        {
        semTake (semSmId2, WAIT_FOREVER);
        }
 
    thisTick = tickGet();
    printf("Task1 stop  tick = %d\n", thisTick);
 
    return OK;
    }                                                                           

/********************************************************************************
* semTask2 - User-callable test function for shared semphores.
*
...
STATUS semTask2 (int loop)
    ...
    s = smNameFind (SEM_NAME1, (void **) &semSmId1, &objType, WAIT_FOREVER);
    if (s == ERROR)
        {
    ...
    s = smNameFind (SEM_NAME2, (void **) &semSmId2, &objType, WAIT_FOREVER);
    if (s == ERROR)
        {
        printf ("semTask2: can't find %s\n", SEM_NAME2);
        return ERROR;
...
sysPciTlbSet(void)
{
#define PCI_MEMORY_SPACE    0x40000000  /* must be 32Mb boundary */
#define PCI_TLB_PAGE_SIZE   0x02000000  /* 32M */
