 ...
 *
 * $Id: swapi.h,v 1.3 2007/10/08 22:31:11 iakramov Exp $
 *
 *  TODO: Support for DiffServ, Layer3, Filtering
 *
 */
...
#ifdef IL_BIGENDIAN
#define BRIDGE_GROUP_MAC_ADDR {0x00,0x00,0x00,0xc2,0x80,0x01}
#define IGMP_MAC_ADDR {0x00,0x00,0x00,0x5e,0x00,0x01}
#define DEFAULT_MAC_ADDR {0x00,0x65,0x53,0x18,0x10,0x00}
#else
#define BRIDGE_GROUP_MAC_ADDR {0x01,0x80,0xc2,0x00,0x00,0x00}
#define IGMP_MAC_ADDR {0x01,0x00,0x5e,0x00,0x00,0x00}
#define DEFAULT_MAC_ADDR {0x00,0x10,0x18,0x53,0x65,0x00}
#endif
#define PBMP_NONE		0
 ...
 * VLAN init call
 */
extern void bcms5_vlan_init(void);
 ...
 */
extern int bcms5_write_reg(int unit, uint8 page, uint8 addr, uint8 *bufptr, uint len);
extern int bcms5_read_reg(int unit, uint8 page, uint8 addr, uint8 *bufptr, uint len);
 ...
 */
extern int bcms5_port_speed_get(bcm_port_t port, int *speed);
extern int bcms5_port_speed_set(bcm_port_t port, int speed);
extern int bcms5_port_autoneg_get(bcm_port_t port, int *autoneg);
extern int bcms5_port_autoneg_set(bcm_port_t port, int autoneg);
extern int bcms5_port_restart_autoneg(bcm_port_t port);
extern int bcms5_port_duplex_get(bcm_port_t port, int *duplex);
extern int bcms5_port_duplex_set(bcm_port_t port, int duplex);
extern int bcms5_port_pause_get(bcm_port_t port,
			      int *pause_tx, int *pause_rx);
extern int bcms5_port_pause_set(bcm_port_t port,
			      int pause_tx, int pause_rx);
extern int bcms5_port_loopback_set(bcm_port_t port, int loopback);
extern int bcms5_port_loopback_get(bcm_port_t port, int *loopback);
extern int bcms5_port_enable_set(bcm_port_t port, int enable);
extern int bcms5_port_enable_get(bcm_port_t port, int *enable);
extern int bcms5_port_stp_get(bcm_port_t port, int *state);
extern int bcms5_port_stp_set(bcm_port_t port, int state);
extern int bcms5_port_link_status_get(bcm_port_t port, int *up);
extern int bcms5_port_info_get(bcm_port_t port,
			     bcm_port_info_t* portinfo);
extern int bcms5_port_poweroff_get(bcm_port_t port, int *poweroff);
extern int bcms5_port_poweroff_set(bcm_port_t port, int poweroff);
typedef struct _ROBO_MII_AN_ADVERT_STRUC bcm_an_advert_t;
extern int bcms5_port_advert_get(bcm_port_t port,
			     bcm_an_advert_t* an_advert);
extern int bcms5_port_advert_set(bcm_port_t port,
			     bcm_an_advert_t* an_advert);
extern int bcms5_port_advert_remote_get(bcm_port_t port,
			     bcm_an_advert_t* an_advert);
#if 0 /* remove for vxworks */
...
extern int bcms5_stat_clear(bcm_port_t port);
extern int bcms5_stat_clear_all(int unit);

extern int bcms5_stat_get(bcm_port_t port, 
			bcm_stat_val_t type, uint64 *value);

extern int bcms5_stat_get32(bcm_port_t port, 
 ...
 */
extern int bcms5_vlan_enable(int unit, int enable);

extern int bcms5_vlan_create(int unit, bcm_vlan_t vid);

extern int bcm_vlan_destroy(int unit, bcm_vlan_t vid);
extern int bcm_vlan_destroy_all(int unit);

extern int bcms5_vlan_port_add(int unit,
			     bcm_vlan_t vid,
...
extern int bcms5_vlan_default_get(int unit, bcm_vlan_t *vid_ptr);
extern int bcms5_vlan_default_set(int unit, bcm_vlan_t vid);
 ...
 */
extern void bcms5_l2_addr_init(bcm_l2_addr_t *l2addr,
			     bcm_mac_t mac_addr, bcm_vlan_t vid);

/*
 * Add address to L2 table
 */
extern int bcms5_l2_addr_add(int unit, bcm_l2_addr_t *l2addr);
...
extern int bcms5_l2_addr_remove(int unit,
			      bcm_mac_t mac_addr, bcm_vlan_t vid);
extern int bcms5_l2_addr_remove_by_port(int unit,
				      bcm_pbmp_t pbmp, uint32 flags);
extern int bcms5_l2_addr_remove_by_vlan(int unit,
				      bcm_vlan_t vid, uint32 flags);

/*
 * Look up address in L2 table
 */
extern int bcms5_l2_addr_get(int unit, bcm_mac_t mac_addr, bcm_vlan_t vid,
...
extern int bcm_reg_brcmtag_dev(char *base_name, char *suffix);
extern int bcm_unreg_brcmtag_dev(char *dev_name);

/* macro to get vlan/port from interface name */
/* note that it is necessary for vlan and port IDs to be the same for this */
#define bcm_get_vlan(ifname,vlan) do {  \
  char *curptr = NULL;                  \
  if ((curptr = strrchr(ifname,'.'))) { \
    if (!sscanf(++curptr,"%d",vlan))    \
      *(vlan) = 0;                      \
  } else                                \
    *(vlan) = 0;                        \
  } while (0);
...
#define ROBO_RREG(unit,page,addr,bufptr,len) \
    (*(robo_driver->robo_read))(robo,unit,page, \
		addr,bufptr,len)
#define ROBO_WREG(unit,page,addr,bufptr,len) \
    (*(robo_driver->robo_write))(robo,unit,page, \
		addr,bufptr,len)
