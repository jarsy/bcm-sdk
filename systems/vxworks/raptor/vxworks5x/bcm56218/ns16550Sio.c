...
/* $Id: ns16550Sio.c,v 1.3 2011/07/21 16:14:55 yshtil Exp $
...
#ifdef BCM56218_UART_BUSY_WRITE_PROTECT
#define IS_UART_REG(a, r)  \
            (((UINT32) a & 0x7) == (r))

void uart_delay_loop(int d)
{
    int i;
    for (i = 0; i < d * 100; i++)
        ;
...
void uart_busy_write_protect_set(UINT8 * reg, UINT8 data)
{
    volatile UINT8   *b = (UINT8*)(((UINT32)reg) & ~0x7);

    if (IS_UART_REG(reg, LCR)) {
        uart_delay_loop(100000);
        *(b + MCR) = MCR_OUT2 | MCR_LOOP;

        uart_delay_loop(10000);
        *(b + FCR) = 7;
        uart_delay_loop(100000);
    }
}

void uart_busy_write_protect_clear(UINT8 * reg)
{
    volatile UINT8   *b = (UINT8*)(((UINT32)reg) & ~0x7);
...
#endif /* BCM56218_UART_BUSY_WRITE_PROTECT */
...
LOCAL   void    ns16550RegWr(volatile UINT8 *reg, UINT8 data)
{
    ...
    ikey = intLock();
#ifdef BCM56218_UART_BUSY_WRITE_PROTECT
    uart_busy_write_protect_set(reg, data);
    *reg = data;
    uart_busy_write_protect_clear(reg);
#else
    *reg = data;
#endif /* BCM56218_UART_BUSY_WRITE_PROTECT */
    intUnlock(ikey);
