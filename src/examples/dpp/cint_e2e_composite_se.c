...
/*
 * $Id: cint_tm_fap_config4.c,v 1.1 Broadcom SDK $
 *
...
int e2e_composite_se_setup(int kbits_sec_max_port, int port_idx)
{
...
/*
    rv = bcm_stk_my_modid_set(c4_unit, c4_my_modid);
    if (rv != BCM_E_NONE) {
...
*/
    ...
    rv = bcm_stk_sysport_gport_set(c4_unit, sys_gport[idx], dest_gport[idx]);
    ...
    rv = bcm_cosq_gport_add(c4_unit, dest_gport[idx], 1, 
    ...
    rv = bcm_cosq_gport_sched_set(c4_unit,
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_e2e_gport[idx], 
    ...
    /* CL (sf2) setup */
    BCM_COSQ_GPORT_COMPOSITE_SF2_CORE_SET(c4_gport_ucast_scheduler_sf2[idx],c4_gport_ucast_scheduler[idx], 0);
    ...
    BCM_COSQ_GPORT_COMPOSITE_SF2_CORE_SET(c4_gport_ucast_scheduler1_sf2[idx],c4_gport_ucast_scheduler1[idx], 0);
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler[idx], 
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler1[idx], 
    ...
    config.numq = num_cos;
    config.remote_modid = c4_my_modid;
    config.nof_remote_cores = 1;
    rv = bcm_cosq_voq_connector_gport_add(c4_unit, &config, &c4_gport_ucast_voq_connector_group0[idx]);
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler_fq0[idx], 
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler_fq1[idx], 
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler_fq2[idx], 
    ...
    rv = bcm_cosq_gport_attach(c4_unit, c4_gport_ucast_scheduler_fq3[idx], 
    ...
    rv = bcm_cosq_gport_add(c4_unit, dest_gport[idx], num_cos, 
    ...
    config.remote_modid = c4_my_modid;
    config.nof_remote_cores = 1;
    rv = bcm_cosq_voq_connector_gport_add(c4_unit, &config, &c4_gport_ucast_voq_connector_group1[idx]);
    ...
    rv = bcm_fabric_port_get(c4_unit,
                     c4_e2e_gport[idx],
                     0,
    ...
    rv = bcm_cosq_gport_bandwidth_set(c4_unit,
                      c4_e2e_parent_gport[idx],
                      0,
                      0,
                      kbits_sec_max_port,
    ...
    rv = bcm_cosq_control_set(c4_unit,
                      c4_gport_ucast_scheduler[idx],
                      0,
...
int e2e_composite_se_teardown(int port_idx)
{
    int                           rc = BCM_E_NONE;
    int                           idx = port_idx;
        ...
        switch(nbr_sched) {
            case 0:
                for (nbr_sched_flw = 0; nbr_sched_flw < C4_NBR_SCHEDULER_FLOWS; nbr_sched_flw++) {
                    ...
                    rc = bcm_cosq_gport_detach(c4_unit, c4_e2e_gport[idx], gport_ucast_scheduler, 0);
                    if (BCM_FAILURE(rc)) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C4_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        rc = bcm_cosq_gport_connection_set(c4_unit, &connection);
        if (BCM_FAILURE(rc)) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C4_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        rc = bcm_cosq_gport_delete(c4_unit, gport_ucast_queue_group);
        if (BCM_FAILURE(rc)) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C4_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        for (cosq = 0; cosq < 4; cosq++) {
            ...
            rc = bcm_cosq_gport_detach(c4_unit, gport_scheduler_fq, gport_ucast_voq_connector_group, cosq);
            if (BCM_FAILURE(rc)) {
    ...
    for (nbr_queue_groups = 0; nbr_queue_groups < C4_NBR_QUEUE_GROUPS; nbr_queue_groups++) {
        ...
        /* delete connectors */
        rc = bcm_cosq_gport_delete(c4_unit, gport_ucast_voq_connector_group);
        if (BCM_FAILURE(rc)) {
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C4_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        rc = bcm_cosq_gport_detach(c4_unit, gport_ucast_scheduler, gport_scheduler_fq, 0);
        if (BCM_FAILURE(rc)) {
    ...
    for (nbr_fq_sched = 0; nbr_fq_sched < C4_NBR_FQ_SCHEDULERS; nbr_fq_sched++) {
        ...
        rc = bcm_cosq_gport_delete(c4_unit, gport_scheduler_fq);
        if (BCM_FAILURE(rc)) {
    ...
    for (nbr_sched = 0; nbr_sched < C4_NBR_SCHEDULERS; nbr_sched++) {
        ...
        rc = bcm_cosq_gport_delete(c4_unit, gport_ucast_scheduler);
        if (BCM_FAILURE(rc)) {
            printf("delete for SE(0x%x) failed, Error (%d, 0x%x)\n", gport_ucast_scheduler, rc, rc);
            return(rc);
        }
...
int
e2e_composite_se_verify(int kbits_sec_max_port, int port_idx)
{
    int                           rc = BCM_E_NONE;
    int                           idx = port_idx;
    ...
    rc = e2e_composite_se_verify_bandwidth(c4_unit, c4_e2e_parent_gport[idx], 0, kbits_sec_max_port, exp_margin);
    if (rc != BCM_E_NONE) {
        return(rc);
    }

    exp_margin = kbits_sec_max_port /100; /* 1% error */
    rc = e2e_composite_se_verify_bandwidth(c4_unit, c4_e2e_gport[idx], 0, kbits_sec_max_port, exp_margin);
    if (rc != BCM_E_NONE) {
        return(rc);
    }

    /* retrive e2e port scheduling element */
    rc = bcm_cosq_gport_get(c4_unit, c4_e2e_gport[idx], &physical_port, &num_cos_levels, &flags);
    if (rc != BCM_E_NONE) {
    ...
    for (nbr_sched = 0; nbr_sched < C4_NBR_SCHEDULERS; nbr_sched++) {
        ...
        rc = bcm_cosq_gport_get(c4_unit, gport_ucast_scheduler,
        ...
        switch(nbr_sched) {
            case 0:
                for (nbr_sched_flw = 0; nbr_sched_flw < C4_NBR_SCHEDULER_FLOWS; nbr_sched_flw++) {
                    ...
                    rc = e2e_composite_se_verify_mode(c4_unit, gport_ucast_scheduler, 0, exp_mode);
                    if (rc != BCM_E_NONE) {
                        return(rc);
                    }

                    /* retreive rate of Scheduler */
                    rc = e2e_composite_se_verify_bandwidth(c4_unit, gport_ucast_scheduler, 0, exp_kbits_sec_max, exp_margin);
                    if (rc != BCM_E_NONE) {
                        return(rc);
                    }

                    /* retreive scheduler burst size */
                    rc = e2e_composite_se_verify_burst_size(c4_unit, gport_ucast_scheduler, 0, C4_SCHED_MAX_BURST, C4_SCHED_MAX_BURST_MARGIN);
                    if (rc != BCM_E_NONE) {
                        return(rc);
                    }
                }
