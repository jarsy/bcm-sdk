...
int port_extender_dynamic_swithing_coe_service_multiple_flows(int unit)
{
    int index;
    int rv = 0;
    ...
    for (index = 0; index < nof_coe_ports_multi_flows; index++) {
        if (coe_port_info_multi_flows[index].coe_port != 0) {
            rv = port_extender_dynamic_swithing_enable_set_multi_flows(unit, coe_port_info_multi_flows[index].coe_port, 1);
            if (rv != BCM_E_NONE) {
                printf("port_extender_dynamic_swithing_coe_mapping_set failed $rv\n");
                return rv;
            }
