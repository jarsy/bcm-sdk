...
vlan_egress_membership__start_run(int unit, vlan_egress_membership_s *test_data)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        if (soc_property_get(unit ,"bcm886xx_vlan_translate_mode", 0) == 1) {
            rv = port__tag_classification__set(unit, g_vlan_egress_membership.ports[i], /*tag_format*/ 5, 0x8100, 0xFFFFFFFF);
            if (rv != BCM_E_NONE) {
        ...
        vlan_port.egress_vlan = g_vlan_egress_membership.vlan[1];
        rv = bcm_vlan_port_create(unit, &vlan_port);
        if (rv != BCM_E_NONE) {
            printf("Error, bcm_vlan_port_create for port - %d, vlan - %d, rv - %d\n",
