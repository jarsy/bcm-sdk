...
int trill_virtual_rbrdige_ing_nickname_set(/* in */ int unit,
  ...
  result = bcm_field_qualify_L2DestHit(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 0x1, 0x1);
  if (BCM_E_NONE != result) {
...
int trill_virtual_rbrdige_ing_nickname_flooding_set(/* in */ int unit,
  ...
  result = bcm_field_qualify_L2DestHit(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 0x0, 0x1);
  if (BCM_E_NONE != result) {
      printf("Error in bcm_field_qualify_L2DestHit\n");
      return result;
  }
