...
system_vlan_port_lif_only_create(
    ...
    vp->failover_id = 0; /* no FEC allocated */
    vp->match_vlan = vlan; 
    vp->egress_vlan = vlan; 
    vp->vsi = 0; 
...
int system_vlan_port_mp_example(int unit1, int port1, int second_port1, int unit2, int port2){
    ...
    uint32 v1 = 10;
    bcm_gport_t gport1=8200;
    uint32 v2 = 30;
    bcm_gport_t gport2=8200;
    bcm_gport_t second_gport1=8201;
    ...
    if (encoding) {
      rv = system_vlan_port_to_l2_gport_forward_info(unit2, &vp2, &forward_info);
    } else {
    ...
    l2addr.flags = BCM_L2_STATIC;
    if (encoding) {
      rv = system_vlan_port_to_l2_gport_forward_info(unit1, &second_vp1, &forward_info);
    } else {
...
int system_vlan_port_p2p_example(int unit1, int port1, int unit2, int port2){
    ...
    /* remote gport info */
    if (encoding) {
      rv = system_vlan_port_to_l2_gport_forward_info(unit1, &vp1, &forward_info);
    } else {
...
int system_protected_vlan_port_example(int unit1, int port1, int unit2, int port2,int port3, int is_mp){
    ...
    int base = 0;
    /* on unit2 */
    bcm_gport_t gport2_lif_a =8200+base;
    bcm_gport_t gport2_lif_b =8201+base;
    /* on unit1 */
    ...
    uint32 v3 = 30;
    /* on unit1 */
    bcm_gport_t gport1_lif =8200+base;
