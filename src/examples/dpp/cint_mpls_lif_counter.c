...
/* counter profile set */
int lif_counting_profile_set(int unit, int source_type, int command_id, int stif_counter_id, int start_lif, int end_lif, int double_entry, int lif_counting_profile) 
{	
    int rv = BCM_E_NONE;
    if (source_type != bcmStatCounterSourceEgressTransmitOutlif && source_type != bcmStatCounterSourceIngressInlif ){
        printf("Error, source type should be EgressTransmitOutlif or IngressInLif\n");
    ...
    lif_counting.source.stif_counter_id = stif_counter_id;
