...
int mpls_dual_ecmp_main(int unit, int in_port, int out_port, int slb_resolve) {
    ...
    create_l3_egress_s l3eg_fec_1;
    l3eg_fec_1.out_tunnel_or_rif = tunnel_id_1;                                             /* Choose level-1 MPLS tunnel */
    BCM_L3_ITF_FEC_TO_GPORT_FORWARD_GROUP(l3eg_fec_1.out_gport, ecmp1_1.ecmp_intf); /* level-2 MPLS tunnel will be chosen by ECMP */
    l3eg_fec_1.failover_id = failover_id_fec;                                  /* Protect FEC to enable hierarchical FEC */
    l3eg_fec_1.failover_if_id = 0;
    sal_memcpy(l3eg_fec_1.next_hop_mac_addr, mpls_ecmp_info_1.next_hop_mac, 6);
    l3eg_fec_1.l3_flags = BCM_L3_ENCAP_SPACE_OPTIMIZED;
    l3eg_fec_1.fec_id = fec_id1;
    if (!is_device_or_above(unit,JERICHO_B0)) {
        rc = l3__egress__create(unit,&l3eg_fec_1);
        if (rc != BCM_E_NONE) {
            printf("Error, l3__egress__create\n");
            return rc;
        }
        l3eg_fec_1.failover_if_id = l3eg_fec_1.fec_id;
        l3eg_fec_1.arp_encap_id = 0;
    }
    rc = l3__egress__create(unit,&l3eg_fec_1);
    if (rc != BCM_E_NONE) {
        printf("Error, l3__egress__create\n");
	...
	fec_id1 = l3eg_fec_1.fec_id;
    printf("FEC created. ID = 0x%x, for tunnel id = 0x%x\n", fec_id1, tunnel_id_1);

    create_l3_egress_s l3eg_fec_2;
    l3eg_fec_2.out_tunnel_or_rif = tunnel_id_2;
    BCM_L3_ITF_FEC_TO_GPORT_FORWARD_GROUP(l3eg_fec_2.out_gport, ecmp1_2.ecmp_intf);
    l3eg_fec_2.failover_id = failover_id_fec;
    l3eg_fec_2.failover_if_id = 0;
    sal_memcpy(l3eg_fec_2.next_hop_mac_addr, mpls_ecmp_info_1.next_hop_mac, 6);
    l3eg_fec_2.l3_flags = BCM_L3_ENCAP_SPACE_OPTIMIZED;
    l3eg_fec_2.fec_id = fec_id2;
    if (!is_device_or_above(unit,JERICHO_B0)) {
        rc = l3__egress__create(unit,&l3eg_fec_2);
        if (rc != BCM_E_NONE) {
            printf("Error, l3__egress__create\n");
            return rc;
        }
        l3eg_fec_2.failover_if_id = l3eg_fec_2.fec_id;
        l3eg_fec_2.arp_encap_id = 0;
    }
    rc = l3__egress__create(unit,&l3eg_fec_2);
    if (rc != BCM_E_NONE) {
        printf("Error, l3__egress__create\n");
...
int mpls_hierarchal_ecmp_4by4(int unit,int in_port, int out_port) {
    ...
    for (i = 0;i < MPLS_ECMP_NOF_INTER; i++) {
        ...
        mpls_ecmp_cascaded_fec_create(unit, tunnel_id, out_port, out_vlan, &fecLevel2[i]);
    }
