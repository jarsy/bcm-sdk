...
int c1_sched[4] = {130049, 130053, 130057, 130061};
...
int example_setup(void)
{
    ...
    for (idx=0; idx<C1_PETRA_CARD_NUM_PORTS; idx++) {
        ...
        rv =_bcm_petra_cosq_core_get(unit, dest_gport[idx], 0, &core);
        if (rv != BCM_E_NONE) {
            printf("Failed on _bcm_petra_cosq_core_get() for local port 0x%08lX.\n", port);
            return rv;
        }
