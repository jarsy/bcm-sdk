...
vlan_egress_tag_acceptance_eve(int unit)
{
    ...
    if (soc_property_get(unit ,"bcm886xx_vlan_translate_mode", 0) == 1) {
        ...
        for (i = 0; i < 2; i++) {
            rv = port__tag_classification__set(unit, g_vlan_egress_tag_acceptance.ports[i], /*tag_format*/ 6, 0x8100, 0x8100);
            if (rv != BCM_E_NONE) {
...
vlan_egress_tag_acceptance_config_ports(int unit)
{
    ...
    for (i = 0; i < 2; i++) {
        ...
        rv = port__tpids__set(unit, g_vlan_egress_tag_acceptance.ports[i], 0x8100, 0x8100);
        if (rv != BCM_E_NONE) {
        ...
        if (soc_property_get(unit ,"bcm886xx_vlan_translate_mode", 0) == 1) {
            rv = port__tag_classification__set(unit, g_vlan_egress_tag_acceptance.ports[i], /*tag_format*/ 5, 0x8100, 0xFFFFFFFF);
            if (rv != BCM_E_NONE) {
                printf("Error, port__tag_classification__set for port - %d, rv - %d\n", g_vlan_egress_tag_acceptance.ports[i], rv);
                return rv;
            }
        ...
        vlan_port.egress_vlan = g_vlan_egress_tag_acceptance.vlan[1];
    ...
    printf("\n>>> Port %d VLANs %d and %d  created: gport = 0x%x)\n",
           ...
           vlan_port.match_inner_vlan, vlan_port.vlan_port_id);
