        ...
        /* in */ unsigned int range_to_select)
{
  ...
  int presel_id ;
  bcm_field_entry_t presel_flags ;
  uint32 flags[1];
  uint32 minPort[1];
  uint32 maxPort[1];
  int full_show ;
  ...
  if (BCM_E_NONE == result) {
    ...
    result = bcm_field_qualify_VPortRangeCheck(unit, presel_id | presel_flags, range_vport[range_to_select], FALSE);
    if (BCM_E_NONE != result) {
    ...
    if (BCM_E_NONE == result) {
      ...
      result = bcm_field_qualify_VPortRangeCheck_get(unit, presel_id | presel_flags, 3, &range[0], &invert, &count);
      if (BCM_E_NONE != result) {
