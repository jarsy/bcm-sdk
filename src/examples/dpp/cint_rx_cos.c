/*
 * $Id: cint_rx_cos.c,v 1.8 Broadcom SDK $
...
arad_rx_activate_cos(uint32 unit)
{
    ...
    cfg.chan_cfg[3].chains = 2;
  {
...

    {
        ...
        if ((rv =  bcm_rx_queue_register(unit, "RX CMD_OTHER", 7, appl_dcmn_rx_cos_callback_other, 106, NULL, 0x40)) < 0) {
        	  printf("Error in bcm_rx_queue_register 6\n");
            return rv;
        }
        if ((rv =  bcm_rx_queue_register(unit, "RX CMD_OTHER", 8, appl_dcmn_rx_cos_callback_other, 106, NULL, 0x40)) < 0) {
        	  printf("Error in bcm_rx_queue_register 6\n");
            return rv;
        }
        if ((rv =  bcm_rx_queue_register(unit, "RX CMD_OTHER", 9, appl_dcmn_rx_cos_callback_other, 106, NULL, 0x40)) < 0) {
        	  printf("Error in bcm_rx_queue_register 6\n");
            return rv;
        }
        if ((rv =  bcm_rx_queue_register(unit, "RX CMD_OTHER", 10, appl_dcmn_rx_cos_callback_other, 106, NULL, 0x40)) < 0) {
        	  printf("Error in bcm_rx_queue_register 6\n");
            return rv;
        }
...
_rx_stop_cos(int unit)
{
  int rv;
    ...
    rv = bcm_rx_queue_unregister( unit, 1, appl_dcmn_rx_cos_callback_1,100);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 1\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 2, appl_dcmn_rx_cos_callback_2,101);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 2\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 3, appl_dcmn_rx_cos_callback_3,102);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 3\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 4, appl_dcmn_rx_cos_callback_4,103);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 4\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 5, appl_dcmn_rx_cos_callback_5,104);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 5\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 6, appl_dcmn_rx_cos_callback_6,105);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 6\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 7, appl_dcmn_rx_cos_callback_other,106);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 7\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 8, appl_dcmn_rx_cos_callback_other,106);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 8\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 9, appl_dcmn_rx_cos_callback_other,106);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail  9\n");       
    } 
    rv = bcm_rx_queue_unregister( unit, 10, appl_dcmn_rx_cos_callback_other,106);
    if (rv < 0)  {
        printf("bcm_rx_unregister rx_intr_callback fail 10\n");       
    } 
