/* $Id: cint_pon_general_anti_spoofing.c,v 1.3 Broadcom SDK $
...
int pon_anti_spoofing_drop(int unit)
{
    ...
    bcm_field_entry_t *ent[4];
    bcm_field_group_config_t *grp;
    bcm_field_AppType_t app_type[3] = {bcmFieldAppTypeIp4MacSrcBind, bcmFieldAppTypeIp6MacSrcBind,
    ...
    /* For IPV4 DHCP and IPV6 DHCP, 
     * lookup is done and LEM result is found: do nothing 
     */
    result = bcm_field_qualify_InterfaceClassVPort(unit, *(ent[0]), 0x1, 0x1);
    if (BCM_E_NONE != result) {
        printf("Error ent[0] in bcm_field_qualify_InterfaceClassVPort returned %s\n", bcm_errmsg(result));
        auxRes = bcm_field_group_destroy(unit, grp->group);
        return result;
    }
	
    if (sal_strcmp(src_bind_mode, "IPV6") == 0 && sal_strcmp(src_bind_subnet_mode, "IPV6") == 0) 
    {i=1;}
    else
    {i=0;}
    ...
    result = bcm_field_qualify_InterfaceClassVPort(unit, *(ent[1]), 0x1, 0x1);
    if (BCM_E_NONE != result) {
        printf("Error ent[1] in bcm_field_qualify_InterfaceClassVPort %s\n", bcm_errmsg(result));
        auxRes = bcm_field_group_destroy(unit, grp->group);
        return result;
    }
    ...
    result = bcm_field_qualify_InterfaceClassVPort(unit, *(ent[2]), 0x1, 0x1);
    if (BCM_E_NONE != result) {
        printf("Error ent[2] in bcm_field_qualify_InterfaceClassVPort %s\n", bcm_errmsg(result));
        auxRes = bcm_field_group_destroy(unit, grp->group);
        return result;
    }
    if (sal_strcmp(src_bind_mode, "IPV6") == 0 && sal_strcmp(src_bind_subnet_mode, "IPV6") == 0) 
