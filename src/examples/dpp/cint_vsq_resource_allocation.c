...
cint_gl_vsq_resource_allocate_qax(int unit)
{
    ...
    for (core = 0; core < NOF_CORES; ++core) {
        for (pool_id = 0; pool_id < 2; ++pool_id) {
            ...
            amounts[2].nominal_headroom = (pool_id == 0) ? 0 : 2000;
...
cint_src_vsqs_create_qax(int unit, bcm_port_t src_port, bcm_gport_t *src_vsq_gport, bcm_gport_t *pg_base_vsq_gport)
{
    ...
    for (cosq = 0; cosq < vsq_config.numq; cosq++) {
        vsq_config.pg_attributes[cosq].cosq_bitmap = 0x1 << cosq;
        vsq_config.pg_attributes[cosq].pool_id = pool_id;
        vsq_config.pg_attributes[cosq].ocb_only = 0;    /* Not relevant for QAX */
        vsq_config.pg_attributes[cosq].is_lossles = 1;  /* For lossy queue (not using headroom), change to 0.
...
cint_src_vsqs_resource_allocate_qax(int unit, int numq, uint8 pool_id, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
     ...
     * Bytes        - resource = bcmReservationResourceBytes;
     * SRAM-Buffers - resource = bcmReservationResourceOcbBuffers;
     * SRAM-PDs     - resource = bcmResourceOcbPacketDescriptors;
     */
    /* SRAM-Buffers amounts */
    resource = bcmReservationResourceBytes;
    amounts.flags = 0;
    amounts.reserved = 1024;
    amounts.max_shared_pool = (pool_id == 0) ? 4096: 5120;
    amounts.nominal_headroom = (pool_id == 0) ? 0 : 1024;
    amounts.max_headroom = amounts.nominal_headroom; /* max_headroom mast be equal to nominal_headroom for SRC-PORT-VSQ */
    ...
    target.pool_id = pool_id;
    target.is_ocb_only = 0; /* Not relevant for QAX */
    target.color = 0; /* This example uses BCM_COSQ_RSRC_ALLOC_COLOR_BLIND */

    printf("Src Port Rsrc Alloc: gport(0x%x) pool(%d) reserved(0x%x) headroom(0x%x) sh_pool(0x%x)\n",
            vsq_gport, pool_id, reserved, max_headroom, max_shared_pool);

    rv = bcm_cosq_resource_allocation_set(unit, flags, resource, &target, &amounts);
    if (BCM_FAILURE(rv)) {
        printf("resource_allocate: bcm_cosq_resource_allocation_set failed\n");
        return rv;
    }
    ...
    for (offset = 0; offset < numq; offset++) {
        ...
        switch (offset) {
            ...
            case 6:
                reserved = 300;
                nominal_headroom = 100;
                max_headroom = nominal_headroom + 1;
                max_shared_pool = 2000;
            break;
            case 7:
                reserved = 100;
                nominal_headroom = 200;
                max_headroom = nominal_headroom + 1;
                max_shared_pool = 2000;
            break;
        }

        printf("PG Rsrc Alloc: gport(0x%x) reserved(0x%x) headroom(0x%x) sh_pool(0x%x)\n",
                vsq_gport, reserved, max_headroom, max_shared_pool);

        target.gport = vsq_gport;
        target.color = 0; /* This example uses BCM_COSQ_RSRC_ALLOC_COLOR_BLIND */
        target.pool_id = pool_id;     /* target.pool_id should match how this PG-VSQ was created */
        target.is_ocb_only = 0;     /* Not relevant for QAX */
        amounts.reserved = reserved;
        amounts.max_headroom = max_headroom;
        amounts.nominal_headroom = nominal_headroom;
        amounts.max_shared_pool = max_shared_pool;
        amounts.shared_pool_fadt.thresh_max = max_shared_pool;
        amounts.shared_pool_fadt.thresh_min = max_shared_pool; /* This example actually applies taildrop */
        amounts.shared_pool_fadt.alpha = 0;

        rv = bcm_cosq_resource_allocation_set(unit, flags, resource, &target, &amounts);
        if (BCM_FAILURE(rv)) {
            printf("resource_allocate: bcm_cosq_resource_allocation_set failed (offset %d)\n", offset);
            return rv;
        }
...
cint_src_vsqs_pfc_setup_qax(int unit, int numq, bcm_gport_t src_vsq_gport, bcm_gport_t pg_base_vsq_gport)
{
    ...
    int i;
    uint32 flags = BCM_COSQ_PFC_BYTES; /* This example configures Bytes */
    ...
    /* Thresholds for SRC_PORT PFC */
    sal_memset(&config, 0, sizeof(config));
    config.xoff_threshold = 1024;
    config.xon_threshold = 1024;
    ...
    /* Thresholds for PG PFC */
    sal_memset(&config, 0, sizeof(config));
    config.xoff_fadt_threshold.thresh_min = 1024;
    config.xoff_fadt_threshold.thresh_max = 1024;
    config.xoff_fadt_threshold.alpha = 0;
    config.xon_fadt_offset = 0;
