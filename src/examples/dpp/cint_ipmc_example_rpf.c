...
int ipv4_mc_example_trap(int unit, int in_port, int out_port){
    ...
    BCM_GPORT_SYSTEM_PORT_ID_SET(config_l.dest_port,255); /* Usually the CMIC port in Negev application example */
...
0     */
    ...
    for(idx = 0; idx < eg_itf_index; ++idx) {
        /* Create the IP MC entry */
        rv = add_ip4mc_entry_with_egress_itf(unit, mc_ip, src_ip, 0/*VLAN*/, fec[idx]);
        if (rv != BCM_E_NONE) return rv;
        mc_ip++;
