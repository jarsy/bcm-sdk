...
{    
    ...
    result = bcm_field_qualify_RxTrapCode32(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, 206, 0xff); 
	if (BCM_E_NONE != result) {
	...
	result = bcm_field_qualify_ForwardingType(unit, presel_id | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeIp6Ucast);
    if (BCM_E_NONE != result) {
    ...
    result = bcm_field_qualify_Stage(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldStageIngress); 
	if (BCM_E_NONE != result) {
		printf("Error in bcm_field_qualify_Stage Err %x\n",result);
		return result;
	} 
    ...
    result = bcm_field_qualify_ForwardingType(unit, presel_id2 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeIp6Ucast);
    if (BCM_E_NONE != result) {
    ...
    if (remote_destination_trap_code) {
        ...
        result = bcm_field_qualify_Stage(unit, presel_id3 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldStageIngress); 
    	if (BCM_E_NONE != result) {
    		printf("Error in bcm_field_qualify_Stage Err %x\n",result);
    		return result;
    	} 
        ...
        /* match criteria acording to the HW trap code - bcmRxTrapOamBfdIpv6*/
        result = bcm_field_qualify_RxTrapCode32(unit, presel_id3 | BCM_FIELD_QUALIFY_PRESEL, remote_destination_trap_code, 0xff); 
    	if (BCM_E_NONE != result) {
    		printf("Error in bcm_field_qualify_RxTrapCode32 Err %x\n",result);
    		return result;
    	} 
    	...
    	/* match criteria acording to the 2nd LEM lookup*/
    	result = bcm_field_qualify_L2DestHit(unit, presel_id3 | BCM_FIELD_QUALIFY_PRESEL, 1,1); 
    	if (BCM_E_NONE != result) {
    		printf("Error in bcm_field_qualify_L2DestHit Err %x\n",result);
    		return result;
    	} 
        ...
        if (!is_inject) {
        	/* match criteria Packet-Format-Qualifier[0] for not injected packet*/
        	result = bcm_field_qualify_Ptch(unit, presel_id3 | BCM_FIELD_QUALIFY_PRESEL, 0,0x7); 
            if (BCM_E_NONE != result) {
        		printf("Error in bcm_field_qualify_Ptch Err %x\n",result);
        		return result;
        	} 
        ...
        result = bcm_field_qualify_ForwardingType(unit, presel_id3 | BCM_FIELD_QUALIFY_PRESEL, bcmFieldForwardingTypeIp6Ucast);
        if (BCM_E_NONE != result) {
              printf("Error in bcm_field_qualify_ForwardingType\n");
              return result;
        }
