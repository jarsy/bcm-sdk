...
static void pp_dump_parser_print_entries(int unit, void *curTop, int depth, int offset, int base, int macro, int first, int last)
{
    ...
    if(first == last)
        ...
        sprintf(entry_str, "Entries:%d-%d", first, last);
    ...
    if(base != PARSER_PROGRAM_END) {
        if(base != PARSER_PROGRAM_FIN) {
            int segment = base >> 2;
            RHDATA_SET_INT(cur, "Next-Segment", segment);
            next_segment_map[segment]++;
        }
    ...
    if((base == PARSER_PROGRAM_END) || (base == PARSER_PROGRAM_FIN)) {
        for(i = first; i <= last; i++)  {
            ...
            if(i == first) {
                ...
                sprintf(entry_str, "%s(0x%X)", dpp_parser_pfc_string_by_hw(first_pfc), first_pfc);
                if(first_addr == (i - 1))
                    sprintf(attr_str, "PFC:%d", first_addr);
                else
                    sprintf(attr_str, "PFC:%d-%d", first_addr, (i -1));
                RHDATA_SET_STR(cur, attr_str, entry_str);
                sprintf(entry_str, "%s(0x%X)", dpp_parser_plc_string_by_hw(first_pfc, first_plc), first_plc);
                if(first_addr == (i - 1))
                    sprintf(attr_str, "PLC:%d", first_addr);
                else
                    sprintf(attr_str, "PLC:%d-%d", first_addr, (i -1));
                RHDATA_SET_STR(cur, attr_str, entry_str);
                first_pfc = curr_pfc;
                first_plc = curr_plc;
                first_addr = i;
            ...
            if(i == last) {
                ...
                if(first_addr == last)
                    ...
                    sprintf(attr_str, "PFC:%d-%d", first_addr, last);
                RHDATA_SET_STR(cur, attr_str, entry_str);
                sprintf(entry_str, "%s(0x%X)", dpp_parser_plc_string_by_hw(first_pfc, curr_plc), curr_plc);
                if(first_addr == last)
                    sprintf(attr_str, "PLC:%d", first_addr);
                else
                    sprintf(attr_str, "PLC:%d-%d", first_addr, last);
                RHDATA_SET_STR(cur, attr_str, entry_str);
            }
        }
    }
...
static uint32 pp_dump_parser_programs(int unit, void *curTop, int depth)
{
    ...
    {
        sprintf(entry_str, "%s_%d", "Segment", i_seg);
        if((curSegment = dbx_xml_child_add(curTop, entry_str, depth)) == NULL) {
            cli_out("Failed to add Program Selection section to IRPP Parser\n");
            res = CMD_FAIL;
            goto exit;
        }
    ...
    {
        sprintf(entry_str, "%s_%d", "Segment", i_seg);
        curSegment = dbx_xml_child_get_first(curTop, entry_str);
