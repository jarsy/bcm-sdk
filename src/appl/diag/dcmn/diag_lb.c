...
#define DPP_LB_SUB_ITEM_DEC(_item) {                                                                                  \
    if ((sub_item_cnt < sizeof(item_header_str)/sizeof(char*)) && (sub_item_cnt < DPP_LB_NOF_SUB_ITEM_MAX)) {         \
...
#define _DPP_LB_CFG_RX_GLOBAL_ITEM_STR_INIT  {   \
        "BufferSize(M)",                         \
        "SeqNumWidth"                            \
        }
#define _DPP_LB_CFG_RX_LBG_ITEM_STR_INIT     {   \
        "Lbg",                                   \
        "Modems",                                \
        "LogicPort",                             \
        "Ptc",                                   \
...
#define _DPP_LB_CFG_RX_MODEM_ITEM_STR_INIT   {   \
        "Modem",                                 \
        "Lbg",                                   \
        "Port",                                  \
        "VLAN",                                  \
...
#define _DPP_LB_CFG_TX_GLOBAL_ITEM_STR_1_INIT {  \
        "SegmentMode ",                          \
        "VlanTpid",                              \
        "LbgTpid",                               \
        "PktCrcEna"                              \
...
#define _DPP_LB_CFG_TX_LBG_ITEM_STR_INIT     {   \
        "Lbg",                                   \
        "Enable",                                \
        "Modem ",                                \
        "LogicPort",                             \
  ...
  )
{
    char *block_line = BLOCK_LINE_STR;
    char *split_line = SPLIT_LINE_STR;
    DPP_LB_SUB_ITEM_VAR_INIT(_DPP_LB_CFG_RX_GLOBAL_ITEM_STR_INIT)
    ...
    /*** 2. print item header ***/
    cli_out("%s\n", split_line);
    _dpp_lb_item_print(item_header_str, sizeof(item_header_str)/sizeof(char*));
    ...
    cli_out("%s\n", split_line);
    _dpp_lb_item_print(item_pstr, sizeof(item_header_str)/sizeof(char*));
  ...
  )
{
    ...
    char *block_line = BLOCK_LINE_STR;
    char *split_line = SPLIT_LINE_STR;
    DPP_LB_SUB_ITEM_VAR_INIT(_DPP_LB_CFG_RX_LBG_ITEM_STR_INIT)
  ...
  )
{
    ...
    char *block_line = BLOCK_LINE_STR;
    char *split_line = SPLIT_LINE_STR;
    DPP_LB_SUB_ITEM_VAR_INIT(_DPP_LB_CFG_RX_MODEM_ITEM_STR_INIT)
    ...
    for (modem_tmp = modem_start; modem_tmp <= modem_end; modem_tmp++) {
        ...
        DPP_LB_SUB_ITEM_DEC2(rx_modem_info_tmp->vlan, BCM_VLAN_INVALID);
        DPP_LB_SUB_ITEM_HEX(rx_modem_info_tmp->fifo_start);
        DPP_LB_SUB_ITEM_HEX(rx_modem_info_tmp->fifo_end);
...
{
    char *split_line = SPLIT_LINE_STR;
    DPP_LB_SUB_ITEM_VAR_INIT(_DPP_LB_CFG_TX_GLOBAL_ITEM_STR_1_INIT)
    ...
    DPP_LB_SUB_ITEM_STR(tx_glb_info->is_pkt_crc_ena ? "Y" : "N");

    cli_out("%s\n", split_line);
    _dpp_lb_item_print(item_pstr, sizeof(item_header_str)/sizeof(char*));
    cli_out("%s\n", split_line);
    
    _dpp_lb_config_info_tx_global_pri_map_print(tx_glb_info);
  ...
  )
{
    ...
    char *block_line = BLOCK_LINE_STR;
    char *split_line = SPLIT_LINE_STR;
    DPP_LB_SUB_ITEM_VAR_INIT(_DPP_LB_CFG_TX_LBG_ITEM_STR_INIT)
...
{  
    ...
    if ((socLbDirectionRx == lb_key->direction) || (socLbDirectionBoth == lb_key->direction)) {
        if ((socLbObjGlobal == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            _dpp_lb_config_info_rx_global_print(&(lb_cfg->rx_cfg_info.rx_glb_info));
        }

        if ((socLbObjLbg == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            _dpp_lb_config_info_rx_lbg_print(lb_key->lbg_id, lb_cfg->rx_cfg_info.rx_lbg_info);
        }

        if ((socLbObjModem == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            _dpp_lb_config_info_rx_modem_print(lb_key->modem_id, lb_cfg->rx_cfg_info.rx_modem_info);
        }
    }

    if ((socLbDirectionTx == lb_key->direction) || (socLbDirectionBoth == lb_key->direction)) {
        if ((socLbObjGlobal == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            _dpp_lb_config_info_tx_global_print(&(lb_cfg->tx_cfg_info.tx_glb_info));
        }

        if ((socLbObjLbg == lb_key->obj_type) || (socLbObjAll == lb_key->obj_type)) {
            _dpp_lb_config_info_tx_lbg_print(lb_key->lbg_id, lb_cfg->tx_cfg_info.tx_lbg_info);
        }
  ...
  )
{
    ...
    sal_snprintf(counter_name, sizeof(counter_name), "TDrop");
    ...
    cmd_dpp_lb_config_show(int unit, args_t *a)
{
    ...
    if (CMD_OK != _dpp_lb_config_info_print(&lb_key, lb_cfg_info)) {
