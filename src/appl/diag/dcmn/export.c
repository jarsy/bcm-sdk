...
diag_dnx_export_script(int unit, args_t* args)
{
     ...
     */
    dbx_xml_property_set_str(outTop, "device", (char *)soc_dev_name(unit));
    dbx_xml_property_set_str(outTop, "family", SOC_CHIP_STRING(unit));
    /*
     ...
     */
    {
        ...
        dbx_xml_property_set_str(expDesc, "type", "script");
        dbx_xml_property_set_str(expDesc, "script", in_file);
        dbx_xml_property_set_str(expDesc, "source", is_script_in_common_dir ? "common directory" : "local");
        dbx_xml_node_end(expDesc, 1);
    }
    /*
     ...
     */
    {
        char buffer[2*EXPORT_DCMN_SHELL_CMD_BUFFER_SIZE];
        xml_node curCmd;
...
diag_dnx_export_path(int unit, args_t* args)
{
     ...
     */
    dbx_xml_property_set_int(pathNode, "in_port",      port);
    dbx_xml_property_set_int(pathNode, "ingress_core", path_dump_info.ingress_core);
    dbx_xml_property_set_int(pathNode, "egress_core",  path_dump_info.egress_core);
    dbx_xml_property_set_str(pathNode, "drop",         path_dump_info.drop_name);
    dbx_xml_property_set_int(pathNode, "is_olp",       path_dump_info.is_olp_port);
    dbx_xml_property_set_int(pathNode, "is_oamp",      path_dump_info.is_oamp_port);
    /*
     * Add path - close path node
     */
    dbx_xml_node_end(pathNode, 1);
    /*
     * Save and close XML file
     */
