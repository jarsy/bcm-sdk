...
cmd_result_t cmd_dpp_parser(int unit, args_t * a);
    ...
    args_t * a)
{
    ...
    parse_table_init(unit, &pt);
    parse_table_add(&pt, "plc",   PQ_DFL | PQ_INT, &plc_flag, &plc_flag, NULL);
    if (parse_arg_eq(a, &pt) < 0) {
        return CMD_USAGE;
    }

    PRT_TITLE_SET("Parser Configuration Matrix");

    PRT_COLUMN_ADD("PFC Name");
    PRT_COLUMN_ADD("SW PFC");
    PRT_COLUMN_ADD("HW PFC");
    PRT_COLUMN_ADD("SEC");
    PRT_COLUMN_ADD("VTT PFC");
    PRT_COLUMN_ADD("PMF PFC");
    PRT_COLUMN_ADD("IS_PROTOS");
    if(plc_flag == FALSE)
    {
    ...
    {
        PRT_COLUMN_ADD("PLC Profile/Name");
        PRT_COLUMN_ADD("SW PLC");
        PRT_COLUMN_ADD("HW PLC");
    }
    ...
    for(pfc_info = dpp_parser_pfc_info; pfc_info->sw != -1; pfc_info++)
    {
        ...
        if(pfc_info->is_proto & IS_ETH)
            sal_strcpy(proto_str, "eth");
        else
            sal_strcpy(proto_str, "");
        if(pfc_info->is_proto & IS_IPV4)
            sal_strcpy(proto_str + strlen(proto_str), " ipv4");
        if(pfc_info->is_proto & IS_IPV6)
            sal_strcpy(proto_str + strlen(proto_str), " ipv6");
        if(pfc_info->is_proto & IS_MPLS)
            sal_strcpy(proto_str + strlen(proto_str), " mpls");
        if(pfc_info->is_proto & IS_TRILL)
            sal_strcpy(proto_str + strlen(proto_str), " trill");
        if(pfc_info->is_proto & IS_UD)
            sal_strcpy(proto_str + strlen(proto_str), " user defined");
        ...
        if(plc_profile->id != -1)
        {
            ...
            if((plc_flag == TRUE) && (plc_profile->plc_info != NULL))
            {
                for(plc_info = plc_profile->plc_info; plc_info->plc_sw != -1; plc_info++)
                {
                    ...
                    PRT_CELL_SET("0x%X", plc_info->plc_sw);
                    PRT_CELL_SET("0x%X", plc_info->plc_hw);
                }
            }
        }
        else
...
{   
  ...
  for (core=first_core; core < last_core; core++) {
      ...
      cli_out("%s\r\n", get_core_str(unit, core));
      SOC_PPC_DIAG_PARSING_INFO_print(&prm_pars_info);
  }
  return CMD_OK; 
} 
 ...
 * Print last packet(s) termination information (per core) to an xml file
 */
int frwrding_decision_info_to_xml(int unit, SOC_PPC_FRWRD_DECISION_TYPE_INFO *info, SOC_PPC_FRWRD_DECISION_TYPE frwrd_type, xml_node cur_node);
                      ...
                      if(frwrd_info_per_core[core].is_kbp) {
                        ...
                        if (frwrd_info_per_core[core].ip_reply_record.match_status) {
                            ...
                            if (frwrd_info_per_core[core].ip_reply_record.identifier == 0x0) {
                                ...
                                SHR_IF_ERR_EXIT(dbx_xml_property_set_str(ip_reply_record, "destination_type", (char *)(SOC_PPC_FRWRD_DECISION_TYPE_to_string(frwrd_info_per_core[core].ip_reply_record.destination.type))));
                                SHR_IF_ERR_EXIT(dbx_xml_property_set_int(ip_reply_record, "destination_id", (frwrd_info_per_core[core].ip_reply_record.destination.dest_id)));
                                SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit, &frwrd_info_per_core[core].ip_reply_record.destination.additional_info, frwrd_info_per_core[core].ip_reply_record.destination.type, ip_reply_record));
                            }
...
#endif /* defined(INCLUDE_KBP) && !defined(BCM_88030_A0) */
                      {
                        ...
                        if (frwrd_info_per_core[core].key_found)
                        {
                            ...
                            case SOC_PPC_DIAG_FWD_LKUP_TYPE_MACT:
                                    ...
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_int(frwrding_info, "dest_id", frwrd_info_per_core[core].lkup_res.mact.frwrd_info.forward_decision.dest_id));
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_str(frwrding_info, "forward_decision_type", (char *)SOC_PPC_FRWRD_DECISION_TYPE_to_string(frwrd_info_per_core[core].lkup_res.mact.frwrd_info.forward_decision.type)));
                                    SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit, &frwrd_info_per_core[core].lkup_res.mact.frwrd_info.forward_decision.additional_info, frwrd_info_per_core[core].lkup_res.mact.frwrd_info.forward_decision.type, frwrding_info));
                                ...
                                case SOC_PPC_DIAG_FWD_LKUP_TYPE_TRILL_MC:
                                    ...
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_str(frwrd_decision, "type", (char *)SOC_PPC_FRWRD_DECISION_TYPE_to_string(frwrd_info_per_core[core].lkup_res.frwrd_decision.type)));
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_int(frwrd_decision, "dest_id", frwrd_info_per_core[core].lkup_res.frwrd_decision.dest_id));
                                    SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit, &frwrd_info_per_core[core].lkup_res.frwrd_decision.additional_info, frwrd_info_per_core[core].lkup_res.frwrd_decision.type, frwrd_decision));
                                break;
                                case SOC_PPC_DIAG_FWD_LKUP_TYPE_IPV4_HOST:
                                    ...
                                    SHR_NULL_CHECK((frwrd_decision = dbx_xml_child_add(lkup_res, "frwrd_decision", 6)), _SHR_E_FAIL, "frwrd_decision");
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_str(frwrd_decision, "type", (char *)SOC_PPC_FRWRD_DECISION_TYPE_to_string(frwrd_info_per_core[core].lkup_res.host_info.frwrd_decision.type)));
                                    SHR_IF_ERR_EXIT(dbx_xml_property_set_int(frwrd_decision, "dest_id", frwrd_info_per_core[core].lkup_res.host_info.frwrd_decision.dest_id));
                                    SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit, &frwrd_info_per_core[core].lkup_res.host_info.frwrd_decision.additional_info, frwrd_info_per_core[core].lkup_res.host_info.frwrd_decision.type, frwrd_decision));
...
int frwrding_decision_info_to_xml(int unit, SOC_PPC_FRWRD_DECISION_TYPE_INFO *info, 
                                  SOC_PPC_FRWRD_DECISION_TYPE      frwrd_type, 
                                  xml_node cur_node) {
    ...
    case SOC_PPC_FRWRD_DECISION_TYPE_MC: 
        ...
        SHR_IF_ERR_EXIT(dbx_xml_property_set_str(eei, "type", (char *)SOC_PPC_EEI_TYPE_to_string(info->eei.type)));
        switch(info->eei.type)
        {
        case SOC_PPC_EEI_TYPE_MPLS:
            SHR_IF_ERR_EXIT(dbx_xml_property_set_str(eei, "mpls_command", (char *)SOC_PPC_MPLS_COMMAND_TYPE_to_string(info->eei.val.mpls_command.command)));
            SHR_IF_ERR_EXIT(dbx_xml_property_set_int(eei, "mpls_label", info->eei.val.mpls_command.label));
            SHR_IF_ERR_EXIT(dbx_xml_property_set_int(eei, "mpls_push_profile", info->eei.val.mpls_command.push_profile));
        break;
        case SOC_PPC_EEI_TYPE_TRILL:
            SHR_IF_ERR_EXIT(dbx_xml_property_set_int(eei, "trill_nick", info->eei.val.trill_dest.dest_nick));
            SHR_IF_ERR_EXIT(dbx_xml_property_set_int(eei, "trill_is_mc", info->eei.val.trill_dest.is_multicast));
        break;
        case SOC_PPC_EEI_TYPE_MIM:
            SHR_IF_ERR_EXIT(dbx_xml_property_set_int(eei, "mim_isid", info->eei.val.isid));
        break;
        default:
        break;
        }
    ...
    case SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG:
        ...
        SHR_IF_ERR_EXIT(dbx_xml_property_set_str(outlif, "type", (char *)SOC_PPC_OUTLIF_ENCODE_TYPE_to_string(info->outlif.type)));
        SHR_IF_ERR_EXIT(dbx_xml_property_set_int(outlif, "val", (info->outlif.val)));      
    break;
    case SOC_PPC_FRWRD_DECISION_TYPE_TRAP: 
        SHR_NULL_CHECK((trap = dbx_xml_child_add(cur_node, "trap", 5)), _SHR_E_FAIL, "trap");
        SHR_IF_ERR_EXIT(dbx_xml_property_set_int(trap, "frwrd_action_strength", info->trap_info.action_profile.frwrd_action_strength));
        SHR_IF_ERR_EXIT(dbx_xml_property_set_int(trap, "snoop_action_strength", info->trap_info.action_profile.snoop_action_strength));
        SHR_IF_ERR_EXIT(dbx_xml_property_set_str(trap, "trap_code", (char *)SOC_PPC_TRAP_CODE_to_string(info->trap_info.action_profile.trap_code)));
        SHR_IF_ERR_EXIT(dbx_xml_property_set_str(trap, "bcm_api", (char *)SOC_PPC_TRAP_CODE_to_api_string(info->trap_info.action_profile.trap_code)));
    break;
    default:
        break;
    }
...
int ipv6_print_to_xml(int unit, xml_node cur_node, char *attribute_name, SOC_SAND_PP_IPV6_ADDRESS *ipv6_addr){
    ...
    uint32 indx;
    char ipv6_addr_string[48] = "" ;
    char ipv6_addr_string_nibble[4] = "";
    SHR_FUNC_INIT_VARS(unit);
                ...
                if (lrn_info_per_core[core].valid) {
                    ...
                    SHR_NULL_CHECK((value = dbx_xml_child_add(curCore, "value", 5)), _SHR_E_FAIL, "value");
                    {
                        ...
                        SHR_NULL_CHECK((frwrd_info = dbx_xml_child_add(value, "frwrd_info", 6)), _SHR_E_FAIL, "frwrd_info");
                        {
                            ...
                            {
                                SHR_IF_ERR_EXIT(dbx_xml_property_set_str(frwr_decision, "type", (char *)SOC_PPC_FRWRD_DECISION_TYPE_to_string(lrn_info_per_core[core].value.frwrd_info.forward_decision.type)));
                                SHR_IF_ERR_EXIT(dbx_xml_property_set_int(frwr_decision, "dest_id", lrn_info_per_core[core].value.frwrd_info.forward_decision.dest_id));
                                SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit, &lrn_info_per_core[core].value.frwrd_info.forward_decision.additional_info, lrn_info_per_core[core].value.frwrd_info.forward_decision.type, frwr_decision));
                            }
        ...
        SHR_IF_ERR_EXIT(dbx_xml_property_set_str(curDiag, "name", "PKT_associated_TM_info"));
        {
            ...
            SOC_DPP_CORES_ITER(SOC_CORE_ALL, core) {
                ...
                SHR_IF_ERR_EXIT(dbx_xml_property_set_int(curCore, "id", core));
                  ...
                  {
                    SHR_IF_ERR_EXIT(dbx_xml_property_set_str(curCore, "frwrd_decision", (char *)SOC_PPC_FRWRD_DECISION_TYPE_to_string(pkt_tm_info_to_xml[core].frwrd_decision.type)));
                    SHR_IF_ERR_EXIT(dbx_xml_property_set_int(curCore, "dest_id", pkt_tm_info_to_xml[core].frwrd_decision.dest_id));
                    SHR_IF_ERR_EXIT(frwrding_decision_info_to_xml(unit,&pkt_tm_info_to_xml[core].frwrd_decision.additional_info, pkt_tm_info_to_xml[core].frwrd_decision.type, curCore));
                  }
                  else
                  {
    ...
    cmd_ppd_api_diag_rif_info_get(int unit, args_t* a)
{
  ...
  for (core=first_core; core < last_core; core++) {
      ...
      if (!b_print_to_xml) {
          ...
          if (SOC_IS_JERICHO(unit)) {      
              arad_pp_signal_mngr_signal_get(soc_sand_dev_id, core, SOC_DPP_SIGNAL_MNGR_SIGNAL_TYPE_RIF_PROFILE, val, &size_of_signal_in_bits);
...
cmd_ppd_api_diag_ipv4_mc_route_get(int unit, args_t* a) 
{
    ...
    uint32                              prm_nof_entries=0, prev_prm_nof_entries=0, index, start_index;
    char decimal_ip[SOC_SAND_PP_IPV4_MAX_IP_STRING];
    ...
    for (index = start_index; index < 2 ; index++) {
        ...
        while(!SOC_PPC_IP_ROUTING_TABLE_ITER_IS_END(&prm_block_range_key.start.payload))
        {
            ...
            for(i = 0; i < prm_nof_entries - prev_prm_nof_entries; i++)
            {
            ...
            prev_prm_nof_entries = prm_nof_entries;
        }
    }
    cli_out(" ---------------------------------------------------------------------------------- \n\r");
    return CMD_OK;
    ...
    cmd_ppd_api_diag_pp_last_parser(int unit, int core, cmd_diag_pp_packet_action_t * action_resolution, int print_details)
{
    ...
    pkt_format = dpp_parser_pfc_string_by_hw(prm_pars_info.pfc_hw);
    ...
    cmd_ppd_api_diag_pp_last_fec(int unit, int core, cmd_diag_pp_packet_action_t * action_resolution, int print_details)
{
    ...
    dpp_dsig_read(unit, core, "IRPP", "LBP", NULL, "Dst", &dest, 1);
...
{   
	...
	uint32 	print_db            = 0;
	uint32  print_advanced      = 0;
	uint32  print_debug         = 0;
	...
	parse_table_add(&pt, "iterate"  		, PQ_NO_EQ_OPT|PQ_STRING, (void *) "all", (void *) &iterate_table, NULL);
	...
	parse_table_add(&pt, "advanced"	 		, PQ_DFL|PQ_NO_EQ_OPT|PQ_BOOL, 0, &print_advanced, NULL);
	parse_table_add(&pt, "debug"	 		, PQ_DFL|PQ_NO_EQ_OPT|PQ_BOOL, 0, &print_debug, NULL);
	...
	if (0 >= nof_arg) {
	    cli_out(
	            ...
	            "\n\t\t[iterator][iterate=table]   - Dumps current KBP entries"
                    "\n\t\t                              [iterate=table] optional : iterates on specific table; defaults to All"
                    "\n\t\t                                  valid inputs for table (case insensitive)"
                    "\n\t\t                                      All       ACL"
                    "\n\t\t                                      IPv4      IPv4_UC    IPv4_UC_RPF    IPv4_MC"
                    "\n\t\t                                      IPv6      IPv6_UC    IPv6_UC_RPF    IPv6_MC"
                    "\n\t\t                                      MPLS      TRILL      TRILL_UC       TRILL_MC"
	            ...
	            "\n\t\t[print][advanced|debug]     - Print out the current configuration of tables and DB."
	            "\n\t\t                              [advanced] optional : Print additional information."
                "\n\t\t                              [debug] optional : Print additional and debug information"
	...
	if (print_db == TRUE ) {
        arad_kbp_sw_config_print(unit, (print_debug ? 2 : print_advanced));
	}
