/* $Id: utils_d_os_ssf.c,v 1.9 Broadcom SDK $
...
*INPUT:
*  SOC_SAND_DIRECT:
*    See VxWorks documentation. taskDelay().
*  SOC_SAND_INDIRECT:
*    See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskSpawn(VxWorks parameters,private_task_id)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskSpawn. See VxWorks documentation
*    In addition:
*      int private_task_id -
...
*  Implementation is specific to VxWorks.
*CALLING SEQUENCE:
*  d_is_task_safe()
*INPUT:
*  SOC_SAND_DIRECT:
...
*CALLING SEQUENCE:
*  d_ioGlobalStdGet(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
*    Same as taskDelete. See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
/* { */
  char
      *proc_name ;
  proc_name = "d_ioGlobalStdGet" ;
  ret = ioGlobalStdGet(stdFd) ;
  gen_err(TRUE,TRUE,(int)ret,ERROR,"ioGlobalStdGet - ",
      proc_name,SVR_FTL,D_IOGLOBALSTDGET_ERR_01,FALSE,0,-1,FALSE) ;
/* } __VXWORKS__*/
#else
  ret = stdFd;
#endif
 ...
 * start with d_<Original VxWorks Name>
 * {
 */
/*****************************************************
*NAME
...
*  operating system (single location of invocation).
*CALLING SEQUENCE:
*  d_write(VxWorks parameters,std_out_target)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as write. See VxWorks documentation
*    int std_out_target -
  ...
  )
{
  ...
  {
#ifdef __VXWORKS__
    proc_name = "d_taskLock" ;
/* { */
    status = taskLock() ;
    gen_err(TRUE,TRUE,(int)status,ERROR,"taskLock - ",
        proc_name,SVR_FTL,D_TASK_LOCK_ERR_01,FALSE,0,-1,FALSE) ;
/* } */
#else
/* { */
    status = soc_sand_os_task_lock() ;
  ...
  )
{
  ...
  {
#ifdef __VXWORKS__
    proc_name = "d_taskUnlock" ;
/* { */
    status = taskUnlock() ;
    gen_err(TRUE,TRUE,(int)status,ERROR,"taskUnlock - ",
        proc_name,SVR_FTL,D_TASK_UNLOCK_ERR_01,FALSE,0,-1,FALSE) ;
/* } */
#else
/* { */
    status = soc_sand_os_task_unlock();
...
*  invocation).
*CALLING SEQUENCE:
*  d_clock_gettime(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as clock_gettime. See VxWorks documentation
*  SOC_SAND_INDIRECT:
*    Same as clock_gettime. See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskDelete(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
...
*  operating system (single location of invocation).
*CALLING SEQUENCE:
*  d_ioGlobalStdSet(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
...
*       D_SOFT_REBOOT: Software reboot -- call to VxWorks 'reboot()'
*    int start_type -
*       If soft-reboot, how the boot ROMS will reboot (refer to VxWorks help)
*  SOC_SAND_INDIRECT:
*    None.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*DATE: 12/APR/2002
*FUNCTION:
*  Delete the shell task (if it is alive) and
*  restart it with this system's imposed shell
*  priority.
*  This may be viewed as a private wrap allowing for
*  control over operating system.
*CALLING SEQUENCE:
...
*    None
*  SOC_SAND_INDIRECT:
*    VxWorks details concerning shell.
*    IMPOSED_SHELL_PRIORITY, STACK_SIZE
*OUTPUT:
*  SOC_SAND_DIRECT:
*    int -
*      task ID of shell or ERROR, if shell task
*      does not exist.
*  SOC_SAND_INDIRECT:
*    Restarted shell
*REMARKS:
  ...
  )
{
  ...
  char
    *proc_name ;
  proc_name = "d_restart_shell" ;
  /*
   * Handle shell task only if it is alive.
   */
  ret = taskNameToId("tShell") ;
  if (ret != ERROR)
  {
    /*
     * Enter here only if shell is alive.
     */
    status = taskDelete(ret) ;
    gen_err(TRUE,TRUE,(int)status,ERROR,
          "d_restart_shell - taskDelete",
          proc_name,SVR_FTL,D_RESTART_SHELL_ERR_01,FALSE,0,-1,FALSE) ;
    ret =
      taskSpawn("tShell",IMPOSED_SHELL_PRIORITY,0,
#if (_WRS_VXWORKS_MAJOR >= 6)
            STACK_SIZE,(FUNCPTR)shellGenericInit,
#else
            STACK_SIZE,(FUNCPTR)shell,
#endif
            TRUE,0,0,0,0,0,0,0,0,0);
    gen_err(TRUE,TRUE,(int)ret,ERROR,
          "d_restart_shell - taskSpawn",
          proc_name,SVR_FTL,D_RESTART_SHELL_ERR_02,FALSE,0,-1,FALSE) ;
  }
  return (ret) ;
}
/*****************************************************
...
*  operating system (single location of invocation).
*CALLING SEQUENCE:
*  d_timer_create(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
         ...
         )
{
  ...
  char
      *proc_name ;
  proc_name = "d_timer_create" ;
  ret = timer_create(CLOCK_REALTIME,evp,pTimer) ;
  gen_err(TRUE,TRUE,(int)ret,ERROR,
      "d_timer_create - ",proc_name,SVR_FTL,
      D_TIMER_CREATE_ERR_01,FALSE,0,-1,FALSE) ;
  return (ret) ;
}
...
*DATE: 12/APR/2002
*FUNCTION:
*  Set standard IO for specified shell.
*  This is a private wrap allowing for control over
*  operating system (single location of invocation).
*CALLING SEQUENCE:
*  d_ioTaskStdSet(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
...
*    Same as taskDelete. See VxWorks documentation.
*REMARKS:
*  This routine has no effect if it is called at
*  interrupt level.
*
*  If stdFd is not 0, 1, or 2, this routine
*  has no effect.
*SEE ALSO:
*****************************************************/
void
  ...
  )
{
  char
      *proc_name ;
  proc_name = "d_ioTaskStdSet" ;
  ioTaskStdSet(taskId,stdFd,newFd) ;
  return ;
}
...
*DATE: 23/APR/2002
*FUNCTION:
*  Get the name associated with a task ID.
*  This is a private wrap allowing for control
*  over operating system (single location
*  of invocation).
*CALLING SEQUENCE:
*  d_taskName(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
...
*    Same as taskDelete. See VxWorks documentation.
*REMARKS:
*  This routine returns a pointer to the name of
*  a task of a specified ID, if the task has a name.
*  If the task has no name, it returns an empty
*  string.
*
*SEE ALSO:
*****************************************************/
  ...
  )
{
  ...
  char
      *proc_name ;
  proc_name = "d_taskName" ;
  ret = taskName(PTR_TO_INT(tid)) ;
  gen_err(TRUE,TRUE,(int)ret,(int)NULL,"taskName - ",
      proc_name,SVR_WRN,D_TASKNAME_ERR_01,FALSE,0,-1,FALSE) ;
  return (ret) ;
}
/*****************************************************
*NAME
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskIdSelf(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskDelete. See VxWorks documentation
*  SOC_SAND_INDIRECT:
...
*  SOC_SAND_DIRECT:
*    int -
*      Id of task to force (VxWorks ID).
*  SOC_SAND_INDIRECT:
*    VxWorks system.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
/* { */
  ...
  task_id = taskIdSelf() ;
  wind_tcb = taskTcb(task_id) ;
  if (semaphore->state.owner  == wind_tcb)
  {
    ret = TRUE ;
  }
  else
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskRestart(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskRestart. See VxWorks documentation
*  SOC_SAND_INDIRECT:
*    Same as taskRestart. See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
#ifndef SIM_ON_WINDOWS
  vxworks_tid = PTR_TO_INT(tid);
  if ((!vxworks_tid) || (vxworks_tid == d_taskIdSelf())) {
    ret = ERROR;
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskResume(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskResume. See VxWorks documentation
*  SOC_SAND_INDIRECT:
*    Same as taskResume. See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*  of invocation).
*CALLING SEQUENCE:
*  d_taskSuspend(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    Same as taskSuspend. See VxWorks documentation
*  SOC_SAND_INDIRECT:
*    Same as taskSuspend. See VxWorks documentation.
*OUTPUT:
*  SOC_SAND_DIRECT:
...
*CALLING SEQUENCE:
*  d_taskIdVerify(VxWorks parameters)
*INPUT:
*  SOC_SAND_DIRECT:
*    See VxWorks documentation
*  SOC_SAND_INDIRECT:
