...
*/
int test_run_bfd(int unit, int port_1, int port_2, int numEndpoints, int seed, int numStates, int BFDTimeTest) {
   int ret;
   int desc_ep_1;
   int desc_ep_2;
   int ep_i;
   ...
   if (BFDTimeTest) {
      ...
      if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
         ...
         /* max_total_endpoint_create_time  = 1.1 * (32300 + 4500 * (MEPnum - 2) + (9 * (MEPnum - 2) * (MEPnum - 2)) / 2);  Max time in usec */
         
         max_total_endpoint_create_time  = 90000 * MEPnum;
      } else {
         /* First 2 EP create take 82000usec due to configuration of OAM1/2 entries.
            Following EP creates takes 3000usec. Every EP create after take 8usec
            longer (in average)
   ...
   for (ep_i = 0; ep_i < numEndpoints; ep_i++) {
      ...
      ret = test_bfd_createEndpoint(unit, port_1, test_bfd_local_encap, test_bfd_IP_address[ip_i_1], test_bfd_IP_address[ip_i_2], desc_ep_1, desc_ep_2);
      BCMDNX_IF_ERR_EXIT(ret);
      

      ret = test_bfd_createEndpoint(unit, port_2, test_bfd_remote_encap, test_bfd_IP_address[ip_i_2], test_bfd_IP_address[ip_i_1], desc_ep_2, desc_ep_1);
      BCMDNX_IF_ERR_EXIT(ret);
