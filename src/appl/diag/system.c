...
#if defined(MRV_STANDALONE) /* Jayant comment below loop */
    while (1) {
    sh_process(-1, "BCM", TRUE);
#ifdef NO_SAL_APPL
    return;
...
#endif /* Jayant comment below loop */
}
...
/*
 * Function:
 *  diag_shell_base_initialize
 * Purpose:
 *    Function used to to initialize base diag shell.
 */

int diag_shell_base_initialize(){
...
/*
 * Function:
 *  diag_unit_initialize
 * Purpose:
 *    Function used to to initialize individual fe unit.
 */

int
diag_unit_initialize(int unit)
{
    uint32  flags;
#ifndef NO_SAL_APPL
    char    *script;
     ...
     */
    flags = sal_boot_flags_get();
//	cli_out("diag_unit_initialize (%d): SOC unit %d attach failed\n", unit);
    rv =  sysconf_probe_main(unit);
    if (rv < 0) {
       cli_out("ERROR: SOC unit %d probe failed\n", unit);
    }
    ...
    if (rv < 0) {
       cli_out("ERROR: SOC unit %d attach failed\n", unit);
            ...
            if (soc_attached(unit)) {
             ...
             char * sbx_soc_file;
             if ((sbx_soc_file = getenv("SBX_SOC_FILE")) != NULL){
               diag_rc_set(unit, sbx_soc_file);
             } else{
#endif
               diag_rc_set(unit, "sbx.soc");
#ifndef __KERNEL__
             }
#endif
             }
            ...
            if (SOC_IS_ROBO(unit)){
             ...
             char * robo_soc_file;
             if ((robo_soc_file = getenv("ROBO_SOC_FILE")) != NULL){
               diag_rc_set(unit, robo_soc_file);
             } else{
#endif
               diag_rc_set(unit, "robo.soc");
#ifndef __KERNEL__
             }
#endif
            }
    ...
    if (flags & BOOT_F_RELOAD) {
      ...
      easy_reload = TRUE;
        ...
        SOC_RELOAD_MODE_SET(SOC_NDEV_IDX2DEV(unit), TRUE);
    } else {
      cli_out("Boot flags: regular load\n");
        SOC_RELOAD_MODE_SET(SOC_NDEV_IDX2DEV(unit), FALSE);
    }
#endif
#endif
    ...
    if (flags & BOOT_F_WARM_BOOT) {
        ...
        warm_boot = TRUE;
            ...
            diag_rc_set(SOC_NDEV_IDX2DEV(unit), "reload.soc");
    } else {
        cli_out("Boot flags: Cold boot\n");
            SOC_WARM_BOOT_DONE(SOC_NDEV_IDX2DEV(unit));
    }

        stable_filename = soc_property_get_str(SOC_NDEV_IDX2DEV(unit), spn_STABLE_FILENAME);

        if (soc_property_get_str(SOC_NDEV_IDX2DEV(unit), "scache_nh") != NULL) {
            stable_location = _SHR_SWITCH_STABLE_DEVICE_NEXT_HOP;
            stable_flags = 0;
            stable_size = 10 * 1024;      /* 10 Kbytes */
        } else if (soc_property_get_str(SOC_NDEV_IDX2DEV(unit), "scache_nh_basic") != NULL) {
            stable_location = _SHR_SWITCH_STABLE_DEVICE_NEXT_HOP;
            stable_flags = SOC_STABLE_BASIC;
            stable_size = 2 * 1024;      /* 2 Kbyte */
            /* Treat all virtual ports as MIM for limited Level 2 */
            SOC_WARM_BOOT_MIM(SOC_NDEV_IDX2DEV(unit));
        } else if ((NULL == stable_filename) &&
                   (stable_filename =
                    soc_property_get_str(SOC_NDEV_IDX2DEV(unit), "scache_filename"))) {
            stable_location = _SHR_SWITCH_STABLE_APPLICATION;
            stable_flags = 0;
            stable_size = SOC_DEFAULT_LVL2_STABLE_SIZE;
            if (soc_feature(SOC_NDEV_IDX2DEV(unit), soc_feature_ser_parity) &&
                soc_property_get(SOC_NDEV_IDX2DEV(unit), "memcache_in_scache", 0)) {
                if (SOC_IS_TD2_TT2(SOC_NDEV_IDX2DEV(unit))) {
                    uint16 dev_id;
                    uint8  rev_id;
                    soc_cm_get_id(SOC_NDEV_IDX2DEV(unit), &dev_id, &rev_id);
                    if (dev_id == BCM56830_DEVICE_ID) {
                        stable_size *= 20;
                    } else {
                        stable_size *= 12;
                    }
                } else if (SOC_IS_TRIUMPH3(SOC_NDEV_IDX2DEV(unit))) {
                    stable_size *= 30;
                } else if (SOC_IS_TD_TT(SOC_NDEV_IDX2DEV(unit))) {
                    stable_size *= 14;
                } else {
                    stable_size *= 16;
                }
        ...
        stable_location = soc_property_get(SOC_NDEV_IDX2DEV(unit), spn_STABLE_LOCATION,
                                           stable_location);

        if (BCM_SWITCH_STABLE_NONE != stable_location) {
            /* Otherwise, nothing to do */
            stable_flags = soc_property_get(SOC_NDEV_IDX2DEV(unit), spn_STABLE_FLAGS,
                                            stable_flags);
            stable_size = soc_property_get(SOC_NDEV_IDX2DEV(unit), spn_STABLE_SIZE,
                                           stable_size);

            if (!(stable_flags & SOC_STABLE_BASIC)) {
                if (soc_property_get(SOC_NDEV_IDX2DEV(unit), "scache_diffserv", FALSE)) {
                    stable_flags |= SOC_STABLE_DIFFSERV;
                }
            }
            ...
            if ((BCM_SWITCH_STABLE_APPLICATION == stable_location) &&
...
#if (defined(LINUX) && !defined(__KERNEL__)) || defined(UNIX)
                /* Try to open the scache file */
                if (appl_scache_file_open(SOC_NDEV_IDX2DEV(unit), (flags & BOOT_F_WARM_BOOT) ?
                                          TRUE : FALSE,
                                          stable_filename) < 0) {
               ...
               if (appl_scache_user_buffer(SOC_NDEV_IDX2DEV(unit))) {
                   cli_out("Unit %d: error when initializing user buffer", SOC_NDEV_IDX2DEV(unit));
            ...
            if (soc_stable_set(SOC_NDEV_IDX2DEV(unit), stable_location, stable_flags) < 0) {
                cli_out("Unit %d: soc_stable_set failure\n", SOC_NDEV_IDX2DEV(unit));
            } else if (soc_stable_size_set(SOC_NDEV_IDX2DEV(unit), stable_size) < 0) {
                cli_out("Unit %d: soc_stable_size_set failure\n",SOC_NDEV_IDX2DEV(unit));
            }

#if defined(BCM_PETRA_SUPPORT)
            if (flags & BOOT_F_WARM_BOOT && SOC_IS_ARAD(SOC_NDEV_IDX2DEV(unit))) {
                if (soc_stable_size_set(SOC_NDEV_IDX2DEV(unit), stable_size)) {
                    cli_out("Unit %d: soc_stable_size_set failed. stable_size=%d\n", SOC_NDEV_IDX2DEV(unit), stable_size);
                }     
                if (soc_scache_recover(SOC_NDEV_IDX2DEV(unit))) {
                    cli_out("Unit %d: soc_scache_recover failed\n", SOC_NDEV_IDX2DEV(unit));
                }    
                if (soc_warmboot_is_allowed_verify(SOC_NDEV_IDX2DEV(unit))) {
                    cli_out("Scache version check failed\n");
                }    
            }    
#endif /* defined(BCM_PETRA_SUPPORT) */
            if (1 == soc_property_get(SOC_NDEV_IDX2DEV(unit),
                        spn_WARMBOOT_EVENT_HANDLER_ENABLE, 0)) {
                if (soc_event_register(SOC_NDEV_IDX2DEV(unit),
                        appl_warm_boot_event_handler_cb, NULL) < 0) {
                    cli_out("Unit %d: soc_event_register failure\n",SOC_NDEV_IDX2DEV(unit));
                }
            }
            ...
            /* EMPTY SCACHE INITIALIZATION ->
               ...
               initiating scache with size 0(zero). in order that scache commits 
               wont fail and cause application exit upon startup */
            if (soc_switch_stable_register(SOC_NDEV_IDX2DEV(unit),
                                           &scache_read_dummy_func,
                                           &scache_write_dummy_func,
                                           NULL, NULL) < 0) {
                cli_out("Unit %d: soc_switch_stable_register failure\n",SOC_NDEV_IDX2DEV(unit));
        ...
        if (SOC_IS_SAND(SOC_NDEV_IDX2DEV(unit))){
            if (bsldnx_mgmt_init(SOC_NDEV_IDX2DEV(unit)) < 0) {
                cli_out("Unit %d: bslmgmt_dnx_init failure\n", SOC_NDEV_IDX2DEV(unit));
            }
        }
#endif
    ...
    if (!(flags & BOOT_F_NO_RC)) {
           if (soc_attached(SOC_NDEV_IDX2DEV(unit))) {
                ...
                if (diag_rc_load(SOC_NDEV_IDX2DEV(unit)) != CMD_OK) {
                    cli_out("ERROR loading rc script on unit %d\n", SOC_NDEV_IDX2DEV(unit));
                }
            }
    } else if (no_rc_warning) {
...
#if defined(BCM_TK371X_SUPPORT)
    if(BCM_E_NONE == soc_ea_do_init(soc_ndev)){
            if(soc_attached(SOC_NDEV_IDX2DEV(unit)) && SOC_IS_TK371X(SOC_NDEV_IDX2DEV(unit))){
                bcm_init(SOC_NDEV_IDX2DEV(unit));
               
            	// The following code is a workaround till extraction will be detected by interrupt
            	// If Counters DMA is interrupted in the middle, the driver will fail and exit
            	// By: Yoram Shechori
