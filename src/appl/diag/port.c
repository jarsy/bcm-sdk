 ...
 */
{
    ...
    const int speeds[] = { 10, 100, 1000, 2500, 10000, 40000 };
...
typedef struct flexport_pm_info_s {
    ...
    char *pm_str;
    int ports[BCMI_XGS5_PORTS_PER_PBLK];
    int port_cnt;

    /* Parsed info */
    bcm_port_resource_t pr[BCMI_XGS5_PORTS_PER_PBLK];
    uint32 flags;
...
{
    int blk_idx, blk_type, pm, port, rv ;
    ...
    for (blk_idx = 0; blk_idx < SOC_MAX_NUM_BLKS; blk_idx++) {
        ...
        if ((blk_type == SOC_BLK_CLPORT) &&
            (pm < MAX_PM_NUM) &&
            (f->pm[pm].flags & FLXP_F_SEEN)) {
            cli_out("PM%d:\n", pm);
            if (SOC_PBMP_IS_NULL(SOC_INFO(unit).block_bitmap[blk_idx])) {
                cli_out("\tNone\n");
            } else {
                SOC_PBMP_ITER(SOC_INFO(unit).block_bitmap[blk_idx], port) {
                    bcm_port_resource_t_init(&pr);
                    rv = bcm_port_resource_get(unit, port, &pr);
                    if (rv < 0) {
                    ...
                    cli_out("\t%s   Port(%d) PhysicalPort(%d) Speed(%s) "
                            ...
                            _if_fmt_speed(buf, pr.speed), pr.lanes,
                            encap_mode[pr.encap]);
                }
            }
        }
...
flexport_parse_pm_mode(int unit, flexport_info_t *f, int *auto_assign)
{
    ...
    bcm_port_resource_t *pr;
    int blk_idx, blk_type, pm, phy_base, i, hg_speed;
    int del_cnt, req_cnt;
    char *s;
    ...
    for (blk_idx = 0; blk_idx < SOC_MAX_NUM_BLKS; blk_idx++) {
        blk_type = SOC_BLOCK_TYPE(unit, blk_idx);
        pm = SOC_BLOCK_NUMBER(unit, blk_idx);
        if ((blk_type == SOC_BLK_CLPORT) && (pm < MAX_PM_NUM) &&
            (pi[pm].pm_str != NULL)) {
            s = pi[pm].pm_str;
            pr = pi[pm].pr;
            phy_base = pm * BCMI_XGS5_PORTS_PER_PBLK + 1;

            /* Del entries num */
            if (SOC_INFO(unit).block_valid[blk_idx]) {
                SOC_PBMP_COUNT(SOC_INFO(unit).block_bitmap[blk_idx], del_cnt);
                f->del_cnt += del_cnt;
            }
            ...
            if (!sal_strncasecmp(s, "1x100G", strlen(s)) ||
                !sal_strncasecmp(s, "1xHG[106]", strlen(s)) ||
                !sal_strncasecmp(s, "1xHG[100]", strlen(s))) {
                req_cnt = 1;
                ...
                pr[0].lanes = 4;
                pr[0].encap = encap;
            } else if(!sal_strncasecmp(s, "1x40G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[42]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[40]", strlen(s))) {
                req_cnt = 1;
                if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                    pr[0].port = pi[pm].ports[0];
                }
                ...
                pr[0].lanes = 4;
                pr[0].encap = encap;
            } else if(!sal_strncasecmp(s, "2x50G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[53]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[50]", strlen(s))) {
                req_cnt = 2;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
            ...
            } else if(!sal_strncasecmp(s, "2x40G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[42]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[40]", strlen(s))) {
                req_cnt = 2;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "2x20G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[21]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[20]", strlen(s))) {
                req_cnt = 2;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "1x40G+1x20G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[42]+1xHG[21]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[40]+1xHG[20]", strlen(s))) {
                req_cnt = 2;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "1x20G+1x40G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[21]+1xHG[42]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[20]+1xHG[40]", strlen(s))) {
                req_cnt = 2;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "1x50G+2x25G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[53]+2xHG[27]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[50]+2xHG[25]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "2x25G+1x50G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[27]+1xHG[53]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[25]+1xHG[50]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "1x40G+2x10G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[42]+2xHG[11]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[40]+2xHG[10]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "2x10G+1x40G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[11]+1xHG[42]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[10]+1xHG[40]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "1x20G+2x10G", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[21]+2xHG[11]", strlen(s)) ||
                      !sal_strncasecmp(s, "1xHG[20]+2xHG[10]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "2x10G+1x20G", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[11]+1xHG[21]", strlen(s)) ||
                      !sal_strncasecmp(s, "2xHG[10]+1xHG[20]", strlen(s))) {
                req_cnt = 3;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "4x25G", strlen(s)) ||
                      !sal_strncasecmp(s, "4xHG[27]", strlen(s)) ||
                      !sal_strncasecmp(s, "4xHG[25]", strlen(s))) {
                req_cnt = 4;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "4x10G", strlen(s)) ||
                      !sal_strncasecmp(s, "4xHG[11]", strlen(s)) ||
                      !sal_strncasecmp(s, "4xHG[10]", strlen(s))) {
                req_cnt = 4;
                for (i = 0; i < req_cnt; i++) {
                    if (pi[pm].flags & FLXP_F_PORT_ASSIGNED) {
                        pr[i].port = pi[pm].ports[i];
            ...
            } else if(!sal_strncasecmp(s, "None", strlen(s))) {
...
{
#ifdef BCM_TOMAHAWK2_SUPPORT
    bcm_pbmp_t pipe_pbmp[4]; /* Free-port pool for each pipe */
    int i, pm, port, blk_idx, blk_type, phy_port, pipe, cnt;
    int ffport[4]; /* First free port in each pipe */
    static const struct {
        int start_port;
        int end_port;
    } pbs[] = { /* General port boundary for each pipe */
        { 1, 32 },
        { 34, 65 },
        { 68, 99 },
        { 102, 133 }
    ...
    for (blk_idx = 0; blk_idx < SOC_MAX_NUM_BLKS; blk_idx++) {
        ...
        pipe = pm/16;
        if ((blk_type == SOC_BLK_CLPORT) && (pm < MAX_PM_NUM) &&
            (f->pm[pm].pm_str != NULL) && SOC_INFO(unit).block_valid[blk_idx]) {
            SOC_PBMP_OR(pipe_pbmp[pipe], SOC_INFO(unit).block_bitmap[blk_idx]);
        }
    }
        ...
        for (i = 0; i < f->pm[pm].port_cnt; i++ ) {
            ...
            for (port = ffport[pipe]; port <= pbs[pipe].end_port;
                ...
                if (SOC_PBMP_MEMBER(pipe_pbmp[pipe], port)) {
                    f->pm[pm].pr[i].port = port;
                    SOC_PBMP_PORT_REMOVE(pipe_pbmp[pm/16], port);
                    break;
                }
            }
...
{
    bcm_port_resource_t *pr;
    int blk_idx, blk_type, pm, port ;
    int nports, pr_idx, len;
