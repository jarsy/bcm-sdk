/*
 * $Id: switchctl.c,v 1.5 Broadcom SDK $
...
STATIC char *sbx_switch_control_names[] = {
    BCM_SWITCH_CONTROL_STR
};

char cmd_sbx_switch_control_usage[] =
    "\n\tSwitchControl [PortBitMap=<pbmp>] [<name> | <name>=<value>]\n";

cmd_result_t
cmd_sbx_switch_control(int unit, args_t *a)
{
    parse_table_t               pt;
    int                         r;
    bcm_pbmp_t                  arg_pbmp;
    ...
    if (COUNTOF(sbx_switch_control_names) != bcmSwitch__Count) {
    ...
    if ((spec = ARG_GET(a)) == NULL) {
        if (!arg_pbmp_given) {
            for (type = 0; type < bcmSwitch__Count; type++) {
                cli_out("%-40s", sbx_switch_control_names[type]);
        ...
        for (type = 0; type < bcmSwitch__Count; type++) {
            if (sal_strcasecmp(name, sbx_switch_control_names[type]) == 0) {
                break;
            }
        }
