                  ...
                  soc_pbmp_t pall)
{
    ...
    DPORT_SOC_PBMP_ITER(unit, pall, dport, tport) {        /* a subset of ports */
        ...
        if (SOC_PBMP_MEMBER(pbmp, tport)) {
            if (pfirst < 0) {
                pfirst = plast = (SOC_IS_DPP(unit))?dport:DPORT_FROM_DPORT_INDEX(unit, dport, port);
            } else {
                plast = (SOC_IS_DPP(unit))?dport:DPORT_FROM_DPORT_INDEX(unit, dport, port);
            }
            continue;
        }
        if (pfirst >= 0) {
...
parse_memory_name(int unit, soc_mem_t *result, char *str, int *copyno, unsigned *array_index)
{
...
#if defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (cp != NULL) {
#if defined(BCM_ROBO_SUPPORT) || defined(BCM_ESW_SUPPORT) || defined(BCM_CALADAN3_SUPPORT) || defined(BCM_SAND_SUPPORT) || defined(BCM_SIRIUS_SUPPORT)
    ...
    if (arr_ind != 0xffffffff) {
        ...
        if (!SOC_MEM_IS_ARRAY(unit, mem)) {
            ...
            cli_out("ERROR: array index %u is out of range, should be under %u\n",
...
parse_memory_index(int unit, soc_mem_t mem, char *val_str)
{
    ...
    if (!sal_strcasecmp(val_str, "min")) {
        ...
        if (!soc_robo_mem_index_valid(unit, mem, index)) {
            cli_out("WARNING: index %d out of range for memory %d\n",
                    index, (int)mem);
        }
#endif        
    } else {
...
bcm_field_olp_header_type_t parse_field_olp_hdr_type(char *str)
{
    ...
    for (olp_hdr_type = 0; olp_hdr_type < bcmFieldOlpHeaderTypeCount; olp_hdr_type++) {
        ...
        sal_strncat(lng_str, tbl_str,
                FP_OLP_HDR_TYPE_STR_SZ - 1 - sal_strlen("bcmFieldOlpHeaderType"));
...
parse_field_oam_type(char *str)
{
    ...
    for (oamtype = 0; oamtype < bcmFieldOamTypeCount; oamtype++) {
        ...
        sal_strncat(lng_str, tbl_str,
                FP_OAMTYPE_STR_SZ - 1 - sal_strlen("bcmFieldOamType"));
        if (!sal_strcasecmp(lng_str, str)) {
            break;
        }
    }
