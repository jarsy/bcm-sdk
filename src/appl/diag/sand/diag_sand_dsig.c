    ...
    char *filename)
{
    ...
    RHITERATOR(signal_output, dsig_list)
    {
        ...
        if (dsig_expand_print(signal_output->field_list, 1, prt_ctr, attr_offset, flags) !=
            _SHR_E_NONE)
        {
    ...
    args_t * a)
{
    ...
    switch (command)
    {
        case SIG_COMMAND_STAGE:
            res = dsig_stage_print(device, match_n, print_flags, filename);
            break;
        case SIG_COMMAND_STRUCT:
            res = dsig_struct_print(device, match_n, print_flags, filename);
            break;
        case SIG_COMMAND_PARAM:
            res = dsig_param_print(device, match_n, print_flags, filename);
            break;
        case SIG_COMMAND_DEBUG:
            ...
            if (prm_core == -1)
            {
                ...
                {
                    sand_signal_list_get(device, unit, prm_core, &match_m, dsig_list);
                }
            }
            else
            {
            ...
            res = dsig_print(dsig_list, print_flags, filename);
            sand_signal_list_free(dsig_list);
            break;
        default:
            break;
