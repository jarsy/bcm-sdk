...
static	char *getfn(void)
{
	...
	if(*inptr == NL) {
		nofname=TRUE;
		sal_strncpy(file, fname, sizeof(file) - 1);
	} else {
		nofname=FALSE;
		Skip_White_Space;
...
static	char *gettxt(int num)
{
	...
	sal_memset(txtbuf, 0, MAXLINE);
	lin = getptr(num);
	sal_strncpy(txtbuf,lin->l_buff, sal_strlen(lin->l_buff));
	sal_strncat(txtbuf, "\n", 1);
	return(txtbuf);
}
...
static	int ins(char *str)
{
	...
	for (;;) {
		...
		if((new = MALLOC(sizeof(LINE) + len)) == NULL) {
			return( ERR ); 	/* no memory */
		}

		new->l_stat=0;
		sal_strncpy(new->l_buff, buf, len);	/* build new line */
		new->l_buff[len] = EOS;
