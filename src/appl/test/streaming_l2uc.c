...
l2uc_lossless_flood_cnt(int unit, int port_idx, int hg_en)
{
    ...
    if (param_flood_cnt == 0) {
        if (param_pkt_size == 0) {
            pkt_size = (hg_en == 1) ? HG2_WC_PKT_SIZE : ENET_WC_PKT_SIZE;
        } else {
...
l2uc_parse_test_params(int unit, args_t *a)
{
    ...
    if (parse_arg_eq(a, &parse_table) < 0 || ARG_CNT(a) != 0) {
        ...
        cli_out("\npkt_size_param = %0d", l2uc_p->pkt_size_param);
        cli_out("\nrate_calc_interval_param = %0d",
                l2uc_p->rate_calc_interval_param);
        cli_out("\nflood_pkt_cnt_param = %0d", l2uc_p->flood_pkt_cnt_param);
        ...
        cli_out("\ncheck_packet_integrity_param = %0d",
                l2uc_p->check_packet_integrity_param);
        cli_out("\nmax_num_cells_param = %0d", l2uc_p->max_num_cells_param);
        cli_out("\n -----------------------------------------------------");
    }
