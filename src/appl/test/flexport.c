...
/* TSC Info */
#define TDMV_NUM_TSC 64
#define NUM_PORT_MODULES 65
...
char flexport_test_usage[] =
    ...
    "ServerName:   Name (Linux) or IP address (VxWorks) of the TSC clock server.\n"
...
/* COMMENTED OUT FOR MASTER BRANCH
...
*/
    ...
    if ((flexport_p->loopback_mode_param != BCM_PORT_LOOPBACK_MAC) &&
         ...
         */
        test_error(unit,"*ERROR: Loopback mode must be either 0, 1, or 2\n");
        flexport_p->bad_input = 1;
    }
...
#ifdef VXWORKS
    if (!isdigit((unsigned) serverName[0])) {
	cli_out("\nconnectToSocket : ERROR can't resolve host names in vxworks\n");
    }
...
*/
    ...
    for (l = (flexport_p->last_assigned_port[pipe] + 1); l <= lend; l++) {
...
flex_tsc(int unit, int num_subport, int first_phy_port, int *speed, int encap)
{
    ...
    for (i = 0; i < num_subport; i++) {
        if (speed[i] > 0 && (speed[i] != cur_speed[i] || encap != cur_encap)) {
            ...
            flexport_p->add_resource[flexport_p->num_add].speed =
...
flexport_call_bcm_api(int unit)
{
    ...
    if (flexport_p->emulation_param && flexport_p->tsc_cfg_cmds) {
        for (i = 0; i < 12; i++) {
            if (flexport_p->tsc_cfg_buffer[i][0] != '\0') {
                if (BCM_E_NONE != sendDataToSocket(flexport_p->sockfd,
                                      flexport_p->tsc_cfg_buffer[i])) {
...
/* COMMENTED OUT FOR MASTER BRANCH
                if (SOC_IS_TOMAHAWK2(unit) || SOC_IS_TRIDENT3(unit)) {
*/
                if (SOC_IS_TOMAHAWK2(unit)) {
                    switch (port_speeds[port_phyID]) {
                        case SPEED_UI_100G     :
...
flexport_by_tsc(int unit, uint32 tsc, char *mode)
{
    ...
    if (sal_strstr(mode, "1x100G") || sal_strstr(mode, "1x106G")) {
        ...
        num_subport = 12;
               ...
               (sal_strstr(mode, "50G+2x25G"))) {
        speed[0] = 50000; speed[2] = 25000; speed[3] = 25000;
    } else if (SOC_IS_TOMAHAWK2(unit) &&
               (sal_strstr(mode, "2x25G+50G"))) {
        ...
        speed[0] = 10000;
/* COMMENTED OUT FOR MASTER BRANCH
    } else if ((SOC_IS_TOMAHAWK2(unit) || SOC_IS_TRIDENT3(unit)) &&
*/
    } else if ((SOC_IS_TOMAHAWK2(unit)) &&
               (sal_strstr(mode, "2x50G") || sal_strstr(mode, "2x53G"))) {
        speed[0] = 50000; speed[2] = 50000;
    } else if (SOC_IS_TOMAHAWK2(unit) &&
               (sal_strstr(mode, "2x40G") || sal_strstr(mode, "2x42G"))) {
        speed[0] = 40000; speed[2] = 40000;
    } else if (SOC_IS_TOMAHAWK2(unit) && sal_strstr(mode, "40G+2x10G")) {
        speed[0] = 40000; speed[2] = 10000; speed[3] = 10000;
    } else if (SOC_IS_TOMAHAWK2(unit) && sal_strstr(mode, "2x10G+40G")) {
        speed[0] = 10000; speed[1] = 10000; speed[2] = 40000;
    } else if (SOC_IS_TOMAHAWK2(unit) && sal_strstr(mode, "40G+20G")) {
        speed[0] = 40000; speed[2] = 20000;
    } else if (SOC_IS_TOMAHAWK2(unit) && sal_strstr(mode, "20G+40G")) {
                         ...
                         int max_num_cells_param)
{
  ...
  stream_rate_calc_p->pkt_size          = flexport_p->pkt_size_param;
...
flexport_test(int unit, args_t *a, void *pa)
{
    ...
    if (flexport_p->emulation_param) {
        for(tsc = 0; tsc < TDMV_NUM_TSC; tsc++) {
            if(flexport_p->flex_tsc_map[tsc] == 1) {
                ...
                cli_out("\nFlex TSC SHIM: %0d %0d %0d %0d : %0d", speed[0], speed[1], speed[2], speed[3], encap);
