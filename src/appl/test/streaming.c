 ...
 * back to itself using port bridging and higig lookup for HG ports. Test also
...
static int
streaming_dma_chan_check_done(int unit, int vchan, soc_dma_poll_type_t type,
                              int *detected)
{
    int rv = SOC_E_NONE;
...
send_pkts(int unit)
{
    ...
    for (i = 0; i < streaming_p->num_fp_ports; i++) {
        ...
        for (j = 0; j < flood_cnt; j++) {
            ...
            rv = soc_dma_chan_config(unit, TX_CHAN, DV_TX, SOC_DMA_F_POLL);
            if (SOC_FAILURE(rv)) {
            ...
            while (channel_done == 0) {
                streaming_dma_chan_check_done(unit, TX_CHAN,
...
check_rates(int unit)
{
    ...
    for (i = 0; i < streaming_p->num_fp_ports; i++) {
        if (!(COMPILER_64_IS_ZERO(streaming_p->exp_rate[i]))) {
            ...
            format_uint64_decimal(avg_rate_str, streaming_p->rate[i], 0);
...
check_packet_integrity(int unit)
{
    ...
    for (i = 0; i < streaming_p->num_fp_ports; i++) {
        ...
        for (j = 0; j < flood_cnt; j++) {
            ...
            rv = soc_dma_chan_config(unit, RX_CHAN, DV_RX, SOC_DMA_F_POLL);
            if (SOC_FAILURE(rv)) {
                test_error(unit, "\nError calling soc_dma_chan_config");
                streaming_p->test_fail = 1;
            }
            ...
            while (channel_done == 0 && timeout > 0) {
                streaming_dma_chan_check_done(unit, RX_CHAN,
                                              SOC_DMA_POLL_CHAIN_DONE,
                                              &channel_done);
                timeout--;
            }
...
streaming_test_cleanup(int unit, void *pa)
{
    ...
    sal_free(streaming_p->port_list);
    ...
    sal_free(streaming_p->tbyt_start);
    sal_free(streaming_p->tbyt_end);
    for (i = 0; i < streaming_p->num_fp_ports; i++) {
        sal_free(streaming_p->rand_pkt_sizes[i]);
    }
    sal_free(streaming_p->rand_pkt_sizes);
