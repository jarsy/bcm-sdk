...
;        > 7.   switch 2
         ...
         > 1.   mov tsr_Unload, 0
         > 1.   mov t0, 0x3fff003f
         > 1.   hstorep  h(DESC__BUFFERandSRCQ_OFFSET):4, t0
         > 0.   nop
         > 0.   nop
         > 0.   nop
         > 0.   nop
         ...
         > 0.   nop
         > 0.   nop
STREAM11_GET_NEXT_LIST_ENTRY:
         > 1.   load LearningList_RingPointer, p0.205( DEQUEUE )
         > 0.   nop
         > 0.   nop
...
STREAM11__s1:
         ...
         < 2.   mov P2_EntryIndex, rnr[dLRP_PORT_ACCESS_BIT_RANGE]
         < 2.   mov P1_EntryIndex, rnr[dLRP_PORT_ACCESS_BIT_RANGE]
         > 1.   jz  STREAM11__THE_END, LearningList_RingPointer
         > 1.   add LearningList_Offset, LEARNING_LIST_BASE, LearningList_RingPointer
         ...
         < 2.   load P1_ActualResults, p1.6( P1_EntryIndex )
STREAM11_GET_CREDIT:
         > 1.   load TMU_InsertCredit, p0.203( 0 )
         < 2.   movifnz tsr_F9Flag, 1, rnr[dDREQ_PROBABILITY]
         < 2.   load P8_ActualResults, p8.6( P8_EntryIndex )
...
STREAM11__NO_TSR_ERROR_s1:
         ...
         < 2.   mov IPV6_SK0_ExpectResults_2[19:8], IPV6_SubKey0_139to128
         < 2.   mov IPV6_SK0_ExpectResults_3[19:8], IPV6_SubKey0_139to128
; > 1.   movifz dcr, 1, TMU_InsertCredit
         > 0.   nop
         > 1.   jz  [1].STREAM11_GET_CREDIT, TMU_InsertCredit 
         > 0.   nop
  ...
  < (F9) ? 2.   jne STREAM11__DM_MISCOMPARE_s1, DM_ExpectResults_15, DM_ActualResults_15
         ...
         > 0.   nop
         > 0.   nop
         > 1.   load COP_LearningListEntry, cop.32( LearningList_Offset + DontCare + COHTAB_READ )
         > 0.   nop
         < 0.   nop
         < 0.   nop
         < 0.   nop
...
STREAM11__s2:
         > 1.   mov Key_63_0[31:0],     0x00000054
         > 1.   mov Key_63_0[63:32],    0x00000000
         > 1.   mov Key_127_64[31:0],   0x80000000
         > 1.   mov Key_127_64[63:32],  0x00000088
         > 1.   mov Key_191_128[31:0],  0x80000111
         > 1.   mov Key_191_128[63:32], 0x80000199
         > 1.   mov Key_255_192[31:0],  0x00000003
         > 1.   mov Key_63_0[26:11], COP_LearningListEntry_Key  
         > 1.   mov Key_127_64[26:11], COP_LearningListEntry_Key 
         > 1.   mov Key_127_64[58:43], COP_LearningListEntry_Key
         > 1.   mov Key_191_128[26:11], COP_LearningListEntry_Key
         > 1.   mov Key_191_128[58:43], COP_LearningListEntry_Key
         > 1.   key 14.3, Key_639_576
         > 1.   key 14.2, Key_575_512, Key_511_448, Key_447_384
         > 1.   key 14.1, Key_383_320, Key_319_256, Key_255_192
         > 1.   key 14.0, Key_191_128, Key_127_64,  Key_63_0

         > 1.   mov ErrorMessage, COP_LearningListEntry_Key
         > 7.   switch
   > (!E)? 1.   j STREAM11__NO_TSR_ERROR_s2
   >  (E)? 1.   mov tsr_ErrorFlag, 0
         > 1.   or ErrorMessage, TSR_ERROR_FLAG, ErrorMessage
         > 1.   j  STREAM11__TSR_ERROR_s2
STREAM11__NO_TSR_ERROR_s2:
         > 0.   nop
         > 1.   mov Key_63_0[31:0],     0x00000000
         > 1.   mov Key_63_0[63:32],    0x00000000
         > 1.   mov Key_127_64[31:0],   0x00000000
         > 1.   mov Key_127_64[63:32],  0x00000000
         > 1.   mov Key_191_128[31:0],  0x00000000
         > 1.   mov Key_191_128[63:32], 0x00000009
         > 1.   mov Key_255_192[31:0],  0x00000000
         > 0.   nop
         > 1.   j  [1].STREAM11_GET_NEXT_LIST_ENTRY
         ...
         > 0.   nop
         > 0.   nop
STREAM11__TSR_ERROR_s2:
         > 1.   mov dcr, 1
         > 1.   load BitBucket0, p3.128(ErrorMessage)
         > 0.   nop
         > 0.   nop

; > 1.   load BitBucket0, p3.128( COP_LearningListEntry[31:0] ) 
STREAM11__THE_END:
         > 1.   mov tsr_Unload, 1
         > 1.   mov tsr_SrcType, 3
         > 0.   nop
