/*
 * $Id: ccmdma_test.c,v 1.0 Broadcom SDK $
...
#if defined(BCM_ESW_SUPPORT) && defined(BCM_CCMDMA_SUPPORT) && defined(BCM_CMICM_SUPPORT)
...
clear_internal_memory(int unit, unsigned int start_addr, unsigned int num_words)
{
    ...
    for (i = 0; i < num_words; i++)  {
        soc_pci_mcs_write(unit, (start_addr + sizeof(unsigned int) * i), 0x0);
    }
}
          ...
          volatile unsigned int* transfer_count)
{
    ...
    do {
            rv = soc_ccmdma_copy(unit, host0_mem_start_addr, host1_mem_start_addr,
...
#ifdef PTRS_ARE_64BITS
                    soc_pci_mcs_write(unit, (uint64)cd_test_data.host0_base[cmc] +
                                      sizeof(unsigned int) * i, sal_rand());
#else
                    soc_pci_mcs_write(unit, (uint32)cd_test_data.host0_base[cmc] +
                                      sizeof(unsigned int) * i, sal_rand());
#endif
                }
            }
...
#ifdef PTRS_ARE_64BITS
                    src_word = soc_pci_mcs_read(unit,
...
#ifdef PTRS_ARE_64BITS
                    dest_word = soc_pci_mcs_read(unit,
