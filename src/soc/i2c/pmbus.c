...
pmbus_init(int unit, int devno, void* data, int len)
{
    ...
    SOC_IF_ERROR_RETURN
        (pmbus_write(unit, devno, PMBUS_CMD_CLEAR_FAULTS, &data8, 0));

    /* enable operation commands over the PMBus */
    datalen = 1;
    SOC_IF_ERROR_RETURN
        (pmbus_read(unit, devno, PMBUS_CMD_ON_OFF_CONFIG, &data8,
                    &datalen));
    data8 |= 0xf;
    SOC_IF_ERROR_RETURN
        (pmbus_write(unit, devno, PMBUS_CMD_ON_OFF_CONFIG, &data8, 1));

    /* get MFR ID and MFR MODEL */
    SOC_IF_ERROR_RETURN
        (pmbus_read(unit, devno, PMBUS_CMD_MFR_ID, &mfr_id, &datalen));

    datalen = 2;
    SOC_IF_ERROR_RETURN
        (pmbus_read(unit, devno, PMBUS_CMD_MFR_MODEL, (uint8*)&mfr_model,
                    &datalen));
    ...
    if((mfr_id == 0x1) && (mfr_model == 0x2)) {
        ...
        SOC_IF_ERROR_RETURN
            (pmbus_write(unit, devno, PMBUS_CMD_VOUT_COMMAND, &data8, 1));
        if (soc_property_get(unit, "voltage_control", 1)) {
            datalen = 1;
            SOC_IF_ERROR_RETURN
               (pmbus_read(unit, devno, I2C_ADP4000_CMD_CONFIG_1A, &data8, &datalen));
            data8 |= 1 << 3;
            SOC_IF_ERROR_RETURN
               (pmbus_write(unit, devno, I2C_ADP4000_CMD_CONFIG_1A, &data8, 1));
            datalen = 1;
            SOC_IF_ERROR_RETURN
               (pmbus_read(unit, devno, I2C_ADP4000_CMD_CONFIG_1B, &data8, &datalen));
            data8 |= 1 << 3;
            SOC_IF_ERROR_RETURN
               (pmbus_write(unit, devno, I2C_ADP4000_CMD_CONFIG_1B, &data8, 1));
        } else {
            datalen = 1;
            SOC_IF_ERROR_RETURN
               (pmbus_read(unit, devno, I2C_ADP4000_CMD_CONFIG_1A, &data8, &datalen));
        ...
        SOC_IF_ERROR_RETURN
            (pmbus_ioctl(unit, devno, PMBUS_IOC_SET_DAC, &ncp4200_dac_params, 1));
    } else if ((mfr_id == 0x1A) && (mfr_model == 0x1233)) {
        /* NCP81233 device */
        SOC_IF_ERROR_RETURN
            (pmbus_ioctl(unit, devno, PMBUS_IOC_SET_DAC, &ncp81233_dac_params, 1));
    }
