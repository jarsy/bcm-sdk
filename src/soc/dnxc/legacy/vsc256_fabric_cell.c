...
/* the offset should be 0 in fe1600 and 512 in jer2_arad */ 
{
  ...
  if(DNXC_VSC256_INBAND_PAYLOAD_CELL_FORMAT_START + offset + DNXC_VSC256_INBAND_PAYLOAD_CELL_FORMAT_LENGTH > 8*buf_size_bytes) {
    DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Buffer size is not large enough")));
  }
  /*cell format*/
  ...
  for(i=0 ; i<cell->payload.inband.nof_commands ; i++) {
    ...
    switch(cell->payload.inband.commands[i].opcode) {
            ...
            default: DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("unsupported opcode"))); 
    }
...
soc_dnxc_vsc256_build_header(int unit, const dnxc_vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf) 
{
  ...
  if(buf_size_bytes < DNXC_VSC256_SR_DATA_CELL_HEADER_SIZE) {
    DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("SR header minimum buffer size is %d"),DNXC_VSC256_SR_DATA_CELL_HEADER_SIZE));
  }
...
soc_error_t soc_dnxc_vsc256_parse_header(int unit, soc_reg_above_64_val_t reg_val, dnxc_vsc256_sr_cell_t* cell)
{
  ...
  */
   ...
   * bits  - source id
   */
  SHR_BITCOPY_RANGE(&cell->header.source_device, 0, reg_val, DNXC_VSC256_SR_DATA_CELL_SOURCE_ID_START, DNXC_VSC256_SR_DATA_CELL_SOURCE_ID_LENGTH);
  ...
  DNXC_IF_ERR_EXIT(soc_dnxc_device_entity_value(unit, tmp_actual_value, &cell->header.source_level));
  ...
  DNXC_IF_ERR_EXIT(soc_dnxc_device_entity_value(unit, tmp_actual_value, &cell->header.destination_level));
...
/*
 * Function: 
 *      soc_dnxc_vsc256_to_vsc128_build_header 
 * Role: 
 *      This function get dnxc_vsc256_sr_cell_t as an input and creats a buffer with the equivalent VSC128 header. 
 * Remarks: 
 *      Implemented and tested just for Source routed cell. Should be tuned for inband cells too. 
 */
soc_error_t 
soc_dnxc_vsc256_to_vsc128_build_header(int unit, const dnxc_vsc256_sr_cell_t* cell, int buf_size_bytes, uint32* buf) 
{
  soc_dnxc_device_type_actual_value_t tmp_actual_value;
  uint32 tmp_u32_data_cell_field;
  DNXC_INIT_FUNC_DEFS;
     ...
     */
    tmp_u32_data_cell_field = cell->header.cell_type;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_CELL_TYPE_START, &(tmp_u32_data_cell_field), 0, DNXC_VSC256_TO_VSC128_DATA_CELL_CELL_TYPE_LENGTH);  

    /*
     * Copy source id
     */
    tmp_u32_data_cell_field = cell->header.source_device;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_SOURCE_ID_START, &(tmp_u32_data_cell_field), 0, DNXC_VSC256_TO_VSC128_DATA_CELL_SOURCE_ID_LENGTH);  
    ...
    DNXC_IF_ERR_EXIT(soc_dnxc_actual_entity_value(unit,cell->header.source_level, &tmp_actual_value));
    tmp_u32_data_cell_field = (uint32)tmp_actual_value;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_SRC_LEVEL_START, &(tmp_u32_data_cell_field), 0, DNXC_VSC256_TO_VSC128_DATA_CELL_SRC_LEVEL_LENGTH);
    ...
    DNXC_IF_ERR_EXIT(soc_dnxc_actual_entity_value(unit, cell->header.destination_level, &tmp_actual_value));
    tmp_u32_data_cell_field = (uint32)tmp_actual_value;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_DEST_LEVEL_START, &(tmp_u32_data_cell_field), 0, DNXC_VSC256_TO_VSC128_DATA_CELL_DEST_LEVEL_LENGTH);

    /*
     * Copy fip switch
     */
    tmp_u32_data_cell_field = cell->header.fip_link;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_FIP_SWITCH_START, &(tmp_u32_data_cell_field), 0,  DNXC_VSC256_TO_VSC128_DATA_CELL_FIP_SWITCH_LENGTH);
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FIP_SWITCH_POSITION, &(tmp_u32_data_cell_field), DNXC_VSC256_TO_VSC128_DATA_CELL_FIP_SWITCH_LENGTH,  DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FIP_SWITCH_LENGTH);

    /*
     * Copy fe1 switch
     */
    tmp_u32_data_cell_field = cell->header.fe1_link;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_FE1_SWITCH_START, &(tmp_u32_data_cell_field), 0,  DNXC_VSC256_TO_VSC128_DATA_CELL_FE1_SWITCH_LENGTH);
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE1_SWITCH_POSITION, &(tmp_u32_data_cell_field), DNXC_VSC256_TO_VSC128_DATA_CELL_FE1_SWITCH_LENGTH,  DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE1_SWITCH_LENGTH);

    /*
     * Copy fe2 switch
     */
    tmp_u32_data_cell_field = cell->header.fe2_link;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_FE2_SWITCH_START, &(tmp_u32_data_cell_field), 0,  DNXC_VSC256_TO_VSC128_DATA_CELL_FE2_SWITCH_LENGTH);
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE2_SWITCH_POSITION, &(tmp_u32_data_cell_field), DNXC_VSC256_TO_VSC128_DATA_CELL_FE2_SWITCH_LENGTH,  DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE2_SWITCH_LENGTH);

    /*
     * Copy fe3 switch
     */
    tmp_u32_data_cell_field = cell->header.fe3_link;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_DATA_CELL_FE3_SWITCH_START, &(tmp_u32_data_cell_field), 0,  DNXC_VSC256_TO_VSC128_DATA_CELL_FE3_SWITCH_LENGTH);
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE3_SWITCH_POSITION, &(tmp_u32_data_cell_field), DNXC_VSC256_TO_VSC128_DATA_CELL_FE3_SWITCH_LENGTH,  DNXC_VSC256_TO_VSC128_SR_DATA_CELL_FE3_SWITCH_LENGTH);

    /*
    * Copy In-band cell
    */
    tmp_u32_data_cell_field = cell->header.is_inband;
    SHR_BITCOPY_RANGE(buf, DNXC_VSC256_TO_VSC128_SR_DATA_CELL_INBAND_CELL_POSITION, &(tmp_u32_data_cell_field), 0,  DNXC_VSC256_TO_VSC128_SR_DATA_CELL_INBAND_CELL_LENGTH);
exit:    
  DNXC_FUNC_RETURN;  
}
