...
soc_dnxc_ser_init_cb(int unit, soc_regaddrinfo_t *ainfo, void *data)
{
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        if ((reg == OAMP_ECC_ERR_1B_MONITOR_MEM_MASKr) && SOC_IS_QAX(unit)) {
            soc_reg_above_64_field32_set(unit, reg, above_64, FLEX_VER_MASK_TEMP_ECC_1B_ERR_MASKf, 0);
        }
        if ((reg == OAMP_ECC_ERR_2B_MONITOR_MEM_MASKr) && SOC_IS_QAX(unit)) {
...
soc_dnxc_ser_init(int unit)
{
    ...
    DNXC_INIT_FUNC_DEFS;
    DNXC_IF_ERR_EXIT(soc_reg_iterate(unit, soc_dnxc_ser_init_cb, NULL));
