...
int soc_dnxc_cmic_device_hard_reset(int unit, int reset_action)
{
    ...
    if ((reset_action == SOC_DNXC_RESET_ACTION_IN_RESET) || (reset_action == SOC_DNXC_RESET_ACTION_INOUT_RESET)) {
        DNXC_IF_ERR_EXIT(WRITE_CMIC_CPS_RESETr(unit, 0x1)); 
        ...
        if (SOC_IS_DNX(unit) && SOC_DNX_CONFIG(unit)->emulation_system != 0 ) {
            usleep*=1000;
        }
#endif /* BCM_DNX_SUPPORT */
#if defined(BCM_DNX_SUPPORT)
        ...
        for (;;) {
            DNXC_IF_ERR_EXIT(READ_CMIC_CPS_RESETr(unit, &reg32_val));
            if (reg32_val == 0x0) {
                break;
            }
            if (soc_timeout_check(&to)) {
                DNXC_EXIT_WITH_ERR(_SHR_E_INIT, (_BSL_SOC_MSG("Error: CPS reset field not asserted correctly.")));
                break;
            }
        }   
    }
...
int
soc_dnxc_cmic_sbus_timeout_set(int unit, uint32 core_freq_khz, int schan_timeout)
{
...
int soc_dnxc_cmic_pcie_userif_purge_ctrl_init(int unit)
{
    uint32 reg;
    ...
    if (soc_feature(unit, soc_feature_cmicm) && 
        ...
        DNXC_IF_ERR_EXIT(READ_CMIC_PCIE_USERIF_PURGE_CONTROLr(unit, &reg));
        ...
        DNXC_IF_ERR_EXIT(WRITE_CMIC_PCIE_USERIF_PURGE_CONTROLr(unit, reg));
    }
...
int soc_dnxc_cmic_mdio_config(int unit, int dividend, int divisor, int delay)
{
    ...
    DNXC_IF_ERR_EXIT(READ_CMIC_MIIM_CONFIGr(unit, &rval));
...
int soc_dnxc_cmic_mdio_set(int unit)
{
    ...
    DNXC_IF_ERR_EXIT(WRITE_CMIC_RATE_ADJUST_INT_MDIOr(unit, reg));
    ...
    DNXC_IF_ERR_EXIT(WRITE_CMIC_RATE_ADJUST_EXT_MDIOr(unit, reg));

    /* Delay */
    reg = 0;
    mdio_delay = 0xf;
    soc_reg_field_set(unit, CMIC_MIIM_CONFIGr, &reg, MDIO_OUT_DELAYf, mdio_delay);
    DNXC_IF_ERR_EXIT(WRITE_CMIC_MIIM_CONFIGr(unit, reg));

    /* Set MDIO Master */
    DNXC_IF_ERR_EXIT(READ_CMIC_MIIM_SCAN_CTRLr(unit, &reg));
    soc_reg_field_set(unit, CMIC_MIIM_SCAN_CTRLr, &reg, OVER_RIDE_EXT_MDIO_MSTR_CNTRLf, 0x1);
    DNXC_IF_ERR_EXIT(WRITE_CMIC_MIIM_SCAN_CTRLr(unit, reg));
