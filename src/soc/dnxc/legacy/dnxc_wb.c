...
#if (defined(BCM_DNXF_SUPPORT) || defined(BCM_DNX_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)
...
int    _soc_dnxc_wb_warmboot_test_mode[SOC_MAX_NUM_DEVICES];
/* temporary disabling WB test mode */
int    _soc_dnxc_wb_override_wb_test[SOC_MAX_NUM_DEVICES];
/* temporary disabling WB test mode due to temporarily field unstability */
int    _soc_dnxc_wb_field_test_mode_stop[SOC_MAX_NUM_DEVICES][_dnxcWbFieldFlagNOF];
/* disabling WB test mode for one BCM API call */
int    _soc_dnxc_wb_disable_once_wb_test[SOC_MAX_NUM_DEVICES];
/* counter, counting the number of warmboot test performed */
int    _soc_dnxc_wb_warmboot_test_counter[SOC_MAX_NUM_DEVICES];
/* counter, counting the number of APIs in current stack */
int    _soc_dnxc_wb_nested_api_counter[SOC_MAX_NUM_DEVICES];
 ...
 */
int dnxc_wb_cr_all_reset_test_utils_preconditions (int unit) {
    ...
    if (!dnxc_wb_api_counter_is_top_level(unit)) {
        return 0;
    }
 ...
 */
int dnxc_wb_cr_common_api_preconditions (int unit) {
    int no_wb_test;
    int field_wb_stop;
    int disable_once;
    ...
    soc_dnxc_wb_field_test_mode_get(unit, _dnxcWbFieldFlagDirectExtraction, &field_wb_stop);
    if (field_wb_stop) {
        return 0;
    }
    soc_dnxc_wb_field_test_mode_get(unit, _dnxcWbFieldFlagDataQual, &field_wb_stop);
    if (field_wb_stop) {
        return 0;
    }

    /* check if we were signaled not to perform any resets */
    soc_dnxc_wb_no_wb_test_get(unit, &no_wb_test);
    if (no_wb_test) {
        return 0;
    }

    /* check if we were signaled not to perform any resets */
    soc_dnxc_wb_disable_once_get(unit, &disable_once);
    if (disable_once) {
        return 0;
    }
...
void dnxc_bcm_warm_boot_api_test_enable_if_disabled_once(int unit) {
    int disable_once;

    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));

    soc_dnxc_wb_disable_once_get(unit, &disable_once);
    if (1 == disable_once) {
        soc_dnxc_wb_disable_once_set(unit, 0);
    }
    return;
}

/*!
 * \brief dnxc_bcm_warm_boot_api_test_reset
 *
...
int dnxc_bcm_warm_boot_api_test_reset(int unit) {
    ...
    if (_DNXC_BCM_WARM_BOOT_API_TEST_MODE_AFTER_EVERY_API != warmboot_test_mode_enable)
    {
        return 0;
    }
...
void
soc_dnxc_wb_test_mode_set(int unit, int enable)
{
    _soc_dnxc_wb_warmboot_test_mode[unit] = enable;
}

void
soc_dnxc_wb_test_mode_get(int unit, int *enable)
{
    *enable =  _soc_dnxc_wb_warmboot_test_mode[unit];
}

void
soc_dnxc_wb_test_counter_set(int unit, int counter)
{
    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));
    _soc_dnxc_wb_warmboot_test_counter[unit] = counter;
}

void
soc_dnxc_wb_test_counter_get(int unit, int *counter)
{
    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));
    *counter =  _soc_dnxc_wb_warmboot_test_counter[unit];
}

void
soc_dnxc_wb_test_counter_plus_plus(int unit)
{
    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));
    _soc_dnxc_wb_warmboot_test_counter[unit]++;
}

void
soc_dnxc_wb_test_counter_reset(int unit)
{
    _soc_dnxc_wb_warmboot_test_counter[unit] = 0;
}
...
void
soc_dnxc_wb_no_wb_test_set(int unit, int wb_flag)
{
    if (wb_flag == 0)
    {
        _soc_dnxc_wb_override_wb_test[unit]--;
    }
    else
    {
        _soc_dnxc_wb_override_wb_test[unit]++;
...
void
soc_dnxc_wb_no_wb_test_get(int unit, int *wb_flag)
{
    *wb_flag =  _soc_dnxc_wb_override_wb_test[unit];
}


void soc_dnxc_wb_field_test_mode_set(int unit, _dnxc_wb_field_falgs_e type, int wb_flag)
{
    /* only verify it's not called from other thread */
    assert(sal_thread_self() == sal_thread_main_get());

    _soc_dnxc_wb_field_test_mode_stop[unit][type] = wb_flag;
}  
void soc_dnxc_wb_field_test_mode_get(int unit, _dnxc_wb_field_falgs_e type, int *wb_flag)
{
    /* only verify it's not called from other thread */
    assert(sal_thread_self() == sal_thread_main_get());

    *wb_flag =  _soc_dnxc_wb_field_test_mode_stop[unit][type];
} 
...
void
soc_dnxc_wb_disable_once_set(int unit, int wb_flag)
{
    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));
    _soc_dnxc_wb_disable_once_wb_test[unit] = wb_flag;
}

void
soc_dnxc_wb_disable_once_get(int unit, int *wb_flag)
{
    assert(dnxc_wb_cr_all_reset_test_utils_preconditions(unit));
    *wb_flag = _soc_dnxc_wb_disable_once_wb_test[unit];
}

int dnxc_bcm_warm_boot_api_test_mode_skip_wb_sequence(int unit) {
    int rv = BCM_E_NONE;
    if (dnxc_wb_cr_all_reset_test_utils_preconditions(unit)) {
        soc_dnxc_wb_disable_once_set(unit, 1);
    }
    return rv;
}
...
int dnxc_wb_exp_backoff(int test_counter, int *nof_skip) {
    int i = DNXC_WARM_EXP_PARAM;
    int skip = 0;

    while (i < test_counter) {
        i *= DNXC_WARM_EXP_PARAM;
    }
    if (test_counter % (i/DNXC_WARM_EXP_PARAM) == 0 || test_counter == 0) {
      return 1;
    }
    skip = i / DNXC_WARM_EXP_PARAM - test_counter % (i/DNXC_WARM_EXP_PARAM);
    if (nof_skip != NULL && skip + 1 == i/DNXC_WARM_EXP_PARAM) *nof_skip = skip;
    else if (nof_skip != NULL) *nof_skip = 0;
    return 0;
}

int dnxc_wb_api_counter_increase(int unit)
{
    if (!(SOC_UNIT_NUM_VALID(unit) &&
    ...
    _soc_dnxc_wb_nested_api_counter[unit]++;

    assert(_soc_dnxc_wb_nested_api_counter[unit] >= 0);
    assert(_soc_dnxc_wb_nested_api_counter[unit] < 3);

    return 0;
}

int dnxc_wb_api_counter_decrease(int unit)
{
    if (!(SOC_UNIT_NUM_VALID(unit) &&
          SOC_IS_DONE_INIT(unit)   &&
          (!SOC_WARM_BOOT(unit))   && 
...
int dnxc_wb_api_counter_get(int unit)
{
    if (!(SOC_UNIT_NUM_VALID(unit) &&
         SOC_IS_DONE_INIT(unit)   &&
         (!SOC_WARM_BOOT(unit))   && 
    ...
    return _soc_dnxc_wb_nested_api_counter[unit];
}

int dnxc_wb_api_counter_reset(int unit)
{
    if (!(SOC_UNIT_NUM_VALID(unit) &&
         SOC_IS_DONE_INIT(unit)   &&
         (!SOC_WARM_BOOT(unit))   && 
    ...
    _soc_dnxc_wb_nested_api_counter[unit] = 0;

    return 0;
}

int dnxc_wb_api_counter_is_top_level(int unit)
{
    /* we don't expect counter to be zero - should always be at least inside one API */
    assert(dnxc_wb_api_counter_get(unit));

    /* if used not in crash recovery context */
    if (!SOC_CR_ENABALED(unit)) {
        return (dnxc_wb_api_counter_get(unit) == 1);
    }
    else {
        return ((!soc_dnxc_cr_is_journaling_per_api(unit)) || /* always return true if not in API commit mode */
                (dnxc_wb_api_counter_get(unit) == 1));
    }
}

#endif /* BCM_WARM_BOOT_API_TEST */
#endif /*(defined(BCM_DNXF_SUPPORT) || defined(BCM_DNX_SUPPORT)) && defined(BCM_WARM_BOOT_SUPPORT)*/
