/*
 * $Id: nlmrangemgr.c,v 1.1.6.2 Broadcom SDK $
                                        ...
                                        )
{
     ...
     */
    newNode = (NlmRangeNode*)NlmCmAllocator__malloc(alloc_p, sizeof(NlmRangeNode));
                               ...
                               )
{
     ...
     */
    newNode = (NlmRangeDbNode*)NlmCmAllocator__calloc(alloc_p, 1, sizeof(NlmRangeDbNode));
    NlmCmDemand((newNode != NULL), "Out of memory.\n");
    ...
    )
{
    ...
    encodedHead = (NlmRangeEncoded*)NlmCmAllocator__calloc(alloc_p, 1, sizeof(NlmRangeEncoded));
    ...
    encodedHead->m_entries_p = (NlmRangeDMFormat*)NlmCmAllocator__calloc(alloc_p,
                                                      encodingsCount,
    ...
    )
{
    ...
    newRange = (NlmRange*)NlmCmAllocator__malloc(self->m_alloc_p, sizeof(NlmRange));
        ...
        while(node)
        {
            ...
            if (rangeDb->m_use_mcor && ((ret_val = NlmRangeMgr__pvt_IsTopRange(rangeDb, node)) >= 0))
            {
                ...
                encoded_head->m_entries_p =
                    (NlmRangeDMFormat*)NlmCmAllocator__calloc(self->m_alloc_p, 1,  sizeof(NlmRangeDMFormat));
    ...
    )
{
    ...
    {
        if( (self->m_bankNum == NLMDEV_BANK_0 && ltrNum >= NLM_MAX_NUM_LTR_PER_SMT) ||
            (self->m_bankNum == NLMDEV_BANK_1 && ltrNum < NLM_MAX_NUM_LTR_PER_SMT) )
        {
            if(o_reason)
                *o_reason = NLMRSC_INVALID_RANGE_ATTRIBUTES;
            return NLMERR_NULL_PTR;
    ...
    )
{
    ...
    newDb = (NlmRangeDb*)NlmCmAllocator__calloc(self->m_alloc_p, 1, sizeof(NlmRangeDb));
    ...
    )
{
    ...
    for(iter = 0; iter < NLM_RANGE_HT_DEPTH; iter++)
    {
        ...
        while(rangeNode_p)
        {
            ...
            if(NLMERR_OK != (catchErr = NlmRangeMgr__pvt_DeleteRange
                (
                self,
                rangeDb,
...
{
    return kbp_rm_add_range( self, rangeDb, range, o_reason );
}
...
{
    return kbp_rm_delete_range( self, rangeDb, id, o_reason );
}
...
{
    return kbp_rm_assign_range( self, rangeDb, rangeType, o_reason );
}
...
{
    return kbp_rm_create_encodings( self, rangeDb, o_reason );
}
...
{
    return kbp_rm_get_range_encoding( self, rangeDb, id, o_reason );
}
...
{
    return kbp_rm_config_range_matching( self, ltrNum, srchAttrs, o_reason );
}
...
{
    return kbp_rm_get_db_stats( self, rangeDb, o_reason );
}
...
{
    return kbp_rm_create_db( self, id, dbAttributes, o_reason );
}
...
{
    return kbp_rm_destroy_db( self, rangeDb, o_reason );
}
...
{
    return kbp_rm_init( alloc_p, devMgr, devType, bankNum, portNum, encodingType, o_reason );
}
...
{
    return kbp_rm_destroy( self, o_reason );
}



/****************************************
 * Function : kbp_rm_add_range
 *
 * Parameters:
 ...
 ****************************************/
NlmErrNum_t kbp_rm_delete_range(
    NlmRangeMgr     *self,
    NlmRangeDb      *rangeDb,
    nlm_u32         id,
    NlmReasonCode   *o_reason
 ...
 * MCORs; 
 *****************************************/
NlmErrNum_t kbp_rm_assign_range(
    NlmRangeMgr     *self,
    NlmRangeDb      *rangeDb,
    NlmRangeType    rangeType,
    NlmReasonCode   *o_reason
...
/****************************************
 * Function : kbp_rm_create_encodings
 *
 * Parameters:
 * NlmRangeMgr  *pRangeMgr  = Pointer to the Range Manager
 ...
 ****************************************/
NlmErrNum_t kbp_rm_create_encodings(
    NlmRangeMgr     *self,
    NlmRangeDb      *rangeDb,
    NlmReasonCode   *o_reason
    )
...
/****************************************
 * Function : kbp_rm_get_range_encoding
 *
 * Parameters:
 * NlmRangeMgr *pRangeMgr   = Pointer to the Range Manager
 ...
 ****************************************/
NlmRangeEncoded* kbp_rm_get_range_encoding(
    NlmRangeMgr         *self,
    NlmRangeDb          *rangeDb,
    nlm_u32              id,
    NlmReasonCode       *o_reason
...
/****************************************
 * Function : kbp_rm_config_range_matching
 *
 * Parameters:
 ...
 ****************************************/
NlmErrNum_t kbp_rm_config_range_matching(
    NlmRangeMgr *self,
    nlm_u8 ltrNum,
    NlmRangeSrchAttrs *srchAttrs,
    NlmReasonCode *o_reason
 ...
 ****************************************/
NlmRangeDbStats* kbp_rm_get_db_stats(
    NlmRangeMgr         *self,
    NlmRangeDb          *rangeDb,
    NlmReasonCode       *o_reason
    )
 ...
 * NlmPortMode portMode     = single or dual port mode
 * NlmSMTMode  smtMode      = non-smt or smt mode
 * NlmBankNum               = bankNum 0 or 1
 * NlmPortNum               = PORT 0/1,
    ...
    )
{
    ...
    NlmErrNum_t catchErr = 0;
    NlmRangeType rangeType;
    nlm_u32 rangeCodeValue =  NLM_RANGE_3B_ENCODING_CODE_VALUE;
    NlmDevMgr *devMgr_p = NULL;

    NlmSMTNum  smtNum    = NLMDEV_SMT_0;
    NlmSMTMode smtMode   = NLMDEV_NO_SMT_MODE;
    NlmPortMode portMode = NLMDEV_SINGLE_PORT;
...
#else
    if( (devType != NLM_DEVTYPE_3) && (devType != NLM_DEVTYPE_3_N) &&
    ...
    /* get the SMT/PORT mode from the device manager */
    smtMode  = devMgr_p->m_smtMode;
    portMode = devMgr_p->m_portMode;
    ...
    self = (NlmRangeMgr*)NlmCmAllocator__malloc(alloc_p, sizeof(NlmRangeMgr));
    ...
    self->m_bankNum  = bankNum;
    self->m_portNum  = portNum;

    /* init range manager */
    if(NLMERR_OK != (catchErr = NlmRangeMgr2__pvt_Init( self, o_reason )))
    {
    ...
    if(bankNum == NLMDEV_BANK_1)
        smtNum = NLMDEV_SMT_1;
    
    for(rangeType = NLM_RANGE_TYPE_A; rangeType < NLM_RANGE_TYPE_END;)
    {
        ...
        rangeType = (NlmRangeType)(rangeType + 1);
    }
    ...
    )
{
    ...
    while(self->m_rangeDb_count)
    {
        ...
        if(NLMERR_OK != (catchErr = kbp_rm_destroy_db   (
                                            self,
                                            rangeDb,
                                            o_reason
                                            )))
 ...
 NlmRangeDb* bcm_kbp_rm_create_db(
 ...
 NlmErrNum_t bcm_kbp_rm_destroy_db(
     NlmRangeMgr     *pRangeMgr,
     NlmRangeDb      *existingDb,
     NlmReasonCode   *o_reason
     )
 {
     return kbp_rm_destroy_db(
     pRangeMgr,
     existingDb,
     o_reason
     );
 ...
 NlmErrNum_t bcm_kbp_rm_add_range(
     NlmRangeMgr     *pRangeMgr,
     NlmRangeDb      *existingDb,
     NlmRange        *newRange,
     NlmReasonCode   *o_reason
 ...
 NlmErrNum_t bcm_kbp_rm_delete_range(
     NlmRangeMgr     *pRangeMgr,
     NlmRangeDb      *existingDb,
     nlm_u32         id,
     NlmReasonCode   *o_reason
 ...
 NlmErrNum_t bcm_kbp_rm_create_encodings(
     NlmRangeMgr     *pRangeMgr,
     NlmRangeDb      *existingDb,
     NlmReasonCode   *o_reason
     )
 {
     return kbp_rm_create_encodings(
     pRangeMgr,
     existingDb,
     o_reason
     );
 ...
 NlmErrNum_t bcm_kbp_rm_assign_range(
     NlmRangeMgr     *pRangeMgr,
     NlmRangeDb      *existingDb,
 ...
 NlmRangeEncoded* bcm_kbp_rm_get_range_encoding(
     NlmRangeMgr         *pRangeMgr,
     NlmRangeDb          *existingDb,
     nlm_u32             id,
     NlmReasonCode       *o_reason
 ...
 {
     return kbp_rm_get_range_encoding(
     pRangeMgr,
     existingDb,
     id,
     o_reason
 ...
 NlmRangeDbStats* bcm_kbp_rm_get_db_stats(
     NlmRangeMgr         *pRangeMgr,
     NlmRangeDb          *existingDb,
     NlmReasonCode       *o_reason
     )
 {
     return kbp_rm_get_db_stats(
     pRangeMgr,
     existingDb,
     o_reason
     );
 ...
 NlmErrNum_t bcm_kbp_rm_config_range_matching(
     NlmRangeMgr *self,
     nlm_u8 ltrNum,
     NlmRangeSrchAttrs *srchAttrs,
     NlmReasonCode *o_reason
 ...
 {
     return kbp_rm_config_range_matching(
     self,
     ltrNum,
     srchAttrs,
     o_reason
 ...
 NlmErrNum_t bcm_kbp_rm_destroy(
     NlmRangeMgr      *pRangeMgr,
     NlmReasonCode   *o_reason
     )
 {
     return kbp_rm_destroy(
     pRangeMgr,
     o_reason
     );
 }
