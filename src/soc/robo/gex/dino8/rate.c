...
static int dino8_ingress_rate_init_flag[SOC_MAX_NUM_DEVICES] = {0};

/*
 *  Function : _drv_dino8_port_irc_set
 *
 *  Purpose :
 ...
 */
static int
_drv_dino8_port_irc_set(int unit, uint32 port, uint32 limit, 
    uint32 burst_size)
{
    ...
    if (limit == 0) {
        ...
        if (!dino8_ingress_rate_init_flag[unit]) {
            pbmp = PBMP_ALL(unit);
            ...
            dino8_ingress_rate_init_flag[unit] = 1;
        }
...
/*
 *  Function : _drv_dino8_port_irc_get
 *
 *  Purpose :
 ...
 */
static int
_drv_dino8_port_irc_get(uint32 unit, uint32 port, uint32 *limit, 
    uint32 *burst_size)
{
    ...
    if (dino8_ingress_rate_init_flag[unit] != 0) {
        temp = 0;
        SOC_IF_ERROR_RETURN(DRV_RATE_CONFIG_GET
            (unit, SOC_PBMP_WORD_GET(PBMP_ALL(unit), 0), 
            DRV_RATE_CONFIG_PKT_MASK, &temp));

        /* If dino8_ingress_rate_init_flag[unit] = 0, only next ingress rate setting 
         * may set the properly PKT_MASK0 again currenly.
         */ 
        dino8_ingress_rate_init_flag[unit] = (temp == 0) ? 0 : 1;
    }
...
/*
 *  Function : drv_dino8_rate_config_set
 *
 *  Purpose :
 ...
 */
int 
drv_dino8_rate_config_set(int unit, soc_pbmp_t pbmp, uint32 config_type, 
    uint32 value)
{
    uint32  reg_value, temp;
...
/*
 *  Function : drv_dino8_rate_config_get
 *
 *  Purpose :
 ...
 */
int 
drv_dino8_rate_config_get(int unit, uint32 port, uint32 config_type, 
     uint32 *value)
{
    uint32  reg_value, temp = 0;
 ...
 */
int 
drv_dino8_rate_set(int unit, soc_pbmp_t bmp, uint8 queue_n, int direction, 
    ...
    uint32 kbits_sec_max, uint32 burst_size)
{
    ...
    LOG_INFO(BSL_LS_SOC_PORT,
             (BSL_META_U(unit,
                         "drv_dino8_rate_set: unit = %d, bmp = 0x%x, %s, flasg = 0x%x, \
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            PBMP_ITER(bmp, port) {
                SOC_IF_ERROR_RETURN(_drv_dino8_port_irc_set
 ...
 */
int 
drv_dino8_rate_get(int unit, uint32 port, uint8 queue_n, int direction, 
    uint32 *flags, uint32 *kbits_sec_min, 
    uint32 *kbits_sec_max, uint32 *burst_size)
{
    ...
    switch (direction) {
        case DRV_RATE_CONTROL_DIRECTION_INGRESSS:
            SOC_IF_ERROR_RETURN(_drv_dino8_port_irc_get
                (unit, port, kbits_sec_max, burst_size));
            break;
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS:
        case DRV_RATE_CONTROL_DIRECTION_EGRESSS_PER_QUEUE:
    ...
    LOG_INFO(BSL_LS_SOC_PORT,
             (BSL_META_U(unit,
                         "drv_dino8_rate_get: unit = %d, port = %d, %s, flags = 0x%x, \
                         kbits_sec_min = %dK, kbits_sec_max = %dK, burst size = %dKB\n"),
