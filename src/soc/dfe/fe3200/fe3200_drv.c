...
soc_fe3200_set_dcm_config(int unit)
{
    ...
    for (i=0 ; i < SOC_DFE_MAX_NOF_PIPES; i++) /* iterate over all possible pipes*/
    {
        ...
        /* init fifo size threshold*/
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_0, pipes[i], 1 /*is_fe1*/, 1/*is_fe3*/, dcm_thresholds_default_values.fifo_size[i], 0));
        SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], 1 /*is_fe1*/, 1/*is_fe3*/, dcm_thresholds_default_values.fifo_size[i], 0));
    ...
    if (SOC_DFE_IS_FE13(unit) && SOC_DFE_CONFIG(unit).fabric_local_routing_enable)
    {
        ...
        for (i=0 ; i < SOC_DFE_MAX_NOF_PIPES; i++) /* iterate over all possible pipes*/
        {
            ...
            /*fifo size*/
            SOCDNX_IF_ERR_EXIT(soc_fe3200_fabric_flow_control_mid_fifo_size_threshold_set(unit, soc_dfe_fabric_link_fifo_type_index_1, pipes[i], 1 /*is_fe1*/, 1/*is_fe3*/, dcm_thresholds_default_values.local_switch_fifo_size[i], 0));
...
soc_fe3200_drv_test_brdc_blk_info_get(int unit, int max_size, soc_reg_brdc_block_info_t *brdc_info, int *actual_size)
{
    ...
    *actual_size = 0;
    ...
    if (!(dcmn_device_block_for_feature(unit,DCMN_FABRIC_12_QUADS_FEATURE)) &&
        ...
        !(dcmn_device_block_for_feature(unit,DCMN_FABRIC_18_QUADS_FEATURE))) /*SKUs do not support broadcast fsrd*/
    {
        if (max_size > *actual_size)
        {
