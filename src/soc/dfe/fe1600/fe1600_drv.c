/*
 * $Id: fe1600_drv.c,v 1.72 Broadcom SDK $
 *
...
/*Defines*/
#define SOC_FE1600_GRACEFUL_SHUT_DOWN_DISABLE_DELAY 2000000 /*2 seconds*/
...
soc_fe1600_set_operation_mode(int unit) {
    ...
    multistage_config = (!SOC_DFE_IS_MULTISTAGE(unit)) ? 0 :                                                                                        /*for single stage set to 0*/
                        (SOC_DFE_IS_FE2(unit) && SOC_IS_FE1600_B0_AND_ABOVE(unit) && SOC_DFE_CONFIG(unit).system_is_vcs_128_in_system) ? 0 : 1;     /*for multistage FE2 iff B0 and VSC128_in system set to 0*/ 
...
soc_fe1600_set_rtp_config(int unit) {
    ...
    core_clock_speed = SOC_DFE_CONFIG(unit).core_clock_speed;
     ...
     /*RTPWP CALC*/
    wp_at_core_clock_steps = ((SOC_FE1600_RTP_REACHABILTY_WATCHDOG_RATE / 1000)/*micro sec*/ * core_clock_speed /*KHz*/) / 1000 /*convert micro sec to mili */;
...
soc_fe1600_set_dch_config(int unit) {
    ...
    if (SOC_DFE_IS_REPEATER(unit)) {
        ...
        /*FE1*/
        for (port=0 ; port < SOC_DFE_DEFS_GET(unit, nof_instances_dch)/2; port++) { /*FE1 threshold*/
            SOCDNX_IF_ERR_EXIT(READ_DCH_REG_0091r(unit, port, &reg32_val));
            soc_reg_field_set(unit, DCH_REG_0091r, &reg32_val, FIELD_0_0f, 0X1); /*enable internal threshold*/
            soc_reg_field_set(unit, DCH_REG_0091r, &reg32_val, FIELD_4_22f, SOC_FE1600_DCH_REG_0091_FIELD_4_24_FE1_FE2_VSC256_DEF);
            SOCDNX_IF_ERR_EXIT(WRITE_DCH_REG_0091r(unit, port, reg32_val));
        ...
        if (SOC_DFE_CONFIG(unit).system_is_vcs_128_in_system)
        {
            for (port=SOC_DFE_DEFS_GET(unit, nof_instances_dch)/2 ; port < SOC_DFE_DEFS_GET(unit, nof_instances_dch); port++) { /*FE3 threshold*/
                SOCDNX_IF_ERR_EXIT(READ_DCH_REG_0091r(unit, port, &reg32_val));
                soc_reg_field_set(unit, DCH_REG_0091r, &reg32_val, FIELD_0_0f, 0X0); /*disable internal threshold*/
                SOCDNX_IF_ERR_EXIT(WRITE_DCH_REG_0091r(unit, port, reg32_val));
            }
...
/*
 * fe1600 interrupt initialization
 */
STATIC int
...
soc_fe1600_reset_device(int unit)
{
    ...
    if (SOC_IS_FE1600_B0_AND_ABOVE(unit)) {
        rc = soc_fe1600_bo_hw_bug_fixes_enable(unit, 1);
        SOCDNX_IF_ERR_EXIT(rc);
    }
#endif
...
#ifdef BCM_88750_B0
int
soc_fe1600_bo_hw_bug_fixes_enable(int unit, int enable)
{
    uint32 reg32_val;
    int i;
    SOCDNX_INIT_FUNC_DEFS;
    ...
    for(i=0; i< SOC_DFE_DEFS_GET(unit, nof_instances_dch); i++) {
        ...
        /*Enable FE1600 B0 bug fix of fragment number translation in multistage systems with petraa*/
        ...
        if (SOC_DFE_IS_MULTISTAGE_FE2(unit) || (SOC_DFE_IS_FE13(unit) && i>=2/*DCH2 or DCH3*/)) {
 ...
 */
#define SOC_FE1600_DRV_BCM88754_SIF_POLLING_TIMEOUT                 (1000000) /*1 second*/
#define SOC_FE1600_DRV_BCM88754_SIF_POLLING_MIN_POLLS               (100)     /*Number of times to check without a sleep*/
...
soc_fe1600_drv_bcm88754_async_fifo_set(int unit, int fmac_index, int lane, soc_field_t field, uint32 data) 
{
    ...
    SOCDNX_IF_ERR_EXIT(READ_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_index, 0 /*not an array register*/, &reg64_val));
    ...
    SOCDNX_IF_ERR_EXIT(WRITE_FMAC_ASYNC_FIFO_CONFIGURATIONr(unit, fmac_index, 0 /*not an array register*/, reg64_val));
...
{   
    ...
    if (SOC_DFE_IS_FE13_ASYMMETRIC(unit) && (block_type == SOC_BLK_DCH || block_type == SOC_BLK_DCM))
    {
        ...
        if (asymmetric_quad == 0 || asymmetric_quad == 1)
        {
            ...
            *inner_link = SOC_DFE_DEFS_GET(unit, nof_links_in_dcq) + link % SOC_DFE_DEFS_GET(unit, nof_links_in_quad); /* 36 + inner quad link */
            SOC_EXIT;
        }
    }
    switch (block_type)
