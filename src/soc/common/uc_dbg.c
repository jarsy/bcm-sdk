...
_soc_cmic_ucdebug_dump_sram_info(int unit)
{
    ...
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 0));
    debug[0]= soc_ntohl(tmp);
    bsl_printf("debug[0]: %08x \n", debug[0]);

    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 1));
    debug[1]= soc_ntohl(tmp);
    bsl_printf("debug[1]: %08x \n", debug[1]);

    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 2));
    debug[2]= soc_ntohl(tmp);
    bsl_printf("debug[2]: %08x \n", debug[2]);

    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 3));
    debug[3] = soc_ntohl(tmp);
    bsl_printf("debug[3]: %08x \n", debug[3]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 4));
    debug[4] = soc_ntohl(tmp);
    bsl_printf("debug[4]: %08x \n", debug[4]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 5));
    debug[5] = soc_ntohl(tmp);
    bsl_printf("debug[5]: %08x \n", debug[5]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 6));
    debug[6] = soc_ntohl(tmp);
    bsl_printf("debug[6]: %08x \n", debug[6]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 7));
    debug[7] = soc_ntohl(tmp);
    bsl_printf("debug[7]: %08x \n", debug[7]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 8));
    debug[8] = soc_ntohl(tmp);
    bsl_printf("debug[8]: %08x \n", debug[8]);
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 9));
    debug[9] = soc_ntohl(tmp);
    bsl_printf("debug[9]: %08x \n", debug[9]);

    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 10));
    debug[10] = soc_ntohl(tmp);
    bsl_printf("debug[10]: %08x \n", debug[10]);

    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base+ 11));
...
_soc_cmic_ucdebug_allocate_shared_memory(int unit)
{
    ...
    for (tmp=0; tmp < MOS_UCDBG_MAX_LOG_ENTRIES_UC; tmp++) {
        ...
        soc_cm_sinval(unit, phostdbg_stat->puclog[tmp],
                      sizeof(mos_msg_ucdbg_entry_t));
        puclog_buffarray[tmp] = 
            (mos_msg_ucdbg_entry_t *)soc_htonl(soc_cm_l2p(unit, 
            phostdbg_stat->puclog[tmp]));
   }
                           ...
                           void *unused_2, void* unused_3)
{
    ...
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base + UCDBG_OFFSET_STATE_UC));
    ...
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base + UCDBG_OFFSET_WR_INDX));
...
soc_cmic_ucdebug_status(int unit)
{
    ...
    tmp = soc_uc_mem_read(unit, (uint32)((uint32 *)ucdbg_msg_base + UCBDG_OFFSET_STATE_HOST));
