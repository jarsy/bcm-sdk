    ...
    )
{
    ...
    if (SOC_IS_QAX(unit) && (flags & SOC_SAT_GTF_RATE_IN_PACKETS)) {
        if (clocks_per_cycle != (clocks_per_sec)) {
            clocks_per_cycle = clocks_per_sec;

            rv = READ_OAMP_SAT_GEN_CONFIGr_REG64(unit, &field64);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_SAT,
                          (BSL_META_U(unit,
                                      "Fail(%s) in read OAMP_SAT_GEN_CONFIGr\n"), soc_errmsg(rv)));
            ...
            soc_reg64_field32_set(unit,OAMP_SAT_GEN_CONFIGr, &field64, RATE_NUM_CLKS_CYCLEf, clocks_per_cycle);

            rv = WRITE_OAMP_SAT_GEN_CONFIGr_REG64(unit, field64);
            if (SOC_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_SOC_SAT,
                          (BSL_META_U(unit,
    ...
    )
{
    ...
    rv = READ_OAMP_SAT_GEN_CONFIGr_REG64(unit, &data);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in read OAMP_SAT_GEN_CONFIGr\n"), soc_errmsg(rv)));
    ...
    rv = WRITE_OAMP_SAT_GEN_CONFIGr_REG64(unit, data);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in write OAMP_SAT_GEN_CONFIGr\n"), soc_errmsg(rv)));
    ...
    )
{
    ...
    rv = READ_OAMP_SAT_GEN_CONFIGr_REG64(unit, &reg_val);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in read OAMP_SAT_GEN_CONFIGr\n"), soc_errmsg(rv)));
    ...
    rv = WRITE_OAMP_SAT_GEN_CONFIGr_REG64(unit, reg_val);
    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_SAT,
                  (BSL_META_U(unit,
                              "Fail(%s) in write OAMP_SAT_GEN_CONFIGr\n"), soc_errmsg(rv)));
    ...
    )
{
    ...
    if ((priority == SOC_SAT_GTF_OBJ_COMMON)) {
        ...
        if (SOC_IS_QAX(unit) && (bw->flags & SOC_SAT_GTF_RATE_IN_PACKETS)) {
            soc_reg_above_64_field32_set(unit, OAMP_SAT_EVC_RATE_CONTROLr, data, CRDT_PER_PKT_EVC__Nf, 1);
        }
        ...
        if (SOC_IS_QAX(unit) && (bw->flags & SOC_SAT_GTF_RATE_IN_PACKETS)) {
            soc_reg_above_64_field32_set(unit, OAMP_SAT_GEN_RATE_CONTROLr, data, CRDT_PER_PKT__Nf, 1);
        }
