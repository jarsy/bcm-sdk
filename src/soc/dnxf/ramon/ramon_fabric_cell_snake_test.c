    ...
    uint32 flags)
{
    ...
    for(link_idx=0;link_idx<144;link_idx++) {
        DNXF_LINK_INPUT_CHECK(unit, link_idx);
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_enable_set,(unit, link_idx, 0));
        DNXC_IF_ERR_EXIT(rv);
    }
    /* 
     *Disable interrupts
    ...
    if (!(flags & SOC_DNXF_ENABLE_MAC_LOOPBACK)) 
	{
        ...
        PBMP_SFI_ITER(unit, port)
        {
                if(flags & SOC_DNXF_ENABLE_EXTERNAL_LOOPBACK) {
                    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_loopback_set, (unit, port, soc_dnxc_loopback_mode_none));
                    DNXC_IF_ERR_EXIT(rv);
                } else {
                    /*Make sure CL72 is turned off*/
                    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_phy_cl72_set, (unit, port, 0));
                    DNXC_IF_ERR_EXIT(rv);
                    /*Set loopback*/
                    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_loopback_set, (unit, port, soc_dnxc_loopback_mode_phy_gloop));
                    DNXC_IF_ERR_EXIT(rv);
                }
    ...
    SOC_REG_ABOVE_64_WORD_SET(block_bitmap_default, SOC_RAMON_DRV_BLOCKS_RESET_WITHOUT_FSRD_WORD_0, 0);
    SOC_REG_ABOVE_64_WORD_SET(block_bitmap_default, SOC_RAMON_DRV_BLOCKS_RESET_WITHOUT_FSRD_WORD_1, 1);
    SOC_REG_ABOVE_64_WORD_SET(block_bitmap_default, SOC_RAMON_DRV_BLOCKS_RESET_WITHOUT_FSRD_WORD_2, 2);
    ...
    DNXC_IF_ERR_EXIT(soc_ramon_cell_snake_test_reg_reset(unit));
    ...
    *field_val = ((!(flags & SOC_DNXF_ENABLE_MAC_LOOPBACK)) ? 0x1:0x0);
    ...
    for(link_idx=0;link_idx<144;link_idx++) {
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_enable_set,(unit, link_idx, 1));
        DNXC_IF_ERR_EXIT(rv);
    }
...
{  
    ...
    DNXC_IF_ERR_EXIT(soc_ramon_cell_snake_test_interrupts_clear(unit));
    ...
    if (flags & SOC_DNXF_SNAKE_STOP_TEST)
    {
        ...
        if (field_val != 0x1)
        {
          DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("test is not on %d"),field_val)); 
        }
    }
    ...
    while(1)
    {
        ...
        if (soc_timeout_check(&to)) {
            DNXC_EXIT_WITH_ERR(SOC_E_TIMEOUT, (_BSL_DNXC_MSG("timeout - test was not triggered"))); 
        }
    }
    ...
    int unit)
{
    ...
    for(i=0; i<SOC_DNXF_DEFS_GET(unit, nof_links_in_mac); i++) {
    ...
    int unit)
{
    ...
    DNXC_IF_ERR_EXIT(WRITE_BRDC_DCL_INTERRUPT_REGISTERr(unit, 0xFFFFFFFF));
