/*
 * $Id: ramon_fabric_cell.c,v 1.13.16.1 Broadcom SDK $
 *
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * SOC RAMON FABRIC CELL
 */
 
#ifdef _ERR_MSG_MODULE_NAME
  #error "_ERR_MSG_MODULE_NAME redefined"
#endif

#define _ERR_MSG_MODULE_NAME BSL_SOC_FABRIC 
#include <shared/bsl.h>

#include <soc/mcm/memregs.h>
#include <soc/error.h>
#include <soc/defs.h>

#ifdef BCM_88790

#include <soc/dnxc/legacy/error.h>
#include <soc/dnxc/legacy/dnxc_cells_buffer.h>
#include <soc/dnxc/legacy/dnxc_mem.h>

#include <soc/dnxf/cmn/dnxf_drv.h>
#include <soc/dnxf/cmn/dnxf_config_defs.h>

#include <soc/dnxf/ramon/ramon_fabric_cell.h>
#include <soc/dnxf/fe1600/fe1600_fabric_cell.h>

soc_error_t 
soc_ramon_cell_filter_clear(int unit)
{
    int i,nof_instances_dch;
	soc_reg_above_64_val_t reg_val;
    DNXC_INIT_FUNC_DEFS;

    nof_instances_dch = SOC_DNXF_DEFS_GET(unit, nof_instances_dch);

	SOC_REG_ABOVE_64_CLEAR(reg_val);
    for(i=0 ; i<nof_instances_dch; i++) {  
        DNXC_IF_ERR_EXIT(WRITE_DCH_MATCH_FILTr(unit, i, reg_val));
        DNXC_IF_ERR_EXIT(WRITE_DCH_MATCH_FILT_MASKr(unit, i, reg_val));
    }

exit:
     DNXC_FUNC_RETURN;
}

soc_error_t
soc_ramon_cell_filter_set(int unit, uint32 flags, uint32 array_size, soc_dnxc_filter_type_t* filter_type_arr, uint32* filter_type_val) 
{
	int i, rc;
	uint32 table_size;
	uint64 reg_val64;
	soc_reg_above_64_val_t filter_reg, mask_reg;
	soc_dnxf_cell_filter_mode_t filter_mode = soc_dnxf_cell_filter_copy_mode;

	DNXC_INIT_FUNC_DEFS;

	if((flags & SOC_CELL_FILTER_FLAG_DONT_FORWARD) && (flags & SOC_CELL_FILTER_FLAG_DONT_TRAP)) {
        filter_mode = soc_dnxf_cell_filter_filter_mode;
    } else if (flags & SOC_CELL_FILTER_FLAG_DONT_FORWARD) {
        filter_mode = soc_dnxf_cell_filter_capture_mode;
    } else if (flags & SOC_CELL_FILTER_FLAG_DONT_TRAP) {
        filter_mode = soc_dnxf_cell_filter_normal_mode;
    }

    for(i=0 ; i<SOC_DNXF_DEFS_GET(unit, nof_instances_dch) ; i++) {
        DNXC_IF_ERR_EXIT(READ_DCH_DCH_ENABLERS_REGISTER_1r(unit, i, &reg_val64));
        soc_reg64_field32_set(unit, DCH_DCH_ENABLERS_REGISTER_1r, &reg_val64, DCH_MTCH_ACTf, filter_mode);
        DNXC_IF_ERR_EXIT(WRITE_DCH_DCH_ENABLERS_REGISTER_1r(unit, i, reg_val64));
    }

	SOC_REG_ABOVE_64_CLEAR(filter_reg);
	SOC_REG_ABOVE_64_ALLONES(mask_reg);

	for (i = 0 ; i < array_size ; i++)
	{
		switch (filter_type_arr[i])
		{
			case soc_dnxc_filter_type_source_id:
				if((filter_type_val[i] & (~0xFF)) != 0) {
					DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("source id %d out of range"), filter_type_val[i]));
				}
				/* relevant bits are 114-124 */
				SOC_REG_ABOVE_64_RANGE_COPY(filter_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_SOURCE_ID_START, &(filter_type_val[i]), 0, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_SOURCE_ID_LENGTH);
				SHR_BITCLR_RANGE(mask_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_SOURCE_ID_START, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_SOURCE_ID_LENGTH);
				break;

			case soc_dnxc_filter_type_multicast_id:
            case soc_dnxc_filter_type_dest_id:
				rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_multicast_table_size_get,(unit, &table_size));
				DNXC_IF_ERR_EXIT(rc);
				if(filter_type_val[i] >= table_size) {
					DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("dest id out of range")));
				}
				SOC_REG_ABOVE_64_RANGE_COPY(filter_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_DEST_ID_START, &(filter_type_val[i]), 0, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_DEST_ID_LENGTH);
				SHR_BITCLR_RANGE(mask_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_DEST_ID_START, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_DEST_ID_LENGTH);
				break;

			case soc_dnxc_filter_type_priority:
				if((filter_type_val[i] & (~0x3)) != 0) {
					DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("the priority type is out of range")));
				}
				SOC_REG_ABOVE_64_RANGE_COPY(filter_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_PRIORITY_TYPE_START, &(filter_type_val[i]), 0, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_PRIORITY_TYPE_LENGTH);
				SHR_BITCLR_RANGE(mask_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_PRIORITY_TYPE_START, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_PRIORITY_TYPE_LENGTH);
				break;
           case soc_dnxc_filter_type_traffic_cast:
				if((filter_type_val[i] & (~0x3)) != 0) {
					DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("the traffic cast is out of range")));
				}
				SOC_REG_ABOVE_64_RANGE_COPY(filter_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_TRAFFIC_CAST_START, &(filter_type_val[i]), 0, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_TRAFFIC_CAST_LENGTH);
				SHR_BITCLR_RANGE(mask_reg, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_TRAFFIC_CAST_START, SOC_RAMON_FABRIC_CELL_MATCH_FILTER_REG_TRAFFIC_CAST_LENGTH);
				break;
			default:
				DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Unsupported Filter Type")));

		}

	}

	for (i = 0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_dch); i++)
	{
		DNXC_IF_ERR_EXIT(WRITE_DCH_MATCH_FILTr(unit, i, filter_reg));
		DNXC_IF_ERR_EXIT(WRITE_DCH_MATCH_FILT_MASKr(unit, i, mask_reg));
	}

exit:
     DNXC_FUNC_RETURN;
}

soc_error_t 
soc_ramon_cell_filter_count_get(int unit, int *count)
{
    int i;
    int temp_count;
    uint32 reg32_val;
    DNXC_INIT_FUNC_DEFS;
    
    *count = 0;
    for(i=0 ; i<SOC_DNXF_DEFS_GET(unit, nof_instances_dch) ; i++) {  
        DNXC_IF_ERR_EXIT(READ_DCH_PROGRAMMABLE_CELLS_COUNTER_1r(unit, i, &reg32_val));

        if (*count == -1) {
            continue;
        }

        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_1r, reg32_val, PRG_CELL_CNT_Af);
        *count += temp_count;
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_1r, reg32_val, PRG_CELL_CNT_Bf);
        *count += temp_count;

        /*check overflow*/
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_1r, reg32_val, PRG_CELL_CNT_AOf);
        if (temp_count) {
            *count = -1;
            continue;
        }
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_1r, reg32_val, PRG_CELL_CNT_BOf);
        if (temp_count) {
            *count = -1;
            continue;
        }

    }

    
    for(i=0 ; i<SOC_DNXF_DEFS_GET(unit, nof_instances_dch) ; i++) {  
        DNXC_IF_ERR_EXIT(READ_DCH_PROGRAMMABLE_CELLS_COUNTER_2r(unit, i, &reg32_val));

        if (*count == -1) {
            continue;
        }

        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_Cf);
        *count += temp_count;
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_Df);
        *count += temp_count;

        /*check overflow*/
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_COf);
        if(temp_count) {
            *count = -1;
            continue;
        }
        temp_count = soc_reg_field_get(unit, DCH_PROGRAMMABLE_CELLS_COUNTER_2r, reg32_val, PRG_CELL_CNT_DOf);
        if(temp_count) {
            *count = -1;
            continue;
        }
    }
    
exit:
     DNXC_FUNC_RETURN;
}

soc_error_t 
soc_ramon_control_cell_filter_set(int unit, uint32 flags, soc_dnxc_control_cell_types_t cell_type, uint32 array_size, soc_dnxc_control_cell_filter_type_t* filter_type_arr, uint32* filter_type_val) 
{
	int i;
	uint32 reg_val32;
	soc_reg_above_64_val_t reg_val, mask_val;
	soc_dnxf_cell_filter_mode_t filter_mode = soc_dnxf_control_cell_filter_copy_mode;

    DNXC_INIT_FUNC_DEFS;
    
	if((flags & SOC_CELL_FILTER_FLAG_DONT_FORWARD) && (flags & SOC_CELL_FILTER_FLAG_DONT_TRAP)) {
		filter_mode = soc_dnxf_control_cell_filter_filter_mode;
	} else if (flags & SOC_CELL_FILTER_FLAG_DONT_FORWARD) {
		filter_mode = soc_dnxf_control_cell_filter_capture_mode;
	} else if (flags & SOC_CELL_FILTER_FLAG_DONT_TRAP) {
		filter_mode = soc_dnxf_control_cell_filter_normal_mode;
	}

	for (i = 0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_ccs) ; i++)
	{
		DNXC_IF_ERR_EXIT(READ_CCS_DEBUG_CONFIGURATIONSr(unit, i, &reg_val32));
		soc_reg_field_set(unit, CCS_DEBUG_CONFIGURATIONSr, &reg_val32, CAPTURE_OPERATIONf, filter_mode);
        /* We would like to capture the control cells in VSC256 mode, even if the cell enter the device in VSC128 mode */
        /*Due the fact that the VSC128 cell bits are on the msbs of the filter/mask */
        soc_reg_field_set(unit, CCS_DEBUG_CONFIGURATIONSr, &reg_val32, FIELD_7_7f, 1);
        /*fe2 filter mode*/
        soc_reg_field_set(unit, CCS_DEBUG_CONFIGURATIONSr, &reg_val32, FIELD_5_5f, (cell_type == soc_dnxc_fe2_filtered_cell) ? 1 : 0);
        DNXC_IF_ERR_EXIT(WRITE_CCS_DEBUG_CONFIGURATIONSr(unit, i, reg_val32));
	}

	SOC_REG_ABOVE_64_CLEAR(reg_val);
    /* mask of "1"s mean to ignore when using the filter */
    SOC_REG_ABOVE_64_ALLONES(mask_val);

	if (cell_type == soc_dnxc_flow_status_cell || cell_type == soc_dnxc_credit_cell)
	{
		for (i = 0 ; i < array_size ; i++)
		{
			switch(filter_type_arr[i]) {
                case soc_dnxc_filter_control_cell_type_source_device:
                    if((filter_type_val[i] & (~0x7FF)) != 0) {
                        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("source device id is out of range")));
                    }
                    SHR_BITCOPY_RANGE(reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_START, &(filter_type_val[i]), 0, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
                    /* add mask for bits 103:93 in the cell*/
                    SHR_BITCLR_RANGE(mask_val, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
                    break;
                case soc_dnxc_filter_control_cell_type_dest_device:
                    if((filter_type_val[i] & (~0x7FF)) != 0) {
                        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("dest device id is out of range")));
                    }
                    SHR_BITCOPY_RANGE(reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_START, &(filter_type_val[i]), 0, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_LENGTH);
                    /* add mask for bits 114:104 in the cell*/
                    SHR_BITCLR_RANGE(mask_val, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_START, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_LENGTH);
                    break;
                default:
                    DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("unsupported filter type")));
            }
		}
	} else if (cell_type == soc_dnxc_reachability_cell) /* no support for dest_device filter */
	{
	   SHR_BITCOPY_RANGE(reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_SOURCE_ID_START, &(filter_type_val[0]), 0, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
       /* add mask for bits 111:101 in the cell*/
       SHR_BITCLR_RANGE(mask_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_SOURCE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
	} else { /* soc_dnxc_fe2_filtered_cell  - do nothing*/
	}

    /* Add the control cell type to the filter */ 
    SHR_BITCOPY_RANGE(reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_START, &cell_type, 0, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_LENGTH);
    /* add mask for bits 117:115 in the cell*/
    SHR_BITCLR_RANGE(mask_val, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_START, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_LENGTH);

	/* Set the filter and mask registers*/
   for(i=0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_ccs) ; i++) {
       DNXC_IF_ERR_EXIT(WRITE_CCS_CAPTURE_FILTER_CELL_0r(unit, i, reg_val));
       DNXC_IF_ERR_EXIT(WRITE_CCS_CAPTURE_FILTER_MASK_0r(unit, i, mask_val));
   }

exit:
     DNXC_FUNC_RETURN;

}

soc_error_t 
soc_ramon_control_cell_filter_clear(int unit)
{
    soc_reg_above_64_val_t reg_val;
    uint32 reg_val32;
    int i;
    DNXC_INIT_FUNC_DEFS;
	
    SOC_REG_ABOVE_64_CLEAR(reg_val);

	for (i = 0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_ccs); i++)
	{
		DNXC_IF_ERR_EXIT(WRITE_CCS_CAPTURE_FILTER_CELL_0r(unit, i, reg_val));
		DNXC_IF_ERR_EXIT(WRITE_CCS_CAPTURE_FILTER_MASK_0r(unit, i, reg_val));

		DNXC_IF_ERR_EXIT(READ_CCS_DEBUG_CONFIGURATIONSr(unit, i, &reg_val32));
		soc_reg_field_set(unit, CCS_DEBUG_CONFIGURATIONSr, &reg_val32, FIELD_6_6f, 0); /* CaptureFilterGckCells */
		DNXC_IF_ERR_EXIT(WRITE_CCS_DEBUG_CONFIGURATIONSr(unit, i, reg_val32));
	}

exit:
     DNXC_FUNC_RETURN;
}

soc_error_t 
soc_ramon_control_cell_filter_receive(int unit,  soc_dnxc_captured_control_cell_t* data_out)
{
	soc_reg_above_64_val_t  reg_val;
    int i;
	uint32 valid_cell_reg;
    DNXC_INIT_FUNC_DEFS;
	
	sal_memset(data_out, 0 ,sizeof(soc_dnxc_captured_control_cell_t));

	for (i = 0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_ccs); i++)
	{
		DNXC_IF_ERR_EXIT(READ_CCS_CAPTURED_CELL_VALIDr(unit, i, &valid_cell_reg));
		if (valid_cell_reg)
		{
			data_out->valid = 1;
			break;
		}
	}

	if (data_out->valid)
	{
		DNXC_IF_ERR_EXIT(READ_CCS_CAPTURED_CELLr(unit, i, reg_val));

        SHR_BITCOPY_RANGE((uint32*)&(data_out->control_type), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_START, RAMON_FE1600_VSC256_CONTROL_CELL_CONTROL_TYPE_LENGTH);

        switch(data_out->control_type) {
                case soc_dnxc_flow_status_cell:
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->dest_port), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_DEST_PORT_START, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_DEST_PORT_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->src_queue_num), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_SRC_QUEUE_NUM_START, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_SRC_QUEUE_NUM_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->flow_id), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_FLOW_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_FSM_FLOW_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->source_device), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->dest_device), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_START, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_LENGTH);
                    break;
                case soc_dnxc_credit_cell:
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->dest_queue_num),0 , reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_DEST_Q_NUM_START, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_DEST_Q_NUM_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->sub_flow_id), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_SUB_FLOW_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_SUB_FLOW_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->flow_id), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_FLOW_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_CREDIT_FLOW_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->source_device), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->dest_device), 0, reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_START, RAMON_FE1600_VSC256_CONTROL_CELL_DEST_DEVICE_LENGTH);
                    break;
                case soc_dnxc_reachability_cell:
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->reachability_bitmap),0 , reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_BITMAP_START, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_BITMAP_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->base_index),0 , reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_BASE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_BASE_ID_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->source_link_number),0 , reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_SRC_LINK_NUM_START, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_SRC_LINK_NUM_LENGTH);
                    SHR_BITCOPY_RANGE((uint32*)&(data_out->source_device),0 , reg_val, RAMON_FE1600_VSC256_CONTROL_CELL_REACHABILITY_SOURCE_ID_START, RAMON_FE1600_VSC256_CONTROL_CELL_SOURCE_ID_LENGTH);
                    break;
                default:
                    DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("unsupported filter type")));
        }
	}

exit:
    DNXC_FUNC_RETURN; 
}


/*
 * Function:
 *      soc_ramon_fabric_cell_get
 * Purpose:
 *      get cpu cell
 * Parameters:
 *      unit            - (IN)  Unit number.
 *      entry           - (OUT) Received data
 * Returns:
 *      SOC_E_XXX
 */
soc_error_t
soc_ramon_fabric_cell_get(int unit, soc_dnxc_fabric_cell_entry_t entry)
{
    uint32 reg, blk_idx;
    uint32 nof_cell;
    int found_cell;
    DNXC_INIT_FUNC_DEFS;   

    found_cell = 0;
    
    for(blk_idx=0 ; blk_idx<SOC_DNXF_DEFS_GET(unit, nof_instances_dcl) ; blk_idx++) 
    {

        DNXC_IF_ERR_EXIT(READ_DCL_CPU_CELL_FIFO_STATUSr(unit, blk_idx, &reg));

        nof_cell = soc_reg_field_get(unit, DCL_CPU_CELL_FIFO_STATUSr, reg, CPU_CELL_FIFO_STATUSf);       
   
        sal_memset(entry, 0x00, sizeof(soc_dnxc_fabric_cell_entry_t));
        if(nof_cell) {
            DNXC_IF_ERR_EXIT(dnxc_mem_array_wide_access(unit, SOC_MEM_NO_FLAGS, DCL_CPU_Hm, 0, blk_idx, 0, entry, 1));
            found_cell = 1;
            break;
        }
    }

    if (!found_cell)
    {
        DNXC_EXIT_WITH_ERR_NO_MSG(SOC_E_EMPTY);
    }

exit:
    DNXC_FUNC_RETURN;
}


soc_error_t
soc_ramon_fabric_cell_parse_table_get(int unit, soc_dnxc_fabric_cell_type_t cell_type, uint32 max_nof_lines, soc_dnxc_fabric_cell_parse_table_t *parse_table, uint32 *nof_lines, int is_two_parts)
{
    /*soc_dnxc_fabric_cell_parse_table_t* temp_table;*/
    int actual_nof_lines;
    DNXC_INIT_FUNC_DEFS;

    
    if (cell_type == soc_dnxc_fabric_cell_type_sr_cell)
    {
        soc_dnxc_fabric_cell_parse_table_t temp_table_src[] = SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT;

        if (max_nof_lines < SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES)
        {
            DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("Bigger table is required\n")));
        } 
        
        sal_memcpy(parse_table, temp_table_src, sizeof(soc_dnxc_fabric_cell_parse_table_t) * SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES);
        *nof_lines = SOC_RAMON_FABRIC_CELL_CAPTURED_SRC_FORMAT_NOF_LINES;
    } else { /*cpu data cell*/       
        soc_dnxc_fabric_cell_parse_table_t temp_table_captured[] = SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT;
        actual_nof_lines = is_two_parts ? SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES_DOUBLE_CELL:SOC_RAMON_FABRIC_CELL_CPU_CAPTURED_FORMAT_NOF_LINES_ONE_CELL ; 

        if (max_nof_lines < actual_nof_lines)
        {
            DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("Bigger table is required\n")));
        }
        sal_memcpy(parse_table, temp_table_captured, sizeof(soc_dnxc_fabric_cell_parse_table_t) * actual_nof_lines);
        *nof_lines = actual_nof_lines;
    }
   
exit:
    DNXC_FUNC_RETURN; 
}

#define _SOC_RAMON_PARSING_CELL_TYPE_START         (1115)
#define _SOC_RAMON_PARSING_CELL_TYPE_LENGTH        (1)
soc_error_t
soc_ramon_fabric_cell_type_get(int unit, soc_dnxc_fabric_cell_entry_t entry, soc_dnxc_fabric_cell_type_t *cell_type)
{
    DNXC_INIT_FUNC_DEFS;

    *cell_type = 0;
    SHR_BITCOPY_RANGE((uint32 *) cell_type, 0, (uint32 *) entry, _SOC_RAMON_PARSING_CELL_TYPE_START, _SOC_RAMON_PARSING_CELL_TYPE_LENGTH);

    DNXC_FUNC_RETURN; 
}

soc_error_t
soc_ramon_fabric_cell_is_cell_in_two_parts(int unit, soc_dnxc_fabric_cell_entry_t entry, int* is_two_parts)
{
    uint32 cell_size = 0 ;
    soc_dnxc_fabric_cell_type_t cell_type;
    DNXC_INIT_FUNC_DEFS;

    DNXC_IF_ERR_EXIT(soc_ramon_fabric_cell_type_get(unit, entry, &cell_type));

    if(cell_type == soc_dnxc_fabric_cell_type_sr_cell) /* SRC */
    {
        *is_two_parts = 0;
        
    }
    else 
    {
        /* Check cell size */
        SHR_BITCOPY_RANGE(&cell_size, 0, entry, 1117, 8);
        if (cell_size + 1 > 128)
        {
            *is_two_parts = 1;
        }
        else
        {
            *is_two_parts = 0;
        }
        
    }
exit:
    DNXC_FUNC_RETURN;
}

soc_error_t
soc_ramon_fabric_cell_dcl_cpu_wide_memory_read(int unit, int blk_idx, void* entry)
{
    uint32 reg_val32;
    soc_reg_above_64_val_t low_entry, hi_entry;
    uint32* mem_val = (uint32*) entry;
    int indirect_command_trigger, indirect_command_status;

    DNXC_INIT_FUNC_DEFS;

    SOC_REG_ABOVE_64_CLEAR(low_entry);
    SOC_REG_ABOVE_64_CLEAR(hi_entry);

    DNXC_IF_ERR_EXIT(READ_DCL_INDIRECT_COMMAND_ADDRESSr(unit, blk_idx, &reg_val32));
    soc_reg_field_set(unit, DCL_INDIRECT_COMMAND_ADDRESSr, &reg_val32, INDIRECT_COMMAND_ADDRf, 0x02300000);
    soc_reg_field_set(unit, DCL_INDIRECT_COMMAND_ADDRESSr, &reg_val32, INDIRECT_COMMAND_TYPEf, 1);
    DNXC_IF_ERR_EXIT(WRITE_DCL_INDIRECT_COMMAND_ADDRESSr(unit, blk_idx, reg_val32));

    DNXC_IF_ERR_EXIT(READ_DCL_INDIRECT_COMMANDr(unit, blk_idx, &reg_val32));
    soc_reg_field_set(unit, DCL_INDIRECT_COMMANDr, &reg_val32, INDIRECT_COMMAND_TIMEOUTf, 100);
    soc_reg_field_set(unit, DCL_INDIRECT_COMMANDr, &reg_val32, INDIRECT_COMMAND_TRIGGERf, 1);
    DNXC_IF_ERR_EXIT(WRITE_DCL_INDIRECT_COMMANDr(unit, blk_idx, reg_val32));

    DNXC_IF_ERR_EXIT(READ_DCL_INDIRECT_COMMANDr(unit, blk_idx, &reg_val32));
    indirect_command_trigger = soc_reg_field_get(unit, DCL_INDIRECT_COMMANDr, reg_val32, INDIRECT_COMMAND_TRIGGERf);
    indirect_command_status = soc_reg_field_get(unit, DCL_INDIRECT_COMMANDr, reg_val32, INDIRECT_COMMAND_STATUSf);
    if ((indirect_command_trigger != 0) || (indirect_command_status != 0))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("Indirect command failed - DCL indirect command register holds false values \n")));
    }

    sal_usleep(20000);

    DNXC_IF_ERR_EXIT(READ_DCL_INDIRECT_COMMAND_RD_DATAr(unit, blk_idx, low_entry));
    sal_usleep(20000);
    DNXC_IF_ERR_EXIT(READ_DCL_INDIRECT_COMMAND_RD_DATAr(unit, blk_idx, hi_entry));

    SHR_BITCOPY_RANGE(mem_val, 0, low_entry, 0, 640);
    SHR_BITCOPY_RANGE(mem_val, 640, hi_entry, 0, 488);

exit:
    DNXC_FUNC_RETURN;

}

#endif /*BCM_88790*/

#undef _ERR_MSG_MODULE_NAME
