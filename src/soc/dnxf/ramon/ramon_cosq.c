/*
 * $Id: ramon_cosq.c,v 1.13.16.1 Broadcom SDK $
 *
...
_soc_ramon_cosq_pipe_weight_validate(int unit, int pipe, soc_dnxf_cosq_weight_mode_t mode, int weight)
{
    ...
    if ((mode == soc_dnxf_cosq_weight_mode_dynamic_0) || (mode == soc_dnxf_cosq_weight_mode_dynamic_1)) 
    {
        if (pipe > 1) 
        {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid pipe id, dynamic weights can be set only for pipe 0 or 1\n")));
        }
    }
...
STATIC int
_soc_ramon_cosq_pipe_threshold_validate(int unit, int pipe, int threshold, int max_threshold)
{
    DNXC_INIT_FUNC_DEFS;

    if (pipe > 1) 
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid pipe id, dynamic thresholds can be set only for pipe 0 or 1\n")));
    }

    if ((threshold > max_threshold) || (threshold < 0))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid threshold\n")));
    }
...
soc_error_t
soc_ramon_cosq_pipe_rx_weight_set(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int weight)
{
    soc_error_t rc;
    ...
    if (port == -1) {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &dch_instance, &dch_inner_link, SOC_BLK_DCH));
        DNXC_IF_ERR_EXIT(rc);
    ...
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0) 
    {
        for (i=dch_loop_start ; i < dch_loop_end ; i++)
        {
            DNXC_IF_ERR_EXIT(READ_DCH_DYN_PIPES_WEIGHTS_REGISTERr(unit, i, &reg_val32));
            ...
            DNXC_IF_ERR_EXIT(WRITE_DCH_DYN_PIPES_WEIGHTS_REGISTERr(unit, i, reg_val32));
        }
    }
...
soc_error_t
soc_ramon_cosq_pipe_rx_weight_get(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int *weight)
{
    soc_error_t rc;
    uint32 reg_val32;
    int dch_instance, dch_inner_link;
    ...
    if (mode == soc_dnxf_cosq_weight_mode_regular) 
    {
        DNXC_IF_ERR_EXIT(READ_DCH_PIPES_WEIGHTS_REGISTERr(unit, dch_instance, &reg_val32));
    ...
    {
        DNXC_IF_ERR_EXIT(READ_DCH_DYN_PIPES_WEIGHTS_REGISTERr(unit, dch_instance, &reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_mid_weight_set(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int weight)
{
    soc_error_t rc;
    ...
    if (port == -1) {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &dcm_instance, &dcm_inner_link, SOC_BLK_DCM));
        DNXC_IF_ERR_EXIT(rc);
    ...
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0) 
    {
        for (i=dcm_loop_start ; i < dcm_loop_end ; i++)
        {
            DNXC_IF_ERR_EXIT(READ_DCM_DYN_PIPES_WEIGHTS_REGISTERr(unit, i, &reg_val32));
            ...
            DNXC_IF_ERR_EXIT(WRITE_DCM_DYN_PIPES_WEIGHTS_REGISTERr(unit, i, reg_val32));
        }
    }
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_1) 
    {
...
soc_error_t  
soc_ramon_cosq_pipe_mid_weight_get(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int *weight)
{
    soc_error_t rc;
    uint32 reg_val32;
    int dcm_instance, dcm_inner_link;
    ...
    if (mode == soc_dnxf_cosq_weight_mode_regular) 
    {
        DNXC_IF_ERR_EXIT(READ_DCM_PIPES_WEIGHTS_REGISTERr(unit, dcm_instance, &reg_val32));
    ...
    {
        DNXC_IF_ERR_EXIT(READ_DCM_DYN_PIPES_WEIGHTS_REGISTERr(unit, dcm_instance, &reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_tx_weight_set(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int weight)
{
    soc_error_t rc;
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &dcl_instance, &dcl_inner_link, SOC_BLK_DCL));
    DNXC_IF_ERR_EXIT(rc);
    ...
    if (mode == soc_dnxf_cosq_weight_mode_regular) 
    {
        DNXC_IF_ERR_EXIT(READ_DCL_LINKS_WEIGHT_CONFIGr(unit, dcl_instance, dcl_inner_link ,&reg_val32));

        /* modify specific pipe */
        field_val32 = soc_reg_field_get(unit, DCL_LINKS_WEIGHT_CONFIGr, reg_val32, LINK_W__Nf);
        SHR_BITCOPY_RANGE(&field_val32, (pipe*SOC_RAMON_COSQ_WFQ_WEIGHT_NOF_BITS), &temp_weight, 0, SOC_RAMON_COSQ_WFQ_WEIGHT_NOF_BITS);
        soc_reg_field_set(unit, DCL_LINKS_WEIGHT_CONFIGr, &reg_val32, LINK_W__Nf, field_val32);

        DNXC_IF_ERR_EXIT(WRITE_DCL_LINKS_WEIGHT_CONFIGr(unit, dcl_instance, dcl_inner_link, reg_val32));
    }
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0) 
    {
        DNXC_IF_ERR_EXIT(READ_DCL_LINKS_WEIGHT_CONFIG_P_0_GT_P_1r(unit, dcl_instance, dcl_inner_link ,&reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCL_LINKS_WEIGHT_CONFIG_P_0_GT_P_1r(unit, dcl_instance, dcl_inner_link, reg_val32));
    }
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_1) 
    {
        DNXC_IF_ERR_EXIT(READ_DCL_LINKS_WEIGHT_CONFIG_P_1_GT_P_0r(unit, dcl_instance, dcl_inner_link ,&reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCL_LINKS_WEIGHT_CONFIG_P_1_GT_P_0r(unit, dcl_instance, dcl_inner_link, reg_val32));
    }
...
soc_error_t  
soc_ramon_cosq_pipe_tx_weight_get(int unit, int pipe, int port, soc_dnxf_cosq_weight_mode_t mode, int *weight)
{
    soc_error_t rc;
	int dcl_inner_link, dcl_instance;
    ...
    if (mode == soc_dnxf_cosq_weight_mode_regular) 
    {
        ...
        /* read specific pipe */
        field_val32 = soc_reg_field_get(unit, DCL_LINKS_WEIGHT_CONFIGr, reg_val32, LINK_W__Nf);
        SHR_BITCOPY_RANGE(&temp_weight, 0, &field_val32, (pipe*SOC_RAMON_COSQ_WFQ_WEIGHT_NOF_BITS), SOC_RAMON_COSQ_WFQ_WEIGHT_NOF_BITS);
    }
    else if (mode == soc_dnxf_cosq_weight_mode_dynamic_0) 
    {
        DNXC_IF_ERR_EXIT(READ_DCL_LINKS_WEIGHT_CONFIG_P_0_GT_P_1r(unit, dcl_instance, dcl_inner_link ,&reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_rx_threshold_set(int unit, int pipe, int port, int threshold)
{
    soc_error_t rc;
    ...
    /* validation */
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, threshold, SOC_RAMON_COSQ_WFQ_DCH_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    for (i=dch_loop_start; i < dch_loop_end; i++) 
    {
        DNXC_IF_ERR_EXIT(READ_DCH_DYN_WFQ_WEIGHT_THD_REGISTEr(unit, i, &reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCH_DYN_WFQ_WEIGHT_THD_REGISTEr(unit, i, reg_val32));
    }
...
soc_error_t  
soc_ramon_cosq_pipe_rx_threshold_get(int unit, int pipe, int port, int *threshold)
{
    soc_error_t rc;
    uint32 reg_val32;
    int dch_instance, dch_inner_link;
    ...
    /* validation */
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, 0, SOC_RAMON_COSQ_WFQ_DCH_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    DNXC_IF_ERR_EXIT(READ_DCH_DYN_WFQ_WEIGHT_THD_REGISTEr(unit, dch_instance, &reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_mid_threshold_set(int unit, int pipe, int port, int threshold)
{
    soc_error_t rc;
    ...
    /* validation */
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, threshold, SOC_RAMON_COSQ_WFQ_DCM_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    for (i=dcm_loop_start; i < dcm_loop_end; i++) 
    {
        DNXC_IF_ERR_EXIT(READ_DCM_DYN_WFQ_WEIGHT_THD_REGISTERr(unit, i, &reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCM_DYN_WFQ_WEIGHT_THD_REGISTERr(unit, i, reg_val32));
    }
...
soc_error_t  
soc_ramon_cosq_pipe_mid_threshold_get(int unit, int pipe, int port, int *threshold)
{
    soc_error_t rc;
    uint32 reg_val32;
    int dcm_instance, dcm_inner_link;
    ...
    /* validation */
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, 0, SOC_RAMON_COSQ_WFQ_DCM_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    DNXC_IF_ERR_EXIT(READ_DCM_DYN_WFQ_WEIGHT_THD_REGISTERr(unit, dcm_instance, &reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_tx_threshold_set(int unit, int pipe, int port, int threshold)
{
    soc_error_t rc;
    ...
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, threshold, SOC_RAMON_COSQ_WFQ_DCL_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    {
        DNXC_IF_ERR_EXIT(READ_DCL_LINK_WEIGHT_CONFIG_THRESHOLDSr(unit, i, &reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCL_LINK_WEIGHT_CONFIG_THRESHOLDSr(unit, i, reg_val32));
    }
...
soc_error_t  
soc_ramon_cosq_pipe_tx_threshold_get(int unit, int pipe, int port, int *threshold)
{
    soc_error_t rc;
    uint32 reg_val32;
    int dcl_instance, dcl_inner_link;
    ...
    /* validation */
    rc = _soc_ramon_cosq_pipe_threshold_validate(unit, pipe, 0, SOC_RAMON_COSQ_WFQ_DCL_THRESH_MAX);
    DNXC_IF_ERR_EXIT(rc);
    ...
    DNXC_IF_ERR_EXIT(READ_DCL_LINK_WEIGHT_CONFIG_THRESHOLDSr(unit, dcl_instance, &reg_val32));
...
soc_error_t  
soc_ramon_cosq_pipe_rx_rate_set(int unit, int pipe, int port, soc_dnxf_cosq_shaper_mode_t shaper_mode, uint32 rate)
{
    ...
    /* validation */
    if (shaper_mode != soc_dnxf_cosq_shaper_mode_packet) 
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid shaping mode, only packet based shaper is allowed for dch\n")));
    }
    ...
    */ 
    ...
    max_cells_per_sec = SOC_DNXF_CONFIG(unit).core_clock_speed * 1000;                     
    ...
    if (period > SOC_RAMON_COSQ_RATE_DCH_PERIOD_MAX) 
    {
        period = SOC_RAMON_COSQ_RATE_DCH_PERIOD_MAX;
    }
    ...
    for (i=dch_loop_start; i < dch_loop_end; i++) 
    {
        DNXC_IF_ERR_EXIT(READ_DCH_LP_PIPE_SHAPER_REGISTERr(unit, i, &reg_val32));
        ...
        DNXC_IF_ERR_EXIT(WRITE_DCH_LP_PIPE_SHAPER_REGISTERr(unit, i, reg_val32));
    }
...
soc_error_t  
soc_ramon_cosq_pipe_rx_rate_get(int unit, int pipe, int port, soc_dnxf_cosq_shaper_mode_t *shaper_mode, uint32 *rate)
{
    ...
    */
    ...
    DNXC_IF_ERR_EXIT(READ_DCH_LP_PIPE_SHAPER_REGISTERr(unit, dch_instance, &reg_val32));
    ...
    *shaper_mode = soc_dnxf_cosq_shaper_mode_packet;
...
soc_error_t  
soc_ramon_cosq_pipe_tx_rate_set(int unit, int port, soc_dnxf_cosq_shaper_mode_t shaper_mode, uint32 rate)
{
    ...
    */ 
    ...
    core_clock_speed = SOC_DNXF_CONFIG(unit).core_clock_speed; /* in KHZ*/                  
    
    if (shaper_mode == soc_dnxf_cosq_shaper_mode_byte) /* Byte rate is given in KBps*/
    { 
        inc_size = 0;
        if (rate != 0)
        {
        ...
        COMPILER_64_SET(val64, 0, SOC_RAMON_COSQ_RATE_DCL_DEC_INC_VAL_MAX);
        if (COMPILER_64_GT(shaper_rate_val64, val64))
        {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid rate\n")));
        }
        else
        {
    ...
    else if (shaper_mode == soc_dnxf_cosq_shaper_mode_packet) /* cell rate is given in cells per second*/
    {
        rate = rate/1000; /* change from cells to Kilo cells (per second)*/
        inc_size = SOC_RAMON_COSQ_RATE_DCL_DEC_INC_VAL_MAX + 1;
        ...
        COMPILER_64_SET(val64, 0, SOC_RAMON_COSQ_RATE_DCL_DEC_INC_VAL_MAX);
        while ( COMPILER_64_GT(shaper_rate_val64, val64) && (i < SOC_RAMON_COSQ_RATE_DCL_DEC_INC_NOF_BITS) ) 
        {
    ...
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid shaping mode\n")));
    }
...
soc_error_t
soc_ramon_cosq_pipe_tx_rate_get(int unit, int port, soc_dnxf_cosq_shaper_mode_t *shaper_mode, uint32 *rate)
{
    uint32 inc_size, shaper_rate, reg_val32, core_clock_speed;
    int dcl_instance, dcl_inner_link;
    int rc;
    ...
    DNXC_IF_ERR_EXIT(READ_DCL_SHAPER_AND_WFQ_CFGr(unit, dcl_instance, &reg_val32));
    ...
    if (inc_size == 0)
    {
        *shaper_mode = soc_dnxf_cosq_shaper_mode_byte;
    }
    else
    {
        *shaper_mode = soc_dnxf_cosq_shaper_mode_packet;
...
soc_error_t  
soc_ramon_cosq_pipe_tx_rate_enable_set(int unit, int pipe, int port, int enable)
{
...
soc_error_t  
soc_ramon_cosq_pipe_tx_rate_enable_get(int unit, int pipe, int port, int *enable)
{
    uint32 reg_val32, pipes_bmp;
    int rc;
    int dcl_instance, dcl_inner_link;
