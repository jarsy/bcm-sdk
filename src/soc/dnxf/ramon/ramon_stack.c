/*
 * $Id: ramon_stack.c,v 1.3.164.1 Broadcom SDK $
 *
...
#define LINKS_ARRAY_SIZE (SOC_DNXF_MAX_NOF_LINKS/32 + (SOC_DNXF_MAX_NOF_LINKS%32 ? 1:0) )
 ...
 */
soc_error_t 
soc_ramon_stk_modid_set(int unit, uint32 fe_id)
{
    uint32 reg_val32;
    DNXC_INIT_FUNC_DEFS;
        
    DNXC_IF_ERR_EXIT(READ_ECI_GLOBAL_GENERAL_CFG_2r(unit, &reg_val32));
    soc_reg_field_set(unit, ECI_GLOBAL_GENERAL_CFG_2r, &reg_val32, DEV_ID_0f, fe_id);
    DNXC_IF_ERR_EXIT(WRITE_ECI_GLOBAL_GENERAL_CFG_2r(unit, reg_val32));
 ...
 */
soc_error_t 
soc_ramon_stk_modid_get(int unit, uint32* fe_id)
{
    uint32 reg_val32;
    DNXC_INIT_FUNC_DEFS;
...
soc_ramon_stk_valid_module_id_verify(int unit, bcm_module_t module, int verify_divisble_by_32)
{
    ...
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid module id , above max\n")));
    }

    if (module < 0)
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid module id , below zero \n")));
    }

    if (verify_divisble_by_32 && (module % SOC_RAMON_STK_FAP_GROUP_SIZE != 0) && (module != SOC_RAMON_STK_MAX_MODULE))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid module id , must be divisible by 32 \n")));
    }

    if (verify_divisble_by_32 && (module == 0))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Invalid module id , can't be 0 \n")));
    }
...
soc_ramon_stk_module_max_all_reachable_verify(int unit, bcm_module_t module)
{
    ...
    DNXC_IF_ERR_EXIT(soc_ramon_stk_valid_module_id_verify(unit, module, 1));

    /* Verify that AlrcMaxBaseIndex <= MaxBaseIndex */
    ...
    if ((module/SOC_RAMON_STK_FAP_GROUP_SIZE) > field_val) /* AlrcMaxBaseIndex to be configured is > MaxBaseIndex */
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Max Fap Id for all reachable vector can't be higher than Max number of Faps in the system configured \n")));
    }
...
soc_ramon_stk_module_max_fap_verify(int unit, bcm_module_t module)
{
    ...
    DNXC_IF_ERR_EXIT(READ_RTP_ALL_REACHABLE_CFGr(unit, &reg_val32));
    field_val = soc_reg_field_get(unit, RTP_ALL_REACHABLE_CFGr, reg_val32, ALRC_MAX_BASE_INDEXf);
    if (field_val > (module/SOC_RAMON_STK_FAP_GROUP_SIZE)) /* MaxBaseIndex to be configured is > AlrcMaxBaseIndex */
    {
        DNXC_IF_ERR_EXIT(soc_ramon_stk_module_max_all_reachable_set(unit, module));
...
soc_ramon_stk_module_all_reachable_ignore_id_set(int unit, bcm_module_t module, int arg)
{
    ...
    for (i=0 ; i < SOC_RAMON_STK_MAX_IGNORED_FAP_IDS ; i++) /* Loop over all ignoring register */
    {
        DNXC_IF_ERR_EXIT(READ_RTP_ALRC_EXCLUDEr(unit, i, &reg_val));
        ...
        if (valid_field == 1) /* The register is holding a fap id which is ignored */
        {
            if (id_field == module) 
            {
                if (arg == 0) /* need to clear this register */ 
                {
                    ...
                    DNXC_IF_ERR_EXIT(WRITE_RTP_ALRC_EXCLUDEr(unit, i, reg_val));
    ...
    {
        DNXC_IF_ERR_EXIT(READ_RTP_ALRC_EXCLUDEr(unit, first_empty_cell, &reg_val));
        ...
        DNXC_IF_ERR_EXIT(WRITE_RTP_ALRC_EXCLUDEr(unit, first_empty_cell, reg_val));
    }
exit:
    DNXC_FUNC_RETURN;
