...
void *soc_ramon_portmod_user_access_data[SOC_MAX_NUM_DEVICES] = {NULL};
...
soc_ramon_port_soc_init(int unit)
{
    ...
    for (port_i = 0; port_i < SOC_RAMON_NOF_LINKS ; ++port_i) {
        ...
        DNXC_IF_ERR_EXIT(soc_reg32_get(unit, ECI_POWERUP_CONFIGr, REG_PORT_ANY, 0, &puc));
        if(!SHR_BITGET(&puc,SOC_RAMON_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT) || SAL_BOOT_PLISIM) {
            /*if ECI_POWERUP_CONFIG[SOC_RAMON_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT]==0 then LC PLL config is taken from CMIC regs*/
            switch(lcpll) {
            case 0:
                DNXC_IF_ERR_EXIT(READ_ECI_MISC_PLL_0_CONFIGr(unit, reg_above_64_val));
                break;
            case 1:
                DNXC_IF_ERR_EXIT(READ_ECI_MISC_PLL_1_CONFIGr(unit, reg_above_64_val));
                break;
            case 2:
                DNXC_IF_ERR_EXIT(READ_ECI_MISC_PLL_2_CONFIGr(unit, reg_above_64_val));
                break;
            case 3:
            default:
                DNXC_IF_ERR_EXIT(READ_ECI_MISC_PLL_3_CONFIGr(unit, reg_above_64_val));
                break;
            }
            SOC_REG_ABOVE_64_WORD_GET(reg_above_64_val, &pll_control, 0);
            if(SOC_RAMON_PORT_PLL_CONFIG_OUT_125_MHZ_WORD_0 == pll_control) { /*control word for 125MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 125;
            } else if(SOC_RAMON_PORT_PLL_CONFIG_OUT_156_25_MHZ_WORD_0 == pll_control){ /*control word for 156.25MHz*/
                SOC_INFO(unit).port_refclk_int[port_i] = 156;
            } else {
                DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("Unrecognized LC-PLL value")));
            }
        } else {
            /*if ECI_POWERUP_CONFIG[SOC_RAMON_PORT_ECI_POWER_UP_CONFIG_STATIC_PLL_BIT]==1 then LC PLL config is taken from the PUC*/
            switch(lcpll) {
            case 0:
        ...
        if (SOC_DNXF_IS_FE13_ASYMMETRIC(unit) || SOC_DNXF_IS_FE2(unit)) /* if fe13 is in asymmetric mode || FE2 , need to change the clk frequence of changed quads to the values configured*/
        {
            if (port_i >= 104 && port_i <= 107 && SOC_DNXF_CONFIG(unit).fabric_clk_freq_out_quad_26 != -1)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = SOC_DNXF_CONFIG(unit).fabric_clk_freq_out_quad_26 ? 156 : 125;
            }
            else if (port_i >=140 && port_i <= 143 && SOC_DNXF_CONFIG(unit).fabric_clk_freq_out_quad_35 != -1)
            {
                SOC_INFO(unit).port_refclk_int[port_i] = SOC_DNXF_CONFIG(unit).fabric_clk_freq_out_quad_35 ? 156 : 125;
            }
    ...
    if(!SOC_WARM_BOOT(unit)) {
        ...
        sync_e_master_port = soc_dnxf_property_suffix_num_get(unit, 0, spn_SYNC_ETH_CLK_TO_PORT_ID_CLK, "", 1, SOC_DNXF_PROPERTY_UNAVAIL);
        sync_e_slave_port  = soc_dnxf_property_suffix_num_get(unit, 1, spn_SYNC_ETH_CLK_TO_PORT_ID_CLK, "", 1, SOC_DNXF_PROPERTY_UNAVAIL);
    
        if (sync_e_master_port != SOC_DNXF_PROPERTY_UNAVAIL)
        {
            DNXC_IF_ERR_EXIT(soc_ramon_port_sync_e_link_set(unit, 1, sync_e_master_port));
        }
    
        if (sync_e_slave_port != SOC_DNXF_PROPERTY_UNAVAIL)
        {
            DNXC_IF_ERR_EXIT(soc_ramon_port_sync_e_link_set(unit, 0, sync_e_slave_port));
        }

        sync_e_master_divider =  soc_dnxf_property_suffix_num_get(unit, 0, spn_SYNC_ETH_CLK_DIVIDER, "", 0, SOC_DNXF_PROPERTY_UNAVAIL);

        DNXC_IF_ERR_EXIT(soc_ramon_port_sync_e_divider_set(unit, sync_e_master_divider));
    }
...
soc_ramon_port_sync_e_link_set(int unit, int is_master, int port)
{
    ...
    if ((port < 0) || (port >= SOC_RAMON_NOF_LINKS))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("invalid port for syncE configuration")));
    }
    ...
    fsrd_block = port/SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd);

    DNXC_IF_ERR_EXIT(READ_ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr(unit, &reg_val));
    DNXC_IF_ERR_EXIT(READ_FSRD_SYNC_E_SELECTr(unit, fsrd_block, &reg_val2));
    ...
    if (is_master)
    {
        ...
        soc_reg_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, MASTER_CLK_SELf,  port % SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd));
    ...
    else
    {
        ...
        /*update fsrd register*/
        soc_reg_field_set(unit, FSRD_SYNC_E_SELECTr, &reg_val2, SLAVE_CLK_SELf,  port % SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd));
    }
    DNXC_IF_ERR_EXIT(WRITE_ECI_SELECT_OUTPUT_OF_SYNCHRONOUS_ETHERNET_PADSr(unit, reg_val));
    DNXC_IF_ERR_EXIT(WRITE_FSRD_SYNC_E_SELECTr(unit, fsrd_block, reg_val2));
...
soc_ramon_port_sync_e_divider_set(int unit, int divider)
{
    ...
    for (i = 0 ; i < SOC_DNXF_DEFS_GET(unit, nof_instances_mac_fsrd); i++)
    {
        ...
        if (!disabled)
        {
            DNXC_IF_ERR_EXIT(READ_FSRD_SYNC_E_SELECTr(unit, i, &reg_val));
            ...
            DNXC_IF_ERR_EXIT(WRITE_FSRD_SYNC_E_SELECTr(unit, i, reg_val));
        }
...
soc_ramon_port_sync_e_link_get(int unit, int is_master, int *port)
{
    ...
    *port = fsrd_block*SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd) + inner_fsrd_port;
...
STATIC soc_error_t
soc_ramon_port_config_get(int unit, soc_port_t port, int is_init_sequence, dnxc_port_fabric_init_config_t* port_config)
{
    DNXC_INIT_FUNC_DEFS;
    ...
    if(is_init_sequence) {
        DNXC_IF_ERR_EXIT(soc_dnxc_port_config_get(unit, port, port_config));
    }
...
soc_ramon_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp, int is_init_sequence)
{
    ...
    int rc = SOC_E_NONE, is_first_link, i, fsrd_block, fmac_block;
    soc_port_t port;
    ...
    if (is_init_sequence) {
        ...
        if(fw_load_method == phymodFirmwareLoadMethodExternal &&
           !(dnxc_device_block_for_feature(unit,DNXC_FABRIC_12_QUADS_FEATURE)) &&
           !(dnxc_device_block_for_feature(unit,DNXC_FABRIC_24_QUADS_FEATURE)) &&
           !(dnxc_device_block_for_feature(unit,DNXC_FABRIC_18_QUADS_FEATURE))) {
             broadcast_load = 1;
        }
    ...
    SOC_PBMP_ITER(pbmp, port) {
        ...
        rc = soc_ramon_port_first_link_in_fsrd_get(unit, port , &is_first_link, 1);
        DNXC_IF_ERR_EXIT(rc);
        ...
        if (is_first_link == 1)
        {
            ...
            fmac_block = fsrd_block * SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd);

            rc = soc_ramon_port_update_valid_block_database(unit, SOC_BLK_FSRD, fsrd_block, 1);
            DNXC_IF_ERR_EXIT(rc);

            for (i = fmac_block; i < fmac_block +  SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd) ; i++)
            {
        ...
        rc = soc_ramon_port_config_get(unit, port, is_init_sequence, &port_config);
        DNXC_IF_ERR_EXIT(rc);

        stage = broadcast_load ? dnxc_port_init_until_fw_load : dnxc_port_init_full;
    ...
    {
        rc = soc_dnxc_fabric_broadcast_firmware_loader(unit, tscf_ucode_len, tscf_ucode);
        DNXC_IF_ERR_EXIT(rc);
    }
    ...
    SOC_PBMP_ITER(pbmp, port) {
        ...
        if(broadcast_load)
        {
            ...
            rc= soc_dnxc_fabric_port_probe(unit, port, dnxc_port_init_resume_after_fw_load, fw_verify, &port_config);
            DNXC_IF_ERR_EXIT(rc);
        }
...
soc_ramon_port_update_fsrd_block(int unit, soc_port_t port, int enable)
{
    ...
    DNXC_IF_ERR_EXIT(READ_ECI_BLOCKS_POWER_DOWNr(unit, &reg_val64));
    ...
    switch (fsrd_block)
    {
        ...
        case 11:
            ...
            break;
        default:
            DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("invalid fsrd block")));
            break;
    }

    DNXC_IF_ERR_EXIT(WRITE_ECI_BLOCKS_POWER_DOWNr(unit, reg_val64));
...
soc_error_t
soc_ramon_port_first_link_in_fsrd_get(int unit, soc_port_t port, int *is_first_link, int enable)
{
    int fsrd_block, i;
    ...
    for (i=fsrd_block*SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd) ; i<fsrd_block*SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd) + SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd); i++) /* create a mask for all the links in the fsrd*/
    {
...
soc_ramon_dynamic_port_restore(int unit)
{
    ...
    for (port = 0; port < SOC_DNXF_DEFS_GET(unit, nof_links) ; port++) {
        ...
        DNXC_IF_ERR_EXIT(portmod_port_is_valid(unit, port, &valid));
...
soc_ramon_port_init(int unit)
{
    int rc;
    int quads_in_fsrd;
    portmod_pm_instances_t pm_types_and_instances[] = {{portmodDispatchTypeDnx_fabric, SOC_RAMON_NOF_INSTANCES_MAC}};
    DNXC_INIT_FUNC_DEFS;
    
    if(soc_ramon_portmod_user_access_data[unit] != NULL){
        DNXC_EXIT_WITH_ERR(SOC_E_INIT, (_BSL_DNXC_MSG("unit user data memory already allocated")));
    }

    quads_in_fsrd = SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd);

    /*portmod create for the unit*/
    rc = portmod_create(unit, 0, SOC_RAMON_NOF_LINKS, SOC_RAMON_NOF_LINKS, 1 , pm_types_and_instances);
    DNXC_IF_ERR_EXIT(rc);
    rc = soc_dnxc_fabric_pms_add(unit, SOC_RAMON_NOF_INSTANCES_MAC, 0, FALSE, quads_in_fsrd, soc_ramon_core_address_get, &soc_ramon_portmod_user_access_data[unit]);
    DNXC_IF_ERR_EXIT(rc);


    if (SOC_WARM_BOOT(unit))
    {
        DNXC_IF_ERR_EXIT(soc_ramon_dynamic_port_restore(unit));
    }
...
soc_ramon_port_deinit(int unit)
{
    ...
    if(soc_ramon_portmod_user_access_data[unit] != NULL){
        sal_free(soc_ramon_portmod_user_access_data[unit]);
        soc_ramon_portmod_user_access_data[unit] = NULL;
    }
...
soc_ramon_port_detach(int unit, soc_port_t port)
{
    ...
    rc = soc_ramon_port_first_link_in_fsrd_get(unit, port , &is_first_link, 0);
...
{
    DNXC_INIT_FUNC_DEFS;
    *speed=SOC_RAMON_PORT_SPEED_MAX;
 ...
 */
soc_error_t 
soc_ramon_port_phy_enable_set(int unit, soc_port_t port, int enable)
{
 ...
 */
soc_error_t 
soc_ramon_port_phy_enable_get(int unit, soc_port_t port, int *enable)
{
    int portmod_enable_flags=0;
    DNXC_INIT_FUNC_DEFS;
...
soc_ramon_port_speed_get(int unit, soc_port_t port, int *speed)
{
    ...
    DNXC_IF_ERR_EXIT(portmod_port_interface_config_get(unit, port, &config, PORTMOD_INIT_F_EXTERNAL_MOST_ONLY));
    *speed = config.speed;
exit:
    DNXC_FUNC_RETURN; 
}
...
soc_ramon_port_speed_set(int unit, soc_port_t port, int speed)
{
    ...
    DNXC_IF_ERR_EXIT(portmod_port_interface_config_get(unit, port, &config, 
                                          PORTMOD_INIT_F_EXTERNAL_MOST_ONLY));
    config.speed = speed;
    DNXC_IF_ERR_EXIT(portmod_port_interface_config_set(unit, port, &config, 
                                          PORTMOD_INIT_F_EXTERNAL_MOST_ONLY));
...
soc_ramon_port_interface_set(int unit, soc_port_t port, soc_port_if_t intf)
{
    ...
    DNXC_IF_ERR_EXIT(dnxc_soc_to_phymod_ref_clk(unit, SOC_INFO(unit).port_refclk_int[port], &ref_clk));
...
soc_error_t
soc_ramon_port_burst_control_set(int unit, soc_port_t port, soc_dnxc_loopback_mode_t loopback)
{
    ...
    DNXC_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &fmac_block, &fmac_inner_link, SOC_BLK_FMAC)));

    /*Comma burst configuration*/
    DNXC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr(unit, fmac_block, fmac_inner_link, &reg32_val));
    if (loopback == soc_dnxc_loopback_mode_mac_outer || loopback == soc_dnxc_loopback_mode_mac_async_fifo)
    {
        ...
        DNXC_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_device_mode_get,(unit, port, 0/*tx*/, &link_mode)));
        ...
        switch (link_mode)
        {
            ...
            case soc_dnxf_fabric_link_device_mode_repeater:
                ...
                break;
            default:
                DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("invalid link mode")));
                break;
        }
    }
    DNXC_IF_ERR_EXIT(WRITE_FMAC_FMAL_TX_COMMA_BURST_CONFIGURATIONr(unit, fmac_block, fmac_inner_link, reg32_val));
    ...
    switch (block_type)
    {
        case SOC_BLK_FSRD:
            if(block_number < SOC_MAX_NUM_FSRD_BLKS) {
                ...
                DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("soc_dnxf_drv_update_block_database: too much FSRD blocks")));
            }
            break;
        ...
        case SOC_BLK_FMAC:
            if(block_number < SOC_MAX_NUM_FMAC_BLKS) {
                ...
                DNXC_EXIT_WITH_ERR(SOC_E_INTERNAL, (_BSL_DNXC_MSG("soc_dnxf_drv_update_block_database: too much FMAC blocks")));
            }
            break;

        default:
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("soc_dnxf_drv_update_block_database: block not supported/recognized")));
            break;
...
soc_ramon_port_pump_enable_set(int unit, soc_port_t port, int enable)
{
    ...
    DNXC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr_REG32(unit, blk, inner_link, &reg32_val));
...
soc_ramon_port_dynamic_soc_init(int unit, soc_port_t port, int is_first_link)
{
    ...
    DNXC_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_link_to_block_mapping, (unit, port, &fmac_block, &fmac_inner_link, SOC_BLK_FMAC)));
    fsrd_block = port/SOC_DNXF_DEFS_GET(unit, nof_links_in_fsrd);
    fmac_first_block = fsrd_block * SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd);
    

    /*Leacky Bucket*/
    if (is_first_link)
    {
        for (blk = fmac_first_block; blk < fmac_first_block +  SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd) ; blk++)
        {
            DNXC_IF_ERR_EXIT(READ_FMAC_LEAKY_BUCKET_CONTROL_REGISTERr(unit, blk, &reg32_val));
            soc_reg_field_set(unit, FMAC_LEAKY_BUCKET_CONTROL_REGISTERr, &reg32_val, BKT_FILL_RATEf, SOC_DNXF_CONFIG(unit).fabric_mac_bucket_fill_rate);
            ...
            DNXC_IF_ERR_EXIT(WRITE_FMAC_LEAKY_BUCKET_CONTROL_REGISTERr(unit, blk, reg32_val));
        }
    }
    ...
    if(SOC_DNXF_IS_FE13(unit)) {
        ...
        {
            DNXC_IF_ERR_EXIT(READ_FMAC_LINK_TOPO_MODE_REG_0r(unit, fmac_block, &reg32_val));
            ...
            *field = soc_reg_field_get(unit, FMAC_LINK_TOPO_MODE_REG_0r, reg32_val, LINK_TOPO_MODE_1f);
            ...
            DNXC_IF_ERR_EXIT(WRITE_FMAC_LINK_TOPO_MODE_REG_0r(unit, fmac_block, reg32_val));
        }
    }

    /*LLFC enable*/
    DNXC_IF_ERR_EXIT(READ_FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr(unit, fmac_block, &reg32_val));
    ...
    *field = soc_reg_field_get(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, reg32_val, LNK_LVL_FC_RX_ENf);
    SHR_BITSET(field, fmac_inner_link);
    soc_reg_field_set(unit, FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr, &reg32_val, LNK_LVL_FC_RX_ENf, *field);
    DNXC_IF_ERR_EXIT(WRITE_FMAC_LINK_LEVEL_FLOW_CONTROL_ENABLE_REGISTERr(unit, fmac_block, reg32_val));

    /*RX LOS interrupt*/
    DNXC_IF_ERR_EXIT(READ_FMAC_FPS_CONFIGURATION_RX_SYNCr(unit, fmac_block, fmac_inner_link, &reg32_val));
    ...
    DNXC_IF_ERR_EXIT(WRITE_FMAC_FPS_CONFIGURATION_RX_SYNCr(unit, fmac_block, fmac_inner_link, reg32_val));

    /*Link pump*/
    DNXC_IF_ERR_EXIT(READ_FMAC_FMAL_TX_GENERAL_CONFIGURATIONr_REG32(unit, fmac_block, fmac_inner_link, &reg32_val));
    ...
    DNXC_IF_ERR_EXIT(READ_FMAC_REG_0102r(unit, fmac_block, &reg32_val));
    *field = soc_reg_field_get(unit, FMAC_REG_0102r, reg32_val, FIELD_0_3f);
    ...
    DNXC_IF_ERR_EXIT(WRITE_FMAC_REG_0102r(unit, fmac_block, reg32_val));
    ...
    if (SOC_DNXF_IS_FE13_ASYMMETRIC(unit) || SOC_DNXF_IS_FE2(unit)) 
    {
        if (is_first_link)
        {
            for (blk = fmac_first_block; blk < fmac_first_block +  SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd) ; blk++)
            {
                if (SOC_DNXF_CONFIG(unit).fabric_clk_freq_in_quad_26 != -1 && blk == 26)
                {
                    DNXC_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 8, 2, &reg32_val)); /* 8*3 + 2 */
                    soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg32_val, SRD_QUAD_N_LCREF_ENf, 0);
                    DNXC_IF_ERR_EXIT(WRITE_FSRD_SRD_QUAD_CTRLr(unit, 8, 2, reg32_val));       
                }
                if (SOC_DNXF_CONFIG(unit).fabric_clk_freq_in_quad_35 != -1 && blk == 35)
                {
                    DNXC_IF_ERR_EXIT(READ_FSRD_SRD_QUAD_CTRLr(unit, 11, 2, &reg32_val)); /* 11*3 + 2 */
                    soc_reg_field_set(unit, FSRD_SRD_QUAD_CTRLr, &reg32_val, SRD_QUAD_N_LCREF_ENf, 0);
                    DNXC_IF_ERR_EXIT(WRITE_FSRD_SRD_QUAD_CTRLr(unit, 11, 2, reg32_val));
                }
            }
        }
    }
...
soc_ramon_port_quad_disabled(int unit, int quad, int *disabled)
{
    ...
    if (dnxc_device_block_for_feature(unit,DNXC_FABRIC_18_QUADS_FEATURE)) {
        if ((quad>=6 && quad<=8)||(quad>=15 && quad<=17)||(quad>=21 && quad<=26)||(quad>=30 && quad<=35)) {
            *disabled = 1;
        } else {
            *disabled = 0;
        }
    } else if (dnxc_device_block_for_feature(unit,DNXC_FABRIC_24_QUADS_FEATURE)) {
        /*only fsrds 0,1, 3, 4, 6, 7, 9, 10*/
        fsrd_id = quad / SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd);
        if ((fsrd_id % 3 == 0) ||
            ((fsrd_id - 1) % 3 == 0))
        {
            *disabled = 0;
    ...
    } else if (dnxc_device_block_for_feature(unit,DNXC_FABRIC_12_QUADS_FEATURE)) {
        /*only fsrds 0, 3, 6, 9*/
        fsrd_id = quad / SOC_DNXF_DEFS_GET(unit, nof_quads_in_fsrd);
        if (fsrd_id % 3 == 0)
        {
