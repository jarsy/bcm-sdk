/*
 * $Id: dnxf_diag.c,v 1.11 Broadcom SDK $
 *
...
void soc_dnxf_queues_info_init(int unit, soc_dnxf_queues_info_t* fe_queues_info)
{
    ...
    for (i=0;i<SOC_DNXF_MAX_NOF_PIPES;i++)
    {
        for (j=0; j < SOC_DNXF_DEFS_GET(unit, nof_instances_dch);j++)
        {
		...
		for (j=0; j < SOC_DNXF_DEFS_GET(unit, nof_instances_dcma); j++)
		{
		...
		for (j=0; j < SOC_DNXF_DEFS_GET(unit, nof_instances_dcmb); j++)
		{
...
const soc_dnxf_diag_flag_str_t soc_dnxf_diag_flag_str_fabric_cell_snake_test_stages[] =  {
    ...
    {SOC_DNXF_SNAKE_TEST_FAILURE_STAGE_DATA_CELL_FILTER_READ_COMMAND,    "DATA_CELL_FILTER_READ_COMMAND"},
    {SOC_DNXF_SNAKE_TEST_FAILURE_STAGE_CONTROL_CELL_FILTER_READ_COMMAND, "CONTROL_CELL_FILTER_READ_COMMAND"},
    {0x0,                                                               NULL},
};
