/*
 * $Id: ramon_fe1600_diag.c,v 1.11 Broadcom SDK $
 *
...
soc_ramon_fe1600_counters_get_info(int unit,soc_dnxf_counters_info_t* fe_counters_info){
    ...
    /*       **********************DCH PART ************************************/
        ...
        {
             DNXC_IF_ERR_EXIT(READ_DCH_DCH_TOTAL_CELL_CNT_Pr(unit,i,&reg_val));
             DNXC_IF_ERR_EXIT(READ_DCH_DCH_TOTAL_OUT_CELL_CNT_Pr(unit,i,&reg_val2));
             DNXC_IF_ERR_EXIT(READ_DCH_FIFO_DISCARD_COUNTER_Pr(unit,i,&reg_val3));
             DNXC_IF_ERR_EXIT(READ_DCH_DCH_REORD_DISCARD_COUNTER_Pr(unit,i,&reg_val4));
             DNXC_IF_ERR_EXIT(READ_DCH_UN_REACH_DEST_0_Pr(unit,i,&reg_val64));
             DNXC_IF_ERR_EXIT(READ_DCH_DCH_OVERFLOWS_AND_FIFOS_STATUSES_Pr(unit,i,&reg_val6));
        ...
        if (nof_pipes==2)    /* secondary */
        {
            curr_max=0;

                for (i=0;i< SOC_DNXF_DEFS_GET(unit, nof_instances_dch) ;i++) 
                {
                    DNXC_IF_ERR_EXIT(READ_DCH_DCH_TOTAL_CELL_CNT_Sr(unit,i,&reg_val));
                    DNXC_IF_ERR_EXIT(READ_DCH_DCH_TOTAL_OUT_CELL_CNT_Sr(unit,i,&reg_val2));
                    DNXC_IF_ERR_EXIT(READ_DCH_FIFO_DISCARD_COUNTER_Sr(unit,i,&reg_val3));
                    DNXC_IF_ERR_EXIT(READ_DCH_DCH_REORD_DISCARD_COUNTER_Sr(unit,i,&reg_val4));
                    DNXC_IF_ERR_EXIT(READ_DCH_UN_REACH_DEST_0_Sr(unit,i,&reg_val64));
                    DNXC_IF_ERR_EXIT(READ_DCH_DCH_OVERFLOWS_AND_FIFOS_STATUSES_Sr(unit,i,&reg_val6));
    ...
    /*              ************************DCM PART **********************************                 */
            ...
            /*A*/
            curr_max=0;
            for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMA;i++)
            {
                DNXC_IF_ERR_EXIT(READ_DCMA_DCMUA_TOTAL_IN_CELL_CNT_0r(unit,i,&reg_val));
                ...
                DNXC_IF_ERR_EXIT(READ_DCMA_DCMUA_TOTAL_IN_CELL_CNT_1r(unit,i,&reg_val2));
                ...
                DNXC_IF_ERR_EXIT(READ_DCMA_DCMUA_DROPPED_TOTAL_CNTr(unit,i,&reg_val3));

                dcm_drp+=soc_reg_field_get(unit,DCMA_DCMUA_DROPPED_TOTAL_CNTr,reg_val3,DROPPED_TOTALf);

                DNXC_IF_ERR_EXIT(READ_DCMA_DCMUA_DCM_MAX_OCUPP_PRIMERYr(unit,i,&reg_val64));


                for (j=0;j<=27;j=j+9)                   /*looping over the dch a/b/c/d fifo to find the max */
                {                                       /* dcha: 0-8, dchb: 9-17 , dchc: 18-26, dchd: 27-35 */
                    COMPILER_64_SHR(reg_val64,j? 9:0);  /* first time don't shift */
            ...
            dcm_drp=0;
            curr_max=0;
            for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMB;i++)
            {
                DNXC_IF_ERR_EXIT(READ_DCMB_DCMUB_TOTAL_IN_CELL_CNT_0r(unit,i,&reg_val));
                ...
                DNXC_IF_ERR_EXIT(READ_DCMB_DCMUB_TOTAL_IN_CELL_CNT_1r(unit,i,&reg_val2));
                ...
                DNXC_IF_ERR_EXIT(READ_DCMB_DCMUB_DROPPED_TOTAL_CNTr(unit,i,&reg_val3));

                dcm_drp+=soc_reg_field_get(unit,DCMB_DCMUB_DROPPED_TOTAL_CNTr,reg_val3,DROPPED_TOTALf);

                DNXC_IF_ERR_EXIT(READ_DCMB_DCMUB_DCM_MAX_OCUPP_PRIMERYr(unit,i,&reg_val64));
            ...
            if (nof_pipes==2) /* Dealing with dcmm */
            {
                dcm_total=0;
                dcm_drp=0;
                curr_max=0;
                for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMA;i++)
                {
                    DNXC_IF_ERR_EXIT(READ_DCMA_DCMMA_TOTAL_IN_CELL_CNT_0r(unit,i,&reg_val));
                    dcm_total+=soc_reg_field_get(unit,DCMA_DCMUA_TOTAL_IN_CELL_CNT_0r,reg_val,TOTAL_IN_CELL_CNT_Af);
                    dcm_total+=soc_reg_field_get(unit,DCMA_DCMUA_TOTAL_IN_CELL_CNT_0r,reg_val,TOTAL_IN_CELL_CNT_Bf);

                    DNXC_IF_ERR_EXIT(READ_DCMA_DCMMA_TOTAL_IN_CELL_CNT_1r(unit,i,&reg_val2));
                    dcm_total+=soc_reg_field_get(unit,DCMA_DCMUA_TOTAL_IN_CELL_CNT_1r,reg_val2,TOTAL_IN_CELL_CNT_Cf);
                    dcm_total+=soc_reg_field_get(unit,DCMA_DCMUA_TOTAL_IN_CELL_CNT_1r,reg_val2,TOTAL_IN_CELL_CNT_Df);

                    DNXC_IF_ERR_EXIT(READ_DCMA_DCMMA_DROPPED_TOTAL_CNTr(unit,i,&reg_val3));

                    dcm_drp+=soc_reg_field_get(unit,DCMA_DCMMA_DROPPED_TOTAL_CNTr,reg_val3,DROPPED_TOTALf);

                    DNXC_IF_ERR_EXIT(READ_DCMA_DCMMA_DCM_MAX_OCUPP_SECONDARYr(unit,i,&reg_val64));
                ...
                for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMB;i++)
                {
                    DNXC_IF_ERR_EXIT(READ_DCMB_DCMMB_TOTAL_IN_CELL_CNT_0r(unit,i,&reg_val));
                    ...
                    DNXC_IF_ERR_EXIT(READ_DCMB_DCMMB_TOTAL_IN_CELL_CNT_1r(unit,i,&reg_val2));
                    ...
                    DNXC_IF_ERR_EXIT(READ_DCMB_DCMMB_DROPPED_TOTAL_CNTr(unit,i,&reg_val3));

                    dcm_drp+=soc_reg_field_get(unit,DCMB_DCMMB_DROPPED_TOTAL_CNTr,reg_val3,DROPPED_TOTALf);

                    DNXC_IF_ERR_EXIT(READ_DCMB_DCMMB_DCM_MAX_OCUPP_SECONDARYr(unit,i,&reg_val64));
     ...
     /*         *****************************END OF DCM PART *****************************************/
            ...
            {
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_TOTAL_IN_CELL_CNT_Pr(unit,i,&reg_val));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_TOTAL_OUT_CELL_CNT_Pr(unit,i,&reg_val2));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_0_CNT_Pr(unit,i,&reg_val3));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_1_CNT_Pr(unit,i,&reg_val4));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_2_CNT_Pr(unit,i,&reg_val5));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_3_CNT_Pr(unit,i,&reg_val6));
                DNXC_IF_ERR_EXIT(READ_DCL_MAX_OCUPANCY_FIFO_Pr(unit,i,&reg_val7));
            ...
            for (i=0;i<SOC_DNXF_DEFS_GET(unit, nof_instances_dcl);i++)   
            {
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_TOTAL_IN_CELL_CNT_Sr(unit,i,&reg_val));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_TOTAL_OUT_CELL_CNT_Sr(unit,i,&reg_val2));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_0_CNT_Sr(unit,i,&reg_val3));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_1_CNT_Sr(unit,i,&reg_val4));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_2_CNT_Sr(unit,i,&reg_val5));
                DNXC_IF_ERR_EXIT(READ_DCL_DCL_DROPPED_P_3_CNT_Sr(unit,i,&reg_val6));
                DNXC_IF_ERR_EXIT(READ_DCL_MAX_OCUPANCY_FIFO_Sr(unit,i,&reg_val7));
            ...
            /* DCH DCMU FLOW CONTROL*/
        
            for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCH;i++)
            {
                DNXC_IF_ERR_EXIT(READ_DCH_DCMU_FLOW_CNTROL_CNTr(unit,i,&reg_val));  
            ...
            /*  DCM DCL FLOW CONTROL */

            for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMA;i++)
            {
                DNXC_IF_ERR_EXIT(READ_DCMA_DCLU_FLOW_CNTROL_CNTr(unit,i,&reg_val));
                DNXC_IF_ERR_EXIT(READ_DCMB_DCLU_FLOW_CNTROL_CNTr(unit,i,&reg_val2));
            ...
            for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_CCS;i++)
            {
                ...
                DNXC_IF_ERR_EXIT(READ_CCS_FLOW_STATUS_CELLS_CNTr(unit,i,&reg_val2));
                DNXC_IF_ERR_EXIT(READ_CCS_CREDIT_CELLS_CNTr(unit,i,&reg_val3));
                DNXC_IF_ERR_EXIT(READ_CCS_REACHABILITY_CELLS_CNTr(unit,i,&reg_val4));
                DNXC_IF_ERR_EXIT(READ_CCS_UNREACHABLE_DESTINATION_CELLS_CNTr(unit,i,&reg_val5));
...
soc_ramon_fe1600_queues_get_info(int unit,soc_dnxf_queues_info_t* fe_queues_info)
{
    ...
    for (i=0;i<SOC_DNXF_DEFS_GET(unit, nof_instances_dch);i++) 
    {
        DNXC_IF_ERR_EXIT(READ_DCH_DCH_OVERFLOWS_AND_FIFOS_STATUSES_Pr(unit,i,&reg_val));

        nof_fifo=soc_reg_field_get(unit,DCH_DCH_OVERFLOWS_AND_FIFOS_STATUSES_Pr,reg_val,MAX_IFMF_NUMA_Pf);
        nof_link=i*SOC_RAMON_FE1600_NOF_LINKS_IN_DCQ+nof_fifo;
        ...
        nof_link=i*SOC_RAMON_FE1600_NOF_LINKS_IN_DCQ+SOC_RAMON_FE1600_NOF_LINKS_IN_DCQ/2+nof_fifo;
    ...
    if (nof_pipes==2) /*secondary*/
    {
        for (i=0;i<SOC_DNXF_DEFS_GET(unit, nof_instances_dch);i++) 
        {
            DNXC_IF_ERR_EXIT(READ_DCH_DCH_OVERFLOWS_AND_FIFOS_STATUSES_Sr(unit,i,&reg_val));
    ...
    if(nof_pipes==2)/* secondary */
    {
        for (i=0;i<SOC_RAMON_FE1600_NOF_INSTANCES_DCMA;i++)
        {
            ...
            for (j=0;j<4;j++) /* dcma is at 0:8,dcmb 9:17,dcmc 18:26, dcmd 27:35 */
            {
                COMPILER_64_SHR(reg_val64,j? 9:0); /* first time don't shift */
                temp1=COMPILER_64_LO(reg_val64);
                temp1=temp1 & mask_9_bits;
    ...
    for (i=0;i<SOC_DNXF_DEFS_GET(unit, nof_instances_dcl);i++)
    {   
        DNXC_IF_ERR_EXIT(READ_DCL_MAX_OCUPANCY_FIFO_Pr(unit,i,&reg_val));
    ...
    {   
         for (i=0;i<SOC_DNXF_DEFS_GET(unit, nof_instances_dcl);i++) 
         {   
             DNXC_IF_ERR_EXIT(READ_DCL_MAX_OCUPANCY_FIFO_Sr(unit,i,&reg_val));
...
const soc_dnxf_diag_flag_str_t soc_ramon_fe1600_diag_flag_str_interrupts_names[] = 
{
    {0, "ECI_GLOBAL"},
    {1, ""},
    {2, "DCH0"},
...
soc_ramon_fe1600_diag_fabric_cell_snake_test_interrupts_name_get(int unit, const soc_dnxf_diag_flag_str_t **intr_names)
{
    ...
    *intr_names = soc_ramon_fe1600_diag_flag_str_interrupts_names;
