...
#define OS_ILKN_IS_PORT_QMX_AND_OVER_FABRIC_GET(port, pm_info, is_port_qmx_and_over_fabric)                       \
    do {                                                                                                          \
        int is_over_fabric, core_id;                                                                              \
        OS_ILKN_CORE_ID_GET(port, pm_info, core_id);                                                              \
        _SOC_IF_ERR_EXIT(PM_ILKN_IS_PORT_OVER_FABRIC_GET(unit, pm_info, &is_over_fabric, core_id));               \
        is_port_qmx_and_over_fabric = (pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en == 2 && is_over_fabric);   \
    } while(0)
...
#define OS_ILKN_IS_PORT_OVER_FABRIC_GET(port, pm_info, is_port_over_fabric)                                       \
    do {                                                                                                          \
        ...
        OS_ILKN_CORE_ID_GET(port, pm_info, core_id);                                                              \
        _SOC_IF_ERR_EXIT(PM_ILKN_IS_PORT_OVER_FABRIC_GET(unit, pm_info, &is_over_fabric, core_id));               \
        is_port_over_fabric = (pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en && is_over_fabric);                \
    } while(0)
...
int pmOsIlkn_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pmOsIlkn", NULL, NULL, VERSION(2), 1, SOC_WB_ENGINE_POST_RELEASE);
    _SOC_IF_ERR_EXIT(rv);
...
pmOsILKN_port_logical_lane_order_init(int unit, int port, pm_info_t pm_info)
{
    ...
    for (pm = 0; pm < OS_ILKN_MAX_ILKN_AGGREGATED_PMS; pm++)
    {
        ...
        if (is_pm_aggregated) {
            ...
            /*get PM lane map*/
            rv = __portmod__dispatch__[ilkn_data->pms[pm]->type]->f_portmod_port_lane_map_get(unit, port, ilkn_data->pms[pm], &pm_lane_map);
            _SOC_IF_ERR_EXIT(rv);
...
int pmOsILKN_port_polarity_override_set(int unit, int port, pm_info_t pm_info) {
    ...
    for (i = 0; i < OS_ILKN_MAX_ILKN_AGGREGATED_PMS; ++i) {
        ...
        if (is_pm_aggregated) {
            ...
            _SOC_IF_ERR_EXIT(__portmod__dispatch__[ilkn_data->pms[i]->type]->f_portmod_port_lane_map_get(unit, port, ilkn_data->pms[i], &pm_lane_map));
...
int pmOsILKN_port_attach(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    portmod_pbmp_t mirror_bmp, phys_temp, temp_phys_aligned;
    ...
    int is_port_over_fabric, is_port_qmx_and_over_fabric, is_pm_aggregated=0;
    ...
    OS_ILKN_IS_PORT_QMX_AND_OVER_FABRIC_GET(port, pm_info, is_port_qmx_and_over_fabric);
    ...
    if (pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en) /* ILKN over Fabric */
    {
        ...
        {
            lane = is_port_over_fabric ? port_i : port_i - 1; /*Align one base / zero base*/
            if (lane < 0) { /* Coverity protection*/
                _SOC_EXIT_WITH_ERR(SOC_E_FAIL, (_SOC_MSG("lane index %d is out of bounds"), lane));
            }
            lane =  lane % OS_ILKN_TOTAL_LANES_PER_CORE;    /*Align to per core lane numbers*/
            PORTMOD_PBMP_PORT_ADD(temp_phys_aligned, lane);
            lane = is_port_qmx_and_over_fabric ? lane + 8 : lane; /*Align to QMX over fabric*/
    ...
    else
    {
        ...
        for (i=0 ; i < OS_ILKN_MAX_ILKN_AGGREGATED_PMS ; i++)
        {
            ...
            if (is_pm_aggregated)
            {
                ...
                _SOC_IF_ERR_EXIT(__portmod__dispatch__[ilkn_data->pms[i]->type]->f_portmod_pm_phys_get(unit, ilkn_data->pms[i], &phys_temp));
                PORTMOD_PBMP_AND(phys_temp, ilkn_data->phys);
                PORTMOD_PBMP_ITER(phys_temp, port_i)
                {
    ...
    /*penetrating temp_phys_aligned to WB engine*/
    _SOC_IF_ERR_EXIT(PM_ILKN_PHYS_ALIGNED_SET(unit, pm_info, temp_phys_aligned, ilkn_core_id));
...
pmOsILKN_port_logical_lane_order_set(int unit, int port, pm_info_t pm_info, const int* lane_order, int lane_order_size)
{
    ...
    int core_id;
    int port_i;
    int field_index, reg_index, is_port_qmx_and_over_fabric;
    uint32 reg_val;
    soc_pbmp_t temp_phys_aligned;
    SOC_INIT_FUNC_DEFS;
    ...
    PORTMOD_PBMP_ITER(temp_phys_aligned, port_i){
        ...
        if (is_port_qmx_and_over_fabric) {
            lane_index += 8;
        }
...
int pmOsILKN_port_detach(int unit, int port, pm_info_t pm_info)
{
    ...
    pm_info->pm_data.pmOsIlkn_db->is_over_fabric_en = 0;
...
{
    int i, core_id, arr_inx, rv, first_phy_in_pm, active_phys_in_pm, is_port_qmx_and_over_fabric, is_pm_aggregated=0;
    pmOsIlkn_internal_t *ilkn_data ;
    ...
    *nof_phys = arr_inx = 0;
    ...
    for (i = 0; i < OS_ILKN_MAX_ILKN_AGGREGATED_PMS; ++i) {
        ...
        if (is_pm_aggregated) {
            ...
            if (params->lane != PORTMOD_ALL_LANES_ARE_ACTIVE){
                if (is_port_qmx_and_over_fabric && i > 1) {
                    phy_mask = 0xf << ((i - 2) * OS_ILKN_LANES_PER_PHY_CORE);
                } else {
        ...
        if ((params_copy.lane >= 0) && (is_port_qmx_and_over_fabric == 0 || i > 1)) {
