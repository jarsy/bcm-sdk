...
int portmod_port_legacy_ability_remote_get(int unit, int port, phymod_autoneg_ability_t* an_ability) {
    int rv = SOC_E_NONE;
    soc_port_ability_t port_ability;
    ...
    rv = portmod_port_interface_config_get(unit, port, &interface_config, PORTMOD_INIT_F_EXTERNAL_MOST_ONLY);
    if (rv < SOC_E_NONE) {
        return rv;
    }
    port_num_lanes = interface_config.port_num_lanes;
    line_interface = portmod_ability.interface;
