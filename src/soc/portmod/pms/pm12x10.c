...
int pm12x10_pm_interface_type_is_supported(int unit, soc_port_if_t interface, int* is_supported)
{
    *is_supported = FALSE;
    ...
    if((interface == SOC_PORT_IF_CAUI) ||  (interface == SOC_PORT_IF_CR10)) {
       *is_supported = TRUE;
    }
    return SOC_E_NONE;
}
                          ...
                          int flags, int reg_addr, uint32* value)
{
    ...
    rv = PM_4X10_DRIVER(pm_info, 2)->f_portmod_port_phy_reg_read(unit, 
                      port, PM_4X10_INFO(pm_info, 2), flags, reg_addr, value);
    _SOC_IF_ERR_EXIT(rv);
                           ...
                           int flags, int reg_addr, uint32 value)
{
    ...
    for( i = 0 ; i < 3 ; i++){
        rv = PM_4X10_DRIVER(pm_info, i)->f_portmod_port_phy_reg_write(unit, 
                      port, PM_4X10_INFO(pm_info, i), flags, reg_addr, value);
        _SOC_IF_ERR_EXIT(rv);
    }
