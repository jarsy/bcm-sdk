...
#define PM8X50_FABRIC_LANES_PER_CORE (4)
#define MAX_PORTS_PER_PM8X50_FABRIC_PM (4)
...
struct pm8x50_fabric_s{
    ...
    portmod_lane_connection_t phy_chain[MAX_PHYN][PM8X50_FABRIC_LANES_PER_CORE];
...
STATIC
int pm8x50_fabric_phy_tx_lane_control_set(int unit, int port, pm_info_t pm_info, phymod_phy_tx_lane_control_t control)
{
...
int pm8x50_fabric_lane_phychain_core_access_get(int unit, int lane_index, int max_cores, pm_info_t pm_info, 
...
STATIC
int pm8x50_fabric_port_most_ext_phy_lane_access_get(int unit, int port, pm_info_t pm_info, phymod_phy_access_t* phy_access)
{
    pm8x50_fabric_t fabric_data = pm_info->pm_data.pm8x50_fabric;
    portmod_access_get_params_t params;
    int nof_phys;
...
int pm8x50_fabric_port_phy_index_get(int unit, int port, pm_info_t pm_info, uint32 *phy_index, uint32 *bitmap){
    ...
    *phy_index = (first_phy - fabric_data->first_phy_offset) % PM8X50_FABRIC_LANES_PER_CORE;

    if (is_bypass) {
        *phy_index = -1;
        for (i = 0; i < MAX_PORTS_PER_PM8X50_FABRIC_PM; i++) {
...
int pm8x50_fabric_wb_buffer_init(int unit, int wb_buffer_index, pm_info_t pm_info)
{
    ...
    SOC_WB_ENGINE_ADD_BUFF(SOC_WB_ENGINE_PORTMOD, wb_buffer_index, "pm8x50_fabric", NULL, NULL, VERSION(3), 1, SOC_WB_ENGINE_PRE_RELEASE);
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "ports", wb_buffer_index, sizeof(int), NULL, MAX_PORTS_PER_PM8X50_FABRIC_PM, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[wb_ports] = wb_var_id;

    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "polarity_rx", wb_buffer_index, sizeof(uint32), NULL, MAX_PORTS_PER_PM8X50_FABRIC_PM, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
    pm_info->wb_vars_ids[wb_polarity_rx] = wb_var_id;

    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "polarity_tx", wb_buffer_index, sizeof(uint32), NULL, MAX_PORTS_PER_PM8X50_FABRIC_PM, VERSION(1));
    SOCDNX_IF_ERR_EXIT(rv);
    ...
    SOCDNX_IF_ERR_EXIT(portmod_next_wb_var_id_get(unit, &wb_var_id));
    SOC_WB_ENGINE_ADD_ARR(SOC_WB_ENGINE_PORTMOD, wb_var_id, "lane2port_map", wb_buffer_index, sizeof(int), NULL, MAX_PORTS_PER_PM8X50_FABRIC_PM, VERSION(3));
    SOCDNX_IF_ERR_EXIT(rv);
...
{   
    ...
    pm_info->pm_data.pm8x50_fabric = fabric_data;
        ...
        if (pm_add_info->pm_specific_info.pm8x50_fabric.is_over_nif) {
            ...
            pm_info->pm_data.pm8x50_fabric->fabric_o_nif_data.pms = fabric_data->fabric_o_nif_data.pms;
        } else
    ...
    if (info->lane_map.num_of_lanes != PM8X50_FABRIC_LANES_PER_CORE)
    {
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("lane map is not supported for pm8x50_fabric")));
    }
    ...
    fabric_data->fmac_schan_id = pm_add_info->pm_specific_info.pm8x50_fabric.fmac_schan_id;
    fabric_data->fsrd_schan_id = pm_add_info->pm_specific_info.pm8x50_fabric.fsrd_schan_id;
    fabric_data->fsrd_internal_quad = pm_add_info->pm_specific_info.pm8x50_fabric.fsrd_internal_quad;
    ...
    for(i = 0; i < MAX_PHYN; i++){
        for(j = 0; j < PM8X50_FABRIC_LANES_PER_CORE; j++){
...
{   
    if(pm_info->pm_data.pm8x50_fabric != NULL){
            ...
            if (pm_info->type == portmodDispatchTypeDnx_fabric_o_nif) {
                if(pm_info->pm_data.pm8x50_fabric->fabric_o_nif_data.pms != NULL) {
                    sal_free(pm_info->pm_data.pm8x50_fabric->fabric_o_nif_data.pms);
                    pm_info->pm_data.pm8x50_fabric->fabric_o_nif_data.pms = NULL;
                }
            }
        #endif /*PORTMOD_DNX_FABRIC_O_NIF_SUPPORT*/
...
int pm8x50_fabric_core_init_phase1(int unit, int port, pm_info_t pm_info, const portmod_port_add_info_t* add_info)
{
    ...
    fsrd_internal_quad = pm_info->pm_data.pm8x50_fabric->fsrd_internal_quad;
    ...
    if(!PORTMOD_PORT_ADD_F_INIT_PASS2_GET(add_info)) {
        ...
        for(lane_index = 0; lane_index < PM8X50_FABRIC_LANES_PER_CORE; lane_index++){            
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_lane_phychain_core_access_get(unit, lane_index, 1, pm_info, core_access));
    ...
    for(lane_index = 0; lane_index < PM8X50_FABRIC_LANES_PER_CORE; lane_index++)
    {
        SOCDNX_IF_ERR_EXIT(pm8x50_fabric_lane_phychain_core_access_get(unit, lane_index, fabric_data->nof_phys, pm_info, core_access));
        for (phyn = 0; phyn < fabric_data->nof_phys; phyn++) {
...
int pm8x50_fabric_port_disable_init(int unit, int port, pm_info_t pm_info)
{
    ...
    *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
    for (i = 0; i < (is_bypass ? PM8X50_FABRIC_LANES_PER_CORE - phy_index : 1); i++)
    {
    ...
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_phy_tx_lane_control_set(unit, port, pm_info, phymodTxElectricalIdleEnable));
...
int pm8x50_fabric_port_enable_set(int unit, int port, pm_info_t pm_info, int flags, int enable)
{
             ...
             PORTMOD_PORT_ENABLE_RX_GET(flags_mask) && PORTMOD_PORT_ENABLE_TX_GET(flags_mask))
    {
        ...
        if(enable){
            ...
            SOCDNX_IF_ERR_EXIT(pm8x50_fabric_phy_tx_lane_control_set(unit, port, pm_info, phymodTxElectricalIdleDisable));
...
int pm8x50_fabric_port_enable_get(int unit, int port, pm_info_t pm_info, int flags, int *enable)
{
    ...
    SOCDNX_IF_ERR_EXIT(phymod_phy_access_t_init(&phy_access));    
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_most_ext_phy_lane_access_get(unit, port, pm_info, &phy_access));
    SOCDNX_IF_ERR_EXIT(pm8x50_fabric_port_phy_index_get(unit, port, pm_info, &phy_index, &bitmap));
    SOCDNX_IF_ERR_EXIT(PM8X50_FABRIC_IS_BYPASSED_GET(unit, pm_info, &is_bypass));
    SOCDNX_IF_ERR_EXIT(READ_FMAC_RECEIVE_RESET_REGISTERr(unit, fabric_data->fmac_schan_id | SOC_REG_ADDR_SCHAN_ID_MASK, &reg_val));
    *field_val = soc_reg_field_get(unit, FMAC_RECEIVE_RESET_REGISTERr, reg_val, FMAC_RX_RST_Nf);
...
STATIC int
_pm8x50_fabric_force_signal_detect_set(int unit, pm_info_t pm_info)
{
    int i, port_index;
    int rv = 0;
    ...
    for(port_index = 0 ; port_index < MAX_PORTS_PER_PM8X50_FABRIC_PM; port_index++){
...
int pm8x50_fabric_mac_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    uint32 reg_val = 0;
    uint32 field[1] = {0};
    uint32 fmac_blk_id_masked = pm_info->pm_data.pm8x50_fabric->fmac_schan_id | SOC_REG_ADDR_SCHAN_ID_MASK;
    uint32 phy_index, bitmap;
    SOCDNX_INIT_FUNC_DEFS;
...
int pm8x50_fabric_port_loopback_set(int unit, int port, pm_info_t pm_info, portmod_loopback_mode_t loopback_type, int enable)
{
    ...
    SOCDNX_IF_ERR_EXIT(_pm8x50_fabric_force_signal_detect_set(unit, pm_info));
                                    ...
                                    phymod_phy_access_t* phy_access_arr, int* nof_phys, int* is_most_ext)
{
    ...
    #ifdef PORTMOD_DNX_FABRIC_O_NIF_SUPPORT
        pm8x50_fabric_internal_t* fabric_o_nif_data = &(pm_info->pm_data.pm8x50_fabric->fabric_o_nif_data);
    #endif /*PORTMOD_DNX_FABRIC_O_NIF_SUPPORT*/
    portmod_ext_phy_core_info_t core_info;
...
{   portmod_lane_connection_t *candidate_lane_connection = NULL;
    ...
    for(lane_index = 0; lane_index < PM8X50_FABRIC_LANES_PER_CORE; lane_index++){
        candidate_lane_connection = &(pm_info->pm_data.pm8x50_fabric->phy_chain[phyn - 1][lane_index]);
        if(PORTMOD_LANE_INDEX_INVALID != candidate_lane_connection->lane_index){
            break;
        }
    }
    if(PM8X50_FABRIC_LANES_PER_CORE == lane_index){
        pm_info->pm_data.pm8x50_fabric->nof_phys++;
    }    
    lane_index = 0;
    PORTMOD_PBMP_ITER(pm_info->pm_data.pm8x50_fabric->phys, phy_index) {
        if (phy_index == iphy) {
            break;
        }
        lane_index++;
...
{  
    ...
    /* Validate parameters */
    if(((pm_info->pm_data.pm8x50_fabric->nof_phys - 1) < phyn) || (0 >= phyn)){
        SOCDNX_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOCDNX_MSG("phyn is out of the range of allowed external phys")));
    }
    PORTMOD_PBMP_ITER(pm_info->pm_data.pm8x50_fabric->phys, phy_index){
        if(phy_index == iphy){
