/* 
 * $Id: sirius_counter.c,v 1.31 Broadcom SDK $
 *
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * File:        sirius_counter.c
 * Purpose:     Software Counter Collection module for SIRIUS (QE4K).
 */
...
static int soc_sirius_counter_enable_flag[SOC_MAX_NUM_DEVICES];
static int soc_sirius_counter_current_base[SOC_MAX_NUM_DEVICES];

/*
 * SIRIUS SLQ has 16 counter blocks at a time.
 * The base can be moved as desired.
 *
 */
#define SIRIUS_COUNTER_BLOCK_QM_NUM_SETS    16
#define SIRIUS_COUNTER_REG_QM_WIDTH         32

STATIC int _soc_sirius_counter_qm_read(int unit, int set, int counter,
                                       uint64 *val, int *width);
STATIC int _soc_sirius_counter_qm_write(int unit, int set, int counter,
                                        uint64 val);
 ...
 */
typedef enum soc_sirius_counter_block_e {
    siriusCounterBlockQm = 0,
    siriusCounterBlockCount
} soc_sirius_counter_block_t;
 ...
 */
soc_sbx_counter_block_info_t    sirius_counter_blocks[] = {
    { siriusCounterBlockQm,
      SIRIUS_COUNTER_BLOCK_QM_NUM_SETS,
      SIRIUS_COUNTER_QM_COUNT,
      _soc_sirius_counter_qm_read,
      _soc_sirius_counter_qm_write,
    },
};
 ...
 */
int
soc_sbx_sirius_counter_init(int unit, uint32 flags, int interval, pbmp_t pbmp)
{
    int         rv = SOC_E_NONE;
    int         block;
    int         set;
    ...
    SOC_IF_ERROR_RETURN(soc_sbx_counter_init(unit, &sirius_counter_blocks[0],
                                             siriusCounterBlockCount));
    ...
    for (set = 0; set < SIRIUS_COUNTER_BLOCK_QM_NUM_SETS; set++) {
   
        soc_sbx_counter_bset_add(unit, block, set);

    }

    soc_sirius_counter_enable_flag[unit] = FALSE;
    soc_sirius_counter_current_base[unit] = -1;
...
/*
 * Function:
 *     soc_sbx_sirius_process_global_stats
 * Purpose:
 ...
 */
int
soc_sbx_sirius_process_global_stats(int unit)
{
      int rv = SOC_E_NONE;
	...
	stat_update = (soc_sbx_cs_update_t *) &sir->cs.gbl_stats[gbl_idx * 2];
...
/*
 * Function:
 *     soc_sbx_sirius_process_fd_drop_stats
 * Purpose:
 ...
 */
int
soc_sbx_sirius_process_fd_drop_stats(int unit, int clear)
{
      int rv = SOC_E_NONE;
      soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
      soc_sbx_cs_update_t *stat_update = NULL;
	  ...
	  for (fd_idx = 0; fd_idx <= SOC_MEM_INFO(unit, EG_FD_PER_PORT_DROP_COUNT1m).index_max; fd_idx++) {
	      ...
	      stat_update = (soc_sbx_cs_update_t *) &sir->cs.fd_drop[drop1][fd_idx * 2];
	  ...
	  for (fd_idx = 0; fd_idx <= SOC_MEM_INFO(unit, EG_FD_PER_PORT_DROP_COUNT2m).index_max; fd_idx++) {
	      ...
	      stat_update = (soc_sbx_cs_update_t *) &sir->cs.fd_drop[drop2][fd_idx * 2];
...
/*
 * Function:
 *     soc_sbx_sirius_process_slq_stats
 * Purpose:
 ...
 */
int
soc_sbx_sirius_process_slq_stats(int unit)
{
      int rv = SOC_E_NONE;
      soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
      soc_sbx_cs_update_t *stat_update = NULL;
	 ...
	 * been activated
	 */
	if ((sir->cs.slq_q_active & (1 << slq_base_idx)) == 0)
	    continue;
	...
	for (slq_q_idx = 0; slq_q_idx < 16; slq_q_idx++) {
	  ...
	  stat_update = (soc_sbx_cs_update_t *) &sir->cs.slq_stats[slq_cntr_addr*2];
...
/*
 * Function:
 *     soc_sbx_sirius_process_cs_dma_fifo
 * Purpose:
 ...
 */
int
soc_sbx_sirius_process_cs_dma_fifo(int unit)
{
      int rv = SOC_E_NONE;
      soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
      soc_sbx_cs_rbuf_entry_t *fifo_entry = (soc_sbx_cs_rbuf_entry_t *) sir->cs.fifo_dma_rbuf_read_ptr;
      soc_sbx_cs_update_t *stat_update = NULL;
      uint32 regval = 0;
      ...
      if (sir->cs.fifo_dma_rbuf_read_ptr == NULL) 
	return SOC_E_INIT;
	 ...
	 */
	if (fifo_entry->mem_addr < (SB_FAB_DEVICE_SIRIUS_NUM_QUEUES << 4)) {
	...
	if (fifo_entry->mem_addr & CS_DMA_CNTR_MSG_LAST) {
	  ...
	  if (sir->cs.flags & CS_FLUSHING) {
	    sal_sem_give(sir->cs.CsFifoSem);
	  } else {
	...
	if ((sir->cs.fifo_dma_rbuf_read_ptr += CS_DMA_FIFO_MSG_SIZE) >= sir->cs.fifo_dma_rbuf_end)
	  sir->cs.fifo_dma_rbuf_read_ptr = sir->cs.fifo_dma_rbuf_begin;
	
	fifo_entry = (soc_sbx_cs_rbuf_entry_t *) sir->cs.fifo_dma_rbuf_read_ptr;
	...
	regval = soc_cm_l2p(unit, sir->cs.fifo_dma_rbuf_read_ptr);
 ...
 */
int
soc_sbx_sirius_get_segment(uint32 unit, uint8 cu, int32 *rSegment)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    uint8      segment = 0;
    ...
    for(segment=0; segment < CS_NUM_SEGMENTS; segment++) {
      if ((sir->cs.segmentInUse & (1 << segment)) == 0) {
	sir->cs.segmentInUse |= (1 << segment);
	sir->cs.segment[segment].cu_num = cu;
	*rSegment = segment;
	return SOC_E_NONE;
      }
    }
 ...
 */
int
soc_sbx_sirius_free_segment(uint32 unit, uint32 segment)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    uint32                   regval = 0;
    ...
    if (COMPILER_64_IS_ZERO(sir->cs.segment[segment].cntrIdMap) == FALSE) {
      LOG_INFO(BSL_LS_SOC_COUNTER,
               (BSL_META("CS:Free segment unit %d segment %d cntrIdMap %x%08x\n"),
                unit, segment, COMPILER_64_HI(sir->cs.segment[segment].cntrIdMap), COMPILER_64_LO(sir->cs.segment[segment].cntrIdMap)));
      return SOC_E_FAIL;
    }

    if (sir->cs.segment[segment].segmentSize != 0) {
      LOG_INFO(BSL_LS_SOC_COUNTER,
               (BSL_META("CS:Free segment unit %d segment %d Segment Size %d\n"),
                unit, segment, sir->cs.segment[segment].segmentSize));
      return SOC_E_FAIL;
    }
    ...
    if (sir->cs.segment[segment].ullCountOrig != NULL) 
      sal_free(sir->cs.segment[segment].ullCountOrig);

    sir->cs.segment[segment].ullCountOrig = NULL;
    sir->cs.segment[segment].ullCount = NULL;
    sir->cs.segment[segment].cu_num = 0;
    sir->cs.segment[segment].shared = 0;
    sal_memset(sir->cs.segment[segment].flushTimeDelta, 0, sizeof(sir->cs.segment[segment].flushTimeDelta));
    sal_memset(sir->cs.segment[segment].group, 0, sizeof(sir->cs.segment[segment].group));
...
isBrickSameCu (uint32 unit, uint32 brick, uint32 cu_num)
{
  ...
  for ( group = 0; group < CS_NUM_GROUPS; group++) {
    if ( (sir->cs.groupConfig[brick][group].group_used != CS_GROUP_FREE) &&
	 (sir->cs.groupConfig[brick][group].cu_num != cu_num ))
	     return FALSE;
  }
  return TRUE;
}
...
{
    soc_sbx_sirius_config_t *sir    = SOC_SBX_CFG_SIRIUS(unit);
    cs_segment_t            *segInfo = &sir->cs.segment[segment];
    ...
    for (brick = 0; brick < CS_NUM_BRICKS; brick++) {
      for (group = 0; group < CS_NUM_GROUPS; group++) {
	grpCfg = &sir->cs.groupConfig[brick][group];
	if ((grpCfg->segment == segment) &&
	    (grpCfg->group_used == CS_GROUP_PROVISIONING)) {
          uint64 temp = COMPILER_64_INIT(0,1);
	  ...
	  curr_cfg_grp = grpCfg->offset >> CS_CFG_GRP_SHIFT;
          COMPILER_64_SHL(temp, curr_cfg_grp);
 ...
 */
int
soc_sbx_sirius_create_group(uint32 unit, uint32 segment, uint32 cu_num, uint32 cntrId, uint32 size)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    cs_segment_t *segInfo = &sir->cs.segment[segment];
    cs_grp_cfg_t *grpCfg = NULL;
    ...
    while (curr_cfg_grp <= cfg_grp_end ) {
      ...
      if (COMPILER_64_BITTEST(segInfo->cntrIdMap, curr_cfg_grp) != 0) {
...
got_group:
    ...
    SOC_IF_ERROR_RETURN(soc_sbx_sirius_provision_group(unit, segment, segInfo->ullCount));
...
/*
 * Function:
 *     soc_sbx_sirius_provision_group
 * Purpose:
 ...
 */
int
soc_sbx_sirius_provision_group(uint32 unit, uint32 segment, uint64 *ullCntAddr)
{
    soc_sbx_sirius_config_t       *sir    = SOC_SBX_CFG_SIRIUS(unit);
    cs_segment_t                  *segInfo = &sir->cs.segment[segment];
    cs_grp_cfg_t                  *grpCfg = NULL;
	...
	if ( grpCfg->group_used == CS_GROUP_PROVISIONING &&
	  ...
	  soc_sbx_sirius_init_group(unit, segment, grpCfg->offset, CS_GRP_SIZE);
 ...
 */
int
soc_sbx_sirius_init_group(uint32 unit, uint32 segment, uint32 cntr_id, uint32 num_cntrs)
{
 ...
 */
int
soc_sbx_sirius_remove_group(uint32 unit, uint32 segment, uint32 cu_num, uint32 cntrId, uint32 size)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    cs_segment_t *segInfo = &sir->cs.segment[segment];
    cs_grp_cfg_t *grpCfg = NULL;
	...
	SOC_IF_ERROR_RETURN(WRITE_CS_CONFIG_BACKGROUND_ENABLEr(unit, regval));
      ...
      rv = soc_sbx_sirius_free_segment(unit, segment);
 ...
 */
int
soc_sbx_sirius_flush_segment(uint32 unit, uint32 segment, uint32 cntrId, uint32 num_cntrs)
{
    soc_sbx_sirius_config_t *sir = SOC_SBX_CFG_SIRIUS(unit);
    cs_segment_t *segInfo = &sir->cs.segment[segment];
...
flush:
    ...
    LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                (BSL_META("soc_sbx_sirius_flush_segment: sleep %d\n"), to_usec));

    (void)sal_sem_take(sir->cs.CsFifoSem, to_usec);
...
uint32
soc_sirius_qm_counter_base_set(int unit,
			       int32 nBaseQueue,
			       int32 enable)
{
...
uint32
soc_sirius_qm_counter_base_get(int unit,
			       int32 *nBaseQueue)
{
    uint32 regval = 0;
...
uint32
soc_sirius_qm_counter_read(int unit,
			   int32 set,
			   uint32 *puCounterBase)
{
    ...
    for ( nCounter=0; nCounter < (SIRIUS_COUNTER_QM_COUNT/2); nCounter++) {
	
	SOC_IF_ERROR_RETURN(READ_QMB_SLQ_PKT_CNTr(unit, &uPacketCount));
	if (nCounter == ((SIRIUS_COUNTER_QM_COUNT/2)-1)) {
	  soc_reg_field_set(unit, QMB_SLQ_PTRr, &regval, AUTO_INCRf, 0);
	  SOC_IF_ERROR_RETURN(WRITE_QMB_SLQ_PTRr(unit, regval));
	}
 ...
 */
STATIC int
_soc_sirius_counter_qm_read(int unit, int set, int counter,
                            uint64 *val, int *width)
{
    int     rv;
    uint32  data[SIRIUS_COUNTER_QM_COUNT];
    int counter_index;

    LOG_VERBOSE(BSL_LS_SOC_COUNTER,
                (BSL_META_U(unit,
                            "_soc_sirius_counter_qm_read set %d cntr %d enb %d\n"),
                 set, counter, soc_sirius_counter_enable_flag[unit]));

    rv = SOC_E_NONE;

    if (soc_sirius_counter_enable_flag[unit] != TRUE) {
        for ( counter_index = 0 ; counter_index < SIRIUS_COUNTER_QM_COUNT; counter_index++) {
    ...
    rv = soc_sirius_qm_counter_read(unit, set, &data[0]);

    if (SOC_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_SOC_COMMON,
                  (BSL_META_U(unit,
                              "sirius counter read failure\n")));
        return rv;
    }
    ...
    *width = SIRIUS_COUNTER_REG_QM_WIDTH;
 ...
 */
STATIC int
_soc_sirius_counter_qm_write(int unit, int set, int counter, uint64 val)
{
    int     rv = SOC_E_NONE;
...
/*
 * Function:
 *     soc_sbx_sirius_counter_port_get
 * Purpose:
 ...
 */
int
soc_sbx_sirius_counter_port_get(int unit, int base_queue, 
                                int block, int set,
...
/*
 * Function:
 *     soc_sbx_sirius_counter_port_set
 * Purpose:
 ...
 */
int
soc_sbx_sirius_counter_port_set(int unit, int base_queue,
                                int block, int set,
                                int counter, uint64 val)
{
    int rv = SOC_E_NONE;
    ...
    num_counters = sirius_counter_blocks[block].num_counters;
...
int
soc_sirius_counter_enable_get(int unit, int *base, int *enable)
{
    *enable = soc_sirius_counter_enable_flag[unit];
    return BCM_E_UNAVAIL;
}

int
soc_sirius_counter_enable_set(int unit, int base)
{
    if(soc_sirius_counter_enable_flag[unit]) return BCM_E_INTERNAL;

    soc_sbx_counter_bset_clear( unit, 0);

    soc_sirius_counter_enable_flag[unit] = TRUE;
    soc_sirius_counter_current_base[unit] = base ;
    return BCM_E_NONE;
}
 ...
 */
int
soc_sirius_counter_enable_clear(int unit)
{
    soc_sirius_counter_enable_flag[unit] = FALSE;
    soc_sirius_counter_current_base[unit] = -1;
