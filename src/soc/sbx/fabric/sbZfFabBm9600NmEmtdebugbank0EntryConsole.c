...
sbZfFabBm9600NmEmtdebugbank0Entry_Print(sbZfFabBm9600NmEmtdebugbank0Entry_t *pFromStruct) {
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("FabBm9600NmEmtdebugbank0Entry:: esetfullstatusmode=0x%01x"), (unsigned int)  pFromStruct->m_uEsetFullStatusMode));
  LOG_WARN(BSL_LS_SOC_COMMON,
  ...
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("FabBm9600NmEmtdebugbank0Entry:: esetmember1=0x%02x"), (unsigned int)  pFromStruct->m_uEsetMember1));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));

  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("FabBm9600NmEmtdebugbank0Entry:: esetmember0=0x%08x%08x"),   COMPILER_64_HI(pFromStruct->m_uuEsetMember0), COMPILER_64_LO(pFromStruct->m_uuEsetMember0)));
  LOG_WARN(BSL_LS_SOC_COMMON,
           (BSL_META("\n")));
...
sbZfFabBm9600NmEmtdebugbank0Entry_SPrint(sbZfFabBm9600NmEmtdebugbank0Entry_t *pFromStruct, char *pcToString, uint32 lStrSize) {
  ...
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600NmEmtdebugbank0Entry:: esetfullstatusmode=0x%01x", (unsigned int)  pFromStruct->m_uEsetFullStatusMode);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt]," mfem=0x%01x", (unsigned int)  pFromStruct->m_uMfem);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600NmEmtdebugbank0Entry:: esetmember1=0x%02x", (unsigned int)  pFromStruct->m_uEsetMember1);
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");

  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"FabBm9600NmEmtdebugbank0Entry:: esetmember0=0x%08x%08x",   COMPILER_64_HI(pFromStruct->m_uuEsetMember0), COMPILER_64_LO(pFromStruct->m_uuEsetMember0));
  WrCnt += SB_SPRINTF(&pcToString[WrCnt],"\n");
