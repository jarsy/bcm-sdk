/*
 * $Id: g3p1_cmu_sim.c,v 1.4 Broadcom SDK $
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * g3p1_cmu_sim.c: Guadalupe2k V1.3 CMU sim
 *
 */
...
int soc_sbx_g3p1_cmu_counter_read_ext(int unit, int segment, int start, int numCounters, uint64 *counters) {
    int nextPrintPos = 0;
...
int soc_sbx_g3p1_cmu_counter_clear_ext(int unit, int segment)  {
   int nextPrintPos = 0;
    int bufferSizeRemain = MAX_BUFFER_SIZE;
    int numCharPrinted;
    int rv, status;
