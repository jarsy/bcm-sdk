    ...
    dnx_data_submodule_t *submodule_data)
{
     ...
     * Features
     */
    submodule_data->nof_features = dnx_data_l2_vsi_feature_nof;
    submodule_data->features = sal_alloc(sizeof(dnx_data_feature_t) * submodule_data->nof_features, "_dnx_data l2 vsi features");
    if (submodule_data->features == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY, "failed to allocate dnx_data l2_vsi features\n");
    }
     ...
     * Defines
     */
    submodule_data->nof_defines = dnx_data_l2_vsi_define_nof;
    submodule_data->defines = sal_alloc(sizeof(dnx_data_define_t) * submodule_data->nof_defines, "_dnx_data l2 vsi defines");
    if (submodule_data->defines == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY, "failed to allocate dnx_data l2_vsi defines\n");
    }
    ...
    /* Set data type flag */
    submodule_data->defines[dnx_data_l2_vsi_define_nof_vsis].flags |= DNX_DATA_F_NUMERIC;

    /*
     * Tables
     */
    submodule_data->nof_tables = dnx_data_l2_vsi_table_nof;
    submodule_data->tables = sal_alloc(sizeof(dnx_data_table_t) * submodule_data->nof_tables, "_dnx_data l2 vsi tables");
    if (submodule_data->tables == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY, "failed to allocate dnx_data l2_vsi tables\n");
    }
...
{
    return dnx_data_mgmt_feature_data_get(unit, dnx_data_module_l2, dnx_data_l2_submodule_vsi, feature);
}
...
dnx_data_l2_vsi_nof_vsis_get(
    int unit)
{
    return dnx_data_mgmt_define_data_get(unit, dnx_data_module_l2, dnx_data_l2_submodule_vsi, dnx_data_l2_vsi_define_nof_vsis);
}
    ...
    dnx_data_module_t *module_data)
{
    ...
    /* Data Struct Init */
    module_data->name = "l2";
    module_data->nof_submodules = dnx_data_l2_submodule_nof;
    module_data->submodules = sal_alloc(sizeof(dnx_data_submodule_t) * module_data->nof_submodules, "_dnx_data l2 submodules");
    if (module_data->submodules == NULL)
    {
        SHR_ERR_EXIT(_SHR_E_MEMORY, "failed to allocate dnx_data l2 submodules\n");
    }
