...
#define SOC_JER2_JER_OFP_RATES_NOF_CALS_IN_DUAL_MODE 2
/*
 * Static functions 
 */

static int
soc_jer2_jer_ofp_rates_interface_internal_rate_set(int unit, int core, uint32 egr_if_id /* offset of the calendar or interface id*/, uint32 internal_rate, int is_cal_shaper)
{
    ...
    /* read */
    DNXC_IF_ERR_EXIT(READ_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), egr_if_id, &data));

    if (is_cal_shaper) { /* cal shaper */
        /* update calendar rate */
        if (internal_rate > SOC_DNX_DEFS_GET(unit, cal_internal_rate_max)) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("calendar rate for interface is too big \n")));
        }
        ...
        if (internal_rate > JER2_ARAD_OFP_RATES_EGRESS_SHAPER_MAX_INTERNAL_IF_RATE) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("interface rate for interface is too big \n")));
        }           
        soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data, CH_IF_SHAPER_RATEf, internal_rate);
    }

    /* write */
    DNXC_IF_ERR_EXIT(WRITE_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), egr_if_id, &data));
...
int
soc_jer2_jer_ofp_rates_init(DNX_SAND_IN int unit)
{
    ...
    SOC_DNX_CORES_ITER(SOC_CORE_ALL, core) {
        ...
        DNXC_SAND_IF_ERR_EXIT(jer2_arad_fill_table_with_entry(unit, EGQ_PMCm, EGQ_BLOCK(unit, core), &init_max_burst));

        DNXC_SAND_IF_ERR_EXIT(jer2_arad_fill_table_with_entry(unit, EGQ_TCG_PMCm, EGQ_BLOCK(unit, core), &init_max_burst));

        DNXC_SAND_IF_ERR_EXIT(jer2_arad_fill_table_with_entry(unit, EGQ_QP_PMCm, EGQ_BLOCK(unit, core), &init_max_burst));
        ...
        DNXC_SAND_IF_ERR_EXIT(WRITE_EGQ_EGRESS_SHAPER_ENABLE_SETTINGSr(unit, core, data));

        DNXC_IF_ERR_EXIT(sw_state_access[unit].dnx.soc.jer2_arad.tm.jer2_arad_egr_ports.egq_tcg_qpair_shaper_enable.set(unit,TRUE));

        /* iterate over all calendars */
        for (cal_id = 0; cal_id < SOC_DNX_DEFS_GET(unit, nof_channelized_calendars); cal_id++) {
            /* read */
            DNXC_IF_ERR_EXIT(READ_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), cal_id, &data64));

            /* set interface rate to max*/    
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_IF_SHAPER_RATEf, JER2_ARAD_OFP_RATES_EGRESS_SHAPER_MAX_INTERNAL_IF_RATE);
            /* set interface burst to max */
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_IF_SHAPER_MAX_BURSTf, JER2_ARAD_OFP_RATES_EGRESS_SHAPER_MAX_INTERNAL_IF_BURST);
            /* set calendar rate to max*/    
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_SPR_RATE_Af, SOC_DNX_DEFS_GET(unit, cal_internal_rate_max));
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_SPR_RATE_Bf, SOC_DNX_DEFS_GET(unit, cal_internal_rate_max));
            /* set calendar burst to max */
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_SPR_MAX_BURST_Af, JER2_ARAD_OFP_RATES_EGRESS_SHAPER_MAX_INTERNAL_CAL_BURST);
            soc_EGQ_EGRESS_SHAPER_CONFIGURATIONm_field32_set(unit, &data64, CH_SPR_MAX_BURST_Bf, JER2_ARAD_OFP_RATES_EGRESS_SHAPER_MAX_INTERNAL_CAL_BURST);
            ...
            /* write */
            DNXC_IF_ERR_EXIT(WRITE_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), cal_id, &data64));
        }
    }
  ...
  )
{
    ...
    if (is_channelized) {
        DNXC_IF_ERR_EXIT(soc_jer2_jer_egr_port2egress_offset(unit, core, tm_port, &egr_if_id));
    } else {
        egr_if_id = SOC_DNX_DEFS_GET(unit, non_channelized_cal_id);
    }

    /* translate Kbps rate to internal rate (clocks) */
    DNXC_IF_ERR_EXIT(jer2_arad_ofp_rates_egq_shaper_rate_to_internal(unit, JER2_ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB, if_shaper_rate, &internal_rate));
    ...
    DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_interface_internal_rate_set(unit, core, egr_if_id, internal_rate, FALSE /*is_cal_shaper*/));

    /* 2. set calendars shapers rate (fairness) */
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_is_single_cal_mode_get(unit, port, &is_single_cal_mode)); 
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_low_priority_cal_get(unit, port, &low_priority_cal));
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_high_priority_cal_get(unit, port, &high_priority_cal));
    ...
    if (is_single_cal_mode) { /* single calendar mode  - NO fairness!!!! */
        ...
        if (cal_id != INVALID_CALENDAR) {
            DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_interface_internal_rate_set(unit, core, cal_id, 
                                                                             SOC_DNX_DEFS_GET(unit, cal_internal_rate_max), TRUE /*is_cal_shaper*/));
        }
        ...
        if (low_priority_cal != INVALID_CALENDAR) {
            DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_interface_internal_rate_set(unit, core, low_priority_cal, internal_rate, TRUE /*is_cal_shaper*/));
        }

        if (high_priority_cal != INVALID_CALENDAR) {
            DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_interface_internal_rate_set(unit, core, high_priority_cal, internal_rate, TRUE /*is_cal_shaper*/));
        }
    }
  ...
  )
{
    ...
    DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_interface_internal_rate_get(unit, core, egr_if_id, &internal_rate));

    /* translate from internal rate (clocks) to Kbps */
    DNXC_IF_ERR_EXIT(jer2_arad_ofp_rates_egq_shaper_rate_from_internal(unit, JER2_ARAD_OFP_RATES_EGQ_CAL_CHAN_ARB, internal_rate, if_shaper_rate));
    ...
    )
{
    ...
    soc_pbmp_t ports_bm;
    DNX_SAND_OCC_BM_PTR modified_cals_occ;
    ...
    rv = sw_state_access[unit].dnx.soc.jer2_arad.tm.jer2_arad_egr_ports.modified_channelized_cals_occ.get(unit, core, &modified_cals_occ);
    DNXC_IF_ERR_EXIT(rv);
    ...
    if (is_channelized) {
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_high_priority_cal_get(unit, port, &high_priority_cal));
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_low_priority_cal_get(unit, port, &low_priority_cal));
        DNXC_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_egr_queuing_is_high_priority_port_get,(unit, core, tm_port, &is_high_priority_port)));
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_is_single_cal_mode_get(unit, port, &is_single_cal_mode));
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_ports_to_same_interface_get(unit, port, &ports_bm));        
        ...
        if (is_single_cal_mode) {
            ...
            if (high_priority_cal == INVALID_CALENDAR && low_priority_cal == INVALID_CALENDAR) {
                ...
                if (high_priority_cal != INVALID_CALENDAR && low_priority_cal != INVALID_CALENDAR && high_priority_cal != low_priority_cal) {
                    DNXC_EXIT_WITH_ERR(SOC_E_CONFIG, (_BSL_DNXC_MSG("port %d is in single cal mode but has 2 calendars\n"), port));
                }
            ...
            SOC_PBMP_ITER(ports_bm, port_i) {
                ...
                DNXC_IF_ERR_EXIT(dnx_port_sw_db_tm_port_to_base_q_pair_get(unit, core_i, tm_port_i,  &base_q_pair_i));
                DNXC_SAND_IF_ERR_EXIT(jer2_arad_sw_db_egq_port_rate_get(unit, core_i, base_q_pair_i, &rate_i));
            ...
            DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_calendar_allocate(unit, core, tm_port, num_required_slots, old_cal, &new_cal));
            ...
            DNXC_IF_ERR_EXIT(dnx_port_sw_db_high_priority_cal_set(unit, port, new_cal));
            DNXC_IF_ERR_EXIT(dnx_port_sw_db_low_priority_cal_set(unit, port, new_cal));
            ...
            SOC_PBMP_ITER(ports_bm, port_i) {
                DNXC_IF_ERR_EXIT(dnx_port_sw_db_local_to_tm_port_get(unit, port_i, &tm_port_i, &core_i));
                DNXC_IF_ERR_EXIT(dnx_port_sw_db_tm_port_to_base_q_pair_get(unit, core_i, tm_port_i,  &base_q_pair_i));
                DNXC_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_egr_queuing_is_high_priority_port_get,(unit, core_i, tm_port_i, &is_high_priority_port_i)));
                DNXC_SAND_IF_ERR_EXIT(jer2_arad_sw_db_egq_port_rate_get(unit, core_i, base_q_pair_i, &rate_i));
                /* if port rate is not 0 add it to slots requirement */
                if ( ((tm_port_i != tm_port) && (rate_i != 0)) || ((tm_port_i == tm_port) && (rate != 0)) ) {
            ...
            for (i = 0; i < SOC_JER2_JER_OFP_RATES_NOF_CALS_IN_DUAL_MODE; i++) {
        ...
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_high_priority_cal_set(unit, port, SOC_DNX_DEFS_GET(unit, non_channelized_cal_id)));
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_low_priority_cal_set(unit, port, SOC_DNX_DEFS_GET(unit, non_channelized_cal_id)));
        /* mark calendar as modified */
        DNXC_SAND_IF_ERR_EXIT(dnx_sand_occ_bm_occup_status_set(unit, modified_cals_occ, SOC_DNX_DEFS_GET(unit, non_channelized_cal_id), TRUE));
    }
    ...
    /* Setting rate to sw */
    DNXC_SAND_IF_ERR_EXIT(jer2_arad_sw_db_egq_port_rate_set(unit, core, base_q_pair, rate));

    /* Mark port as valid */
    DNXC_SAND_IF_ERR_EXIT(jer2_arad_sw_db_is_port_valid_set(unit, core, base_q_pair, TRUE));
...
int 
soc_jer2_jer_ofp_rates_calendar_allocate(
    DNX_SAND_IN  int             unit,
    DNX_SAND_IN  int             core,
    DNX_SAND_IN  uint32          tm_port,
    ...
    DNX_SAND_OUT uint32          *new_cal)
{
    ...
    *new_cal = INVALID_CALENDAR;
    ...
    num_of_calendars = SOC_DNX_DEFS_GET(unit, nof_channelized_calendars);
    num_of_big_calendars = SOC_DNX_DEFS_GET(unit, nof_big_channelized_calendars);
    small_cal_size = SOC_DNX_DEFS_GET(unit, small_channelized_cal_size);

    rv = sw_state_access[unit].dnx.soc.jer2_arad.tm.jer2_arad_egr_ports.channelized_cals_occ.get(unit, core, &cals_occ);
    DNXC_IF_ERR_EXIT(rv);
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_tm_to_local_port_get(unit, core, tm_port, &port));
    ...
    if((interface == _SHR_PORT_IF_ILKN) && SOC_DNX_CONFIG(unit)->jer2_jer->tm.is_ilkn_big_cal && (num_required_slots>0))
    {
        num_required_slots_final = SOC_DNX_DEFS_GET(unit, big_channelized_cal_size);
    } 
    else
    {
    ...
    if (old_cal == INVALID_CALENDAR) { /* allocate new calendar */
        if (num_required_slots_final != 0) {
            ...
            if (num_required_slots_final <= small_cal_size) { /* allocate samll calendar */
                DNXC_SAND_IF_ERR_EXIT(dnx_sand_occ_bm_get_next_in_range(unit, cals_occ, num_of_big_calendars, (num_of_calendars-2), TRUE, new_cal, &found));  
            } 

            if (!found) { /* try to allocate big calendar since all small calendars are occupied */
                DNXC_SAND_IF_ERR_EXIT(dnx_sand_occ_bm_get_next_in_range(unit, cals_occ, 0, (num_of_big_calendars -1), TRUE, new_cal, &found));
                if (!found) {
            ...
            DNXC_SAND_IF_ERR_EXIT(dnx_sand_occ_bm_occup_status_set(unit, cals_occ, *new_cal, TRUE));
        }

    } else { /* calendar was already allocated, check if need to switch to new calendar */

        if (num_required_slots_final == 0) {
                DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_calendar_deallocate(unit, core, old_cal));
    ...
    DNX_SAND_IN  uint32          cal_id)
{
    ...
    DNXC_SAND_IF_ERR_EXIT(dnx_sand_occ_bm_occup_status_set(unit, cals_occ, cal_id, FALSE));
    ...
    DNX_SAND_OUT uint32     *calendar)
{
    ...
    if (is_high_priority_port) {
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_high_priority_cal_get(unit, port, calendar));
    } else {
        DNXC_IF_ERR_EXIT(dnx_port_sw_db_low_priority_cal_get(unit, port, calendar));
    }
        ...
        cal_info->cal_type == JER2_ARAD_OFP_RATES_EGQ_CAL_TCG)
    {
        *memory_name = EGQ_EGRESS_SHAPER_CONFIGURATIONm;
        ...
        case DNX_TMC_OFP_RATES_EGQ_CHAN_ARB_FIELD_RATE:
            *field_name = (cal2set == JER2_ARAD_OFP_RATES_CAL_SET_A) ? CH_SPR_RATE_Af:CH_SPR_RATE_Bf;
            break;
        case DNX_TMC_OFP_RATES_EGQ_CHAN_ARB_FIELD_MAX_BURST:
            *field_name = (cal2set == JER2_ARAD_OFP_RATES_CAL_SET_A) ? CH_SPR_MAX_BURST_Af:CH_SPR_MAX_BURST_Bf;      
            break;
        case DNX_TMC_OFP_RATES_EGQ_CHAN_ARB_FIELD_CAL_LEN:
            *field_name = (cal2set == JER2_ARAD_OFP_RATES_CAL_SET_A) ? CH_SPR_CAL_LEN_Af:CH_SPR_CAL_LEN_Bf;      
            break;
        default:
    ...
    )
{
    ...
    switch (cal_info->cal_type) {
        case DNX_TMC_OFP_RATES_EGQ_CAL_CHAN_ARB:
            offset = cal_info->chan_arb_id;
            break;
        case DNX_TMC_OFP_RATES_EGQ_CAL_TCG:
            offset = JER2_JERICHO_OFP_RATES_EGRESS_SHAPER_CONFIG_CAL_ID_TCG;
            break;
        case DNX_TMC_OFP_RATES_EGQ_CAL_PORT_PRIORITY:
            offset = JER2_JERICHO_OFP_RATES_EGRESS_SHAPER_CONFIG_CAL_ID_QUEUE_PAIR;
            break;
        default:
            break;
    }
    ...
    /* read */
    DNXC_IF_ERR_EXIT(READ_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), offset, &data_above_64));
    ...
    )
{
    ...
    /* write */
    DNXC_IF_ERR_EXIT(WRITE_EGQ_EGRESS_SHAPER_CONFIGURATIONm(unit, EGQ_BLOCK(unit, core), offset, &data_above_64));
...
{  
  ...
  switch (chan_arb_id)
  {
      ...
      case 31:
        ...
        break;
      case JER2_JERICHO_OFP_RATES_EGRESS_SHAPER_CONFIG_CAL_ID_NON_CHN:
    ...
    )
{
    ...
    DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_retrieve_egress_shaper_reg_field_names(
                                                                unit,
                                                                cal_info,
                                                                cal2set,    
                                                                field_type,
    ...
    /* write */
    DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_egress_shaper_mem_field_write (
                                                              unit,
                                                              core,
                                                              cal_info,   
                                                              memory_name,
    ...
    )
{
    ...
    /* write */
    DNXC_IF_ERR_EXIT(soc_jer2_jer_ofp_rates_egress_shaper_mem_field_read (
                                                              unit,
                                                              core,
                                                              cal_info,   
                                                              memory_name,
