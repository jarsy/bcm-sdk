/* $Id: sand_integer_arithmetic.c,v 1.11 Broadcom SDK $
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
*/



#include <shared/bsl.h>
#include <soc/dnx/legacy/drv.h>



#include <soc/dnx/legacy/SAND/Utils/sand_integer_arithmetic.h>
#include <soc/dnx/legacy/SAND/Utils/sand_os_interface.h>
#include <soc/dnx/legacy/SAND/Management/sand_general_macros.h>
#include <soc/dnx/legacy/SAND/Utils/sand_u64.h>


/* $Id: sand_integer_arithmetic.c,v 1.11 Broadcom SDK $
 * Integer Logarithmic
 * {
 */

/*****************************************************
*NAME
*  dnx_sand_log2_round_up
*TYPE:
*  PROC
*DATE:
*  12-Nov-02
*FUNCTION:
*  Given number - x.
*  Return the integer round up of log2(x)
*CALLING SEQUENCE:
*  dnx_sand_log2_round_up(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to log.
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*       Log with base 2. Round up
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*  Examples:
*    dnx_sand_log2_round_up(0) = 0 -- definition
*    dnx_sand_log2_round_up(1) = 0
*    dnx_sand_log2_round_up(2) = 1
*    dnx_sand_log2_round_up(3) = 2
*    dnx_sand_log2_round_up(4) = 2
*    dnx_sand_log2_round_up(5) = 3
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_log2_round_up(
    DNX_SAND_IN uint32 x
  )
{
  uint32
    msb_bit;

  msb_bit = dnx_sand_msb_bit_on(x) ;

  if ( x > (((uint32)1)<<msb_bit) )
  {
    /*
     * need to make round up.
     */
    msb_bit++;
  }

  return msb_bit;
}

/*****************************************************
*NAME
* dnx_sand_power_of_2
*FUNCTION:
*  Return:
*   for power p, returns 2^p.
*   If power is out of range, returns 0.
*INPUT:
*  DNX_SAND_DIRECT:
*   DNX_SAND_IN uint32 power
*      The power of 2 to calculate 2^p for.
*REMARKS:
*  Examples:
*    dnx_sand_power_of_2(0) = 1
*    dnx_sand_power_of_2(1) = 2
*    dnx_sand_power_of_2(31) = 2^31
*    dnx_sand_power_of_2(32) = 0
*
*****************************************************/
uint32
  dnx_sand_power_of_2(
    DNX_SAND_IN uint32 power
  )
{
  uint32
    answer;

  if (power > DNX_SAND_REG_MAX_BIT)
  {
    answer = 0;
  }
  else
  {
    answer = 1 << power;
  }

  return answer;
}

/*****************************************************
*NAME
* dnx_sand_log2_round_down
*TYPE:
*  PROC
*DATE:
*  13-Nov-02
*FUNCTION:
*  Given number - x.
*  Return the integer round down of log2(x)
*CALLING SEQUENCE:
* dnx_sand_log2_round_down(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to log.
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*       Log with base 2. Round down
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*  Examples:
*    dnx_sand_log2_round_up(0) = 0 -- definition
*    dnx_sand_log2_round_up(1) = 0
*    dnx_sand_log2_round_up(2) = 1
*    dnx_sand_log2_round_up(3) = 1
*    dnx_sand_log2_round_up(4) = 2
*    dnx_sand_log2_round_up(5) = 2
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_log2_round_down(
    DNX_SAND_IN uint32 x
  )
{
  return dnx_sand_msb_bit_on(x);
}


/*****************************************************
*NAME
* dnx_sand_is_power_of_2
*FUNCTION:
*  Return TRUE: iff the number x is power of 2.
*  Otherwise return FALSE
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN uint32 x -
*      Number to check
*REMARKS:
*  Examples:
*    dnx_sand_is_power_of_2(0) = FALSE -- definition
*    dnx_sand_is_power_of_2(1) = TRUE
*    dnx_sand_is_power_of_2(2) = TRUE
*    dnx_sand_is_power_of_2(3) = FALSE
*
*****************************************************/
uint32
  dnx_sand_is_power_of_2(
    DNX_SAND_IN uint32 x
  )
{
  uint32
    answer;

  /*
   * Power of 2: only one bit is ZERO.
   */
  answer = (dnx_sand_nof_on_bits_in_long(x) == 1);

  return answer;
}

/*
 * }
 */

/*
 * Bit Manipulation
 * {
 */

/*
 * This struct holds the number of ON bits pre entry.
 * Entry N corresponds to the number N.
 */
const
  unsigned char
    Dnx_soc_sand_nof_on_bit_in_char[256]=
{
  0, 1, 1, 2, 1, 2, 2, 3,
  1, 2, 2, 3, 2, 3, 3, 4,
  1, 2, 2, 3, 2, 3, 3, 4,
  2, 3, 3, 4, 3, 4, 4, 5,
  1, 2, 2, 3, 2, 3, 3, 4,
  2, 3, 3, 4, 3, 4, 4, 5,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  1, 2, 2, 3, 2, 3, 3, 4,
  2, 3, 3, 4, 3, 4, 4, 5,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  3, 4, 4, 5, 4, 5, 5, 6,
  4, 5, 5, 6, 5, 6, 6, 7,
  1, 2, 2, 3, 2, 3, 3, 4,
  2, 3, 3, 4, 3, 4, 4, 5,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  3, 4, 4, 5, 4, 5, 5, 6,
  4, 5, 5, 6, 5, 6, 6, 7,
  2, 3, 3, 4, 3, 4, 4, 5,
  3, 4, 4, 5, 4, 5, 5, 6,
  3, 4, 4, 5, 4, 5, 5, 6,
  4, 5, 5, 6, 5, 6, 6, 7,
  3, 4, 4, 5, 4, 5, 5, 6,
  4, 5, 5, 6, 5, 6, 6, 7,
  4, 5, 5, 6, 5, 6, 6, 7,
  5, 6, 6, 7, 6, 7, 7, 8
};

/*****************************************************
*NAME
*  dnx_sand_msb_bit_on
*TYPE:
*  PROC
*DATE:
*  13-Nov-02
*FUNCTION:
*  Given number - x.
*  Return the msb bit number that is on.
*CALLING SEQUENCE:
*   dnx_sand_msb_bit_on(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to evaluate
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*     the bit number
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*    dnx_sand_msb_bit_on(0) = 0 -- definition
*    dnx_sand_msb_bit_on(1) = 0
*    dnx_sand_msb_bit_on(2) = 1
*    dnx_sand_msb_bit_on(3) = 1
*    dnx_sand_msb_bit_on(4) = 2
*    dnx_sand_msb_bit_on(5) = 2
*    dnx_sand_msb_bit_on(0xFF121212) = 31
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_msb_bit_on(
    DNX_SAND_IN uint32 x_in
  )
{
  uint32
    n,
    x;
  int
    i;
   x = x_in;

  if (x == 0)
  {
    return(0);
  }

  n = 0;
  for (i=16 ; i>0 ; i>>=1)
  {
    if (x & ((~((uint32)0))<<i))
    {
      n += i;
      x >>= i;
    }
  }
  return n;
}


/*****************************************************
*NAME
*  dnx_sand_nof_on_bits_in_long
*TYPE:
*  PROC
*DATE:
*  03-Mar-05
*FUNCTION:
*  Given number - x.
*  Get the number of bits are on in uint32
*CALLING SEQUENCE:
*   dnx_sand_nof_on_bits_in_long(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to evaluate
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*     the number of ON
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*    dnx_sand_nof_on_bits_in_long(0) = 0
*    dnx_sand_nof_on_bits_in_long(1) = 1
*    dnx_sand_nof_on_bits_in_long(2) = 1
*    dnx_sand_nof_on_bits_in_long(3) = 2
*    dnx_sand_nof_on_bits_in_long(4) = 1
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_nof_on_bits_in_long(
    DNX_SAND_IN uint32 x
  )
{
  uint32
    nof_on_bits;

  nof_on_bits = 0;
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_0(x)];
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_1(x)];
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_2(x)];
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_3(x)];

  return nof_on_bits;
}

/*****************************************************
*NAME
*  dnx_sand_nof_on_bits_in_short
*TYPE:
*  PROC
*DATE:
*  03-Mar-05
*FUNCTION:
*  Given number - x.
*  Get the number of bits are on in SHORT (16 bits)
*CALLING SEQUENCE:
*   dnx_sand_nof_on_bits_in_short(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const unsigned short x -
*     Number to evaluate
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*     the number of ON
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*    dnx_sand_nof_on_bits_in_short(0) = 0
*    dnx_sand_nof_on_bits_in_short(1) = 1
*    dnx_sand_nof_on_bits_in_short(2) = 1
*    dnx_sand_nof_on_bits_in_short(3) = 2
*    dnx_sand_nof_on_bits_in_short(4) = 1
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_nof_on_bits_in_short(
    DNX_SAND_IN unsigned short x
  )
{
  uint32
    nof_on_bits;

  nof_on_bits = 0;
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_2(x)];
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[DNX_SAND_GET_BYTE_3(x)];

  return nof_on_bits;
}

/*****************************************************
*NAME
*  dnx_sand_nof_on_bits_in_char
*TYPE:
*  PROC
*DATE:
*  03-Mar-05
*FUNCTION:
*  Given number - x.
*  Get the number of bits are on in CHAR (8 bits)
*CALLING SEQUENCE:
*   dnx_sand_nof_on_bits_in_char(x)
*INPUT:
*  DNX_SAND_DIRECT:
*    const unsigned char x -
*     Number to evaluate
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*     the number of ON
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*    dnx_sand_nof_on_bits_in_short(0) = 0
*    dnx_sand_nof_on_bits_in_short(1) = 1
*    dnx_sand_nof_on_bits_in_short(2) = 1
*    dnx_sand_nof_on_bits_in_short(3) = 2
*    dnx_sand_nof_on_bits_in_short(4) = 1
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_nof_on_bits_in_char(
    DNX_SAND_IN unsigned char x
  )
{
  uint32
    nof_on_bits;

  nof_on_bits = 0;
  nof_on_bits += Dnx_soc_sand_nof_on_bit_in_char[x];

  return nof_on_bits;
}

/*
 * This struct the reverse persantation of byte.
 */
#define DNX_SAND_REVERSE_BYTES_NOF_ENTRIES (256)
static const unsigned char
  Dnx_soc_sand_reverse_bytes[DNX_SAND_REVERSE_BYTES_NOF_ENTRIES] =
{
,
};



/*****************************************************
*NAME
*  dnx_sand_bits_reverse_long
*  dnx_sand_bits_reverse_short
*  dnx_sand_bits_reverse_char
*FUNCTION:
*  Given number - x,
*  reverse (bit wise) the number for uint32/SHORT/CHAR (32/16/8 bits)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to evaluate
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*    unsigned short -
*    unsigned char -
*     Bit reverese of the number
*REMARKS:
*    dnx_sand_bits_reverse_char(0) = 0
*    dnx_sand_bits_reverse_char(1) = 0x80
*    dnx_sand_bits_reverse_char(2) = 0x40
*    dnx_sand_bits_reverse_char(3) = 0xC0
*    dnx_sand_bits_reverse_char(4) = 0x20
*SEE ALSO:
*****************************************************/
uint32
  dnx_sand_bits_reverse_long(
    DNX_SAND_IN uint32 x
  )
{
  uint32
    reverse;


  reverse  = (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_0(x)])<<0;
  reverse |= (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_1(x)])<<8;
  reverse |= (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_2(x)])<<16;
  reverse |= (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_3(x)])<<24;

  return reverse;
}

/*
 */
unsigned short
  dnx_sand_bits_reverse_short(
    DNX_SAND_IN unsigned short x
  )
{
  unsigned short
    reverse;

  reverse  = (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_2(x)])<<0;
  reverse |= (Dnx_soc_sand_reverse_bytes[DNX_SAND_GET_BYTE_3(x)])<<8;

  return reverse;
}

unsigned char
  dnx_sand_bits_reverse_char(
    DNX_SAND_IN unsigned char x
  )
{
  unsigned char
    reverse;


  reverse = Dnx_soc_sand_reverse_bytes[x];

  return reverse;
}


/*
 * }
 */

/*****************************************************
*NAME
*   dnx_sand_break_to_mnt_exp_round_down
*TYPE:
*  PROC
*DATE:
*  13-Nov-02
*FUNCTION:
*  Given: 1. Number - x, .
*         2. Number of mantissa bits - mnt_nof_bits
*         3. Number of exponent bits - exp_nof_bits
*  Return mantissa (mnt) and exponent (exp).
*  Such that, y = (mnt+mnt_inc)<<exp - is the closest smaller equal number than x.
*CALLING SEQUENCE:
*   dnx_sand_break_to_mnt_exp_round_down(x, mnt_nof_bits, exp_nof_bits, mnt_inc, mnt, exp)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to break
*    const uint32 mnt_nof_bits -
*     Size in bits of mantissa.
*     Range 1:10
*    const uint32 exp_nof_bits -
*     Size in bits of exponent
*     Range 1:10
*    const uint32 mnt_inc -
*     A number added to the mantissa for the calculation
*     Range 0:1
*    uint32* mnt -
*     Buffer to load mantissa result
*    uint32* exp -
*     Buffer to load exponent result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_RET -
*
*  DNX_SAND_INDIRECT:
*    mnt, exp.
*REMARKS:
* 1. the mantissa and exponnent number of bits
*     ranges from 1 to 10 each. This range may
*     be enlarge with some minor code changes
*     (if needed at all)
*SEE ALSO:
*****************************************************/
DNX_SAND_RET
  dnx_sand_break_to_mnt_exp_round_down(
    DNX_SAND_IN  uint32 x,
    DNX_SAND_IN  uint32  mnt_nof_bits,
    DNX_SAND_IN  uint32  exp_nof_bits,
    DNX_SAND_IN  uint32  mnt_inc,
    DNX_SAND_OUT uint32* mnt,
    DNX_SAND_OUT uint32* exp
  )
{
  uint32
    y;
  uint32
    mnt_max,
    exp_max,
    msb_bit_on;
  int
    msb_bit_diff;
  DNX_SAND_RET
    ex ;

  ex = DNX_SAND_OK ;

  /*
   * Inputs checking
   */
  if ( (mnt == NULL) ||
       (exp == NULL) ||
       (mnt_nof_bits > 15) ||
       (exp_nof_bits > 10) ||
       (mnt_nof_bits==0) ||
       (exp_nof_bits==0) ||
       (mnt_inc > 1)
     )
  {
    /*
     * Invalid Inputs
     */
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  mnt_max = (0x1<<mnt_nof_bits)-1 ;
  exp_max = (0x1<<exp_nof_bits)-1 ;
  msb_bit_on = 0 ;
  msb_bit_diff = 0 ;

  if(exp_max>= (sizeof(uint32)*8-1))
  {
    /*
     * Case we got exp_max>=31,
     * make it only 30.
     */
    exp_max = sizeof(uint32)*8-2;
  }


  *mnt = 0;
  *exp = 0;

  if (x <= mnt_inc)
  { /*
     * x is too small
     * Put the smallest number it represent
     * Actually we should have returned ERR for (x < mnt_inc), because
     * there is no solution that gives a smaller equal number than x,
     * but for backwards compitability we'll return the smallest solution here
     * as well.
     */
    goto exit ;
  }
  y = ((1 /* mnt_max+mnt_inc */)<<exp_max)*mnt_max;
  if (x > y)
  { /*
     * x is too big for our represantation
     * Put the biggest number it can
     */
    *mnt = mnt_max;
    *exp = exp_max;
    goto exit ;
  }

  msb_bit_on = dnx_sand_msb_bit_on(x) ;

  msb_bit_diff = (msb_bit_on+1) - mnt_nof_bits;
  if (msb_bit_diff<=0)
  {
    msb_bit_diff = 0 ;
  }

  *mnt = mnt_max << msb_bit_diff ;
  *mnt &= x ;

  /* decrease mnt_inc .. because the reverse calculation is (mnt+mnt_inc)<<exp */
  if ((*mnt) >= mnt_inc)
  {
    (*mnt) -= mnt_inc;
  }
  else
  {
    if ( (*exp) > 0)
    {
      (*exp) -- ;
    }
    else
    {
      /* The function does not reach here.*/
      *exp = 0 ;
    }
  }
  *mnt >>= msb_bit_diff;
  *exp = msb_bit_diff ;
exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_break_to_mnt_exp_round_up
*TYPE:
*  PROC
*DATE:
*  13-Nov-02
*FUNCTION:
*  Get a number, x.
*  Return mantissa (mnt) and exponent (exp).
*  Such that, y = (mnt+mnt_inc)<<exp is the closest bigger equal number than x.
*CALLING SEQUENCE:
*   dnx_sand_break_to_mnt_exp_round_up(x, mnt_nof_bits, exp_nof_bits, mnt_inc, mnt, exp)
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 x -
*     Number to dis-assemble
*    const uint32 mnt_nof_bits -
*     Size in bits of mantissa.
*    const uint32 exp_nof_bits -
*     Size in bits of exponent
*    const uint32 mnt_inc -
*     A number added to the mantissa for the calculation
*     Range 0:1
*    uint32* mnt -
*     Buffer to load mantissa result
*    uint32* exp -
*     Buffer to load exponent result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    uint32 -
*
*  DNX_SAND_INDIRECT:
*    mnt, exp.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/
DNX_SAND_RET
  dnx_sand_break_to_mnt_exp_round_up(
    DNX_SAND_IN  uint32 x,
    DNX_SAND_IN  uint32  mnt_nof_bits,
    DNX_SAND_IN  uint32  exp_nof_bits,
    DNX_SAND_IN  uint32  mnt_inc,
    DNX_SAND_OUT uint32* mnt,
    DNX_SAND_OUT uint32* exp
  )
{
  uint32
    mnt_max,
    exp_max ;
  uint32
    num ;
  DNX_SAND_RET
    ex ;

  mnt_max = (0x1<<mnt_nof_bits)-1,
  exp_max = (0x1<<exp_nof_bits)-1 ;
  ex = DNX_SAND_OK;

  /*
   * dnx_sand_break_to_mnt_exp_round_down() is checking the valifdity of the inputs.
   */
  ex = dnx_sand_break_to_mnt_exp_round_down(x, mnt_nof_bits, exp_nof_bits, mnt_inc, mnt, exp);
  if (ex != DNX_SAND_OK)
  {
    goto exit ;
  }
  if(exp_max>= (sizeof(uint32)*8-1))
  {
    exp_max = sizeof(uint32)*8-2;
  }

  num = (*mnt + mnt_inc) << (*exp);
  if (num<x)
  {
    /* Round it up.*/
    (*mnt) ++;
    if (*mnt>mnt_max)
    {
      *mnt = *mnt/2;
      (*exp) ++;
      if ((*exp)> exp_max)
      {
        *exp = exp_max ;
        *mnt = mnt_max ;
      }
    }
  }

exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_complex_comlicated_to_mnt_exp
*TYPE:
*  PROC
*DATE:
*  06/06/2010
*FUNCTION:
*  Get mantissa and exponent .
*  Return Number x,
*  Such that, "x = eq_const_multi * (eq_const_mnt_inc + mnt) * 2^exp".
*CALLING SEQUENCE:
*   dnx_sand_compute_comlicated_to_mnt_exp(mnt, exp, eq_const_multi, eq_const_mnt_inc, x)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  mnt,
*       mantissa to compute from
*    DNX_SAND_IN  uint32  exp,
*       exponent to compute from
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Added const number to the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*       Multiple const number to the equation
*    DNX_SAND_OUT uint32* x,
*       Buffer to load Number result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    x.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/

DNX_SAND_RET
  dnx_sand_compute_complex_to_mnt_exp(
    DNX_SAND_IN  uint32  mnt,
    DNX_SAND_IN  uint32  exp,
    DNX_SAND_IN  uint32 eq_const_multi,
    DNX_SAND_IN  uint32 eq_const_mnt_inc,
    DNX_SAND_OUT uint32* x
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;

  if (x == NULL)
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  if (exp > DNX_SAND_REG_MAX_BIT)
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }

  *x = eq_const_multi * (mnt + eq_const_mnt_inc) * (1 << exp);

exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_compute_complex_to_mnt_reverse_exp
*TYPE:
*  PROC
*DATE:
*  06/06/2010
*FUNCTION:
*  Get mantissa and exponent .
*  Return Number x,
*  Such that, "x = eq_const_multi * (eq_const_mnt_inc + mnt) / 2^exp".
*CALLING SEQUENCE:
*   dnx_sand_compute_comlicated_to_mnt_reverse_exp(mnt, exp, eq_const_multi, eq_const_mnt_inc, x)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  mnt,
*       mantissa to compute from
*    DNX_SAND_IN  uint32  exp,
*       exponent to compute from
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Added const number to the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*       Multiple const number to the equation
*    DNX_SAND_OUT uint32* x,
*       Buffer to load Number result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    x.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/

DNX_SAND_RET
  dnx_sand_compute_complex_to_mnt_reverse_exp(
    DNX_SAND_IN  uint32  mnt,
    DNX_SAND_IN  uint32  exp,
    DNX_SAND_IN  uint32 eq_const_multi,
    DNX_SAND_IN  uint32 eq_const_mnt_inc,
    DNX_SAND_OUT uint32* x
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;
  DNX_SAND_U64
    tmp_result,
    result;
  uint32
    res;

  if (x == NULL)
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  if (exp > DNX_SAND_REG_MAX_BIT)
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }


  dnx_sand_u64_multiply_longs(eq_const_multi, (mnt + eq_const_mnt_inc), &tmp_result); /* eq_const_multi * (mnt + eq_const_mnt_inc) */

  res = dnx_sand_u64_devide_u64_long(&tmp_result, (1 << exp), &result); /* tmp_result / 2^exp */ 

  res = dnx_sand_u64_to_long(&result, x);
  if (res) /* overflow */
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }

exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_compute_complex_to_mnt_exp_reverse_exp
*TYPE:
*  PROC
*DATE:
*  29/01/2012
*FUNCTION:
*  Get mantissa reverse-exponent and exponent .
*  Return Number x,
*  Such that, "x = eq_const_multi * (eq_const_mnt_inc + mnt) * 2^exp / (eq_const_div * 2^rev_exp)".
*CALLING SEQUENCE:
*   dnx_sand_compute_comlicated_to_mnt_reverse_exp(mnt, exp, rev_exp, eq_const_multi, eq_const_div, eq_const_mnt_inc, x)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  mnt,
*       mantissa to compute from
*    DNX_SAND_IN  uint32  exp,
*       exponent to compute from
*    DNX_SAND_IN  uint32  rev_exp,
*       reverse_exponent to compute from
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Multiple const number to the equation
*    DNX_SAND_IN  uint32 eq_const_div,
*       Divided const number from the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*       Added const number to the equation
*    DNX_SAND_OUT uint32* x,
*       Buffer to load Number result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    x.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/

DNX_SAND_RET
  dnx_sand_compute_complex_to_mnt_exp_reverse_exp(
    DNX_SAND_IN  uint32  mnt,
    DNX_SAND_IN  uint32  exp,
    DNX_SAND_IN  uint32  rev_exp,
    DNX_SAND_IN  uint32 eq_const_multi,
    DNX_SAND_IN  uint32 eq_const_div,
    DNX_SAND_IN  uint32 eq_const_mnt_inc,
    DNX_SAND_OUT uint32* x
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;
  DNX_SAND_U64
    tmp_result,
    tmp_result_2,
    result;
  uint32
    tmp_res,
    res;

  if (x == NULL)
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }


  dnx_sand_u64_multiply_longs(eq_const_multi, (1 << exp), &tmp_result); /* eq_const_multi * 2^exp */

  /* check if the multiply result is > uint32_max */
  res = dnx_sand_u64_to_long(&tmp_result, &tmp_res);
  if (res) /* overflow */
  {
    /* divide first and then continue multiplying */
    res = dnx_sand_u64_devide_u64_long(&tmp_result, (1 << rev_exp) * eq_const_div, &tmp_result_2); /* tmp_result / (2^rev_exp * eq_const_div) */ 
    
    res = dnx_sand_u64_to_long(&tmp_result_2, &tmp_res);
    if (res) /* overflow */
    {
      ex = DNX_SAND_ERR ;
      goto exit ;
    }

    dnx_sand_u64_multiply_longs(tmp_res, (mnt + eq_const_mnt_inc), &result); /* tmp_res * (mnt + eq_const_mnt_inc) */
  }
  else {
    /* continue multiplying and then divide */
    dnx_sand_u64_multiply_longs(tmp_res, (mnt + eq_const_mnt_inc), &tmp_result); /* tmp_res * (mnt + eq_const_mnt_inc) */

    res = dnx_sand_u64_devide_u64_long(&tmp_result, (1 << rev_exp) * eq_const_div, &result); /* tmp_result / (2^rev_exp * eq_const_div) */  
  }

  res = dnx_sand_u64_to_long(&result, x);
  if (res) /* overflow */
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }

exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_break_complex_to_mnt_exp_round_down
*TYPE:
*  PROC
*DATE:
*  06/06/2010
*FUNCTION:
*  Get a number, x.
*  Return mantissa (mnt) and exponent (exp).
*  Such that, "x = eq_const_multi * (eq_const_mnt_inc + mnt) * 2^exp" is the closest smaller equal number to x.
*CALLING SEQUENCE:
*   dnx_sand_break_comlicated_to_mnt_exp_round_down(x, max_mnt, max_exp, eq_const_multi, eq_const_mnt_inc, mnt, exp)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  x,
*       Number to dis-assemble
*    DNX_SAND_IN  uint32 max_mnt,
*       Max size of mantissa
*    DNX_SAND_IN  uint32 max_exp,
*       Max size of exponent
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Added const number to the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*       Multiple const number to the equation
*    DNX_SAND_OUT uint32* mnt,
*       Buffer to load mantissa result
*    DNX_SAND_OUT uint32* exp
*       Buffer to load exponent result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    mnt, exp.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/

DNX_SAND_RET
  dnx_sand_break_complex_to_mnt_exp_round_down(
    DNX_SAND_IN  uint32  x,
    DNX_SAND_IN  uint32 max_mnt,
    DNX_SAND_IN  uint32 max_exp,
    DNX_SAND_IN  uint32 eq_const_multi,
    DNX_SAND_IN  uint32 eq_const_mnt_inc,
    DNX_SAND_OUT uint32* mnt,
    DNX_SAND_OUT uint32* exp
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;
  uint32
    bin_serach_beg,
    bin_serach_end,
    bin_serach_num_elements,
    bin_serach_mid,
    bin_serach_low_val,
    bin_serach_high_val,
    exp_man;  

  if ((mnt == NULL) || (exp == NULL) || (eq_const_multi == 0))
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  exp_man = eq_const_multi * (eq_const_mnt_inc + max_mnt);

  if (x <= exp_man)
  {
    *exp = 0;
    if (x < (eq_const_multi * eq_const_mnt_inc))
    {
      *mnt = 0;
    } 
    else
    {
      *mnt = (x / eq_const_multi) - eq_const_mnt_inc;
    }
    goto exit;
  }

  if (x >= exp_man * (1 << max_exp))
  {
    *exp = max_exp;
    *mnt = max_mnt;
    goto exit;
  }

  bin_serach_beg = 0;  
  bin_serach_end = max_exp;
  bin_serach_num_elements = max_exp;
  bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
  bin_serach_low_val = exp_man * (1 << bin_serach_mid);
  bin_serach_high_val = exp_man * (1 << (bin_serach_mid+1));

  while((bin_serach_beg <= bin_serach_end) && 
        !((x > bin_serach_low_val) && (x <= bin_serach_high_val)))  
  {
    if (x <= bin_serach_low_val)	  
    {      
      bin_serach_end = bin_serach_mid - 1;
      bin_serach_num_elements = bin_serach_num_elements/2;      
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;  
    }
    else    
    {      
      bin_serach_beg = bin_serach_mid + 1;     
      bin_serach_num_elements = bin_serach_num_elements/2;
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
    }
    bin_serach_low_val = exp_man * (1 << bin_serach_mid);
    bin_serach_high_val = exp_man * (1 << (bin_serach_mid+1));
  }

  *exp = bin_serach_mid + 1;

  if (x < (eq_const_multi * eq_const_mnt_inc * (1 << *exp))) {
      *exp -= 1;
      *mnt = max_mnt;
  } else {
      *mnt = (x -(eq_const_multi * eq_const_mnt_inc * (1 << *exp))) /  (eq_const_multi * (1 << *exp));
  }
  
exit:
  return ex ;

}

/*****************************************************
*NAME
*   dnx_sand_break_complex_to_mnt_exp_round_down_2
*TYPE:
*  PROC
*DATE:
*  06/06/2010
*FUNCTION:
*  Get a number, x.
*  Return mantissa (mnt) and exponent (exp).
*  Such that, "x = eq_const_multi * (eq_const_mnt_inc + mnt) * 2^exp / eq_const_div" is the closest smaller equal number to x.
*  If x < eq_const_multi * (eq_const_mnt_inc + min_mnt) * 2^0 / eq_const_div,
*  i.e. even though the exp is 0 and the mantissa is the minimal mantissa, we get more than x,
*  The function will set exp to 0 and mnt to min_mnt.
*  Else
*  If x <= eq_const_multi * (eq_const_mnt_inc + max_mnt) * 2^0 / eq_const_div,
*  will extract the mantissa out of the equation.
*CALLING SEQUENCE:
*   dnx_sand_break_comlicated_to_mnt_exp_round_down_2(x, max_mnt, max_exp, eq_const_multi, eq_const_div, eq_const_mnt_inc, mnt, exp)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  x,
*       Number to dis-assemble
*    DNX_SAND_IN  uint32 max_mnt,
*       Max size of mantissa
*    DNX_SAND_IN  uint32 min_mnt,
*       Min size of mantissa
*    DNX_SAND_IN  uint32 max_exp,
*       Max size of exponent
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Multiple const number to the equation
*    DNX_SAND_IN  uint32 eq_const_div,
*       Divide const number from the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*   	Add const number to the equation
*    DNX_SAND_OUT uint32* mnt,
*       Buffer to load mantissa result
*    DNX_SAND_OUT uint32* exp
*       Buffer to load exponent result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    mnt, exp.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/
DNX_SAND_RET
  dnx_sand_break_complex_to_mnt_exp_round_down_2(
    DNX_SAND_IN  uint32  x,
    DNX_SAND_IN  uint32  max_mnt,
    DNX_SAND_IN  uint32  min_mnt,
    DNX_SAND_IN  uint32  max_exp,
    DNX_SAND_IN  uint32  eq_const_multi,
    DNX_SAND_IN  uint32  eq_const_div,
    DNX_SAND_IN  uint32  eq_const_mnt_inc,
    DNX_SAND_OUT uint32* mnt,
    DNX_SAND_OUT uint32* exp
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;
  uint32
    bin_serach_beg,
    bin_serach_end,
    bin_serach_num_elements,
    bin_serach_mid,
    bin_serach_low_val,
    bin_serach_high_val,
    exp_man,
    res,
    tmp; 
  DNX_SAND_U64
    tmp_64,
    tmp_res_64,
	tmp_val,
	quotient_mul_dividor_64,
	remainder,
	divisor_half; 

  if ((mnt == NULL) || (exp == NULL) || (eq_const_multi == 0) || (eq_const_div == 0))
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  dnx_sand_u64_multiply_longs(eq_const_multi, (max_mnt + eq_const_mnt_inc), &tmp_64); 
  res = dnx_sand_u64_devide_u64_long(&tmp_64, eq_const_div, &tmp_res_64); 
  res = dnx_sand_u64_to_long(&tmp_res_64, &exp_man);
  if (res) /* overflow */
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }

  /* if x <= (eq_const_multi * (eq_const_mnt_inc + max_mnt) / eq_const_div)*/
  if (x <= exp_man)
  {
    *exp = 0;
    dnx_sand_u64_multiply_longs(eq_const_multi, eq_const_mnt_inc, &tmp_64); 
    res = dnx_sand_u64_devide_u64_long(&tmp_64, eq_const_div, &tmp_res_64);
    res = dnx_sand_u64_to_long(&tmp_res_64, &tmp);
    if (res) /* overflow */
    {
      ex = DNX_SAND_ERR;
      goto exit ;
    } 
    /* if (x < (eq_const_multi * (eq_const_mnt_inc + 0) / eq_const_div)) */
    if (x < tmp)
    {
      *mnt = min_mnt;
    }
	/* (x >= (eq_const_multi * (eq_const_mnt_inc + 0) / eq_const_div)) */ 
    else
    {
      /* mantissa + eq_const_mnt_inc =< (x * eq_const_div / eq_const_multi) */
      dnx_sand_u64_multiply_longs(x, eq_const_div, &tmp_64);
      res = dnx_sand_u64_devide_u64_long(&tmp_64, eq_const_multi, &tmp_res_64);
      res = dnx_sand_u64_to_long(&tmp_res_64, &tmp);
      if (res) /* overflow */
      {
        ex = DNX_SAND_ERR;
        goto exit ;
      } 

	  /* get the closest whole number, either cieling or floor*/
	  dnx_sand_u64_multiply_longs(tmp, eq_const_multi, &quotient_mul_dividor_64);
	  dnx_sand_u64_copy(&tmp_64, &remainder);
	  if (dnx_sand_u64_subtract_u64(&remainder, &quotient_mul_dividor_64))
		  /*(remainder = (x * eq_const_div) - (eq_const_multi * quotient)) >= 0 */
	  {
		  dnx_sand_long_to_u64(eq_const_multi / 2, &divisor_half);
		  if (dnx_sand_u64_is_bigger(&remainder, &divisor_half)
			  && (tmp - eq_const_mnt_inc < max_mnt)){
			  /* if (remainder > (eq_const_multi / 2))
				 and we're not exceeding the maximum mantissa take the round up*/
				  tmp += 1;
		  }
	  }
	  
      *mnt = tmp - eq_const_mnt_inc;
      if(*mnt < min_mnt) {
        *mnt = min_mnt;
      }
    }
    goto exit;
  }

  /* if (x >= exp_man * (1 << max_exp)) */
  dnx_sand_u64_multiply_longs(exp_man, (uint32)(1 << max_exp), &tmp_val);
  res = dnx_sand_u64_to_long(&tmp_val, &tmp); 
  if (!res /* no overflow */ && (x >= tmp))
  {
    *exp = max_exp;
    *mnt = max_mnt;
    goto exit;
  }

  bin_serach_beg = 0;  
  bin_serach_end = max_exp;
  bin_serach_num_elements = max_exp;
  bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
  bin_serach_low_val = exp_man * (1 << bin_serach_mid);
  bin_serach_high_val = exp_man * (1 << (bin_serach_mid+1));

  while((bin_serach_beg <= bin_serach_end) && 
        !((x > bin_serach_low_val) && (x <= bin_serach_high_val)))  
  {
    if (x <= bin_serach_low_val)  
    {      
      bin_serach_end = bin_serach_mid - 1;
      bin_serach_num_elements = bin_serach_num_elements/2;      
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;  
    }
    else    
    {      
      bin_serach_beg = bin_serach_mid + 1;     
      bin_serach_num_elements = bin_serach_num_elements/2;
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
    }
    bin_serach_low_val = exp_man * (1 << bin_serach_mid);
    bin_serach_high_val = exp_man * (1 << (bin_serach_mid+1));
  }

  *exp = bin_serach_mid + 1;
  /* (x * eq_const_div) / (eq_const_multi * (1 << *exp)) should be equal to eq_const_mnt_inc + mnt */
  dnx_sand_u64_multiply_longs(x, eq_const_div, &tmp_64); 
  res = dnx_sand_u64_devide_u64_long(&tmp_64, eq_const_multi, &tmp_res_64);
  res = dnx_sand_u64_devide_u64_long(&tmp_res_64, (1 << *exp), &tmp_64);
  res = dnx_sand_u64_to_long(&tmp_64, &tmp);
  if (res) /* overflow */
  {
    ex = DNX_SAND_ERR;
    goto exit ;
  }
  *mnt = tmp - eq_const_mnt_inc;

  /* It must hold that *mnt <= max_mnt:
   * With the current exponent and the maximal mantissa the value is not greater than x (the binary search uses max_mnt to find the exponent). 
   */
  /* assert(*mnt <= max_mnt); */
  if (mnt && *mnt < min_mnt)
  {
    /* The exponent must be greater than 0 - this is checked in the beginning of the function. */
    /* assert(*exp > 0); */
    --(*exp);
    *mnt = 2*tmp - eq_const_mnt_inc;

    /* we don't really need to check that *mnt >= min_mnt becuase according to the results of the binary search of adequate exponent (searched with max_mnt),
       after the decrement of the exponent, with max_mnt we get a value smaller than x. */
    /* assert(*mnt >= min_mnt); */
    if(*mnt > max_mnt) {
      *mnt = max_mnt;
    }
  }

exit:
  return ex ;
}

/*****************************************************
*NAME
*   dnx_sand_break_complex_to_mnt_reverse_exp_round_down
*TYPE:
*  PROC
*DATE:
*  06/06/2010
*FUNCTION:
*  Get a number, x.
*  Return mantissa (mnt) and exponent (exp).
*  Such that, "y = eq_const_multi * (eq_const_mnt_inc + mnt) / 2^exp" is the closest smaller equal number to x.
*CALLING SEQUENCE:
*   dnx_sand_break_comlicated_to_mnt_exp_round_down(x, max_mnt, max_exp, eq_const_multi, eq_const_mnt_inc, mnt, exp)
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN  uint32  x,
*       Number to dis-assemble
*    DNX_SAND_IN  uint32 max_mnt,
*       Max size of mantissa
*    DNX_SAND_IN  uint32 max_exp,
*       Max size of exponent
*    DNX_SAND_IN  uint32 eq_const_multi,
*       Added const number to the equation
*    DNX_SAND_IN  uint32 eq_const_mnt_inc,
*       Multiple const number to the equation
*    DNX_SAND_OUT uint32* mnt,
*       Buffer to load mantissa result
*    DNX_SAND_OUT uint32* exp
*       Buffer to load exponent result
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    None.
*  DNX_SAND_INDIRECT:
*    mnt, exp.
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/

DNX_SAND_RET
  dnx_sand_break_complex_to_mnt_reverse_exp_round_down(
    DNX_SAND_IN  uint32  x,
    DNX_SAND_IN  uint32 max_mnt,
    DNX_SAND_IN  uint32 max_exp,
    DNX_SAND_IN  uint32 eq_const_multi,
    DNX_SAND_IN  uint32 eq_const_mnt_inc,
    DNX_SAND_OUT uint32* mnt,
    DNX_SAND_OUT uint32* exp
  )
{
  DNX_SAND_RET
    ex = DNX_SAND_OK;
  uint32
    bin_serach_beg,
    bin_serach_end,
    bin_serach_num_elements,
    bin_serach_mid,
    bin_serach_low_val,
    bin_serach_high_val,
    exp_man;

  if ((mnt == NULL) || (exp == NULL) || (eq_const_multi == 0))
  {
    ex = DNX_SAND_ERR ;
    goto exit ;
  }

  exp_man = eq_const_multi * (eq_const_mnt_inc + max_mnt);

  if (x <= exp_man / (1 << max_exp))
  {
    *exp = max_exp;
    *mnt = 0;
    goto exit;
  }

  if (x >= exp_man)
  {
    *exp = 0;
    *mnt = max_mnt;
    goto exit;
  }

  bin_serach_beg = 0;  
  bin_serach_end = max_exp;
  bin_serach_num_elements = max_exp;
  bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
  bin_serach_low_val = exp_man / (1 << bin_serach_mid);
  bin_serach_high_val = exp_man / (1 << (bin_serach_mid+1));

  while((bin_serach_beg <= bin_serach_end) && 
        !((x >= bin_serach_high_val) && (x <= bin_serach_low_val)))  
  {
    if (x > bin_serach_low_val)	  
    {      
      bin_serach_end = bin_serach_mid - 1;
      bin_serach_num_elements = bin_serach_num_elements/2;      
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;  
    }
    else    
    {      
      bin_serach_beg = bin_serach_mid + 1;     
      bin_serach_num_elements = bin_serach_num_elements/2;
      bin_serach_mid = bin_serach_beg + bin_serach_num_elements/2;
    }
    bin_serach_low_val = exp_man / (1 << bin_serach_mid);
    bin_serach_high_val = exp_man / (1 << (bin_serach_mid+1));
  }

  *exp = bin_serach_mid;
  if ((x * (1 << *exp) / eq_const_multi) < eq_const_mnt_inc)
  {
    *mnt = 0;
  }
  else
  {
    *mnt = (x * (1 << *exp) / eq_const_multi) - eq_const_mnt_inc;
  }
 
exit:
  return ex ;

}
    

/*****************************************************
*NAME
*   dnx_sand_abs_val_to_mnt_binary_fraction_exp
*TYPE:
*  PROC
*DATE:
*  16-Jun-05
*FUNCTION:
*  Get an absolute value and a maximum (reference) value, and calculate
*  the fraction (in a binary fraction mantissa and an exponent) that matches this value.
*  By default the value is rounded down, but a 'round up' directive can
*  be optionally specified by the caller.
*RESULT:
* m0 = mantissa[bit0]
*
*   (m0/(2^1) + m1/(2^2) + ... +mN/(2^N)/2^exp = 1/abs_val
*
*CALLING SEQUENCE:
*
*INPUT:
*  DNX_SAND_DIRECT:
*    const uint32 abs_val -
*     An absolute value that should be converted to a fraction
*    const uint32 mnt_nof_bits -
*     Size in bits of the binary fraction mantissa
*    const uint32 exp_nof_bits -
*     Size in bits of the exponent
*    const uint32 max_val -
*     The maximum (reference) value from which to calculate a fraction
*    const uint32 round_up -
*     Indicator:
*     '1' - Round up the result.
*     '0' - Don't round up the result.
*    uint32* mnt_bin_fraction -
*     loaded with the result mantissa (binary fraction)
*    uint32* exp -
*     loaded with the result exponent
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_RET -
*
*  DNX_SAND_INDIRECT:
*    mnt_bin_fraction
*    exp
*REMARKS:
* None.
*SEE ALSO:
*****************************************************/
DNX_SAND_RET
  dnx_sand_abs_val_to_mnt_binary_fraction_exp(
    DNX_SAND_IN  uint32  abs_val_numerator,
    DNX_SAND_IN  uint32  abs_val_denominator,
    DNX_SAND_IN  uint32   mnt_nof_bits,
    DNX_SAND_IN  uint32   exp_nof_bits,
    DNX_SAND_IN  uint32  max_val,
    DNX_SAND_OUT uint32* mnt_bin_fraction,
    DNX_SAND_OUT uint32* exp
  )
{
  DNX_SAND_RET
    ex ;
  uint32
    tmp_num,
    tmp_exp,
    real_max_val;

  ex = DNX_SAND_OK ;
  real_max_val = (1<<((1<<exp_nof_bits) + mnt_nof_bits - 1));

  if(abs_val_numerator <= abs_val_denominator)
  {
    *mnt_bin_fraction = (1<< mnt_nof_bits) -1;
    DNX_SAND_LIMIT_FROM_ABOVE(*mnt_bin_fraction, max_val-1);
    *exp              = 0;
    goto exit;
  }
  tmp_exp = (1<<exp_nof_bits) - 1;

  if(abs_val_numerator > max_val * abs_val_denominator)
  {
    tmp_num = (real_max_val * abs_val_denominator) / max_val;
  }
  else
  {
    tmp_num = (real_max_val * abs_val_denominator) / abs_val_numerator;
  }

  if(tmp_num == 0)
  {
    tmp_num = 1;
  }

  while(tmp_num >= (uint32)(1<< mnt_nof_bits))
  {
    tmp_num /= 2;
    tmp_exp -= 1;
  }

  *mnt_bin_fraction = tmp_num;
  *exp = tmp_exp;
exit:
  return ex ;
}

DNX_SAND_RET
  dnx_sand_mnt_binary_fraction_exp_to_abs_val(
    DNX_SAND_IN  uint32   mnt_nof_bits,
    DNX_SAND_IN  uint32   exp_nof_bits,
    DNX_SAND_IN  uint32  max_val,
    DNX_SAND_IN  uint32  mnt_bin_fraction,
    DNX_SAND_IN  uint32  exp,
    DNX_SAND_OUT uint32  *abs_val_numerator,
    DNX_SAND_OUT uint32  *abs_val_denominator
  )
{
  DNX_SAND_RET
    ex ;
  uint32
    tmp_num,
    tmp_exp,
    real_max_val;

  ex = DNX_SAND_OK ;

  *abs_val_denominator = 1;
  if((mnt_bin_fraction == (uint32)((1<< mnt_nof_bits) -1)) &&
     (exp == 0)
    )
  {
    *abs_val_numerator   = 1;
    goto exit;
  }
  tmp_num = mnt_bin_fraction;
  tmp_exp = (1<<exp_nof_bits) - 1;
  real_max_val = (1<<((1<<exp_nof_bits) + mnt_nof_bits - 1));

  while(tmp_exp > exp)
  {
    tmp_num *= 2;
    tmp_exp -= 1;
  }

  *abs_val_numerator = 0;
  if(tmp_num)
  {
    if(tmp_num > ((1<< mnt_nof_bits) -1))
    {
      *abs_val_denominator = (1<< mnt_nof_bits);
    }
    *abs_val_numerator = (real_max_val * (*abs_val_denominator)) / tmp_num;
  }

  if((*abs_val_numerator) / (*abs_val_denominator)> max_val)
  {
    (*abs_val_numerator) = max_val * (*abs_val_denominator);
  }
exit:
  return ex ;
}



uint32
  dnx_sand_abs(
    DNX_SAND_IN int x
  )
{
  int
    y;
  if(x<0)
  {
     y = -x;
  }
  else
  {
     y = x;
  }
  return y;
}

int
  dnx_sand_sign(
    DNX_SAND_IN int x
  )
{
  int
    sign;

  if (x < 0)
  {
    sign = -1;
  }
  else if (x == 0)
  {
    sign = 0;
  }
  else
  {
    sign = 1;
  }
  return sign;
}

/*****************************************************
*NAME
*  dnx_sand_is_even
*TYPE:
*  PROC
*DATE:
*  23-Dec-02
*FUNCTION:
*  return TRUE iff x is even.
*INPUT:
*  DNX_SAND_DIRECT:
*    DNX_SAND_IN uint32 x -
*  DNX_SAND_INDIRECT:
*    None.
*OUTPUT:
*  DNX_SAND_DIRECT:
*    int -
*      TRUE iff x is even
*  DNX_SAND_INDIRECT:
*    NON
*REMARKS:
*  None.
*SEE ALSO:
*****************************************************/
int
  dnx_sand_is_even(
    DNX_SAND_IN uint32 x
  )
{
  int ret;

  if(x%2)
  {
    ret = FALSE;
  }
  else
  {
    ret = TRUE;
  }

  return ret;
}


#if defined (DNX_SAND_DEBUG)
/* { */
/*
 */

static uint32
  dnx_sand_msb_bit_on_TEST(
    DNX_SAND_IN uint32 x
  )
{
  int
    i;

  if (x==0)
  {
    return 0;
  }

  i=(sizeof(uint32)*8) - 1;
  for (; i>=0; --i)
  {
    if ((DNX_SAND_BIT(i)&x) != 0)
    {
      return (i);
    }
  }
  /*
   * the function can't reach here.
   */
  return 0;
}

uint32
  dnx_sand_mnt_binary_fraction_test(
    DNX_SAND_IN uint32 silent
  )
{
  uint32
    res = 0;
  uint32
    abs_val_numerator_i,
    abs_val_denominator_i,
    exact_abs_val_numerator_i,
    exact_abs_val_denominator_i,
    max_val_i,
    mnt,
    exp;

  for(max_val_i = 100; max_val_i < 4000; max_val_i+=100)
  {
    for(abs_val_denominator_i = 1; abs_val_denominator_i < 60; abs_val_denominator_i++)
    {
      for(abs_val_numerator_i = abs_val_denominator_i;
          abs_val_numerator_i < 600;
          abs_val_numerator_i++
         )
      {
        dnx_sand_abs_val_to_mnt_binary_fraction_exp(
          abs_val_numerator_i,abs_val_denominator_i,5,3,max_val_i,&mnt,&exp
        );
        dnx_sand_mnt_binary_fraction_exp_to_abs_val(
          5,3,max_val_i,mnt,exp,&exact_abs_val_numerator_i, &exact_abs_val_denominator_i
        );
        if((abs_val_numerator_i * 100 / abs_val_denominator_i) >
           (exact_abs_val_numerator_i * 110 / exact_abs_val_denominator_i)
          )
        {
          if(max_val_i * abs_val_denominator_i < abs_val_numerator_i)
          {
            if((abs_val_numerator_i * 100 / abs_val_denominator_i) >
               (max_val_i * 110 / abs_val_denominator_i)
              )
            {
              continue;
            }
          }
          res =1;
          goto exit;
        }
        if((abs_val_numerator_i * 100 / abs_val_denominator_i) <
           (exact_abs_val_numerator_i * 50 / exact_abs_val_denominator_i)
          )
        {
          res =2;
          goto exit;
        }
      }

    }
  }

exit:
  if(res && !silent)
  {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("dnx_sand_mnt_binary_fraction_test() returned with an error.\n\r")));
  }
  return res;
}

uint32
  dnx_sand_integer_arithmetic_test(
    DNX_SAND_IN uint32 silent
  )
{
  uint32
    pass,
    seed;
  uint32
    seconds,
    nano_seconds,
    nof_test,
    test_number,
    answer_1,
    answer_2;
  const char
    *proc_name = "dnx_sand_integer_arithmetic_test";

  pass = TRUE;

  if(!silent)
  {
    LOG_INFO(BSL_LS_SOC_COMMON,
             (BSL_META("****%s**** START\n"), proc_name));
  }


  dnx_sand_os_get_time(&seconds, &nano_seconds);
  seed = (uint32) (nano_seconds ^ seconds);

  dnx_sand_os_srand( seed );


  /*
   * Test Type 1:
   * Random Number
   */
  nof_test = 20000;
  while(--nof_test)
  {
    test_number = dnx_sand_os_rand();

    answer_1 = dnx_sand_msb_bit_on_TEST(test_number);
    answer_2 = dnx_sand_msb_bit_on(test_number);

    if( answer_1 != answer_2 )
    {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("**** Test Type 1: %s - FAIL\n"), proc_name));
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("**** answer_1 != answer_2, test_number = 0x%X, nof_test=%u, seed=%u \n"), test_number, nof_test, seed));
      pass = FALSE;
      goto exit;
    }
  }

  /*
   * Test Type 1:
   * Random leadinf zeros Number
   */
  nof_test = 20000;
  while(--nof_test)
  {
    test_number = dnx_sand_os_rand();
    test_number >>= (dnx_sand_os_rand()%32);

    answer_1 = dnx_sand_msb_bit_on_TEST(test_number);
    answer_2 = dnx_sand_msb_bit_on(test_number);

    if( answer_1 != answer_2 )
    {
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("**** Test Type 2: %s - FAIL\n"), proc_name));
      LOG_INFO(BSL_LS_SOC_COMMON,
               (BSL_META("**** answer_1 != answer_2, test_number = 0x%X, nof_test=%u, seed=%u \n"), test_number, nof_test, seed));
      pass = FALSE;
      goto exit;
    }
  }


exit:
  return pass;
}

void
  dnx_sand_print_u_long_binary_format(
    DNX_SAND_IN uint32 x,
    DNX_SAND_IN uint32 max_digits_in_number
  )
{
  uint32
    bit_i;

  for(bit_i=DNX_SAND_MIN(32, max_digits_in_number);bit_i>0;)
  {
    if(x & DNX_SAND_BIT(--bit_i))
    {
      LOG_CLI((BSL_META("1")));
    }
    else
    {
      LOG_CLI((BSL_META("0")));
    }
  }
}

/*
 * }
 */
#endif
