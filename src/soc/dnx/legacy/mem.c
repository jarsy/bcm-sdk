 ...
 *
 * SOC memory access implementation for DNX
 */
 ...
 */
typedef struct soc_dnx_mem_access_info_s {
...
} soc_dnx_mem_access_info_t;

CONST soc_dnx_mem_access_info_t soc_petrab_mem_access_info [] = {
...
#define SOC_PETRAB_MEM_ACCESS_INFO_LEN (sizeof(soc_petrab_mem_access_info)/ \
                                        sizeof(soc_dnx_mem_access_info_t))
...
int
_soc_dnx_indirect_mem_access_info_get(int unit, soc_mem_t mem, 
                                CONST soc_dnx_mem_access_info_t **pp_mem_access_info)
{
    int             idx;
    soc_block_t     blk;
...
int
_soc_dnx_indirect_mem_access_trigger(int unit, int trig_offset)
{
    int     rv;
    uint32          data;
                             ...
                             void *entry_data, int mem_op)
{
    ...
    while (words_accessed < words_to_access) {
        if (mem_op == MEM_TABLE_READ) {
            ...
            /* trigger read op */
            rv = _soc_dnx_indirect_mem_access_trigger(unit, trig_offset);
            if (SOC_FAILURE(rv)) {
               LOG_ERROR(BSL_LS_SOC_MEM,
                         (BSL_META_U(unit,
                                     "Indirect memory access failed(%s). "
...
int
_soc_dnx_mem_access_valid(int unit, soc_mem_t mem, int copyno, int index)
{
...
int
soc_dnx_mem_read(int unit, soc_mem_t mem, int copyno, int index, void *entry_data)
{
    int rv;
    ...
    rv = _soc_dnx_mem_access_valid(unit, mem, copyno, index);
    if (SOC_FAILURE(rv)) {
       LOG_ERROR(BSL_LS_SOC_MEM,
                 (BSL_META_U(unit,
    ...
    rv = _soc_dnx_indirect_mem_access(unit, mem, copyno, index, entry_data, 
                                      MEM_TABLE_READ);
    if (SOC_FAILURE(rv)) {
       LOG_ERROR(BSL_LS_SOC_MEM,
                 (BSL_META_U(unit,
...
int
soc_dnx_mem_write(int unit, soc_mem_t mem, int copyno, int index, void *entry_data)
{
    int rv;
