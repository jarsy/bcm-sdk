/* 
 * $Id: headers.c,v 1.5 Broadcom SDK $
 *
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * SOC utility routines for managing DNX headers
...
#include <soc/dnx/legacy/headers.h>
#include <soc/cm.h>
#include <soc/debug.h>
...
static char *soc_dnx_itmh_field_names[] = {
    SOC_ITMH_FIELD_NAMES_INIT
};

static char *soc_dnx_ftmh_field_names[] = {
    SOC_FTMH_FIELD_NAMES_INIT
};

static char *soc_dnx_otmh_field_names[] = {
    SOC_OTMH_FIELD_NAMES_INIT
};

static char *soc_dnx_hdr_type_names[] = {
    SOC_DNX_HDR_NAMES_INIT
};

/* Return the field type corresponding to name */
soc_dnx_itmh_field_t
soc_dnx_itmh_name_to_field(int unit, char *name)
{
    soc_dnx_itmh_field_t   field;
    COMPILER_REFERENCE(unit);
    
    assert((COUNTOF(soc_dnx_itmh_field_names) - 1) == ITMH_COUNT);
    
    for (field = 0; soc_dnx_itmh_field_names[field] != NULL; field++) {
        if (sal_strcmp(name, soc_dnx_itmh_field_names[field]) == 0) {
            return field;
        }
    }
...
/* Return the field name corresponding to type */
char *
soc_dnx_itmh_field_to_name(int unit, soc_dnx_itmh_field_t field)
{
    COMPILER_REFERENCE(unit);
    ...
    return soc_dnx_itmh_field_names[field];
}

/* Set the fields in Ingress TM header */
void
soc_dnx_itmh_field_set(int unit, soc_dnx_itmh_t *itmh, 
                       soc_dnx_itmh_field_t field, uint32 val)
{
    soc_dnx_itmh_base_t     *base;
    soc_dnx_itmh_sp_ext_t   *ext;
...
/* Get the fields in Ingress TM header */
uint32
soc_dnx_itmh_field_get(int unit, soc_dnx_itmh_t *itmh, 
                       soc_dnx_itmh_field_t field)
{
    soc_dnx_itmh_base_t     *base;
    soc_dnx_itmh_sp_ext_t   *ext;
...
soc_dnx_ftmh_name_to_field(int unit, char *name)
{
    soc_dnx_ftmh_field_t   field;
    COMPILER_REFERENCE(unit);
    
    assert((COUNTOF(soc_dnx_ftmh_field_names) - 1) == FTMH_COUNT);
    
    for (field = 0; soc_dnx_ftmh_field_names[field] != NULL; field++) {
        if (sal_strcmp(name, soc_dnx_ftmh_field_names[field]) == 0) {
            return field;
        }
    }
...
/* Return the field name corresponding to type */
char *
soc_dnx_ftmh_field_to_name(int unit, soc_dnx_ftmh_field_t field)
{
    COMPILER_REFERENCE(unit);
    ...
    return soc_dnx_ftmh_field_names[field];
}

/* Set the fields in Fabric TM header */
void
soc_dnx_ftmh_field_set(int unit, soc_dnx_ftmh_t *ftmh, 
                       soc_dnx_ftmh_field_t field, uint32 val)
{
    ...
    case FTMH_is_flow:
        ...
        break;
    case FTMH_flow_id:
        outlif_ext->unicast_flow.is_flow = 1; /* mark it as flow */
        FIELD_SET_HI_LO(outlif_ext->unicast_flow.flow_id, val, 7);
        break;
    case FTMH_dest_port:
        outlif_ext->unicast_dest_port.is_flow = 0; /* mark it as not a flow */
...
/* Get the fields in Fabric TM header */
uint32
soc_dnx_ftmh_field_get(int unit, soc_dnx_ftmh_t *ftmh, 
                       soc_dnx_ftmh_field_t field)
{
    soc_dnx_ftmh_base_t         *base;
    soc_dnx_ftmh_outlif_ext_t   *outlif_ext;
...
soc_dnx_otmh_name_to_field(int unit, char *name)
{
    soc_dnx_otmh_field_t   field;
    COMPILER_REFERENCE(unit);
    
    assert((COUNTOF(soc_dnx_otmh_field_names) - 1) == OTMH_COUNT);
    
    for (field = 0; soc_dnx_otmh_field_names[field] != NULL; field++) {
        if (sal_strcmp(name, soc_dnx_otmh_field_names[field]) == 0) {
            return field;
        }
    }
...
/* Return the field name corresponding to type */
char *
soc_dnx_otmh_field_to_name(int unit, soc_dnx_otmh_field_t field)
{
    COMPILER_REFERENCE(unit);
    ...
    return soc_dnx_otmh_field_names[field];
}

/* Set the fields in Outgoing TM header */
void
soc_dnx_otmh_field_set(int unit, soc_dnx_otmh_t *otmh, 
...
/* Get the fields in Outgoing TM header */
uint32
soc_dnx_otmh_field_get(int unit, soc_dnx_otmh_t *otmh, 
                       soc_dnx_otmh_field_t field)
{
    soc_dnx_otmh_base_t         *base;
    soc_dnx_otmh_outlif_ext_t   *outlif_ext;
...
/* returns length of the header based on type */
int
soc_dnx_hdr_len_get(int unit, DNX_HDR_hdr_type_t type)
{
    switch (type) {
    case DNX_HDR_none:
        return 0;
    case DNX_HDR_itmh_base:
        return DNX_HDR_ITMH_BASE_LEN + 15; 
    case DNX_HDR_itmh_sp_ext:
        return DNX_HDR_ITMH_SP_EXT_LEN;
    case DNX_HDR_ftmh_base:
        return DNX_HDR_FTMH_BASE_LEN;
    case DNX_HDR_ftmh_outlif_ext:
        return DNX_HDR_FTMH_OUTLIF_EXT_LEN;
    case DNX_HDR_ftmh_mc_lb_ext:
        return DNX_HDR_FTMH_MC_LB_EXT_LEN;
    case DNX_HDR_ftmh_outlif_mc_lb_ext:
        return DNX_HDR_FTMH_OUTLIF_MC_LB_EXT_LEN;
    case DNX_HDR_otmh_base:
        return DNX_HDR_OTMH_BASE_LEN;
    case DNX_HDR_otmh_outlif_ext:
        return DNX_HDR_OTMH_OUTLIF_EXT_LEN;
    case DNX_HDR_otmh_sp_ext:
        return DNX_HDR_OTMH_SP_EXT_LEN;
    case DNX_HDR_otmh_dp_ext:
        return DNX_HDR_OTMH_DP_EXT_LEN;
    case DNX_HDR_otmh_outlif_sp_ext:
        return DNX_HDR_OTMH_OUTLIF_SP_EXT_LEN;
    case DNX_HDR_otmh_outlif_dp_ext:
        return DNX_HDR_OTMH_OUTLIF_DP_EXT_LEN;
    case DNX_HDR_otmh_outlif_sp_dp_ext:
        return DNX_HDR_OTMH_OUTLIF_SP_DP_EXT_LEN;
    default:
       LOG_VERBOSE(BSL_LS_SOC_HEADERS,
                   (BSL_META_U(unit,
                               "Unit:%d soc_dnx_hdr_len_get: Unknown header"
                               " type =%d \n"), unit, type));
        break;
    }
...
/* Return the header type corresponding to name */
DNX_HDR_hdr_type_t
soc_dnx_hdr_type_get(int unit, char *name)
{
    DNX_HDR_hdr_type_t  type;
    COMPILER_REFERENCE(unit);
    
    assert((COUNTOF(soc_dnx_hdr_type_names) - 1) == DNX_HDR_COUNT);
    
    for (type = 0; soc_dnx_hdr_type_names[type] != NULL; type++) {
        if (sal_strcmp(name, soc_dnx_hdr_type_names[type]) == 0) {
            return type;
        }
    }
...
/* Return the header name corresponding to type */
char *
soc_dnx_hdr_type_to_name(int unit, DNX_HDR_hdr_type_t type)
{
    COMPILER_REFERENCE(unit);

    assert((COUNTOF(soc_dnx_hdr_type_names) - 1) == DNX_HDR_COUNT);

    if (type < 0 || type >= DNX_HDR_COUNT) {
        return "??";
    }

    return soc_dnx_hdr_type_names[type];
}
