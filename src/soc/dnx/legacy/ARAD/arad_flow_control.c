...
#define JER2_ARAD_CAL_MODE_NDX_MAX                                    (DNX_TMC_FC_NOF_CAL_MODES-1)
...
#define JER2_ARAD_FC_PFC_GENERIC_BITMAP_SIZE DNX_TMC_FC_PFC_GENERIC_BITMAP_SIZE
...
#define FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_MAX               DNX_TMC_EGR_NOF_Q_PRIO_JER2_ARAD
...
#define JER2_ARAD_FC_CAL_MODE_IS_ILKN(c_mode) \
  DNX_SAND_NUM2BOOL((c_mode == DNX_TMC_FC_CAL_MODE_ILKN_INBND) || (c_mode ==DNX_TMC_FC_CAL_MODE_ILKN_OOB))
...
#define FC_ENABLE_NOT_SUPPORT_JER2_ARAD {    \
    DNX_TMC_FC_GLB_RSC_TO_NIF_LLFC_EN,  \
    DNX_TMC_FC_GLB_RSC_TO_NIF_PFC_EN,   \
    DNX_TMC_FC_NIF_TO_GEN_PFC_EN,       \
    DNX_TMC_FC_STAT_VSQ_TO_HCFC_EN      \
}
  ...
  )
{
    ...
    if (cal_type == JER2_ARAD_FC_CAL_TYPE_RX){
        if(JER2_ARAD_FC_CAL_MODE_IS_ILKN(cal_mode)) {
            cal_table_tmp = (if_ndx == DNX_TMC_FC_OOB_ID_A ? CFC_ILKN_RX_0_CALm : CFC_ILKN_RX_1_CALm);
        }
        else {
            cal_table_tmp = (if_ndx == DNX_TMC_FC_OOB_ID_A ? CFC_SPI_OOB_RX_0_CALm : CFC_SPI_OOB_RX_1_CALm);
        }
    }
    else {
        if(JER2_ARAD_FC_CAL_MODE_IS_ILKN(cal_mode)) {
            cal_table_tmp = (if_ndx == DNX_TMC_FC_OOB_ID_A ? CFC_ILKN_TX_0_CALm : CFC_ILKN_TX_1_CALm);
        }
        else {
            cal_table_tmp = (if_ndx == DNX_TMC_FC_OOB_ID_A ? CFC_SPI_OOB_TX_0_CALm : CFC_SPI_OOB_TX_1_CALm);
        }
    }
...
/*********************************************************************
* NAME:
*     jer2_arad_flow_control_regs_init
* FUNCTION:
  ...
  )
{
    ...
    rv = jer2_arad_fc_enables_set(unit, &cfc_enables, &cfc_enables);
    DNXC_IF_ERR_EXIT(rv);

    /* Clear Calendars  - ILKN */
    for (ilkn_ndx = 0; ilkn_ndx < SOC_DNX_CONFIG(unit)->tm.max_interlaken_ports; ilkn_ndx++) {
        rv = jer2_arad_fc_clear_calendar_unsafe(unit, DNX_TMC_FC_CAL_MODE_ILKN_INBND, JER2_ARAD_FC_CAL_TYPE_RX, ilkn_ndx);
        DNXC_IF_ERR_EXIT(rv);
        rv = jer2_arad_fc_clear_calendar_unsafe(unit, DNX_TMC_FC_CAL_MODE_ILKN_INBND, JER2_ARAD_FC_CAL_TYPE_TX, ilkn_ndx);
        DNXC_IF_ERR_EXIT(rv);

        if ((SOC_DNX_CONFIG(unit)->tm.fc_inband_intlkn_type[ilkn_ndx] == DNX_TMC_FC_CAL_INB_TYPE_ILKN) && 
            (SOC_DNX_CONFIG(unit)->tm.fc_inband_intlkn_mode[ilkn_ndx] & SOC_DNX_FC_CAL_MODE_TX_ENABLE)) {
            ...
            if (cal_length >= SOC_DNX_DEFS_GET(unit, fc_inb_cal_len_max)){
                DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("in-band calendar length is out of range [0, %d]"), (SOC_DNX_DEFS_GET(unit, fc_inb_cal_len_max)-1)));
            }
    ...
    for (oob_ndx = 0; oob_ndx < SOC_DNX_CONFIG(unit)->tm.max_oob_ports; oob_ndx++) {
        rv = jer2_arad_fc_clear_calendar_unsafe(unit, DNX_TMC_FC_CAL_MODE_SPI_OOB, JER2_ARAD_FC_CAL_TYPE_RX, oob_ndx);
        DNXC_IF_ERR_EXIT(rv);
        rv = jer2_arad_fc_clear_calendar_unsafe(unit, DNX_TMC_FC_CAL_MODE_SPI_OOB, JER2_ARAD_FC_CAL_TYPE_TX, oob_ndx);
        DNXC_IF_ERR_EXIT(rv);
    }
    ...
    for (ilkn_ndx = 0; ilkn_ndx < SOC_DNX_CONFIG(unit)->tm.max_interlaken_ports; ilkn_ndx++) {
        ...
        rv = jer2_arad_fc_ilkn_mub_gen_cal_set(unit, ilkn_ndx, &ilkn_cal);
        DNXC_IF_ERR_EXIT(rv);
    }
...
soc_error_t 
    jer2_arad_fc_shr_mapping(int unit, int fcv_bit, int cl_index, int select, int valid)
{
    soc_error_t 
        rv = SOC_E_NONE;
    uint32
    ...
    if (fcv_bit >= ARDON_FC_FCV_BIT_LEN) {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_SOC_MSG("fcv_bit=%d is out of range [0..%d]"), fcv_bit, ARDON_FC_FCV_BIT_LEN - 1)); 
    }
...
soc_error_t
    jer2_arad_fc_init_shr_mapping(int unit)
{
    soc_error_t
        rv = SOC_E_NONE;
    uint32
    ...
    )
{
    ...
    {
        rv = dnx_port_sw_db_flags_get(unit, logical_port, &flags);
        DNXC_IF_ERR_EXIT(rv);
        if (!(DNX_PORT_IS_ELK_INTERFACE(flags) || DNX_PORT_IS_STAT_INTERFACE(flags))) {
            ...
            if (SOC_IS_ARDON(unit)) {
                ...
                valid = SOC_DNX_CONFIG(unit)->jer2_arad->init.fc.cl_sch_enable ? 0x0 : 0x1;
            } else {
                valid = 0x1;
            }

            entry_offset = ((interface_i * DNX_TMC_EGR_NOF_Q_PRIO_JER2_ARAD) / FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD);
            ...
            if(nof_priorities > FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD) {
                rv = soc_mem_read(unit, CFC_NIF_PFC_MAPm, MEM_BLOCK_ANY, entry_offset + 1, nif_pfc_data_2);
                DNXC_IF_ERR_EXIT(rv);
            }

            for(i = 0; i < nof_priorities; i++) {
                field_offset = i % FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD;
                data_select = (i < FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD) ? nif_pfc_data : nif_pfc_data_2;
  ...
  )
{
    ...
    for(interface_number = 0; interface_number < DNX_TMC_FC_NOF_OOB_IDS; interface_number++)
    {
        ...
        if(tm->fc_oob_mode[interface_number] & SOC_DNX_FC_CAL_MODE_RX_ENABLE)
        {
            ...
            fld_val = 0x0;
            fld = (interface_number == DNX_TMC_FC_OOB_ID_A ? ILKN_RX_0_ENf : ILKN_RX_1_ENf);
            ...
            /* Swapping option between INB IF and OOB IF - only relevant to JER2_ARAD */
            fld = (interface_number == DNX_TMC_FC_OOB_ID_A ? ILKN_RX_0_SELf : ILKN_RX_1_SELf);                                            
            rv = soc_reg_above_64_field32_modify(unit, CFC_ILKN_RX_CONFIGURATIONr, REG_PORT_ANY, 0, fld,  fld_val);
            DNXC_IF_ERR_EXIT(rv);
            ...
            cal_reps = tm->fc_oob_type[interface_number] == DNX_TMC_FC_CAL_TYPE_ILKN ? 1 : 
            ...
            rv = jer2_arad_fc_cal_tbl_get(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_RX, interface_number, &cal_table);
            DNXC_IF_ERR_EXIT(rv);
            for (per_rep_idx = 0; per_rep_idx < cal_length; per_rep_idx++) {
                soc_mem_field32_set(unit, cal_table, cal_table_data, FC_INDEXf, 0);
                soc_mem_field32_set(unit, cal_table, cal_table_data, FC_DST_SELf, jer2_arad_fc_rec_cal_dest_type_to_val_internal(DNX_TMC_FC_REC_CAL_DEST_NONE));
  ...
  )
{
    ...
    for(interface_number = 0; interface_number < DNX_TMC_FC_NOF_OOB_IDS; interface_number++)
    {
        ...
        soc_jer2_arad_ilkn_phy_ports_btmp_set(unit, interface_number, &phy_ports);
#endif 
        ...
        if(interfcae_status_oob_ignore) {
            ...
            DNXC_LEGACY_FIXME_ASSERT;
#ifdef FIXME_DNX_LEGACY
            SHR_BITCOPY_RANGE(&clp0_lanes, 0, &SOC_PBMP_WORD_GET(phy_ports, 0), JER2_ARAD_NIF_CLP0_PORT_MIN+1, JER2_ARAD_NIF_CLP_NOF_PORTS);
            SHR_BITCOPY_RANGE(&clp1_lanes, 0, &SOC_PBMP_WORD_GET(phy_ports, 0), JER2_ARAD_NIF_CLP1_PORT_MIN+1, JER2_ARAD_NIF_CLP_NOF_PORTS);
#endif 
        ...
        if(tm->fc_oob_mode[interface_number] & SOC_DNX_FC_CAL_MODE_TX_ENABLE)
        {
            ...
            /* Disable the FC before configuring it */
            fld_val = 0x0;
            fld = (interface_number == DNX_TMC_FC_OOB_ID_A ? ILKN_TX_0_ENf : ILKN_TX_1_ENf);
            COMPILER_64_SET(field64,0,fld_val);
            ...
            /* Swapping option between INB IF and OOB IF - only relevant to JER2_ARAD */            
            fld = (interface_number == DNX_TMC_FC_OOB_ID_A ? ILKN_TX_0_SELf : ILKN_TX_1_SELf);   
            rv = soc_reg_above_64_field64_modify(unit, CFC_ILKN_TX_CONFIGURATIONr, REG_PORT_ANY, 0, fld,  field64);
			DNXC_IF_ERR_EXIT(rv);
            ...
            if(tm->fc_oob_type[interface_number] == DNX_TMC_FC_CAL_TYPE_ILKN) {
                fld = (interface_number == JER2_ARAD_FC_ILKN_ID_A ? ILKN_TX_0_CAL_LENf : ILKN_TX_1_CAL_LENf);
            ...
            /* Clear Calendar entry */
            cal_mode_ndx = (tm->fc_oob_type[interface_number] == DNX_TMC_FC_CAL_TYPE_ILKN) ? DNX_TMC_FC_CAL_MODE_ILKN_OOB : DNX_TMC_FC_CAL_MODE_SPI_OOB;
            rv = jer2_arad_fc_cal_tbl_get(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_TX, interface_number, &cal_table);
            DNXC_IF_ERR_EXIT(rv);
            for (per_rep_idx = 0; per_rep_idx < cal_length; per_rep_idx++) {
                soc_mem_field32_set(unit, cal_table, cal_table_data, FC_INDEXf, 0);
                soc_mem_field32_set(unit, cal_table, cal_table_data, FC_SRC_SELf, jer2_arad_fc_gen_cal_src_type_to_val_internal(unit, DNX_TMC_FC_GEN_CAL_SRC_CONST));
...
int jer2_aradon_flow_control_init_cl_sch(int unit, int enable)
{
    uint32
        reg32_val,
        field32_val;
    ...
    DNXC_IF_ERR_EXIT(soc_reg32_get(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, &reg32_val));
    field32_val = enable;
    ...
    DNXC_IF_ERR_EXIT(soc_reg32_set(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, reg32_val));


    DNXC_IF_ERR_EXIT(soc_reg32_get(unit, SCH_REG_1BCr, REG_PORT_ANY, 0, &reg32_val)); /* OOB-FC Configuration Register */
    field32_val = enable;
    soc_reg_field_set(unit, SCH_REG_1BCr, &reg32_val, FIELD_0_0f, field32_val); /* OobFcEnable */
    DNXC_IF_ERR_EXIT(soc_reg32_set(unit, SCH_REG_1BCr, REG_PORT_ANY, 0, reg32_val));
    ...
    DNXC_IF_ERR_EXIT(soc_reg32_get(unit, NBI_ENABLE_PKT_BOUNDARY_LLFCr, REG_PORT_ANY, 0, &reg32_val));
    field32_val = 0x1;
    soc_reg_field_set(unit, NBI_ENABLE_PKT_BOUNDARY_LLFCr, &reg32_val, ENABLE_PKT_BOUNDARY_LLFCf, field32_val);
    DNXC_IF_ERR_EXIT(soc_reg32_set(unit, NBI_ENABLE_PKT_BOUNDARY_LLFCr, REG_PORT_ANY, 0, reg32_val));
  ...
  )
{
    ...
    rv = jer2_arad_flow_control_regs_init(unit);
    DNXC_IF_ERR_EXIT(rv);

    rv = jer2_arad_flow_control_init_oob_rx(unit);
    DNXC_IF_ERR_EXIT(rv);

    rv = jer2_arad_flow_control_init_oob_tx(unit);
    DNXC_IF_ERR_EXIT(rv);

    if (SOC_IS_ARDON(unit)) {
        rv = jer2_aradon_flow_control_init_cl_sch(unit, tm->fc_cl_sch_enable);
        DNXC_IF_ERR_EXIT(rv);
    }
  ...
  )
{
    ...
    DNXC_LEGACY_FIXME_ASSERT;
#ifdef FIXME_DNX_LEGACY
  switch(jer2_arad_nif_id2type(nif_ndx))
  {
  case JER2_ARAD_NIF_TYPE_SGMII:
    return 2;
  case JER2_ARAD_NIF_TYPE_XAUI:
    return 8;
  case JER2_ARAD_NIF_TYPE_RXAUI:
    return 8;
  default:
    return 0;
  }
...
{
    int i = 0;
    DNX_TMC_FC_ENABLE fc_enable_not_support[] = FC_ENABLE_NOT_SUPPORT_JER2_ARAD;
    ...
    for (i = 0; i < sizeof(fc_enable_not_support)/sizeof(DNX_TMC_FC_ENABLE); i++) {
        if (SHR_BITGET(cfc_enables->bmp, fc_enable_not_support[i])) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("cfc_enables[i] is not supported by this chip")));
        }
    }   
    ...
    )
{
    ...
    rv = jer2_arad_fc_enables_verify(unit, cfc_enables, ena_info);
    DNXC_IF_ERR_EXIT(rv);
    ...
    SET_FC_ENABLE(DNX_TMC_FC_ILKN_RX_TO_RET_REQ_EN, ILKN_RX_TO_RET_REQ_ENf);
    ...
    SET_FC_ENABLE(DNX_TMC_FC_SPI_OOB_RX_TO_RET_REQ_EN, SPI_OOB_RX_TO_RET_REQ_ENf);
    ...
    )
{
    ...
    GET_FC_ENABLE(DNX_TMC_FC_ILKN_RX_TO_RET_REQ_EN, ILKN_RX_TO_RET_REQ_ENf);
    ...
    GET_FC_ENABLE(DNX_TMC_FC_SPI_OOB_RX_TO_RET_REQ_EN, SPI_OOB_RX_TO_RET_REQ_ENf);
  ...
  )
{
    ...
    rv = dnx_port_sw_db_first_phy_port_get(unit, port, &offset /*one based*/);
    DNXC_IF_ERR_EXIT(rv);
    offset--;
    ...
    if (info->gen_src == DNX_TMC_FC_GEN_SRC_GLB_LOW) { 
    ...
    if (info->gen_src == DNX_TMC_FC_GEN_SRC_NIF) {
        ...
        DNXC_LEGACY_FIXME_ASSERT;
#ifdef FIXME_DNX_LEGACY
        reg_offset = offset / JER2_ARAD_MAX_NIFS_PER_WC;
        fld_offset = offset % JER2_ARAD_MAX_NIFS_PER_WC;
#endif 
  ...
  )
{
    ...
    case DNX_TMC_FC_INBND_MODE_DEVICE_DISABLED:
        SHR_BITSET(cfc_enables.bmp, (DNX_TMC_FC_LLFC_VSQ_TO_NIF_EN | DNX_TMC_FC_PFC_VSQ_TO_NIF_EN));     
        break;
    default:
    ...
    rv = jer2_arad_fc_inbnd_mode_set(unit, port, 1, info->mode);
    DNXC_IF_ERR_EXIT(rv);
    /* setting MAC registers - port level } */
    ...
    case DNX_TMC_FC_INBND_MODE_LL:
        /* Configure LL */
        rv = jer2_arad_fc_gen_inbnd_ll_set_unsafe(
            unit,
            port,
            &info->ll
          );
    ...
    case DNX_TMC_FC_INBND_MODE_SAFC:
        /* Configure PFC */
        rv = jer2_arad_fc_gen_inbnd_pfc_set_unsafe(
            unit,
            port,
            &info->pfc
          );
  ...
  )
{
    ...
    rv = jer2_arad_fc_inbnd_mode_get(unit, port, 1, &(info->mode));
    DNXC_IF_ERR_EXIT(rv);
  
    /* Get the LL configuration */
    if (info->mode == DNX_TMC_FC_INBND_MODE_LL)
    {
        rv = jer2_arad_fc_gen_inbnd_ll_get_unsafe(
                unit,
                port,
                &info->ll
              );
    ...
    {
        rv = jer2_arad_fc_gen_inbnd_pfc_get_unsafe(
                unit,
                port,
                &info->pfc
              );
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_GLB_HP_SET_UNSAFE);

  /* Enable/Disable LL */
  ll_fld_val = (fc_mode == DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_LL || fc_mode == DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_ALL);
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, FRC_NIF_LLVL_ENf,  ll_fld_val));

  /* Enable/Disable PFC */
  pfc_fld_val = (fc_mode == DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_PFC || fc_mode == DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_ALL);
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, FRC_NIF_PFC_ENf,  pfc_fld_val));

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_glb_hp_set_unsafe()", 0, 0);
}
...
{
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_GLB_HP_SET_VERIFY);

  DNX_SAND_ERR_IF_ABOVE_MAX(fc_mode, (DNX_TMC_FC_NOF_INGR_GEN_GLB_HP_MODES-1), JER2_ARAD_FC_MODE_OUT_OF_RANGE_ERR, 10, exit);

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_glb_hp_set_verify()", 0, 0);
}
...
{
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_GLB_HP_GET_VERIFY);

  JER2_ARAD_DO_NOTHING_AND_EXIT;

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_glb_hp_get_verify()", 0, 0);
}
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_GLB_HP_GET_UNSAFE);
  
  /* Get the register value */
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, FRC_NIF_LLVL_ENf, &ll_fld_val));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, FRC_NIF_PFC_ENf, &pfc_fld_val));
  
  /* Set the FC Mode according to the register values */
  if(ll_fld_val && pfc_fld_val)
  {
    *fc_mode = DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_ALL;
  }
  else if (pfc_fld_val)
  {
    *fc_mode = DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_PFC;
  }
  else if (ll_fld_val)
  {
    *fc_mode = DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_LL;
  }
  else
  {
    *fc_mode = DNX_TMC_FC_INGR_GEN_GLB_HP_MODE_NONE;
  }
  
exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_glb_hp_get_unsafe()", 0, 0);
}
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_CNM_MAP_SET_UNSAFE);

  /* 3 Regs, 8 TC Classes */
  tc_reg = tc_regs[cpid_tc_ndx / JER2_ARAD_EGQ_CNM_CPID_TO_FC_TYPE_REG_NOF_FLDS];
  tc_field = tc_fields[cpid_tc_ndx];
  
  /* Bit 0 is LL */
  /* Bits 1:8 are PFC */
  fld_val = DNX_SAND_BOOL2NUM(enable_ll);
  fld_val = fld_val | (fc_class << 1);
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, tc_reg, REG_PORT_ANY, 0, tc_field,  fld_val));

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_cnm_map_set_unsafe()", cpid_tc_ndx, 0);
}
...
{
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_CNM_MAP_SET_VERIFY);

  DNX_SAND_ERR_IF_ABOVE_MAX(cpid_tc_ndx, JER2_ARAD_FC_CLASS_MAX - 1, JER2_ARAD_CPID_TC_NDX_OUT_OF_RANGE_ERR, 10, exit);
  DNX_SAND_ERR_IF_ABOVE_MAX(fc_class, JER2_ARAD_FC_CLASS_MAX - 1, JER2_ARAD_FC_CLASS_OUT_OF_RANGE_ERR, 20, exit);

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_cnm_map_set_verify()", cpid_tc_ndx, 0);
}
...
{
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_CNM_MAP_GET_VERIFY);

  DNX_SAND_ERR_IF_ABOVE_MAX(cpid_tc_ndx, JER2_ARAD_FC_CLASS_MAX - 1, JER2_ARAD_CPID_TC_NDX_OUT_OF_RANGE_ERR, 10, exit);

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_cnm_map_get_verify()", cpid_tc_ndx, 0);
}
/*********************************************************************
*     Map the 3-LSB of the CMN CPID (represent TC), to the FC
 *     indication to generate when using Class Based Flow
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_GEN_INBND_CNM_MAP_GET_UNSAFE);
  ...
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, tc_reg, REG_PORT_ANY, 0, tc_field, &fld_val));
...
exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_gen_inbnd_cnm_map_get_unsafe()", cpid_tc_ndx, 0);
}
...
{
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_RCY_HR_ENABLE_GET_VERIFY);

  JER2_ARAD_DO_NOTHING_AND_EXIT;

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_rcy_hr_enable_get_verify()", 0, 0);
}
  ...
  )
{
    ...
    rv = jer2_arad_fc_gen_cal_set_verify(unit, cal_mode_ndx, if_ndx, cal_conf, cal_buff);
    DNXC_IF_ERR_EXIT(rv);

    SOC_REG_ABOVE_64_CLEAR(cal_table_data);

    rv = jer2_arad_fc_clear_calendar_unsafe(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_TX, if_ndx);
    DNXC_IF_ERR_EXIT(rv);
    ...
    for (per_rep_idx = 0; per_rep_idx < cal_conf->cal_len; per_rep_idx++)
    {
        ...
        source_val = jer2_arad_fc_gen_cal_src_type_to_val_internal(unit, cal_buff[per_rep_idx].source);

        if (source_val == -1)
        {
            DNXC_IF_ERR_EXIT(SOC_E_PARAM);
        }
        else if (cal_buff[per_rep_idx].source == DNX_TMC_FC_GEN_CAL_SRC_RETRANSMIT)
        {
            /* Set the ILKN Retransmit Req bits: 
            * Bit 0: ILKN-0, Bit 2: ILKN-1 */
            tx_cal.fc_source_sel = source_val;
            tx_cal.fc_index = (if_ndx == DNX_TMC_FC_OOB_ID_A ? JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_A_VAL : JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_B_VAL);
        }
        else
        {
            tx_cal.fc_source_sel = source_val;
        ...
        rv = jer2_arad_fc_cal_tbl_get(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_TX, if_ndx, &cal_table);
        DNXC_IF_ERR_EXIT(rv);
    ...
    if ((cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_INBND) || (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_OOB)) {
        /* Select ILKN Type: InBand/OOB */
        fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? ILKN_TX_0_SELf : ILKN_TX_1_SELf);
        field32 = DNX_SAND_BOOL2NUM(cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_OOB);       
        rv = soc_reg_above_64_field32_modify(unit, reg, REG_PORT_ANY, 0, fld,  field32);
		DNXC_IF_ERR_EXIT(rv);

        /* Enable ILKN InBand/OOB */
        fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? ILKN_TX_0_ENf : ILKN_TX_1_ENf);
  ...
  )
{
    ...
    rv = jer2_arad_fc_gen_cal_get_verify(unit, cal_mode_ndx, if_ndx);
    DNXC_IF_ERR_EXIT(rv);
    ...
    if (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_SPI_OOB){
        ...
        sel_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? ILKN_TX_0_SELf : ILKN_TX_1_SELf;
        rv = soc_reg_above_64_field32_read(unit, sel_reg, REG_PORT_ANY, 0, sel_fld, &sel_fld_val);
        DNXC_IF_ERR_EXIT(rv);
        if ((sel_fld_val && (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_INBND)) || 
            (!sel_fld_val && (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_OOB))) {
            en_fld_val = 0;
        }
        else {
            en_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? ILKN_TX_0_ENf : ILKN_TX_1_ENf;
            rv = soc_reg_above_64_field32_read(unit, en_reg, REG_PORT_ANY, 0, en_fld, &en_fld_val);
			DNXC_IF_ERR_EXIT(rv);
        }
    }
    ...
    else
    {
        ...
        cal_len_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? ILKN_TX_0_CAL_LENf : ILKN_TX_1_CAL_LENf);
        rv = soc_reg_above_64_field32_read(unit, cal_len_reg, REG_PORT_ANY, 0, cal_len_fld, &cal_len_val);
        DNXC_IF_ERR_EXIT(rv);
    ...
    for (entry_idx = 0; entry_idx < cal_conf->cal_len; entry_idx++)
    {
        ...
        if(tx_cal.fc_source_sel >= JER2_ARAD_FC_GEN_CAL_SRC_ARR_SIZE)
        {
            cal_buff[entry_idx].source = DNX_TMC_FC_GEN_CAL_SRC_NONE;
            cal_buff[entry_idx].id = 0;
        }
        else if(jer2_arad_fc_gen_cal_src_arr[tx_cal.fc_source_sel] == DNX_TMC_FC_GEN_CAL_SRC_RETRANSMIT)
        {
            /* Set the ILKN Retransmit Req bits: 
            * Bit 0: ILKN-0, Bit 2: ILKN-1 */
            cal_buff[entry_idx].source = jer2_arad_fc_gen_cal_src_arr[tx_cal.fc_source_sel];
            cal_buff[entry_idx].id = (tx_cal.fc_index == JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_A_VAL ? JER2_ARAD_FC_ILKN_ID_A : JER2_ARAD_FC_ILKN_ID_B);
        }
        else
        {
            cal_buff[entry_idx].source = jer2_arad_fc_gen_cal_src_arr[tx_cal.fc_source_sel];
  ...
  )
{
    ...
    rv = jer2_arad_fc_cal_tbl_get(unit, cal_mode_ndx, cal_type, if_ndx, &cal_table);
    DNXC_IF_ERR_EXIT(rv);
    if(cal_type == JER2_ARAD_FC_CAL_TYPE_RX)
    {
        soc_mem_field32_set(unit, cal_table, cal_rx_entry, FC_DST_SELf, jer2_arad_fc_rec_cal_dest_type_to_val_internal(DNX_TMC_FC_REC_CAL_DEST_NONE));
    ...
    {
        soc_mem_field32_set(unit, cal_table, cal_tx_entry, FC_SRC_SELf, jer2_arad_fc_gen_cal_src_type_to_val_internal(unit, DNX_TMC_FC_GEN_CAL_SRC_CONST));
  ...
  )
{
    ...
    rv = jer2_arad_fc_rec_cal_set_verify(unit, cal_mode_ndx, if_ndx, cal_conf, cal_buff);
    DNXC_IF_ERR_EXIT(rv);

    SOC_REG_ABOVE_64_CLEAR(cal_table_data);
    /* Clear calendar */
    rv = jer2_arad_fc_clear_calendar_unsafe(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_RX, if_ndx);
    DNXC_IF_ERR_EXIT(rv);
    ...
    for (per_rep_idx = 0; per_rep_idx < cal_conf->cal_len; per_rep_idx++)
    {
        ...
        dest_sel = jer2_arad_fc_rec_cal_dest_type_to_val_internal(cal_buff[per_rep_idx].destination);
        ...
        else if (cal_buff[per_rep_idx].destination == DNX_TMC_FC_REC_CAL_DEST_RETRANSMIT)
        {
            /* Set the ILKN Retransmit Req bits: 
             * Bit 0: ILKN-0, Bit 2: ILKN-1 */
            rx_cal.fc_dest_sel = dest_sel;
            rx_cal.fc_index = (if_ndx == DNX_TMC_FC_OOB_ID_A ? JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_A_VAL : JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_B_VAL);
        }
        else
        {
            rx_cal.fc_dest_sel = dest_sel;
        ...
        if(cal_buff[per_rep_idx].destination == DNX_TMC_FC_REC_CAL_DEST_NIF_LL)
        {
        ...
        rv = jer2_arad_fc_cal_tbl_get(unit, cal_mode_ndx, JER2_ARAD_FC_CAL_TYPE_RX, if_ndx, &cal_table);
        DNXC_IF_ERR_EXIT(rv);
        for (rep_idx = 0; rep_idx < cal_conf->cal_reps; rep_idx++)
        {
            entry_idx = (rep_idx * cal_conf->cal_len) + per_rep_idx;
    ...
    if ((cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_INBND) || (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_OOB)) {
        /* Select ILKN Type: InBand/OOB */
        fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? ILKN_RX_0_SELf : ILKN_RX_1_SELf);
        field32 = DNX_SAND_BOOL2NUM(cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_OOB);       
        rv = soc_reg_above_64_field32_modify(unit, reg, REG_PORT_ANY, 0, fld,  field32);
        DNXC_IF_ERR_EXIT(rv);
        /* Enable ILKN InBand/OOB */
        fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? ILKN_RX_0_ENf : ILKN_RX_1_ENf);
        field32 = cal_conf->enable;
        rv = soc_reg_above_64_field32_modify(unit, reg, REG_PORT_ANY, 0, fld,  field32);
		DNXC_IF_ERR_EXIT(rv);
    }
    ...
    SHR_BITSET(cfc_enables.bmp, DNX_TMC_FC_ILKN_RX_TO_RET_REQ_EN);
    ...
    SHR_BITSET(cfc_enables.bmp, DNX_TMC_FC_SPI_OOB_RX_TO_RET_REQ_EN);
  ...
  )
{
    ...
    rv = jer2_arad_fc_rec_cal_get_verify(unit, cal_mode_ndx, if_ndx);
    DNXC_IF_ERR_EXIT(rv);
    ...
    if (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_SPI_OOB){
        ...
        sel_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? ILKN_RX_0_SELf : ILKN_RX_1_SELf;
        rv = soc_reg_above_64_field32_read(unit, sel_reg, REG_PORT_ANY, 0, sel_fld, &sel_fld_val);
		DNXC_IF_ERR_EXIT(rv);
        ...
        if ((sel_fld_val && (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_INBND)) || 
            ...
            en_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? ILKN_RX_0_ENf : ILKN_RX_1_ENf;
            rv = soc_reg_above_64_field32_read(unit, en_reg, REG_PORT_ANY, 0, en_fld, &en_fld_val);
			DNXC_IF_ERR_EXIT(rv);
        }
    }
    ...
    else if (cal_mode_ndx == DNX_TMC_FC_CAL_MODE_ILKN_INBND)
    {
        ...
        cal_len_fld = (if_ndx == DNX_TMC_FC_OOB_ID_A ? FC_ILKN_RX_0_CHFC_CAL_LENf : FC_ILKN_RX_1_CHFC_CAL_LENf);
        rv = soc_reg_above_64_field32_read(unit, cal_len_reg, REG_PORT_ANY, 0, cal_len_fld, &cal_len_val);
        DNXC_IF_ERR_EXIT(rv);
        cal_conf->cal_len = cal_len_val;
    }
    ...
    for (entry_idx = 0; entry_idx < cal_conf->cal_len; entry_idx++)
    {
        ...
        if(rx_cal.fc_dest_sel >= JER2_ARAD_FC_REC_CAL_DEST_ARR_SIZE)
        {
            cal_buff[entry_idx].destination = DNX_TMC_FC_REC_CAL_DEST_NONE;
            cal_buff[entry_idx].id = 0;
        }
        else if(jer2_arad_fc_rec_cal_dest_arr[rx_cal.fc_dest_sel] == DNX_TMC_FC_REC_CAL_DEST_RETRANSMIT)
        {
            /* Set the ILKN Retransmit Req bits: 
             * Bit 0: ILKN-0, Bit 2: ILKN-1 */
            cal_buff[entry_idx].destination = jer2_arad_fc_rec_cal_dest_arr[rx_cal.fc_dest_sel];
            cal_buff[entry_idx].id = (rx_cal.fc_index == JER2_ARAD_FC_RETRANSMIT_DEST_ILKN_A_VAL ? JER2_ARAD_FC_ILKN_ID_A : JER2_ARAD_FC_ILKN_ID_B);
        }
        else
        {
            cal_buff[entry_idx].destination = jer2_arad_fc_rec_cal_dest_arr[rx_cal.fc_dest_sel];
  ...
  )
{
    ...
    DNXC_NULL_CHECK(info);
    rv = jer2_arad_fc_ilkn_llfc_set_verify(unit, ilkn_ndx, direction_ndx, info);
    DNXC_IF_ERR_EXIT(rv);
    ...
    if (JER2_ARAD_IS_DIRECTION_GEN(direction_ndx)) {
        rv = soc_reg_above_64_field32_read(unit, ilkn_llfc_reg, REG_PORT_ANY, 0, FC_ILKN_MODEf, &buffer);
        DNXC_IF_ERR_EXIT(rv);
        buffer &= DNX_SAND_RBIT(ilkn_bit_offset);
        buffer |= DNX_SAND_SET_FLD_IN_PLACE(
                          info->cal_channel != DNX_TMC_FC_ILKN_CAL_LLFC_NONE ? 1 : 0,
                          ilkn_bit_offset,
                          DNX_SAND_BIT(ilkn_bit_offset)
                    );
  ...
  )
{
    ...
    rv = jer2_arad_fc_ilkn_llfc_get_verify(unit, ilkn_ndx);
    DNXC_IF_ERR_EXIT(rv);
  ...
  )
{
  ...
  uint32
    reg_index = 0;
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_RETRANSMIT_SET_UNSAFE);

  DNX_SAND_CHECK_NULL_INPUT(ret_info);

  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, ILKN_RX_TO_RET_REQ_ENf,  ret_info->enable));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  11,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, SPI_OOB_RX_TO_RET_REQ_ENf,  ret_info->enable));

  if(ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A)
  {
    reg_index = 0;
  }
  if(ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_B)
  {
    reg_index = 1;
  }

  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_POLARITY_CFGr, REG_PORT_ANY, 0, rx_polarity_field[reg_index],  ret_info->rx_polarity));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_POLARITY_CFGr, REG_PORT_ANY, 0, tx_polarity_field[reg_index],  ret_info->tx_polarity));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_RX_CONFIGURATIONr, REG_PORT_ANY, 0, err_indication_field[reg_index],  ret_info->error_indication));


exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_retransmit_set_unsafe()", ilkn_ndx, 0);
}
  ...
  )
{
  ...
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_CFC_ENABLERSr, REG_PORT_ANY, 0, ILKN_RX_TO_RET_REQ_ENf, &ret_info->enable));
  ...
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_POLARITY_CFGr, REG_PORT_ANY, 0, rx_polarity_field[reg_index], &ret_info->rx_polarity));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_POLARITY_CFGr, REG_PORT_ANY, 0, tx_polarity_field[reg_index], &ret_info->tx_polarity));
  DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_RX_CONFIGURATIONr, REG_PORT_ANY, 0, err_indication_field[reg_index], &ret_info->error_indication));
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_MUB_REC_SET_UNSAFE);
  
  if(ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A)
  {
    DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_MUB_ENABLEr, REG_PORT_ANY, 0, ILKN_0_MUB_RX_ENAf,  bitmap));
  } else {
    DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_MUB_ENABLEr, REG_PORT_ANY, 0, ILKN_1_MUB_RX_ENAf,  bitmap));
  }

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_mub_rec_set_unsafe()", ilkn_ndx, 0);
}
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_MUB_REC_GET_UNSAFE);

  if(ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A)
  {
    DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_MUB_ENABLEr, REG_PORT_ANY, 0, ILKN_0_MUB_RX_ENAf, &fld_val));
  } else {
    DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_MUB_ENABLEr, REG_PORT_ANY, 0, ILKN_1_MUB_RX_ENAf, &fld_val));
  }
...
exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_mub_rec_get_unsafe()", ilkn_ndx, 0);
}


soc_error_t
  jer2_arad_fc_ilkn_mub_channel_verify(
    DNX_SAND_IN  int                            unit,
    DNX_SAND_IN  DNX_TMC_FC_CAL_IF_ID           ilkn_ndx
  )
{
  ...
  )
{
    ...
    res = jer2_arad_fc_ilkn_mub_channel_verify(unit, ilkn_ndx);
...
soc_error_t
  jer2_arad_fc_ilkn_mub_gen_cal_verify(
    DNX_SAND_IN  int                           unit,
    DNX_SAND_IN  DNX_TMC_FC_CAL_IF_ID             ilkn_ndx,
  ...
  )
{
    ...
    uint32
        fields_0[DNX_TMC_FC_ILKN_MUB_GEN_CAL_LEN] = 
    ...
    uint32
        fields_1[DNX_TMC_FC_ILKN_MUB_GEN_CAL_LEN] = 
    ...
    res = jer2_arad_fc_ilkn_mub_gen_cal_verify(unit, ilkn_ndx, cal_info);
    DNXC_IF_ERR_EXIT(res);
    ...
    for(entry_idx = 0; entry_idx < DNX_TMC_FC_ILKN_MUB_GEN_CAL_LEN; entry_idx++)
    {
        ...
        entry_data[0] = jer2_arad_fc_gen_cal_src_type_to_val_internal(unit, cal_info->entries[entry_idx].source) << JER2_ARAD_FC_ILKN_MUB_TX_CAL_SOURCE_ENTRY_OFFSET;
        /* set index (bits 8:0) */
        entry_data[0] |= (cal_info->entries[entry_idx].id & DNX_SAND_BITS_MASK(JER2_ARAD_FC_ILKN_MUB_TX_CAL_SOURCE_ENTRY_OFFSET-1, 0));
  ...
  )
{
    ...
    for(entry_idx = 0; entry_idx < DNX_TMC_FC_ILKN_MUB_GEN_CAL_LEN; entry_idx++)
    {
        ...
        cal_info->entries[entry_idx].source = jer2_arad_fc_gen_cal_src_arr[entry_data[0] >> JER2_ARAD_FC_ILKN_MUB_TX_CAL_SOURCE_ENTRY_OFFSET];

        /* get index (bits 8:0) */
        cal_info->entries[entry_idx].id = (entry_data[0] & DNX_SAND_BITS_MASK(JER2_ARAD_FC_ILKN_MUB_TX_CAL_SOURCE_ENTRY_OFFSET-1, 0));
    }
  ...
  )
{
    ...
    rv = jer2_arad_fc_pfc_generic_bitmap_verify(unit, priority, pfc_bitmap_index);
    DNXC_IF_ERR_EXIT(rv);
    ...
    for(reg_offset = 0; reg_offset < JER2_ARAD_FC_PFC_GENERIC_BITMAP_SIZE / 32; reg_offset++)
    {
...
soc_error_t
  jer2_arad_fc_cat_2_tc_hcfc_bitmap_verify(
    DNX_SAND_IN   int                           unit,
  ...
  )
{
    ...
    rv = jer2_arad_fc_cat_2_tc_hcfc_bitmap_verify(unit, core, tc, is_ocb_only, hcfc_bitmap);
    DNXC_IF_ERR_EXIT(rv);
...
soc_error_t
  jer2_arad_fc_glb_hcfc_bitmap_verify(
    DNX_SAND_IN   int                           unit,
    DNX_SAND_IN   int                           core,
  ...
  )
{
    ...
    res = jer2_arad_fc_glb_hcfc_bitmap_verify(unit, core, is_high_prio, is_ocb_only, pool_id, hcfc_bitmap);
    DNXC_IF_ERR_EXIT(res);
...
soc_error_t
  jer2_arad_fc_hcfc_watchdog_set_verify(
    DNX_SAND_IN   int                           unit,
    DNX_SAND_IN   DNX_TMC_FC_CAL_IF_ID          if_ndx,
  ...
  )
{
    ...
    DNXC_NULL_CHECK(hcfc_watchdog);

    if (hcfc_watchdog->period * SOC_INFO(unit).frequency > SOC_DNX_DEFS_GET(unit, max_oob_wd_period)){
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("watchdog period is out of range")));
    }
  ...
  )
{
    ...
    res = jer2_arad_fc_hcfc_watchdog_set_verify(unit, if_ndx, hcfc_watchdog);
    DNXC_IF_ERR_EXIT(res);
    ...
    fld_val = 0;
    fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? HCFC_OOB_RX_0_ERR_CFGf : HCFC_OOB_RX_1_ERR_CFGf;
    ...
    fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? HCFC_OOB_RX_0_WD_ENf : HCFC_OOB_RX_1_WD_ENf;
    ...
    fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? HCFC_OOB_RX_0_WD_ERR_STATUSf : HCFC_OOB_RX_1_WD_ERR_STATUSf;
    ...
    reg = (if_ndx == DNX_TMC_FC_OOB_ID_A)? CFC_HCFC_OOB_RX_0_WD_PERIOD_CFGr : CFC_HCFC_OOB_RX_1_WD_PERIOD_CFGr;
    
    res = soc_reg_above_64_get(unit, reg, REG_PORT_ANY, 0, reg64_data);
    DNXC_IF_ERR_EXIT(res);

    for (hcfc_channel = 0; hcfc_channel < DNX_TMC_FC_HCFC_CHANNEL_NUM; hcfc_channel++) {
        fld_val = hcfc_watchdog->period * SOC_INFO(unit).frequency;
        fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? period_0_fld[hcfc_channel] : period_1_fld[hcfc_channel];
...
soc_error_t
  jer2_arad_fc_hcfc_watchdog_get_verify(
    DNX_SAND_IN   int                           unit,
    DNX_SAND_IN   DNX_TMC_FC_CAL_IF_ID          if_ndx,
  ...
  )
{
    ...
    res = jer2_arad_fc_hcfc_watchdog_get_verify(unit, if_ndx, hcfc_watchdog);
    DNXC_IF_ERR_EXIT(res);
    ...
    fld = (if_ndx == DNX_TMC_FC_OOB_ID_A) ? period_0_fld[0] : period_1_fld[0];
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_SET_UNSAFE);
  ...
  if(SOC_IS_ARAD_B0_AND_ABOVE(unit))
  {
    if (JER2_ARAD_IS_DIRECTION_REC(direction_ndx))
    {
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_RX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_RX_0_RT_CAL_ENf : ILKN_OOB_RX_1_RT_CAL_ENf,  cal_cfg->enable));
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_RX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_RX_0_RT_CAL_LENf : ILKN_OOB_RX_1_RT_CAL_LENf,  cal_cfg->length - 1));
    }
    if (JER2_ARAD_IS_DIRECTION_GEN(direction_ndx))
    {
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_TX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_TX_0_RT_CAL_ENf : ILKN_OOB_TX_1_RT_CAL_ENf,  cal_cfg->enable));
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_modify(unit, CFC_ILKN_OOB_TX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_TX_0_RT_CAL_LENf : ILKN_OOB_TX_1_RT_CAL_LENf,  cal_cfg->length - 1));
    }
  }

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_retransmit_cal_set_unsafe()", ilkn_ndx, direction_ndx);
}
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_GET_UNSAFE);
  DNX_SAND_CHECK_NULL_INPUT(cal_cfg);
  ...
  if(SOC_IS_ARAD_B0_AND_ABOVE(unit))
  {
    if (JER2_ARAD_IS_DIRECTION_REC(direction_ndx))
    {
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  10,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_RX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_RX_0_RT_CAL_ENf : ILKN_OOB_RX_1_RT_CAL_ENf, &cal_cfg->enable));
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  20,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_RX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_RX_0_RT_CAL_LENf : ILKN_OOB_RX_1_RT_CAL_LENf, &fld_val));
      cal_cfg->length = fld_val + 1;
    }
    if (JER2_ARAD_IS_DIRECTION_GEN(direction_ndx))
    {
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  30,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_TX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_TX_0_RT_CAL_ENf : ILKN_OOB_TX_1_RT_CAL_ENf, &cal_cfg->enable));
      DNX_SAND_SOC_IF_ERROR_RETURN_ERR_VAL(res,  40,  exit, JER2_ARAD_REG_ACCESS_ERR,soc_reg_above_64_field32_read(unit, CFC_ILKN_OOB_TX_RT_CAL_CFGr, REG_PORT_ANY, 0, (ilkn_ndx == JER2_ARAD_NIF_ILKN_ID_A) ? ILKN_OOB_TX_0_RT_CAL_LENf : ILKN_OOB_TX_1_RT_CAL_LENf, &fld_val));
      cal_cfg->length = fld_val + 1;
    }
  }
  
exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_retransmit_cal_get_unsafe()", ilkn_ndx, direction_ndx);
}
  ...
  )
{
  ...
  DNX_SAND_INIT_ERROR_DEFINITIONS(JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_SET_VERIFY);
  DNX_SAND_CHECK_NULL_INPUT(cal_cfg);

  if(SOC_IS_ARAD_B0_AND_ABOVE(unit))
  {
    if(cal_cfg->length < JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_LEN_MIN || cal_cfg->length > JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_LEN_MAX)
    {
      DNX_SAND_SET_ERROR_CODE(JER2_ARAD_FC_ILKN_RETRANSMIT_CAL_LEN_RANGE_ERR, 10, exit);
    }
  }

exit:
  DNX_SAND_EXIT_AND_SEND_ERROR("error in jer2_arad_fc_ilkn_retransmit_cal_set_verify()", ilkn_ndx, direction_ndx);
}

int
  jer2_arad_fc_port_fifo_threshold_verify(
    DNX_SAND_IN  int                        unit,
  ...
  )
{
    ...
    if (if_type == SOC_PORT_IF_CAUI || if_type == SOC_PORT_IF_ILKN) {  
        /* LLFC */
        if (info->llfc.clear > JER2_ARAD_FC_RX_MLF_HRF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("llfc.clear is out of range")));
        }

        if (info->llfc.set > JER2_ARAD_FC_RX_MLF_HRF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("llfc.set is out of range")));
        }

        /* PFC */
        if (info->pfc.clear > JER2_ARAD_FC_RX_MLF_HRF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("pfc.clear is out of range")));
        }

        if (info->pfc.set > JER2_ARAD_FC_RX_MLF_HRF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("pfc.set is out of range")));
        }
    }
    else {
        /* LLFC */
        if (info->llfc.clear > JER2_ARAD_FC_RX_MLF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("llfc.clear is out of range")));
        }

        if (info->llfc.set > JER2_ARAD_FC_RX_MLF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("llfc.set is out of range")));
        }

        /* PFC */
        if (info->pfc.clear > JER2_ARAD_FC_RX_MLF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("pfc.clear is out of range")));
        }

        if (info->pfc.set > JER2_ARAD_FC_RX_MLF_MAX) {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("pfc.set is out of range")));
        }
    }  
  ...
  )
{
    ...
    /* Indexing */
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_first_phy_port_get(unit, port, &phy_port /*one based*/));
    DNXC_IF_ERR_EXIT(dnx_port_sw_db_interface_type_get(unit, port, &interface_type));

    nif_intern_id = phy_port - 1;
    wc_ndx = JER2_ARAD_NIF2WC_GLBL_ID(nif_intern_id);

    res = jer2_arad_fc_port_fifo_threshold_verify(unit, interface_type, info);
    DNXC_IF_ERR_EXIT(res);

    /* Setting val */
    if (interface_type == SOC_PORT_IF_CAUI || interface_type == SOC_PORT_IF_ILKN) {
        reg_offset = JER2_ARAD_NIF_CLP_NDX(wc_ndx);
...
soc_error_t
  jer2_arad_fc_pfc_mapping_verify(
      DNX_SAND_IN int  unit,
    ...
    )
{
    ...
    res = jer2_arad_fc_pfc_mapping_verify(unit, nif_id, src_pfc_ndx, pfc_map);
    DNXC_IF_ERR_EXIT(res);

    entry_offset = ((nif_id * DNX_TMC_EGR_NOF_Q_PRIO_JER2_ARAD) / FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD) + 
        src_pfc_ndx / FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD;
    field_offset = src_pfc_ndx % FC_NOF_INDICATION_IN_NIF_PFC_MAP_ENTRY_JER2_ARAD;
...
soc_error_t
  jer2_arad_fc_inbnd_mode_verify(
      DNX_SAND_IN int                           unit,
  ...
  )
{
    ...
    if (JER2_ARAD_NIF_IS_TYPE_ID(ILKN, nif_ndx))
    {
        DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("API is not applicable for ilkn")));
    }
      ...
      )
{
    ...
    /* Enable / Disable - LL */
    DNXC_IF_ERR_EXIT(soc_pm_pause_get(unit, port, &pause_tx, &pause_rx));
    if(is_generate) {
        pause_tx = (mode == DNX_TMC_FC_INBND_MODE_LL ? 1 : 0);
    } else {
        pause_rx = (mode == DNX_TMC_FC_INBND_MODE_LL ? 1 : 0);
    }
    DNXC_IF_ERR_EXIT(soc_pm_pause_set(unit, port, pause_tx, pause_rx));

    /* Enable / Disable - PFC */
    pfc_en = (mode == DNX_TMC_FC_INBND_MODE_PFC ? 1 : 0);
    DNXC_IF_ERR_EXIT(soc_pm_pfc_set(unit, port, is_generate ? 0 : 1, pfc_en));

    /* Enable / Disable - SAFC */
    llfc_en = (mode == DNX_TMC_FC_INBND_MODE_SAFC ? 1 : 0);
    DNXC_IF_ERR_EXIT(soc_pm_llfc_set(unit, port, is_generate ? 0 : 1, llfc_en));

    /* 2. setting port config - port level */
    fld_val = DNX_SAND_BOOL2NUM(mode == DNX_TMC_FC_INBND_MODE_LL);
  ...
  )
{
    ...
    case JER2_ARAD_ITM_VSQ_GROUP_STTSTCS_TAG:
        vsq_ndx_tmp += JER2_ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS_SIZE;
    case JER2_ARAD_ITM_VSQ_GROUP_CTGRY_2_3_CNCTN_CLS:
        vsq_ndx_tmp += JER2_ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS_SIZE;
        /* No missing break. It is on purpose*/
        /* coverity[unterminated_case] */
    case JER2_ARAD_ITM_VSQ_GROUP_CTGRY_TRAFFIC_CLS:
        vsq_ndx_tmp += JER2_ARAD_ITM_VSQ_GROUP_CTGRY_SIZE;
  ...
  )
{
    ...
    DNXC_IF_ERR_EXIT(soc_reg64_get(unit, CFC_CMIC_RX_FC_CFGr, REG_PORT_ANY, 0, &reg_val64));
    ...
    *priority_bmp_lcl = priority_bmp;
    ...
    DNXC_IF_ERR_EXIT(soc_reg64_set(unit, CFC_CMIC_RX_FC_CFGr, REG_PORT_ANY, 0, reg_val64));
...
static soc_error_t
  _jer2_arad_fc_parse_tx_cal_tbl(
    int                      unit,
    uint32                   intf_type,
    uint32                   is_oob,
  ...
  )
{
    ...
    DNXC_IF_ERR_EXIT(jer2_arad_fc_cal_tbl_get(unit, cal_mode, cal_type, port, &cal_table));
    ...
    fc_src = jer2_arad_fc_gen_cal_src_arr[tx_cal.fc_source_sel];
...
static soc_error_t
  _jer2_arad_fc_parse_rx_cal_tbl(
    int                      unit,
    uint32                   intf_type,
    uint32                   is_oob,
    DNX_TMC_FC_DIRECTION     direction,
...
static soc_error_t
  _jer2_arad_fc_trigger_status_info_get(
    DNX_SAND_IN int                      unit,
  ...
  )
{
    ...
    case DNX_TMC_FC_SRC_TYPE_VSQ:  /* VSQ A/B/C/D/E/F */
        ...
        if (vsq_type == DNX_TMC_ITM_VSQ_GROUP_LLFC) {
...
static soc_error_t
  _jer2_arad_fc_llfc_stop_tx_status_info_get(
    DNX_SAND_IN int                      unit,
...
static soc_error_t
  _jer2_arad_fc_queue_pair_to_egr_if_get(
    DNX_SAND_IN int                      unit,
  ...
  )
{
    ...
    SOC_PBMP_ITER(pbmp, port_i) {
        ...
        if ((queue_pair >= base_q_pair_i) && (queue_pair < (base_q_pair_i + nof_priorities))) {
            ...
            DNXC_SAND_IF_ERR_EXIT(jer2_arad_port2egress_offset(unit, core, tm_port, egr_if));
...
static soc_error_t
  _jer2_arad_fc_egq_pfc_status_info_get(
    DNX_SAND_IN int                      unit,
...
static soc_error_t
  _jer2_arad_fc_egq_inf_status_info_get(
    DNX_SAND_IN int                      unit,
  ...
  )
{
    ...
    *fld_val32 = soc_reg_field_get(unit, reg, reg_val32, EGQ_IF_FC_STATUSf);
    ...
    /* 2. Status in SCH */
    DNXC_IF_ERR_EXIT(READ_SCH_FC_MAP_FCMm(unit, SCH_BLOCK(unit, 0), egr_if, &entry_val32));
...
static soc_error_t
  _jer2_arad_fc_egq_dev_status_info_get(
    DNX_SAND_IN int                      unit,
    DNX_SAND_OUT DNX_TMC_FC_STATUS_INFO  *fc_status_info
  )
{
...
static soc_error_t
  _jer2_arad_fc_nif_status_info_get(
    DNX_SAND_IN int                      unit,
    DNX_SAND_IN DNX_TMC_FC_STATUS_KEY   *fc_status_key,
    DNX_SAND_OUT DNX_TMC_FC_STATUS_INFO *fc_status_info
  )
{
    ...
    if (fc_status_key->direction == DNX_TMC_FC_DIRECTION_GEN) {
        /* IQM & CFC block */
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_trigger_status_info_get(unit, fc_status_key, fc_status_info));
        ...
        case DNX_TMC_FC_NIF_TYPE_SAFC:
            ...
            *fld_val32 = (first_phy*8)/64;
            ...
            /* CFC & EGQ & SCH block */
            for (i = 0; i < 8; i++) {
                DNXC_IF_ERR_EXIT(jer2_arad_fc_pfc_mapping_get(unit, first_phy, i, &pfc_map));
            ...
            if (is_generic_map_used) {
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_pfc_status_info_get(unit, TRUE, 0, 0, fc_status_info));
            }
            else {
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_queue_pair_to_egr_if_get(unit, base_q_pair, &egr_if));

                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_pfc_status_info_get(unit, FALSE, base_q_pair, nof_priorities, fc_status_info));
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_inf_status_info_get(unit, egr_if, fc_status_info));
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_dev_status_info_get(unit, fc_status_info));
            }
            break;
        default:
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Not support nif_fc_type(%d)\n"), fc_status_key->nif_fc_type));
...
static soc_error_t
  _jer2_arad_fc_ilkn_inb_status_info_get(
    DNX_SAND_IN int                      unit,
    DNX_SAND_IN DNX_TMC_FC_STATUS_KEY   *fc_status_key,
    DNX_SAND_OUT DNX_TMC_FC_STATUS_INFO *fc_status_info
  )
{
    ...
    if (fc_status_key->direction == DNX_TMC_FC_DIRECTION_GEN) {
        ...
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_parse_tx_cal_tbl(unit, intf_type, FALSE, direction, port, calendar_id,
    ...
    else
    {
        ...
        {
            /* Parse tx calendar entry */
            DNXC_IF_ERR_EXIT(_jer2_arad_fc_parse_rx_cal_tbl(unit, intf_type, FALSE, direction, port, calendar_id,
            ...
            *reg_val32_hi = COMPILER_64_HI(reg_val64);
            ...
            case DNX_TMC_FC_REC_CAL_DEST_NIF_LL:
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_llfc_stop_tx_status_info_get(unit, fc_index, fc_status_info));
                break;
            ...
            case DNX_TMC_FC_REC_CAL_DEST_PORT_8_PRIORITY:
                ...
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_queue_pair_to_egr_if_get(unit, fc_index, &egr_if));

                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_pfc_status_info_get(unit, FALSE, fc_index, nof_priorities, fc_status_info));
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_inf_status_info_get(unit, egr_if, fc_status_info));
                DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_dev_status_info_get(unit, fc_status_info));
                break;
...
static soc_error_t
  _jer2_arad_fc_mub_inb_status_info_get(
    DNX_SAND_IN int                      unit,
    DNX_SAND_IN DNX_TMC_FC_STATUS_KEY   *fc_status_key,
    DNX_SAND_OUT DNX_TMC_FC_STATUS_INFO *fc_status_info
  )
{
    ...
    if (fc_status_key->direction == DNX_TMC_FC_DIRECTION_GEN) {
        /* Parse tx calendar entry */
        DNXC_IF_ERR_EXIT(jer2_arad_fc_ilkn_mub_gen_cal_get(unit, port, &cal_info));
        ...
        *fld_val32 = soc_reg_field_get(unit, reg, reg_val32, fld);
        ...
        else if (fc_status_key->calendar_fc_type == DNX_TMC_FC_CALENDAR_TYPE_CHFC) /* CHFC */
        {
            ...
            *fld_val32 = soc_reg_field_get(unit, reg, reg_val32, fld);
            ...
            /* 2.4 EGQ block */
            DNXC_IF_ERR_EXIT(_jer2_arad_fc_egq_pfc_status_info_get(unit, TRUE, fc_index, 0, fc_status_info));
        }
        else {
            DNXC_EXIT_WITH_ERR(SOC_E_PARAM, (_BSL_DNXC_MSG("Not support calendar_fc_type(%d)\n"), fc_status_key->calendar_fc_type));
        }
...
static soc_error_t
  _jer2_arad_fc_oob_status_info_get(
    DNX_SAND_IN int                      unit,
    DNX_SAND_IN DNX_TMC_FC_STATUS_KEY   *fc_status_key,
    DNX_SAND_OUT DNX_TMC_FC_STATUS_INFO *fc_status_info
  )
{
    ...
    if (fc_status_key->direction == DNX_TMC_FC_DIRECTION_GEN) {
        /* Parse tx calendar entry */
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_parse_tx_cal_tbl(unit, intf_type, TRUE, direction, port, calendar_id,
            &fc_status_info->cal_tx_src_type,
            &fc_status_info->cal_tx_src_vsq_type,
            &fc_status_info->cal_tx_src_id));
        ...
        /* Parse tx calendar entry */
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_parse_rx_cal_tbl(unit, intf_type, TRUE, direction, port, calendar_id,
            &fc_status_info->cal_rx_dst_type,
            &fc_status_info->cal_rx_dst_id));
        fc_index = fc_status_info->cal_rx_dst_id;
  ...
  )
{
    ...
    case DNX_TMC_FC_INTF_TYPE_NIF:
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_nif_status_info_get(unit, fc_status_key, fc_status_info));
        break;
    case DNX_TMC_FC_INTF_TYPE_ILKN:
        if (fc_status_key->is_oob) {
            DNXC_IF_ERR_EXIT(_jer2_arad_fc_oob_status_info_get(unit, fc_status_key, fc_status_info));
        }
        else {
            DNXC_IF_ERR_EXIT(_jer2_arad_fc_ilkn_inb_status_info_get(unit, fc_status_key, fc_status_info));
        }
        break;
    case DNX_TMC_FC_INTF_TYPE_MUB:
        DNXC_IF_ERR_EXIT(_jer2_arad_fc_mub_inb_status_info_get(unit, fc_status_key, fc_status_info));
        break;
