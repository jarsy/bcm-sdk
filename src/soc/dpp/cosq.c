  ...
  )
{
  ...
  if(SOC_IS_QAX(unit)){
    flow_and_up_info->sch_consumer.sub_flow[0].id = BCM_COSQ_FLOW_ID_QAX_SUB_OFFSET(unit, flow_and_up_info->sch_consumer.sub_flow[0].id);
    flow_and_up_info->sch_consumer.sub_flow[0].credit_source.id -= QAX_SE_ID_OFFSET;
    if(flow_and_up_info->sch_consumer.sub_flow[1].is_valid){
        flow_and_up_info->sch_consumer.sub_flow[1].id -= QAX_FLOW_ID_OFFSET(unit);
        flow_and_up_info->sch_consumer.sub_flow[1].credit_source.id -= QAX_SE_ID_OFFSET;
    }
  }
