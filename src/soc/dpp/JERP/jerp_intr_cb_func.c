/*
 * $Id: jerp_appl_intr_cb_func.c, v1 Broadcom SDK $
...
int jerp_intr_handle_generic_none(int unit, int block_instance, jerp_interrupt_type en_jerp_interrupt,char *msg)
{
    ...
    LOG_ERROR(BSL_LS_SOC_INTR, (BSL_META_U(unit,"%d %s\n"), en_jerp_interrupt, SOC_CONTROL(unit)->interrupts_info->interrupt_db_info[en_jerp_interrupt].name));
...
void jerp_interrupt_cb_init(int unit)
{
    ...
    dcmn_intr_add_handler_ext(unit, JERP_INT_IPT_CRC_ERR_PKT, 100,1,jerp_special_handling_normal_0,jerp_hard_reset,"the sw process should clear the FIFO and consider each buffer in it whether it may be returned to the free buffers pool.");
    dcmn_intr_add_handler_ext(unit, JERP_INT_IPT_CRC_DELETED_BUFFERS_FIFO_FULL, 0,0,jerp_special_handling_normal_0,NULL,"the sw process should clear the FIFO and consider each buffer in it whether it may be returned to the free buffers pool.");
    dcmn_intr_add_handler_ext(unit, JERP_INT_IPT_CRC_DELETED_BUFFERS_FIFO_NOT_EMPTY, 0,0,jerp_special_handling_normal_0,NULL,"the sw process should clear the FIFO and consider each buffer in it whether it may be returned to the free buffers pool.");
