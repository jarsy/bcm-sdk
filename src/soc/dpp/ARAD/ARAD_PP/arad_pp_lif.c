...
/* { */
/*********************************************************************
*     Helper utilities for Jericho/Arad  lif table acces
*     
*********************************************************************/
uint32 
    ...
    SOC_SAND_IN  uint32                 sem_result) 
{
    ...
    if (SOC_IS_JERICHO(unit)) {
         /*get the local inlif info into SW database*/
...
/* Indicate whether an allocated lif is a wide entry (fetch information from SW DB).
   For the Egress side, return the extension type (data/protection) if required. */
soc_error_t
    ...
    *is_wide_entry = FALSE;
    ...
    if (is_ingress) {
        ...
        *is_wide_entry = outlif_info.local_lif_flags & BCM_DPP_AM_OUT_LIF_FLAG_WIDE ;
        ...
        /* If it's an extended entry and the extension type was requested */
 ...
 * Purpose:
 *      Convert the protection fields from the SOC layer values to specific HW
 *      format. This conversion function is relevant for all types of LIFs that
 *      incorporate the protection fields.
 * Parameters:
 *      unit     - Device Number
 *      protection_pointer - The SOC layer protection-pointer value.
 *      protection_pass_value - The SOC layer protection-pass value.
 *      hw_protection_pointer - Return value for the HW device specific protection-pointer
 *      hw_protection_path - Return value for the HW device specific protection-path
 * Returns:
        ...
        SOC_SAND_OUT uint32 *hw_protection_path)
{
    ...
    *hw_protection_pointer &= ARAD_PP_LIF_PROTECTION_POINTER_MASK_ARAD;
    ...
    if (SOC_IS_JERICHO(unit)) {
        /* In Jericho Coupled mode, the protection-pointer has to be limited as
           it doesn't use the full range of protection-paths. In De-Coupled mode,
           the protecion-path bit holds the MSB of the Protection-Pointer. */
 ...
 * Purpose:
 *      Convert the protection fields from specific HW format to SOC layer values.
 *      This conversion function is relevant for all types of LIFs that
 *      incorporate the protection fields.
 * Parameters:
 *      unit     - Device Number
 *      hw_protection_pointer - The HW device specific protection-pointer
 *      hw_protection_path - The HW device specific protection-path
 *      protection_pointer - Return value for the SOC layer protection-pointer value.
 *      protection_pass_value - Return value for the SOC layer protection-pass value.
 * Returns:
 *      BCM_E_INTERNAL - Unsupported failover type
 */
void
        ...
        SOC_SAND_OUT uint8  *protection_pass_value)
{
    ...
    *protection_pass_value = hw_protection_path;
    ...
    if (SOC_IS_JERICHO(unit)) {
        /* In Jericho De-Coupled mode, the protecion-path bit holds the MSB of
           the Protection-Pointer. */
        if (!(SOC_DPP_IS_PROTECTION_INGRESS_COUPLED(unit))) {
  ...
  )
{
  ...
  if(pwe_additional_info->vlan_domain_valid) 
  {
     ...
     isem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_VLAN_DOMAIN_MPLS_L1;
  ...
  { 
    ...
    if (SOC_DPP_CONFIG(unit)->pp.vtt_pwe_vid_search) {
        isem_key->key_info.mpls.label2 = pwe_additional_info->outer_vid;              /* 12 bit field*/
        isem_key->key_info.mpls.label2 |= pwe_additional_info->outer_vid_valid << 12;  /* 1 bit field*/
        isem_key->key_info.mpls.label2 |= pwe_additional_info->inner_vid << 13;         /* 12 bit field*/
        isem_key->key_info.mpls.label2 |= pwe_additional_info->inner_vid_valid << 25;   /* 1 bit field*/
    }
  } else 
  {
  ...
  )
{
  ...
  /* The default HW of protection pointer is -1 */
  ...
  */
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    if (pwe_info->service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_P2P)
    {
      ...
      {
        /* ARAD:     Entry has OutLIF: EEI-or-Out-LIF[16] = Out-LIF-Valid, EEI-or-Out-LIF[15:0] = Out-LIF */
        /* JERICHO:  Entry has OutLIF: EEI-or-Out-LIF[18] = Out-LIF-Valid, EEI-or-Out-LIF[17:0] = Out-LIF */
        data.eei_or_out_lif = temp; /* Out-LIF */        
        res = soc_sand_bitstream_set_bit(&data.eei_or_out_lif, SOC_DPP_DEFS_GET(unit, inlif_p2p_out_lif_valid_bit)); /* Valid */
        SOC_SAND_CHECK_FUNC_RESULT(res, 39, exit);     
      }
    ...
    if (pwe_info->service_type == SOC_PPC_L2_LIF_PWE_SERVICE_TYPE_MP)
    {
      ...
      /* In Jericho, termination profile is not split into two fields anymore. */
      if (SOC_IS_JERICHO(unit)) {
  ...
  )
{
  ...
  if (found)
  {
     ...
     * set entry type
     */
    if (!is_gal) { /* In case of GAL, the lif table entry will be removed with the PWE SEM entry (if it's removed) */
        res = arad_pp_sw_db_lif_table_entry_use_set(
                unit,
                *lif_index,
                                          ...
                                          SOC_SAND_OUT SOC_PPC_FP_QUAL_VAL qual_vals[])
{
    ...
    { /*sem_key->key_type = ARAD_PP_ISEM_ACCESS_KEY_TYPE_CUSTOM_PP_PORT_TUNNEL */
  ...
  )
{
  ...
  SOC_DPP_DBAL_SW_TABLE_IDS  dbal_tables_id[3]={0};/*ARAD_PP_ISEM_ACCESS_NOF_TABLES*/
  uint32  nof_tables;
  int i;
  ...
  if (*success == SOC_SAND_SUCCESS || *success == SOC_SAND_FAILURE_OUT_OF_RESOURCES_2)
  {
    if (ac_info->service_type == SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2AC)
    {
      ...
      if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
        /* Remove this lif from the lif group of the rif (or VSI) in case it was. */
    ...
    if (ac_info->service_type == SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PWE)
    {
      ...
      {
          if (SOC_IS_JERICHO(unit)) {
              /* In Jericho, the eei_or_out_lif field contains both the lif/label and the identifier. The identifier field is only 1 or 0 */
          ...
          if (ac_info->default_frwrd.default_forwarding.additional_info.outlif.type != SOC_PPC_OUTLIF_ENCODE_TYPE_NONE){
              ...
              /* Stamping outlif-valid bit according to device */
              res = soc_sand_bitstream_set_bit(&temp, SOC_DPP_DEFS_GET(unit, inlif_p2p_out_lif_valid_bit)); /* Valid */     
          } else {
              /* Entry points to something that's neither a lif nor an eei. Don't turn on the outlif-valid bit. */
          }
          data.eei_or_out_lif = temp; /* Out-LIF */
    ...
    if (ac_info->service_type == SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PBB)
    {
      ...
      if (SOC_IS_JERICHO(unit)) {
           ...
           data.eei_or_out_lif_type = 1; /* set EEI value */
      }
    ...
    if (ac_info->service_type == SOC_PPC_L2_LIF_AC_SERVICE_TYPE_MP)
    {
      ...
      /* 
       * if type = SOC_PPC_L2_LIF_AC_LEARN_DISABLE: then learning is disabled
       * if type = SOC_PPC_L2_LIF_AC_LEARN_SYS_PORT: then valid = 0
      ...
      */
      ...
      if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
        ...
        if (data.vsi < SOC_DPP_DEFS_GET(unit, nof_vsi_lowers)) { /* low VSI with its own RIF */
          rif_id = data.vsi;
  ...
  )
{
  ...
  uint32
      sub_type, flags = 0;
  SOC_DPP_DBAL_SW_TABLE_IDS  dbal_tables_id[3];/*ARAD_PP_ISEM_ACCESS_NOF_TABLES*/
  uint32  nof_tables;
  uint8 hit_bit;
    ...
    else
    {
        ...
        else /* new implementation using DBAL*/
        {
           ...
           for (i = 0; i < nof_tables; i++) {
               ...
               res = arad_pp_dbal_entry_get(unit, dbal_tables_id[i], qual_vals, &sem_entry, 0, &hit_bit, found);
               SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
           }
        }
    }
        ...
        case SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2PWE:
        {
          ...
          if (data.type == 0x1) /* ac_p2p_to_pwe */
          {
            ...
            if (data.eei_or_out_lif_identifier) 
            {
                flags |= ARAD_PP_FWD_DECISION_PARSE_EEI;
                if (SOC_IS_ARADPLUS_AND_BELOW(unit)) {
                  /* In Jericho, the identifier is already in the field.
                     Entry has EEI: EEI-or-Out-LIF-Identifier 4b (mpls label command), Out-LIF-or-Label 20b (label) */
  ...
  )
{
  ...
  if (!ignore_key) {
          ...
          else /* new implementation using DBAL*/
          {
              ...
              for (i = 0; i < nof_tables; i++) {
                  if (i > 0 && found_in_isem)
                  {
                   ...
                   res = arad_pp_dbal_entry_get(unit, dbal_tables_id[i], qual_vals, &sem_entry, 0, &hit_bit, &found_in_isem);
                   SOC_SAND_CHECK_FUNC_RESULT(res, 14, exit);
             }
          }
      ...
      if ((lif_index != NULL) && (found_in_isem || found_vd)) {
          *lif_index = arad_pp_sem_result_to_lif_index(unit, tmp_lif_index);
      }
  }
  ...
  if (ignore_key || found_in_isem || found_vd) {
      ...
      if (found_in_isem)
      {
          if(nof_tables == 0){/* old implementation */
              ...
              for (i = 0; i < nof_tables; i++) {
                  /* success always is SOC_SAND_SUCCESS for arad_pp_dbal_entry_delete()
                   * Keep same logic for old implementation (none-dbal)
                   */
      ...
      if (lif_index != NULL) {
          ...
          if (SOC_IS_ARADPLUS_A0(unit) && soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
              ...
              /* If type is SOC_PPC_L2_LIF_AC_SERVICE_TYPE_MP then remove the lif from the its rif group. */
  ...
  )
{
  ...
  if (isid_info->service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_P2P)
  {
      ...
      else 
      {
          ...
          /* ARAD:  Entry has OutLIF: EEI-or-Out-LIF[16] = Out-LIF-Valid, EEI-or-Out-LIF[15:0] = Out-LIF */
          /* JERICHO:  Entry has OutLIF: EEI-or-Out-LIF[18] = Out-LIF-Valid, EEI-or-Out-LIF[17:0] = Out-LIF */
          res = soc_sand_bitstream_set_bit(eei_or_out_lif, SOC_DPP_DEFS_GET(unit, inlif_p2p_out_lif_valid_bit)); /* Out-LIF-Valid */
    ...
    if (isid_info->service_type == SOC_PPC_L2_LIF_ISID_SERVICE_TYPE_MP)
    {
      ...
      if (!ignore_key && !(SOC_DPP_CONFIG(unit)->pp.mim_vsi_mode))
      {
        ...
        /*In Jericho, if LIF is wide (has additional data) set the extension entry*/
        if (SOC_IS_JERICHO(unit) && isid_info->is_extended)
        {
            uint64 additional_data;
            COMPILER_64_SET(additional_data,0,0);
  ...
  if (*found)
  {
    *lif_index = arad_pp_sem_result_to_lif_index(unit, sem_entry.sem_result_ndx);
    ...
    do
    {
      ...
      if (((data.type == 0x1) && SOC_IS_ARADPLUS_AND_BELOW(unit)) || ((data.type == 0x3) && SOC_IS_JERICHO(unit)) ) /* isid_mp */
      {
    ...
    SOC_SAND_OUT void** in_lif_data)
{
    ...
    if (extender_info->service_type == SOC_PPC_L2_LIF_AC_SERVICE_TYPE_AC2AC)
    {
        ...
        *in_lif_data = in_lif_ac2ac_data;
        ...
        in_lif_ac2ac_data->type = 0x0;   /* ac_p2p_to_ac */
        ...
        in_lif_ac2ac_data->out_lif_valid = 1;    /* out-lif */
        ...
        *in_lif_data = in_lif_ac_mp_data;
        ...
        in_lif_ac_mp_data->type = 0x3;    /* ac_mp */
        ...
        {
            /* fill the 16b learn info field according to the learn info (reverse forwarding decision info)
               in_lif_ac_mp_data.learn_info = extender_info->learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_FEC ? (1<<15) ^ extender_info->learn_record.learn_info.dest_id : 
               extender_info->learn_record.learn_info.type == SOC_PPC_FRWRD_DECISION_TYPE_UC_FLOW ? 
               extender_info->learn_record.learn_info.dest_id >> 2 : 0; */
            ...
            SOC_SAND_CHECK_FUNC_RESULT(res, 20, exit);
        } else { /* learning is disabled or set by FLP write zero to learn-info */
    ...
    SOC_SAND_OUT SOC_PPC_FP_QUAL_VAL            qual_vals[])
{
    ...
    /* Reset the return qualifiers array */
    for (qual_index = 0; qual_index < SOC_PPC_FP_NOF_QUALS_PER_PFG_MAX; ++qual_index) {
        SOC_PPC_FP_QUAL_VAL_clear(&qual_vals[qual_index]);
    }
                           ...
                           SOC_PPC_FP_QUAL_VAL                      qual_vals[])
{
    ...
    /* Get the DBAL tables IDs according to the ISEM Key */
    ...
    /* Get the Extender key qualifiers */
  ...
  )
{
    ...
    if (extender_key) {
        /* Get the lif from the extender key. */
  ...
  )
{
    ...
    /* If the ISEM entry is valid, add an ISEM entry */
    if (!ignore_key) {
        /* Format the ISEM info that is only In-LIF pointer data */
        ARAD_PP_CLEAR(&sem_entry, ARAD_PP_ISEM_ACCESS_ENTRY, 1);
...
/*********************************************************************
 *     Enable an Port Extender and set the PE attributes.
  ...
  )
{
    ...
    /*
     * Step 2: Remove lif entry.
     */
    /* Mark the LIF entry as EMPTY in the SW. The HW remains untouched. */
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
    if (is_ingress) {
        ...
        /*the half entry which need to be set. if TRUE - higher Half*/
        half_offset = ARAD_PP_LIF_LOCAL_INLIF_HANDLE_TO_ADDITIONAL_OFFSET(lif_id, inlif_info.ext_lif_id);


        /*Get the current entire data from the additional data offset*/
        ...
        /*Set the entire data from the additional data offset*/
        res = arad_pp_ihp_lif_table_tbl_set_unsafe(  
          unit,
          inlif_info.ext_lif_id,
          &tbl_data
);
    ...
    else
    {
        ...
        /*get the local outlif info from SW database*/
        res = SOC_PPC_OUTLIF_SW_STATE_INFO_GET(unit, lif_id, &outlif_info);
        SOCDNX_IF_ERR_EXIT(res);

        /*the 1/4 entry which need to be set, based on the lif id.*/
   ...
   )
{
  ...
  if (SOC_IS_JERICHO(unit))
  {
      if (is_ingress) {
          ...
          /*Get the correct half entry*/
          start_bit_off = (half_offset) ? 57 : 0; 
          SHR_BITCOPY_RANGE(data_get, 0, tbl_data.lif_table_entry,start_bit_off, 57);
...
/*********************************************************************
 *   Get the In-LIF Protection-Pointer according to the lif type
 *********************************************************************/
#if 0
    ...
    SOC_SAND_IN  uint32                     *protection_pointer)
{
    ...
    /* Get the In-LIF format from the SW DB */
    res = arad_pp_sw_db_lif_table_entry_use_get(
          unit, in_lif_ndx, &lif_type, &lif_sub_type);
    SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);

    /* Retrieve the Protection-Pointer according to the In-LIF entry format */
    switch (lif_type) {
