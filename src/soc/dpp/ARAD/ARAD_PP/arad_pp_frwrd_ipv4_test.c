  ...
  SOC_SAND_CHECK_FUNC_RESULT(res, 10, exit);
    ...
    for(indx = 0; indx < nof_routes && rule_count < 10000 ; indx++) {
      ...
      else if (in_cache &&  ((last_cached_index + till_commit == indx)))
      {
        ...
        /*soc_ppd_frwrd_ip_routes_cache_mode_enable_set(0,0,0);*/
        soc_ppd_frwrd_ip_routes_cache_commit(unit,/*vrf*/SOC_PPC_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM,SOC_PPC_FRWRD_IP_ALL_VRFS_ID,&fail_inidcation);
      ...
      if (in_cache &&  ((indx == nof_routes-1 &&till_cache==0)))
      {
        ...
        /*soc_ppd_frwrd_ip_routes_cache_mode_enable_set(0,0,0);*/
        soc_ppd_frwrd_ip_routes_cache_commit(unit,/*vrf*/SOC_PPC_FRWRD_IP_CACHE_MODE_IPV4_UC_LPM,0,&fail_inidcation);
        soc_ppd_frwrd_ip_routes_cache_mode_enable_set(unit,SOC_PPC_FRWRD_IP_ALL_VRFS_ID,SOC_PPC_FRWRD_IP_CACHE_MODE_NONE);
        in_cache = FALSE;
      }
