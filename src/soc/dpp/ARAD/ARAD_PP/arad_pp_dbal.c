                           ...
                           SOC_PPC_FP_QUAL_VAL *qual_vals_array, void* payload,  uint32 *nof_entries)
{
    ...
    do {
        ...
        if (table_id == entry_tbl_id)
        {
            SOCDNX_IF_ERR_EXIT(arad_pp_dbal_entry_get_id_tcam(unit, table_id, data_indx, qual_vals_array + (entry_index - block_range_key->start.payload.arr[0])*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX, &tcam_payload,  &priority, &hit_bit, &found));
            if (found) {                
                if (is_vrf_mode) {
                ...
                if ((is_vrf_mode == 0) || (qual_vals_array[(entry_index - block_range_key->start.payload.arr[0])*SOC_PPC_FP_NOF_QUALS_PER_DB_MAX + vrf_qual_index].val.arr[0] == vrf_ndx)){
                    (*((uint32 *)payload + (entry_index - block_range_key->start.payload.arr[0]))) = tcam_payload.value[0];
                    (*nof_entries)++;                    
                }              
            }/*found*/
        }
        ...
        SOCDNX_SAND_IF_ERR_EXIT(soc_sand_hash_table_get_next(unit, hash_tbl, &iter, stream, &data_indx));
        /* increment entry_index in case the last entry was in the required table */
        if (entry_tbl_id != -1) {
        	entry_index++;
        }
    ...
    arad_pp_dbal_table_info_dump(int unit, SOC_DPP_DBAL_SW_TABLE_IDS table_id)
{
    ...
    if (table.db_prefix == DBAL_PREFIX_NOT_DEFINED) {
        ...
        case SOC_DPP_DBAL_PHYSICAL_DB_TYPE_LEM:
            arad_pp_lem_access_app_to_prefix_get(unit, table.db_prefix, &lem_prefix);
            LOG_CLI((BSL_META("  App type: %s, logical prefix value: 0x%x, prefix length: %d\n"), ARAD_PP_LEM_ACCESS_KEY_TYPE_to_string(table.additional_table_info), lem_prefix, (table.db_prefix_len)));
            break;
