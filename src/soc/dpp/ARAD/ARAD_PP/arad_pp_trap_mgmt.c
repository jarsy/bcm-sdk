  ...
  )
{
    ...
    for (i = 0; i < oamp_port_count; i++) {
        ...
        if (packet_info->src_sys_port == oamp_port[i]) {
  ...
  )
{
  ...
  uint32            user_header_0_size, user_header_1_size, user_header_egress_pmf_offset_0_dummy, 
                    user_header_egress_pmf_offset_1_dummy, user_header_size;
  ...
  )
{
  ...
  if (profile_info->bitmap_mask & SOC_PPC_TRAP_ACTION_PROFILE_USE_ARR) 
  { 
