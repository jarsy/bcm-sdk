/***********************************************************************************
 ***********************************************************************************
 *  File Name     :  merlin_koi_internal.c                                         *
 *  Created On    :  13/02/2014                                                    *
 *  Created By    :  Justin Gaither                                                *
 *  Description   :  APIs for Serdes IPs                                           *
 *  Revision      :  $Id: merlin_koi_internal.c 1062 2015-06-02 22:12:41Z kirand $ *
 *                                                                                 *
...
static uint8_t _merlin_koi_rdb_uc_var(err_code_t *err_code_p, uint16_t addr) {
    ...
    EPSTM(rddata = merlin_koi_rdb_uc_ram(err_code_p, addr)); /* Use Micro register interface for reading RAM */
...
#endif
  EPSTM(rddata = merlin_koi_rdw_uc_ram(err_code_p, (addr))); /* Use Micro register interface for reading RAM */
...
static err_code_t _merlin_koi_wrb_uc_var(uint16_t addr, uint8_t wr_val) {
    ...
    return (merlin_koi_wrb_uc_ram(addr, wr_val));                                /* Use Micro register interface for writing RAM */
}
...
static err_code_t _merlin_koi_wrw_uc_var(uint16_t addr, uint16_t wr_val) {
    ...
    return (merlin_koi_wrw_uc_ram(addr, wr_val));                                 /* Use Micro register interface for writing RAM */
}
...
SDK_STATIC err_code_t _merlin_koi_get_osr_mode(merlin_koi_osr_mode_st *imode) {
    merlin_koi_osr_mode_st mode;

     ENULL_MEMSET(&mode, 0, sizeof(merlin_koi_osr_mode_st));
...
SDK_STATIC err_code_t _merlin_koi_read_lane_state(merlin_koi_lane_state_st *istate) {
  ...
  merlin_koi_lane_state_st state;
  uint8_t ladder_range = 0;
  ENULL_MEMSET(&state, 0, sizeof(merlin_koi_lane_state_st));

  if(!istate)
          return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  
  EFUN(_merlin_koi_pmd_lock_status(&state.rx_lock, &state.rx_lock_chg));
  ...
  EFUN(_merlin_koi_sigdet_status(&state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(merlin_koi_get_clk90_offset(&state.clk90));
...
static err_code_t _merlin_koi_display_lane_state_no_newline(void) {
  ...
  EFUN(_merlin_koi_read_lane_state(&state));
  
  lane_idx = merlin_koi_get_lane();
...
SDK_STATIC err_code_t _merlin_koi_read_core_state(merlin_koi_core_state_st *istate) {
  merlin_koi_core_state_st state;
  struct merlin_koi_uc_core_config_st core_cfg;

  ENULL_MEMSET(&state, 0, sizeof(merlin_koi_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct merlin_koi_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(merlin_koi_read_die_temperature(&die_temp));
  ...
  EFUN(_merlin_koi_pll_lock_status(&state.pll_lock, &state.pll_lock_chg));
...
static err_code_t _merlin_koi_display_core_state_no_newline(void) {
    ...
    ENULL_MEMSET(&state     , 0, sizeof(state     ));
        EFUN(_merlin_koi_read_core_state(&state));
    EFUN_PRINTF(("%02d  "              ,  merlin_koi_get_core()));
    ...
    EFUN(_merlin_koi_display_pll_to_divider(  state.pll_div));
...
SDK_STATIC err_code_t _merlin_koi_pll_lock_status(uint8_t *pll_lock, uint8_t *pll_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(merlin_koi_pmd_rdt_reg(PLL_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_koi_pmd_lock_status(uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(merlin_koi_pmd_rdt_reg(PMD_LOCK_STATUS_ADDR, &rddata));
...
SDK_STATIC err_code_t _merlin_koi_sigdet_status(uint8_t *sig_det, uint8_t *sig_det_chg) {
    ...
    uint16_t rddata;
    EFUN(merlin_koi_pmd_rdt_reg(SIGDET_STATUS_ADDR, &rddata));
...
static err_code_t _merlin_koi_display_ber_scan_data(uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    merlin_koi_osr_mode_st osr_mode;
    struct merlin_koi_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(merlin_koi_get_uc_core_config(&core_config));
    EFUN(_merlin_koi_get_osr_mode(&osr_mode));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(merlin_koi_pmd_uc_cmd( CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(merlin_koi_pmd_uc_cmd( CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(merlin_koi_pmd_uc_cmd(CMD_CAPTURE_BER_END,0x00,2000));

  return(ERR_CODE_NONE);
}

void get_dsc_param_koi(void) {
    merlin_koi_display_lane_state_hdr();
    merlin_koi_display_lane_state();
}
