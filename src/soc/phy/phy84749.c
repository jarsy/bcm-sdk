/*
 * $Id: phy84749.c,v 1.7 Broadcom SDK $
 *
...
/*
 * File:        phy84749.c
 * Purpose:     SDK PHY driver for BCM84749 (MACSec)
 *
 * Supported BCM546X Family of PHY devices:
 *
 *      Device  Ports   Media                           MAC Interface
 *      84749    4       2 10G SFP+, 2 1G SFP            XFI
 *
 * Workarounds:
 *
 * References:
...
#define PHY84749_REG_READ(_unit, _phy_ctrl, _addr, _val) \
            READ_PHY_REG((_unit), (_phy_ctrl), (_addr), (_val))

#define READ_PHY84749_PMA_PMD_REG(_unit, _phy_ctrl, _reg,_val) \
            PHY84749_REG_READ((_unit), (_phy_ctrl), \
              SOC_PHY_CLAUSE45_ADDR(1, (_reg)), (_val))


STATIC int phy_84749_init(int unit, soc_port_t port);
STATIC int phy_84749_link_get(int unit, soc_port_t port, int *link);
STATIC int phy_84749_enable_set(int unit, soc_port_t port, int enable);
STATIC int phy_84749_duplex_set(int unit, soc_port_t port, int duplex);
STATIC int phy_84749_duplex_get(int unit, soc_port_t port, int *duplex);
STATIC int phy_84749_speed_set(int unit, soc_port_t port, int speed);
STATIC int phy_84749_speed_get(int unit, soc_port_t port, int *speed);
#if 0
STATIC int phy_84749_master_set(int unit, soc_port_t port, int master);
STATIC int phy_84749_master_get(int unit, soc_port_t port, int *master);
#endif
STATIC int phy_84749_an_set(int unit, soc_port_t port, int autoneg);
STATIC int phy_84749_an_get(int unit, soc_port_t port,
                                int *autoneg, int *autoneg_done);
STATIC int phy_84749_lb_set(int unit, soc_port_t port, int enable);
STATIC int phy_84749_lb_get(int unit, soc_port_t port, int *enable);
STATIC int phy_84749_medium_config_set(int unit, soc_port_t port, 
                                      soc_port_medium_t  medium,
                                      soc_phy_config_t  *cfg);
STATIC int phy_84749_medium_config_get(int unit, soc_port_t port, 
                                      soc_port_medium_t medium,
                                      soc_phy_config_t *cfg);
STATIC int phy_84749_ability_advert_set(int unit, soc_port_t port, 
                                       soc_port_ability_t *ability);
STATIC int phy_84749_ability_advert_get(int unit, soc_port_t port,
                                       soc_port_ability_t *ability);
STATIC int phy_84749_ability_remote_get(int unit, soc_port_t port,
                                       soc_port_ability_t *ability);
STATIC int phy_84749_ability_local_get(int unit, soc_port_t port, 
                                soc_port_ability_t *ability);
STATIC int phy_84749_firmware_set(int unit, int port, int offset, 
                                 uint8 *data, int len);
STATIC int phy_84749_control_set(int unit, soc_port_t port,
                     soc_phy_control_t type, uint32 value);
STATIC int phy_84749_control_get(int unit, soc_port_t port,
                     soc_phy_control_t type, uint32 *value);
STATIC int phy_84749_linkup(int unit, soc_port_t port);

/*
 * Function:
 *      _phy_84749_medium_config_update
 * Purpose:
...
STATIC int
_phy_84749_medium_config_update(int unit, soc_port_t port,
                                soc_phy_config_t *cfg)
{

    SOC_IF_ERROR_RETURN
        (phy_84749_speed_set(unit, port, cfg->force_speed));
    SOC_IF_ERROR_RETURN
        (phy_84749_duplex_set(unit, port, cfg->force_duplex));
    SOC_IF_ERROR_RETURN
        (phy_84749_ability_advert_set(unit, port, &cfg->advert_ability));
    SOC_IF_ERROR_RETURN
        (phy_84749_an_set(unit, port, cfg->autoneg_enable));
...
STATIC int
phy_84749_init(int unit, soc_port_t port)
{
    phy_ctrl_t     *pc;
    ...
    rv = bmacsec_phy84749_phy_mac_addr_set(phy_dev_addr, 
                                          macsec_dev_addr, 
                                          pc->macsec_dev_port,
    ...
    if(rv != BMACSEC_E_NONE) {
        LOG_WARN(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
                             "phy_84749_init: bmacsec_phy_mac_addr_set "
    ...
    if (fiber_preferred) {
        line_mode = BMACSEC_PHY84749_INTF_SFI;
        ...
        line_mode = BMACSEC_PHY84749_INTF_SGMII;
    ...
    phy_sys_xaui_rx_lane_map = (temp) ? 0x3210 : 0;
    ...
    phy_sys_xaui_tx_lane_map = (temp) ? 0x3210 : 0;



    /* Init PHYS and MACs to defaults */
    rv = bmacsec_phy84749_init(phy_dev_addr, 1, 
                               line_mode, 
                               macsec_enable, 
    ...
    if (line_mode != BMACSEC_PHY84749_INTF_SGMII) {
        SOC_IF_ERROR_RETURN
            (_phy_84749_medium_config_update(unit, port,
    ...
    if (IS_HG_PORT(unit, port)) {
        rv = bmacsec_phy84749_higig_mode(phy_dev_addr, 1);
        if (rv != BMACSEC_E_NONE) {
            return SOC_E_FAIL;
        }
    }
    ...
    if (macsec_enable) {
        rv = soc_macsecphy_init(unit, port, pc, 
                    BMACSEC_CORE_BCM84749, bmacsec_io_mmi1_cl45);
        if (!SOC_SUCCESS(rv)) {
...
STATIC int
phy_84749_link_get(int unit, soc_port_t port, int *link)
{
    phy_ctrl_t    *pc;
    int rv = SOC_E_NONE;
    bmacsec_dev_addr_t phy_dev_addr;
    bmacsec_phy84749_intf_t bmacsec_mode;
    int bmacsec_link;
    ...
    *link = FALSE;      /* Default return */
    ...
    rv = bmacsec_phy84749_line_intf_get(phy_dev_addr, 0, &bmacsec_mode);
    if (!BMACSEC_E_SUCCESS(rv)) {
        return SOC_E_FAIL;
    }

    if (bmacsec_mode == BMACSEC_PHY84749_INTF_SGMII) {
        PHY_FLAGS_SET(unit, port, PHY_FLAGS_COPPER);
        PHY_FLAGS_CLR(unit, port, PHY_FLAGS_FIBER);
    } else {
        PHY_FLAGS_SET(unit, port, PHY_FLAGS_FIBER);
    ...
    rv = bmacsec_phy84749_link_get(phy_dev_addr, &bmacsec_link);
    if (!BMACSEC_E_SUCCESS(rv)) {
        return SOC_E_FAIL;
    }
    if (bmacsec_link) {
    ...
    LOG_VERBOSE(BSL_LS_SOC_PHY,
                (BSL_META_U(unit,
                            "phy_84749_link_get: u=%d p=%d link=%d\n"),
                 unit, port,
                 *link));
...
STATIC int
phy_84749_enable_set(int unit, soc_port_t port, int enable)
{
    phy_ctrl_t    *pc;
    int rv = SOC_E_NONE;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_enable_set(phy_dev_addr, enable);
 ...
 */
STATIC int
phy_84749_duplex_set(int unit, soc_port_t port, int duplex)
{
    int                          rv;
    phy_ctrl_t                   *pc;
    bmacsec_dev_addr_t           phy_dev_addr;
    bmacsec_phy84749_duplex_t    bmacsec_duplex;
    ...
    if (duplex) {
        bmacsec_duplex = BMACSEC_PHY84749_FULL_DUPLEX;
    } else {
        bmacsec_duplex = BMACSEC_PHY84749_HALF_DUPLEX;
    }
    rv = bmacsec_phy84749_duplex_set(phy_dev_addr, bmacsec_duplex);
    ...
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "phy_84749_duplex_set: u=%d p=%d d=%d rv=%d\n"),
              unit, port, duplex, rv));
    return (SOC_ERROR(rv));
}
...
STATIC int
phy_84749_duplex_get(int unit, soc_port_t port, int *duplex)
{
    ...
    rv = bmacsec_phy84749_duplex_get(phy_dev_addr, &bmacsec_duplex);
    if (rv != BMACSEC_E_NONE) {
        return SOC_E_FAIL;
    }
    if (bmacsec_duplex == BMACSEC_PHY84749_FULL_DUPLEX) {
        *duplex = TRUE;
    } else {
        *duplex = FALSE;
    }
    ...
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "phy_84749_duplex_get: u=%d p=%d d=%d rv=%d\n"),
              unit, port, *duplex, rv));
    return SOC_E_NONE;
}
 ...
 */
STATIC int
phy_84749_speed_set(int unit, soc_port_t port, int speed)
{
    phy_ctrl_t *pc;
    phy_ctrl_t  *int_pc;
    int            rv; 
    ...
    rv = bmacsec_phy84749_speed_set(phy_dev_addr, speed);
    if(BMACSEC_E_SUCCESS(rv)) {
        if (NULL != int_pc) {
    ...
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "phy_84749_speed_set: u=%d p=%d s=%d fiber=%d rv=%d\n"),
 ...
 *      phy_84749_speed_get
 * Purpose:
 *      Get the current operating speed for a 84749 device.
 * Parameters:
 *      unit - StrataSwitch unit #.
 *      port - StrataSwitch port #.
...
STATIC int
phy_84749_speed_get(int unit, soc_port_t port, int *speed)
{
    int rv = SOC_E_NONE;
    bmacsec_dev_addr_t phy_dev_addr;
    phy_ctrl_t    *pc;
    ...
    rv = bmacsec_phy84749_speed_get(phy_dev_addr, speed);

    if(!BMACSEC_E_SUCCESS(rv)) {
        LOG_WARN(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
                             "phy_84749_speed_get: u=%d p=%d invalid speed\n"),
                  unit, port));
    } else {
        LOG_INFO(BSL_LS_SOC_PHY,
                 (BSL_META_U(unit,
                             "phy_84749_speed_get: u=%d p=%d speed=%d"),
                  unit, port, *speed));
    }
...
STATIC int
phy_84749_an_set(int unit, soc_port_t port, int autoneg)
{
    int                 rv;
    phy_ctrl_t   *pc;
    ...
    rv = bmacsec_phy84749_an_set(phy_dev_addr, autoneg, autoneg_advert);
    if(BMACSEC_E_SUCCESS(rv)) {
        if(PHY_COPPER_MODE(unit, port)) {
    ...
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "phy_84749_an_set: u=%d p=%d autoneg=%d rv=%d\n"),
...
STATIC int
phy_84749_an_get(int unit, soc_port_t port,
                     int *autoneg, int *autoneg_done)
{
    int           rv;
    phy_ctrl_t   *pc;
    ...
    *autoneg_done = FALSE;
    ...
    rv = bmacsec_phy84749_an_get(phy_dev_addr, autoneg, autoneg_done);
...
STATIC int
phy_84749_lb_set(int unit, soc_port_t port, int enable)
{
    int           rv;
#if 0
    int           link = 0;
    ...
    rv = bmacsec_phy84749_loopback_set(phy_dev_addr, enable);
    ...
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "phy_84749_lb_set: u=%d p=%d en=%d rv=%d\n"), 
              unit, port, enable, rv));
...
STATIC int
phy_84749_lb_get(int unit, soc_port_t port, int *enable)
{
    int                  rv;
    phy_ctrl_t    *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_loopback_get(phy_dev_addr, enable);
...
STATIC int
phy_84749_medium_config_set(int unit, soc_port_t port, 
                           soc_port_medium_t  medium,
                           soc_phy_config_t  *cfg)
{
    phy_ctrl_t    *pc;
   ...
   switch (medium) {
    ...
    if (medium_update) {
         ...
         */
        SOC_IF_ERROR_RETURN
            (_phy_84749_medium_change(unit, port, TRUE, medium));
    } else {
        ...
        if (active_medium == change_medium) {
            ...
            SOC_IF_ERROR_RETURN
                (_phy_84749_medium_config_update(unit, port, change_medium));
        }
    }
#endif
    return SOC_E_NONE;
...
STATIC int
phy_84749_medium_config_get(int unit, soc_port_t port, 
                           soc_port_medium_t medium,
                           soc_phy_config_t *cfg)
{
    int            rv;
 ...
 */
STATIC int
phy_84749_ability_advert_set(int unit, soc_port_t port,soc_port_ability_t *ability)
{
    int        rv;
    phy_ctrl_t *pc;
    bmacsec_phy84749_port_ability_t local_ability;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    if(ability->speed_full_duplex & SOC_PA_SPEED_1000MB) {
        local_ability |= BMACSEC_PHY84749_PA_1000MB_FD;
    }
    if(ability->speed_full_duplex & SOC_PA_SPEED_10GB) {
        local_ability |= BMACSEC_PHY84749_PA_10000MB_FD;
    }

    if(ability->pause & SOC_PA_PAUSE_TX) {
        local_ability |= BMACSEC_PHY84749_PA_PAUSE_TX;
    }
    if(ability->pause & SOC_PA_PAUSE_RX) {
        local_ability |= BMACSEC_PHY84749_PA_PAUSE_RX;
    }
    if(ability->pause & SOC_PA_PAUSE_ASYMM) {
        local_ability |= BMACSEC_PHY84749_PA_PAUSE_ASYMM;
    }

    if(ability->loopback & SOC_PA_LB_NONE) {
        local_ability |= BMACSEC_PHY84749_PA_LB_NONE;
    }
    if(ability->loopback & SOC_PA_LB_PHY) {
        local_ability |= BMACSEC_PHY84749_PA_LB_PHY;
    }
    if(ability->flags & SOC_PA_AUTONEG) {
        local_ability |= BMACSEC_PHY84749_PA_AN;
    }

    phy_dev_addr = SOC_MACSECPHY_MDIO_ADDR(unit, pc->phy_id, 1);

    rv = bmacsec_phy84749_ability_advert_set(phy_dev_addr, local_ability);
 ...
 */
STATIC int
phy_84749_ability_advert_get(int unit, soc_port_t port,soc_port_ability_t *ability)
{
    phy_ctrl_t      *pc;
    int        rv;
    bmacsec_phy84749_port_ability_t local_ability;
    ...
    rv = bmacsec_phy84749_ability_advert_get(phy_dev_addr, &local_ability);
    if (rv != BMACSEC_E_NONE) {
        return SOC_E_FAIL;
    }

    if(local_ability & BMACSEC_PHY84749_PA_1000MB_FD) {
        ability->speed_full_duplex |= SOC_PA_SPEED_1000MB;
    }
    if(local_ability & BMACSEC_PHY84749_PA_10000MB_FD) {
        ability->speed_full_duplex |= SOC_PA_SPEED_10GB;
    }

    if(local_ability & BMACSEC_PHY84749_PA_PAUSE_TX) {
        ability->pause |= SOC_PA_PAUSE_TX;
    }
    if(local_ability & BMACSEC_PHY84749_PA_PAUSE_RX) {
        ability->pause |= SOC_PA_PAUSE_RX;
    }
    if(local_ability & BMACSEC_PHY84749_PA_PAUSE_ASYMM) {
        ability->pause |= SOC_PA_PAUSE_ASYMM;
    }

    if(local_ability & BMACSEC_PHY84749_PA_LB_NONE) {
        ability->loopback |= SOC_PA_LB_NONE;
    }
    if(local_ability & BMACSEC_PHY84749_PA_LB_PHY) {
        ability->loopback |= SOC_PA_LB_PHY;
    }
    if(local_ability & BMACSEC_PHY84749_PA_AN) {
 ...
 */
STATIC int
phy_84749_ability_remote_get(int unit, soc_port_t port,soc_port_ability_t *ability)
{
    phy_ctrl_t      *pc;
    int        rv;
    bmacsec_phy84749_port_ability_t remote_ability;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_remote_ability_advert_get(phy_dev_addr,
                                                   &remote_ability);
    if (rv != BMACSEC_E_NONE) {
        return SOC_E_FAIL;
    }

    if(remote_ability & BMACSEC_PHY84749_PA_1000MB_FD) {
        ability->speed_full_duplex |= SOC_PA_SPEED_1000MB;
    }
    if(remote_ability & BMACSEC_PHY84749_PA_10000MB_FD) {
        ability->speed_full_duplex |= SOC_PA_SPEED_10GB;
    }

    if(remote_ability & BMACSEC_PHY84749_PA_PAUSE_TX) {
        ability->pause |= SOC_PA_PAUSE_TX;
    }
    if(remote_ability & BMACSEC_PHY84749_PA_PAUSE_RX) {
        ability->pause |= SOC_PA_PAUSE_RX;
    }
    if(remote_ability & BMACSEC_PHY84749_PA_PAUSE_ASYMM) {
        ability->pause |= SOC_PA_PAUSE_ASYMM;
    }

    if(remote_ability & BMACSEC_PHY84749_PA_LB_NONE) {
        ability->loopback |= SOC_PA_LB_NONE;
    }
    if(remote_ability & BMACSEC_PHY84749_PA_LB_PHY) {
        ability->loopback |= SOC_PA_LB_PHY;
    }
    if(remote_ability & BMACSEC_PHY84749_PA_AN) {
        ability->flags = SOC_PA_AUTONEG;
    }
...
STATIC int
phy_84749_ability_local_get(int unit, soc_port_t port, soc_port_ability_t *ability)
{
    int         rv = SOC_E_NONE;
    phy_ctrl_t *pc;
    bmacsec_phy84749_port_ability_t local_ability = 0;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_ability_local_get(phy_dev_addr, &local_ability);
    if (rv != BMACSEC_E_NONE) {
        return SOC_E_FAIL;
    }
...
STATIC int
phy_84749_firmware_set(int unit, int port, int offset, uint8 *data,int len)
{
    int            rv;
    phy_ctrl_t *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_spi_firmware_update(phy_dev_addr, data, len);
    if (rv != BMACSEC_E_NONE) {
            LOG_WARN(BSL_LS_SOC_PHY,
                     (BSL_META_U(unit,
                                 "PHY84749 firmware upgrade possibly failed:"
                                 "u=%d p=%d\n"), unit, port));
        return (SOC_E_FAIL);
    }
    LOG_INFO(BSL_LS_SOC_PHY,
             (BSL_META_U(unit,
                         "PHY84749 firmware upgrade successful:"
                         "u=%d p=%d\n"), unit, port));
...
STATIC int
phy_84749_medium_status(int unit, soc_port_t port, soc_port_medium_t *medium)
{
 ...
 */
STATIC int
phy_84749_control_set(int unit, soc_port_t port,
                     soc_phy_control_t type, uint32 value)
{
    int rv = BMACSEC_E_NONE; 

#ifdef INCLUDE_MACSEC
#ifdef PHY84749_SWITCH_FIXED_MODE
    bmacsec_dev_addr_t phy_dev_addr;
    bmacsec_phy84749_intf_t line_mode;
...
*/
    ...
    phy_dev_addr = SOC_MACSECPHY_MDIO_ADDR(unit, pc->phy_id, 1); 
    rv = bmacsec_phy84749_line_intf_get(phy_dev_addr,
    ...
    switch(type) {
        case SOC_PHY_CONTROL_MACSEC_SWITCH_FIXED:
            ...
            if(value) {
                rv = bmacsec_phy84749_switch_side_set_params(phy_dev_addr, 
                                BMACSEC_BCM84749_SWITCH_FIXED, 
                                pc->macsec_switch_fixed_speed,
                                pc->macsec_switch_fixed_duplex,
                                pc->macsec_switch_fixed_pause);
            } else { /* Switch side follows line side */
                SOC_IF_ERROR_RETURN
                    (phy_84749_speed_get(unit, port, &line_speed));
                SOC_IF_ERROR_RETURN
                    (phy_84749_duplex_get(unit, port, &line_duplex));
#if 0 
                SOC_IF_ERROR_RETURN
                    (phy_84749_pause_get(unit, port, &line_pause));
#endif
                rv = bmacsec_phy84749_switch_side_set_params(phy_dev_addr, 
                                BMACSEC_BCM84749_SWITCH_FOLLOWS_LINE, 
        ...
        case SOC_PHY_CONTROL_MACSEC_SWITCH_FIXED_SPEED:
            ...
            if (pc->macsec_dev_port > 2) {
                ...
                if (line_mode == BMACSEC_PHY84749_INTF_1000X) {
                    if (value != 1000) {
                        return SOC_E_CONFIG;
                    }
                } else {
        ...
        case SOC_PHY_CONTROL_MACSEC_PAUSE_RX_FORWARD:
            ...
            rv = bmacsec_phy84749_line_side_pause_forward(phy_dev_addr,
                                                         pause_fwd);
            if (rv == BMACSEC_E_NONE) {
                rv = bmacsec_phy84749_switch_side_pause_forward(
        ...
        case SOC_PHY_CONTROL_MACSEC_PAUSE_TX_FORWARD:
            ...
            rv = bmacsec_phy84749_line_side_pause_forward(phy_dev_addr,
                                                          pause_fwd);
            if (rv == BMACSEC_E_NONE) {
                rv = bmacsec_phy84749_switch_side_pause_forward(phy_dev_addr,
                                                                pause_fwd);
            }
            break;
        case SOC_PHY_CONTROL_MACSEC_LINE_IPG:
            pc->macsec_line_ipg = value;
            rv = bmacsec_phy84749_line_side_ipg_set(phy_dev_addr, value);
            break;
        case SOC_PHY_CONTROL_MACSEC_SWITCH_IPG:
            pc->macsec_switch_ipg = value;
            rv = bmacsec_phy84749_switch_side_ipg_set(phy_dev_addr, value);
            break;
        default:
            rv = SOC_E_UNAVAIL;
            break;
    }
#endif /* PHY84749_SWITCH_FIXED_MODE */
#else
    bmacsec_dev_addr_t phy_dev_addr;
    bmacsec_phy84749_intf_t line_mode;
    int pause_fwd =0, line_speed=0, line_duplex=0, line_pause=1;
 ...
 */
STATIC int 
phy_84749_control_get(int unit, soc_port_t port,
                     soc_phy_control_t type, uint32 *value)
{
    int rv = SOC_E_NONE;
#ifdef INCLUDE_MACSEC
 ...
 */
STATIC int 
phy_84749_linkup(int unit, soc_port_t port)
{
    int rv = SOC_E_NONE;
    phy_ctrl_t    *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_line_intf_get(phy_dev_addr, 0, &line_mode);
    if (!BMACSEC_E_SUCCESS(rv)) {
        return SOC_E_FAIL;
    }

    rv = bmacsec_84749_phy_linkup(phy_dev_addr, pc->macsec_enable);
 ...
 */
STATIC int
phy_84749_interface_get(int unit, soc_port_t port, soc_port_if_t *pif)
{
 ...
 */
STATIC int
phy_84749_probe(int unit, phy_ctrl_t *pc)
{
    uint16  data1, data2;

    if (READ_PHY84749_PMA_PMD_REG(unit, pc, 0xc802, &data1) < 0) {
        return SOC_E_NOT_FOUND;
    }

    if (READ_PHY84749_PMA_PMD_REG(unit, pc, 0xc803, &data2) < 0) {
        return SOC_E_NOT_FOUND;
    }
    ...
    if ((data2 & 0xf) == 0x8) {
        ...
        switch (data1) {
            ...
            case 0x4728:
            case 0x4748:
                if (0x8700 != soc_property_port_get(unit, 
                                                    pc->port,
                                                    spn_PORT_PHY_ID1, 0)){
                    return SOC_E_NOT_FOUND;
                }
...
int
phy_84749_timesync_config_set(int unit, soc_port_t port, soc_port_phy_timesync_config_t *timesync_config)
{
    int                          rv;
    phy_ctrl_t                   *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_timesync_config_set(phy_dev_addr, (bmacsec_port_phy_timesync_config_t *)timesync_config);
    return (SOC_ERROR(rv));
...
int
phy_84749_timesync_config_get(int unit, soc_port_t port, soc_port_phy_timesync_config_t *timesync_config)
{
    int                          rv;
    phy_ctrl_t                   *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_timesync_config_get(phy_dev_addr, (bmacsec_port_phy_timesync_config_t *)timesync_config);
    return (SOC_ERROR(rv));
...
/*
 * Function:
 *      phy_84749_timesync_control_set
 * Description:
 *      
 * Parameters:
 *
...
int
phy_84749_timesync_control_set(int unit, soc_port_t port, soc_port_control_phy_timesync_t control_type, uint64 value)
{
    int                          rv;
    phy_ctrl_t                   *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_timesync_control_set(phy_dev_addr, control_type, btemp64);
    return (SOC_ERROR(rv));
...
/*
 * Function:
 *      phy_84749_timesync_control_get
 * Description:
 *      
 * Parameters:
 *
 ...
 */
int
phy_84749_timesync_control_get(int unit, soc_port_t port, soc_port_control_phy_timesync_t control_type, uint64 *value)
{
    int                          rv;
    phy_ctrl_t                   *pc;
    bmacsec_dev_addr_t phy_dev_addr;
    ...
    rv = bmacsec_phy84749_timesync_control_get(phy_dev_addr, control_type, &btemp64);
 ...
 *    phy_84749_drv
 * Purpose:
 *    Phy Driver for BCM84749 MACSEC PHY
 */

phy_driver_t phy_84749drv_xe = {
    "84749 1G/10-Gigabit PHY Driver",
    phy_84749_init,        /* Init */
    phy_null_reset,       /* Reset */
    phy_84749_link_get,    /* Link get   */
    phy_84749_enable_set,  /* Enable set */
    phy_null_enable_get,  /* Enable get */
    phy_84749_duplex_set,  /* Duplex set */
    phy_84749_duplex_get,  /* Duplex get */
    phy_84749_speed_set,   /* Speed set  */
    phy_84749_speed_get,   /* Speed get  */
    phy_null_set,          /* Master set */
    phy_null_zero_get,     /* Master get */
    phy_84749_an_set,      /* ANA set */
    phy_84749_an_get,      /* ANA get */
    NULL,                 /* Local Advert set, deprecated */
    NULL,                 /* Local Advert get, deprecated */
    NULL,                 /* Remote Advert get, deprecated */
    phy_84749_lb_set,      /* PHY loopback set */
    phy_84749_lb_get,      /* PHY loopback set */
    phy_null_interface_set, /* IO Interface set */
    phy_84749_interface_get, /* IO Interface get */
    NULL,                   /* pd_ability, deprecated */
    phy_84749_linkup,
    NULL,
    phy_null_mdix_set,        /* phy_84749_mdix_set */
    phy_null_mdix_get,        /* phy_84749_mdix_get */
    phy_null_mdix_status_get, /* phy_84749_mdix_status_get */
    phy_84749_medium_config_set, /* medium config setting set */
    phy_84749_medium_config_get, /* medium config setting get */
    phy_84749_medium_status,        /* active medium */
    NULL,                    /* phy_cable_diag  */
    NULL,                    /* phy_link_change */
    phy_84749_control_set,    /* phy_control_set */
    phy_84749_control_get,    /* phy_control_get */
    NULL,                    /* phy_reg_read */
    NULL,                    /* phy_reg_write */
    NULL,                    /* phy_reg_modify */
    NULL,                    /* phy_notify */
    phy_84749_probe,         /* pd_probe  */
    phy_84749_ability_advert_set,  /* pd_ability_advert_set */
    phy_84749_ability_advert_get,  /* pd_ability_advert_get */
    phy_84749_ability_remote_get,  /* pd_ability_remote_get */
    phy_84749_ability_local_get,   /* pd_ability_local_get  */
    phy_84749_firmware_set,
    phy_84749_timesync_config_set,
    phy_84749_timesync_config_get,
    phy_84749_timesync_control_set,
...
#else /* INCLUDE_PHY_84749 */
int _phy_84749_not_empty;
