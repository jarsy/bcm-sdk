...
{
    unsigned char *addr;
    unsigned char *addr_orig;

    /* Allocate additional memory to store padding size for alignment */
    if((addr_orig = BFCMAP_SAL_MALLOC((size + 0x8), s)) == NULL){
        return NULL;
    }
     ...
     * n = 3
     */
    addr = (unsigned char *) (((size_t)addr_orig + 0x8) & (~0x7));

    /* store padding size for alignment in previous location */
    *(addr - 1) = addr - addr_orig;
...
{
    unsigned char *addr_orig = addr;

    /* calculate original malloc'ed address by subtracting
     * padding size and freeing that memory
     */
    addr_orig = addr_orig - *(addr_orig - 1);

    BFCMAP_SAL_FREE(addr_orig);
    return;
}
