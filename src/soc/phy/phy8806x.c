...
STATIC int _phy8806x_dump_buffer(int unit, int port, char *arg);
...
/*     soc_phy_info_t *pi = NULL; */
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) ||
...
#if defined(INCLUDE_FCMAP)
                SOC_IF_ERROR_RETURN
                    (_phy_8806x_axi_read(unit, pc, 0x27fff0 + (pc->phy_id & 0x3) * 0x100000, &temp32, 1));
                temp32 = LE2HOST32(temp32);
        ...
        /* Store the operational mode in BTCM 0x40300 of mHost 1 */
#if defined(INCLUDE_FCMAP)
        temp32 = HOST2LE32(pc->fcmap_enable ? 1 : 0);
#else
        temp32 = 0;
#endif
        if (!(SOC_WARM_BOOT(unit) || SOC_IS_RELOADING(unit))) {
            SOC_IF_ERROR_RETURN
                (_phy_8806x_axi_write(unit, pc, 0x27fff0 + (pc->phy_id & 0x3) * 0x100000, &temp32, 1));
        }

        if ( ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS1) && (FLAGS(pc) & PHY8806X_FLEX)) && 
            ...
            */
            ...
            /* Place mHosts of the lane in reset */
            SOC_IF_ERROR_RETURN
    ...
    if ((PHYCTRL_INIT_STATE(pc) == PHYCTRL_INIT_STATE_PASS4) ||
        ...
        /* Start all non BMPs */
        if (( FLAGS(pc) & PHY8806X_FLEX) || ( !( FLAGS(pc) & (PHY8806X_BMP0 | PHY8806X_BMP1)) )) {
                               ...
                               soc_port_ability_t *ability)
{
   ...
   speed_full_duplex|= PHYMOD_BAM_CL73_CAP_25G_CR1_GET(reg73_ability) ?SOC_PA_SPEED_25GB:0;
   speed_full_duplex|= PHYMOD_BAM_CL73_CAP_25G_KR1_GET(reg73_ability) ?SOC_PA_SPEED_25GB:0;
   speed_full_duplex|= PHYMOD_BAM_CL73_CAP_20G_CR1_GET(reg73_ability) ?SOC_PA_SPEED_20GB:0;
   speed_full_duplex|= PHYMOD_BAM_CL73_CAP_20G_KR1_GET(reg73_ability) ?SOC_PA_SPEED_20GB:0;
...
phy_8806x_lb_set(int unit, soc_port_t port, int enable)
{
    ...
    if (pc->flags & PHYCTRL_SYS_SIDE_CTRL) {
        SOC_IF_ERROR_RETURN
            (phymod_phy_loopback_set(pm_phy, phymodLoopbackSysGlobal, enable));
    } else {
        SOC_IF_ERROR_RETURN
    ...
    switch (offset) {
        ...
        if ( FLAGS(pc) & PHY8806X_BMP0 ) {
            ...
            SOC_IF_ERROR_RETURN
                (READ_TOP_RTS_MISC_CTRL_REG(unit, pc, &temp32));
                temp32 &= ~0xff;
                if (!(FLAGS(pc) & PHY8806X_MODE_CHANGE)) {
...
#if defined(INCLUDE_FCMAP)
    /* Skip all loopback set for FC */
    if ((pc->fcmap_enable) && 
        ((type == SOC_PHY_CONTROL_LOOPBACK_INTERNAL) ||
         (type == SOC_PHY_CONTROL_LOOPBACK_PMD)      ||
         (type == SOC_PHY_CONTROL_LOOPBACK_REMOTE)   ||
    ...
    void *arg)       /* command argument based on op_type/op_cmd */
{
    ...
    switch(op_cmd) {
        ...
        case PHY_DIAG_CTRL_STATE_GENERIC:
            ...
            rv = _phy8806x_dump_buffer(unit, port, arg);
            break;
        default:
    ...
    while (size-- > 0) {
        ...
        if (count >= AXI_MAX_POLLS) {
            LOG_CLI((BSL_META_U(unit,
                                ...
                                "AXI_MAX_POLLS reached !.\n"), unit, pc->port, addr, control, status));
            return (SOC_E_FAIL);
        }
