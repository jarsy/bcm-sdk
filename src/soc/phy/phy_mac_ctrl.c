                          ...
                          int speed, int duplex, int pause_enable)
{
    ...
    case  PHY_MAC_SWITCH_FIXED :
        ...
        rv = BMACSEC_MAC_AUTO_CONFIG_SET(phy_info->unimac, port, 0);
        if (!BMACSEC_E_SUCCESS(rv)) {
            return BMACSEC_E_FAIL;
        }
        /* Set MAC Speed */
        rv = BMACSEC_MAC_SPEED_SET(phy_info->unimac, port, speed);
        if (!BMACSEC_E_SUCCESS(rv)) {
            return BMACSEC_E_FAIL;
        }
        /* Set MAC Duplex */
        rv = BMACSEC_MAC_DUPLEX_SET(phy_info->unimac, port, duplex);
        if (!BMACSEC_E_SUCCESS(rv)) {
            return BMACSEC_E_FAIL;
        }
        /* Set Pause settings */
        rv = BMACSEC_MAC_PAUSE_SET(phy_info->unimac, port, pause_enable);
        if (!BMACSEC_E_SUCCESS(rv)) {
            return BMACSEC_E_FAIL;
        }
        break;
    ...
    case  PHY_MAC_SWITCH_FOLLOWS_LINE :
        ...
        rv = BMACSEC_MAC_AUTO_CONFIG_SET(phy_info->unimac, port, 1);
        if (!BMACSEC_E_SUCCESS(rv)) {
            return BMACSEC_E_FAIL;
        }
        break;
