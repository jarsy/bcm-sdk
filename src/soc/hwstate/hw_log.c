    ...
    for (; i < Hw_Log_List[unit].header->nof_elements; i++) {
        ...
        if (entry->Entry.Type == LOG_ENTRY_TYPE_Memory)
        {
            if(entry->Entry.Data.MemData.DescDMA) {
                res = jer_sbusdma_desc_add(unit, entry->Entry.Data.MemData.Mem, entry->Entry.Data.MemData.ArrayIndex, entry->Entry.Data.MemData.Blk, entry->Entry.Data.MemData.Index, entry->Entry.Data.MemData.EntryData);
        ...
        if (entry->Entry.Type == LOG_ENTRY_TYPE_Memory)
        {
            ...
            if ((mem == saved_in_hw_log_mem) &&
                ...
                (HWLOG_MAX(interval_A_start,interval_B_start) <= HWLOG_MIN(interval_A_end,interval_B_end))) /* check if the two intervals are overlapping */ {
                    ...
                    /* return an error if a dma read tries to retrieve values from journaled dma entries*/
                    LOG_ERROR(BSL_LS_SOC_HWLOG,
...
{
    void *pt = Hw_Log_List[unit].header;
