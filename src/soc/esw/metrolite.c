...
int _saber2_ledup_init(int unit, uint16 dev_id);
 ...
 */
STATIC
int _soc_ml_mmu_init_helper(int unit)
{
    soc_info_t    *si= &SOC_INFO(unit);
    uint16        dev_id=0;
    uint8         rev_id=0;
...
int _soc_ml_mmu_reconfigure(int unit)
{
    SOC_IF_ERROR_RETURN(_soc_ml_mmu_init_helper(unit));
    LOG_CLI((BSL_META_U(unit,
                    "MMU is reconfigured \n")));
    return SOC_E_NONE;
}
...
soc_metrolite_pbmp_init(int unit, ml_pbmp_t ml_pbmp, soc_pbmp_t *pbmp_pp_unused) 
{
    ...
    sal_memcpy(ml_selected_port_details,
            ml_port_details[ml_port_details_index],
            sizeof(soc_port_details_t) * (ML_MAX_PHYSICAL_PORTS+1));

    /* Keep the default config to compare later */
    sal_memcpy(ml_selected_port_details_orig,
            ml_port_details[ml_port_details_index],
            sizeof(soc_port_details_t) * (ML_MAX_PHYSICAL_PORTS+1));
