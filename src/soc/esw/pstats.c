...
#ifdef BCM_TOMAHAWK2_SUPPORT
    if (SOC_IS_TOMAHAWK2(unit) && tbl_desc[tbl_ctrl->tindex].mor_dma) {
        soc_tomahawk2_mmu_pstats_mor_disable(unit);
    }
#endif
...
#define PSTATS_TBLS "PSTATS TBLS"
        sal_strncpy(ctrl.name, PSTATS_TBLS, sizeof(PSTATS_TBLS));
        ...
        if (tbl_ctrl && desc_handle) {
            ...
            LOG_INFO(BSL_LS_SOC_COMMON,
                     (BSL_META_U(unit,
                      ...
                      unit, ti,
                      tbl_desc[ti].mor_dma && (soc->pstats_flags & _TH2_MMU_PSTATS_MOR_EN) ? "MOR" : "Non-MOR",
...
int
soc_pstats_mem_get(int unit, soc_mem_t mem, uint8 *buf, int sync)
{
    soc_control_t *soc = SOC_CONTROL(unit);
    soc_pstats_tbl_desc_t *tbl_desc;
    soc_pstats_tbl_ctrl_t *tbl_ctrl = NULL;
