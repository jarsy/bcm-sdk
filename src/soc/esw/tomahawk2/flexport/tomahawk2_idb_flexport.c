...
soc_tomahawk2_idb_lpbk_ca_reset_buffer(int unit, int pipe_num, int reset_buffer)
{
    soc_reg_t reg;
    uint32 rval;
    const soc_reg_t idb_ca_lpbk_ctrl[_TH2_PIPES_PER_DEV]= {
 ...
 *         specified in document "TH2_uA_flexport.pdf", Section 6.6.1
 *  @returns SOC_E_NONE
 */
int
    ...
    int unit, soc_port_schedule_state_t *port_schedule_state)
{
     ...
     * "TH2_uA_flexport.pdf", Section 6.6.1 */
 ...
 *         to port. Follows the sequence specified in document "TH2_uA_flexport.pdf",
 ...
 *         Follows the sequence specified in document "TH2_uA_flexport.pdf", Section
 *         6.6.1.1.
 *         Two main parts:
                                 ...
                                 soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (i = 0; i < port_schedule_state->nport; i++) {
        if (port_schedule_state->resource[i].physical_port == -1) {
            ...
            pm_num = soc_tomahawk2_get_pm_from_phy_pnum(phy_port);
            subp = soc_tomahawk2_get_subp_from_phy_pnum(phy_port);
            SOC_IF_ERROR_RETURN(soc_tomahawk2_idb_obm_poll_buffer_empty(unit,
                                                                        pipe_num,
                                                                        pm_num,
                                                                        subp));
            SOC_IF_ERROR_RETURN(soc_tomahawk2_idb_ca_poll_buffer_empty(unit,
                                                                       pipe_num,
                                                                       pm_num,
                                                                       subp));
        }
