    ...
    for (pipe = 0; pipe < _TH2_PIPES_PER_DEV; pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == TH2_OVSB_TOKEN) {
                ...
                mmu_port = _SOC_TH_TDM_UNUSED_TOKEN; /* _SOC_TH_TDM_IDL2_TOKEN; */
    ...
    for (pipe = 0; pipe < NUM_PIPE(unit); pipe++) {
        ...
        for (slot = 0; slot < length; slot += 2) {
            ...
            if (phy_port == TH2_OVSB_TOKEN) {
                ...
                idb_port = _SOC_TH_TDM_UNUSED_TOKEN; /* _SOC_TH_TDM_IDL2_TOKEN; */
            } else if (phy_port == TH2_NULL_TOKEN) {
                               ...
                               soc_port_schedule_state_t *port_schedule_state)
{
    ...
    for (port=0; port<_TH2_DEV_PORTS_PER_DEV; port++) {
        ...
        soc_tomahawk2_get_hsp_info(unit, port_schedule_state->out_port_map.log_port_speed[port],
