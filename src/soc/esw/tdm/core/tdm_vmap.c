...
**/
void
tdm_vmap_print(tdm_mod_t *_tdm, unsigned short **vmap)
{
...
/**
@name: tdm_vmap_chk_sister_1D
@param:

**/
int
tdm_vmap_chk_sister_1D(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_vmap_chk_sister_xy
@param:

**/
int
tdm_vmap_chk_sister_xy(tdm_mod_t *_tdm, unsigned short **vmap, int idx_x, int idx_y)
{
    ...
    if ( !(idx_x>=0 && idx_x<param_vmap_wid && idx_y>=0 && idx_y<param_vmap_len)){
        ...
        if (port_token>=param_phy_lo &&
            ...
            port_tsc = tdm_vmap_get_port_pm(port_token, _tdm);
            for (i=0; i<param_vmap_wid; i++){
                for (k=1; k<param_space_sister; k++){
                    ...
                    if (port_tmp>=param_phy_lo &&
                        ...
                        tsc_tmp = tdm_vmap_get_port_pm(port_tmp, _tdm);
                        if (tsc_tmp==port_tsc){
                            result = FAIL;
                            break;
                        }
...
**/
int
tdm_vmap_chk_sister_col(tdm_mod_t *_tdm, unsigned short **vmap, int col_idx)
{
    ...
    if ( !(col_idx>=0 && col_idx<param_vmap_wid)){
        ...
        for (row_idx=0; row_idx<param_vmap_len; row_idx++){
            ...
            if (port_token>=param_phy_lo &&
                port_token<=param_phy_hi ){
                chk_result = tdm_vmap_chk_sister_xy(_tdm, vmap, col_idx, row_idx);
                if (chk_result!=PASS){
                    result = FAIL;
                    break;
                }
...
/**
@name: tdm_vmap_chk_sister_2D
@param:

**/
int
tdm_vmap_chk_sister_2D(tdm_mod_t *_tdm, unsigned short **vmap)
{
    int i, result=PASS;
    ...
    if (param_space_sister>0){
        for (i=0; i<param_vmap_wid; i++){
            if (tdm_vmap_chk_sister_col(_tdm, vmap, i)!=PASS){
                result = FAIL;
                break;
            }
        }
...
/**
@name: tdm_vmap_chk_shift_cond
@param:

**/
int
tdm_vmap_chk_shift_cond(int slot, int dir, int cal_len, int *cal_main, tdm_mod_t *_tdm)
{
    ...
    if (slot < cal_len) {
        ...
        idx     = src;
        port    = cal_main[idx];
        port_pm = tdm_vmap_get_port_pm(port, _tdm);
        if (port >= param_phy_lo && port <= param_phy_hi) {
            ...
            for (k=1; k<=param_space_sister; k++) {
                ...
                if (port_k >= param_phy_lo && port_k <= param_phy_hi) {
                    if (tdm_vmap_get_port_pm(port_k, _tdm) == port_pm) {
                        result = FAIL;
                        break;
                    }
                }
...
tdm_vmap_chk_same(tdm_mod_t *_tdm)
{
    ...
    if (param_lr_en==BOOL_TRUE && param_space_same>0){
        for (i=0; i<param_cal_len; i++){
            port_i = param_cal_main[i];
            port_speed = tdm_vmap_get_port_speed(port_i, param_speeds, 
                                                 param_phy_lo, param_phy_hi);
            port_slots = tdm_vmap_get_speed_slots(port_speed);
...
/**
@name: tdm_vmap_get_port_pm
@param:

**/
int
tdm_vmap_get_port_pm(int port_token, tdm_mod_t *_tdm)
{
...
/**
@name: tdm_vmap_get_port_speed
@param:

**/
int
tdm_vmap_get_port_speed(int port_token, enum port_speed_e *speeds, int phy_lo, int phy_hi)
{
...
/**
@name: tdm_vmap_get_speed_slots
@param:

**/
int
tdm_vmap_get_speed_slots(int port_speed)
{
    int port_spd_slot;
...
**/
int
tdm_vmap_switch_col_xy(tdm_mod_t *_tdm, unsigned short **vmap, int col_x, int col_y)
{
...
/**
@name: tdm_vmap_rotate
@param:

**/
int
tdm_vmap_rotate(tdm_mod_t *_tdm, unsigned short **vmap)
{
    ...
    for (i=(param_vmap_wid-1); i>0; i--){
        chk_result = tdm_vmap_chk_sister_col(_tdm, vmap, i);
        if (chk_result!=PASS){
            TDM_PRINT1("TDM: detect sister port violation at column %d\n", i);
            for (j=i-1; j>=0; j--){
                if(tdm_vmap_switch_col_xy(_tdm, vmap, i, j)==PASS){
                    result_i = tdm_vmap_chk_sister_col(_tdm, vmap, i);
                    result_j = tdm_vmap_chk_sister_col(_tdm, vmap, j);
                    if (result_i==PASS && result_j==PASS) {
                        ...
                        tdm_vmap_switch_col_xy(_tdm, vmap, i, j);
...
/**
@name: tdm_vmap_shift_1D
@param:

**/
int
tdm_vmap_shift_1D(tdm_mod_t *_tdm, int port_token)
{
            ...
            if (param_cal_main[i]==port_token){
                ...
                for (k=1; k<param_space_sister; k++){
                    ...
                    if (param_cal_main[j]>=param_phy_lo &&
                        ...
                        tsc_tmp = tdm_vmap_get_port_pm(param_cal_main[j], _tdm);
                        if (tsc_tmp==port_tsc){
                            k_dn = k;
                            break;
                        }
                ...
                if (k_dn>0){
                    for (j=0; j<param_cal_len; j++){
                        if (param_cal_main[j]==port_token){
                            ...
                            tsc_tmp = tdm_vmap_get_port_pm(param_cal_main[k], _tdm);
                            if (tsc_tmp!=port_tsc){
                                x1 = j;
...
tdm_vmap_filter_same(tdm_mod_t *_tdm)
{
            ...
            for (v=1; v<param_space_same; v++) {
                ...
                if (port_j==port_i) {
                    for (m=1; m<(param_space_same-v); m++) {
                        ...
                        y = (x + 1) % param_cal_len;
                        chk_x = tdm_vmap_chk_shift_cond(x, TDM_DIR_DN,
                                    param_cal_len, param_cal_main, _tdm);
                        chk_y = tdm_vmap_chk_shift_cond(y, TDM_DIR_UP,
                                    param_cal_len, param_cal_main, _tdm);
            ...
            for (v=1; v<param_space_same; v++) {
                ...
                if (port_j==port_i) {
                    for (m=1; m<(param_space_same-v); m++) {
                        ...
                        chk_x = tdm_vmap_chk_shift_cond(x, TDM_DIR_UP,
                                    param_cal_len, param_cal_main, _tdm);
                        chk_y = tdm_vmap_chk_shift_cond(y, TDM_DIR_DN,
                                    param_cal_len, param_cal_main, _tdm);
                        if (chk_x == PASS || chk_y == PASS) {
                            port = param_cal_main[y];
                            param_cal_main[y] = param_cal_main[x];
    ...
    result = tdm_vmap_chk_sister_1D(_tdm);
    return (result);
}
...
tdm_vmap_alloc_one_port(int port_token, int slot_req, int slot_left, int cal_len, int vmap_idx, int vmap_wid_max, int vmap_len_max, unsigned short **vmap, int **r_a_arr)
{
    ...
    if (vmap!=NULL  && r_a_arr!=NULL &&
        ...
        for (i=0; i<slot_req; i++){
            pos = tdm_vmap_calc_slot_pos(vmap_idx, i,
...
tdm_vmap_alloc(tdm_mod_t *_tdm)
{
        ...
        for (i=0; i<param_vmap_wid ; i++){
            ...
            if (port_phynum>=param_phy_lo && port_phynum<=param_phy_hi){
                ...
                if (port_state==PORT_STATE__LINERATE    ||
                    ...
                    port_slot_num = tdm_vmap_get_speed_slots(port_speed);
    ...
    if (lr_en==BOOL_TRUE && result==PASS){
        ...
        for (i=0; i<param_vmap_wid; i++){
            ...
            if (port_phynum>=param_phy_lo && port_phynum<=param_phy_hi){
                port_pm = tdm_vmap_get_port_pm(port_phynum, _tdm);
                ...
                if (pm_port_cnt[i]==2){
                    ...
                    if (port_0>=param_phy_lo && port_0<=param_phy_hi &&
                        ...
                        if ((spd_0/1000)!=(spd_1/1000)){
                            ...
                            slot_req = tdm_vmap_get_speed_slots(spd_1);
                ...
                if (port_token>=param_phy_lo && port_token<=param_phy_hi){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            ...
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum_curr += tdm_vmap_get_speed_slots(port_speed);
                ...
                for (i=0; i<(param_num_phy_pm-1); i++){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            port_phynum<=param_phy_hi){
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum += tdm_vmap_get_speed_slots(port_speed);
                        }
                    }
                ...
                for (i=0; i<(param_num_phy_pm-1); i++){
                    ...
                    for (j=0; j<param_num_pm_lanes; j++){
                        ...
                        if (port_phynum>=param_phy_lo && 
                            port_phynum<=param_phy_hi){
                            port_speed = param_speeds[port_phynum];
                            pm_slot_sum_tmp += tdm_vmap_get_speed_slots(port_speed);
                        }
                    }
        ...
        for (i=0; i<param_num_phy_pm && i<pm_cnt; i++){
            ...
            if (port_token==param_token_unused && port_cnt==1){
                ...
                k        = pm_pmap[i][2];
                for (j=0; j<k; j++){
                    result_tmp = tdm_vmap_alloc_one_port(
                                    port_token, 
                                    slot_req, slot_left, 
                ...
                for (j=0; j<port_cnt && j<param_num_pm_lanes; j++){
                    ...
                    if (port_phynum>=param_phy_lo && 
                        port_phynum<=param_phy_hi){
                        port_speed = param_speeds[port_phynum];
                        slot_req += tdm_vmap_get_speed_slots(port_speed);
                    }
                }
        ...
        if (tdm_vmap_chk_sister_2D(_tdm, vmap)!=PASS){
            tdm_vmap_rotate(_tdm, vmap);
        }
...
tdm_core_vmap_alloc(tdm_mod_t *_tdm)
{
    ...
    if (tdm_vmap_chk_sister_1D(_tdm)!=PASS){
                    ...
                    for (v=1; v<param_space_sister; v++){
                        ...
                        if (port_j>=param_phy_lo && port_j<=param_phy_hi){
                            ...
                            if (tsc_i==tsc_j && port_i!=port_j){
                                ...
                                if (spd_i<spd_j){
                                    tdm_vmap_shift_1D(_tdm, port_i);
                                }
                                else {
                                    tdm_vmap_shift_1D(_tdm, port_j);
                                }
                                sister_swap = BOOL_TRUE;
                                break;
                            }
...
/**
@name: tdm_vmap_alloc_mix
@param:

**/
int
tdm_vmap_alloc_mix(tdm_mod_t *_tdm)
{
    int i, j, k, n, port_token, result_tmp, result=PASS;
    int port_phynum, port_state, port_speed, port_existed,
        port_pm, port_pm_existed, 
    ...
    /* allocate memory */
    lr_buff     = (int*) TDM_ALLOC(param_vmap_wid*sizeof(int), 
                                   "tdm_vmap_alloc_lr, lr_buff");
    pm_buff     = (int*) TDM_ALLOC(param_num_phy_pm*sizeof(int), 
                                   "tdm_vmap_alloc_lr, pm_buff");
    pm_port_cnt = (int*) TDM_ALLOC(param_num_phy_pm*sizeof(int), 
                                   "tdm_vmap_alloc_lr, pm_port_cnt");
    pm_slot_req = (int*) TDM_ALLOC(param_num_phy_pm*sizeof(int), 
                                   "tdm_vmap_alloc_lr, pm_slot_req");
    pm_modes    = (int*) TDM_ALLOC(param_num_phy_pm*sizeof(int), 
                                   "tdm_vmap_alloc_lr, pm_modes");
    pm_pmap     = (int**) TDM_ALLOC(param_num_phy_pm*sizeof(int*), 
                                    "tdm_vmap_alloc_lr, pm_pmap_l1");
    for (i=0; i<param_num_phy_pm; i++) {
        pm_pmap[i] = (int*) TDM_ALLOC(param_num_pm_lanes*sizeof(int),
                                      "tdm_vmap_alloc_lr, pm_pmap_l2");
    }
    r_a_arr = (int**) TDM_ALLOC(param_vmap_wid*sizeof(int*), 
                                "tdm_vmap_alloc_lr, r_a_arr_l1");
    for (i=0; i<param_vmap_wid; i++) {
        r_a_arr[i] = (int*) TDM_ALLOC(2*sizeof(int),
                                      "tdm_vmap_alloc_lr, r_a_arr_l2");
    }
    vmap = (unsigned short**) TDM_ALLOC(param_vmap_wid*sizeof(unsigned short*),
                                        "tdm_vmap_alloc_lr, vmap_l1");
    for (i=0; i<param_vmap_wid; i++) {
        vmap[i] = (unsigned short*)
                  TDM_ALLOC(param_vmap_len*sizeof(unsigned short),
                            "tdm_vmap_alloc_lr, vmap_l2");
    }
        ...
        for (i=0; i<param_vmap_wid; i++) {
            ...
            if (port_phynum>=param_phy_lo && port_phynum<=param_phy_hi) {
                ...
                if (port_pm<param_num_phy_pm){
                    ...
                    if (pm_port_cnt[pm_idx]<param_num_pm_lanes) {
                        pm_pmap[pm_idx][pm_port_cnt[pm_idx]] = port_phynum;
                        pm_slot_req[pm_idx] += tdm_vmap_get_speed_slots(port_speed);
                        pm_port_cnt[pm_idx] += 1;
                    } else {
                        result = FAIL;
                        TDM_ERROR4("%s, port %0d, pm_idx %d, pm_lanes %d\n", 
        ...
        if (pm_special_mode_en==BOOL_TRUE) {
            ...
            for (i=0; i<param_num_phy_pm && i<pm_cnt; i++) {
                if (pm_modes[i]==TDM_VMAP_PM_MODE_1_1_1_0 ||
                    ...
                    prt_slot = tdm_vmap_get_speed_slots(spd_1);
                    switch (pm_modes[i]) {
                        case TDM_VMAP_PM_MODE_1_1_1_0:
                        case TDM_VMAP_PM_MODE_2_1_0_0:
        ...
        for (n=0; n<act_pm_num; n++) {
            ...
            for (i=(act_pm_num-1); i>n; i--) {
                if (pm_slot_req[i]>pm_slot_req[i-1]) {
                    port_speed = tdm_vmap_get_port_speed(pm_pmap[i][0],
...
tdm_core_vmap_alloc_mix(tdm_mod_t *_tdm)
{
    ...
    TDM_SEL_CAL(_tdm->_core_data.vars_pkg.cal_id,param_cal_main);
    if (param_lr_en==BOOL_TRUE){
        if(tdm_vmap_alloc_mix(_tdm)!=PASS){
            return (TDM_EXEC_CORE_SIZE+1);
        }
        for (j=0; j<param_vmap_len && j<param_cal_len; j++){
            port_token = param_token_empty;
...
tdm_core_vmap_prealloc(tdm_mod_t *_tdm)
{
    ...
    lr_en      = (lr_en_1g  || lr_en_10g || lr_en_20g  || lr_en_25g || 
                  lr_en_40g || lr_en_50g || lr_en_100g || lr_en_120g);
