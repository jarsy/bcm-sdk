...
/**
@name: tdm_th_chk_get_port_tsc
@param:

Returns the TSC to which the input port belongs
**/
int
tdm_th_chk_get_port_tsc(tdm_mod_t *_tdm, int port)
{
    int i, j, port_tsc;
    int num_ext_ports;
    int num_pm_lanes;
...
/**
@name: tdm_th_chk_get_cal_len
@param:

Returns the length of selected calendar
**/
int
tdm_th_chk_get_cal_len(tdm_mod_t *_tdm, int cal_id)
{
    ...
    switch(cal_id){
        case TH_IDB_PIPE_0_CAL_ID: idx_s=1;  idx_e=32; break;
        case TH_IDB_PIPE_1_CAL_ID: idx_s=33; idx_e=64; break;
        case TH_IDB_PIPE_2_CAL_ID: idx_s=65; idx_e=96; break;
        case TH_IDB_PIPE_3_CAL_ID: idx_s=97; idx_e=128;break;
        case TH_MMU_PIPE_0_CAL_ID: idx_s=1;  idx_e=32; break;
        case TH_MMU_PIPE_1_CAL_ID: idx_s=33; idx_e=64; break;
        case TH_MMU_PIPE_2_CAL_ID: idx_s=65; idx_e=96; break;
        case TH_MMU_PIPE_3_CAL_ID: idx_s=97; idx_e=128;break;
    ...
    if (idx_s!=0 && idx_e!=0 && cal_main!=NULL){
        ...
        if ( (_tdm->_chip_data.soc_pkg.soc_vars.th.mgmt_pm_hg==BOOL_TRUE) &&
             (cal_id==1 || cal_id==2) &&
             ( (cal_main[i]!=TH_NUM_EXT_PORTS && clk_freq>=MIN_HG_FREQ)||
               (cal_main[i]==TH_NUM_EXT_PORTS) ) ){
            higig_mgmt = BOOL_TRUE;
        }
        /* check booting time cal_type */
        if (_tdm->_chip_data.soc_pkg.soc_vars.th.cal_hg_en==BOOL_TRUE){
        ...
        if (higig_port==BOOL_FALSE && 
            ...
            higig_boot==BOOL_FALSE ){
            switch (clk_freq){
                case TH_CLK_950MHZ: cal_len = TH_LEN_950MHZ_EN; break;
                case TH_CLK_850MHZ: cal_len = TH_LEN_850MHZ_EN; break;
                case TH_CLK_765MHZ: cal_len = TH_LEN_765MHZ_EN; break;
                case TH_CLK_672MHZ: cal_len = TH_LEN_672MHZ_EN; break;
                case TH_CLK_645MHZ: cal_len = TH_LEN_645MHZ_EN; break;
                case TH_CLK_545MHZ: cal_len = TH_LEN_545MHZ_EN; break;
                default:  cal_len = 0; break;
            }
        }
        else {
            switch (clk_freq){
                case TH_CLK_950MHZ: cal_len = TH_LEN_950MHZ_HG; break;
                case TH_CLK_850MHZ: cal_len = TH_LEN_850MHZ_HG; break;
                case TH_CLK_765MHZ: cal_len = TH_LEN_765MHZ_HG; break;
                case TH_CLK_672MHZ: cal_len = TH_LEN_672MHZ_HG; break;
                case TH_CLK_645MHZ: cal_len = TH_LEN_645MHZ_HG; break;
                case TH_CLK_545MHZ: cal_len = TH_LEN_545MHZ_HG; break;
                default:  cal_len = 0; break;
            }
        }
    }
...
/**
@name: tdm_th_chk_get_speed_slot_num
@param:

Return 
**/
int 
tdm_th_chk_get_speed_slot_num(int port_speed)
{
...
/**
@name: tdm_th_chk_get_cal_token_num
@param:

Return number of slots of the given token in selected calendar
**/
int 
tdm_th_chk_get_cal_token_num(tdm_mod_t *_tdm, int cal_id, int port_token)
{
    int i, cnt=0;
    int *cal_main=NULL;
    int cal_main_len;
    ...
    TDM_SEL_CAL(cal_id, cal_main);
    cal_main_len = tdm_th_chk_get_cal_len(_tdm, cal_id);
...
/**
@name: tdm_th_chk_is_pipe_ethernet
@param:
...
**/
int 
tdm_th_chk_is_pipe_ethernet(tdm_mod_t *_tdm) 
{
    ...
    if(idx_s>0 && idx_e<TH_NUM_EXT_PORTS){
        for (i=idx_s; i<=idx_e; i++) {
            if (port_speeds[i]!=SPEED_0){
...
/**
@name: tdm_th_chk_struct_freq
@param:

Check frequency validity
**/
int 
tdm_th_chk_struct_freq(tdm_mod_t *_tdm)
{
    int result=PASS;
    int freq;
...
/**
@name: tdm_th_chk_struct_calLength
@param:

Checks calendar length of all MMU/IDB pipes
**/
int 
tdm_th_chk_struct_calLength(tdm_mod_t *_tdm)
{
    ...
    for (cal_id=0; cal_id<8; cal_id++){
        switch(cal_id){
            case TH_IDB_PIPE_0_CAL_ID: pipe_idx = 0; break;
            case TH_IDB_PIPE_1_CAL_ID: pipe_idx = 1; break;
            case TH_IDB_PIPE_2_CAL_ID: pipe_idx = 2; break;
            case TH_IDB_PIPE_3_CAL_ID: pipe_idx = 3; break;
            case TH_MMU_PIPE_0_CAL_ID: pipe_idx = 0; break;
            case TH_MMU_PIPE_1_CAL_ID: pipe_idx = 1; break;
            case TH_MMU_PIPE_2_CAL_ID: pipe_idx = 2; break;
            case TH_MMU_PIPE_3_CAL_ID: pipe_idx = 3; break;
/*             default:
                pipe_idx = 0;
        ...
        len_max = tdm_th_chk_get_cal_len(_tdm, cal_id);
        /* actual calendar length */
        TDM_SEL_CAL(cal_id, cal_main);
        cal_main_len = TH_LR_VBS_LEN;
        for (i=(TH_LR_VBS_LEN-1); i>=0; i--){
            if(cal_main[i]==TH_NUM_EXT_PORTS){
                cal_main_len--;
            }
            else{
                break;
...
/**
@name: tdm_th_chk_struct_speed
@param:

Check if speed type is supported
**/
int 
tdm_th_chk_struct_speed(tdm_mod_t *_tdm)
{
...
/**
@name: tdm_th_chk_struct
@param:

Checks length, frequency, and speed type
**/
int tdm_th_chk_struct(tdm_mod_t *_tdm)
{
    int result_x, result=PASS;
    /* frequency */
    result_x = tdm_th_chk_struct_freq(_tdm);
    result = (result_x!=PASS)? (result_x): (result);
    /* calendar length */
    result_x = tdm_th_chk_struct_calLength(_tdm);
    result = (result_x!=PASS)? (result_x): (result);
    /* speed */
    result_x = tdm_th_chk_struct_speed(_tdm);
    result = (result_x!=PASS)? (result_x): (result);
    ...
    for (port_phynum=port_lo; port_phynum<=port_hi; port_phynum++){
        ...
        if (port_speed>SPEED_0 && 
            (port_state==PORT_STATE__LINERATE    || 
             ...
             port_state==PORT_STATE__MANAGEMENT ) ){
            port_result = PASS;
            port_tsc = tdm_th_chk_get_port_tsc(_tdm, port_phynum);
...
/**
@name: tdm_th_chk_sister
@param:

Checks sister spacing constraint
**/
int 
tdm_th_chk_sister(tdm_mod_t *_tdm)
{
...
/*             default:
        ...
        if (cal_main!=NULL){
            for (i=0; i<cal_main_len; i++){
                port_result = PASS;
                port_i = cal_main[i];
                TH_TOKEN_CHECK(port_i){
                    tsc_i = tdm_th_chk_get_port_tsc(_tdm, port_i);
                    for (j=1; j<sister_min; j++){
                        k = ((i+j)<cal_main_len)?(i+j):(i+j-cal_main_len);
                        port_k = cal_main[k];
                        tsc_k  = tdm_th_chk_get_port_tsc(_tdm,port_k);
                        if (tsc_i==tsc_k){
                            port_result = FAIL;
...
/**
@name: tdm_th_chk_sub_cpu
@param:

Check bandwidth for CPU and/or Management port in MMU/IDB calendars
**/
int 
tdm_th_chk_sub_cpu(tdm_mod_t *_tdm)
{
    int slot_cnt, result=PASS;
    
    /* IDB Pipe 0: CPU port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_0_CAL_ID, TH_CMIC_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
    ...
    /* IDB Pipe 1: Management port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_1_CAL_ID, TH_MGM1_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 1, insufficient MGMT port bandwidth",
                slot_cnt);
    }
    /* IDB Pipe 2: Management port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_2_CAL_ID, TH_MGM2_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 2, insufficient MGMT port bandwidth",
                slot_cnt);
    }
    
    /* MMU Pipe 0: CPU port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_0_CAL_ID, TH_CMIC_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 0, insufficient CPU port bandwidth",
                slot_cnt);
    }
    /* MMU Pipe 1: Management port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_1_CAL_ID, TH_MGM1_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 1, insufficient MGMT port bandwidth",
                slot_cnt);
    }
    /* MMU Pipe 2: Management port */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_2_CAL_ID, TH_MGM2_TOKEN);
    if (slot_cnt<4){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 2, insufficient MGMT port bandwidth",
                slot_cnt);
    }
...
/**
@name: tdm_th_chk_sub_lpbk
@param:

Check bandwidth for Loopback port in MMU/IDB calendars
**/
int 
tdm_th_chk_sub_lpbk(tdm_mod_t *_tdm)
{
    int slot_cnt, result=PASS;
    
    /* IDB Pipe 0 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_0_CAL_ID, TH_LPB0_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 0, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* IDB Pipe 1 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_1_CAL_ID, TH_LPB1_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 1, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* IDB Pipe 2 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_2_CAL_ID, TH_LPB2_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 2, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* IDB Pipe 3 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_IDB_PIPE_3_CAL_ID, TH_LPB3_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "IDB Pipe 3, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    
    /* MMU Pipe 0 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_0_CAL_ID, TH_LPB0_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 0, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* MMU Pipe 1 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_1_CAL_ID, TH_LPB1_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 1, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* MMU Pipe 2 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_2_CAL_ID, TH_LPB2_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 2, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
    /* MMU Pipe 3 */
    slot_cnt = tdm_th_chk_get_cal_token_num(
                    _tdm, TH_MMU_PIPE_3_CAL_ID, TH_LPB3_TOKEN);
    if (slot_cnt<2){
        result = FAIL;
        TDM_ERROR3("%s, %s - %d slots\n",
                "[Port Subscription]",
                "MMU Pipe 3, insufficient LOOPBACK port bandwidth",
                slot_cnt);
    }
...
tdm_th_chk_sub_acc(tdm_mod_t *_tdm)
{
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_0_CAL_ID, TH_LPB0_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_0_CAL_ID, TH_IDL1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_0_CAL_ID, TH_IDL2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_0_CAL_ID, TH_NULL_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_1_CAL_ID, TH_LPB1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_1_CAL_ID, TH_IDL1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_1_CAL_ID, TH_IDL2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_1_CAL_ID, TH_NULL_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_2_CAL_ID, TH_LPB2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_2_CAL_ID, TH_IDL1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_2_CAL_ID, TH_IDL2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_2_CAL_ID, TH_NULL_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_3_CAL_ID, TH_LPB3_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_3_CAL_ID, TH_IDL1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_3_CAL_ID, TH_IDL2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_3_CAL_ID, TH_NULL_TOKEN);
    ovsb_slot_cnt = tdm_th_chk_get_cal_token_num(
                _tdm, TH_IDB_PIPE_3_CAL_ID, TH_OVSB_TOKEN);
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_0_CAL_ID, TH_LPB0_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_0_CAL_ID, TH_NULL_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_0_CAL_ID, TH_IDL2_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_1_CAL_ID, TH_LPB1_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_1_CAL_ID, TH_NULL_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_1_CAL_ID, TH_IDL2_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_2_CAL_ID, TH_LPB2_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_2_CAL_ID, TH_NULL_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_2_CAL_ID, TH_IDL2_TOKEN);
    if (slot_cnt<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
    ...
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_3_CAL_ID, TH_LPB3_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_3_CAL_ID, TH_NULL_TOKEN);
    slot_cnt+= tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_3_CAL_ID, TH_IDL2_TOKEN);
    ovsb_slot_cnt = tdm_th_chk_get_cal_token_num(
                _tdm, TH_MMU_PIPE_3_CAL_ID, TH_OVSB_TOKEN);
    if (slot_cnt<acc_num && (slot_cnt+ovsb_slot_cnt)<acc_num){
        result = FAIL;
        TDM_ERROR4("%s, %s - allocated %d slots, required %d slots\n",
                "[Port Subscription]",
...
/**
@name: tdm_th_chk_sub_lr_pipe
@param:

Check bandwidth for linerate ports for the given pipe
**/
int 
tdm_th_chk_sub_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
    int i, port_speed, port_state, slot_req, slot_cnt=0, idx_s=0, idx_e=0,
        result=PASS;
    int num_40g=0, num_100g=0, case_th_4x100g_3x40g=BOOL_FALSE;
    const char *pipe_str;
    int clk_freq;
    enum port_speed_e *port_speeds;
    enum port_state_e *port_states;
    ...
    if (result==PASS){
        if (clk_freq==TH_CLK_850MHZ){
            ...
            if (num_40g==3 && num_100g==4){
                case_th_4x100g_3x40g = BOOL_TRUE;
            }
        }
    }
    
    /* special case: 4x106HG+3x40G */
    if (result==PASS && case_th_4x100g_3x40g==BOOL_TRUE){
        for (i=idx_s; i<=idx_e; i++){
            port_state = port_states[i-1];
            port_speed = port_speeds[i];
            if (port_state==PORT_STATE__LINERATE    ||
                port_state==PORT_STATE__LINERATE_HG ){
                slot_req = tdm_th_chk_get_speed_slot_num(port_speed);
                slot_cnt = tdm_th_chk_get_cal_token_num(_tdm, cal_id, i);
...
/**
@name: tdm_th_chk_sub_os_pipe
@param:

Check bandwidth for Oversub ports for the given pipe
**/
int 
tdm_th_chk_sub_os_pipe(tdm_mod_t *_tdm, int cal_id)
{
...
/**
@name: tdm_th_chk_sub_lr
@param:

Check bandwidth for linerate ports in MMU/IDB calendars
**/
int 
tdm_th_chk_sub_lr(tdm_mod_t *_tdm)
{
    int pipe_result, result=PASS;
    
    /* IDB pipe 0 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_IDB_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 1 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_IDB_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 2 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_IDB_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 3 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_IDB_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 0 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_MMU_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 1 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_MMU_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 2 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_MMU_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 3 */
    pipe_result = tdm_th_chk_sub_lr_pipe(_tdm, TH_MMU_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
...
/**
@name: tdm_th_chk_sub_os
@param:

Check bandwidth for linerate ports in MMU/IDB calendars
**/
int 
tdm_th_chk_sub_os(tdm_mod_t *_tdm)
{
    int pipe_result, result=PASS;
    
    /* IDB pipe 0 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_IDB_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 1 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_IDB_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 2 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_IDB_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 3 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_IDB_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 0 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_MMU_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 1 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_MMU_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 2 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_MMU_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 3 */
    pipe_result = tdm_th_chk_sub_os_pipe(_tdm, TH_MMU_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
...
**/
int 
tdm_th_chk_sub(tdm_mod_t *_tdm)
{
    int result=PASS;
    
    /* CPU and Management */
    if (tdm_th_chk_sub_cpu(_tdm)!=PASS){
        result = FAIL;
    }
    /* Loopback */
    if (tdm_th_chk_sub_lpbk(_tdm)!=PASS){
        result = FAIL;
    }
    /* ACC */
    if (tdm_th_chk_sub_acc(_tdm)!=PASS){
        result = FAIL;
    }
    /* Linerate ports */
    if(tdm_th_chk_sub_lr(_tdm)!=PASS){
        result = FAIL;
    }
    /* Oversub ports */
    if(tdm_th_chk_sub_os(_tdm)!=PASS){
        result = FAIL;
    }
...
/**
@name: tdm_th_chk_jitter_os_pipe_tdm5
@param:

Checks distribution of oversub tokens for the given pipe
**/
int 
tdm_th_chk_jitter_os_pipe_tdm5(tdm_mod_t *_tdm, int cal_id)
{	
    ...
    switch(cal_id){
        case TH_IDB_PIPE_0_CAL_ID: pipe_str = "IDB Pipe 0"; break;
        case TH_IDB_PIPE_1_CAL_ID: pipe_str = "IDB Pipe 1"; break;
        case TH_IDB_PIPE_2_CAL_ID: pipe_str = "IDB Pipe 2"; break;
        case TH_IDB_PIPE_3_CAL_ID: pipe_str = "IDB Pipe 3"; break;
        case TH_MMU_PIPE_0_CAL_ID: pipe_str = "MMU Pipe 0"; break;
        case TH_MMU_PIPE_1_CAL_ID: pipe_str = "MMU Pipe 1"; break;
        case TH_MMU_PIPE_2_CAL_ID: pipe_str = "MMU Pipe 2"; break;
        case TH_MMU_PIPE_3_CAL_ID: pipe_str = "MMU Pipe 3"; break;
        default:
            pipe_str = "Unknown Pipe";
            result = UNDEF;
            TDM_ERROR2("%s, unrecognized calendar ID %d\n",
    ...
    for (i=1; i<cal_main_len; i++) {
        if (cal_main[i]==TH_OVSB_TOKEN) {
            ...
            while (os_cnt_idx<cal_main_len && 
                   cal_main[++os_cnt_idx]==TH_OVSB_TOKEN) {
                os_clump_size++;
            }
            group_size=os_clump_size;
            while(lr_cnt_idx>0 && 
                  cal_main[--lr_cnt_idx]!=TH_OVSB_TOKEN) {
                group_size++;
            }
            ...
            while (cal_main[i+1]==TH_OVSB_TOKEN) {i++;}
        }
    }
    if(clump_cnt!=0){
...
/**
@name: tdm_th_chk_jitter_os_tdm5
@param:

Checks distribution of oversub tokens for IDB/MMU calendars
**/
int 
tdm_th_chk_jitter_os_tdm5(tdm_mod_t *_tdm)
{
    int pipe_result, result=PASS;
    
    /* IDB pipe 0 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_IDB_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 1 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_IDB_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 2 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_IDB_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 3 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_IDB_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 0 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_MMU_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 1 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_MMU_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 2 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_MMU_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 3 */
    pipe_result = tdm_th_chk_jitter_os_pipe_tdm5(_tdm, TH_MMU_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
...
tdm_th_chk_jitter_lr_pipe_tdm5(tdm_mod_t *_tdm, int cal_id)
{
    ...
    for (i=0; i<TH_NUM_EXT_PORTS; i++) {ports_done[i]=TH_NUM_EXT_PORTS;}
    ...
    for (i=0; i<cal_main_len && i<512; i++) {
        ...
        TH_TOKEN_CHECK(port_i){
            ...
            for (j=0; j<done_idx && j<TH_NUM_EXT_PORTS; j++) {
            ...
            if (has_checked==BOOL_FALSE){
                ...
                if (slot_req>1){
                    ...
                    for (j=0; j<cal_main_len && j<512; j++) {
                        ...
                        if (port_i==port_j){
                            /* max */
                            if (tracker[j]>(space_max+TH_LR_CHK_TRHD)){
                                /* 
                                result = FAIL; */
                            ...
                            /* min */
                            if (tracker[j]<(space_min-TH_LR_CHK_TRHD) && tracker[j]>0){
                                /* 
                                result = FAIL;
...
/**
@name: tdm_th_chk_jitter_lr_tdm5
@param:

Checks distribution of linerate tokens for IDB/MMU calendars
**/
int 
tdm_th_chk_jitter_lr_tdm5(tdm_mod_t *_tdm)
{
    int pipe_result, result=PASS;
    
    /* IDB pipe 0 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_IDB_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 1 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_IDB_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 2 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_IDB_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 3 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_IDB_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 0 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_MMU_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 1 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_MMU_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 2 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_MMU_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 3 */
    pipe_result = tdm_th_chk_jitter_lr_pipe_tdm5(_tdm, TH_MMU_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
...
/**
@name: tdm_th_chk_tdm5
@param: 

**/
int
tdm_th_chk_tdm5( tdm_mod_t *_tdm)
{
    int i, fail[6];
    ...
    if (fail[1]==2) {
        ...
        TDM_SML_BAR
        fail[1] = tdm_th_chk_transcription(_tdm);
    }
    ...
    if (fail[4]==2) {
        ...
        TDM_SML_BAR
        fail[4] = tdm_th_chk_jitter_os_tdm5(_tdm);
    }
    ...
    if (fail[5]==2) {
        ...
        TDM_SML_BAR
        fail[5] = tdm_th_chk_jitter_lr_tdm5(_tdm);
    }
...
/**
@name: tdm_th_chk_jitter_os_pipe
@param:

Checks distribution of oversub tokens for the given pipe
**/
int 
tdm_th_chk_jitter_os_pipe(tdm_mod_t *_tdm, int cal_id)
{
    ...
    /* set parameters */
    param_cal_len     = tdm_th_chk_get_cal_len(_tdm, cal_id);
...
/**
@name: tdm_th_chk_jitter_os
@param:

Checks distribution of oversub tokens for IDB/MMU calendars
**/
int 
tdm_th_chk_jitter_os(tdm_mod_t *_tdm)
{
    ...
    /* IDB pipe 0 - 4 */
    pipe_result[0] = tdm_th_chk_jitter_os_pipe(_tdm, TH_IDB_PIPE_0_CAL_ID);
    pipe_result[1] = tdm_th_chk_jitter_os_pipe(_tdm, TH_IDB_PIPE_1_CAL_ID);
    pipe_result[2] = tdm_th_chk_jitter_os_pipe(_tdm, TH_IDB_PIPE_2_CAL_ID);
    pipe_result[3] = tdm_th_chk_jitter_os_pipe(_tdm, TH_IDB_PIPE_3_CAL_ID);
    /* MMU pipe 0 - 4 */
/*     
    pipe_result[4] = tdm_th_chk_jitter_os_pipe(_tdm, TH_MMU_PIPE_0_CAL_ID);
    pipe_result[5] = tdm_th_chk_jitter_os_pipe(_tdm, TH_MMU_PIPE_1_CAL_ID);
    pipe_result[6] = tdm_th_chk_jitter_os_pipe(_tdm, TH_MMU_PIPE_2_CAL_ID);
    pipe_result[7] = tdm_th_chk_jitter_os_pipe(_tdm, TH_MMU_PIPE_3_CAL_ID); */
...
/**
@name: tdm_th_chk_jitter_lr_pipe
@param:

Checks distribution of linerate slots of the same port for the given pipe
**/
int 
tdm_th_chk_jitter_lr_pipe(tdm_mod_t *_tdm, int cal_id)
{
    int i, j, k, port_i, port_j, port_k, port_speed, 
        slot_req, distance, tracker[512], 
        has_checked, ports_done[TH_NUM_EXT_PORTS], done_idx=0, 
...
/**
@name: tdm_th_chk_jitter_lr
@param:

Checks distribution of linerate tokens for IDB/MMU calendars
**/
int 
tdm_th_chk_jitter_lr(tdm_mod_t *_tdm)
{
    int pipe_result, result=PASS;
    
    /* IDB pipe 0 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_IDB_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 1 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_IDB_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 2 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_IDB_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* IDB pipe 3 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_IDB_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 0 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_MMU_PIPE_0_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 1 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_MMU_PIPE_1_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 2 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_MMU_PIPE_2_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
    /* MMU pipe 3 */
    pipe_result = tdm_th_chk_jitter_lr_pipe(_tdm, TH_MMU_PIPE_3_CAL_ID);
    result = (pipe_result!=PASS)? (pipe_result): (result);
...
tdm_th_chk( tdm_mod_t *_tdm)
{
    ...
    if (fail[4]==2) {
        TDM_PRINT0("\nTDM: Checking Oversub Jitter\n");
        TDM_SML_BAR
        fail[4] = tdm_th_chk_jitter_os(_tdm);
    }
    ...
    if (fail[5]==2) {
        TDM_PRINT0("\nTDM: Checking Linerate Jitter\n");
        TDM_SML_BAR
        fail[5] = tdm_th_chk_jitter_lr(_tdm);
    }
