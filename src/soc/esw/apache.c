...
STATIC _soc_ap_mmu_buffer_config_t soc_ap_buffer_config[_AP_NUM_SKEWS] = {
    ...
    { 34816, 46016, 45372, 13, 72, 0, 0, 0, 0, 72, 0, 0, 0, 0 },
                               ...
                               _soc_ser_sram_info_t *sram_info)
{
    ...
    case L2_ENTRY_ONLY_ECCm:
        ...
        if (index < SOC_APACHE_NUM_ENTRIES_L2_BANK) {
            ...
            entries_per_ram = SOC_APACHE_RAM_OFFSET_L2_BANK;
            offset = index % entries_per_ram;
            base = offset;
            if (index >= SOC_APACHE_NUM_ENTRIES_PER_L2_BANK) {
                base = offset + SOC_APACHE_NUM_ENTRIES_PER_L2_BANK;
            }
        } else {
            sram_info->ram_count = SOC_APACHE_NUM_EL_SHARED;
            entries_per_ram = SOC_APACHE_RAM_OFFSET_L2_SHARED_BANK;
            base_index = SOC_APACHE_NUM_ENTRIES_L2_BANK;
            entries_per_bank = entries_per_ram * sram_info->ram_count;
            offset = base_index + (index % entries_per_ram);
            base = offset;
            for (i = 1; i < SOC_APACHE_NUM_EL_L2; i++) {
    ...
    case L3_ENTRY_ONLY_ECCm:
        if (index < SOC_APACHE_NUM_ENTRIES_L3_BANK) {
            ...
            entries_per_ram = SOC_APACHE_NUM_ENTRIES_L3_BANK;
            base = index;
        } else {
            sram_info->ram_count = SOC_APACHE_NUM_EL_SHARED;
            entries_per_ram = SOC_APACHE_RAM_OFFSET_L3_NARROW;
            base_index = SOC_APACHE_NUM_ENTRIES_L3_BANK;
            entries_per_bank = entries_per_ram * sram_info->ram_count;
            offset = base_index + (index % entries_per_ram);
            base = offset;
            for (i = 1; i < SOC_APACHE_NUM_EL_L3_IPMC; i++) {
                if (index >= (base_index + (entries_per_bank * i))) {
                    base = offset + (entries_per_bank * i);
                }
            }
    ...
    case ING_L3_NEXT_HOPm:
        ...
        break;
    case L3_IPMCm:
        sram_info->ram_count = SOC_APACHE_NUM_EL_L3_IPMC;
...
soc_apache_mem_config(int unit)
{
    ...
    max_l3_entries =  min_l2_entries + uft_size; /* Dedicated (16K) + Shared (256K/128K)*/; /* Dedicated (4K) + Shared (256K)*/
...
soc_apache_num_cosq_init(int unit)
{
    ...
    int hsp_uc_cosq_base, hsp_mc_cosq_base, uc_cosq_per_port;
    int lls_uc_cosq_base, lls_mc_cosq_base;
    int count;
    ...
    hsp_uc_cosq_base = 0;
    hsp_mc_cosq_base = 0;
    lls_uc_cosq_base = hsp_uc_cosq_base  + (10 * count);
    lls_mc_cosq_base = hsp_mc_cosq_base  + (10 * count);
    ...
    for (mmu_port = 0; mmu_port < _APACHE_MMU_PORTS_PER_PIPE; mmu_port++) {
        ...
        if ((hsp_uc_cosq_base > lls_uc_cosq_base) ||
            (hsp_mc_cosq_base > lls_mc_cosq_base) ||
            ((hsp_uc_cosq_base + 10) > _APACHE_NUMBER_OF_UC_QUEUE) ||
            ((lls_uc_cosq_base + 12) > _APACHE_NUMBER_OF_UC_QUEUE) ||
            ((hsp_mc_cosq_base + 10) > _APACHE_NUMBER_OF_MC_QUEUE) ||
            ((lls_mc_cosq_base + 10) > _APACHE_NUMBER_OF_MC_QUEUE)) {
            return SOC_E_INTERNAL;
        }
        ...
        if (SOC_PBMP_MEMBER(si->eq_pbm, port)) { /* VBS (HSP) port */
            ...
            si->port_num_cosq[port] = 10;
            si->port_cosq_base[port] = lls_mc_cosq_base + ((mmu_port - count) *
                                                            si->port_num_cosq[port]);
            uc_cosq_per_port =
                soc_property_port_get(unit, port, spn_LLS_NUM_L2UC, 16);
...
soc_apache_mmu_ports_assign(int unit)
{
    ...
    soc_info_t *si;
    soc_pbmp_t pbmp;
    int num_phy_ports = APACHE_PORTS_PER_PIPE;
    ...
    for (phy_port = 1; phy_port <= num_phy_port; phy_port++) {
        ...
        port = si->port_p2l_mapping[phy_port];
        if ((port == -1) || ((phy_port - 1) % APACHE_PORTS_PER_TSC)) {
            continue;
        }

        if ((IS_OVERSUB_PORT(unit, port)) &&
            (si->port_speed_max[port] >= 50000)) {
            si->port_p2m_mapping[phy_port] = mmu_port;
            si->port_m2p_mapping[mmu_port] = phy_port;
            mmu_port++;
        }
    ...
    /* Assign MMU port for all remaining OS ports with speed >= 50G */
    for (phy_port = 1; phy_port <= num_phy_port; phy_port++) {
        if (si->port_p2m_mapping[phy_port] != -1) {
            continue;
        }
    ...
    /* Assign MMU port for all LR ports with speed >= 100G */
    for (phy_port = 1; phy_port <= num_phy_port; phy_port++) {
        if (si->port_p2m_mapping[phy_port] != -1) {
            continue;
        }
    ...
    /* Assign MMU port for all LR ports with speed >= 40G */
    for (phy_port = 1; phy_port <= num_phy_port; phy_port++) {
        if (si->port_p2m_mapping[phy_port] != -1) {
            continue;
        }
    ...
    /* Assign MMU port for 1st phy port in PM4x25 */
...
/*
 * Function: _soc_apache_tsc_disable
 *
 * Purpose:
 ...
 * */
STATIC int
_soc_apache_tsc_disable(int unit)
{
    soc_info_t *si = &SOC_INFO(unit);
    uint32 tsc_enable = 0;
...
soc_apache_chip_reset(int unit)
{
    ...
    if (soc_property_get(unit, "disable_unused_port_blocks", TRUE)) {
        SOC_IF_ERROR_RETURN(_soc_apache_tsc_disable(unit));
    }
...
static const soc_reg_t pgw_hsp_config_regs[APACHE_TSCS_PER_PGW] = {
...
static const soc_reg_t pgw_hsp_config_fields[APACHE_TSCS_PER_PGW] = {
...
soc_apache_tdm_calc(int unit, soc_apache_tdm_t *apache_tdm)
{
    ...
    switch (si->frequency) {
        default:
            ...
            apache_chip_pkg.clk_freq = (si->frequency == 841) ? 840 : si->frequency;
            break;
    }
                      ...
                      soc_apache_tdm_t *apache_tdm)
{
    ...
    soc_port_t pgw_hsp_ports[2][APACHE_TSCS_PER_PGW] = {{0}, {0}};
    ...
    for (pgw = 0; pgw < APACHE_PGWS_PER_DEV; pgw++) {
        ...
        for (slot= 0; slot < APACHE_PGW_TDM_OVS_SIZE; slot++) {
            ...
            if (SOC_INFO(unit).port_speed_max[port] >= 40000) {
                ...
                hsp_values[1] = (SOC_INFO(unit).port_speed_max[port] / 10000);
        ...
        while (pgw_hsp_config_index[pgw] < APACHE_TSCS_PER_PGW) {
            hsp_reg = pgw_hsp_config_regs[pgw_hsp_config_index[pgw]];
            hsp_fields[0] = pgw_hsp_config_fields[pgw_hsp_config_index[pgw]];
            hsp_values[0] = 0x0;
            hsp_fields[1] = ENTRIES_IN_CALf;
...
STATIC int
_soc_apache_ledup_init(int unit)
{
    ...
    pm_id = ((phy_port - 1) / APACHE_PORTS_PER_TSC) % APACHE_TSCS_PER_PGW;
        ...
        if (pm_id < APACHE_NUM_XLP_PER_PGW) { /* Eagle Cores */
            if (si->port_speed_max[port] <= 21000) {  /* 0-21G */
                ...
                edb_values[0] = (linerate) ? (asf ? 9 : 0) : (asf ? 15 : 9);
                edb_values[1] = (linerate) ? (asf ? 0 : 1) : 0;
                edb_values[2] = 0; /* linerate/oversub and cut-thru or no cut-thru */
...
_soc_apache_misc_init (int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        SOC_IF_ERROR_RETURN(_soc_apache_ledup_init(unit));
    }
...
STATIC soc_error_t
_soc_apache_ser_mem_info_get (int unit, soc_mem_t mem, void** ser_mem_info)
{
    int i, j;
    _soc_mem_ser_en_info_t *ser_en_info = NULL;
...
soc_apache_ser_tcam_test (int unit, _soc_ser_test_t test_type) {
    ...
    for (i = 0; tcams[i].mem != INVALIDm; i++) {
        ...
        _soc_apache_perform_ser_test(unit, &test_data, test_type,
