...
soc_th_alpm_bucket_sw_dump(int u)
{
    ...
    for (i = 0; i < SOC_TH_ALPM_BUCKET_COUNT(u); i += step_count) {
        ...
        if (used) {
            lpm_mem =
                soc_alpm_cmn_bkt_view_get(u, SOC_TH_ALPM_AIDX(u, i, 0, 0, 0));
            LOG_CLI((
    ...
    if (soc_alpm_cmn_mode_get(u) == SOC_ALPM_MODE_PARALLEL &&
        ...
        for (i = 0; i < SOC_TH_ALPM_BUCKET_COUNT(u); i += step_count) {
            SHR_BITTEST_RANGE(
                SOC_TH_ALPM_GLB_BUCKET_BMAP(u), i, step_count, used);
            if (used) {
...
_soc_th_alpm_urpf_tcam_info_update(int u, void *entry)
{
    ...
    if (!SOC_MEM_OPT_F32_GET(u, L3_DEFIPm, entry, MODE0f)) {
...
#define _ALPM_MODE_TCAM_ALPM 3
    ...
    if ((mode = soc_property_get(u, spn_L3_ALPM_ENABLE, 0))) {
        ...
        l3_alpm_sw_prefix_lookup[u] =
            ALPM_CTRL(u).aux_enable ? soc_property_get(u, "l3_alpm_sw_prefix_lookup", 1) : 1;
