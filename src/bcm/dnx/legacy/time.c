...
/****************************************************************************/
static int
_bcm_dnx_time_capture_get (int unit, bcm_time_if_t id, bcm_time_capture_t *time);
static int
...
_bcm_dnx_time_interface_install(int unit, bcm_time_if_t intf_id)
{
    ...
    if (intf->flags & BCM_TIME_INPUT) {
        ...
        if (intf->flags & BCM_TIME_HEARTBEAT) {
            ...
            if (!(intf->flags & BCM_TIME_OFFSET)) {
                BCM_IF_ERROR_RETURN(
                    _bcm_dnx_time_capture_get(unit, intf_id, TIME_CAPTURE(unit, intf_id)));
            }
        }
    }
    if (SOC_IS_JERICHO(unit)) {
...
bcm_dnx_time_capture_get (int unit, bcm_time_if_t id, bcm_time_capture_t *time)
{
    ...
    TIME_LOCK(unit);
    rv = _bcm_dnx_time_capture_get(unit, id, TIME_CAPTURE(unit, id));
    if (BCM_FAILURE(rv)) {
        TIME_UNLOCK(unit);
        return (rv);
    }
...
bcm_dnx_time_heartbeat_enable_set (int unit, bcm_time_if_t id, int enable)
{
    ...
    if (enable) {
        ...
        if (!soc->time_call_ref_count) {
            soc->soc_time_callout = _bcm_dnx_time_hw_interrupt_dflt;
        }
                                 ...
                                 void *user_data)
{
    ...
    soc->soc_time_callout = _bcm_dnx_time_hw_interrupt;
