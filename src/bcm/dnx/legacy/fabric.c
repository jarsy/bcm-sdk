...
#define DNX_TDM_DIRECT_ROUTING_DEFAULT_PROFILE (0)
...
static int 
_bcm_dnx_fabric_vsq_category_set(int unit,
                                   int core_id,
                                   bcm_fabric_control_t control_type,
                                   int queue_id_in,
                                   int *queue_id_out);
static int 
_bcm_dnx_fabric_vsq_category_get(int unit,
                                   int core_id,
                                   bcm_fabric_control_t control_type,
                                   int *queue_id);
static int
_bcm_dnx_fabric_queue_range_set(int unit,
                                  bcm_fabric_control_t control_type,
                                  int queue_id);
static int
_bcm_dnx_fabric_multicast_scheduler_mode_get(int unit,
                                               int core_id,
                                               int* mode);
static int
_bcm_dnx_fabric_multicast_queue_range_get(int unit,
                                            int core_id,
                                            bcm_fabric_control_t control_type,
                                            int* queue_id);
static int
_bcm_dnx_fabric_unicast_queue_range_get(int unit,
                                          int core_id,
                                          bcm_fabric_control_t control_type,
                                          int* queue_id);
static int
_bcm_dnx_fabric_ingress_shaper_queue_range_get(int unit,
                                                 int core_id,
                                                 bcm_fabric_control_t control_type,
                                                 int* queue_id);
static int
_bcm_dnx_fabric_egress_queue_range_set(int unit,
                                         int core_id,
                                          bcm_fabric_control_t type,
                                          int queue_id);
static int
_bcm_dnx_fabric_egress_queue_range_get(int unit,
                                         int core_id,
                                         bcm_fabric_control_t control_type,
                                         int* queue_id);
static int
_bcm_dnx_fabric_multicast_scheduler_mode_set(int unit,
                                               int mode);
static int
_bcm_dnx_fabric_credit_watchdog_range_set(int unit,
                      bcm_fabric_control_t control_type,
                      int queue_id,
                      int common_message_time);
static int
_bcm_dnx_fabric_credit_watchdog_range_get(int unit,
                                            bcm_fabric_control_t control_type,
                                            int *queue_id);
static int
_bcm_dnx_fabric_tdm_range_size_set(int unit,
                                     int minimum_size,
                                     int maximum_size); 

static int
_bcm_dnx_fabric_tdm_range_size_get(int unit,
                                     int *minimum_size,
                                     int *maximum_size);

static int
_bcm_dnx_fabric_tdm_from_action_to_editing_type(int unit,
                                                  int is_ingress,
                                                  DNX_TMC_TDM_FTMH_INFO *ftmh_info,
                                                  bcm_fabric_tdm_editing_type_t *editing_type);

static int
_bcm_dnx_fabric_tdm_to_action_from_editing_type(int unit,
                                                  int is_ingress,
                                                  DNX_TMC_TDM_FTMH_INFO *ftmh_info,
                                                  bcm_fabric_tdm_editing_type_t *editing_type);
int
...
{   
    ...
    if (!SOC_WARM_BOOT(unit)) {  /* if cold boot, initially turn on credit watchdog */
        BCMDNX_IF_ERR_EXIT(DNX_GET_CREDIT_WATCHDOG_MODE(unit, credit_watchdog_mode));
        BCMDNX_IF_ERR_EXIT(bcm_dnx_fabric_control_set(unit, bcmFabricWatchdogQueueEnable, DNX_GET_CREDIT_WATCHDOG_MODE_BASE(credit_watchdog_mode)));
    }
    ...
    if (SOC_IS_JERICHO(unit)) {
        ...
        PBMP_SFI_ITER(unit, link) {
            ...
            if (repeater_link_enable) {
                BCMDNX_IF_ERR_EXIT(bcm_dnx_fabric_link_control_set(unit, link, bcmFabricLinkRepeaterEnable, 1));
            }
        }
    }

    /* Initalize TDM fabric direct routing */
    if (SOC_DNX_CONFIG(unit)->tdm.is_bypass && !SOC_WARM_BOOT(unit)) {
      DNX_TMC_TDM_DIRECT_ROUTING_INFO direct_routing;
      int template_init_id;
      uint32 dnx_sand_rc;
      DNX_TMC_TDM_DIRECT_ROUTING_INFO_clear(&direct_routing);
      ...
      template_init_id = DNX_TDM_DIRECT_ROUTING_DEFAULT_PROFILE;

      dnx_sand_rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_direct_routing_set,(unit, template_init_id, &direct_routing, enable_rpt_reachable)));
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);

      /* Init template management */
      rc = _bcm_dnx_am_template_fabric_tdm_link_ptr_init(unit,template_init_id,&direct_routing);
                          ...
                          bcm_gport_t *parent_port)
{
    ...
    if (BCM_GPORT_IS_LOCAL(child_port)) {
        ...
        interface = port + BCM_DNX_PORT_INTERFACE_START;
        ...
        if (SOC_DNX_IS_MODID_AND_BASE_MODID_ON_SAME_FAP(unit, modid, base_modid)){
            ...
            rv = dnx_port_sw_db_tm_to_local_port_get(unit, core, tm_port, (soc_port_t *)&port);
            BCMDNX_IF_ERR_EXIT(rv);
      ...
      interface = child_port + BCM_DNX_PORT_INTERFACE_START;
      BCM_GPORT_LOCAL_SET(*parent_port, interface);
    ...
    if (port >= BCM_DNX_PORT_INTERFACE_END) {
        *parent_port = -1;
...
int
bcm_dnx_fabric_control_set(int unit,
                             bcm_fabric_control_t type,
                             int arg)
{
    bcm_error_t rc = BCM_E_NONE;    
    ...
    int maximum_size = 0;
    int queue_id; 
    JER2_ARAD_MGMT_OCB_MC_RANGE range;
    uint32 range_ndx;
    ...
    switch (type) {
        ...
        case bcmFabricMulticastQueueMax:
            rc = _bcm_dnx_fabric_queue_range_set(unit, type, arg);
            break;
        ...
        case bcmFabricCellSizeFixed:
            /* Cell fixed size */
            rc = _bcm_dnx_fabric_tdm_range_size_get(unit,&minimum_size,&maximum_size);
            BCMDNX_IF_ERR_EXIT(rc);
            
            minimum_size = maximum_size = arg;            
            rc = _bcm_dnx_fabric_tdm_range_size_set(unit,minimum_size,maximum_size);
        ...
        case bcmFabricCreditSize:
            ...
            /* "credit_size" .                                                                */
            rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mgmt_credit_worth_set,(unit, arg)));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricCreditSizeRemoteDefault:   /* Default remote credit size */
            if (!(SOC_IS_ARADPLUS(unit) || SOC_IS_JERICHO(unit))) {
                ...
                DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_mgmt_credit_worth_remote_set, (unit, arg)));
            }
            break;
        case bcmFabricVsqCategory:
            rc = bcm_dnx_cosq_fmq_vsq_category_mode_set(unit, SOC_CORE_ALL, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            /* Set all VOQs to category 2 when mode is None. */
            if (arg == bcmFabricVsqCatagoryModeNone) {
                queue_id = DNX_DEVICE_DNX_COSQ_MAX_QUEUE(unit);
                rc = _bcm_dnx_fabric_vsq_category_set(unit, BCM_CORE_ALL, bcmFabricQueueMax, queue_id, &queue_id);
                BCMDNX_IF_ERR_EXIT(rc);
            }                       

            break;
        case bcmFabricMulticastSchedulerMode:
            rc = _bcm_dnx_fabric_multicast_scheduler_mode_set(unit,arg);
            break;
        case bcmFabricIsolate:
            dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mgmt_all_ctrl_cells_enable_set,(unit, arg ? FALSE : TRUE)));
        ...
        case bcmFabricRecycleQueueMax:
            ...
            rc = _bcm_dnx_fabric_egress_queue_range_set(unit, BCM_CORE_ALL, type, arg);
            break;
        ...
        case bcmFabricOcbMulticastId2Max:
            ...
            dnx_sand_rv = jer2_arad_mgmt_ocb_mc_range_get(
                            unit,
                            range_ndx,
                            &range
                          );
            ...
            dnx_sand_rv = jer2_arad_mgmt_ocb_mc_range_set(
                            unit,
                            range_ndx,
                            &range
                          );
        ...
        case bcmFabricWatchdogQueueEnable: /* Queue Watchdog Functionality Enable/Disable setting (True/False) */
            ...
            rc = _bcm_dnx_fabric_credit_watchdog_range_set(unit, type, arg, -1);
            break;

       case bcmFabricRCIControlSource:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_rci_enable_set,(unit, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

    case bcmFabricRCIIncrementValue:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_config_set, (unit, DNX_TMC_FABRIC_RCI_CONFIG_TYPE_INCREMENT_VALUE, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

       case bcmFabricGciLeakyBucketEnable:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_enable_set, (unit, DNX_TMC_FABRIC_GCI_TYPE_LEAKY_BUCKET, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

       case bcmFabricGciBackoffEnable:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_enable_set, (unit, DNX_TMC_FABRIC_GCI_TYPE_RANDOM_BACKOFF, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;
       ...
       case bcmFabricMinimalLinksToDestination:
           if (SOC_IS_ARADPLUS(unit) || SOC_IS_JERICHO(unit)) {
               ...
               rc = MBCM_DNX_SOC_DRIVER_CALL(unit,mbcm_dnx_fabric_minimal_links_to_dest_set, (unit, BCM_MODID_ALL, arg));
           } else {
        ...
        case bcmFabricControlCellsEnable:
              dnx_sand_rv= MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mgmt_all_ctrl_cells_enable_set, (unit,arg));
               DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;
        case bcmFabricForceTdmBypassTrafficToFabric:
            DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_force_tdm_bypass_traffic_to_fabric_set, (unit, arg)));
            break;
        default:
...
int
bcm_dnx_fabric_control_get(int unit,
                             bcm_fabric_control_t type,
                             int *arg)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 dnx_sand_rv = DNX_SAND_OK;
    ...
    switch (type) {
        case bcmFabricQueueMin:                                    
        case bcmFabricQueueMax:
            rc = _bcm_dnx_fabric_unicast_queue_range_get(unit, BCM_CORE_ALL, type,arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricShaperQueueMin:                        
        case bcmFabricShaperQueueMax:
            rc = _bcm_dnx_fabric_ingress_shaper_queue_range_get(unit, BCM_CORE_ALL, type, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricMulticastQueueMin:
        case bcmFabricMulticastQueueMax:
            rc = _bcm_dnx_fabric_multicast_queue_range_get(unit, BCM_CORE_ALL, type, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricRecycleQueueMin:
        case bcmFabricRecycleQueueMax:
            /* Set queue range for egress queues */
            rc = _bcm_dnx_fabric_egress_queue_range_get(unit, BCM_CORE_ALL, type, arg);        
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricTrafficManagementCosMode:
            break;
        ...
        case bcmFabricCreditSize:
            rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mgmt_credit_worth_get,(unit, (uint32 *)arg)));
            DNX_BCM_SAND_IF_ERR_EXIT(rc);
            break;
        case bcmFabricCreditSizeRemoteDefault:   /* Default remote credit size */
            if (SOC_IS_ARADPLUS(unit) || SOC_IS_JERICHO(unit)) {
                uint32 credit_size_remote = -1;                                  
                DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_mgmt_credit_worth_remote_get, (unit, &credit_size_remote)));
        ...
        case bcmFabricVsqCategory:
            {
                ...
                rc = bcm_dnx_cosq_fmq_vsq_category_mode_get(unit, SOC_CORE_ALL, &vsq_category_mode);
                BCMDNX_IF_ERR_EXIT(rc);
                *arg = vsq_category_mode;
            }
            break;
        case bcmFabricMulticastSchedulerMode:
            rc = _bcm_dnx_fabric_multicast_scheduler_mode_get(unit, BCM_CORE_ALL, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricIsolate:
            rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mgmt_all_ctrl_cells_enable_get,(unit, &ret_bool)));
            DNX_BCM_SAND_IF_ERR_EXIT(rc);
        ...
        case bcmFabricWatchdogQueueEnable: /* Queue Watchdog Functionality Enable/Disable setting (True/False) */
            /* Set queue range and enable/disable for credit watchdog */
            rc = _bcm_dnx_fabric_credit_watchdog_range_get(unit, type, arg);
            break;

       case bcmFabricRCIControlSource:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_rci_enable_get,(unit, (soc_dnxc_fabric_control_source_t*) arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

       case bcmFabricRCIIncrementValue:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_config_get, (unit, DNX_TMC_FABRIC_RCI_CONFIG_TYPE_INCREMENT_VALUE, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

       case bcmFabricGciLeakyBucketEnable:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_enable_get, (unit, DNX_TMC_FABRIC_GCI_TYPE_LEAKY_BUCKET, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;

       case bcmFabricGciBackoffEnable:
           dnx_sand_rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_enable_get, (unit, DNX_TMC_FABRIC_GCI_TYPE_RANDOM_BACKOFF, arg));
           DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
           break;
       ...
       case bcmFabricMinimalLinksToDestination:
          if (SOC_IS_ARADPLUS(unit) || SOC_IS_JERICHO(unit))
           {
               dnx_sand_rv= MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_minimal_links_to_dest_get, (unit, SOC_MODID_ALL, arg));
               DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
        ...
        case bcmFabricForceTdmBypassTrafficToFabric:
            DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_force_tdm_bypass_traffic_to_fabric_get, (unit, arg)));
            break;
                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
  ...
  BCMDNX_NULL_CHECK(profile_array);
  {
     ...
     * ---------------
     */
    if ((SOC_DNX_DEFS_GET(unit, nof_cores) < 2) && !SOC_IS_QAX(unit))
    {
      /*
    ...
    if (profile_count > (SOC_DNX_DEFS_GET(unit, nof_fabric_links) + 1) * SOC_DNX_DEFS_GET(unit, nof_rci_levels)  || profile_count < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("profile_count invalid")));
    }
    /*
     * First, verify all profiles are valid so as not to call 'mbcm_dnx_sch_device_rate_entry_core_set'
     * unless all input is OK.
     */
    for(ii = 0 ; ii < profile_count ; ii++)
    {
      if (profile_array[ii].rci > SOC_DNX_DEFS_GET(unit, nof_rci_levels) - 1) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("RCI level on profile is out of range")));
      }
      if (profile_array[ii].num_links > SOC_DNX_DEFS_GET(unit, nof_fabric_links)) {
  ...
  for (ii = 0 ; ii < profile_count ; ii++)
  {
    ...
    dnx_sand_rv =
      (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_sch_device_rate_entry_core_set,
                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
  ...
  BCMDNX_NULL_CHECK(profile_array);    
  {
    ...
    /*
     * First, verify all profiles are valid so as not to call 'mbcm_dnx_sch_device_rate_entry_core_get'
     * unless all input is OK.
     */
    for(ii = 0 ; ii < profile_count ; ii++)
    {
  ...
  {  
    dnx_sand_rv =
      (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_sch_device_rate_entry_core_get,
                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
    ...
    BCMDNX_NULL_CHECK(profile_array);
    {
       ...
       * core is operational.
       */
      if (SOC_IS_JERICHO(unit) && SOC_DNX_DEFS_GET(unit, nof_cores) > 1) {
        if (SOC_DNX_CONFIG(unit)->jer2_arad->init.fabric.fabric_links_to_core_mapping_mode !=
          SOC_DNX_FABRIC_LINKS_TO_CORE_MAP_SHARED) {
      ...
      /*
       * First, verify all profiles are valid so as not to call 'mbcm_dnx_sch_device_rate_entry_set'
       * unless all input is OK.
       */
      for(ii = 0 ; ii < profile_count ; ii++)
      {
    ...
    for (ii = 0 ; ii < profile_count ; ii++)
    {
      ...
      max_mbps = profile_array[ii].max_kbps / 1000 ;
      dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_sch_device_rate_entry_set,(unit, profile_array[ii].rci, profile_array[ii].num_links, max_mbps)));
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    }
                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
    ...
    BCMDNX_NULL_CHECK(profile_array);    
    {
      ...
      /*
       * First, verify all profiles are valid so as not to call 'mbcm_dnx_sch_device_rate_entry_get'
       * unless all input is OK.
       */
      for(ii = 0 ; ii < profile_count ; ii++)
      {
    ...
    for(ii = 0 ; ii < profile_count ; ii++)
    {  
      dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_sch_device_rate_entry_get,(unit, profile_array[ii].rci, profile_array[ii].num_links, (uint32*)&(profile_array[ii].max_kbps))));
      /* Convert lower layers units (Mega bps) to units required by API (Kilo bps) */
      profile_array[ii].max_kbps *= 1000 ;
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    }
 ...
 */
int
bcm_dnx_fabric_enhance_ports_get(int unit, bcm_core_t core, int *nbr_ports, int *ports)
{
    bcm_error_t           rc = BCM_E_NONE;
    DNX_SAND_U32_RANGE    queue_range;
    DNX_TMC_MULT_FABRIC_INFO  fabric_info;
    int                   be_class;
    ...
    DNXC_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mult_fabric_enhanced_get,(unit, core, &queue_range)));

    /* The MBCM_DNX_DRIVER_CALL initializing the required varible */ 
    /* coverity[uninit_use:FALSE] */
    if ( (queue_range.start == DNX_DEVICE_COSQ_FMQ_NON_ENHANCED_QID_MIN) && (queue_range.end == DNX_DEVICE_COSQ_FMQ_NON_ENHANCED_QID_MAX) ) {
        BCM_EXIT;
    }

    /* determine associated ports */
    DNX_TMC_MULT_FABRIC_INFO_clear(&fabric_info);
    ...
    for (be_class = 0; be_class < DNX_TMC_MULT_FABRIC_NOF_BE_CLASSES; be_class++) {
                                  ...
                                  int queue_id)
{
    ...
    /* Validate queue id */
    if (queue_id > DNX_DEVICE_DNX_COSQ_MAX_QUEUE(unit) || queue_id < 0) {
    ...
    rc = _bcm_dnx_cosq_queue_range_get(unit, BCM_CORE_ALL, range_type, &queue_range);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    rc = bcm_dnx_am_cosq_queue_region_config_get(unit, core, queue_region_config, queue_config_type);
    BCMDNX_IF_ERR_EXIT(rc);

    /*Default values*/
    current_voq_1k_start = 0;
    current_voq_1k_end = DNX_DEVICE_COSQ_CONFIG_QUEUE_REGIONS(unit);
    for (region = 0; region < (DNX_DEVICE_COSQ_CONFIG_QUEUE_REGIONS(unit)); region++) {
    ...
    rc = _bcm_dnx_cosq_queue_range_set(
           unit, 
           BCM_CORE_ALL, 
           range_type, 
           (is_min ? BCM_DNX_COSQ_QUEUE_RANGE_SET_MIN : BCM_DNX_COSQ_QUEUE_RANGE_SET_MAX),
           FALSE,
           &queue_range);
    BCMDNX_IF_ERR_EXIT(rc);
...
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 tm_ports[DNX_FABRIC_ENHANCE_NOF_PORTS];
    ...
    BCM_DNX_CORES_ITER(_SHR_CORE_ALL, core) {
        ...
        if (mode == 0) {
            if (fabric_info.credits_via_sch == DNX_SAND_TRUE) {
                ...
                rc = bcm_dnx_cosq_fmq_hr_deallocate(unit, core, DNX_FABRIC_ENHANCE_NOF_PORTS, tm_ports);
                BCMDNX_IF_ERR_EXIT(rc);
    
                /* Disable ports configuration */
                fabric_info.guaranteed.gr_sch_port.mcast_class_port_id = 0;
                fabric_info.guaranteed.gr_sch_port.multicast_class_valid = DNX_SAND_FALSE;
                for (index = 0; index < DNX_FABRIC_ENHANCE_NOF_PORTS-1; index++) {
                    ...
                    fabric_info.best_effort.be_sch_port[index].be_sch_port.multicast_class_valid = DNX_SAND_FALSE;
                }     
            ...
            if (fabric_info.credits_via_sch == DNX_SAND_FALSE) {
                ...
                rc = bcm_dnx_cosq_fmq_hr_allocate(unit, core, DNX_FABRIC_ENHANCE_NOF_PORTS, tm_ports);
                BCMDNX_IF_ERR_EXIT(rc);
        
                /* Set ports configruation */
                fabric_info.credits_via_sch = 1;
                fabric_info.guaranteed.gr_sch_port.mcast_class_port_id = tm_ports[0];
                fabric_info.guaranteed.gr_sch_port.multicast_class_valid = DNX_SAND_TRUE;
                for (index = 0; index < DNX_FABRIC_ENHANCE_NOF_PORTS-1; index++) {
                    fabric_info.best_effort.be_sch_port[index].be_sch_port.mcast_class_port_id = tm_ports[index+1];
                    fabric_info.best_effort.be_sch_port[index].be_sch_port.multicast_class_valid = DNX_SAND_TRUE;
                }
...
int
bcm_dnx_fabric_multicast_scheduler_mode_get(int unit, int core_id, int *mode)
{
    int rv;
    BCMDNX_INIT_FUNC_DEFS;
    rv = _bcm_dnx_fabric_multicast_scheduler_mode_get(unit, core_id, mode);
    BCMDNX_IF_ERR_EXIT(rv);
exit:
    BCMDNX_FUNC_RETURN;
}
                                               ...
                                               int* mode)
{
    ...
    rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_mult_fabric_credit_source_get,(unit, core_id, &fabric_info)));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int *link_partner_count)
{
    ...
    uint32 dnx_sand_rv;
    int nof_links = 0;
    DNX_TMC_FABRIC_LINKS_CON_STAT_INFO_ARR connectivity_map;
    ...
    DNX_TMC_FABRIC_LINKS_CON_STAT_INFO_ARR_clear(&connectivity_map);
    
    dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_nof_links_get,(unit, &nof_links)));
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);

    dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_topology_status_connectivity_get,(unit, SOC_DNX_DEFS_GET(unit, first_fabric_link_id), nof_links - 1 + SOC_DNX_DEFS_GET(unit, first_fabric_link_id), &connectivity_map)));
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    ...
    *link_partner_count = 0;
    ...
    PBMP_SFI_ITER(unit, port_i)
    {
        ...
        if(connectivity_map.link_info[i].far_unit != DNX_TMC_DEVICE_ID_IRRELEVANT
    ...
    bcm_fabric_link_connectivity_t *link_partner_info)
{
    ...
    dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_topology_status_connectivity_get,(unit, link_index, link_index, &connectivity_map)));
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    
    if(connectivity_map.link_info[link_index].far_unit != DNX_TMC_DEVICE_ID_IRRELEVANT
...
int 
bcm_dnx_jer2_arad_fabric_link_status_get(
    int unit, 
    bcm_port_t link_id, 
    uint32 *link_status, 
    uint32 *errored_token_count)
{
    ...
    link_i = SOC_DNX_FABRIC_PORT_TO_LINK(unit, link_id);
    
    BCMDNX_NULL_CHECK(link_status);
    BCMDNX_NULL_CHECK(errored_token_count);

    soc_rv = soc_dnx_fabric_link_status_get(unit, link_i, &link_status_tmp, errored_token_count);
    ...
    uint32 *errored_token_count)
{
    ...
    rc = bcm_dnx_jer2_arad_fabric_link_status_get(unit, link_id, link_status, errored_token_count);
    BCMDNX_IF_ERR_EXIT(rc);
                                 ...
                                 bcm_fabric_tdm_editing_t *editing)
{
    ...
    DNX_TMC_TDM_FTMH_INFO_clear(&ftmh_info);

    /* Check if TDM is supported */
    if (!SOC_IS_DNX_TDM_OPTIMIZE(unit) && !SOC_IS_DNX_TDM_STANDARD(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("TDM is not configured")));
    }
    
    /* Validate Gport, retreive port id */
    rc = _bcm_dnx_gport_to_phy_port(unit, gport, _BCM_DNX_GPORT_TO_PHY_OP_LOCAL_IS_MANDATORY, &gport_info); 
    BCMDNX_IF_ERR_EXIT(rc);    
    ...
    /* Check CRC is added*/
    if(!editing->add_packet_crc && !SOC_DNX_CONFIG(unit)->tm.is_petrab_in_system) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("crc must be added to a system with no dnx")));
    }

    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_local_to_tm_port_get, (unit, gport_info.local_port, &p_fap_port, &core)));

    /* Retrieve port TDM configuration */
    dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_ftmh_get,(unit, core, p_fap_port,&ftmh_info)));
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);

    if (ftmh_info.action_ing == DNX_TMC_TDM_NOF_ING_ACTIONS) {
        ftmh_info.action_ing = DNX_TMC_TDM_ING_ACTION_NO_CHANGE;
    }

    if (ftmh_info.action_eg == DNX_TMC_TDM_NOF_EG_ACTIONS) {
        ftmh_info.action_eg = DNX_TMC_TDM_EG_ACTION_NO_CHANGE;
    }
    ...
    if (SOC_DNX_CONFIG(unit)->tdm.is_bypass) { 
        ...
        /* Add packet CRC should be done when sending to JER2_ARAD or Soc_dnx-B packet mode */
        /* Soc_dnx-B ingress can not add CRC in case of Bypass mode */
        dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_port_packet_crc_set,(unit, gport_info.local_port, add_crc, configure_ingress, configure_egress)));
        DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);        
    }
    ...
    if (editing->flags & BCM_FABRIC_TDM_EDITING_INGRESS) {
        ...
        /* 1. Set action ingress */
        is_ingress = 1;
        rc = _bcm_dnx_fabric_tdm_to_action_from_editing_type(unit,is_ingress,&ftmh_info,&(editing->type));
        BCMDNX_IF_ERR_EXIT(rc);

        /* 2. Set destination only if tdm type is append or custom external */
        rc = bcm_dnx_stk_my_modid_get(unit, &my_mod);
        BCMDNX_IF_ERR_EXIT(rc);
        ...
        if (editing->type == bcmFabricTdmEditingAppend || editing->type == bcmFabricTdmEditingCustomExternal) {
            ...
            if (editing->flags & BCM_FABRIC_TDM_EDITING_MULTICAST) {
                ftmh_info.is_mc = DNX_SAND_TRUE;
                /* Multicast destination */
                if (SOC_IS_DNX_TDM_OPTIMIZE(unit)) {
                ...
                ftmh_info.is_mc = DNX_SAND_FALSE;

                if (BCM_GPORT_IS_SYSTEM_PORT(editing->destination_port)) {
                    rc = _bcm_dnx_gport_to_phy_port(unit, editing->destination_port, _BCM_DNX_GPORT_TO_PHY_OP_RETRIVE_SYS_PORT, &gport_info);
                    BCMDNX_IF_ERR_EXIT(rc);              
                    ...
                    if (tm_port == -1) {
                        rv = dnx_port_sw_db_local_to_tm_port_get(unit, local_port, &tm_port, &dest_core);
                        if(BCM_FAILURE(rv)) {   
                            /* In case TM port is not configured assume 1:1 mapping */
                            tm_port = local_port;
                        }
                        modid = SOC_DNX_CORE_TO_MODID(my_mod, dest_core);
                    }
                }
        ...
        if (editing->type == bcmFabricTdmEditingAppend && SOC_IS_DNX_TDM_STANDARD(unit)) {
            if (editing->user_field_count > SOC_DNX_CONFIG(unit)->tdm.max_user_define_bits) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("invalid max user define bits %d. max allowed %d"),
                                               editing->user_field_count,SOC_DNX_CONFIG(unit)->tdm.max_user_define_bits));
            }
            ...
            if (editing->user_field_count != 0) {
                if (editing->flags & BCM_FABRIC_TDM_EDITING_UNICAST) {  
                    ...
                    for (index = 0; index < nof_bytes; index++) {
                        ...
                        if (index < 4) {
                          dnx_sand_rv = dnx_sand_bitstream_set_any_field(
                            &tmp,                
                            index*8,
                            (index+1)*8-1,
    ...
    dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_ftmh_set,(unit, core, p_fap_port,&ftmh_info)));
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
                                 ...
                                 bcm_fabric_tdm_editing_t *editing)
{
    ...
    if (SOC_DNX_CONFIG(unit)->tdm.is_bypass) { 
      ...
      /* validate parameter */
      dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_port_packet_crc_get,(unit, gport_info.local_port, &is_ingress_enabled, &is_egress_enabled)));
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    ...
    if (editing->flags & BCM_FABRIC_TDM_EDITING_INGRESS) {
        is_ingress = 1;
        rc = _bcm_dnx_fabric_tdm_from_action_to_editing_type(unit,is_ingress,&ftmh_info,&(editing->type));
        BCMDNX_IF_ERR_EXIT(rc);

        /* Get destination only if ftmh is appended */
        if (ftmh_info.action_ing != DNX_TMC_TDM_ING_ACTION_NO_CHANGE) {
            if (ftmh_info.is_mc) {
                editing->flags |= BCM_FABRIC_TDM_EDITING_MULTICAST;
                /* Multicast destination */
                if (SOC_IS_DNX_TDM_OPTIMIZE(unit)) {
                    _BCM_MULTICAST_GROUP_SET(editing->multicast_id,_BCM_DNX_MULTICAST_TM_TYPE,ftmh_info.ftmh.opt_mc.mc_id);
                } else {
                    _BCM_MULTICAST_GROUP_SET(editing->multicast_id,_BCM_DNX_MULTICAST_TM_TYPE,ftmh_info.ftmh.standard_mc.mc_id);
                }            
            } else {
                /* Unicast destination */
                ...
                if (SOC_IS_DNX_TDM_OPTIMIZE(unit)) {                                        
                    ...
                    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_tm_to_local_port_get,(unit, core, ftmh_info.ftmh.opt_uc.dest_if, &local_port)));

                    BCM_GPORT_MODPORT_SET(editing->destination_port,modid,local_port);
                } else {
                      
                      rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_tm_to_local_port_get,(unit, core, ftmh_info.ftmh.standard_uc.dest_fap_port, &local_port));
                      BCMDNX_IF_ERR_EXIT(rc);
        ...
        if (ftmh_info.action_ing == DNX_TMC_TDM_ING_ACTION_ADD && SOC_IS_DNX_TDM_STANDARD(unit)) {
            /* 3. User define settings */            
            /* Soc_dnx-B can handle only 32 bits */
            editing->user_field_count = SOC_DNX_CONFIG(unit)->tdm.max_user_define_bits;
            ...
            if (editing->flags & BCM_FABRIC_TDM_EDITING_UNICAST) {  
                ...
                for (index = 0; index <  nof_bytes; index++) {
                    tmp = 0;
                    if (index < 4) {
                        dnx_sand_rv = dnx_sand_bitstream_get_any_field(
                            &(ftmh_info.ftmh.standard_uc.user_def),
                            index*8,
                            (index+1)*8-1,
                            &tmp
                                  ...
                                  bcm_fabric_tdm_direct_routing_t *routing_info)
{
    ...
    DNX_TMC_TDM_DIRECT_ROUTING_INFO direct_routing_information, cleaned_direct_routing_information;
    _bcm_dnx_gport_info_t gport_info;
    ...
    DNX_TMC_TDM_DIRECT_ROUTING_INFO_clear(&direct_routing_information);
    DNX_TMC_TDM_DIRECT_ROUTING_INFO_clear(&cleaned_direct_routing_information);

    /* Verify routing information */
    BCMDNX_NULL_CHECK(routing_info);

    if (routing_info->links_count < 0 || routing_info->links_count > SOC_DNX_DEFS_GET(unit, nof_fabric_links)) {
    ...
    {
      link_i = SOC_DNX_FABRIC_PORT_TO_LINK(unit, routing_info->links[index]);
      ...
      dnx_sand_rc = dnx_sand_bitstream_set_bit(&(direct_routing_information.link_bitmap.arr[0]),link_i);
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
    }
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      rc = _bcm_dnx_am_template_fabric_tdm_link_ptr_exchange(unit,port_i,&direct_routing_information,&old_link_ptr, &is_last, &new_link_ptr,&is_allocated);
      BCMDNX_IF_ERR_EXIT(rc);

      /* Commit Changes in HW according to routing information */
      if (is_allocated) {
          dnx_sand_rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_direct_routing_set,(unit, new_link_ptr, &direct_routing_information, enable_rpt_reachable)));
          DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
      }

      /* Set new mapping */
      dnx_sand_rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_direct_routing_profile_map_set,(unit, port_i, new_link_ptr)));
      DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);

      /* last mapped, remove routing information */
      if (is_last && (new_link_ptr != old_link_ptr)) {
          dnx_sand_rc = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_direct_routing_set,(unit, old_link_ptr, &cleaned_direct_routing_information, enable_rpt_reachable)));
          DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
      }
    }
exit:
                                  ...
                                  bcm_fabric_tdm_direct_routing_t *routing_info)
{
    ...
    bcm_port_t link_i;
    bcm_port_t port_i;
    DNX_TMC_TDM_DIRECT_ROUTING_INFO direct_routing_information;
    _bcm_dnx_gport_info_t gport_info;
    ...
    BCM_PBMP_ITER(gport_info.pbmp_local_ports, port_i) {
      ...
      rc = _bcm_dnx_am_template_fabric_tdm_link_ptr_data_get(unit, port_i ,&direct_routing_information);
      BCMDNX_IF_ERR_EXIT(rc);

      /* Fill routing information, all valid links */
      routing_info->links_count = 0;
      PBMP_SFI_ITER(unit, port_i) {
        link_i = SOC_DNX_FABRIC_PORT_TO_LINK(unit, port_i);
        if (dnx_sand_bitstream_test_bit(direct_routing_information.link_bitmap.arr,link_i)) {
...
_bcm_dnx_fabric_tdm_range_size_set(int unit,
                                     ...
                                     int maximum_size)
{
    uint32 dnx_sand_rv = 0;    

    BCMDNX_INIT_FUNC_DEFS;
    BCM_DNX_UNIT_CHECK(unit);

    if (SOC_IS_DNX_TDM_STANDARD(unit) || !SOC_DNX_CONFIG(unit)->tdm.is_fixed_opt_cell_size) { /* Standard or not fixed Optimize size (not Petra-B in system) */       
        DNX_SAND_U32_RANGE size_range;

        /* TDM standard validate size range */
        
        if (minimum_size < SOC_DNX_CONFIG(unit)->tdm.min_cell_size || maximum_size > SOC_DNX_CONFIG(unit)->tdm.max_cell_size
        ...
        dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_stand_size_range_set,(unit, &size_range)));
        DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
    } else if (SOC_IS_DNX_TDM_OPTIMIZE(unit)) {
        /* TDM optimize */

        /* In Soc_dnx-B cell size is fixed */
        
        if (SOC_DNX_CONFIG(unit)->tdm.is_fixed_opt_cell_size) {
            uint32 cell_size;
            ...
            if (cell_size < SOC_DNX_CONFIG(unit)->tdm.min_cell_size ||  maximum_size > SOC_DNX_CONFIG(unit)->tdm.max_cell_size
                || minimum_size != maximum_size) {
                LOG_ERROR(BSL_LS_BCM_FABRIC,
                          (BSL_META_U(unit,
            ...
            dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_opt_size_set,(unit, cell_size)));
            DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
        } else {       
            /* Not TDM Mode */
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("not supported TDM"))); 
                                     ...
                                     int *maximum_size)
{
    ...
    if (SOC_IS_DNX_TDM_STANDARD(unit) || !SOC_DNX_CONFIG(unit)->tdm.is_fixed_opt_cell_size) { /* Standard or Not fixed Optimize size (not Petra-B in system) */  
        ...
        dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_stand_size_range_get,(unit, &size_range)));
        DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);        
        ...
        *maximum_size = size_range.end;
        ...
        if (SOC_DNX_CONFIG(unit)->tdm.is_fixed_opt_cell_size) {
            uint32 cell_size = 0;
            dnx_sand_rv = (MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_tdm_opt_size_get,(unit, &cell_size)));
            DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv); 
   ...
   if (is_ingress) {
       ...
       switch(*editing_type) {
            case bcmFabricTdmEditingAppend:
                ftmh_info->action_ing = DNX_TMC_TDM_ING_ACTION_ADD;
                break;
            case bcmFabricTdmEditingNoChange:
                ftmh_info->action_ing = DNX_TMC_TDM_ING_ACTION_NO_CHANGE;
                break;
           case bcmFabricTdmEditingCustomExternal:
                ftmh_info->action_ing = DNX_TMC_TDM_ING_ACTION_CUSTOMER_EMBED;
                break;
           default:
       ...
       switch(*editing_type) {
            case bcmFabricTdmEditingRemove:
                ftmh_info->action_eg = DNX_TMC_TDM_EG_ACTION_REMOVE;
                break;
            case bcmFabricTdmEditingNoChange:
                ftmh_info->action_eg = DNX_TMC_TDM_EG_ACTION_NO_CHANGE;
                break;
            case bcmFabricTdmEditingCustomExternal:
                ftmh_info->action_eg = DNX_TMC_TDM_EG_ACTION_CUSTOMER_EXTRACT;
                break;
           default:
                                   ...
                                   int *queue_id_out)
{
    ...
    *queue_id = queue_id_in;
    ...
    /* validate parameters */        
    if (*queue_id < 0 || *queue_id > DNX_DEVICE_DNX_COSQ_MAX_QUEUE(unit)) {
    ...
    case 0:
        ...
        break;
    case 1:
        queue_range.start = DNX_SAND_MIN(info.vsq_ctgry1_end, info.vsq_ctgry0_end + 1);
        queue_range.end = info.vsq_ctgry1_end; 
        break;           
    case 2:
        queue_range.start = DNX_SAND_MIN(info.vsq_ctgry2_end, info.vsq_ctgry1_end + 1);
        queue_range.end = info.vsq_ctgry2_end; 
        break;
    case 3:
        queue_range.start = DNX_SAND_MIN(DNX_DEVICE_DNX_COSQ_MAX_QUEUE(unit), info.vsq_ctgry2_end + 1);
    ...
    rc = _bcm_cosq_dnx_vsq_category_set(unit, core_id, range_type, &queue_range);
    BCMDNX_IF_ERR_EXIT(rc); 
                                   ...
                                   int *queue_id)
{
    ...
    rc = _bcm_cosq_dnx_vsq_category_get(unit, core_id, range_type, &queue_range);
    BCMDNX_IF_ERR_EXIT(rc);
                                          ...
                                          int* queue_id)
{
    ...
    rc = _bcm_dnx_fabric_vsq_category_get(unit, core_id, control_type, queue_id);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
                                          ...
                                          int queue_id)
{
    ...
    rc = _bcm_dnx_fabric_vsq_category_set(unit, core_id, type, queue_id, &queue_id);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
 ...
 * its settings
 */
int bcm_dnx_fabric_config_discard_set(
    int unit, 
    bcm_fabric_config_discard_t *discard)
{

    DNX_TMC_ITM_SYS_RED_EG_INFO info, exact_info;

    BCMDNX_INIT_FUNC_DEFS;

    DNX_TMC_ITM_SYS_RED_EG_INFO_clear(&info);
    DNX_TMC_ITM_SYS_RED_EG_INFO_clear(&exact_info);
    ...
    DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_itm_sys_red_eg_set,(unit, &info, &exact_info)));
...
int bcm_dnx_fabric_config_discard_get(
    int unit, 
    bcm_fabric_config_discard_t *discard)
{
    DNX_TMC_ITM_SYS_RED_EG_INFO info;

    BCMDNX_INIT_FUNC_DEFS;

    sal_memset(&info, 0x0, sizeof(info));

    DNX_BCM_SAND_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_itm_sys_red_eg_get,(unit, &info)));
...
static int
_bcm_dnx_fabric_link_control_set_llfc_control_source(int unit, bcm_port_t link, int arg)
{
    uint32 dnx_sand_rv, mask[1];
    bcm_port_t link_i;
    int nof_pipes;
    BCMDNX_INIT_FUNC_DEFS;

    nof_pipes = SOC_DNX_FABRIC_PIPES_CONFIG(unit).nof_pipes;
    link_i = SOC_DNX_FABRIC_PORT_TO_LINK(unit, link);
    ...
    /*set registers*/
    dnx_sand_rv = jer2_arad_fabric_links_llf_control_source_set(unit, link_i, arg);
    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rv);
...
_bcm_dnx_fabric_link_repeater_enable_set(int unit, bcm_port_t port, int enable)
{
    int rv;
    uint32 burst, pps;
    ...
    if (enable)
    {
         ...
         */
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_port_rate_egress_pps_get,(unit, port, &pps, &burst)));

        rv = bcm_dnx_port_speed_get(unit, port, &speed);
        BCMDNX_IF_ERR_EXIT(rv);

        if (pps == 0)
        {
            rv = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_link_repeater_enable_set,(unit, port, 1, SOC_DNX_DEFS_GET(unit, repeater_default_empty_cell_size)));
            BCMDNX_IF_ERR_EXIT(rv); 
        } else {
            ...
            rv = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_link_repeater_enable_set,(unit, port, 1, empty_cell_size));
            BCMDNX_IF_ERR_EXIT(rv); 
        }

    } else {
        /*Disable repeater link and reset empty cell size*/
        rv = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_repeater_enable_set,(unit, port, 0, SOC_DNX_DEFS_GET(unit, repeater_none_empty_cell_size)));
        BCMDNX_IF_ERR_EXIT(rv);           
    }
...
static int
_bcm_dnx_fabric_link_repeater_enable_get(int unit, bcm_port_t port, int *enable)
{
    int rv;
    int empty_cell_size;
    BCMDNX_INIT_FUNC_DEFS;

    rv = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_link_repeater_enable_get,(unit, port, enable, &empty_cell_size));
    BCMDNX_IF_ERR_EXIT(rv); 
                                ...
                                int arg)
{
    ...
    /*General validation*/
    /*DNX_LINK_INPUT_CHECK(unit, link);*/
    ...
    switch(type)
    {
        case bcmFabricLinkCellInterleavingEnable:
            if (SOC_IS_JERICHO(unit)) {
                ...
                soc_rv = jer2_arad_fabric_links_cell_interleaving_set(unit, link_i, arg ? 1 : 0); 
                BCMDNX_IF_ERR_EXIT(soc_rv);
            }
            break;
        case bcmFabricLLFControlSource:
            rc = _bcm_dnx_fabric_link_control_set_llfc_control_source(unit, link, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;       
        case bcmFabricLinkIsolate:
            soc_rv =  jer2_arad_fabric_links_isolate_set(unit, link_i, arg ? soc_dnxc_isolation_status_isolated : soc_dnxc_isolation_status_active);
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break;
        case bcmFabricLinkTxTrafficDisable:
            soc_rv = MBCM_DNX_SOC_DRIVER_CALL(unit, mbcm_dnx_fabric_link_tx_traffic_disable_set, (unit, link_i, arg));
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break;
        case bcmFabricLinkRepeaterEnable:
            soc_rv = _bcm_dnx_fabric_link_repeater_enable_set(unit, link, arg);
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break; 
        default:
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported Type %d"),type)); 
                                ...
                                int *arg)
{
    ...
    {
        case bcmFabricLinkCellInterleavingEnable:
            soc_rv = jer2_arad_fabric_links_cell_interleaving_get(unit, link_i, arg); 
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break;
        case bcmFabricLLFControlSource:
            soc_rv = jer2_arad_fabric_links_llf_control_source_get(unit, link_i, (soc_dnxc_fabric_control_source_t*)arg);
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break;   
        case bcmFabricLinkIsolate:
            soc_rv = jer2_arad_fabric_links_isolate_get(unit, link_i, (soc_dnxc_isolation_status_t*)arg); 
            BCMDNX_IF_ERR_EXIT(soc_rv);
            break;
        case bcmFabricLinkTxTrafficDisable:
           soc_rv = MBCM_DNX_SOC_DRIVER_CALL(unit, mbcm_dnx_fabric_link_tx_traffic_disable_get, (unit, link_i, arg));
           BCMDNX_IF_ERR_EXIT(soc_rv);
           break;
        case bcmFabricLinkRepeaterEnable:
           soc_rv = _bcm_dnx_fabric_link_repeater_enable_get(unit, link, arg);
           BCMDNX_IF_ERR_EXIT(soc_rv);
           break;
        default:
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported Type %d"),type)); 
...
{    
    ...
    soc_rv = soc_dnx_fabric_reachability_status_get(unit, moduleid, links_max, links_array, links_count);  
    BCMDNX_IF_ERR_EXIT(soc_rv);
exit:     
    BCMDNX_FUNC_RETURN; 
}
                      ...
                      int common_message_time) /* if >= 0, this is the new common message time, for the common message time mode */
{
    ...
    BCM_DNX_UNIT_CHECK(unit);
    flags |= (control_type == bcmFabricWatchdogQueueEnable) ? BCM_DNX_COSQ_CREDIT_WATHDOG_USE_ENABLE: 0;
    flags |= (control_type == bcmFabricWatchdogQueueMin) ? BCM_DNX_COSQ_CREDIT_WATHDOG_USE_MIN: 0;
    flags |= (control_type == bcmFabricWatchdogQueueMax) ? BCM_DNX_COSQ_CREDIT_WATHDOG_USE_MAX: 0;
    ...
    DNX_BCM_SAND_IF_ERR_EXIT(_bcm_dnx_cosq_credit_watchdog_range_set(unit, SOC_CORE_ALL, flags, &range, common_message_time));
                      ...
                      int* queue_id)
{
    ...
    sal_memset(&range, 0x0, sizeof(bcm_cosq_range_t));
    DNX_BCM_SAND_IF_ERR_EXIT(_bcm_dnx_cosq_credit_watchdog_range_get(unit, BCM_CORE_ALL, &range));
    if (control_type == bcmFabricWatchdogQueueEnable) {
        /* The MBCM_DNX_DRIVER_CALL initializing the required varible */ 
        /* coverity[uninit_use:FALSE] */
    ...
    int fabric_priority)
{
    ...
    if (color < 0 || color >= DNX_DEVICE_COSQ_ING_NOF_DP) {
    ...
    if ((flags & BCM_FABRIC_QUEUE_PRIORITY_HIGH_ONLY) == 0 &&  (flags & BCM_FABRIC_QUEUE_PRIORITY_LOW_ONLY) == 0) {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_set,(unit, ingress_pri, color, flags_high, fabric_priority)));

        flags_low = flags | BCM_FABRIC_QUEUE_PRIORITY_LOW_ONLY;
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_set,(unit, ingress_pri, color, flags_low, fabric_priority)));
    } else {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_set,(unit, ingress_pri, color, flags, fabric_priority)));
    }
    ...
    int *fabric_priority)
{
    ...
    if ((flags & BCM_FABRIC_QUEUE_PRIORITY_HIGH_ONLY) == 0 &&  (flags & BCM_FABRIC_QUEUE_PRIORITY_LOW_ONLY) == 0) {
        ...
        flags_high = flags | BCM_FABRIC_QUEUE_PRIORITY_HIGH_ONLY;
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_get,(unit, ingress_pri, color, flags_high, &fabric_priority_low)));

        flags_low = flags | BCM_FABRIC_QUEUE_PRIORITY_LOW_ONLY;
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_get,(unit, ingress_pri, color, flags_low, &fabric_priority_high)));
        ...
        *fabric_priority = fabric_priority_high;
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_priority_get,(unit, ingress_pri, color, flags, fabric_priority)));
    }
        ...
        for(i = 0 ; i<count; i++)
        {
            switch(type[i])
            {
                ...
                case bcmFabricLinkTxGciLvl1FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_0, value[i]));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
                case bcmFabricLinkTxGciLvl2FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_1, value[i]));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
                case bcmFabricLinkTxGciLvl3FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_2, value[i]));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
               ...
               case bcmFabricLinkGciLeakyBucket1Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_1_CONGESTION_TH, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket2Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_2_CONGESTION_TH, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket3Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_3_CONGESTION_TH, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket4Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_4_CONGESTION_TH, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;

               case bcmFabricLinkGciLeakyBucket1Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_1_FULL, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket2Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_2_FULL, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket3Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_3_FULL, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket4Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_set, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_4_FULL, value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
                ...
                case bcmFabricLinkRxFifoLLFC:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_llfc_threshold_set,(unit, value[i]));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
               ...
               case bcmFabricLinkRciFC:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_config_set, (unit, DNX_TMC_FABRIC_RCI_CONFIG_TYPE_LOCAL_RX_TH,value[i]));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;


               default:
                    BCMDNX_IF_ERR_EXIT(bcm_dnx_fabric_link_thresholds_pipe_set(unit,fifo_type,socDnxFabricPipeAll,0,1,(&type[i]),(&value[i])));
                    break;
    ...
    if (fifo_type == -1)
    {
        ...
        for(i = 0 ; i<count; i++)
        {
            switch(type[i])
            {
                 ...
                 */
                case bcmFabricLinkTxGciLvl1FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_0, &(value[i])));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
                case bcmFabricLinkTxGciLvl2FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_1, &(value[i])));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
                case bcmFabricLinkTxGciLvl3FC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_BACKOFF_TH_2, &(value[i])));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
               /*
                * GCI Leaky bucket thresholds
                */
               case bcmFabricLinkGciLeakyBucket1Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_1_CONGESTION_TH, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket2Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_2_CONGESTION_TH, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket3Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_3_CONGESTION_TH, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket4Congestion:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_4_CONGESTION_TH, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;

               case bcmFabricLinkGciLeakyBucket1Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_1_FULL, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket2Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_2_FULL, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket3Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_3_FULL, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
               case bcmFabricLinkGciLeakyBucket4Full:
                   dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_gci_config_get, (unit, DNX_TMC_FABRIC_GCI_CONFIG_TYPE_LB_4_FULL, &(value[i])));
                   DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                   break;
                 ...
                 */
                case bcmFabricLinkRxFifoLLFC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit,mbcm_dnx_fabric_llfc_threshold_get,(unit, &(value[i])));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;
                 ...
                 */
                case bcmFabricLinkRciFC:
                    dnx_sand_rc = MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_config_get, (unit, DNX_TMC_FABRIC_RCI_CONFIG_TYPE_LOCAL_RX_TH, &(value[i])));
                    DNX_BCM_SAND_IF_ERR_EXIT(dnx_sand_rc);
                    break;

            default:
                    BCMDNX_IF_ERR_EXIT(bcm_dnx_fabric_link_thresholds_pipe_get(unit,fifo_type,socDnxFabricPipeAll,0,1,(&type[i]),(&value[i])));
                    break;
            }
         }
...
int bcm_dnx_fabric_link_thresholds_pipe_set(
    ...
    int *value)
{
    soc_dnx_fabric_pipe_t soc_pipe;
    int i;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    if (pipe == bcmFabricPipe0) {
        soc_pipe = socDnxFabricPipe0;
    } else if (pipe == bcmFabricPipe1) {
        soc_pipe = socDnxFabricPipe1;
    } else if (pipe == bcmFabricPipe2) {
        soc_pipe = socDnxFabricPipe2;
    } else {
        soc_pipe = socDnxFabricPipeAll;
    }

    for (i = 0; i < count; ++i) {
        switch (type[i]) {
        case bcmFabricLinkRxFifoLLFC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_set, 
                                                (unit, soc_pipe, socDnxFabricLinkRxFifoLLFC, value[i])));
                break;
        case bcmFabricLinkRxRciLvl1FC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_set, 
                                                (unit, soc_pipe, socDnxFabricLinkRxRciLvl1FC, value[i])));
                break;
        case bcmFabricLinkRxRciLvl2FC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_set, 
                                                (unit, soc_pipe, socDnxFabricLinkRxRciLvl2FC, value[i])));
                break;
        case bcmFabricLinkRxRciLvl3FC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_set, 
                                                (unit, soc_pipe, socDnxFabricLinkRxRciLvl3FC, value[i])));
                break;
        default:
    ...
    int *value)
{
    ...
    for (i = 0; i < count; ++i) {
        switch (type[i]) {
        case bcmFabricLinkRxFifoLLFC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_get, 
                                                (unit, soc_pipe, socDnxFabricLinkRxFifoLLFC, value+i)));
                break;
        case bcmFabricLinkRxRciLvl1FC:
                BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_thresholds_pipe_get, 
    ...
    uint32 *data_out_size)
{
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnx_fabric_sr_cell_receive(unit, flags, (data_out_max_size * 4), &data_out_size_in_bytes, data_out));
    ...
    uint32 *data_in)
{
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnx_fabric_sr_cell_send(unit, flags, &soc_route, sizeof(uint32) * data_in_size, data_in));
    ...
    bcm_fabric_pcp_mode_config_t  	*pcp_config)
{
    ...
    if (fabric->fabric_pcp_enable) {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_pcp_dest_mode_config_set, 
    ...
    bcm_fabric_pcp_mode_config_t  	*pcp_config)
{
    ...
    if (fabric->fabric_pcp_enable) {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_pcp_dest_mode_config_get, 
      ...
      int num_of_links)
{
    ...
    if (flags & BCM_FABRIC_DESTINATION_LINK_MIN_ALL_REACHABLE) {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_minimal_links_all_reachable_set, 
                                                 (unit, num_of_links)));
    } else {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_minimal_links_to_dest_set, 
      ...
      int* num_of_links)
{
    ...
    if (flags & BCM_FABRIC_DESTINATION_LINK_MIN_ALL_REACHABLE) {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_minimal_links_all_reachable_get, 
                                                 (unit, num_of_links)));
    } else {
        if ((module_id != BCM_MODID_ALL) && !(BCM_FABRIC_MODID_IS_VALID(unit, module_id))) {
            BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM,(_BSL_BCM_MSG("module id is invalid")));
        }
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_minimal_links_to_dest_get, 
                                                 (unit, module_id, num_of_links)));
    }
exit:
    BCMDNX_FUNC_RETURN;
    ...
    bcm_port_t *links_array)
{
    ...
    /* validate mesh mode */
    if (!is_mesh) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("bcm_dnx_fabric_modid_group_get is only valid in mesh mode"))); 
    }
    ...
    nof_fabric_links = SOC_DNX_DEFS_GET(unit, nof_fabric_links);
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_local_dest_id_verify, 
                                                 (unit, local_dest_id)));
    if (links_count == 0) { 
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_topology_unset, 
                                                     (unit, local_dest_id)));
    } else {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_topology_set, 
    ...
    bcm_port_t *links_array)
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_link_topology_get, 
    ...
    bcm_module_t *destid_array)
{
    ...
    max_mesh_replication = ((SOC_DNX_DEFS_GET(unit, fabric_max_dest_mesh_mc_mode)) + (SOC_DNX_DEFS_GET(unit, nof_cores))); /*ejacobi - changing the behavior for Petra, JER2_ARAD and Ardon.*/
    ...
    my_core1_fap_id = my_core0_fap_id +
                      ((SOC_DNX_DEFS_GET(unit, nof_cores) == 2) ? 1 : 0); /*1 for 2 cores and 0 for single core*/
    for (i = 0; i < destid_count; ++i) {
        if ((!SOC_DNX_FABRIC_MODID_IS_GROUP(destid_array[i]))
    ...
    if (is_mesh_mc) {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_multicast_set,
                                                 (unit, mc_id, destid_count, destid_array)));
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("bcm_dnx_fabric_multicast_set is supported in MESH_MC mode only")));
    }
    ...
    bcm_module_t *destid_array)
{
    ...
    if (is_mesh_mc) {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_multicast_get, 
                                                 (unit, mc_id, destid_count_max, destid_count, destid_array)));
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("bcm_dnx_fabric_multicast_get is supported in MESH_MC mode only")));
    }
    ...
    bcm_module_t *modid_array)
{
    ...
    if (modid_count > SOC_DNX_DEFS_GET(unit, max_nof_fap_id_mapped_to_dest_limit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("only %d fAPs can be mapped to a single destination"), SOC_DNX_DEFS_GET(unit, max_nof_fap_id_mapped_to_dest_limit)));
    }
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_modid_group_set, 
    ...
    int *modid_count)
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_modid_group_get, 
...
{
    int i;
    soc_dnx_fabric_rci_config_t soc_rci_config;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    for (i = 0; i < BCM_FABRIC_NUM_OF_RCI_LEVELS; ++i) {
        ...
        soc_rci_config.soc_dnx_fabric_rci_core_level_thresholds[i] = rci_config.rci_core_level_thresholds[i];
        soc_rci_config.soc_dnx_fabric_rci_device_level_thresholds[i] = rci_config.rci_device_level_thresholds[i];
    }
    for (i = 0; i < BCM_FABRIC_NUM_OF_RCI_SEVERITIES; ++i) {
        ...
        soc_rci_config.soc_dnx_fabric_rci_severity_factors[i] = rci_config.rci_severity_factors[i];
    }
    ...
    soc_rci_config.soc_dnx_fabric_rci_high_score_fabric_rx_queue = rci_config.rci_high_score_fabric_rx_queue;
    soc_rci_config.soc_dnx_fabric_rci_high_score_fabric_rx_local_queue = rci_config.rci_high_score_fabric_rx_local_queue;
    soc_rci_config.soc_dnx_fabric_rci_threshold_num_of_congested_links = rci_config.rci_threshold_num_of_congested_links;
    soc_rci_config.soc_dnx_fabric_rci_high_score_congested_links = rci_config.rci_high_score_congested_links;

    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_thresholds_config_set, (unit, soc_rci_config)));
      ...
      bcm_fabric_rci_config_t* rci_config)
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_rci_thresholds_config_get, (unit, &soc_rci_config)));

    /*fill rci_config struct with fields*/
    for (i = 0; i < BCM_FABRIC_NUM_OF_RCI_LEVELS; ++i) {
        rci_config->rci_core_level_thresholds[i] = soc_rci_config.soc_dnx_fabric_rci_core_level_thresholds[i];
        rci_config->rci_device_level_thresholds[i]= soc_rci_config.soc_dnx_fabric_rci_device_level_thresholds[i];
    }
    for (i = 0; i < BCM_FABRIC_NUM_OF_RCI_SEVERITIES; ++i) {
        rci_config->rci_severity_factors[i] = soc_rci_config.soc_dnx_fabric_rci_severity_factors[i];
    }
    rci_config->rci_high_score_fabric_rx_queue = soc_rci_config.soc_dnx_fabric_rci_high_score_fabric_rx_queue;
    rci_config->rci_high_score_fabric_rx_local_queue = soc_rci_config.soc_dnx_fabric_rci_high_score_fabric_rx_local_queue;
    rci_config->rci_threshold_num_of_congested_links = soc_rci_config.soc_dnx_fabric_rci_threshold_num_of_congested_links;
    rci_config->rci_high_score_congested_links = soc_rci_config.soc_dnx_fabric_rci_high_score_congested_links;
    ...
    int value)
{
    ...
    switch (control) {
        ...
        case bcmFabricModuleControlFSMSeqNumberEnable:
            BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_efms_enable_set, (unit, (uint32)modid, value)));
            break;
    ...
    int *value)
{
    ...
    switch (control) {
        /* turn on/off NFMS feature per-device  */
        case bcmFabricModuleControlFSMSeqNumberEnable:
            BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_fabric_efms_enable_get, (unit, (uint32)modid, value)));
            break;
    ...
    bcm_module_t *destid_array)
{
    ...
    is_mesh_mc_using_ire = (SOC_DNX_CONFIG(unit)->jer2_arad->init.fabric.fabric_mesh_multicast_enable == 2);

    if (is_mesh_mc_using_ire) {
        BCMDNX_IF_ERR_EXIT(MBCM_DNX_DRIVER_CALL(unit, mbcm_dnx_static_replication_set, (unit, port, flags, destid_count, destid_array)));
    } else {
        BCMDNX_ERR_EXIT_MSG(BCM_E_CONFIG, (_BSL_BCM_MSG("bcm_dnx_fabric_static_replication_set is supported in TDM Mesh MC mode only")));
    }
