/*
 * $Id: dnxf_multicast_id_map.c,v 1.4 Broadcom SDK $
 *
...
int bcm_dnxf_multicst_id_map_init(int unit, soc_dnxf_multicast_table_mode_t mc_mode)
{
    ...
    /*Clear map*/
    rc = SOC_DNXF_WARM_BOOT_VAR_SET(unit, MC_MODE, &mc_mode);
    BCMDNX_IF_ERR_EXIT(rc);
...
int bcm_dnxf_multicst_id_map_clear_id(int unit, bcm_multicast_t id)
{
    ...
    if(id < max_ids) {
        ...
        rv = SOC_DNXF_WARM_BOOT_ARR_VAR_GET(unit, MC_ID_MAP, entry_id, mc_map_entry);
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    rv = SOC_DNXF_WARM_BOOT_ARR_VAR_SET(unit, MC_ID_MAP, entry_id, mc_map_entry);
    BCMDNX_IF_ERR_EXIT(rv);
