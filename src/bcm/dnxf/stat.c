    ...
    uint64 *value)
{
    ...
    if(SOC_E_NONE != MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_stat_is_supported_type,(unit, port, type))) {
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_dnxf_stat_multi_get32 is not available")));
exit:
    BCMDNX_FUNC_RETURN;
}
