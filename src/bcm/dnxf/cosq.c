...
STATIC int
_bcm_dnxf_cosq_gport_pipe_threshold_set(int unit,
                                       bcm_gport_t gport,
                                       bcm_cos_queue_t cosq,
                                       bcm_cosq_threshold_t *threshold);

STATIC int
_bcm_dnxf_cosq_gport_pipe_threshold_get(int unit,
                                       bcm_gport_t gport,
                                       bcm_cos_queue_t cosq,
                                       bcm_cosq_threshold_t *threshold);

STATIC int
_bcm_dnxf_cosq_gport_pipe_bandwidth_set(int unit,
                                       bcm_gport_t gport,
                                       bcm_cos_queue_t cosq,
...
STATIC int
_bcm_dnxf_cosq_gport_pipe_bandwidth_get(int unit,
                                       bcm_gport_t gport,
                                       bcm_cos_queue_t cosq,
                                       uint32 *kbits_sec_min,
                                       uint32 *kbits_sec_max,
...
STATIC int
_bcm_dnxf_cosq_pipe_control_set(int unit,
                               bcm_gport_t gport,
                               bcm_cos_queue_t cosq,
...
STATIC int
_bcm_dnxf_cosq_pipe_control_get(int unit,
                               bcm_gport_t gport,
                               bcm_cos_queue_t cosq,
                               bcm_cosq_control_t type,
                               int *arg);
                                 ...
                                 bcm_cosq_threshold_t *threshold)
{
    ...
    if (threshold->type ==  bcmCosqThresholdDynamicWeight) 
    {
        if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport) || BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport) 
            || BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
        {
            rc = _bcm_dnxf_cosq_gport_pipe_threshold_set(unit, gport, cosq, threshold);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        else
        {
                                 ...
                                 bcm_cosq_threshold_t *threshold)
{
    ...
    if (threshold->type ==  bcmCosqThresholdDynamicWeight) 
    {
        if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport) || BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport) 
            || BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
        {
            rc = _bcm_dnxf_cosq_gport_pipe_threshold_get(unit, gport, cosq, threshold);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        else
        {
                                 ...
                                 uint32 flags)
{
    ...
    {
        rc = _bcm_dnxf_cosq_gport_pipe_bandwidth_set(unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
                                 ...
                                 uint32 *flags)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport) || BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        rc = _bcm_dnxf_cosq_gport_pipe_bandwidth_get(unit, gport, cosq, kbits_sec_min, kbits_sec_max, flags);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
                         ...
                         int arg)
{
    ...
    switch (type) {
        case bcmCosqControlEgressBandwidthEnable:
            ...
            {
                rc = _bcm_dnxf_cosq_pipe_control_set(unit, port, cosq, type, arg);
                BCMDNX_IF_ERR_EXIT(rc);
            }
            else
            {
                         ...
                         int *arg)
{
    ...
    switch (type) {
        case bcmCosqControlEgressBandwidthEnable:
            if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(port))
            {
                rc = _bcm_dnxf_cosq_pipe_control_get(unit, port, cosq, type, arg);
                BCMDNX_IF_ERR_EXIT(rc);
            }
            else
            {
                                           ...
                                           int weight)
{
    ...
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_weight_set,(unit, pipe, port, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }

    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport)) /* middle */
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_mid_weight_set,(unit, pipe, port, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport)) /* egress */
    {
        if (port == -1) /* iterate over all fabric links */
        {
            ...
            {
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_weight_set,(unit, pipe, port, weight_mode, weight));
                BCMDNX_IF_ERR_EXIT(rc);
            }
        }
        else /* specific port */
                                           ...
                                           int *weight)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport)) /* ingress*/
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_weight_get,(unit, pipe, port, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }

    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport)) /* middle */
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_mid_weight_get,(unit, pipe, port, weight_mode, weight));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport)) /* egress */
    {
        if (port == -1) /* iterate over all fabric links */
        {
            PBMP_SFI_ITER(unit , port)
            {
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_weight_get,(unit, pipe, port, weight_mode, weight));
                BCMDNX_IF_ERR_EXIT(rc);
                                       ...
                                       bcm_cosq_threshold_t *threshold)
{
    ...
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_threshold_set,(unit, pipe, port, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport))
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_mid_threshold_set,(unit, pipe, port, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_threshold_set,(unit, pipe, port, threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       bcm_cosq_threshold_t *threshold)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_threshold_get,(unit, pipe, port, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_MIDDLE(gport))
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_mid_threshold_get,(unit, pipe, port, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_threshold_get,(unit, pipe, port, &threshold->value));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       uint32 flags)
{
    ...
    int port, pipe;
    int nof_pipes;
    soc_dnxf_cosq_shaper_mode_t shaper_mode;
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_rate_set,(unit, pipe, port, shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_rate_set,(unit, port, shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
                                       ...
                                       uint32 *flags)
{
    ...
    if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_INGRESS(gport))
    {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_rx_rate_get,(unit, pipe, port, &shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else if (BCM_COSQ_GPORT_IS_FABRIC_PIPE_EGRESS(gport))
    {
        ...
        rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_rate_get,(unit, port, &shaper_mode, kbits_sec_max));
        BCMDNX_IF_ERR_EXIT(rc);
    } else {
                               ...
                               int arg)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_rate_enable_set,(unit, pipe, port, arg));
    BCMDNX_IF_ERR_EXIT(rc);
                               ...
                               int *arg)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_cosq_pipe_tx_rate_enable_get,(unit, pipe, port, arg));
    BCMDNX_IF_ERR_EXIT(rc);
