...
#include <bcm_int/dnxf/dnxf_eyescan.h>
...
_bcm_port_control_pcs_set(int unit, bcm_port_t port, int value)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_pcs_set,(unit, port, (soc_dnxc_port_pcs_t)value));
...
int
_bcm_dnxf_port_deinit(int unit)
{
    int rc;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rc = bcm_dnxf_eyescan_deinit(unit);
    BCMDNX_IF_ERR_CONT(rc);    
    ...
    int value)
{
    ...
    switch(type)
    {
        ...
        case bcmPortControlStripCRC:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_strip_crc_set,(unit, port, value ? 1 :  0));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        
        case bcmPortControlRxEnable:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_rx_enable_set,(unit, port, 0, value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;

        case bcmPortControlTxEnable:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_tx_enable_set,(unit, port, value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
    ...
    int *value)
{
    ...
    {
        case bcmPortControlPCS:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_pcs_get,(unit, port, (soc_dnxc_port_pcs_t*)value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        ...
        case bcmPortControlStripCRC:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_strip_crc_get,(unit, port, value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
            
        case bcmPortControlRxEnable:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_rx_enable_get,(unit, port, value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;

        case bcmPortControlTxEnable:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_control_tx_enable_get,(unit, port, value));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_dnxf_port_stat_get32 is not available")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_dnxf_port_stat_multi_get32 is not available")));
exit:
    BCMDNX_FUNC_RETURN;
}
...
bcm_dnxf_port_probe(int unit, pbmp_t pbmp, pbmp_t *okay_pbmp) {
    ...
    } else
    {
        ...
        BCM_PBMP_ITER(pbmp, port) {
            BCMDNX_IF_ERR_EXIT(bcm_dnxf_fabric_link_control_set(unit, port, bcmFabricLinkPcpEnable, 0x1));
        }
...
bcm_dnxf_port_init(int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        /*SOC_PBMP_ITER(pbmp_disabled, p) {
           if ((rc = bcm_dnxf_port_enable_set(unit, p, 0)) < 0) {
                LOG_WARN(BSL_LS_BCM_PORT,
                         (BSL_META_U(unit,
                                     "Port %s: "
                                     "Failed to %s port: %s\n"),
...
{
    soc_dnxc_loopback_mode_t dnxf_loopback;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    case BCM_PORT_LOOPBACK_MAC:
        dnxf_loopback = soc_dnxc_loopback_mode_mac_async_fifo;
        break;
    case BCM_PORT_LOOPBACK_PHY:
        dnxf_loopback = soc_dnxc_loopback_mode_phy_gloop;
        break;
    case BCM_PORT_LOOPBACK_NONE:
        dnxf_loopback = soc_dnxc_loopback_mode_none;
        break;
    case BCM_PORT_LOOPBACK_PHY_REMOTE:
        dnxf_loopback = soc_dnxc_loopback_mode_phy_rloop;
        break;
    default:
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnxf_port_loopback_set(unit, port, dnxf_loopback));
...
bcm_dnxf_port_fault_get(int unit, bcm_port_t port, uint32 *flags)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);  
    BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_fault_get,(unit, port, flags)));
...
bcm_dnxf_port_selective_get(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (mask & BCM_PORT_ATTR_SPEED_MASK) {
        rc = bcm_dnxf_port_speed_get (unit, port, &info->speed);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    ...
    if (mask & BCM_PORT_ATTR_INTERFACE_MASK) {
       rc = bcm_dnxf_port_interface_get(unit, port, &info->interface);
       BCMDNX_IF_ERR_EXIT(rc);
   }
...
bcm_dnxf_port_selective_set(int unit, bcm_port_t port, bcm_port_info_t *info)
{
    ...
    if (mask & BCM_PORT_ATTR_INTERFACE_MASK) {
        rc = bcm_dnxf_port_interface_set(unit, port, info->interface);
        BCMDNX_IF_ERR_EXIT(rc);
    }
exit:
    BCMDNX_FUNC_RETURN; 
...
bcm_dnxf_port_phy_control_set(int unit, bcm_port_t port, bcm_port_phy_control_t type, uint32 value)
{
    ...
    if(BCM_PORT_PHY_CONTROL_CL72 == type || BCM_PORT_PHY_CONTROL_FIRMWARE_MODE == type) {
        BCMDNX_IF_ERR_EXIT(bcm_dnxf_port_enable_get(unit, port, &enable));
        if(enable) {
            BCMDNX_IF_ERR_EXIT(bcm_dnxf_port_enable_set(unit, port, 0));
        }
    }
    ...
    if(BCM_PORT_PHY_CONTROL_CL72 == type || BCM_PORT_PHY_CONTROL_FIRMWARE_MODE == type) {
        if(enable) {
            BCMDNX_IF_ERR_EXIT(bcm_dnxf_port_enable_set(unit, port, 1));
        }
    }
...
bcm_dnxf_port_speed_set(int unit, bcm_port_t port, int speed)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_speed_set,(unit, port, speed));
    BCMDNX_IF_ERR_EXIT(rc);

    /* 
     * Adjust empty cell size for links connected to repeater 
     */
    rc = _bcm_dnxf_fabric_link_repeater_update(unit, port);
    BCMDNX_IF_ERR_EXIT(rc);
...
bcm_dnxf_port_speed_get(int unit, bcm_port_t port, int *speed)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_speed_get,(unit, port, speed));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    bcm_port_if_t intf)
{
    ...
    rv = (MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_interface_set,(unit, port, intf)));
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_port_if_t *intf)
{
    ...
    rv = (MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_port_interface_get,(unit, port, intf)));
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    uint32 burst)
{
    ...
    core_clock_speed = SOC_DNXF_CONFIG(unit).core_clock_speed * 1000 /*Hz*/;
