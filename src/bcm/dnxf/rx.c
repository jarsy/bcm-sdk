...
_bcm_dnxf_rx_pkt_thread(int unit)
{
    ...
    if (!SOC_DNXF_CONFIG(unit).fabric_cell_fifo_dma_enable)
    {
        interval = 1000000;
    } 
    else 
    ...
    while ((SOC_DNXF_CONTROL(unit))->rx_reception_thread_running == 1)
    {
        ...
        /* Get a cpu cell */
        _rv = _bcm_dnxf_rx_captured_cell_get(unit, &cell);
        ...
        if (cell_received)
        {
            ...
            int i;
            int j=0;
            uint8 pkt_data[BCM_DNXF_RX_MAX_PAYLOAD_SIZE_IN_BYTES];
...
bcm_dnxf_rx_start(int unit, bcm_rx_cfg_t *cfg)
{
    ...
    BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_rx_cpu_address_modid_set, (unit, cfg->num_of_cpu_addresses, cfg->cpu_address)));

    /* get thread prio */

    priority =  soc_dnxf_property_get(unit, spn_BCM_RX_THREAD_PRI, 1, RX_THREAD_PRI_DFLT);
    ...
    int unit)
{
    ...
    if (!SOC_WARM_BOOT(unit)) {
        ...
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_rx_cpu_address_modid_init, (unit)));
    }
    ...
    BCMDNX_NULL_CHECK(SOC_DNXF_CONTROL(unit)->rx_thread_fifo_dma_semaphore);
