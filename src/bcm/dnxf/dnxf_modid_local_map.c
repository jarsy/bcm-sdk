...
int bcm_dnxf_modid_local_map_module_clear(int unit, bcm_module_t local_module_id)
{
    ...
    rv = SOC_DNXF_WARM_BOOT_ARR_VAR_SET(unit, MODID_LOCAL_MAP, local_module_id, &local_modid_map);
...
{  
    ...
    rv = SOC_DNXF_WARM_BOOT_ARR_VAR_GET(unit, MODID_LOCAL_MAP, local_module_id, &local_modid_map);
    BCMDNX_IF_ERR_EXIT(rv);
...
{  
    ...
    for(i = 0; i<DNXF_MODID_LOCAL_MAP_ROWS(unit) ; i++)
    {
        ...
        rv = SOC_DNXF_WARM_BOOT_ARR_VAR_GET(unit, MODID_LOCAL_MAP, i, &local_modid_map);
        BCMDNX_IF_ERR_EXIT(rv);
