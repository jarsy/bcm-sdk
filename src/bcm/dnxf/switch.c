 ...
 */
int 
bcm_dnxf_switch_event_register(int unit, bcm_switch_event_cb_t cb, void *userdata)
{
    int rc = BCM_E_NONE;
 ...
 */
int 
bcm_dnxf_switch_event_unregister(int unit, bcm_switch_event_cb_t cb, void *userdata)
{
    int rc = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;
 ...
 *      BCM_E_xxx
 */
int bcm_dnxf_switch_event_control_set(
    int unit, 
    ...
    uint32 value)
{
    ...
    case bcmSwitchEventMask:
        if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL) {
            rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_interrupt_all_enable_set, (unit, value ? 0 : 1));
            BCMDNX_IF_ERR_EXIT(rc);
    ...
    case bcmSwitchEventStormNominal:
        if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL) {
            if (value>0) {
                ...
                rc = SOC_DNXF_WARM_BOOT_VAR_SET(unit, INTR_STORM_NOMINAL, &value);
                DNXC_IF_ERR_EXIT(rc);
            } else if (value == 0) {
 ...
 *      BCM_E_xxx
 */
int bcm_dnxf_switch_event_control_get(
    int unit, 
    bcm_switch_event_t switch_event,
    bcm_switch_event_control_t type, 
    uint32 *value)
{
    ...
    case bcmSwitchEventMask:
        if (type.event_id == BCM_SWITCH_EVENT_CONTROL_ALL) {
            rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_interrupt_all_enable_get, (unit, &enable));
            BCMDNX_IF_ERR_EXIT(rc);
            *value = enable ? 0 : 1;
        } else {
            /* Get per interrupt */
...
STATIC int _dnxf_switch_control_sync(int unit, int arg)
{
    int rv = BCM_E_NONE;
    BCMDNX_INIT_FUNC_DEFS;

    rv = soc_dnxf_warm_boot_sync(unit);
 ...
 * Notes:
 */
int bcm_dnxf_switch_control_get(int unit, bcm_switch_control_t bcm_type, int *arg)
{
    BCMDNX_INIT_FUNC_DEFS;
    ...
    case bcmSwitchSynchronousPortClockSource:  
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_get, (unit, 1, arg)));
        break;
    case bcmSwitchSynchronousPortClockSourceBkup:
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_get, (unit, 0, arg)));
        break;
    case bcmSwitchSynchronousPortClockSourceDivCtrl: 
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_get, (unit, arg)));
        break;
 ...
 * Notes:
 */
int bcm_dnxf_switch_control_set(int unit, bcm_switch_control_t bcm_type, int arg)
{
    int rv;
    ...
    case bcmSwitchControlSync:
        ...
        break;
    case bcmSwitchControlAutoSync:
        BCMDNX_IF_ERR_EXIT(bcm_dnxf_switch_control_get(unit,
    ...
    case bcmSwitchCacheTableUpdateAll:
        rv = soc_dnxc_cache_table_update_all(unit);
        BCMDNX_IF_ERR_EXIT(rv);
        break;
    case bcmSwitchSynchronousPortClockSource: /* update master link for synce */
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_set, (unit, 1, arg)));
        break;
    case bcmSwitchSynchronousPortClockSourceBkup: /* update slave link for synce */
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_link_set, (unit, 0, arg)));
        break;
    case bcmSwitchSynchronousPortClockSourceDivCtrl: /* updated desired divider */
        BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_port_sync_e_divider_set, (unit, arg)));
        break;
    ...
    int *temperature_count)
{
    ...
    rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_temperature_monitor_get, (unit, temperature_max, temperature_array, temperature_count));
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_dnxf_switch_rov_get(
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnxf_avs_value_get(unit, rov));
