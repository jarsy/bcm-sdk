...
#define DNXF_FABRIC_CONTROL_SOURCE_VALID_CHECK(arg) \
     switch(arg)\
     {\
        case soc_dnxc_fabric_control_source_none:\
        case soc_dnxc_fabric_control_source_primary:\
        case soc_dnxc_fabric_control_source_secondary:\
...
#define DNXF_FABRIC_FIFO_TYPE_INDEX_VALID_CHECK(arg) \
     switch(arg) \
     { \
        case soc_dnxf_fabric_link_fifo_type_index_0: \
        case soc_dnxf_fabric_link_fifo_type_index_1: \
          break; \
        default:  \
          BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("fifo type %d is not valid"), arg)); \
     } 

#define DNXF_FABRIC_PIPE_TYPE_VALID_CHECK(arg) \
     if (SOC_DNXC_FABRIC_PIPE_IS_INIT(arg) ) \
     { \
...
STATIC int
_bcm_dnxf_fabric_link_control_weight_get(int unit,bcm_port_t link, int is_prim,int *arg)
{
    int rc;
    ...
    /*get registers*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_weight_get,(unit, link, is_prim, arg));
...
STATIC int 
_bcm_dnxf_fabric_link_control_set_cell_format(int unit, bcm_port_t link, int arg)
{
    int rc;
    int is_enabled;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    /*Verification*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_cell_format_verify,(unit, link, (soc_dnxf_fabric_link_cell_size_t)arg));
    BCMDNX_IF_ERR_EXIT(rc);

    /*Disable port*/
    rc = bcm_dnxf_port_enable_get(unit, link, &is_enabled);
    BCMDNX_IF_ERR_EXIT(rc);

    if (is_enabled) {
        rc = bcm_dnxf_port_enable_set(unit, link, 0);
        BCMDNX_IF_ERR_EXIT(rc);
    }

    /*Set value*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_cell_format_set,(unit, link, (soc_dnxf_fabric_link_cell_size_t)arg));
    BCMDNX_IF_ERR_EXIT(rc);

    /*Enable port*/
    if (is_enabled) {
        rc = bcm_dnxf_port_enable_set(unit, link, is_enabled);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    ...
    {
        rc = MBCM_DNXF_DRIVER_CALL(unit ,mbcm_dnxf_fabric_links_aldwp_config ,(unit, link));      
        BCMDNX_IF_ERR_EXIT(rc);
    } else {
        BCMDNX_IF_ERR_EXIT(rc);
    }
...
STATIC int 
_bcm_dnxf_fabric_links_cell_interleaving_set(int unit, bcm_port_t link, int arg)
{
    int rc;
    int is_enabled;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rc =  MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_cell_interleaving_set,(unit, link, arg ? 1 : 0));
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dnxf_fabric_link_control_set_weight(int unit,bcm_port_t link, int is_prim,int arg)
{
    int rc;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_weight_validate,(unit, arg));
    BCMDNX_IF_ERR_EXIT(rc);
    
    /*set registers*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_weight_set,(unit, link, is_prim, arg));
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dnxf_fabric_link_control_set_repeater_destination_per_link(int unit,bcm_port_t link, int arg)
{
    int rc;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_topology_repeater_destination_set,(unit, link, arg));
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dnxf_fabric_link_control_validate(int unit, _bcm_dnxf_fifo_type_handle_t fifo_type, bcm_fabric_link_threshold_type_t type)
{
    int rc, is_rx, is_tx;
    BCMDNX_INIT_FUNC_DEFS;
    
    rc = bcm_dnxf_fifo_type_is_rx(unit, fifo_type, &is_rx);
    BCMDNX_IF_ERR_EXIT(rc);
    rc = bcm_dnxf_fifo_type_is_tx(unit, fifo_type, &is_tx);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int 
_bcm_dnxf_fabric_control_set_rci_gci_control_source(int unit, bcm_fabric_control_t type, soc_dnxc_fabric_pipe_t arg)
{
    int rc,nof_pipes;
    ...
    if (SOC_DNXC_FABRIC_PIPE_ALL_PIPES_GET(arg,negative_mask)) /* if some bit is on , and that bit is > number of pipes */
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("GCI/RCI has invalid configuration- more than configured number of pipes ")));
    }
    
    /*set registers*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rci_gci_control_source_set,(unit, type, arg));
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dnxf_fabric_link_repeater_enable_set(int unit, bcm_port_t port, int enable)
{
    int rv;
    uint32 burst, pps;
    ...
    if (enable)
    {
         ...
         */
        rv = bcm_dnxf_port_rate_egress_pps_get(unit, port, &pps, &burst);
        BCMDNX_IF_ERR_EXIT(rv);

        rv = bcm_dnxf_port_speed_get(unit, port, &speed);
        BCMDNX_IF_ERR_EXIT(rv);

        if (pps == 0)
        {
            rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_repeater_enable_set,(unit, port, 1, SOC_DNXF_DEFS_GET(unit, repeater_default_empty_cell_size)));
            BCMDNX_IF_ERR_EXIT(rv); 
        } else {
            ...
            rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_repeater_enable_set,(unit, port, 1, empty_cell_size));
            BCMDNX_IF_ERR_EXIT(rv); 
        }

    } else {
        /*Disable repeater link and reset empty cell size*/
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_repeater_enable_set,(unit, port, 0, SOC_DNXF_DEFS_GET(unit, repeater_none_empty_cell_size)));
        BCMDNX_IF_ERR_EXIT(rv);           
    }
...
STATIC int
_bcm_dnxf_fabric_link_repeater_enable_get(int unit, bcm_port_t port, int *enable)
{
    int rv;
    int empty_cell_size;
    BCMDNX_INIT_FUNC_DEFS;

    rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_repeater_enable_get,(unit, port, enable, &empty_cell_size));
    BCMDNX_IF_ERR_EXIT(rv); 
 ...
 */
int
_bcm_dnxf_fabric_link_repeater_update(int unit, bcm_port_t port)
{
    int rc;
    int enable;
    BCMDNX_INIT_FUNC_DEFS;

    rc = bcm_dnxf_fabric_link_control_get(unit, port, bcmFabricLinkRepeaterEnable, &enable);
    BCMDNX_IF_ERR_EXIT(rc);

    if (enable)
    {
        /*re-eable repeater link will adjust empty cell size*/
        rc = bcm_dnxf_fabric_link_control_set(unit, port, bcmFabricLinkRepeaterEnable, enable);
        BCMDNX_IF_ERR_EXIT(rc);
    }
    ...
    int unit)
{
    ...
    char *vsc_str;
    int vsc;
    _bcm_dnxf_fifo_type_handle_t fifo_type_handle;
    int channel;
    ...
    /*Init FIFO types - just for FE1600 - RAMON do not use FIFO handlers*/
    if (!SOC_WARM_BOOT(unit) && SOC_IS_FE1600(unit)) 
    {
        for(i=0 ; i<soc_dnxf_fabric_nof_link_fifo_types ; i++) 
        {
            rc = SOC_DNXF_WARM_BOOT_ARR_VAR_GET(unit, FIFO_HANDLERS, i, &fifo_type_handle);
            BCMDNX_IF_ERR_EXIT(rc);
            rc = bcm_dnxf_fifo_type_clear(unit, &fifo_type_handle);
            BCMDNX_IF_ERR_EXIT(rc);
            rc = SOC_DNXF_WARM_BOOT_ARR_VAR_SET(unit, FIFO_HANDLERS, i, &fifo_type_handle);
            BCMDNX_IF_ERR_EXIT(rc);
        }
    }
    ...
    rc = dnxc_cells_buffer_create(unit, &SOC_DNXF_CONTROL(unit)->sr_cells_buffer, 30);
    BCMDNX_IF_ERR_EXIT(rc);

    rc = dnxc_captured_buffer_create(unit, &SOC_DNXF_CONTROL(unit)->captured_cells_buffer, 50);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if (SOC_DNXF_CONFIG(unit).fabric_cell_fifo_dma_enable != SOC_DNXF_PROPERTY_UNAVAIL && SOC_DNXF_CONFIG(unit).fabric_cell_fifo_dma_enable)
    {
        for (channel = SOC_DNXF_DEFS_GET(unit, fifo_dma_fabric_cell_first_channel);
              ...
              channel++)
        {
            SOC_DNXF_ALLOW_WARMBOOT_WRITE(soc_dnxf_fifo_dma_channel_init(unit, channel),rc);
            BCMDNX_IF_ERR_EXIT(rc);
        }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        sal_memset(SOC_DNXF_CONTROL(unit)->inband_routes, 0, sizeof(dnxc_soc_fabric_inband_route_t)*SOC_DNXF_NUM_OF_ROUTES);
    }
     ...
     */
    if (!SOC_WARM_BOOT(unit)) {
        if(soc_dnxf_fabric_device_mode_repeater == SOC_DNXF_CONFIG(unit).fabric_device_mode) {
            SOC_PBMP_ITER(valid_link_bitmap, i)
            {
                repeater_dest = soc_property_port_get(unit, i, spn_REPEATER_LINK_DEST, -1);
                if(repeater_dest >= 0) {
                    rc = bcm_dnxf_fabric_link_control_set(unit, i, bcmFabricLinkRepeaterDestinationLink, repeater_dest);
                    BCMDNX_IF_ERR_EXIT(rc);
                }
            }
        } else {
            ...
            {
                repeater_link_enable = soc_dnxf_property_port_get(unit, spn_REPEATER_LINK_ENABLE, link, 0, SOC_DNXF_PROPERTY_UNAVAIL);                                 
                if (repeater_link_enable != SOC_DNXF_PROPERTY_UNAVAIL /*unsupported or not configured*/ && repeater_link_enable)
                {
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        PBMP_SFI_ITER(unit, link)
        {
            soc_dnxf_check_soc_property(unit,spn_FABRIC_CELL_FORMAT ,&is_soc_property_supported,&soc_property_info);
            if (is_soc_property_supported)
            {
                vsc_str = soc_dnxf_property_port_get_str(unit, spn_FABRIC_CELL_FORMAT, link, 0, NULL);
                rc = soc_dnxf_property_str_to_enum(unit, spn_FABRIC_CELL_FORMAT, soc_dnxf_property_str_enum_fabric_cell_format,  vsc_str,  &vsc);
                BCMDNX_IF_ERR_EXIT(rc);

                BCMDNX_IF_ERR_EXIT(bcm_dnxf_fabric_link_control_set(unit, link, bcmFabricLinkCellFormat, vsc));
            }

            /*Enable PCP by default*/
            if (soc_feature(unit, soc_feature_fabric_cell_pcp))
            {
                BCMDNX_IF_ERR_EXIT(bcm_dnxf_fabric_link_control_set(unit, link, bcmFabricLinkPcpEnable, 0x1));
            }
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit)) {
        if (SOC_DNXF_IS_FE2(unit)) {
            BCMDNX_IF_ERR_EXIT(soc_dnxf_control_cell_filter_clear(unit));
            BCMDNX_IF_ERR_EXIT(soc_dnxf_control_cell_filter_set(unit, SOC_CELL_FILTER_FLAG_DONT_FORWARD | SOC_CELL_FILTER_FLAG_DONT_TRAP, soc_dnxc_fe2_filtered_cell,  0, NULL, NULL));
        }
    }
    ...
    if (!SOC_WARM_BOOT(unit))
    {
        isolate_type = soc_dnxf_fabric_isolate_type_none;
        BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_TYPE, &isolate_type));
    }

    if (!SOC_WARM_BOOT(unit))
    {
        if (SOC_DNXF_CONFIG(unit).fe_mc_priority_map_enable == 1) /* set all valid multicast ID's to low priority */
        {
            BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_multicast_table_size_get, (unit, &mc_table_size)));
            BCMDNX_IF_ERR_EXIT(bcm_dnxf_fabric_control_set(unit, bcmFabricMcLowPrioMin, 0));
            BCMDNX_IF_ERR_EXIT(bcm_dnxf_fabric_control_set(unit, bcmFabricMcLowPrioMax, mc_table_size-1));
        }
    }
    ...
    int arg)
{
    ...
    switch(type)
    {
        ...
        case bcmFabricMcMidPrioMax:
            if (!(SOC_DNXF_CONFIG(unit).fe_mc_priority_map_enable == 1))
            {
       ...
       case bcmFabricIsolate:
           if (SOC_DNXF_IS_FE2(unit))
           {
               ...
               if (arg)
               {
                   /*save device status in order to save isolate info*/
                    ...
                    isolate_type = soc_dnxf_fabric_isolate_type_isolate;

                    BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_UNISOLATED_LINKS, &pbmp_unisolated_links));
                    BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_TYPE, &isolate_type));
               } else {
                   /*read the device state prior to isolation*/
                   BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_UNISOLATED_LINKS, &pbmp_unisolated_links));
                   BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_TYPE, &isolate_type));

                   if (isolate_type != soc_dnxf_fabric_isolate_type_isolate)
                   {
               ...
               rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_drv_fe13_isolate_set ,(unit, pbmp_unisolated_links, arg)); 
               BCMDNX_IF_ERR_EXIT(rc);
           } else {
        ...
        case bcmFabricShutdown:
            if (arg) /* shutdown */
            {
                ...
                isolate_type = soc_dnxf_fabric_isolate_type_shutdown;

                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_UNISOLATED_LINKS, &pbmp_unisolated_links));
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_ACTIVE_LINKS, &pbmp_active_links));
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_SET(unit, ISOLATE_ISOLATE_DEVICE, &device_isolation_status));
                
                /*Shut down device*/
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_graceful_shutdown_set, (unit, pbmp_active_links, 1, pbmp_unisolated_links, 0)); 
                BCMDNX_IF_ERR_EXIT(rc);            

            } else { /* bring up */
                /*recover info prior*/ 
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_UNISOLATED_LINKS, &pbmp_unisolated_links));
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_ACTIVE_LINKS, &pbmp_active_links));
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_ISOLATE_DEVICE, &device_isolation_status));
                BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_TYPE, &isolate_type));

                if (isolate_type == soc_dnxf_fabric_isolate_type_shutdown)
                {
                                    /*Shut down device*/
                    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_graceful_shutdown_set, (unit, pbmp_active_links, 0, pbmp_unisolated_links, device_isolation_status == soc_dnxc_isolation_status_isolated ? 1:0)); 
                    BCMDNX_IF_ERR_EXIT(rc);            
                       
                } else {
                    rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_drv_graceful_shutdown_set, (unit, PBMP_SFI_ALL(unit), 0, PBMP_SFI_ALL(unit), 0)); 
                    BCMDNX_IF_ERR_EXIT(rc);                      
                }
                isolate_type = soc_dnxf_fabric_isolate_type_none;
        ...
        case bcmFabricGCIControlSource:
            rc = _bcm_dnxf_fabric_control_set_rci_gci_control_source(unit, type, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        default:
...
{
    int rc = BCM_E_NONE;
    soc_dnxf_fabric_isolate_type_t isolate_type;
    BCMDNX_INIT_FUNC_DEFS;
    ...
    switch(type)
    {
       ...
       case bcmFabricIsolate:
           ...
           {
               BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_TYPE, &isolate_type));
               *arg = (isolate_type == soc_dnxf_fabric_isolate_type_isolate ? 1 : 0);
           } else {
        ...
        case bcmFabricRCIControlSource:
        case bcmFabricGCIControlSource:
            rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rci_gci_control_source_get,(unit, type,(soc_dnxc_fabric_pipe_t*) arg));
            BCMDNX_IF_ERR_EXIT(rc);
            break;
       case bcmFabricShutdown:
           BCMDNX_IF_ERR_EXIT(SOC_DNXF_WARM_BOOT_VAR_GET(unit, ISOLATE_TYPE, &isolate_type));
           *arg = (isolate_type == soc_dnxf_fabric_isolate_type_shutdown ? 1 : 0);
           break;
        default:
            BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported Type %d"), type));       
    }
    ...
    bcm_module_t *modid_array)
{
    ...
    SOC_DNXF_MARK_WARMBOOT_DIRTY_BIT(unit);    
       ...
       bcm_port_t *links_array)
{
    ...
    { 
        ...
        {
                DNXF_LINK_INPUT_CHECK(unit, links_array[i]);
        }
    }
    
    /*validate destination type*/
    if(SOC_DNXF_CONTROL(unit)->cfg.fabric_device_mode == soc_dnxf_fabric_device_mode_multi_stage_fe2)
    {
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_topology_set,(unit, local_id, links_count, links_array));
    BCMDNX_IF_ERR_EXIT(rc);

    /*MIN number of links*/
   DNXC_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_topology_nof_links_to_min_nof_links_default, (unit, links_count, &min_nof_links)));
   DNXC_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_topology_min_nof_links_set, (unit, min_nof_links)));
       ...
       bcm_port_t *links_array)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_topology_get,(unit, local_id, links_count_max, links_count, links_array));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    bcm_module_t *destid_array)
{
    ...
    {
        /*convert to ports*/
        if(soc_dnxf_fabric_device_mode_multi_stage_fe2 == SOC_DNXF_CONFIG(unit).fabric_device_mode)
        {
        ...
        else
        {
          ...
          else
          {
               if(BCM_FABRIC_MODID_IS_LOCAL(destid_array[i]))
               {
                   ...
                   rc = bcm_dnxf_modid_local_map_is_valid(unit, port_array[i], &is_valid);
                   BCMDNX_IF_ERR_EXIT(rc);
                   if(!is_valid) {
               ...
               {
                   if((soc_dnxf_multicast_mode_direct == SOC_DNXF_CONFIG(unit).fabric_multicast_mode) &&  DNXF_MODID_LOCAL_MAP_ROW_VALIDATE(unit, destid_array[i]))
                   {
    ...
    bcm_module_t *destid_array)
{
    ...
    for(i=0 ; i<*destid_count ; i++)
    {
        ...
        {
             if((soc_dnxf_multicast_mode_direct == SOC_DNXF_CONFIG(unit).fabric_multicast_mode) &&  DNXF_MODID_LOCAL_MAP_ROW_VALIDATE(unit, port_array[i]))
             {
    ...
    bcm_fabric_module_vector_t *dest_array)
{
    ...
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_multicast_multi_get,(unit, flags, ngroups, groups, dest_array));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    bcm_fabric_module_vector_t *dest_array)
{
    ...
    /*Write to reg*/
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_multicast_multi_set,(unit, flags, ngroups, groups, dest_array));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int arg)
{
    ...
    switch(type)
    {
        case bcmFabricLinkCellFormat:
            rc = _bcm_dnxf_fabric_link_control_set_cell_format(unit, link, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricLinkCellInterleavingEnable:
            rc = _bcm_dnxf_fabric_links_cell_interleaving_set(unit, link, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        case bcmFabricLinkPrimaryWeight:
              rc = _bcm_dnxf_fabric_link_control_set_weight(unit, link, 1, arg);
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkSecondaryWeight:
              rc = _bcm_dnxf_fabric_link_control_set_weight(unit, link, 0, arg);
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkIsSecondaryOnly:
              rc =  MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_secondary_only_set,(unit, link, arg ? 1 : 0));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        ...
        case bcmFabricLinkRepeaterDestinationLink:
              rc = _bcm_dnxf_fabric_link_control_set_repeater_destination_per_link(unit, link, arg);
              BCMDNX_IF_ERR_EXIT(rc);
              break;   
       ...
       case bcmFabricLinkRepeaterEnable:
           rc = _bcm_dnxf_fabric_link_repeater_enable_set(unit, link, arg);
           BCMDNX_IF_ERR_EXIT(rc);
           break;
        default:
              BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported Type %d"),type)); 
    ...
    int *arg)
{
    ...
    {
        case bcmFabricLinkCellFormat:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_cell_format_get,(unit,link,(soc_dnxf_fabric_link_cell_size_t*)arg));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkCellInterleavingEnable:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_cell_interleaving_get,(unit, link, arg)); 
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkPrimaryWeight:
              rc = _bcm_dnxf_fabric_link_control_weight_get(unit, link, 1, arg);
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkSecondaryWeight:
              rc = _bcm_dnxf_fabric_link_control_weight_get(unit, link, 0, arg);
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLinkIsSecondaryOnly:
              rc =  MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_secondary_only_get,(unit, link, arg));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
        case bcmFabricLLFControlSource: 
              ...
              break;   
        case bcmFabricLinkRepeaterDestinationLink:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_topology_repeater_destination_get,(unit, link, arg));
              BCMDNX_IF_ERR_EXIT(rc);
              break;   
        case bcmFabricLinkIsolate:
        ...
        case bcmFabricLinkPcpEnable:
            ...
            break;
        case bcmFabricLinkRepeaterEnable:
            rc = _bcm_dnxf_fabric_link_repeater_enable_get(unit, link, arg);
            BCMDNX_IF_ERR_EXIT(rc);
            break;
        default:
              BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Unsupported Type %d"),type)); 
...
{
    int i, rc, is_overlap;
    _bcm_dnxf_fifo_type_handle_t new_fifo_type, fifo_type_handle;
    int is_rx=0, is_tx=0, is_fe1=0, is_fe3=0, is_primary=0, is_secondary=0;
    BCMDNX_INIT_FUNC_DEFS;
    
    BCMDNX_NULL_CHECK(fifo_type);
    
    rc = bcm_dnxf_fifo_type_clear(unit, &new_fifo_type);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    if(flags & BCM_FABRIC_LINK_THRESHOLD_WITH_ID)
    {
        ...
        rc = bcm_dnxf_fifo_type_get_id(unit, *fifo_type, &i);
        BCMDNX_IF_ERR_EXIT(rc);
        DNXF_FABRIC_FIFO_TYPE_INDEX_VALID_CHECK(i);

        /*all relevant bits are free*/
        rc = SOC_DNXF_WARM_BOOT_ARR_VAR_GET(unit, FIFO_HANDLERS, i, &fifo_type_handle);
        BCMDNX_IF_ERR_EXIT(rc);
        rc = bcm_dnxf_fifo_type_is_overlap(unit, *fifo_type, fifo_type_handle, &is_overlap);
        BCMDNX_IF_ERR_EXIT(rc);
        if(!is_overlap)
        {
             bcm_dnxf_fifo_type_add(unit, i, *fifo_type);
        }
        else
        {
        ...
        rc = bcm_dnxf_fifo_type_set_handle_flag(unit, (uint32*)fifo_type); /* this is required for the new attach implementation , we need to indicate that we are using a "real" handle */
        BCMDNX_IF_ERR_EXIT(rc);
    }
    else
    {
        ...
        if(SOC_DNXF_CONFIG(unit).fabric_device_mode == soc_dnxf_fabric_device_mode_multi_stage_fe13)
        {
        ...
        rc = bcm_dnxf_fifo_type_set(unit, &new_fifo_type, is_rx, is_tx, is_fe1, is_fe3, is_primary, is_secondary);
        BCMDNX_IF_ERR_EXIT(rc);
        ...
        for(i=0 ; i< soc_dnxf_fabric_nof_link_fifo_types ; i++)
        {
            ...
            rc = bcm_dnxf_fifo_type_is_overlap(unit, new_fifo_type, fifo_type_handle, &is_overlap);
            BCMDNX_IF_ERR_EXIT(rc);
            if(!is_overlap)
            {
                bcm_dnxf_fifo_type_add(unit, i, new_fifo_type);
                bcm_dnxf_fifo_type_set_id(unit, &new_fifo_type, i);
                rc = bcm_dnxf_fifo_type_set_handle_flag(unit, &new_fifo_type);
                BCMDNX_IF_ERR_EXIT(rc);
...
/*
 * Function:
 *      _bcm_dnxf_fabric_fifo_info_get
 * Purpose:
 *      Getting the parameters required for bcm_dnxf_fabric_link_thresholds_pipe_get
 * Parameters:
 *      unit                            - (IN)    Unit number.
 *      pipe_id                         - (IN)    0/1/2   
 *      direction                       - (IN)    rx/tx/middle
 *      fifo_type, pipe_get, flags_get  - (OUT) should be used when calling to bcm_dnxf_fabric_link_thresholds_pipe_get
 * Returns:
 *      BCM_E_xxx
 * Notes:
 */
   ...
   uint32 *flags_get)
{
    ...
    bcm_port_t links[SOC_DNXF_MAX_NOF_LINKS];
    uint32 links_count;
    ...
    if (direction  == soc_dnxc_fabric_direction_middle)
    {
        *fifo_type = soc_dnxf_fabric_link_fifo_type_index_0;
        *pipe_get = pipe_id;
        *flags_get = 0;
        BCM_EXIT;
    } else if (direction  == soc_dnxc_fabric_direction_middle_local_switch_low ||
               direction  == soc_dnxc_fabric_direction_middle_local_switch_high)
    {
        *fifo_type = soc_dnxf_fabric_link_fifo_type_index_1;
        *pipe_get = pipe_id;
        *flags_get = (direction  == soc_dnxc_fabric_direction_middle_local_switch_low) ? BCM_FABRIC_LINK_TH_FE1_LINKS_ONLY : BCM_FABRIC_LINK_TH_FE3_LINKS_ONLY;
        BCM_EXIT;
    }

    /*Verify*/
    if (pipe_id >= SOC_DNXF_FABRIC_PIPES_CONFIG(unit).nof_pipes)
    {
    ...
    is_rx = (direction == soc_dnxc_fabric_direction_rx ? 1 : 0);
    is_tx = (direction == soc_dnxc_fabric_direction_tx ? 1 : 0);

    rv = bcm_dnxf_fifo_type_clear(unit, fifo_type);
    BCMDNX_IF_ERR_EXIT(rv);

    BCMDNX_IF_ERR_EXIT(MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_link_device_mode_get,(unit, port, is_rx, &link_mode)));
    if (link_mode == soc_dnxf_fabric_link_device_mode_multi_stage_fe1)
    {
        is_fe1 = 1;
        is_fe3 = 0;
    } else if (link_mode == soc_dnxf_fabric_link_device_mode_multi_stage_fe3) {
        is_fe1 = 0;
        is_fe3 = 1;
    } else {
        is_fe1 = is_fe3 = 1;
    ...
    found = 0;
    for(fifo_index = 0 ; fifo_index < soc_dnxf_fabric_nof_link_fifo_types; fifo_index++) 
    {
        /*Check if the relvant fifo is attached to the fifo type*/
        rv = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_link_type_get,(unit, pipe_id, fifo_index, is_rx, is_tx, is_fe1, is_fe3, SOC_DNXF_MAX_NOF_LINKS, links, &links_count));
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        for (i = 0; i < links_count; i++)
        {
            if (port == links[i])
            {
                ...
                if (SOC_IS_FE1600(unit))
                {
                    ...
                    /*Build the handle*/
                    rv = bcm_dnxf_fifo_type_set(unit, fifo_type , is_rx, is_tx, is_fe1, is_fe3, is_primary, is_secondary);
                    BCMDNX_IF_ERR_EXIT(rv);
                    rv = bcm_dnxf_fifo_type_set_id(unit, fifo_type, fifo_index);
                    BCMDNX_IF_ERR_EXIT(rv);
                    rv = bcm_dnxf_fifo_type_set_handle_flag(unit, fifo_type);
                    BCMDNX_IF_ERR_EXIT(rv);
    ...
    int fifo_type)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);
    /*clear fifo_type*/
    rc = bcm_dnxf_fifo_type_sub(unit, i, fifo_type);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int *value)
{
    ...
    rc = bcm_dnxf_fabric_link_thresholds_pipe_set(unit, fifo_type, bcmFabricPipeAll, 0, count, type, value);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int *value)
{
    ...
    rc = bcm_dnxf_fabric_link_thresholds_pipe_get(unit, fifo_type, bcmFabricPipeAll, 0,count, type, value);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int *value)
{
    ...
    int rc = BCM_E_NONE;
    soc_dnxc_fabric_pipe_t fabric_pipe;

    BCMDNX_INIT_FUNC_DEFS;

    BCMDNX_NULL_CHECK(type);
    BCMDNX_NULL_CHECK(value);
    SOC_DNXC_FABRIC_PIPE_INIT(fabric_pipe);
 
    if (SOC_IS_FE1600(unit) && (pipe !=  bcmFabricPipeAll))
    {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("invalid pipe for ramon_fe1600 "))); 
    }
    ...
    /* validate flags */
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_thresholds_flags_validate,(unit, flags));
    BCMDNX_IF_ERR_EXIT(rc);

    /*get type*/
    rc = bcm_dnxf_fifo_type_get_handle_flag(unit, fifo_type, &is_handle_flag);
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    {
        rc = bcm_dnxf_fifo_type_get_id(unit, fifo_type, &type_idx); 
        BCMDNX_IF_ERR_EXIT(rc);

        rc =  bcm_dnxf_fifo_type_is_primary(unit, fifo_type, &is_primary);
        BCMDNX_IF_ERR_EXIT(rc);
        rc =  bcm_dnxf_fifo_type_is_secondary(unit, fifo_type, &is_secondary);
        BCMDNX_IF_ERR_EXIT(rc);

        rc = bcm_dnxf_fifo_type_is_fe1(unit, fifo_type, &is_fe1);
        BCMDNX_IF_ERR_EXIT(rc);
        rc = bcm_dnxf_fifo_type_is_fe3(unit, fifo_type, &is_fe3);
        BCMDNX_IF_ERR_EXIT(rc); 
    }
    DNXF_FABRIC_FIFO_TYPE_INDEX_VALID_CHECK(type_idx);

    if (pipe == bcmFabricPipeAll)
    {
        SOC_DNXC_FABRIC_PIPE_ALL_PIPES_SET(&fabric_pipe, nof_pipes);
        if (!is_primary)
        {
            SOC_DNXC_FABRIC_PIPE_CLEAR(&fabric_pipe, 0);
        }
        if (!is_secondary)
        {
            SOC_DNXC_FABRIC_PIPE_CLEAR(&fabric_pipe, 1);
        }
    }
    else
    {
        SOC_DNXC_FABRIC_PIPE_SET(&fabric_pipe, pipe - bcmFabricPipe0);
    }
    ...
    for(i = 0 ; i<count ; i++) {
        ...
        if (is_handle_flag)
        {
            rc = _bcm_dnxf_fabric_link_control_validate(unit, fifo_type, type[i]);
            BCMDNX_IF_ERR_EXIT(rc);
        }
        ...
        {
            case bcmFabricLinkRxFifoLLFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_llfc_threshold_validate,(unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkFE1TxBypassLLFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_bypass_llfc_threshold_validate,(unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRciFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_rci_threshold_validate,(unit, type[i],type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkTxGciLvl2FC:
            case bcmFabricLinkTxGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_gci_threshold_validate,(unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxGciLvl2FC:
            case bcmFabricLinkRxGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_gci_threshold_validate,(unit, type[i], type_idx, fabric_pipe, value[i],is_fe1,is_fe3));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkTxPrio2Drop:
            case bcmFabricLinkTxPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_drop_threshold_validate,(unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxPrio2Drop:
            case bcmFabricLinkRxPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_drop_threshold_validate,(unit, type[i], fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxRciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_rci_threshold_validate, (unit, type[i], type_idx, fabric_pipe, value[i], is_fe1, is_fe3));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_full_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_fifo_size_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxMcLowPrioDrop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_multicast_low_prio_drop_threshold_validate, (unit,type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_gci_threshold_validate, (unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidRciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_rci_threshold_validate, (unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_prio_drop_threshold_validate, (unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkMidAlmostFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_almost_full_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break; 

            case bcmFabricLinkMidFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_fifo_size_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkTxRciLvl3FC:
                rc =  MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_rci_threshold_fc_validate, (unit, type[i], type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkTxAlmostFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_almost_full_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkTxFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_fifo_size_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkMidFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_full_threshold_validate, (unit, type_idx, fabric_pipe, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
    ...
    for (i = 0 ; i<count ; i++)
    {
        ...
        {
            case bcmFabricLinkRxFifoLLFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_llfc_threshold_set,(unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkFE1TxBypassLLFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_bypass_llfc_threshold_set,(unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRciFC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_rci_threshold_set,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkTxGciLvl2FC:
            case bcmFabricLinkTxGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_gci_threshold_set,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxGciLvl2FC:
            case bcmFabricLinkRxGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_gci_threshold_set,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkTxPrio2Drop:
            case bcmFabricLinkTxPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_drop_threshold_set,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxPrio2Drop:
            case bcmFabricLinkRxPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_drop_threshold_set,(unit, type[i], fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkRxRciLvl2FC:
            case bcmFabricLinkRxRciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_rci_threshold_set, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_full_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_fifo_size_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkRxMcLowPrioDrop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_multicast_low_prio_drop_threshold_set, (unit,type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidGciLvl2FC:
            case bcmFabricLinkMidGciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_gci_threshold_set, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidRciLvl2FC:
            case bcmFabricLinkMidRciLvl3FC:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_rci_threshold_set, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
            ...
            case bcmFabricLinkMidPrio2Drop:
            case bcmFabricLinkMidPrio3Drop:
                rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_prio_drop_threshold_set, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkMidAlmostFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_almost_full_threshold_set, (unit, type_idx, fabric_pipe, -1, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
                 
            case bcmFabricLinkMidFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_fifo_size_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i], 1));
                BCMDNX_IF_ERR_EXIT(rc);
                break; 
            ...
            case bcmFabricLinkTxRciLvl2FC:
            case bcmFabricLinkTxRciLvl3FC:
                rc =  MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_rci_threshold_fc_set, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;                                                

            case bcmFabricLinkTxAlmostFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_almost_full_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i], 1));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkTxFifoSize:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_fifo_size_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i], 1));
                BCMDNX_IF_ERR_EXIT(rc);
                break;

            case bcmFabricLinkMidFull:
                rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_full_threshold_set, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, value[i]));
                BCMDNX_IF_ERR_EXIT(rc);
                break;
    ...
    int *value)
{
    ...
    for(i = 0 ; i<count; i++)
    {
        ...
        {
          case bcmFabricLinkRxFifoLLFC: 
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_llfc_threshold_get,(unit, type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          
          case bcmFabricLinkFE1TxBypassLLFC: 
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_bypass_llfc_threshold_get,(unit, type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          
          case bcmFabricLinkRciFC: 
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_rci_threshold_get,(unit, type[i],type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkTxGciLvl2FC:
          case bcmFabricLinkTxGciLvl3FC:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_gci_threshold_get,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkRxGciLvl2FC:
          case bcmFabricLinkRxGciLvl3FC:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_gci_threshold_get,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkTxPrio2Drop:
          case bcmFabricLinkTxPrio3Drop:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_tx_drop_threshold_get,(unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkRxPrio2Drop:
          case bcmFabricLinkRxPrio3Drop:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_drop_threshold_get,(unit, type[i], fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkRxRciLvl2FC:
          case bcmFabricLinkRxRciLvl3FC:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_rci_threshold_get, (unit, type[i], type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;

          case bcmFabricLinkRxFull:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_full_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, is_fe3,  &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;

          case bcmFabricLinkRxFifoSize:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_fifo_size_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;

          case bcmFabricLinkRxMcLowPrioDrop:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_rx_multicast_low_prio_drop_threshold_get, (unit,type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkMidGciLvl2FC:
          case bcmFabricLinkMidGciLvl3FC:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_gci_threshold_get, (unit, type[i], type_idx, fabric_pipe, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkMidRciLvl2FC:
          case bcmFabricLinkMidRciLvl3FC:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_rci_threshold_get, (unit, type[i], type_idx, fabric_pipe, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
          ...
          case bcmFabricLinkMidPrio2Drop:
          case bcmFabricLinkMidPrio3Drop:
              rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_flow_control_mid_prio_drop_threshold_get, (unit, type[i], type_idx, fabric_pipe, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;

          case bcmFabricLinkMidAlmostFull:
              rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_almost_full_threshold_get, (unit, type_idx, fabric_pipe, 0, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break; 

          case bcmFabricLinkMidFifoSize:
              rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_fifo_size_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break; 
          ...
          case bcmFabricLinkTxRciLvl2FC:
          case bcmFabricLinkTxRciLvl3FC:
              rc =  MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_rci_threshold_fc_get, (unit, type[i],type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break; 
              
          case bcmFabricLinkTxAlmostFull:
              rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_almost_full_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;
              
          case bcmFabricLinkTxFifoSize:
              rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_tx_fifo_size_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, is_fe3, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break; 
              
          case bcmFabricLinkMidFull:
              rc = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_flow_control_mid_full_threshold_get, (unit, type_idx, fabric_pipe, is_fe1, &(value[i])));
              BCMDNX_IF_ERR_EXIT(rc);
              break;                                                
    ...
    bcm_port_t* links)
{
    ...
    soc_dnxc_fabric_pipe_t pipe;
    BCMDNX_INIT_FUNC_DEFS;
    
    BCMDNX_NULL_CHECK(links);
    SOC_DNXC_FABRIC_PIPE_INIT(pipe);
    ...
    if (!is_handle_flag) /* direct attach - without fifo handle */
    {
        ...
        is_fe1 = 1;
        is_fe3 = 1;
        SOC_DNXC_FABRIC_PIPE_ALL_PIPES_SET(&pipe, nof_pipes);
    }
    else 
    {
        rc = bcm_dnxf_fifo_type_get_id(unit, fifo_type, &fifo_type_index);
        BCMDNX_IF_ERR_EXIT(rc);
        ...
        if(is_primary)
        {
            SOC_DNXC_FABRIC_PIPE_SET(&pipe,0);
        }
        if(is_secondary)
        {
            SOC_DNXC_FABRIC_PIPE_SET(&pipe,1);
        }

        DNXF_FABRIC_CONTROL_SOURCE_VALID_CHECK(pipe);
    ...
    DNXF_FABRIC_FIFO_TYPE_INDEX_VALID_CHECK(fifo_type_index);

    DNXF_UNIT_LOCK_TAKE(unit);
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_link_type_set,(unit, pipe, fifo_type_index, is_rx, is_tx, is_fe1, is_fe3, links_count, links));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    uint32*     links_count)
{
    ...
    DNXF_UNIT_LOCK_TAKE(unit);  
    rc = MBCM_DNXF_DRIVER_CALL(unit,mbcm_dnxf_fabric_links_link_type_get,(unit, pipe, fifo_type_index, is_rx, is_tx, is_fe1, is_fe3, links_count_max, links, links_count));
    BCMDNX_IF_ERR_EXIT(rc);
    ...
    int *link_partner_count)
{
    ...
    *link_partner_count = 0;
    ...
    max_link_id = SOC_DNXF_DEFS_GET(unit, nof_links);
    ...
    uint32 *data_out_size)
{
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnxf_sr_cell_payload_receive(unit, flags, data_out_max_size, data_out_size, data_out));
    ...
    uint32 *data_in)
{
    ...
    BCMDNX_IF_ERR_EXIT(soc_dnxf_sr_cell_send(unit, flags, &soc_route, data_in_size, data_in));
    ...
    bcm_fabric_link_remote_pipe_mapping_t *mapping_config)
{
    ...
    {
        rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_links_repeater_nof_remote_pipe_get, (unit, port, &mapping_config->num_of_remote_pipes));
        BCMDNX_IF_ERR_EXIT(rv);
    }
    else
    {
    ...
    bcm_fabric_link_remote_pipe_mapping_t *mapping_config)
{
    ...
    {
        /*Verify*/
        if (mapping_config->num_of_remote_pipes > SOC_DNXF_MAX_NOF_PIPES)
        {
        ...
        rv = MBCM_DNXF_DRIVER_CALL(unit, mbcm_dnxf_fabric_links_repeater_nof_remote_pipe_set, (unit, port, mapping_config->num_of_remote_pipes));
        BCMDNX_IF_ERR_EXIT(rv);
    ...
    else 
    {
        ...
        for (i = 0; i < mapping_config->num_of_remote_pipes; i++)
        {
            if ((mapping_config->remote_pipe_mapping[i] - bcmFabricPipe0) < 0 ||
                 (mapping_config->remote_pipe_mapping[i] - bcmFabricPipe0) >= SOC_DNXF_FABRIC_PIPES_CONFIG(unit).nof_pipes)
            {
