    ...
    case _BCM_QOS_MAP_TYPE_MIML_ING_PRI_CNG_MAP:
        ...
        if (!_BCM_QOS_ING_PRI_CNG_USED_GET(unit, id)) {
            ...
            if (untagged) {
                ...
                soc_mem_field32_set(unit, ING_PRI_CNG_MAPm, entry, PRIf,
                                    map->int_pri);
                soc_mem_field32_set(unit, ING_PRI_CNG_MAPm, entry, CNGf,
                                    _BCM_COLOR_ENCODING(unit, map->color));
            }
        ...
        if (_BCM_QOS_GPORT_IS_VFI_VP_PORT(port) ||
            ...
            if ((flags & BCM_QOS_MAP_INGRESS) == BCM_QOS_MAP_INGRESS) {
                ...
                if ((flags & BCM_QOS_MAP_L2) == BCM_QOS_MAP_L2) {
                    ...
                    use_profile = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm,
                                      &egr_nh, dot1p_pri_select_field);
                    if (use_profile == 1) {
                        profile_idx = soc_mem_field32_get(unit, EGR_L3_NEXT_HOPm,
                                          &egr_nh, dot1p_mapping_ptr_field);
                        rv = _bcm_tr2_qos_idx2id(unit, profile_idx,
