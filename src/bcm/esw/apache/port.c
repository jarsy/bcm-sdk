 ...
 */
#define BCM_AP_PORT_RESOURCE_OP_NONE     0
#define BCM_AP_PORT_RESOURCE_OP_PMAP    (1 << 0)
#define BCM_AP_PORT_RESOURCE_OP_LANES   (1 << 1)
#define BCM_AP_PORT_RESOURCE_OP_SPEED   (1 << 2)
#define BCM_AP_PORT_RESOURCE_OP_ENCAP   (1 << 3)
#define BCM_AP_PORT_RESOURCE_OP_ALL           \
    (BCM_AP_PORT_RESOURCE_OP_PMAP  |          \
     BCM_AP_PORT_RESOURCE_OP_LANES |          \
     BCM_AP_PORT_RESOURCE_OP_SPEED |          \
     BCM_AP_PORT_RESOURCE_OP_ENCAP) 

/*
 * Internal BCM Port Resource Configuration
 */
typedef struct bcm_apache_port_resource_s {
 ...
 */
STATIC int
_bcm_apache_port_resource_multi_set(int unit, 
                                    int nport, bcm_port_resource_t *resource);
STATIC int
_bcm_apache_port_resource_speed_set(int unit, bcm_port_t port, int speed);
...
static bcmi_xgs5_port_redirection_egr_port_table_t
            bcmi_apache_port_redirection_egr_port_table = {
    /* mem */ 	              EGR_PORTm,
...
/* HW Definitions */
static bcmi_xgs5_port_hw_defs_t  bcmi_apache_port_hw_defs;


/*
 * Function Vector
 */
static bcm_esw_port_drv_t bcm_apache_port_drv = {
    /* fn_drv_init                */ bcmi_apache_port_fn_drv_init,
    /* resource_set               */ bcmi_xgs5_port_resource_set,
    /* resource_get               */ bcmi_xgs5_port_resource_get,
    /* resource_multi_set         */ _bcm_apache_port_resource_multi_set,
    /* resource_speed_set         */ _bcm_apache_port_resource_speed_set,
...
bcmi_apache_port_fn_drv_init(int unit)
{
    ...
    sal_memset(&bcmi_apache_port_hw_defs, 0,
               sizeof(bcmi_apache_port_hw_defs));
    bcmi_apache_port_hw_defs.egr_port =
        &bcmi_apache_port_redirection_egr_port_table;

    /* Initialize Common XGS5 Port module */
    BCM_IF_ERROR_RETURN
        (bcmi_xgs5_port_fn_drv_init(unit, &bcm_apache_port_drv, NULL,
                            &bcmi_apache_port_hw_defs));
 ...
 */
STATIC int
_bcm_apache_port_attach(int unit, bcm_port_t port)
{
    int rv = BCM_E_NONE;

    BCM_IF_ERROR_RETURN(bcm_esw_td2p_flexport_port_attach(unit, port));

    rv = bcm_ap_cosq_default_llfc_profile_attach(unit, port);
    if (BCM_FAILURE(rv)) {
    ...
    BCM_IF_ERROR_RETURN(bcm_ap_cosq_flex_port_update( unit , port , TRUE));

    /* Setup CosQ hierarchy  */
    if (!IS_AP_HSP_PORT(unit, port)) {
        rv = _bcm_ap_port_lls_config_set(unit, port);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "Error: Unable to setup LLS sched tree "
                                  "unit=%d port=%d rv=%d\n"),
                       unit, port, rv));
            return rv;
        }
        if (soc_ap_is_skip_default_lls_creation(unit)) {
                SOC_IF_ERROR_RETURN(
                    soc_apache_port_mmu_tx_enable_set(unit, port, 0));
        }
        

    } else {
        rv = soc_apache_cosq_enable_hsp_sched(unit,port);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
                                  "Error: Unable to setup HSP sched tree "
                                  "unit=%d port=%d rv=%d\n"),
                       unit, port, rv));
            return rv;
        }

        rv = _bcm_ap_port_hsp_config_set(unit, port);
        if (BCM_FAILURE(rv)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
        ...
        if (soc_ap_is_skip_default_lls_creation(unit)) {
                SOC_IF_ERROR_RETURN(
                    soc_apache_port_mmu_tx_enable_set(unit, port, 1));
        }
 
    }

    rv = soc_apache_mmu_config_init(unit, 0, port);
 ...
 */
STATIC int
_bcm_apache_port_detach(int unit, bcm_port_t port)
{
    bcm_gport_t gport;
    int rv = BCM_E_NONE;
    ...
    rv = bcm_ap_cosq_llfc_profile_detach(unit, port);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
    ...
    if (IS_AP_HSP_PORT(unit, port)) {
        /* Clear HSP port bitmap */
        SOC_IF_ERROR_RETURN(soc_apache_cosq_disable_hsp_sched(unit, port));
    }
    ...
    rv = bcm_ap_cosq_gport_delete(unit, gport);
    if (BCM_FAILURE(rv)) {
        LOG_ERROR(BSL_LS_BCM_PORT,
                  (BSL_META_U(unit,
    ...
    BCM_IF_ERROR_RETURN(
    bcm_ap_cosq_flex_port_update( unit , port , FALSE));
...
/*
 * Function:
 *      bcm_apache_flexport_pbmp_update
 * Description:
...
int
bcm_apache_flexport_pbmp_update(int unit, bcm_pbmp_t *pbmp)
{
    bcm_port_t port;
    ...
    for (port = 0; port < AP_FLEX_MAX_NUM_PORTS; port++) {
...
/*
 * Function:
 *      _bcm_apache_port_resource_attach
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_attach(int unit,
                               int nport,
                               bcm_apache_port_resource_t *resource)
{
    int rv = BCM_E_NONE;
    bcm_apache_port_resource_t *pr;
    int i;
    int my_modid;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        if (pr->flags & SOC_PORT_RESOURCE_ATTACH) {
            rv = _bcm_apache_port_attach(unit, pr->port);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
...
/*
 * Function:
 *      _bcm_apache_port_resource_detach
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_detach(int unit,
                               int nport,
                               bcm_apache_port_resource_t *resource)
{
    int rv = BCM_E_NONE;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        if (pr->flags & SOC_PORT_RESOURCE_DETACH) {
            rv = _bcm_apache_port_detach(unit, pr->port);
            if (BCM_FAILURE(rv)) {
                LOG_ERROR(BSL_LS_BCM_PORT,
                          (BSL_META_U(unit,
                                      "Error: Unable to detach BCM port "
...
/*
 * Function:
 *      _bcm_apache_port_resource_resolve
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_resolve(int unit, 
                                  int nport,
                                  bcm_apache_port_resource_t *resource)
{
    int i;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
         ...
         */
        SOC_IF_ERROR_RETURN
            (soc_ap_port_addressable(unit, pr->port));

        /* Check that physical port is within the valid range */
        if (pr->physical_port != -1) {
            SOC_IF_ERROR_RETURN
                (soc_ap_phy_port_addressable(unit, pr->physical_port));
        }

        /* Check that ports, logical and physical, are not a management port */
        if ((si->port_l2p_mapping[pr->port] == AP_PHY_PORT_CPU) ||
            (si->port_l2p_mapping[pr->port] == AP_PHY_PORT_LB) ||
            (pr->physical_port == AP_PHY_PORT_CPU) ||
            (pr->physical_port == AP_PHY_PORT_LB)) {
            LOG_ERROR(BSL_LS_BCM_PORT,
                      (BSL_META_U(unit,
...
/*
 * Function:
 *      _bcm_apache_port_resource_input_validate
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_input_validate(int unit, 
                                         int nport,
                                         bcm_apache_port_resource_t *resource)
{
    int i;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
         ...
         * have linkscan mode set.  See _bcm_apache_port_resource_pmap_get().
         */
        if (pr->flags & SOC_PORT_RESOURCE_DETACH) {
...
/*
 * Function:
 *      _bcm_apache_port_resource_pmap_get
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_pmap_get(int unit, int nport,
                                   bcm_apache_port_resource_t *resource)
{
    int i;
    int j;
...
/*
 * Function:
 *      _bcm_apache_port_resource_op_get
 * Purpose:
 ...
 *      op       - (OUT) FlexPort operation BCM_AP_PORT_RESOURCE_OP_...
 * Returns:
 *      BCM_E_XXX
 * Note:
 *      Assumes caller has lock.
 ...
 */
STATIC int
_bcm_apache_port_resource_op_get(int unit, int nport,
                                bcm_apache_port_resource_t *resource, int *op)
{
    int i;
    bcm_apache_port_resource_t *pr;
    soc_info_t *si = &SOC_INFO(unit);
    ...
    *op = BCM_AP_PORT_RESOURCE_OP_NONE;
    ...
    for (i = 0, pr = &resource[0]; i < nport; i++, pr++) {
        ...
        if (pr->flags & SOC_PORT_RESOURCE_SPEED) {
             ...
             * _bcm_ap_port_resource_speed_execute will be called.
             */
            *op |= BCM_AP_PORT_RESOURCE_OP_SPEED;
            LOG_VERBOSE(BSL_LS_BCM_PORT,
                        (BSL_META_U(unit,
        ...
        if (pr->flags &
            ...
            *op |= BCM_AP_PORT_RESOURCE_OP_PMAP;
         ...
         */
        if (!(*op & BCM_AP_PORT_RESOURCE_OP_SPEED)) {
        ...
        /* Check port lanes */
        if (!(*op & BCM_AP_PORT_RESOURCE_OP_LANES)) {
            if (pr->lanes != si->port_num_lanes[pr->port]) {
                *op |= BCM_AP_PORT_RESOURCE_OP_LANES;
            }
        }

        /* Check port encap */
        if (!(*op & BCM_AP_PORT_RESOURCE_OP_ENCAP)) {
            BCM_IF_ERROR_RETURN(bcm_esw_port_encap_get(unit, pr->port, &encap));
            if (pr->encap != encap) {
                *op |= BCM_AP_PORT_RESOURCE_OP_ENCAP;
            }
        }

        /* If all are set, no more checking is necessary */
        if (*op == BCM_AP_PORT_RESOURCE_OP_ALL) {
            break;
        }
    }
...
/*
 * Function:
 *      _bcm_apache_port_resource_loopback_clear
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_loopback_clear(int unit,
                                         int nport,
                                         bcm_apache_port_resource_t *resource)
{
    int i;
...
/*
 * Function:
 *      _bcm_apache_port_resource_speed_execute
 * Purpose:
                                  ...
                                  int nport, bcm_apache_port_resource_t *resource)
{
    ...
    for (i = 0; i < nport; i++) {
        ...
        /* Use CL91 settings in case it is already enabled on the port */
        rv = bcmi_esw_portctrl_phy_control_get(unit, resource[i].port,
                                               BCM_PORT_PHY_CONTROL_FORWARD_ERROR_CORRECTION_CL91,
                                               &(soc_resource[i].cl91_enabled));
        if (BCM_FAILURE(rv)) {
            /* All PHYs don't support CL91 */
            soc_resource[i].cl91_enabled = 0;
        }
    }

    rv = soc_ap_port_resource_validate(unit, nport, soc_resource);
    if (SOC_FAILURE(rv)) {
    ...
    rv = soc_ap_port_resource_configure(unit, nport, soc_resource,
...
/*
 * Function:
 *      _bcm_apache_port_resource_execute
 * Purpose:
 ...
 */
STATIC int
_bcm_apache_port_resource_execute(int unit, 
                                  int nport, bcm_apache_port_resource_t *resource)
{
    int rv;
    int i;
    ...
    for (i = 0; i < nport; i++) {
        ...
        soc_resource[i].num_lanes = resource[i].lanes;
        soc_resource[i].encap = resource[i].encap;
        /* In case full flexport sequence is required assume cl91 is disabled.
         * If user wants to enable CL91 on new ports, it is expected that call to
         * bcmi_esw_portctrl_phy_control_set() will be followed by a speed_set()
         */
        soc_resource[i].cl91_enabled = 0;
    }
    ...
    rv = _bcm_apache_port_resource_detach(unit, nport, resource);
    ...
    if (BCM_SUCCESS(rv)) {
        rv = _bcm_apache_port_resource_attach(unit, nport, resource);
    }
...
/*
 * Function:
 *      _bcm_apache_port_resource_configure
 * Purpose:
 ...
 *      - It takes the internal bcm_apache_port_resource_t data type.
 */
STATIC int
_bcm_apache_port_resource_configure(int unit, 
                                    int nport,
                                    bcm_apache_port_resource_t *resource)
{
    int op;
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_resolve(unit,
                                         nport, resource));

    /* Get port mapping change */
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_pmap_get(unit, nport, resource));

    /* clear loopback */
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_loopback_clear(unit, nport, resource));
     ...
     */
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_input_validate(unit, nport, resource));
     ...
     */
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_op_get(unit, nport, resource, &op));
    ...
    LOG_VERBOSE(BSL_LS_BCM_PORT,
                (BSL_META_U(unit,
                 ...
                 op,
                 (op & BCM_AP_PORT_RESOURCE_OP_PMAP) ? "pmap" : "",
                 (op & BCM_AP_PORT_RESOURCE_OP_SPEED) ? "speed" : "",
                 (op & BCM_AP_PORT_RESOURCE_OP_LANES) ? "lanes" : "",
                 (op & BCM_AP_PORT_RESOURCE_OP_ENCAP) ? "encap" : ""));

    if (op == BCM_AP_PORT_RESOURCE_OP_NONE) {
     ...
     */
    if ((op & BCM_AP_PORT_RESOURCE_OP_PMAP) ||
        (op & BCM_AP_PORT_RESOURCE_OP_LANES)) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "--- Execute FlexPort sequence ---\n")));
        BCM_IF_ERROR_RETURN
            (_bcm_apache_port_resource_execute(unit,
                                           nport, resource));
        for (i = 0; i < nport; i++) {
    ...
    } else if (op & BCM_AP_PORT_RESOURCE_OP_SPEED) {
        LOG_VERBOSE(BSL_LS_BCM_PORT,
                    (BSL_META_U(unit,
                                "--- Execute Speed Set sequence ---\n")));
        BCM_IF_ERROR_RETURN
            (_bcm_apache_port_resource_speed_execute(unit,
                                                     nport, resource));
    }
...
/*
 * Function:
 *      _bcm_apache_port_resource_multi_set
 * Purpose:
 *      Modify the following port resources:
 *      - Logical to physical port mapping
 *      - Speed
 ...
 *      - It takes the public bcm_apache_port_resource_t data type.
 */
STATIC int
_bcm_apache_port_resource_multi_set(int unit, 
                                    int nport,
                                    bcm_port_resource_t *resource)
{
    int rv;
    int i;
    bcm_apache_port_resource_t *port_resource;
     ...
     */
    port_resource = sal_alloc(sizeof(bcm_apache_port_resource_t) * nport,
                              "port_resource");
    ...
    /* Clear data structure */
    sal_memset(port_resource, 0, sizeof(bcm_apache_port_resource_t) * nport);

    /* Copy initial information */
    for (i = 0; i < nport; i++) {
        /*
         * No current flags defined in bcm_apache_port_resource_t, otherwise
    ...
    rv = _bcm_apache_port_resource_configure(unit, nport, port_resource);
...
/*
 * Function:
 *      _bcm_apache_port_resource_speed_set
 * Purpose:
...
{
    soc_info_t *si = &SOC_INFO(unit);
    bcm_apache_port_resource_t resource[2]; /* resource for pre and post */
    ...
    BCM_IF_ERROR_RETURN(bcmi_apache_port_lanes_get(unit, port, &cur_lanes));
    ...
    BCM_IF_ERROR_RETURN(_bcm_apache_port_resource_configure(unit, 2, resource));
...
{
    soc_info_t *si = &SOC_INFO(unit);
    bcm_apache_port_resource_t resource[8];  /* Max size to support up to 4 lanes */
    int max_array_cnt;
    int i;
    int cur_lanes;
    int cur_speed;
    ...
    SOC_IF_ERROR_RETURN(soc_ap_port_lanes_valid(unit, port, lanes));
    ...
    case 1:
        if (cur_encap == BCM_PORT_ENCAP_IEEE) {    /* IEEE */
            speed = PORT_IS_PM4x25_PORT(unit, port) ? 25000: 10000;
        } else {                                   /* HG2  */
            speed = PORT_IS_PM4x25_PORT(unit, port) ? 27000: 11000;
        }
        break;
    case 2:
        if (cur_encap == BCM_PORT_ENCAP_IEEE) {    /* IEEE */
            speed = PORT_IS_PM4x25_PORT(unit, port) ? 50000: 20000;
        } else {                                   /* HG2  */
            speed = PORT_IS_PM4x25_PORT(unit, port) ? 53000: 21000;
        }

        break;
    case 4:
        if (cur_encap == BCM_PORT_ENCAP_IEEE) {    /* IEEE */
            speed = PORT_IS_PM4x25_PORT(unit, port) ? 100000: 40000;
        } else {                                   /* HG2  */
    ...
    BCM_IF_ERROR_RETURN
        (_bcm_apache_port_resource_configure(unit,
 ...
 */
STATIC int
_bcm_apache_port_drain_cells(int unit, int port)
{
    return bcmi_esw_portctrl_egress_queue_drain(unit, port);
}

int
bcm_apache_port_drain_cells(int unit, int port)
{
    return (_bcm_apache_port_drain_cells(unit, port));
}

#else /* BCM_APACHE_SUPPORT */
int bcm_esw_ap_port_not_empty;
