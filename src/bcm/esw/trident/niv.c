        ...
        uint16 virtual_interface_id, int vp, int allow_loop)
{
    ...
    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
            L3MC__VNTAG_Pf, (flags & BCM_NIV_EGRESS_P_BIT_CLEAR) ? 0 : 1);
    soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh, 
...
_bcm_trident_niv_match_info_get(
    ...
    bcm_vlan_t *match_vlan, uint16 *virtual_interface_id)
{
    if ((NIV_PORT_INFO(unit, vp)->flags & BCM_NIV_PORT_MATCH_NONE) &&
        ...
        int *egr_vt_added)
{
    ...
    *egr_vt_added = FALSE;
    ...
    if (!BCM_VLAN_VALID(match_vlan)) {
         ...
         */
        soc_EGR_VLAN_XLATEm_field32_set(unit, &vent, NEW_OVIDf, match_vlan);
        bcm_vlan_action_set_t_init(&action);
        ...
        bcm_vlan_t vid)
{
    ...
    if (!(NIV_PORT_INFO(unit, vp)->flags & BCM_NIV_PORT_MATCH_DO_NOT_ADD)) {
        ...
        if (BCM_GPORT_IS_TRUNK(gport)) {
            ...
            soc_VLAN_XLATEm_field32_set(unit, &key_data, VIF__MODULE_IDf,
                                        mod_out);
            soc_VLAN_XLATEm_field32_set(unit, &key_data, VIF__PORT_NUMf,
                                        port_out);
        }
        ...
        for (index = index_min; index <= index_max; index++) {
            ...
            /* Replace entry's new VLAN */
            soc_VLAN_XLATEm_field32_set(unit, vent, VIF__NEW_OVIDf, vid);
            cnt++;
        }
        ...
        rv = soc_mem_write_range(unit, VLAN_XLATEm, MEM_BLOCK_ALL, index_min,
                                 index_max, vt_buf);
        soc_cm_sfree(unit, vt_buf);
        soc_mem_unlock(unit, VLAN_XLATEm);
    }
    ...
    if (!(NIV_PORT_INFO(unit, vp)->flags & BCM_NIV_PORT_MATCH_DO_NOT_ADD)) {
        ...
        *vid = soc_VLAN_XLATEm_field32_get(unit, &entry_data, VIF__NEW_OVIDf);
        ...
        for (index = index_min; index <= index_max; index++) {
            ...
            /* Get entry's VLAN */
            *vid = soc_VLAN_XLATEm_field32_get(unit, vent, VIF__NEW_OVIDf);
            break;
        }
        soc_cm_sfree(unit, vt_buf);
        soc_mem_unlock(unit, VLAN_XLATEm);
...
_bcm_td2_niv_match_vp_replace(int unit, int vp, int new_vp, int *old_vp)
{
    ...
    /* Construct lookup key */
    sal_memset(&key_data, 0, sizeof(vlan_xlate_entry_t));
    _bcm_trident_niv_match_info_get(unit, vp, &port,
                                    &match_vlan, &virtual_interface_id);
    if (BCM_VLAN_VALID(match_vlan)) {
        BCM_IF_ERROR_RETURN(_bcm_esw_vlan_xlate_key_type_value_get(unit,
                    VLXLT_HASH_KEY_TYPE_VIF_VLAN, &key_type));
        soc_VLAN_XLATEm_field32_set(unit, &key_data, VIF__VLANf, match_vlan);
    } else {
        BCM_IF_ERROR_RETURN(_bcm_esw_vlan_xlate_key_type_value_get(unit,
                    VLXLT_HASH_KEY_TYPE_VIF, &key_type));
    }
    ...
    bcm_niv_egress_t *niv_egress_array)
{
    ...
    intf = BCM_XGS3_DVP_EGRESS_IDX_MIN + NIV_INFO(unit)->invalid_next_hop_index;
    ...
    for (vp = soc_mem_index_min(unit, SOURCE_VPm);
        ...
        if (_bcm_vp_used_get(unit, vp, _bcmVpTypeNiv)) {
            ...
            if (!(NIV_PORT_INFO(unit, vp)->flags & BCM_NIV_PORT_MULTICAST)) {
                ...
                if (dst_vif_match && (is_trunk == local_is_trunk)) {
                    if (is_trunk == 1) {
