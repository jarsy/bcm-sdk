...
STATIC int _bcm_td2_vxlan_egr_xlate_entry_get( int unit, int vfi, int vp, egr_vlan_xlate_entry_t *vxlate_entry);
...
bcm_td2_vxlan_vpn_create(int unit, bcm_vxlan_vpn_config_t *info)
{
    ...
    if (info->flags & BCM_VXLAN_VPN_WITH_VPNID) {
        ...
        if (info->flags & BCM_VXLAN_VPN_REPLACE) {
            ...
            rv = _bcm_td2_vxlan_egr_xlate_entry_get(unit, vfi_index, -1, &old_vxlate_entry);
            if (rv == BCM_E_NONE) {
...
STATIC int 
_bcm_td2_vxlan_egr_xlate_entry_get( int unit, int vfi, int vp, egr_vlan_xlate_entry_t *vxlate_entry)
{
    ...
    if (vp != -1) {
        soc_EGR_VLAN_XLATEm_field32_set(unit, &key_vxlate, VXLAN_VFI__DVPf, vp);
        soc_EGR_VLAN_XLATEm_field32_set(
            unit, &key_vxlate, ENTRY_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP);        
    } else {
        soc_EGR_VLAN_XLATEm_field32_set(
    ...
    if (!vxlan_info->vxlan_vpn_info[vfi].vxlan_vpn_type) {
        ...
        if (rv == BCM_E_NONE) {         
            ...
            rv = _bcm_td2_vxlan_match_vnid_entry_reset(
                     unit, vnid, vxlan_info->vxlan_vpn_info[vfi].vxlan_vpn_type);
            if ((rv != BCM_E_NONE) && (rv != BCM_E_NOT_FOUND)) {
                return rv;    
            }
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
...
bcm_td2_vxlan_tunnel_terminator_destroy_all(int unit) 
{
    ...
    if (!SOC_HW_RESET(unit)) {
        ...
        for (i = 0; i < nent; i++) {
            ...
            if (soc_VLAN_XLATEm_field32_get(unit, vtabp,
                        KEY_TYPEf) != _BCM_VXLAN_KEY_TYPE_LOOKUP_DIP) {
                continue;
            }
            /* Delete the entry from HW */
            rv = soc_mem_delete(unit, VLAN_XLATEm, MEM_BLOCK_ALL, vtabp);
            if (BCM_FAILURE(rv)) {
                soc_mem_unlock(unit, VLAN_XLATEm);
                soc_cm_sfree(unit, vtab);
                return rv;
              ...
              _bcm_td2_vxlan_nh_info_t  *egr_nh_info, int new_entry_type)
{
    ...
    if ((new_entry_type == _BCM_VXLAN_EGR_NEXT_HOP_SDTAG_VIEW) &&
        ...
        if (soc_mem_field_valid(unit, EGR_L3_NEXT_HOPm, L3MC__HG_MC_DST_PORT_NUMf) &&
            soc_mem_field_valid(unit, EGR_L3_NEXT_HOPm, L3MC__USE_MAC_DA_PROFILEf)) {
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                L3MC__HG_MC_DST_PORT_NUMf, 0);
            soc_mem_field32_set(unit, EGR_L3_NEXT_HOPm, &egr_nh,
                                L3MC__HG_MC_DST_MODIDf, 0);
                                             ...
                                             mpls_entry_entry_t *return_ment)
{
    ...
    soc_MPLS_ENTRYm_field32_set(unit, new_ment, VALIDf, 1);
    soc_MPLS_ENTRYm_field32_set(unit, new_ment, KEY_TYPEf, key_type);
    if (key_type == _BCM_VXLAN_KEY_TYPE_TUNNEL_VNID_VFI) {
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__SIPf);
        soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_VN_ID__SIPf, value);
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__VFIf);
        soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_VN_ID__VFIf, value);
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__VN_IDf);
        soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_VN_ID__VN_IDf, value);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__OVIDf);
            soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_VN_ID__OVIDf, value);
        }
#endif        
    } else {
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_SIP__SIPf);
        soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_SIP__SIPf, value);
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_SIP__SVPf);
        soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_SIP__SVPf, value);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_SIP__OVIDf);
            soc_MPLS_ENTRYm_field32_set(unit, new_ment, VXLAN_SIP__OVIDf, value);
        }
#endif        
    }
    return BCM_E_NONE;
...
STATIC int
_bcm_td2_vxlan_match_tunnel_entry_set(int unit, mpls_entry_entry_t *ment, mpls_entry_entry_t *return_ment)
{
    int rv = BCM_E_UNAVAIL;
    int index = 0;
    mpls_entry_entry_t tmp_ment;
    sal_memset(&tmp_ment, 0, sizeof(tmp_ment));
...
STATIC int
_bcm_td2_vxlan_match_tunnel_entry_delete(int unit, mpls_entry_entry_t *ment)
{
    int rv = BCM_E_UNAVAIL;
    uint32 value=0, key_type=0;
    mpls_entry_entry_t tmp_ment;
    ...
    /* Check if Key_Type identical */
    key_type = soc_MPLS_ENTRYm_field32_get (unit, ment, KEY_TYPEf);
    soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, KEY_TYPEf, key_type);

    soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, VALIDf, 1);
    if (key_type == _BCM_VXLAN_KEY_TYPE_TUNNEL_VNID_VFI) {
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__SIPf);
        soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, VXLAN_VN_ID__SIPf, value);
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__VN_IDf);
        soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, VXLAN_VN_ID__VN_IDf, value);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__OVIDf);
            soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, VXLAN_VN_ID__OVIDf, value);
        }
#endif
    } else {
        value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_VN_ID__SIPf);
        ...
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            value = soc_MPLS_ENTRYm_field32_get(unit, ment, VXLAN_SIP__OVIDf);
            soc_MPLS_ENTRYm_field32_set(unit, &tmp_ment, VXLAN_SIP__OVIDf, value);
        }
#endif
    }
...
STATIC int
_bcm_td2_vxlan_match_tunnel_entry_reset(int unit, mpls_entry_entry_t *ment, mpls_entry_entry_t *return_ment)
{
    int rv = BCM_E_UNAVAIL;
    int index = 0;   
    rv = soc_mem_search(unit, MPLS_ENTRYm, MEM_BLOCK_ANY, &index,
    ...
    if (rv == SOC_E_NONE) {
         rv = soc_mem_write(unit, MPLS_ENTRYm, MEM_BLOCK_ALL, index,
                            return_ment);
    }
    if ( (rv != BCM_E_NOT_FOUND) && (rv != BCM_E_NONE) ) {
       return rv;
...
_bcm_td2_vxlan_match_egr_vxlate_entry_set(int unit, bcm_vpn_t vpn, 
                                                         ...
                                                         egr_vlan_xlate_entry_t *return_vent)
{
    int rv = BCM_E_NONE;
    int index = -1;    
    rv = soc_mem_search(unit, EGR_VLAN_XLATEm,
                        MEM_BLOCK_ANY, &index, vent, return_vent, 0);
    if (BCM_E_NONE == rv) {
        BCM_IF_ERROR_RETURN(soc_mem_write(unit,
                                          EGR_VLAN_XLATEm, MEM_BLOCK_ALL, index, 
                                          vent));
    } else if (BCM_E_NOT_FOUND == rv) {
        if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
            return BCM_E_NOT_FOUND;
        ...
        BCM_IF_ERROR_RETURN(soc_mem_insert(unit,
                                          EGR_VLAN_XLATEm, MEM_BLOCK_ALL, vent));
    } else {
        return rv;
    }
    return BCM_E_NONE;
...
STATIC int
_bcm_td2_vxlan_match_egr_vxlate_entry_delete(int unit, egr_vlan_xlate_entry_t *ment)
{
    int rv = BCM_E_UNAVAIL;
    uint32 value = 0, entry_type = 0;
    egr_vlan_xlate_entry_t tmp_ment;
    sal_memset(&tmp_ment, 0, sizeof(tmp_ment));
    /* Check if Key_Type identical */
    entry_type = soc_EGR_VLAN_XLATEm_field32_get(unit, ment, ENTRY_TYPEf);
    soc_EGR_VLAN_XLATEm_field32_set(unit, &tmp_ment, ENTRY_TYPEf, entry_type);

    soc_EGR_VLAN_XLATEm_field32_set(unit, &tmp_ment, VALIDf, 1);
    if (entry_type == _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP) {
        value = soc_EGR_VLAN_XLATEm_field32_get(unit, ment, VXLAN_VFI__VFIf);
        soc_EGR_VLAN_XLATEm_field32_set(unit, &tmp_ment, VXLAN_VFI__VFIf, value);
        value = soc_EGR_VLAN_XLATEm_field32_get(unit, ment, VXLAN_VFI__DVPf);
        soc_EGR_VLAN_XLATEm_field32_set(unit, &tmp_ment, VXLAN_VFI__DVPf, value);
...
_bcm_td2_vxlan_match_add(int unit, bcm_vxlan_port_t *vxlan_port, int vp, bcm_vpn_t vpn)
{
    ...
    if (vxlan_port->criteria == BCM_VXLAN_PORT_MATCH_PORT_VLAN ) {
        ...
        mpls_entry_entry_t ment_svp;
        mpls_entry_entry_t ment_vnid;
        egr_vlan_xlate_entry_t ment_evt;
        mpls_entry_entry_t orignal_ment_svp;
        mpls_entry_entry_t orignal_ment_vnid;
        egr_vlan_xlate_entry_t orignal_ment_egr_vt;
        int tunnel_idx=-1;
        uint32 tunnel_sip;
        ...
        sal_memset(&orignal_ment_svp, 0, sizeof(orignal_ment_svp));
        sal_memset(&orignal_ment_egr_vt, 0, sizeof(orignal_ment_egr_vt));
        sal_memset(&orignal_ment_vnid, 0, sizeof(orignal_ment_vnid));
        sal_memset(&ment_svp, 0, sizeof(ment_svp));
        sal_memset(&ment_vnid, 0, sizeof(ment_vnid));
        sal_memset(&ment_evt, 0, sizeof(ment_evt));
        
        tunnel_sip = vxlan_info->vxlan_tunnel_term[tunnel_idx].sip;

        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VXLAN_SIP__SIPf, tunnel_sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
        if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
            VLAN_CHK_ID(unit, vxlan_port->match_vlan);
            vxlan_info->match_key[vp].match_vlan = vxlan_port->match_vlan;
            soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VXLAN_SIP__OVIDf,
                                        vxlan_port->match_vlan);
        }
#endif
        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VXLAN_SIP__SVPf, vp);
        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, KEY_TYPEf, _BCM_VXLAN_KEY_TYPE_TUNNEL);
        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VALIDf, 1);
        ...
        if (vxlan_vpn_type) {
            ...
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, ENTRY_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VALIDf, 0x1);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VXLAN_VFI__VFIf, vfi_index);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VXLAN_VFI__DVPf, vp);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VXLAN_VFI__VN_IDf, vxlan_port->vnid);
            /* Configure EGR_VLAN_XLATE - SD_TAG setting */
            if (vxlan_port->flags & BCM_VXLAN_PORT_REPLACE) {
            ...
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__SIPf, tunnel_sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
                VLAN_CHK_ID(unit, vxlan_port->match_vlan);
                vxlan_info->match_key[vp].match_vlan = vxlan_port->match_vlan;
                soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__OVIDf,
                                            vxlan_port->match_vlan);
            }
#endif
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__VFIf, vfi_index);
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__VN_IDf, vxlan_port->vnid);
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, KEY_TYPEf, _BCM_VXLAN_KEY_TYPE_TUNNEL_VNID_VFI);
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VALIDf, 1);   
            ...
            return rv;
match_add_clean2:
            if (soc_EGR_VLAN_XLATEm_field32_get(unit, &orignal_ment_egr_vt, VALIDf)) {
...
_bcm_td2_vxlan_match_delete(int unit,  int vp , bcm_vxlan_port_t vxlan_port, bcm_vpn_t vpn)
{
    ...
    mpls_entry_entry_t ment_vnid;
    egr_vlan_xlate_entry_t ment_evt;
    mpls_entry_entry_t original_ment_svp; 
    mpls_entry_entry_t original_ment_vnid;
    egr_vlan_xlate_entry_t original_ment_evt;    
    bcm_module_t mod_out = -1;
    bcm_port_t port_out = -1;
    int    gport_id =-1;
    uint8 vxlan_vpn_type = 0;
    ...
    if  (vxlan_info->match_key[vp].flags == _BCM_VXLAN_PORT_MATCH_TYPE_VLAN) {
        ...
        sal_memset(&ment_vnid, 0, sizeof(ment_vnid));
        sal_memset(&ment_evt, 0, sizeof(ment_evt));
        sal_memset(&original_ment_svp, 0, sizeof(original_ment_svp));
        sal_memset(&original_ment_vnid, 0, sizeof(original_ment_vnid));
        sal_memset(&original_ment_evt, 0, sizeof(original_ment_evt));

        tunnel_index = vxlan_info->match_key[vp].match_tunnel_index;
        
        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VXLAN_SIP__SIPf, 
        ...
        soc_MPLS_ENTRYm_field32_set(unit, &ment_svp, VALIDf, 1);
        BCM_IF_ERROR_RETURN(
            _bcm_td2_vxlan_match_tunnel_entry_reset(unit, &ment_svp, 
                                                    &original_ment_svp));  
        ...
        if (vxlan_vpn_type) {            
            ...
            /* set egr_vlan_xlate table */
            soc_EGR_VLAN_XLATEm_field32_set(
                unit, &ment_evt, ENTRY_TYPEf, _BCM_VXLAN_KEY_TYPE_LOOKUP_VFI_DVP);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VALIDf, 0x1);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VXLAN_VFI__VFIf, vfi_index);
            soc_EGR_VLAN_XLATEm_field32_set(unit, &ment_evt, VXLAN_VFI__DVPf, vp);
                    
            rv = soc_mem_search(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ANY, &index,
                                &ment_evt, &original_ment_evt, 0);
            if (BCM_FAILURE(rv)) {
                goto match_delete_clean1;
            }            
            sal_memset(&ment_evt, 0, sizeof(ment_evt));
            rv = soc_mem_write(unit, EGR_VLAN_XLATEm, MEM_BLOCK_ALL, index, &ment_evt);
            if (BCM_FAILURE(rv)) {
                goto match_delete_clean1;
            ...
            /* set mpls_entry table */
            soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__SIPf, 
                                        vxlan_info->vxlan_tunnel_term[tunnel_index].sip);
#if defined(BCM_TRIDENT2PLUS_SUPPORT)
            if (soc_feature(unit, soc_feature_vrf_aware_vxlan_termination)) {
                soc_MPLS_ENTRYm_field32_set(unit, &ment_vnid, VXLAN_VN_ID__OVIDf,
            ...
            rv = soc_mem_search(unit, MPLS_ENTRYm, MEM_BLOCK_ANY, &index,
                                &ment_vnid, &original_ment_vnid, 0);
            if (BCM_FAILURE(rv)) {
                goto match_delete_clean2;
            }

            sal_memset(&ment_vnid, 0, sizeof(ment_vnid));
            rv = soc_mem_write(unit, MPLS_ENTRYm, MEM_BLOCK_ALL, index, &ment_vnid);
            if (BCM_FAILURE(rv)) {
                goto match_delete_clean2;                
            }
...
match_delete_clean2:
        ...
        BCM_IF_ERROR_RETURN(rv);
match_delete_clean1:   
        rv = _bcm_td2_vxlan_match_tunnel_entry_set(unit, &original_ment_svp, &ment_svp);
        BCM_IF_ERROR_RETURN(rv);
        return rv;
    }
