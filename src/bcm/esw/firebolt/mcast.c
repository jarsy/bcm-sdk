    ...
    if (SOC_WARM_BOOT(unit)) {
...
#endif /* BCM_TRIUMPH3_SUPPORT */
        {
            ...
            if (SOC_IS_TRIDENT2PLUS(unit)) {
                /* Trident2 has a Max L2_ENTRY Table Size of 294911 and TD2Plus
                 * has double the entries, So for the DMA allocation to succeed
                 * we are reading the L2_Entries in 2 chuncks for TD2Plus.
                 */
                if ((index_max - index_min) > 294911) {
                    dma_iter = 2;
                    index_half = (index_max - index_min) / 2;
                    l2_tbl_sz = sizeof(l2x_entry_t) *  (index_half + 1);
                }
            }
#endif /* BCM_TRIDENT2PLUS_SUPPORT */
            ...
            for (iter = 0; iter < dma_iter; iter++) {
                ...
                if (SOC_IS_TRIDENT2PLUS(unit)) {
                    if (dma_iter == 2) {
                        if(iter == 0) {
                            index_max = index_half;
                        } else if (iter == 1) {
                            index_min = index_half + 1;
                            index_max = soc_mem_index_max(unit,
                                                          L2MC_L2MEM(unit));
                        }
                    } 
                }
