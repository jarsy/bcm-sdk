...
#if defined(BCM_HURRICANE3_SUPPORT)
    if (soc_reg_field_valid(unit, METER_ING_PORT_GRANr, METER_ING_PORT_GRANf)) {
        ...
        soc_reg_field_set(unit, METER_ING_PORT_GRANr, &regval,
                          METER_ING_PORT_GRANf, fval);
        SOC_IF_ERROR_RETURN(WRITE_METER_ING_PORT_GRANr(unit, regval));
    }
#endif /* BCM_HURRICANE3_SUPPORT */
                              ...
                              uint32 *kbits_sec, uint32 *kbits_burst)
{
    ...
    defined(BCM_GREYHOUND_SUPPORT)
        if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || 
            ...
            SOC_IS_HURRICANEX(unit) || SOC_IS_GREYHOUND(unit) ||
            SOC_IS_GREYHOUND2(unit)) {
            uint32 miscval, granularity = 3, flags = 0, fval = 0;
            if (soc_reg_field_valid(unit, MISCCONFIGr, ITU_MODE_SELf)) {
                BCM_IF_ERROR_RETURN(READ_MISCCONFIGr(unit, &miscval));
                if (soc_reg_field_get(unit, MISCCONFIGr,
                                             miscval, ITU_MODE_SELf)) {
...
#if defined(BCM_HURRICANE3_SUPPORT)
        if (soc_reg_field_valid(unit, METER_EGR_PORT_GRANr,
                                METER_EGR_PORT_GRANf)) {

            rv = READ_METER_EGR_PORT_GRANr(unit, &regval);
            if (BCM_FAILURE(rv)) {
                if (soc_feature(unit, soc_feature_ipmc_repl_freeze)) {
                    SOC_EGRESS_METERING_UNLOCK(unit);
                }
            ...
            fval = soc_reg_field_get(unit, METER_EGR_PORT_GRANr,
                                     regval, METER_EGR_PORT_GRANf);
            if (granularity == 5) {
                fval |= (1 << port);
            } else {
                fval &= ~(1 << port);
            }
            soc_reg_field_set(unit, METER_EGR_PORT_GRANr, &regval,
                              METER_EGR_PORT_GRANf, fval);
            rv = WRITE_METER_EGR_PORT_GRANr(unit, regval);
            if (BCM_FAILURE(rv)) {
                if (soc_feature(unit, soc_feature_ipmc_repl_freeze)) {
                    SOC_EGRESS_METERING_UNLOCK(unit);
                }
    ...
    defined(BCM_GREYHOUND_SUPPORT)
        if (SOC_IS_FIREBOLT2(unit) || SOC_IS_RAVEN(unit) || 
            ...
            if (soc_reg_field_valid(unit, METER_EGR_PORT_GRANr,
                ...
                SOC_IF_ERROR_RETURN(READ_METER_EGR_PORT_GRANr(unit, &regval));
                fval = soc_reg_field_get(unit, METER_EGR_PORT_GRANr,
                                         regval, METER_EGR_PORT_GRANf);
