...
defined(BCM_SABER2_SUPPORT)
        if (soc_feature(unit, soc_feature_timesync_v3) || 
            SOC_IS_TRIUMPH3(unit) || SOC_IS_TD2_TT2(unit) || SOC_IS_SABER2(unit)) {
            if (!SOC_IS_SABER2(unit)) {
            ...
            if (val == SOC_BLK_XLPORT) {
                if (SOC_IS_HURRICANE2(unit) || SOC_IS_TD2P_TT2P(unit)) {
                    SOC_IF_ERROR_RETURN(
                        ...
                        SOC_IS_GREYHOUND2(unit) || SOC_IS_TOMAHAWK(unit)) { 
                    SOC_IF_ERROR_RETURN(
                        soc_reg_field32_modify(
                            unit, XLPORT_MODE_REGr,
                            port, EGR_1588_TIMESTAMPING_MODEf,
                            (timestamping_mode ==
                            bcmTimesyncTimestampingMode48bit) ? 1: 0));
                } else if (SOC_IS_SABER2(unit) || SOC_IS_APACHE(unit)) {
                    SOC_IF_ERROR_RETURN(
                        soc_reg_field32_modify(
                            unit, XLPORT_MODE_REGr,
                            port, EGR_1588_TIMESTAMPING_MODEf,
                ...
                if(SOC_IS_TD2P_TT2P(unit)) {
                    SOC_IF_ERROR_RETURN(
                        soc_reg_field32_modify(
                            unit, CPORT_MODE_REGr,
                            port, EGR_1588_TIMESTAMPING_MODEf,
                            (timestamping_mode ==
                            bcmTimesyncTimestampingMode48bit) ? 0: 1));
                } else if (SOC_IS_APACHE(unit) || SOC_IS_TOMAHAWK(unit)) {
                    SOC_IF_ERROR_RETURN(
                        soc_reg_field32_modify(
                            unit, CLPORT_MODE_REGr,
                            port, EGR_1588_TIMESTAMPING_MODEf,
                            (timestamping_mode ==
                            bcmTimesyncTimestampingMode48bit) ? 1: 0));
                }
 ...
 * Parameters:
 *      unit                - (IN) bcm device
 *      timestamping_mode   - (IN/OUT) 32bit ot 48bit timestamping mode
 *
 * Returns:
 *      BCM_E_NONE - Success
 *      BCM_E_XXX
...
defined(BCM_TRIDENT2PLUS_SUPPORT) || defined(BCM_TOMAHAWK_SUPPORT)
    ...
    if (soc_feature(unit, soc_feature_timesync_v3) ||
        SOC_IS_KATANA2(unit) || SOC_IS_TRIUMPH3(unit) ||
        SOC_IS_SABER2(unit) || SOC_IS_TD2P_TT2P(unit) ||
        SOC_IS_APACHE(unit) || SOC_IS_TOMAHAWK(unit)) {
    ...
    if (SOC_IS_HURRICANE2(unit) || SOC_IS_GREYHOUND(unit) || 
        ...
        for(index=0;
            ...
            } else if (val == SOC_BLK_CLPORT || val == SOC_BLK_CLG2PORT) {
                if (SOC_IS_APACHE(unit) || SOC_IS_TOMAHAWK(unit)) {
                    SOC_IF_ERROR_RETURN(
