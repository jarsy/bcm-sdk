...
#define _BCM_MULTICAST_MTU_TABLE_OFFSET(unit) \
        ((SOC_IS_HURRICANEX(unit) || SOC_IS_GREYHOUND(unit) || \
          SOC_IS_METROLITE(unit) || SOC_IS_GREYHOUND2(unit)) ? 512 : 8192)
    ...
    if (q_count_decrease){
        for ( index  = 0; index <  port_count; index++){
            ...
            if (SOC_IS_KATANA2(unit) || SOC_IS_SABER2(unit)){
                BCM_IF_ERROR_RETURN(bcm_kt2_cosq_port_get(unit, q_array[index], &temp_port));
            } else
#endif
            {
                BCM_IF_ERROR_RETURN(bcm_kt_cosq_port_get(unit, q_array[index], &temp_port));
            }
#if defined (BCM_SABER2_SUPPORT)
            if (SOC_IS_SABER2(unit)){
                _bcm_sb2_ipmc_port_ext_queue_count_decrement(unit, mc_index, temp_port);
            } else
#endif
            {
