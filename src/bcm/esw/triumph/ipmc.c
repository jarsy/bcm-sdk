...
_tr_ipmc_enable(int unit, int enable)
{
    ...
    if (soc_feature(unit, soc_feature_lport_tab_profile)) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_lport_profile_field32_modify(unit, LPORT_PROFILE_LPORT_TAB,
                                               V4IPMC_ENABLEf, enable));
        BCM_IF_ERROR_RETURN
            (_bcm_lport_profile_field32_modify(unit, LPORT_PROFILE_LPORT_TAB,
                                               V6IPMC_ENABLEf, enable));
        BCM_IF_ERROR_RETURN
            (_bcm_lport_profile_field32_modify(unit, LPORT_PROFILE_LPORT_TAB,
                                               IPMC_DO_VLANf,
                                               (enable && do_vlan) ? 1 : 0));
...
_bcm_tr_ipmc_replace(int unit, bcm_ipmc_addr_t *ipmc, _bcm_l3_cfg_t *old_l3cfg)
{
    ...
    if (old_ipmc_index != ipmc->group) {
        rv = _bcm_tr_ipmc_l3entry_list_del(unit, old_ipmc_index,
                                           &l3cfg);
        ...
        rv = _bcm_tr_ipmc_l3entry_list_add(unit, ipmc->group,
                                           &l3cfg);
        if (BCM_FAILURE(rv)) {
            (void) _bcm_tr_ipmc_l3entry_list_add(unit, old_ipmc_index,
                                                &l3cfg);
            return rv;
        }
    } else {
