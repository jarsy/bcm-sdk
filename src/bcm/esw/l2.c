...
bcm_esw_l2_init(int unit)
{
    ...
    if (SOC_IS_XGS_FABRIC(unit) && !SOC_IS_TITAN2X(unit)) {
    ...
    if (flags & BCM_L2_REPLACE_DELETE) {
        ...
        if (flags & BCM_L2_REPLACE_NO_CALLBACKS) {
            ...
            if (fifo_modified) {
                if (SOC_REG_IS_VALID(unit, L2_MOD_FIFO_ENABLEr)) {
                    rv2 = soc_reg_field32_modify(unit, L2_MOD_FIFO_ENABLEr,
                            REG_PORT_ANY, L2_DELETEf, 1);
                } else {
                    if (soc_reg_field_valid(unit, AUX_ARB_CONTROLr,
                                        L2_MOD_FIFO_ENABLE_L2_DELETEf)) {
                        rv2 = soc_reg_field32_modify(unit, AUX_ARB_CONTROLr,
                                REG_PORT_ANY, L2_MOD_FIFO_ENABLE_L2_DELETEf, 1);
                    }
                }
...
_bcm_esw_l2_wb_alloc(int unit)
{
    ...
    /* TH does not support SW aging. Added only for warmboot upgrade purpose. */
    sw_aging_support = sw_aging_support || SOC_IS_TOMAHAWKX(unit);
...
#endif /* defined(BCM_TRIDENT_SUPPORT) */
    return BCM_E_NONE;
}
...
_bcm_esw_l2_wb_sync(int unit)
{
    ...
    if (sw_aging_support ||
        ...
        if (sw_aging_support) {
            /* aging time */
            BCM_IF_ERROR_RETURN(
                _bcm_esw_l2_age_timer_get(unit, &aging_time));
            sz = sizeof(int);
        ...
        if (scache_ptr != NULL) {
            if (sw_aging_support) {
                if (recovered_ver >= BCM_WB_VERSION_1_0) {
                    ...
                    BCM_IF_ERROR_RETURN(
                        _bcm_esw_l2_age_timer_set(unit, aging_time, aging_time > 0));
                    scache_ptr += sizeof(int);
                } else {
            ...
            if ((!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit)) && station_control) {
                ...
                if (recovered_ver >= BCM_WB_VERSION_1_1) {
                    ...
                    _bcm_tr_l2_station_entry_last_sid_set(unit, FALSE,
                                                          cur_last_sid);
#ifdef BCM_RIOT_SUPPORT
                    cur_last_sid = 0;
                    ...
                    _bcm_tr_l2_station_entry_last_sid_set(unit, TRUE,
                                                          cur_last_sid);
#endif /* BCM_RIOT_SUPPORT */
                } else {
                    /* Underlay L2 station entry - sid/prio */
