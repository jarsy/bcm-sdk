...
static uint32 flowcnt_scache_allocated_size=0;
...
#define FLOWCNT_SCACHE_SIZE_2_RECOVER (flowcnt_scache_allocated_size\
...
_bcm_flowcnt_ctrl_common_init(int unit, _bcm_esw_flowcnt_control_t *fc)
{
    ...
    _FLOWCNT_ALLOC(unit, fc->default_attr_mode_id, uint32,
...
_bcm_esw_flowcnt_cleanup(int unit)
{
    ...
    if (flowcnt_control[unit] != NULL) {
        ...
        if (fc->default_attr_mode_id != NULL) {
            _FLOWCNT_FREE(unit, fc->default_attr_mode_id, 0);
        }
...
_bcm_esw_flowcnt_init(int unit)
{
    ...
    soc_counter_extra_register(unit, _bcm_esw_flowcnt_callback);
    ...
    /* WARMBOOT related */
#ifdef BCM_WARM_BOOT_SUPPORT
    flowcnt_scache_allocated_size = 0;
    ...
    if (flowcnt_scache_allocated_size != local_scache_size) {
        if (!SOC_WARM_BOOT(unit)) {
            _bcm_esw_flowcnt_cleanup(unit);
            return BCM_E_INTERNAL;
        } else if (flowcnt_scache_allocated_size > local_scache_size){
            LOG_WARN(BSL_LS_BCM_FLEXCTR,
                     (BSL_META_U(unit,
    ...
    int group_index)
{
    ...
    d_profile_index = -1;
    d_profile_group = -1;
    if (fc->default_attr_mode_id[attr] != 0) {
        _bcm_esw_flowcnt_custom_mode_id_get(fc->default_attr_mode_id[attr],
                                            &d_profile_index, &d_profile_group);
        if ((d_profile_index == profile_index) &&
            (d_profile_group == group_index)) {
            fc->default_attr_mode_id[attr] = 0;
        }
    }
            ...
            uint32                *num_entries)
{
    ...
    _FLOWCNT_LOCK(fc);
    if (fc->default_attr_mode_id[attr] == 0) {
        _FLOWCNT_ALLOC(unit, attr_selectors, bcm_stat_group_mode_attr_selector_t,
                       num_selectors * 
                       sizeof(bcm_stat_group_mode_attr_selector_t),
        ...
        _bcm_esw_flowcnt_custom_mode_id_set(profile_index, profile_group,
                                            &fc->default_attr_mode_id[attr]);
         fc->default_attr_mode_id[attr] |= _BCM_FLOWCNT_MODE_ID_VALID;
        _FLOWCNT_FREE(unit, attr_selectors, 0);
    } else {
        default_attr_mode_id = fc->default_attr_mode_id[attr];
...
int _bcm_esw_flowcnt_sync(int unit)
{
    ...
    scache_group_mode_num_limit = FLOWCNT_SCACHE_SIZE_2_RECOVER / sizeof(uint32);
    ...
    for (i = 0; i < bcmStatGroupModeAttrMaxValue; i ++) {
        if (i >= scache_group_mode_num_limit) {
            break;
        }
        sal_memcpy(&flowcnt_wb_info->default_attr_mode_id[i],
                   &fc->default_attr_mode_id[i],
                   sizeof(uint32));
    }
...
_bcm_esw_flowcnt_warmboot_info_recover(int unit, uint8 *scache_ptr)
{
    ...
    if (flowcnt_scache_allocated_size == 0) {
         LOG_ERROR(BSL_LS_BCM_FLEXCTR,
                   (BSL_META_U(unit,
    ...
    scache_group_mode_num = FLOWCNT_SCACHE_SIZE_2_RECOVER / sizeof(uint32);
    scache_size += FLOWCNT_SCACHE_SIZE_2_RECOVER;
    flowcnt_wb_group_info = (_bcm_flowcnt_warmboot_group_info_t *)
                             (scache_ptr + scache_size);
    ...
    if (_bcm_esw_stat_sync_version_above_equal(unit,BCM_WB_VERSION_1_3)) {
        ...
        for (i = 0; i < bcmStatGroupModeAttrMaxValue; i ++){
            ...
            sal_memcpy(&fc->default_attr_mode_id[i],
                       &flowcnt_wb_info->default_attr_mode_id[i],
                       sizeof(uint32));
        }
