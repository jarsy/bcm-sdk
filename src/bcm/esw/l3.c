...
{
    int rv;
    int i, alloc_size, intf_count, old_intf_count, primary_count = 0;
    ...
    if (SOC_IS_XGS3_SWITCH(unit) &&
        ...
        if (soc_feature(unit, soc_feature_dgm) &&
            ...
            alternate_count = BCM_XGS3_L3_ALTERNATE_CNT_PTR(unit)
                              [ecmp_group_id - BCM_XGS3_MPATH_EGRESS_IDX_MIN];
            ...
            if (ecmp_member->flags & BCM_L3_ECMP_MEMBER_DGM_ALTERNATE) {
                ...
                for (i = ecmp_member_count - 1; i >= primary_count; i--) {
                    sal_memcpy(&ecmp_member_array[i + 1],
                               &ecmp_member_array[i],
                               sizeof(bcm_l3_ecmp_member_t));
                }
                sal_memcpy(&ecmp_member_array[primary_count], ecmp_member,
                           sizeof(bcm_l3_ecmp_member_t));
                primary_count++;
            }
        } else
        ...
        rv = _bcm_esw_l3_ecmp_create(unit, &ecmp_info, ecmp_member_count + 1,
                                     ...
                                     1, &ecmp_member->egress_if, primary_count);
        if (BCM_FAILURE(rv)) {
            goto clean_up;
        }
        ...
        int intf_count, bcm_if_t *intf_array)
{
    ...
    if ((soc_feature(unit, soc_feature_ecmp_resilient_hash) ||
        ...
        rv = _bcm_esw_l3_ecmp_create(unit, ecmp, intf_count, intf_array,
                BCM_L3_ECMP_MEMBER_OP_SET, 0, NULL, intf_count);
    }
