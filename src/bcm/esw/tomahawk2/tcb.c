...
#define _TH2_MMU_CELLS_TO_BYTES(cells)  cells * _TH_MMU_BYTES_PER_CELL
#define _TH2_MMU_BYTES_TO_CELLS(bytes)  (bytes + (_TH_MMU_BYTES_PER_CELL - 1)) \
...
_bcm_th2_cosq_tcb_pkt_order_fix(uint8 *pkt_data)
{
    ...
    for (i = 36; i >= 20; i -= 4) {
        for (j = 0; j < 4; j++) {
            temp8 = pkt_data[i + j];
            pkt_data[i + j] = pkt_data[36 - i + j];
            pkt_data[36 - i + j] = temp8;
        }
    }
                                ...
                                int num, bcm_cosq_tcb_buffer_t *record_entries)
{
    ...
    if (wrapped) {
        ...
        for (i = idx + 1, j = 0; j < num; i++, j++) {
            ...
            qnum -= _bcm_th2_cosq_tcb_ucq_offset[pipe];
            cosq = qnum % si->port_num_cosq[dgpp];
            ...
            record_entries[j].queue_size = _TH2_MMU_CELLS_TO_BYTES(depth);

            depth = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, SRVPOOL_DEPTHf);
            record_entries[j].service_pool_size = _TH2_MMU_CELLS_TO_BYTES(depth);
        }
    } else {
        ...
        for(i = 0; i < num; i++) {
            ...
            depth = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, Q_DEPTHf);
            record_entries[i].queue_size = _TH2_MMU_CELLS_TO_BYTES(depth);

            depth = soc_TCB_BUFFER_METADATAm_field32_get(unit, pmd, SRVPOOL_DEPTHf);
            record_entries[i].service_pool_size = _TH2_MMU_CELLS_TO_BYTES(depth);
        }
    }
                                 ...
                                 int num, bcm_cosq_tcb_event_t *record_entries)
{
    ...
    int idx;
    bcm_cos_queue_t cosq;
    int dgpp, sgpp, pipe, qnum, drop_reason = 0, drop_vector, depth;
    soc_info_t *si;
    ...
    for (idx = 0; idx < num; idx++) {
        ...
        record_entries[idx].queue_size = _TH2_MMU_CELLS_TO_BYTES(depth);

        depth = soc_TCB_EVENT_BUFFERm_field32_get(unit, ped, SRVPOOL_DEPTHf);
        record_entries[idx].service_pool_size = _TH2_MMU_CELLS_TO_BYTES(depth);
...
_bcm_th2_cosq_tcb_deinit(int unit)
{
    ...
    if (NULL != tcb_unit_info->tcb_evt_cb) {
        sal_free(tcb_unit_info->tcb_evt_cb);
        tcb_unit_info->tcb_evt_cb = NULL;
    }

    if (NULL != tcb_unit_info->tcb_evt_user_data) {
        sal_free(tcb_unit_info->tcb_evt_user_data);
                                     ...
                                     int *index)
{
    ...
    uint64 rval64;
    int start_threshold, stop_threshold, i, start_cell, stop_cell;
    ...
    start_threshold = _TH2_MMU_BYTES_TO_CELLS(threshold->start_threshold_bytes);
    stop_threshold = _TH2_MMU_BYTES_TO_CELLS(threshold->stop_threshold_bytes);
    if (start_threshold> _TCB_THRESHOLD_MAX || start_threshold <= 0 ||
        stop_threshold > _TCB_THRESHOLD_MAX || stop_threshold <= 0 ||
    ...
    if (flags & BCM_COSQ_TCB_THRESHOLD_PROFILE_REPLACE) {
        ...
        for (i = 0; i < _BCM_TH2_TCB_PROFILE_MAX_NUM; i++) {
            BCM_IF_ERROR_RETURN(soc_reg64_get(unit, TCB_THRESHOLD_PROFILEr,
                                              buffer_id, i, &rval64));
            start_cell = soc_reg_field_get(unit, TCB_THRESHOLD_PROFILEr, rval64,
                                           CAPTURE_START_THRESHOLDf);
            stop_cell = soc_reg_field_get(unit, TCB_THRESHOLD_PROFILEr, rval64,
                              ...
                              bcm_cosq_tcb_threshold_profile_t *threshold)
{
    ...
    SOC_IF_ERROR_RETURN(soc_reg64_get(unit, TCB_THRESHOLD_PROFILEr, buffer_id,
                                      index, &rval64));
    cells = soc_reg_field_get(unit, TCB_THRESHOLD_PROFILEr, rval64,
                              CAPTURE_START_THRESHOLDf);
    threshold->start_threshold_bytes = _TH2_MMU_CELLS_TO_BYTES(cells);
    cells = soc_reg_field_get(unit, TCB_THRESHOLD_PROFILEr, rval64,
                              CAPTURE_END_THRESHOLDf);
    threshold->stop_threshold_bytes = _TH2_MMU_CELLS_TO_BYTES(cells);
