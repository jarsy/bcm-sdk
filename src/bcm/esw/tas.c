...
{
    return _bcm_esw_port_gport_validate(unit, gport, local_port);
}
...
bcmi_tas_current_time(int unit, bcm_ptp_timestamp_t *current_time)
{
    ...
    if (soc_feature(unit, soc_feature_ptp)) {
        if (_bcm_ptp_running(unit) == BCM_E_NONE) {
            /* Suggested way to get the PTP time. */
...
#endif
    {
        /* If EAPP 1588 is not running, get the current time from register. */
        BCM_IF_ERROR_RETURN(READ_IEEE1588_TIME_SECr(unit, &sec));
        BCM_IF_ERROR_RETURN(READ_IEEE1588_TIME_FRAC_SEC_UPPERr(unit, &u_frac));
        BCM_IF_ERROR_RETURN(READ_IEEE1588_TIME_FRAC_SEC_LOWERr(unit, &l_frac));
        /*
         * Format seconds for NTP and 1588 time registers.
         * |B31|B30...B00|
         * B31: One second.
         * B00..B30: Upper seconds.
         */
        sec = (sec & 0x7FFFFFFF) << 1 | (sec >> 31);
        COMPILER_64_SET(current_time->seconds, 0, sec);
        current_time->nanoseconds = (u_frac << 6) | (l_frac >> 26);
    }
    return rv;
}
...
bcmi_tas_ns_to_ptp(uint64 ns, bcm_ptp_timestamp_t *ptp_time)
{
    ...
    nsec = COMPILER_64_LO(ns);
    nsec &= TAS_PTP_TIME_NANOSEC_MAX;
 ...
 *     And the remap would be device specific since the delay parmeters
    ...
    bcm_cosq_tas_entry_t **new_entries)
{
    ...
    BCM_IF_ERROR_RETURN(
        bcm_esw_port_preemption_status_get(unit, gport,
                                           bcmPortPreemptStatusTx, &status));
    if (status == bcmPortPreemptStatusTxActive) {
        preemption_enable = 1;
        BCM_IF_ERROR_RETURN(
            bcm_esw_port_preemption_control_get(
                unit, gport, bcmPortPreemptControlQueueBitmap, &preempt_bmp));
    }
    ...
    for (i = 1; i < num_entries; i++) {
        ...
        if (n_tas_entry->ticks == BCM_COSQ_TAS_ENTRY_TICKS_GO_FIRST) {
            ...
            if (preemption_enable) {
                /*
                 * When preemption is enabled, any expess queue is open, the phold
                 * should be 1.
                 */
 ...
 *      In the callback function, the profile state should be changed to committed.
 * Parameters:
 *     unit              - (IN) unit number
 *     lport             - (IN) logical port
    ...
    bcmi_tas_profile_event_type_t event)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_TAS,
              (BSL_META_UP(unit, lport,
                           ...
                           "active profile(pid 0x%x) state = %s "
                           "pending profile (pid 0x%x) state = %s \n"),
               event, active_pid,
    ...
    void *fn_data)
{
    ...
    LOG_DEBUG(BSL_LS_BCM_TAS,
              (BSL_META_UP(unit, lport,
                           "bcmi_tas_commit_callback pid %d state %s\n"),
    ...
    for (i = 0; i < tc->profile_list_count; i++) {
        ...
        if (process_pid) {
            ...
            if ((tp_process->hw_index != -1) &&
                (tp_process->hw_index != config_idx)) {
                /* hw_index != config_idx means the process pid become active */
