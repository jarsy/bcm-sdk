...

{
    ...
    if (BCM_GPORT_IS_TRUNK(endpoint_info->gport)) {
        ...
        BCM_IF_ERROR_RETURN
            (_bcm_esw_trunk_active_member_get(unit, *trunk_id, NULL, 0, NULL, &member_count));
        if (0 == member_count) {
                              ...
                              bcm_bfd_endpoint_info_t *endpoint_info)
{
    ...
    if (is_micro_bfd) {
        endpoint_config->is_micro_bfd = 1;
    }
    ...
    if (BCM_GPORT_INVALID != endpoint_info->tx_gport) {
        BCM_IF_ERROR_RETURN
        (bcmi_xgs5_bfd_endpoint_gport_resolve(unit, 1, endpoint_info, &module_id,
            &port_id, &trunk_id, &local_id, &is_trunk_bfd, &is_micro_bfd));
        /* Get local port used for TX BFD packet */
        BCM_IF_ERROR_RETURN
            (_bcm_esw_modid_is_local(unit, module_id, &is_local));
...
bcmi_xgs5_bfd_scache_alloc(int unit)
{
    ...
    /* Calculate the size of egress_label */
    alloc_size += (sizeof(bcm_mpls_egress_label_t) * bfd_info->endpoint_count);
...
bcmi_xgs5_bfd_sync(int unit)
{
    ...
    do {
        ...
        for (idx = 0; idx < bfd_info->endpoint_count; idx++) {
             ...
             sal_memcpy(bfd_scache, &endpoint_config->info.egress_label, sizeof(bcm_mpls_egress_label_t));
             bfd_scache += sizeof(bcm_mpls_egress_label_t);
    ...
    for (idx = 0; idx < bfd_info->endpoint_count; idx++) {
        ...
        if (endpoint_config) {
            ...
            if (pkt_vlan_id) {
                ...
                endpoint_config->info.flags |= BCM_BFD_ENDPOINT_USE_PKT_VLAN_ID;
            }
            ...
            if (recovered_ver >= BCM_WB_VERSION_1_2) {
                ...
                *bfd_scache += sizeof(bcm_mpls_label_t);
                ...
                sal_memcpy(&endpoint_config->info.egress_label, *bfd_scache, sizeof(bcm_mpls_egress_label_t));
                *bfd_scache += sizeof(bcm_mpls_egress_label_t);
...
bcmi_xgs5_bfd_reinit(int unit)
{
    ...
    if (!SOC_WARM_BOOT_SCACHE_IS_LIMITED(unit) && (stable_size > 0)) {
        ...
        /* In BCM_WB_VERSION_1_1 onwards we allocate memory for storing pkt_vlan_id
        ...
        if (recovered_ver < BCM_WB_VERSION_1_2) {
            ...
            realloc_size += (sizeof(bcm_mpls_egress_label_t) * bfd_info->endpoint_count);
