 ...
 */
#define _BCM_SAT_GLP_TRUNK_BIT_GET(_glp_) (0x1 & ((_glp_) >> 15))
#define _BCM_SAT_GLP_TRUNK_ID_GET(_glp_)  (0xFF & (_glp_))
#define _BCM_SAT_GLP_MODULE_ID_GET(_glp_) (0xFF & ((_glp_) >> 7))
#define _BCM_SAT_GLP_PORT_GET(_glp_)      (0x7F & (_glp_))
...
bcm_sb2_sat_init(int unit)
{
    ...
    SOC_IF_ERROR_CLEAN_RETURN((soc_phyctrl_detach(0, port)),
            _bcm_sb2_sat_control_free(unit, satc));
...
_bcm_sb2_sat_wb_upmep_endpoints_recover(int unit)
{
    ...
    for (index = 0; index < satc->upsamp_ep_count; index++) {
        ...
        if (soc_EGR_SAT_SAMP_TCAMm_field32_get(unit, &entry, VALIDf)) {
            ...
            if(_BCM_SAT_GLP_TRUNK_BIT_GET(sglp)) {
                gport_dest.tgid = _BCM_SAT_GLP_TRUNK_ID_GET(sglp);
                gport_dest.gport_type = _SHR_GPORT_TYPE_TRUNK;
            } else {
                gport_dest.gport_type = _SHR_GPORT_TYPE_MODPORT;
                gport_dest.modid = _BCM_SAT_GLP_MODULE_ID_GET(sglp);
                gport_dest.port = _BCM_SAT_GLP_PORT_GET(sglp);
            }
            ...
            _bcm_gport_dest_t_init(&gport_dest);
            if(_BCM_SAT_GLP_TRUNK_BIT_GET(dglp)) {
                gport_dest.tgid = _BCM_SAT_GLP_TRUNK_ID_GET(dglp);
                gport_dest.gport_type = _SHR_GPORT_TYPE_TRUNK;
            } else {
                gport_dest.gport_type = _SHR_GPORT_TYPE_MODPORT;
                gport_dest.modid = _BCM_SAT_GLP_MODULE_ID_GET(dglp);
                gport_dest.port = _BCM_SAT_GLP_PORT_GET(dglp);
            }
            rv = _bcm_esw_gport_construct(unit, &gport_dest, 
