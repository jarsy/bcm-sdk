...
#define _BCM_L3_SOURCE_BIND_IP6_COMPRESSION_SAV(unit, port) ((_BCM_PPD_IS_PON_PP_PORT(port)) && (IS_PON_PORT(unit, _BCM_PPD_GPORT_PON_TO_PHY_PORT(unit, port))))
  ...
  if(info->l3a_flags2 &  BCM_L3_FLAGS2_SCALE_ROUTE){
      if (SOC_IS_JERICHO(unit)) {
          ...
          if(ARAD_KBP_ENABLE_IPV4_DC && ARAD_KBP_IPV4DC_24BIT_FWD) {
	          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_l3_host_add is not supported for IPv4 DC 24bit FWD; Use bcm_l3_route_add instead")));
          }
#endif
...
#if defined(INCLUDE_KBP) && !defined(BCM_88030)
          if(ARAD_KBP_ENABLE_IPV4_DC && ARAD_KBP_IPV4DC_24BIT_FWD) {
	          BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("bcm_l3_host_find is not supported for IPv4 DC 24bit FWD; Use bcm_l3_route_get instead")));
          }
#endif
          host_key.route_scale = 1;
      }else{
  ...
  )
{
    ...
    for (i=0; i<tables_num; i++) {
        ...
        if (is_table_initiated) {
            ...
            ret = soc_ppd_frwrd_ipv4_uc_route_iterator_get_next(unit, table_ids[i], 0/*flags*/, qual_vals, payload, NULL/*priority*/, NULL/*hit_bit*/, &found);
            if (soc_sand_get_error_code_from_error_word(ret) != SOC_SAND_OK)
            {
              LOG_ERROR(BSL_LS_BCM_L3,
                        (BSL_META_U(unit,
    ...
    bcm_if_t *intf_id)
{
    ...
    uint8 routing_enablers_bm_id_tmp = 0;
    ...
    if (if_id != SOC_PPC_RIF_NULL) {
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_l3_rif_mac_termination_combination_profile_get(unit, rif_info.routing_enablers_bm, &routing_enablers_bm_id_tmp));
        ...
        if (SOC_IS_JERICHO(unit)) {
            ...
            if (soc_property_get(unit, spn_BCM886XX_L3_INGRESS_URPF_ENABLE, 0)) {
                /* Each time a profile is created with any new set of enablers an additional profiles are allocated
                 * and some of them are with RPF but loose (unless it RPF strict).
                 * As this is a strict RPF configuration the loose profile that match this profile enablers won't work
                 * and a new profile allocation needed, the routing_enablers_bm is modified for the template manager
                 * to allocate a new profile for the strict RPF if none exists. */
                if ((rif_info.uc_rpf_enable == TRUE) && (rif_info.uc_rpf_mode == SOC_PPC_FRWRD_FEC_RPF_MODE_UC_STRICT)) {
            ...
            ret = _bcm_dpp_am_template_l3_rif_mac_termination_combination_exchange(unit, routing_enablers_bm_id_tmp, &old_is_last, &rif_info.routing_enablers_bm, &routing_enablers_bm_id_tmp, &rif_info.is_configure_enabler_needed);
            BCMDNX_IF_ERR_EXIT(ret);
            if (old_is_last) { /* Need to write all 0 in the old allocated HW */               
    ...
    void *user_data)
{
    ...
    if (flags & BCM_L3_IP6) {
        ...
        if (SOC_IS_JERICHO(unit) && !(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_ipv6_uc_use_tcam", 0))) {
            status = _bcm_ppd_frwrd_ipv6_route_traverse(unit, trav_fn, user_data);
        } else 
#endif /*defined(INCLUDE_KBP) && !defined(BCM_88030)*/
        {
		...
		if (SOC_IS_JERICHO(unit) && !(ARAD_KBP_ENABLE_IPV4_MPLS_EXTENDED)) {
            status = _bcm_ppd_frwrd_ipv4_route_traverse(unit, trav_fn, user_data);
        } else 
#endif /*defined(INCLUDE_KBP) && !defined(BCM_88030)*/
        {
