    ...
    int unit)
{
    ...
    if (soc_property_get(unit, spn_MPLS_EGRESS_LABEL_ENTROPY_INDICATOR_ENABLE, 0) && SOC_IS_JERICHO_PLUS(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, \
    ...
    if (SOC_IS_JERICHO(unit) && soc_property_get(unit, spn_EVPN_ENABLE, 0)
        ...
        /*This program is loaded only in the following conditions with EVPN_ENABLE defined:
              * 1 - device is jericho and ROO_EXTENSION_LABEL_ENCAPSULATION is defined
              * 2 - device is jericho_plus and MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE is defined
              */
        if ((soc_property_get(unit, spn_ROO_EXTENSION_LABEL_ENCAPSULATION, 0) && !SOC_IS_JERICHO_PLUS(unit))
                           || (soc_property_get(unit, spn_MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE, 0) && SOC_IS_JERICHO_PLUS(unit))) {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_EVPN,
    ...
    if (SOC_IS_JERICHO(unit) && (!SOC_IS_JERICHO_PLUS(unit)) &&
        ...
        if (soc_property_get(unit, spn_MPLS_EGRESS_LABEL_EXTENDED_ENCAPSULATION_MODE, 0)) {
            rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION,
                                                                  ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_ELI_JER, 1 /*value*/,
                                                                  1/*outlif-profile1*/);
            BCMDNX_IF_ERR_EXIT(rv);
            for (i = 1; i <= 4; i++) {
                rv = arad_egr_prog_editor_prog_sel_outlif_profile_single_entry_set(unit, SOC_OCC_MGMT_OUTLIF_APP_ADDITIONAL_LABEL,
                                                                                   ARAD_EGR_PROG_EDITOR_PROG_MPLS_EXTENDED_ELI_JER, i/*entry*/,
                                                                                   1 /*value*/,i/*outlif-profile1*/);
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
    ...
    if ( (SOC_DPP_CONFIG(unit))->pp.routing_preserve_dscp & SOC_DPP_DSCP_PRESERVE_OUTLIF_CTRL_EN
        ...
        rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_PRESERVE_DSCP,
                                                              ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_JER, 1 /*value*/,
                                                              SOC_IS_JERICHO_PLUS(unit)?4:1/*outlif-profile1*/);
        BCMDNX_IF_ERR_EXIT(rv);
        rv = arad_egr_prog_editor_prog_sel_outlif_profile_set(unit, SOC_OCC_MGMT_OUTLIF_APP_PRESERVE_DSCP,
                                                              ARAD_EGR_PROG_EDITOR_PROG_PRESERVING_DSCP_JER_OAMTS, 1 /*value*/,
                                                              SOC_IS_JERICHO_PLUS(unit)?4:1/*outlif-profile1*/);
    ...
    if ((_BCM_PPD_IS_VALID_FAILOVER_ID(mpls_port->failover_id) && _BCM_MULTICAST_IS_SET(mpls_port->failover_mc_group)) ||
        ...
        if (phy_port.type == _bcmDppGportResolveTypeForwardFec) {
            ...
            if ((is_egress) &&(next_eep[0])) {
                BCM_L3_ITF_SET(mpls_port->egress_tunnel_if, BCM_L3_ITF_TYPE_LIF, next_eep[0]);
            }
        }
    }
    ...
    uint8 is_second_mpls_label_in_entry)
{
    ...
    /* Entropy label */
    if (SOC_IS_JERICHO_PLUS(unit) && (label_entry->flags & BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE)) {
    ...
    int unit)
{
    ...
    additional_label_profile_info.label_type = 1;
    ...
    additional_label_profile_info.exp = 0;
    additional_label_profile_info.ttl = 0;
    additional_label_profile_info.label_type = 0;
    ...
    bcm_mpls_egress_label_t * label_array)
{
    ...
    if (!SOC_IS_QAX(unit)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("4 MPLS label push/swap  is supported with only one QAX above chip")));
    }
    ...
    ) /* needed only in case intf is of type EEP*/
{
    ...
    for (indx = 0; indx < num_labels; ++indx) {
        ...
        if (label_array[indx].flags & (BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE)) {
            ...
            if(!SOC_IS_JERICHO_PLUS(unit) && indx) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("Entropy Label Indication flag is only allowed on BOS label,"
                                                           " label array flags 0x%x label array index %d"),label_array[indx].flags,indx));                
            }
        }
        ...
        if ((label_array[indx].flags & (BCM_MPLS_EGRESS_LABEL_ACTION_VALID)) && (label_array[indx].action & BCM_MPLS_EGRESS_ACTION_SWAP_OR_PUSH)) {
            if (!(SOC_IS_JERICHO_B0(unit) || SOC_IS_QMX_B0(unit) || SOC_IS_JERICHO_PLUS_A0(unit) || SOC_IS_QUX(unit))) {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("BCM_MPLS_EGRESS_ACTION_SWAP_OR_PUSH action suppoted only on Jericho_B0, QMX_B0, Jericho Plus and QUX devices")));
            }
    ...
    for (start_indx = num_labels; start_indx > 0; start_indx = start_indx - 1 - skip_entries) {
        ...
        if(!eep_alloced) {
             ...
             */
            if ((indx+is_bind_entries+1 == num_labels) && BCM_L3_ITF_TYPE_IS_LIF(label_array[indx+is_bind_entries].l3_intf_id)) { /* tunnel points to tunnel */
            ...
            if (SOC_IS_JERICHO(unit))
            {
                ...
                &&SOC_IS_QAX(unit) && next_eep_is_mpls_with_data){
                ...
                if (counting_profile_is_allocated) {
                    if (label_array->outlif_counting_profile != BCM_STAT_LIF_COUNTING_PROFILE_NONE && label_array->outlif_counting_profile >= SOC_TMC_CNT_LIF_COUNTING_NOF_PROFILES(unit)) {
                        BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("OutLIF-counting-profile %d is out of range."), label_array->outlif_counting_profile));
                    }
                    BCMDNX_IF_ERR_EXIT(bcm_dpp_counter_lif_local_profile_get(unit, label_array->outlif_counting_profile, &local_out_lif_info.counting_profile_id));                    
                } else {
                    local_out_lif_info.counting_profile_id = BCM_STAT_LIF_COUNTING_PROFILE_NONE;
                }
        ...
        if (label_action_type == _bcm_dpp_mpls_egress_action_type_swap) { /* if swap */
            ...
            if (SOC_IS_JERICHO(unit)) {
                ...
                if ((!SOC_IS_JERICHO_PLUS(unit)) &&
                    (label_array[indx].flags & BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE) &&
                    soc_property_get(unit, spn_MPLS_EGRESS_LABEL_ENTROPY_INDICATOR_ENABLE, 0)) {
					/* Set outlif profile ELI bit to select the PRGE program that generates ELI */
                    soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_set,
                                      (unit, SOC_OCC_MGMT_TYPE_OUTLIF, SOC_OCC_MGMT_OUTLIF_APP_MPLS_ENTROPY_LABEL_INDICATION, 1,
                                          &(mpls_encap_info.outlif_profile)));
                    BCMDNX_IF_ERR_EXIT(soc_sand_rv);
                }
                    ...
                    if (SOC_IS_JERICHO(unit)) {
                        ...
                        if ((!SOC_IS_JERICHO_PLUS(unit)) &&
                            ...
                            soc_sand_rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_occ_mgmt_app_get,(unit, SOC_OCC_MGMT_TYPE_OUTLIF, \
                                                                                              ...
                                                                                              &(encap_entry_info[indx].entry_val.mpls_encap_info.outlif_profile),&val));
                            BCMDNX_IF_ERR_EXIT(soc_sand_rv);
                            if (val && (SOC_IS_JERICHO_PLUS(unit) || (!SOC_IS_JERICHO_PLUS(unit) && !cur_indx))) { /* if ELI bit is on, add the relevant flag */
                                label_array[*label_count].flags |= BCM_MPLS_EGRESS_LABEL_ENTROPY_INDICATION_ENABLE;
                            }
                        }
    ...
    bcm_mpls_tunnel_switch_t * info)
{
    ...
    /* ilm value */
    /* has to be FEC + EEI of type MPLS or FEC+ Outlif */
...
int _bcm_gport_mpls_is_valid_replace(int unit, bcm_mpls_port_t *new_mpls_port, bcm_mpls_port_t *old_mpls_port, bcm_mpls_label_t old_egress_vc, uint32 old_push_profile){
    ...
    if ((!n_is_protected) && (!(new_mpls_port->flags2 & BCM_MPLS_PORT2_EGRESS_ONLY)) && 
        ...
        (new_mpls_port->egress_label.label != old_egress_vc) && 
    ...
    if ((!n_is_protected) && (!(new_mpls_port->flags2 & BCM_MPLS_PORT2_EGRESS_ONLY)) && 
                (!(new_mpls_port->flags & BCM_MPLS_PORT_COUNTED)) && (!(new_mpls_port->flags2 & BCM_MPLS_PORT2_LEARN_ENCAP)) && 
        (((new_push_profile != old_push_profile) && (new_mpls_port->egress_label.label != BCM_MPLS_LABEL_INVALID)) || 
        (rv != BCM_E_NONE))) {
    ...
    bcm_mpls_port_t *   mpls_port)
{
   ...
   _bcm_lif_type_e used_type;    
    ...
    if (mpls_port->egress_tunnel_label.flags & BCM_MPLS_EGRESS_LABEL_ACTION_VALID) {
        ...
        if (mpls_port->egress_tunnel_label.label > 0xfffff) {
