...
{
    int rv = BCM_E_NONE;
    uint32 valid_flags = BCM_STAT_COUNTER_CLEAR_CONFIGURATION | BCM_STAT_COUNTER_CACHE | BCM_STAT_COUNTER_SYMMETRICAL_MULTICORE_ENGINE | BCM_STAT_COUNTER_IGNORE_COLORS | BCM_STAT_COUNTER_IGNORE_DISPOSITION | BCM_STAT_COUNTER_MULTIPLE_SOURCES_PER_ENGINE;
    int index, offset;
    ...
    if(engine->engine_id >= (SOC_DPP_DEFS_GET(unit, counters_per_counter_processor)))
    {
    ...
    if (engine->flags & BCM_STAT_COUNTER_MULTIPLE_SOURCES_PER_ENGINE) {
        if (!SOC_IS_QAX(unit) || config->source.engine_source != bcmStatCounterSourceIngressOam) { 
             BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("unit %d have flags for multiple sources but not in QAX device or not at engine source IngressOam! engine source %d\n"),unit, config->source.engine_source ));
        }
    ...
    bcm_stat_counter_lif_counting_range_t *lif_range)
{
    ...
    rv = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_lif_counting_range_set, (unit, counter_source, range_id, &range, 0, 0));
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_stat_lif_counting_t *lif_counting)
{
    ...
    rv = bcm_dpp_counter_lif_range_set(unit, lif_counting_profile, source, 
        lif_counting->source.command_id, lif_counting->source.stif_counter_id, lif_counting->range.is_double_entry, &range);
    BCMDNX_IF_ERR_EXIT(rv);
...
int bcm_petra_stat_counter_get(
    ...
    BCMDNX_IF_ERR_EXIT(bcm_petra_field_stat_multi_get(
        unit, stat_id, stat_counter_input_data->nstat, stat_counter_input_data->stat_arr, output_value_array));
