...
int _bcm_ip_tunnel_sw_init(int unit) 
{
    ...
    BCMDNX_IF_ERR_EXIT(_bcm_dpp_am_template_ip_tunnel_init(unit,-1,0xFFFFFFFF,-1,0xFFFFFFFF,-1,0xFFFFFFFF, -1, &ip_tunnel_size_protocol_template_val));
    ...
    int eep_alloced)
{
    ...
    /* eep_alloced tells if this is new Tunnel or not, 1 for old */
    rv = _bcm_dpp_am_template_ip_tunnel_exchange_test(unit, eep, !eep_alloced, tunnel->sip, ttl_encoded, tos_encoded,
                                                      &sip_template, &ttl_template, &tos_template, &is_allocated);
    BCMDNX_IF_ERR_EXIT(rv);
    ...
    bcm_tunnel_initiator_t *tunnel)
{
    ...
    tunnel->aux_data = encap_entry_info[0].entry_val.data_info.data_entry[0];
    tunnel->udp_dst_port = encap_entry_info[0].entry_val.data_info.data_entry[1];
   ...
   /* check if this tunnel is allocated */
    ...
    if (!is_ipv6_tunnel) {
        ...
        if (SOC_IS_JERICHO_B0_AND_ABOVE(unit) && (SOC_JER_PP_EG_ENCAP_IP_TUNNEL_SIZE_PROTOCOL_TEMPLATE_ENABLE == 1)) {
            ...
            rv = bcm_dpp_am_template_ip_tunnel_encapsulation_mode_exchange(
               unit,
               (encap_entry_info[0].entry_val.ipv4_encap_info.dest.encapsulation_mode_index), /* current encapsulation mode value */
               &data,
