...
int _bcm_oam_default_profile_get(int unit,
   ...
   if (is_accelerated) {
      BCM_OAM_OAMP_DESTINATION_TRAP_CODE_GET(
         is_upmep, classifier_mep_entry->mep_type, profile_data->mep_profile_data.opcode_to_trap_code_multicast_map[SOC_PPC_OAM_ETHERNET_PDU_OPCODE_CCM]);
   }
    ...
    if (destination != BCM_GPORT_INVALID) {
        ...
        *trap_code = BCM_GPORT_TRAP_GET_ID(destination);
        ...
        if (is_upmep) {
            ...
            BCMDNX_IF_ERR_EXIT(rv);
            if (is_equal) {
                rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_oamp_rx_trap_codes_set, (unit, classifier_mep_entry->mep_type, *trap_code));
                BCMDNX_IF_ERR_EXIT(rv);
            }
        }
    }
