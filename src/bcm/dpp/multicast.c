    ...
    bcm_if_t *cud_array)
{
    ...
    if (rep_array != NULL) { /* The input replications are in the structure we need */
        ...
        if (nof_replications > BCM_DPP_MULTICAST_SMALL_ARRAY_SIZE) {
            BCMDNX_IF_ERR_EXIT(dcmn_alloc_mem(unit, &reps, sizeof(soc_multicast_replication_t) * nof_replications, "ing_reps")); /* Allocate ingress replicaitons */
        } else {
...
exit:
    if (reps && reps != small_reps_array && reps != (soc_multicast_replication_t *)rep_array) {
        BCMDNX_IF_ERR_REPORT(dcmn_free_mem(unit, (void*)&reps));
    }
...
exit:
    if (dest_arr && dest_arr != small_dest_array) {
        BCMDNX_IF_ERR_REPORT(dcmn_free_mem(unit, (void*)&dest_arr));
    }
    if (cud_array && cud_array != small_cud_array) {
        BCMDNX_IF_ERR_REPORT(dcmn_free_mem(unit, (void*)&cud_array));
    }
    BCMDNX_FUNC_RETURN;
}
...
exit:
    if (dest_info_arr && dest_info_arr != small_dests_array) {
        BCMDNX_IF_ERR_REPORT(dcmn_free_mem(unit, (void*)&dest_info_arr));
    }
    if (cud_array && cud_array != small_cud_array) {
        BCMDNX_IF_ERR_REPORT(dcmn_free_mem(unit, (void*)&cud_array));
    }
