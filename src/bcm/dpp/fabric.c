                                       ...
                                       bcm_fabric_bandwidth_profile_t *profile_array)
{
  ...
  BCMDNX_NULL_CHECK(profile_array);
  {
    ...
    if (profile_count > (SOC_DPP_DEFS_GET(unit, nof_fabric_links) + 1) * SOC_DPP_DEFS_GET(unit, nof_rci_levels)  || profile_count < 0) {
        ...
        if (ftmh_info.action_ing != SOC_TMC_TDM_ING_ACTION_NO_CHANGE) {
            if (ftmh_info.is_mc) {
                ...
                if (SOC_IS_DPP_TDM_OPTIMIZE(unit)) {
                    ...
                    /* Get local Base MODID */
                    SOCDNX_IF_ERR_EXIT( MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_mgmt_system_fap_id_get, (unit, &base_modid)));
                    /* Get core from MODID */
                    core=SOC_DPP_MODID_TO_CORE(unit, base_modid, modid);
                    /* Get Local port */
                    rc=MBCM_DPP_DRIVER_CALL(unit, mbcm_dpp_tm_to_local_port_get,(unit, core, ftmh_info.ftmh.opt_uc.dest_if, &local_port));
                    BCMDNX_IF_ERR_EXIT(rc);
