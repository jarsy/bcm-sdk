...
_bcm_petra_lif_get_frwrd_decision(SOC_PPC_LIF_ENTRY_INFO *lif_info,SOC_PPC_FRWRD_DECISION_INFO **frwrd_decision){
    ...
    else if(lif_info->type == SOC_PPC_LIF_ENTRY_TYPE_AC || lif_info->type == SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP ||
    ...
    bcm_vswitch_flexible_connect_fwd_t *port_fwd)
{
    ...
    if (!found) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Match port not found")));
    }
