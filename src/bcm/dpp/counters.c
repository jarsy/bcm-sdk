    ...
    if (!((base + num - 1) < (unitData->proc[proc].mode.num_counters)) && !((base >= base_limit_down) && ((base + num - 1) < base_limit_up)))
    {
        ...
        if ((base < unitData->proc[proc].mode.num_counters) || ((base >= base_limit_down) && (base < base_limit_up)))
        {
            if ( ((proc+1) < (SOC_DPP_DEFS_GET(unit, nof_counter_processors))) && 
                (unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_ING_PP || unitData->proc[proc].mode.src_type == SOC_TMC_CNT_SRC_TYPE_EPNI) && (num == 5) )
            {
 ...
 * Parameters:
 *      unit -  unit number
 *      engines_range_per_core_and_cmd_id - two dimention array of ranges, per command and core
 * Returns:
 *      SOC_E_XXX
 * Notes:
 ***********************************************************************************************************************/
            ...
            {      
                BCM_DPP_COUNTER_SOURCE_MATCH(unitData, crps_index, core_id, SOC_TMC_CNT_SRC_TYPE_EPNI, command_id, SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF)                
                {
                    ...
                    end_engine_counter = ((unitData->proc[crps_index].mode.we_val + 1)*
                                            SOC_TMC_NOF_COUNTERS_IN_ENGINE(unit, unitData->proc[crps_index].proc_id) - 1)*
                                            ((unitData->proc[crps_index].mode.format == SOC_TMC_CNT_FORMAT_PKTS_AND_PKTS) ? 2 : 1);
                    /* find engine that its range match to the start range of the profile */
                    base = range->start*unitData->proc[crps_index].mode.custom_mode_params.set_size;
...
bcm_dpp_counter_lif_range_set(
   ...
   SOC_SAND_U32_RANGE   *range
    ...
    int range_id, base, size, temp, in_lif_profile;
    ...
    {
        BCMDNX_IF_ERR_EXIT(bcm_dpp_counter_out_lif_engine_range_check(unitData, command_id, is_double_entry, range));               
    }

    result = MBCM_DPP_SOC_DRIVER_CALL(unit, mbcm_dpp_cnt_lif_counting_range_set, (unit, source, command_id, range, stif_counter_id, is_double_entry));        
    BCMDNX_IF_ERR_EXIT(result);
        ...
        if (unitData->proc[index].mode.src_type == SOC_TMC_CNT_SRC_TYPE_OAM) {
            if (unitData->proc[index].mode.command_id == 0) {
                ...
                if (SOC_IS_ARADPLUS_AND_BELOW(unitData->cunit)) {
                    ...
                    if (index == prev_oam_eng_proc) { /* if it is actually the first oam engine*/
                        continue;
                    }
                    if (index != (prev_oam_eng_proc + 1)) {
                        LOG_ERROR(BSL_LS_BCM_CNT,
                                  (BSL_META_U(unit,
                                              "unit %d Both OAM EG counter engines must be consecutive. Your configuration {%d,%d}"),
                                   unit, prev_oam_eng_proc , index));
                        result = BCM_E_CONFIG;
                        break;
                    }
                ...
                {
                    if (index == prev_oam_ing_proc) { /* if it is actually the first oam engine*/
                        continue; 
                    }
                    if (index != (prev_oam_ing_proc + 1)){
                        LOG_ERROR(BSL_LS_BCM_CNT,
                                  (BSL_META_U(unit,
                                              "unit %d Both OAM ING counter engines must be consecutive. Your configuration {%d,%d}"),
                                   unit, prev_oam_ing_proc, index));
                        result = BCM_E_CONFIG;
                        break;
                    }
...
int
_bcm_dpp_counter_egress_receive_tm_shift_valid_bits(_bcm_dpp_counter_state_t *unitData,
                                                    int max_bit,
                                                    int value_to_shift,
                                                    int mask,
                                                    unsigned int *valid_bits_result){
    int unit = unitData->cunit;
    int i, temp_bit, bit_mask, temp_mask, temp_value;
    int bit_count = 0;
    int result = 0;
    BCMDNX_INIT_FUNC_DEFS;
    temp_mask = mask;
    temp_value = value_to_shift;
    for(i=0;i<max_bit;i++) {
        /* get lsb*/
        bit_mask = temp_mask % 2; 
        temp_bit = temp_value % 2;
        if(bit_mask != 0) { /* when mask bit is 0 we don't need this bit*/
            result = result + ((bit_mask & temp_bit) * soc_sand_power_of_2(bit_count));
            bit_count++;
        }
        temp_mask = temp_mask / 2;
        temp_value = temp_value / 2;
    }
    *valid_bits_result = result;
    BCMDNX_FUNC_RETURN;
}
int
_bcm_dpp_counter_egress_receive_tm_build_pointer(_bcm_dpp_counter_state_t *unitData,
    ...
    int rv=BCM_E_NONE;
    int unit = unitData->cunit;
    unsigned int temp_lval, temp_qp, temp_tc, temp_cast;
    ...
    _bcm_dpp_counter_egress_receive_tm_shift_valid_bits(unitData,8,((out_port_field + cosq) & pointer_format.queue_pair_mask),pointer_format.queue_pair_mask,&temp_qp);
    _bcm_dpp_counter_egress_receive_tm_shift_valid_bits(unitData,3,(priority & pointer_format.traffic_class_mask),pointer_format.traffic_class_mask,&temp_tc);
    temp_cast = is_mc & pointer_format.cast_mask;

    temp_lval = (temp_qp << queue_pair_shift) | (temp_tc  << traffic_class_shift) | (temp_cast << cast_shift);
    *lval = temp_lval;
...
_bcm_dpp_modify_proc_and_index(_bcm_dpp_counter_state_t * unitData, unsigned int *proc, unsigned int * finalIndex, uint8 subCount)
{
    ...
    /* modification of the ptr and proc should be made only if the source is ING_PP/EPNI and the group size=5 and the we_val !=0 */
    if ((unitData->proc[*proc].mode.src_type != SOC_TMC_CNT_SRC_TYPE_ING_PP && unitData->proc[*proc].mode.src_type != SOC_TMC_CNT_SRC_TYPE_EPNI) || (groupSize != 5))
    {
        SOC_EXIT;
    }
    ...
    )
{
    ...
    switch(counter_source_type)
    {
        case bcmStatCounterSourceIngressInlif:
            src_type = SOC_TMC_CNT_SRC_TYPE_ING_PP;
            eg_type = SOC_TMC_CNT_MODE_EG_TYPE_OUTLIF;
            if(_SHR_GPORT_IS_SET(counter_source_id) == FALSE)
            {
                BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("counter_source_id=%d, must be HPORT for bcmStatCounterSourceIngressInlif\n"),counter_source_id));   
            }
            BCMDNX_IF_ERR_EXIT(bcm_petra_port_encap_local_get(unit, counter_source_id, BCM_PORT_ENCAP_LOCAL_INGRESS, &cntr));
            break;            
        case bcmStatCounterSourceEgressTransmitOutlif: 
            ...
            if(counter_source_gport != -1) {
                ...
                {
                    BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("counter_source_gport=%d, must be HPORT for bcmStatCounterSourceEgressTransmitOutlif\n"),counter_source_gport));   
                }           
