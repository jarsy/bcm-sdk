                                    ...
                                    bcm_mirror_destination_t *mirror_dest, uint8 set_dest)
{
    ...
    if ((flags = mirror_dest->flags & ~(BCM_MIRROR_DEST_WITH_ID | BCM_MIRROR_DEST_REPLACE | BCM_MIRROR_DEST_UPDATE_POLICER | BCM_MIRROR_DEST_DEST_MULTICAST |
    ...
    for (action_profile_id = DPP_MIRROR_ACTION_NDX_MIN; action_profile_id <= DPP_MIRROR_ACTION_NDX_MAX; ++action_profile_id) {
        ...
        if (is_created) {
            ...
            if (info.cmd.meter_ptr_low.enable) { /* lower bank meter is used */
                mirror_dest.policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, info.cmd.meter_ptr_low.value, 0, 0);
            } else if (info.cmd.meter_ptr_up.enable) { /* upper bank meter is used */
                mirror_dest.policer_id = _DPP_POLICER_ID_FROM_METER_GROUP_CORE(unit, info.cmd.meter_ptr_up.value, 1, 0);
            }
