    ...
    int arg)
{
    ...
    if (is_global) {
        ...
        case bcmLbControlSegmentationMode:
            ...
            BCM_LB_VALUE_CHECK(arg, bcmLbSegmentMode128, (bcmLbSegmentModeCount - 1), "lbg_id");

            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_segment_config_set,(unit, arg));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
    ...
    int *arg)
{
    ...
    if (is_global) {
        ...
        case bcmLbControlSegmentationMode:
            ...
            _BCM_LB_SYSTEM_LOCK(unit);
            soc_rc = MBCM_DPP_DRIVER_CALL(unit,mbcm_dpp_lb_egr_segment_config_get,(unit, &segment_mode));
            _BCM_LB_SYSTEM_UNLOCK(unit);
            if (SOC_FAILURE(soc_rc)) {
