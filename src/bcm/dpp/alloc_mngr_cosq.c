...
STATIC int 
   _bcm_dpp_am_cosq_e2e_pool_init(int unit, int core, int region_index, int pool_index, int region_type, _dpp_res_type_cosq_t *dpp_res_cosq_type,
                                  bcm_dpp_am_cosq_pool_ref_type_t pool_type, int is_interdigitaded, int is_odd_even) 
{
    int rv = BCM_E_NONE;
    ...
    if (region_index > (SOC_DPP_DEFS_GET(unit, flow_region_fixed_type0_end)) - 1) {
        ...
        rv = _bcm_petra_flow_region_config(unit, core, region_index, region_index, is_interdigitaded, is_odd_even); 
        BCMDNX_IF_ERR_EXIT(rv);
    }
