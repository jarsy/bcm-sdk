...
int _bcm_oam_profile_single_opcode_set(int unit,
   ...
   if (BCM_OAM_ACTION_GET(*action, bcmOAMActionUcFwdAsData)) {
      ...
      /* Set trap strength to default for default trap codes */
      profile_data->opcode_to_trap_strength_unicast_map[opcode] = _ARAD_PP_OAM_TRAP_STRENGTH;
   }
   if (BCM_OAM_ACTION_GET(*action, bcmOAMActionMcFwdAsData)) {
      ...
      /* Set trap strength to default for default trap codes */
      profile_data->opcode_to_trap_strength_multicast_map[opcode] = _ARAD_PP_OAM_TRAP_STRENGTH;
   }
...
STATIC int _bcm_dpp_oam_outlif_table_set(int unit, SOC_PPC_LIF_ID local_lif, bcm_oam_endpoint_type_t ep_type, uint8 value) {
    ...
    case bcmOAMEndpointTypeBHHMPLS:
        ...
        soc_ppd_eg_encap_mpls_entry_add(unit, local_lif, &encap_entry_info->entry_val.mpls_encap_info, next_eep[0]);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
 ...
 */
STATIC int _bcm_dpp_endpoint_counters_set(int unit, bcm_oam_endpoint_info_t *endpoint_info, SOC_PPC_LIF_ID inlif, SOC_PPC_LIF_ID outlif) {
    int rv;
    uint64 stat;
    ...
    if (endpoint_info->lm_counter_base_id > 0) {
        ...
        rv = _bcm_dpp_oam_lif_table_set(unit, inlif, 1);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        rv = _bcm_dpp_oam_outlif_table_set(unit, outlif, type, 1);
        BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_oam_init_48B_MAID_Entries(int unit) {
   ...
   flex_ver_info.crc_info.mask.lsbyte_mask[2] = 0xFFFFFFE0; /* 27 bytes not masked, 5 bytes masked */
   flex_ver_info.crc_info.mask.lsbyte_mask[3] = 0x003FFFFF; /* 2 bytes masked, 21 bytes not masked, 1 bit not in use - hardware bug! */
   ...
   flex_ver_info.crc_tcam_info.crc_select = 0; /* Use the CHECK_CRC_VALUE_1 field in the OAMP_MEP_DB_EXT_DATA_HEADER entry */
   ...
   flex_ver_info.crc_tcam_info.crc_select = 1; /* Use the CHECK_CRC_VALUE_2 field in the OAMP_MEP_DB_EXT_DATA_HEADER entry */
   rv = MBCM_PP_DRIVER_CALL(unit, mbcm_pp_oam_bfd_flexible_verification_set, (unit, &flex_ver_info));
   BCMDNX_IF_ERR_EXIT(rv);
...
int _bcm_oam_write_group_data_to_oamp_mep_db(int unit, bcm_oam_group_info_t *group_info) {
   ...
   _BCM_OAM_OAMP_CRC_BUFFER CRC_calc_buff;
   int i;
   ...
   sal_memset(&CRC_calc_buff, 0, sizeof(CRC_calc_buff));

   CRC_Calc_Src = (group_info->flags & BCM_OAM_GROUP_RX_NAME) ? group_info->rx_name : group_info->name;

   for (i = 0; i < 6; i++) {
       /* First batch of bytes in reverse order */
       CRC_calc_buff[i] = CRC_Calc_Src[5 - i];
   }

   for (i = 0; i < 10; i++) {
       /* Second batch */
       CRC_calc_buff[22 + i] = CRC_Calc_Src[15 - i];
   }

   for (i = 0; i < 5; i++) {
       /* Third batch - array elements [21] is left blank because of hardware bug */
       CRC_calc_buff[16 + i] = CRC_Calc_Src[20 - i];
   }

   for (i = 0; i < 16; i++) {
       /* Fourth batch */
       CRC_calc_buff[32 + i] = CRC_Calc_Src[36 - i];
   }

   for (i = 0; i < 11; i++) {
       /* Fifth batch */
       CRC_calc_buff[53 + i] = CRC_Calc_Src[47 - i];
   }
   ...
   /* Apparently, the skip is inconsistent - calculate the second CRC! */
   for (i = 0; i < 6; i++) {
       /* Third batch - this time with no skip! */
       CRC_calc_buff[16 + i] = CRC_Calc_Src[21 - i];
   }

   for (i = 0; i < 16; i++) {
       /* Fourth batch */
       CRC_calc_buff[32 + i] = CRC_Calc_Src[37 - i];
   }

   for (i = 0; i < 10; i++) {
       /* Fifth batch */
       CRC_calc_buff[54 + i] = CRC_Calc_Src[47 - i];
   }
   CRC_calc_buff[53] = 0;

   _bcm_dpp_oam_bfd_flexible_verification_expected_crc16_get(CRC_calc_buff, &CRC_Calc_Mask, &Calculated_CRC16);
   maid_CRC_info.crc_info.crc16_val2 = Calculated_CRC16;


   if ((group_info->flags & BCM_OAM_GROUP_REPLACE) == 0) {
       /* Reserve the necessary entries */
       for (i = 0; i < 3; i++, Entry_Index += 0x400) {
           rv = _bcm_dpp_oam_mep_id_alloc(unit, SHR_RES_ALLOC_WITH_ID, 0, 0, &Entry_Index);
           BCMDNX_IF_ERR_EXIT(rv);
       }
   }

   /* Overwrite OAMP DB extended data entries */
   sal_memcpy(oamp_db_group_info.data, group_info->name, 48);
                                                  ...
                                                  void *user_data)
{
    ...
    int global_in_lif, local_out_lif;
    uint32 flags;
    int rv;
    ...
    if ((endpoint_info->type == bcmOAMEndpointTypeBHHMPLS || endpoint_info->type == bcmOAMEndpointTypeBHHPwe || endpoint_info->type == bcmOAMEndpointTypeBHHPweGAL || endpoint_info->type == bcmOAMEndpointTypeMPLSNetwork) &&
        ((endpoint_info->flags2 & BCM_OAM_ENDPOINT2_MPLS_INGRESS_ONLY) || endpoint_info->mpls_out_gport != BCM_GPORT_INVALID)) {
   ...
   if (SOC_WARM_BOOT(unit)) {
    ...
    if (!is_bfd_init) {
     ...
     rv = _bcm_oam_trap_and_mirror_profile_set(unit,
                                               ...
                                               (SOC_IS_ARADPLUS_AND_BELOW(unit) ? SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP : SOC_PPC_TRAP_CODE_OAM_CPU_SNOOP_UP),
...
STATIC int _bcm_oam_endpoint_local_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl) {
    ...
    if ((endpoint_info_lcl->gport == BCM_GPORT_INVALID) && ((endpoint_info_lcl->flags2 & BCM_OAM_ENDPOINT2_MPLS_EGRESS_ONLY) == 0)) {
        if (!is_accelerated)  {
            BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS,
        ...
        if (SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MODE(unit) && !SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit)) {
        ...
        for (is_passive = 0; is_passive <= 1; is_passive++) {
            ...
            /*don't need to create active profile if Egress only flag enable*/
            if(SOC_IS_JERICHO(unit) && (endpoint_info_lcl->flags2 & BCM_OAM_ENDPOINT2_MPLS_EGRESS_ONLY) && !is_passive){
                continue;
            }
        ...
        if (count_disabled_counter_id != endpoint_info_lcl->lm_counter_base_id) {
            ...
            if (is_upmep) {
                rv = _bcm_dpp_endpoint_counters_set(unit, endpoint_info_lcl, classifier_mep_entry.passive_side_lif, classifier_mep_entry.lif);
            } else {
                rv = _bcm_dpp_endpoint_counters_set(unit, endpoint_info_lcl, classifier_mep_entry.lif, classifier_mep_entry.passive_side_lif);
            }
        }
        BCMDNX_IF_ERR_EXIT(rv);

        /*don't create active profile if Egress only enable*/
        ...
        soc_sand_rv = soc_ppd_oam_classifier_oem_mep_add(unit, endpoint_info_lcl->id, &classifier_mep_entry, update_profile ? 2 : 0 /*update*/);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
...
STATIC int _bcm_oam_endpoint_default_create(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl,
	...
	if (SOC_IS_ARAD_B1_AND_BELOW(unit)) { /* Arad default profile */
        ...
        rv = _bcm_dpp_endpoint_counters_set(unit, endpoint_info_lcl, _BCM_OAM_INVALID_LIF, _BCM_OAM_INVALID_LIF);
        BCMDNX_IF_ERR_EXIT(rv);
...
STATIC int _bcm_oam_endpoint_local_replace(int unit, bcm_oam_endpoint_info_t *endpoint_info_lcl,
    ...
    if (is_accelerated) { /* TX - OAMP */
       ...
       if ( (SOC_IS_QAX(unit)) && 
        ...
        if (SOC_IS_ARADPLUS(unit)) {
             ...
             *        (OR) 
             * In case of QAX(and above), CCM_CNT might need to be moved from 0 to 0xfffff
    ...
    if (endpoint_info_lcl->gport != BCM_GPORT_INVALID || _BCM_OAM_IS_SERVER_SERVER(endpoint_info_lcl)
...
int bcm_petra_oam_endpoint_get(int unit, bcm_oam_endpoint_t endpoint, bcm_oam_endpoint_info_t *endpoint_info) {
    ...
    if (_BCM_OAM_MEP_INDEX_IS_REMOTE(endpoint)) {
        ...
        if ((classifier_mep_entry.lif != _BCM_OAM_INVALID_LIF)||(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY)) { /* Indication that RX does not exist */
        ...
        if (_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_ACCELERATED(&classifier_mep_entry)) {
            ...
            /* intf_id is invalid for unidirection mep */
            if(((classifier_mep_entry.mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP)|| (classifier_mep_entry.mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE)
                || (classifier_mep_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE_GAL)|| (classifier_mep_entry.mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1711_MPLS) || (classifier_mep_entry.mep_type ==SOC_PPC_OAM_MEP_TYPE_Y1711_PWE) ||
               ...
               &&(classifier_mep_entry.passive_side_lif == _BCM_OAM_INVALID_LIF)){
                  endpoint_info->intf_id = BCM_GPORT_INVALID;
            }
            ...
            if (SOC_IS_QAX(unit) && (classifier_mep_entry.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) &&
               (_BCM_OAM_DISSECT_IS_CLASSIFIER_MEP_UPMEP(&classifier_mep_entry) == 0)) {
...
int bcm_petra_oam_endpoint_destroy (int unit, bcm_oam_endpoint_t endpoint) {
    ...
    if (is_default) {
        ...
        /* local endpoint */
            ...
            if ((mep_info.lif != _BCM_OAM_INVALID_LIF) || (mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY)) {
                /* accelerated profile */
                if ((SOC_PPD_OAM_IS_CLASSIFIER_ADVANCED_MULTIPLE_MEPS_ON_LIF_MODE(unit)
                ...
                for (is_passive = 0; is_passive <= 1; is_passive++) {
                   ...
                   if(!(mep_info.flags&SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY)){
                ...
                if((mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_EGRESS_ONLY)==0) {
                    rv = _bcm_oam_mep_exist_on_lif_and_direction(unit,
                                                                 mep_info.lif,
                    ...
                    if (!found) { /* Serch meps on the other directions */
                        /*If ingress only, don't need to do this */
                    ...
                    if (!found) { /* No more meps on this lif */
                        ...
                        if(mep_info.passive_side_lif != _BCM_OAM_INVALID_LIF) {
                            ...
                            mep_type = (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_ETH_OAM) ? bcmOAMEndpointTypeEthernet :
                                       ...
                                       ((mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) ||(mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1711_PWE))? bcmOAMEndpointTypeBHHPwe :                                                           (mep_info.mep_type == SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP_SECTION) ? bcmOAMEndpointTypeBhhSection : 
...
int bcm_petra_oam_endpoint_action_set_internal(
    ...
    SOC_PPC_OAM_LIF_PROFILE_DATA *profile_data_pointer;
    SOC_PPC_OAM_LIF_PROFILE_DATA profile_data, old_profile_data;
    SOC_PPC_OAM_LIF_PROFILE_DATA profile_data_acc, old_profile_data_acc;
    SOC_PPC_OAM_LIF_PROFILE_DATA profile_data_passive, old_profile_data_passive;
    SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY classifier_mep_entry;
    uint8 acc_profile_alloc=0;
    int opcode, profile;
    ...
    int rv = BCM_E_NONE;
    uint32 soc_sand_rv = SOC_SAND_OK;
    uint32 trap_code=0, trap_code2=0;
    ...
    if (is_mip) {
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        SOC_PPC_OAM_LIF_PROFILE_DATA_clear(&old_profile_data_passive);
    ...
    if (BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_LBR) && is_sat_loopback != 2) {
        if (is_accelerated) {
    ...
    for (is_passive=0; is_passive<=is_mip; is_passive++) {
        ...
        if (SOC_IS_ARADPLUS(unit)) {
            ...
            if ( BCM_OAM_OPCODE_GET(*action_lcl, SOC_PPC_OAM_ETHERNET_PDU_OPCODE_SLM) ||
...
int bcm_petra_oam_group_destroy(
    ...
    if ((SOC_IS_QAX(unit) || (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "maid48_enabled", 0)))
        ...
        if (SOC_IS_QAX(unit)) {
            ...
            for (i = 0; i < 3; i++, ma_name.index += 0x400) {
...
int _manage_DA_with_existing_entries_internal(int unit , int mep_id ,SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep, uint32 msb, uint32 lsb, 
    ...
    int (*alloc_function) (int, int,SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY * , int *, int *, int*, int*, int) = eth_1731_profile==NULL?  _bcm_oam_am_template_oam_eth_1731_mep_profile_loopback_alloc:
...
int _bcm_oam_lm_dm_pointed_add_shared(int unit, SOC_PPC_OAM_OAMP_LM_DM_MEP_DB_ENTRY *lm_dm_mep_db_entry) {
    ...
    for (entries_allocated = 0;
        ...
        rv = _bcm_dpp_oam_mep_id_alloc(unit, SHR_RES_ALLOC_WITH_ID,
...
int _bcm_oam_lm_dm_add_shared(int unit,SOC_PPC_OAM_OAMP_LM_DM_MEP_DB_ENTRY *mep_db_entry){
    ...
    if ( !mep_db_entry->is_update) {
		...
		rv =  _bcm_dpp_oam_mep_id_alloc(unit,SHR_RES_ALLOC_WITH_ID ,0 /*ignored anyways*/ , 0, &entry); 
...
int _bcm_oam_set_classifier_action(int unit, uint32 mep_id, bcm_oam_endpoint_action_t action,int is_lm,int is_1dm_or_piggy_back,SOC_PPC_OAM_MEP_TYPE mep_type ,int is_slm) {
    ...
    if (is_slm) {
        ...
        if (!SHR_BITGET(lif_profile.mep_profile_data.counter_disable, SOC_PPC_OAM_OPCODE_MAP_DMM)) {
            BCM_OAM_ACTION_SET(action, bcmOAMActionCountEnable);
        }
        if (!SHR_BITGET(lif_profile.mep_profile_data.meter_disable, SOC_PPC_OAM_OPCODE_MAP_DMM)) {
            BCM_OAM_ACTION_SET(action, bcmOAMActionMeterEnable);
        }
        ...
        if (!SHR_BITGET(lif_profile.mep_profile_data.meter_disable, meter_counter_opcode)) {
            BCM_OAM_ACTION_SET(action, bcmOAMActionMeterEnable);
        }
    }
    rv = _bcm_oam_endpoint_action_set_validity_check(unit,mep_id,&action);
...
int _bcm_oam_lm_dm_remove_shared(int unit,bcm_oam_endpoint_t endpoint_id , int is_lm, SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY * classifier_mep_entry) {
    ...
    SOC_PPC_OAM_ETH1731_MEP_PROFILE_ENTRY old_profile = {0};
    ...
    if (_BCM_OAM_MEP_INDEX_IS_SERVER_CLIENT_SIDE(endpoint_id) == 0) {
        ...
        eth1731_profile_data.ccm_cnt        = old_profile.ccm_cnt;
        if (is_lm) {
            ...
            eth1731_profile_data.slm_lm = old_profile.slm_lm;
    ...
    if (classifier_mep_entry->lif != _BCM_OAM_INVALID_LIF) { /* OAM server: do not set the classifier*/
        ...
        rv = _bcm_oam_set_classifier_action(unit,endpoint_id,action,is_lm, (!is_lm || !(is_lm && old_profile.piggy_back_lm) ) ,ccm_entry_type, 0/*is_slm*/);
        BCMDNX_IF_ERR_EXIT(rv);
    }
 ...
 * @return int 
 */
int bcm_petra_oam_loss_update_non_accelerated_internal(
    int unit,
    bcm_oam_endpoint_t id,
    bcm_oam_endpoint_action_t *action,
...
int bcm_petra_oam_delay_add(int unit, bcm_oam_delay_t *delay_ptr) {
    ...
    if (classifier_mep_entry.lif != _BCM_OAM_INVALID_LIF) {
        ...
        rv = _bcm_oam_set_classifier_action(unit, delay_ptr->id, action, 0 /*is_lm */, delay_ptr->flags & BCM_OAM_DELAY_ONE_WAY, classifier_mep_entry.mep_type, 0/* is_slm */);
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
int sat_loopback_add_rx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    bcm_rx_trap_config_t trap_config;
    bcm_port_config_t port_config;
    bcm_gport_t oamp_port[SOC_DPP_DEFS_MAX(NOF_CORES)],sat_port = 0;
    bcm_oam_endpoint_action_t action;
    int dont_care;
    int session_id = 0;
    int tc = 0;
    ...
    if ((classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0) { /* Down mep */
        ...
        if (SOC_IS_QAX(unit)) {
           ...
           if(!found) {
               BCMDNX_ERR_EXIT_MSG(BCM_E_PARAM, (_BSL_BCM_MSG("No Internal SAT port found.")));
           }
        }
        /*And then*/
        trap_config.flags = (BCM_RX_TRAP_UPDATE_DEST | BCM_RX_TRAP_TRAP);
        trap_config.dest_port = SOC_IS_QAX(unit) ? sat_port : oamp_port[0];
        ...
        if(lb_trap_used_cnt == 0){
           rv = bcm_petra_rx_trap_set(unit, trap_id, &trap_config); /*only first  caller set trap*/
           if(rv != BCM_E_EXISTS) {
               BCMDNX_IF_ERR_EXIT(rv);
           }
        }
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        /* action set will update classifier entry, so re-get it */
        rv = _bcm_dpp_oam_bfd_mep_info_db_get(unit, loopback_ptr->id, &classifier_mep_entry, &found);
        BCMDNX_IF_ERR_EXIT(rv);
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        trap_id = (int)lb_trap_id;
        COMPILER_64_SET(arg64,0,lb_trap_id);
        soc_sand_rv = arad_pp_prge_program_var_set(unit, ARAD_EGR_PROG_EDITOR_PROG_OAM_UPMEP_LOOPBACK_SAT, arg64);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
    ...
    if (!(loopback_ptr->flags & BCM_OAM_LOOPBACK_UPDATE)) {
        ...
        /*If trap was set in the 1st calling, it will return exist, so 2nd calling don't need to care of them*/
...
int sat_loopback_add_tx(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++) {
        for (i2 = 0; i2 < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i2++) {
            config->packet_edit[priority].packet_length[i] =  offset +  loopback_ptr->tlvs[i].tlv_length+1; /* endtlv*/
        }
        if(config->packet_edit[priority].packet_length[i] < (66 + udh_size)){
            config->packet_edit[priority].packet_length[i] = 66 + udh_size;  /* 66total - 6sys_header -udh + 4crc = 64 eth packet)*/
        }
...
int sat_loopback_delete(int unit, bcm_oam_loopback_t *loopback_ptr) {
    ...
    if(lb_trap_used_cnt == 0){ /*last user responsble for deleting trap*/
        bcm_rx_trap_config_t_init(&trap_config);
        rv = bcm_petra_rx_trap_set(unit, trap_id, &trap_config);
        BCMDNX_IF_ERR_EXIT(rv);
    }
    ...
    /*If trap was removed in the 1st calling, it will return "not found", so 2nd calling don't need to care of them*/
    if(rv != BCM_E_NOT_FOUND) {
        BCMDNX_IF_ERR_EXIT(rv);
    }
...
int bcm_petra_oam_tst_rx_add(int unit,bcm_oam_tst_rx_t *tst_ptr) {
    ...
    if((classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_UPMEP) == 0)/* Down mep */
    {
       ...
       if (SOC_IS_QAX(unit)) {
        ...
        /* Set the destination in the trap to be the OAMP*/
       bcm_rx_trap_config_t_init(&trap_config); 
    ...
    else 
    {
        ...
        BCMDNX_IF_ERR_EXIT(rv);
        /* Set the Eg.PRGE program variable to the correct trap code   */
        COMPILER_64_SET(arg64,0,tst_trap_id);
        soc_sand_rv = arad_pp_prge_program_var_set(unit, ARAD_EGR_PROG_EDITOR_PROG_OAM_UPMEP_LOOPBACK_SAT, arg64);
        BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
    }
   ...
   if(tst_ptr->flags & BCM_OAM_TST_TX_WITH_ID){  /* user request a specify gtf_id*/
    ...
    for (priority = 0; priority < BCM_SAT_GTF_NUM_OF_PRIORITIES; priority++)
    {
        ...
        for (i = 0; i < BCM_SAT_GTF_PACKET_LENGTH_NUM_OF_PATTERNS; i++) {
           config.packet_edit[priority].packet_length[i] =  offset +  tst_ptr->tlv.tlv_length +1 ; /*add end tlv*/
           if(config.packet_edit[priority].packet_length[i] < (66 + udh_size)){
               config.packet_edit[priority].packet_length[i] = 66 + udh_size;  /* 66total - 6sys_header - udh + 4crc = 64 eth packet)*/
           }
        }
...
int bcm_oam_endpoint_info_diag_print(int unit, bcm_oam_endpoint_info_t *endpoint_info)
{
  ...
  case bcmOAMEndpointTypeMPLSNetwork:
    ...
    if (_BCM_OAM_DISSECT_IS_ENDPOINT_ACCELERATED(endpoint_info)) {
        ...
        if (SOC_IS_ARADPLUS(unit)){
            ...
            if (_BCM_OAM_IS_SERVER_SERVER(endpoint_info)) {
                LOG_CLI((BSL_META_U(unit,"OAM Server endpoint: server side. remote gport: 0x%x.\n"), endpoint_info->remote_gport )); 
            }
        }
    ...
    if (_BCM_OAM_IS_SERVER_CLIENT(endpoint_info)) {
        LOG_CLI((BSL_META_U(unit, "OAM Server endpoint: client side. remote gport: 0x%x.\n"), endpoint_info->remote_gport));
    }
   ...
   for (i = 0; i < (SOC_DPP_DEFS_GET(unit, nof_counter_processors)); i++) {  /* search 16 counter engines which are available for OAM */
      ...
      if (rv != BCM_E_EMPTY) {
         ...
         if (((mep_info.counter >= config.source.pointer_range.start) && (mep_info.counter <= config.source.pointer_range.end)) &&
            ...
            if (config.source.engine_source == bcmStatCounterSourceIngressOam) { /* seek only for oam counter */
               crps_counters->crps_engine    = i;
               if (mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP) {
                  for (j = 0; j < 8; j++) {
                     /* 
                      * Egress counters are set at counter_base_id
                      * and Ingress counters are at counter_base_id + 8
                      */
                  ...
                  /* 
                   * Egress counters are set at counter_base_id , even numbers
                   * and Ingress counters are at counter_base_id + 1, odd numbers
                   */
...
int _bcm_dpp_oam_bfd_diagnostics_LM_counters(int unit, int endpoint_id) {
   ...
   if (SOC_IS_JERICHO(unit)) {
      ...
      if (crps_counters.crps_engine == -1) {
         BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("CRPS counter engine not found.\n")));
      }

      if (mep_info.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_PCP) {
         for (i = 0; i < 8; i++) {
            rv = soc_ppd_oam_get_crps_counter(unit, crps_counters.crps_engine, crps_counters.crps_ingress_counter[i], ing_val);
            BCMDNX_IF_ERR_EXIT(rv);
            rv = soc_ppd_oam_get_crps_counter(unit, crps_counters.crps_engine, crps_counters.crps_egress_counter[i], eg_val);
            BCMDNX_IF_ERR_EXIT(rv);
         ...
         rv = soc_ppd_oam_get_crps_counter(unit, crps_counters.crps_engine, crps_counters.crps_ingress_counter[0], ing_val);
         BCMDNX_IF_ERR_EXIT(rv);
         rv = soc_ppd_oam_get_crps_counter(unit, crps_counters.crps_engine, crps_counters.crps_egress_counter[0], eg_val);
         BCMDNX_IF_ERR_EXIT(rv);
...
int  bcm_petra_oam_sd_sf_detection_add(int unit,bcm_oam_sd_sf_detection_t  *sd_sf_ptr)
{
    ...
    if(classifier_mep_entry.flags & SOC_PPC_OAM_CLASSIFIER_MEP_ENTRY_FLAG_MEP_SD_SF){
        BCMDNX_ERR_EXIT_MSG(BCM_E_EXISTS, (_BSL_BCM_MSG("Endpoint %d has already added sd/sf objext."), sd_sf_ptr->mep_id));
    }
    ...
    if((mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) || (mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) ||
       ...
       BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Y.1711 sd/sf will be support after Y.1711 feature finished. ")));
    }
    ...
    rv = bcm_dpp_am_oam_sd_sf_profile_id_alloc(unit, flags,&sd_sf_profile_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
int  bcm_petra_oam_sd_sf_detection_delete(int unit, bcm_oam_sd_sf_detection_t  *sd_sf_ptr)
{
    ...
    rv = bcm_dpp_am_oam_sd_sf_profile_id_dealloc(unit, profile_id);
    BCMDNX_IF_ERR_EXIT(rv);
...
int  bcm_petra_oam_y_1711_alarm_add(int unit, bcm_oam_y_1711_alarm_t  *alarm_ptr)
{
    ...
    if((alarm_ptr->flags&BCM_OAM_Y_1711_FLAGS_THRESHOLD)&&(alarm_ptr->mep_id == -1)&&(alarm_ptr->rmep_id == -1)){/*global configration*/
        ...
        if((mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_PWE) || (mep_db_entry.mep_type==SOC_PPC_OAM_MEP_TYPE_Y1731_O_MPLSTP) || 
           ...
           BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("Y.1711 alarm will be support after Y.1711 feature finished. ")));
        }
...
int bcm_petra_oam_mpls_tp_channel_type_tx_set(int unit, bcm_oam_mpls_tp_channel_type_t channel_type, int value)
{
    ...
    if (channel_type_ndx < 0) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_NOT_FOUND, (_BSL_BCM_MSG("Unknown channel type\n")));
    }
