  ...
  )
{
    ...
    BCMDNX_ALLOC(l2_traverse_mact_keys, sizeof(SOC_PPC_FRWRD_MACT_ENTRY_KEY)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_mc_bridge_get_block.mact_keys");
    ...
    if (!SOC_SAND_TBL_ITER_IS_END(&(block_range.iter))) {
        BCMDNX_ALLOC(l2_traverse_mact_vals, sizeof(*l2_traverse_mact_vals)*DPP_FRWRD_IP_GET_BLOCK_NOF_ENTRIES, "_bcm_ppd_frwrd_ipv4_mc_bridge_get_block.l2_traverse_mact_vals");
    ...
    void *user_data)
{
    ...
    if (flags & BCM_IPMC_IP6) {
        ...
        if (SOC_IS_JERICHO(unit) && (!(soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0)) || ARAD_KBP_ENABLE_IPV6_MC)) {
            rv = _bcm_ppd_frwrd_ipv6_mc_route_traverse(unit, cb, user_data);
        } else
#endif /*defined(INCLUDE_KBP) && !defined(BCM_88030)*/
        {
        ...
        if (SOC_IS_JERICHO(unit) && !((soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "l3_mc_use_tcam", 0)) || SOC_DPP_CONFIG(unit)->pp.ipmc_l2_ssm_mode)) {
            rv = _bcm_ppd_frwrd_ipv4_mc_route_traverse(unit, cb, user_data);
        } else
#endif /*defined(INCLUDE_KBP) && !defined(BCM_88030)*/
        {
