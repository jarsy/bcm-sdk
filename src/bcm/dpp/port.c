        ...
        if ((loopback != BCM_PORT_LOOPBACK_NONE) && (rv == BCM_E_NONE)) {
            ...
            if (interface != SOC_PORT_IF_ILKN){
                if (loopback == BCM_PORT_LOOPBACK_MAC) {
                    rv = _bcm_link_force(unit, port_i, TRUE, TRUE);
                } else {
    ...
    switch (type)
    {
        ...
        case bcmPortControlReserved281:
            if (SOC_DPP_CONFIG(unit)->pp.custom_feature_vrrp_scaling_tcam) {
                ...
                vtt_in_pp_port_conf_tbl.vlan_translation_profile = SOC_PPC_PORT_DEFINED_VT_PORT_TERMINATION;
                soc_sand_rv = arad_pp_ihp_vtt_in_pp_port_config_tbl_set_unsafe(unit, core, soc_ppd_port, &vtt_in_pp_port_conf_tbl);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);

                vtt_in_pp_port_conf_tbl.termination_profile = SOC_PPC_PORT_DEFINED_TT_PROFILE_PORT_TERMINATION;
                soc_sand_rv = arad_pp_ihp_vtt_in_pp_port_config_tbl_set_unsafe(unit, core, soc_ppd_port, &vtt_in_pp_port_conf_tbl);
                BCM_SAND_IF_ERR_EXIT(soc_sand_rv);
            } else if (soc_property_suffix_num_get(unit, -1, spn_CUSTOM_FEATURE, "conn_to_np_enable", 0)) {
    ...
    if (!SOC_ARAD_STAT_COUNTER_MODE_PACKETS_PER_CHANNEL(unit, gport_info.local_port)) {
        ...
        if (SOC_PBMP_MEMBER(PBMP_IL_ALL(unit), gport_info.local_port) && channel <= SOC_ARAD_NIF_ILKN_COUNTER_PER_CHANNEL_CANNEL_SUPPORTED_MAX) {
            if (stat == bcmPortStatIngressPackets) {
                ...
                rv = SOC_CONTROL(unit)->controlled_counters[index].controlled_counter_f(unit, index, gport_info.local_port, val);
                BCMDNX_IF_ERR_EXIT(rv);
            } else if (stat == bcmPortStatEgressPackets) {
...
{
    uint32      is_valid, out_port_priority, tm_port, pp_port, first_header_size, first_phy_port, first_phy_port_elk, interface_max_rate;
    uint32      temp_flags = 0;
    ...
    int         is_tm_src_syst_port_ext_present, is_stag_enabled, is_snoop_enabled, is_tm_ing_shaping_enabled;
    ...
    if (is_lbg_port == 0) {
        ...
        if (flags & BCM_PORT_ADD_USE_PHY_PBMP) { /* phy ports are given in phy_pbmp */
            ...
            if(SOC_IS_QAX(unit) && interface_info->interface == SOC_PORT_IF_ILKN) {
                ...
                BCMDNX_IF_ERR_EXIT(soc_qax_nif_ilkn_pbmp_from_num_lanes_get(unit, nof_lanes, interface_info->interface_id, &phy_ports));
            } else 
#endif
            {
    ...
    if (!is_lbg_port) {
        ...
        /* for lbg port, set max speed to 10Gbps by default */
        speed = 10000;
        BCMDNX_IF_ERR_EXIT(soc_port_sw_db_speed_set(unit, port, speed));
    }
...
int bcm_petra_port_remove(int unit, bcm_port_t port, uint32 flags) 
{
    ...
    int              base_q_pair, profile, counter_interval, is_channelized, i;
        ...
        case bcmPortControlFloodUnknownUcastGroup:
          {
            ...
            if (gport_parse_info.type == _bcmDppGportParseTypeSimple) {
                ...
                if (BCM_GPORT_IS_VLAN_PORT(port) || BCM_GPORT_IS_MPLS_PORT(port) || BCM_GPORT_IS_EXTENDER_PORT(port)) {
        ...
        case bcmPortControlOamDefaultProfile:
            {
                ...
                if (SOC_IS_ARADPLUS(unit)) {
                    ...
                    case 2:  /* Simple mode - 2 bits */
                        ...
                        rv = _bcm_petra_port_inlif_profile_set(unit, port, value, SOC_OCC_MGMT_INLIF_APP_OAM, &lif_entry);
                        BCMDNX_IF_ERR_EXIT_MSG(rv, (_BSL_BCM_MSG("Failed to set a map from inlif profile to oam profile")));
                        break;
                    default:
       ...
       case bcmPortClassFieldIngressVlanTranslation:
            ...
            case bcmPortClassFieldIngressTunnelTerminated:
           {
               ...
               if (!(SOC_DPP_CONFIG(unit)->pp.fcoe_enable) &&
                   ...
                   BCMDNX_ERR_EXIT_MSG(BCM_E_UNAVAIL, (_BSL_BCM_MSG("applicable when FCoE or DIP_SIP_VRF termination mode are enabled.")));
               }
            ...
            for(indx2 = 0; indx2 < port_tpid_nof_indexes[1]; ++indx2) {
                for(indx3 = 0; indx3 < nof_prirority_types; ++indx3) {
                    ...
                    if (has_prev) {
                        ...
                        if(!sal_memcmp(&prev_pkt_frmt_info, &pkt_frmt_info, sizeof(SOC_PPC_LLP_PARSE_PACKET_FORMAT_INFO))){
                    ...
                    has_prev = 1;
                    sal_memcpy(&prev_pkt_frmt_info, &pkt_frmt_info, sizeof(SOC_PPC_LLP_PARSE_PACKET_FORMAT_INFO));
                }
            }
        }
    } else if (is_egress) {
    ...
    bcm_gport_t *tunnel_port_id)
{
    ...
    if (!_BCM_PPD_IS_PON_PP_PORT(gport_info.local_port) || !IS_PON_PORT(unit,_BCM_PPD_GPORT_PON_TO_PHY_PORT(unit, gport_info.local_port))) {
    ...
    bcm_port_match_info_t *match)
{
    ...
    if (BCM_GPORT_IS_VLAN_PORT(port)) {
        ...
        BCMDNX_ERR_EXIT_MSG(BCM_E_PORT, (_BSL_BCM_MSG("API bcm_petra_port_match receives only VLAN or Extender ports")));
    }
