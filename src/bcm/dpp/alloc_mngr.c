...
#define _DPP_AM_TEMPLATE_BFD_TX_RATE_PROFILE_MAX_ENTITIES (SOC_PPC_OAM_MAX_NUMBER_OF_LOCAL_MEPS(unit))
...
#define _DPP_AM_TEMPLATE_MAID_48_COUNT (341)
#define _DPP_AM_TEMPLATE_MAID_48_COUNT_ARAD (5)
    ...
    break;
        ...
        case dpp_am_res_oam_mep_id:
            if (SOC_IS_JERICHO_PLUS(unit)) {
                p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
                p_info.start = 0;
                p_info.count = SOC_DPP_DEFS_GET(unit, oamp_number_of_meps); /* lower half of the maximal number of MEPs */
        ...
        case dpp_am_res_oam_mep_db_id:
            if (SOC_IS_QAX(unit)) {
                ...
                p_info.count = SOC_DPP_DEFS_GET(unit, oamp_number_mep_db_entries);
      ...
      case dpp_am_res_oam_sd_sf_profile_id:
                if (SOC_IS_QAX(unit)) {
                   p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
                   p_info.start =0;
                   p_info.count = SOC_DPP_DEFS_GET(unit,oamp_number_of_sd_sf_profiles); /* maximal number of SD/SF profile is 16 */
                   desc_str = "OAM SD/SF Profile Index";
                   p_info.max_elements_per_allocation = 1;
                }
                else {
                   BCM_EXIT;
        ...
        case dpp_am_res_map_encap_intpri_color:
            ...
            break;
        case dpp_am_res_field_hw_group_id:
            bcm_dpp_field_get_dev_info(unit, &devInfo);
            p_info.res_type = SW_STATE_RES_ALLOCATOR_BITMAP;
            p_info.count = devInfo->stage[0].maxGroups;
            p_info.start = 0;
            desc_str = "Field HW group ID";
            p_info.max_elements_per_allocation = 1;
            break;
...
int
bcm_dpp_am_oam_sd_sf_profile_id_alloc(
        int unit,
        uint32 flags, 
        uint32  *sd_sf_profile_id)
{
    int elem = *sd_sf_profile_id;
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_alloc(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_res_oam_sd_sf_profile_id, flags, 1, &elem));
    *sd_sf_profile_id = elem;
exit:
    BCMDNX_FUNC_RETURN;
...
int
bcm_dpp_am_oam_sd_sf_profile_id_dealloc(
        int unit,
        uint32 sd_sf_profile_id)
{
    BCMDNX_INIT_FUNC_DEFS;
    BCMDNX_IF_ERR_EXIT(dpp_am_res_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_oam_sd_sf_profile_id, 1, sd_sf_profile_id));
...
int
bcm_dpp_am_oam_sd_sf_profile_id_is_alloced(
        int unit,
        uint32 sd_sf_profile_id)
{
    BCMDNX_INIT_FUNC_DEFS;
    BCM_RETURN_VAL_EXIT(dpp_am_res_check(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_res_oam_sd_sf_profile_id, 1, sd_sf_profile_id));
...
void _bcm_dpp_am_template_oam_profile_dump(int unit, const void* data)
{
    ...
    LOG_CLI((BSL_META("\n  Trap Code      : ")));
    for (i = 0; i < SOC_PPC_OAM_OPCODE_MAP_COUNT; i++) {
        _bcm_dpp_rx_trap_sw_id_to_hw_id(unit, oam_profile->mep_profile_data.opcode_to_trap_code_unicast_map[i], &hw_trap_code);
    ...
    LOG_CLI((BSL_META("\n  Trap Code      : ")));
    for (i = 0; i < SOC_PPC_OAM_OPCODE_MAP_COUNT; i++) {
        _bcm_dpp_rx_trap_sw_id_to_hw_id(unit, oam_profile->mep_profile_data.opcode_to_trap_code_multicast_map[i], &hw_trap_code);
        LOG_CLI((BSL_META("     0x%03x "), hw_trap_code));
    }
    LOG_CLI((BSL_META("\n  MIP profile: \n")));
    LOG_CLI((BSL_META("  count enabled for opcodes: ")));
...
int _bcm_dpp_am_template_bfd_tx_rate_profile_alloc(int unit, int flags, uint32 *data, int *is_allocated, int *tx_rate_profile)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_allocate(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_bfd_tx_rate_profile, flags, data, is_allocated, tx_rate_profile);
    BCMDNX_IF_ERR_EXIT(rc);
exit:
    BCMDNX_FUNC_RETURN;
}
...
int _bcm_dpp_am_template_bfd_tx_rate_profile_data_get(int unit, int tx_rate_profile, uint32* data)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_data_get(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_bfd_tx_rate_profile, tx_rate_profile, data);
...
int _bcm_dpp_am_template_bfd_tx_rate_profile_free(int unit, int tx_rate_profile, int *is_last)
{
    ...
    BCMDNX_INIT_FUNC_DEFS;
    rc = dpp_am_template_free(unit, BCM_DPP_AM_DEFAULT_POOL_IDX, dpp_am_template_bfd_tx_rate_profile, tx_rate_profile, is_last);
    BCMDNX_IF_ERR_EXIT(rc);
...
STATIC int
_bcm_dpp_am_template_lif_term_mapping_get(int unit, SOC_PPC_LIF_ID lif_id,int *template)
{
    bcm_error_t rc = BCM_E_NONE;
    uint32 soc_sand_rv;
    int soc_sand_dev_id;      
...
int _bcm_dpp_am_template_lif_term_profile_exchange(int unit, SOC_PPC_LIF_ID lif_id, SOC_PPC_MPLS_TERM_PROFILE_INFO *term_profile_info,  int *old_template, int *is_last, int *new_term_profile,int *is_allocated)
{
    int rc = BCM_E_NONE;
    ...
    rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_template_lif_term_profile,0,term_profile_info,tmp_old_template,&tmp_is_last,new_term_profile,is_allocated);
    BCMDNX_IF_ERR_EXIT(rc);
...
_bcm_dpp_am_template_l2_flooding_mapping_get(int unit, SOC_PPC_LIF_ID lif_index,int *profile)
{
    ...
    if ((lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_AC) && (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_FIRST_AC_IN_GROUP) &&
        ...
        (lif_entry_info->type != SOC_PPC_LIF_ENTRY_TYPE_EXTENDER)) {
        BCMDNX_ERR_EXIT_MSG(BCM_E_INTERNAL, (_BSL_BCM_MSG("LIF entry type is not as expected AC, PWE, Extender"))); /* expected to be AC, PWE, Extender type*/
    }
...
int _bcm_dpp_am_template_ip_tunnel_init(int unit, int sip_profile , int sip_val, int ttl_profile, int ttl_val,
...
int _bcm_dpp_am_template_ip_tunnel_exchange_test(int unit, int tunnel_id, int is_new_ip_tunnel, int sip, int ttl, int tos, int *sip_template, int *ttl_template, int *tos_template,int *is_allocated)
{
    int rc = BCM_E_NONE;
...
int _bcm_dpp_am_template_l3_rif_mac_termination_combination_exchange(int unit, int old_profile_id, int *old_is_last, uint32 *new_profile_info, uint8 *new_mapping_profile,int *is_allocated)
{
  int rc = BCM_E_NONE;
  int new_profile = 0;
  BCMDNX_INIT_FUNC_DEFS;

  rc = dpp_am_template_exchange(unit, BCM_DPP_AM_DEFAULT_POOL_IDX,dpp_am_template_l3_rif_mac_termination_combination, 0, new_profile_info,
