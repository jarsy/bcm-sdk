/*
 * $Id: dnx_shr_template.h,v 1.6 Broadcom SDK $
 *
 * $Copyright: (c) 2016 Broadcom.
 * Broadcom Proprietary and Confidential. All rights reserved.$
 *
 * File:        dnx_shr_template.h
 * Purpose:     Internal routines to the BCM library for template
 *              gu2 management.
 */

#ifndef _DNX_SHR_TEMPLATE_H_
/* { */
#define _DNX_SHR_TEMPLATE_H_
 ...
 *
 *  DNX_SHR_TEMPLATE_MANAGE_SET_WITH_ID indicates that the template ID is provided by the
 *  caller, and that specific template.  The mapping must fail if the requested
 *  template is invalid or already in use.
 */

#define DNX_SHR_TEMPLATE_MANAGE_SINGLE_FLAGS     0x00000007
#define DNX_SHR_TEMPLATE_MANAGE_SET_WITH_ID      0x00000001
#define DNX_SHR_TEMPLATE_MANAGE_IGNORE_DATA      0x00000002
#define DNX_SHR_TEMPLATE_MANAGE_IGNORE_NOT_EXIST_OLD_TEMPLATE 0x00000004
 ...
 */
typedef enum dnx_shr_template_manage_e {   
    DNX_SHR_TEMPLATE_MANAGE_HASH = 0,
    DNX_SHR_TEMPLATE_MANAGE_COUNT /* last one indicates how many, not valid item */
} dnx_shr_template_manage_t;
 ...
 */
typedef int (*dnx_shr_template_to_stream_t)(const void *data, void *to_stream, size_t size);
typedef int (*dnx_shr_template_from_stream_t)(void *data, const void *from_stream, size_t size);

typedef int dnx_shr_template_manage_hash_compare_extras_t;

/*
 *  That's the max length of types and pools' names. Trying to create a type or a pool
 *  with a longer name would return an error.
 */
#define DNX_SW_STATE_TEMPLATE_NAME_MAX_LENGTH 50

typedef struct dnx_sw_state_template_name_s {
    char name[DNX_SW_STATE_TEMPLATE_NAME_MAX_LENGTH];
} dnx_sw_state_template_name_t;
 ...
 */
typedef struct _dnx_shr_template_type_desc_s {
    int templatePoolId;              /* unit specific template pool ID */
    int refCount;               /* number of elements allocated currently */
    dnx_sw_state_template_name_t name;               /* descriptive name (for diagnostics) */
} _dnx_shr_template_type_desc_t;
 ...
 */
typedef struct _dnx_shr_template_pool_desc_s {
    dnx_shr_template_manage_t templateManagerType; /* which resoource manager to use */
    ...
    dnx_shr_template_manage_hash_compare_extras_t extras; /* additional config per resmgr type */
    dnx_sw_state_template_name_t name;      /* descriptive name for diagnostics */
} _dnx_shr_template_pool_desc_t;
 ...
 */
typedef struct _dnx_shr_template_unit_desc_s {
    uint16 templateTypeCount;          /* maximum presented resource ID */
    uint16 templatePoolCount;          /* maximum resource pool ID */
    PARSER_HINT_ARR_PTR _dnx_shr_template_type_desc_t **template_param;   /* array of type -> pool map pointers */
    PARSER_HINT_ARR_PTR _dnx_shr_template_pool_desc_t **pool;  /* array of pool description pointers */
} _dnx_shr_template_unit_desc_t;

/*This is the prototype of call back function used to print the template data based on type */
typedef void (*dnx_shr_template_print_func_t)(int unit, const void *);

/*
 *   Function
 *      dnx_shr_template_init
 *   Purpose
 ...
 *      data, the pool will have to be reconfigured with dnx_shr_template_pool_set.  If
 ...
 *      have to be reconfigured with dnx_shr_template_type_set.  If the caller wants to
...
/*
 *   Function
 *      dnx_shr_template_exchange_test
 *   Purpose
