...
#define BCM_COSQ_BW_EAV_MODE            0x00000008 /* Prioritize selected
                                                      ...
                                                      before non-selected
                                                      queue(s). */
#define BCM_COSQ_BW_LLFC                0x00000010 /* Link Level Flow Control
                                                      setting */
#define BCM_COSQ_BW_PACKET_MODE         0x00000020 /* The bandwidth specified is
                                                      in packets per second
                                                      instead of kbits per
                                                      second */
#define BCM_COSQ_BW_NOT_COMMIT          0x00000040 /* Set rate to SW DB only,
                                                      commit to HW is done when
                                                      calling the API with the
                                                      same GPort type and
                                                      without NOT_COMMIT flag. */
#define BCM_COSQ_BW_EAV_TAS_MODE        0x00000080 /* Avoid burstiness when gate
                                                      is opening or after queue
                                                      is empty. */
#define BCM_COSQ_BW_TAS_TOKEN_FREEZE    0x00000100 /* Freeze shaper token when
                                                      gate is closed to avoid
                                                      burstiness. */
#define BCM_COSQ_BW_ALT_CALC            0x00000200 /* Use alternative building
...
#define BCM_COSQ_GPORT_SIZE_SRAM            0x0010     /* SRAM configuration */
...
typedef enum bcm_cosq_stat_e {
    ...
    bcmCosqStatTASTxOverrunPackets = 59, /* Number of transmission overrun events
                                           ...
                                           closed. */
    bcmCosqStatMaxCount = 60            /* this should be the last one. */
} bcm_cosq_stat_t;
...
typedef enum bcm_cosq_resource_e {
    ...
    bcmResourceOcbBytes = 4,            /* Resource in OCB bytes. */
    bcmResourceOcbPacketDescriptors = 5 /* Resource in OCB packet descriptors. */
} bcm_cosq_resource_t;
...
#define BCM_COSQ_INGRESS_PORT_DROP_ETH_MAP_KEY(pcp, dei)  ( ( ((pcp << 1) | dei) & 0xF) | 0x1000000 ) 
#define BCM_COSQ_INGRESS_PORT_DROP_TM_MAP_KEY(tc, dp)  ( ( ((tc << 2) | dp) & 0x3F) | 0x2000000 ) 
#define BCM_COSQ_INGRESS_PORT_DROP_IP_MAP_KEY(dscp)  ( ((dscp) & 0x3F) | 0x3000000 ) 
#define BCM_COSQ_INGRESS_PORT_DROP_MPLS_MAP_KEY(exp)  ( ((exp) & 0x7) | 0x4000000 ) 
...
typedef enum bcm_cosq_tas_status_e {
    ...
    bcmCosqTASStatusMaxCalendarEntries = 3, /* The max entries could be support in
                                           calendar table. */
    bcmCosqTASSatusCount = 4            /* This should be the last one. */
