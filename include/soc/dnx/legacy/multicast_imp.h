...
#ifndef __DNX_MULTICAST_IMP_H__
#define __DNX_MULTICAST_IMP_H__

/*
 * This file contains joint multicast implementation between Arad and Jericho.
 * It "inherits" from include/soc/dnx/multicast.h .
 */
...
#define DNX_MCDS_TYPE_VALUE_FREE_BLOCK_START 0  /* free entry at the start of a free block, this value is used when reading mcdb from hardware for all entries */
#define DNX_MCDS_TYPE_VALUE_FREE_BLOCK       1  /* free entry not at the start of a free block */
#define DNX_MCDS_TYPE_VALUE_INGRESS_START    2  /* a used ingress entry, at the start of the group */
#define DNX_MCDS_TYPE_VALUE_INGRESS          3  /* a used ingress entry, not at the start of the group */
#define DNX_MCDS_TYPE_VALUE_EGRESS_START     4  /* The start of a non TDM egress group */
#define DNX_MCDS_TYPE_VALUE_EGRESS           5  /* a used egress entry, non TDM, not at the start of the group */
#define DNX_MCDS_TYPE_VALUE_EGRESS_TDM_START 6  /* The start of a TDM egress group, not used in Jericho */
#define DNX_MCDS_TYPE_VALUE_EGRESS_TDM       7  /* a used egress TDM entry, not at the start of the group, not used in Jericho */

/* check traits of a given type */
#define DNX_MCDS_TYPE_IS_FREE(type) (!((type) & 6))                  /* is the entry free? */
#define DNX_MCDS_TYPE_IS_USED(type) ((type) & 6)                     /* is the entry used? */
#define DNX_MCDS_TYPE_IS_INGRESS(type) (((type) & 6) == 2)           /* is the entry used for ingress multicast */
#define DNX_MCDS_TYPE_IS_EGRESS(type) ((type) & 4)                   /* is the entry used for egress multicast */
#define DNX_MCDS_TYPE_IS_EGRESS_NORMAL(type) (((type) & 6) == 4)     /* is the entry used for non TDM egress multicast */
#define DNX_MCDS_TYPE_IS_NORMAL(type) (((type) ^ ((type) >> 1)) & 2) /* is the entry used for non TDM egress multicast or ingress multicast */
#define DNX_MCDS_TYPE_IS_TDM(type) (((type) & 6) == 6)               /* is the entry used for TDM egress multicast */
#define DNX_MCDS_TYPE_IS_EGRESS_START(type) (((type) & 5) == 4)      /* is the entry the first one of an egress multicast group */
#define DNX_MCDS_TYPE_IS_EGRESS_NOT_START(type) (((type) & 5) == 5)  /* is the entry the none first one of an egress multicast group */
#define DNX_MCDS_TYPE_IS_START(type) (((type) & 1) == 0)             /* is the entry the none first one of an egress multicast group */
#define DNX_MCDS_TYPE_IS_INGRESS_OR_TDM(type) ((type) & 2)           /* is the entry used for TDM egress multicast */
#define DNX_MCDS_TYPES_ARE_THE_SAME(t1, t2) ((((t1)^(t2)) & 6) == 0) /* checks if the two types are the same disregarding start information */
#define DNX_MCDS_TYPE_GET_START(type) ((type) & 6)                   /* returns the same type for the first entry of a group/free block */
#define DNX_MCDS_TYPE_GET_NONE_START(type) ((type) | 1)              /* returns the same type for a none first entry of a group/free block */
...
#define DNX_MCDS_MAX_FREE_BLOCK_SIZE_ALLOCED 8
...
#define DNX_MCDS_MAX_FREE_BLOCK_SIZE_GENERAL 8
/* maximum size in entries of a block of consecutive free entries, in any region  (max of the two macros above) */
#define DNX_MCDS_MAX_FREE_BLOCK_SIZE 8


/* flags for get_free_entries_block() */
#define DNX_MCDS_GET_FREE_BLOCKS_PREFER_SMALL   1  /* Will prefer blocks of the given size or smaller, the default is the given size or bigger. */
#define DNX_MCDS_GET_FREE_BLOCKS_PREFER_SIZE    2  /* Will prefer to return a block of a better size than in a better region. The default is opposite. */
#define DNX_MCDS_GET_FREE_BLOCKS_NO_UPDATES     4  /* Will not change the returned block: will not resize it if a smaller part of it is needed, */
                                                         /* and will not remove it from its free blocks list. */
                                                         /* The caller must handle all of this before another call is made. */
#define DNX_MCDS_GET_FREE_BLOCKS_PREFER_INGRESS 8  /* Will prefer the ingress regions over the egress regions */
#define DNX_MCDS_GET_FREE_BLOCKS_DONT_FAIL      16 /* When failing to allocate, instead of the function failing, return a block size & start of 0 */

#define JER2_ARAD_MCDS_NOF_REGIONS 3 /* number of regions (in MCDS) */



/* encoding of additional memory in MCDB entries in memory - joint encoding used by Arad and Jericho */

/* The bits of word1 of a jer2_arad_mcdb_entry_t are used for:
...
#define DNX_MC_ENTRY_SIZE 2                /* size of a MCDB entry in uint32s which must be equal to the size of jer2_arad_mcdb_entry_t */
/* mask of the hardware data bits in the msb word of an mcdb entry */
#define JER2_ARAD_MC_ENTRY_MASK_VAL 0x7ffff
#define JER2_JER_MC_ENTRY_MASK_VAL 0xffffff

#define JER2_ARAD_MC_ENTRY_MASK_WORD(mcds, index) ((mcds)->mcdb[index].word1) /* The last word of MCDB in which to apply the mask and store data in free bits*/

#define DNX_MCDS_TYPE_SHIFT 29 /* stored in the 3 msb bits */
#define dnx_MAX_NOF_MCDB_INDEX_BITS 18 /* maximum (between device types) of bits in an MCDB table index */
#define dnx_NOF_REMAINDER_PREV_BITS (dnx_MAX_NOF_MCDB_INDEX_BITS - 16) /* number of previous entry (msb) bits stored in MCDS's MCDB entry copies */
#define dnx_NOF_REMAINDER_PREV_MASK ((1 << dnx_NOF_REMAINDER_PREV_BITS) - 1)
#define DNX_MCDS_MSB_PREV_BIT_SHIFT (DNX_MCDS_TYPE_SHIFT - dnx_NOF_REMAINDER_PREV_BITS) /* bits not fitting in prev_entries */
#define DNX_MCDS_BITMAP_OPEN_SHIFT (DNX_MCDS_MSB_PREV_BIT_SHIFT - 1) /* bit previously used for marking if the bitmap groups (egress 0-8191) were open */
#define DNX_MCDS_TEST_BIT_SHIFT (DNX_MCDS_BITMAP_OPEN_SHIFT -1) /* bit saying if the bitmap group (egress 0-8191) is open */


#define DNX_MCDS_WORD1_KEEP_BITS_MASK /* The bits to keep when copying/resetting a MCDB entry */ \
  ((1 << DNX_MCDS_BITMAP_OPEN_SHIFT) | (1 << DNX_MCDS_TEST_BIT_SHIFT))


#define DNX_MCDS_TYPE_MASK 7   /* 3 bits type */
/* get and set the entry type */
#define DNX_MCDS_ENTRY_GET_TYPE(entry) ((entry)->word1 >> DNX_MCDS_TYPE_SHIFT) /* assumes the usage of the msb bits */
#define DNX_MCDS_ENTRY_SET_TYPE(entry, type_value) /* assumes the usage of the msb bits */ \
    do {(entry)->word1 = ((entry)->word1 & ~(DNX_MCDS_TYPE_MASK << DNX_MCDS_TYPE_SHIFT)) | \
      ((type_value) << DNX_MCDS_TYPE_SHIFT); } while (0)
#define DNX_MCDS_GET_TYPE(mcds, index) DNX_MCDS_ENTRY_GET_TYPE((mcds)->mcdb + (index)) /* assumes the usage of the msb bits */
#define DNX_MCDS_SET_TYPE(mcds, index, type_value) DNX_MCDS_ENTRY_SET_TYPE((mcds)->mcdb + (index), (type_value)) /* assumes the usage of the msb bits */


#define DNX_MCDS_MSB_PREV_BIT_MASK (dnx_NOF_REMAINDER_PREV_MASK << 16) /* mask of the bit in the entry index */
/* get and set the previous entry index (the current entry index means no previous entry) */
#define DNX_MCDS_GET_PREV_ENTRY(mcds, index) \
  (((JER2_ARAD_MC_ENTRY_MASK_WORD((mcds), (index)) >> (DNX_MCDS_MSB_PREV_BIT_SHIFT - 16)) & \
    DNX_MCDS_MSB_PREV_BIT_MASK) | (mcds)->prev_entries[index])

#define DNX_MCDS_SET_PREV_ENTRY(mcds, index, prev_entry) \
  do { \
    JER2_ARAD_MC_ENTRY_MASK_WORD((mcds), (index)) = (JER2_ARAD_MC_ENTRY_MASK_WORD((mcds), (index)) & \
      ~(dnx_NOF_REMAINDER_PREV_MASK << DNX_MCDS_MSB_PREV_BIT_SHIFT)) | \
      (((prev_entry) & DNX_MCDS_MSB_PREV_BIT_MASK) << (DNX_MCDS_MSB_PREV_BIT_SHIFT - 16)); \
    (mcds)->prev_entries[index] = (prev_entry) & 0xffff; \
  } while(0);

#define DNX_MCDS_ENTRY_GET_PREV_ENTRY(entry, mcds, index) \
  ((((entry)->word1 >> (DNX_MCDS_MSB_PREV_BIT_SHIFT - 16)) & \
    DNX_MCDS_MSB_PREV_BIT_MASK) | (mcds)->prev_entries[index])
#define DNX_MCDS_ENTRY_SET_PREV_ENTRY(entry, mcds, index, prev_entry) \
  do { \
    (entry)->word1 = ((entry)->word1 & \
      ~(1 << DNX_MCDS_MSB_PREV_BIT_SHIFT)) | \
      (((prev_entry) & DNX_MCDS_MSB_PREV_BIT_MASK) << (DNX_MCDS_MSB_PREV_BIT_SHIFT - 16)); \
    (mcds)->prev_entries[index] = (prev_entry) & 0xffff; \
  } while(0);

#define DNX_MCDS_FREE_NEXT_PREV_MASK 0x3ffff
#define DNX_MCDS_FREE_BLOCK_SIZE_MASK 0xf /* 4 bits for storing block size */
#define DNX_MCDS_FREE_BLOCK_SIZE_SHIFT 19
#define DNX_MCDS_GET_FREE_NEXT_ENTRY(mcds, index) /* get the next free block start */ \
  (JER2_ARAD_MC_ENTRY_MASK_WORD(mcds, index) & DNX_MCDS_FREE_NEXT_PREV_MASK)
#define DNX_MCDS_SET_FREE_NEXT_ENTRY(mcds, index, next_entry) /* set the next free block start */ \
  JER2_ARAD_MC_ENTRY_MASK_WORD(mcds, index) = (JER2_ARAD_MC_ENTRY_MASK_WORD(mcds, index) & ~DNX_MCDS_FREE_NEXT_PREV_MASK) | (next_entry)
#define DNX_MCDS_ENTRY_GET_FREE_PREV_ENTRY(entry) /* get the prev free block start or start of this block */ \
  ((entry)->word0 & DNX_MCDS_FREE_NEXT_PREV_MASK)
#define DNX_MCDS_GET_FREE_PREV_ENTRY(mcds, index) /* get the prev free block start or start of this block */ \
  ((mcds)->mcdb[index].word0 & DNX_MCDS_FREE_NEXT_PREV_MASK)
#define DNX_MCDS_SET_FREE_PREV_ENTRY(mcds, index, prev_entry) /* set the prev free block start or start of this block */ \
  (mcds)->mcdb[index].word0 = ((mcds)->mcdb[index].word0 & ~DNX_MCDS_FREE_NEXT_PREV_MASK) | (prev_entry)
#define DNX_MCDS_GET_FREE_BLOCK_SIZE(mcds, index) /* get the block size, to be called for the first block entry */ \
  (((mcds)->mcdb[index].word0 >> DNX_MCDS_FREE_BLOCK_SIZE_SHIFT) & DNX_MCDS_FREE_BLOCK_SIZE_MASK)
#define DNX_MCDS_SET_FREE_BLOCK_SIZE(mcds, index, size) /* set the block size, to be called for the first block entry */ \
  (mcds)->mcdb[index].word0 = ((mcds)->mcdb[index].word0 & ~(DNX_MCDS_FREE_NEXT_PREV_MASK << DNX_MCDS_FREE_BLOCK_SIZE_SHIFT)) | \
  ((size) << DNX_MCDS_FREE_BLOCK_SIZE_SHIFT)

/* Macros handling the storing of if an egress bitmap is open or not */
#define DNX_MCDS_ENTRY_IS_BITMAP_OPEN(   entry)    (((entry)->word1 >> DNX_MCDS_BITMAP_OPEN_SHIFT) & 1) /* returns 1 if open, 0 of closed */
#define DNX_MCDS_ENTRY_SET_BITMAP_OPEN(  entry) do {(entry)->word1 |=  (1 << DNX_MCDS_BITMAP_OPEN_SHIFT);} while (0) /* marks the bitmap group as open */
#define DNX_MCDS_ENTRY_SET_BITMAP_CLOSED(entry) do {(entry)->word1 &= ~(1 << DNX_MCDS_BITMAP_OPEN_SHIFT);} while (0) /* marks the bitmap group as closed */

/* Macros for setting/getting the test bit */
#define DNX_MCDS_ENTRY_GET_TEST_BIT(     entry)    (((entry)->word1 >> DNX_MCDS_TEST_BIT_SHIFT) & 1)
#define DNX_MCDS_ENTRY_SET_TEST_BIT_ON(  entry) do {(entry)->word1 |=  (1 << DNX_MCDS_TEST_BIT_SHIFT);} while (0)
#define DNX_MCDS_ENTRY_SET_TEST_BIT_OFF( entry) do {(entry)->word1 &= ~(1 << DNX_MCDS_TEST_BIT_SHIFT);} while (0)
#define DNX_MCDS_ENTRY_SET_TEST_BIT(entry, val) do {(entry)->word1 &= ~(1 << DNX_MCDS_TEST_BIT_SHIFT); (entry)->word1 |= (((val) & 1) << DNX_MCDS_TEST_BIT_SHIFT);} while (0)


/* handling of egress hardware format */
#define DNX_MCDS_GET_EGRESS_FORMAT(mcds, index) (SOC_IS_JERICHO(unit) ? JER2_JER_MCDS_GET_EGRESS_FORMAT(mcds, index): JER2_ARAD_MCDS_GET_EGRESS_FORMAT(mcds, index))
#define DNX_MCDS_IS_EGRESS_FORMAT_CONSECUTIVE(format) ((format) & 4)
#define DNX_MCDS_IS_EGRESS_FORMAT_CONSECUTIVE_END(format) (((format) & 5) == 4)
#define JER2_ARAD_MCDS_IS_EGRESS_FORMAT_CONSECUTIVE_NEXT(format) (((format) & 5) == 5)

#define JER2_ARAD_MCDS_GET_EGRESS_FORMAT(mcds, index) (((mcds)->mcdb[index].word1 & 0x70000) >> 16)
#define JER2_JER_MCDS_GET_EGRESS_FORMAT(mcds, index) (((mcds)->mcdb[index].word1 & 0xF00000) >> 20)

#define DNX_MULT_MAX_REPLICATIONS 4095 /* = max {DNX_MULT_MAX_INGRESS_REPLICATIONS, DNX_MULT_MAX_EGRESS_REPLICATIONS} */
#define DNX_MULT_MAX_INGRESS_REPLICATIONS DNX_MULT_MAX_REPLICATIONS /* maximum replications for an ingress multicast group */
#define DNX_MULT_MAX_EGRESS_REPLICATIONS DNX_MULT_MAX_REPLICATIONS  /* maximum replications for an egress multicast group */

/* The value used for occupied empty entries (empty MC groups) of different types */
#define DNX_MC_ING_DESTINATION_DISABLED 0x3ffff /* value of destination field that causes no replication */
#define DNX_MC_EGR_OUTLIF_DISABLED 0            /* value of outlif fields that causes no replication in formats 2,6,7 where a port is not specified */
#define DNX_MC_EGR_BITMAP_DISABLED 0            /* value of bitmap pointer field that causes no replication in format 1 */
#define DNX_MC_NOF_EGRESS_PORTS 256             /* number of PP ports that can be specified in egress MC */
#define DNX_MULT_EGRESS_PORT_INVALID (DNX_MC_NOF_EGRESS_PORTS-1) /* local port value marking invalid/disabled */
#define JER2_ARAD_MULT_EGRESS_SMALL_PORT_INVALID 127 /* local port value stored in a 7 bits field marking invalid/disabled */
#define DNX_MC_NOF_EGRESS_BITMAP_WORDS (DNX_TMC_NOF_FAP_PORTS_JER2_JERICHO/DNX_SAND_NOF_BITS_IN_UINT32) /* number uint32s in a bitmap */

#define DNX_MC_EGRESS_LINK_PTR_END 0
#define DNX_MC_INGRESS_1ST_ENTRY 0
#define DNX_MC_EGR_CUD_INVALID 0 /* This CUD represents no replication in CUD only replications */
#define IRDB_TABLE_ENTRY_WORDS 2

#define DNX_MC_FREE_ENTRIES_BLOCK_LIST_EMPTY ((uint32)(-1))

#define DNX_MC_EGR_BITMAP_ID_BITS 13  /* number of bits in an egress MC bitmap ID */
#define DNX_MC_EGR_NOF_BITMAPS (1 <<  DNX_MC_EGR_BITMAP_ID_BITS) /* number of egress bitmaps (IDs starting at 0) */
#define DNX_MC_EGR_MAX_BITMAP_ID (DNX_MC_EGR_NOF_BITMAPS - 1)

#define DNX_LAST_MCDB_ENTRY(mcds) DNX_MCDS_INGRESS_LINK_END(mcds)
...
typedef uint8 dnx_free_entries_block_size_t; /* the size of a free entries block size */
...
} dnx_free_entries_block_list_t;

typedef struct { /* free block lists of a region */
  dnx_free_entries_block_size_t max_size; /* the max size of block, we have block lists of size 1 to this size */
  uint32 range_start;
  uint32 range_end; /* range start and range end */
  dnx_free_entries_block_list_t lists[DNX_MCDS_MAX_FREE_BLOCK_SIZE]; /* the lists of sizes 1 .. max_size */
...
typedef struct
{
  uint32 word0;
  uint32 word1;
} jer2_arad_mcdb_entry_t;

typedef struct {
    uint32 base;  /* data of the standard replication */
    uint32 extra;    /* extra data */
} dnx_rep_data_t; /* type containing the type and data of any replication */

/*
 * usage of the bits of dnx_rep_data_t: 
...
/* macros to access data inside dnx_rep_data_t */

#define DNX_MCDS_REP_DATA_PORT_NOF_BITS 8
#define DNX_MCDS_REP_DATA_CUD_NOF_BITS 19
#define DNX_MCDS_REP_DATA_BM_ID_NOF_BITS DNX_MC_EGR_BITMAP_ID_BITS
#define DNX_MCDS_REP_DATA_TYPE_NOF_BITS 3
#define DNX_MCDS_REP_DATA_TYPE_OFFSET 10
#define DNX_MCDS_REP_DATA_PORT_MASK ((1 << DNX_MCDS_REP_DATA_PORT_NOF_BITS) - 1)
#define DNX_MCDS_REP_DATA_BM_ID_MASK ((1 << DNX_MCDS_REP_DATA_BM_ID_NOF_BITS) - 1)
#define DNX_MCDS_REP_DATA_TYPE_MASK ((1 << DNX_MCDS_REP_DATA_TYPE_NOF_BITS) - 1)
#define DNX_MCDS_REP_DATA_CUD_MASK ((1 << DNX_MCDS_REP_DATA_CUD_NOF_BITS) - 1)
#define DNX_MCDS_REP_DATA_EGR_CUD_SHIFT 13
#define DNX_MCDS_REP_DATA_EXTRA_CUD_SHIFT 13
#define DNX_MCDS_REP_DATA_INGR_DEST_EXTRA_BITS (18 - DNX_MCDS_REP_DATA_BM_ID_NOF_BITS)
#define DNX_MCDS_REP_DATA_INGR_DEST_EXTRA_BITS_MASK ((1 << DNX_MCDS_REP_DATA_INGR_DEST_EXTRA_BITS) - 1)

#define DNX_MCDS_REP_TYPE_INGRESS 0
#define DNX_MCDS_REP_TYPE_EGR_PORT_CUD 3
#define DNX_MCDS_REP_TYPE_EGR_CUD 2
#define DNX_MCDS_REP_TYPE_EGR_BM_CUD 1

#define DNX_MCDS_REP_DATA_SET_TYPE(data, type) do {(data)->extra |= ((type) << DNX_MCDS_REP_DATA_TYPE_OFFSET);} while (0)
#define DNX_MCDS_REP_DATA_RESET_TYPE(data, type) do {(data)->extra &= ~(((uint32)DNX_MCDS_REP_DATA_TYPE_MASK) << DNX_MCDS_REP_DATA_TYPE_OFFSET); \
  (data)->extra |= ((type) << DNX_MCDS_REP_DATA_TYPE_OFFSET);} while (0)
#define DNX_MCDS_REP_DATA_GET_TYPE(data) (((data)->extra >> DNX_MCDS_REP_DATA_TYPE_OFFSET) & DNX_MCDS_REP_DATA_TYPE_MASK)

#define DNX_MCDS_REP_DATA_SET_EXTRA_CUD(data, cud) do {(data)->extra |= (cud) << DNX_MCDS_REP_DATA_EXTRA_CUD_SHIFT;} while (0)
#define DNX_MCDS_REP_DATA_RESET_EXTRA_CUD(data, cud) do {(data)->extra &= ~(((uint32)DNX_MCDS_REP_DATA_CUD_MASK) << DNX_MCDS_REP_DATA_EXTRA_CUD_SHIFT); \
                                                         (data)->extra |= (cud) << DNX_MCDS_REP_DATA_EXTRA_CUD_SHIFT;} while (0)
#define DNX_MCDS_REP_DATA_GET_EXTRA_CUD(data) ((data)->extra >> DNX_MCDS_REP_DATA_EXTRA_CUD_SHIFT)

#define DNX_MCDS_REP_DATA_SET_EGR_PORT(data, port) do {(data)->base |= (port);} while (0)
#define DNX_MCDS_REP_DATA_RESET_EGR_PORT(data, port) do {(data)->base &= ~(uint32)DNX_MCDS_REP_DATA_PORT_MASK; (data)->base |= (port);} while (0)
#define DNX_MCDS_REP_DATA_GET_EGR_PORT(data) ((data)->base & DNX_MCDS_REP_DATA_PORT_MASK)

#define DNX_MCDS_REP_DATA_SET_EGR_BM_ID(data, bm_id) do {(data)->base |= (bm_id);} while (0)
#define DNX_MCDS_REP_DATA_RESET_EGR_BM_ID(data, bm_id) do {(data)->base &= ~(uint32)DNX_MCDS_REP_DATA_BM_ID_MASK; (data)->base |= (bm_id);} while (0)
#define DNX_MCDS_REP_DATA_GET_EGR_BM_ID(data) ((data)->base & DNX_MCDS_REP_DATA_BM_ID_MASK)

#define DNX_MCDS_REP_DATA_SET_EGR_CUD(data, cud) do {(data)->base |= (cud) << DNX_MCDS_REP_DATA_EGR_CUD_SHIFT;} while (0)
#define DNX_MCDS_REP_DATA_RESET_EGR_CUD(data, cud) do {(data)->base &= ~(((uint32)DNX_MCDS_REP_DATA_CUD) << DNX_MCDS_REP_DATA_EGR_CUD_SHIFT); \
                                                       (data)->base |= (cud) << DNX_MCDS_REP_DATA_EGR_CUD_SHIFT;} while (0)
#define DNX_MCDS_REP_DATA_GET_EGR_CUD(data) ((data)->base >> DNX_MCDS_REP_DATA_EGR_CUD_SHIFT)

#define DNX_MCDS_REP_DATA_SET_INGR_CUD(data, cud) DNX_MCDS_REP_DATA_SET_EGR_CUD(data, cud)
#define DNX_MCDS_REP_DATA_RESET_INGR_CUD(data, cud) DNX_MCDS_REP_DATA_RESET_EGR_CUD(data, cud)
#define DNX_MCDS_REP_DATA_GET_INGR_CUD(data) DNX_MCDS_REP_DATA_GET_EGR_CUD(data)

#define DNX_MCDS_REP_DATA_SET_INGR_DEST(data, dest) do {DNX_MCDS_REP_DATA_SET_EGR_BM_ID((data), (dest) & DNX_MCDS_REP_DATA_BM_ID_MASK); (data)->extra |= \
  (dest) >> DNX_MCDS_REP_DATA_BM_ID_NOF_BITS;} while (0)
#define DNX_MCDS_REP_DATA_RESET_INGR_DEST(data, dest) do {DNX_MCDS_REP_DATA_RESET_EGR_BM_ID((data), (dest) & DNX_MCDS_REP_DATA_BM_ID_MASK); \
  (data)->extra &= ~DNX_MCDS_REP_DATA_INGR_DEST_EXTRA_BITS_MASK; (data)->extra |= (dest) >> DNX_MCDS_REP_DATA_BM_ID_NOF_BITS;} while (0)
#define DNX_MCDS_REP_DATA_GET_INGR_DEST(data) (DNX_MCDS_REP_DATA_GET_EGR_BM_ID(data) | \
  (((data)->extra & DNX_MCDS_REP_DATA_INGR_DEST_EXTRA_BITS_MASK) << DNX_MCDS_REP_DATA_BM_ID_NOF_BITS))

/* get the MCDB index of the start of the egress linked list of the given MCID and core */
#define DNX_MCDS_GET_EGRESS_GROUP_START(mcds, mcid, core_id) ((mcds)->egress_mcdb_offset + (mcds)->nof_egr_ll_groups * (core_id) + (mcid))

/* get the index in mcds->reps according to core and replication number in the core */
#define DNX_MCDS_GET_REP_INDEX(core_id, i) ((DNX_MULT_MAX_REPLICATIONS * (core_id)) + (i))

#define DNX_CUD2CORE_BITS_PER_CUD 2
#define DNX_CUD2CORE_CUD_MASK ((1 << DNX_CUD2CORE_BITS_PER_CUD) - 1)
#define DNX_CUD2CORE_CUDS_PER_WORD (DNX_SAND_NOF_BITS_IN_UINT32 / DNX_CUD2CORE_BITS_PER_CUD)
#define DNX_CUD2CORE_UNDEF_VALUE DNX_CUD2CORE_CUD_MASK
#define DNX_CUD2CORE_GET_CORE(_unit, _cud, _core) do { \
        uint32 _lcl_core = 0;\
        DNXC_IF_ERR_EXIT( \
            sw_state_access[(_unit)].dnx.soc.jer2_arad.tm.jer2_arad_multicast.cud2core.bit_range_read( \
                (_unit), \
                (_cud) * DNX_CUD2CORE_BITS_PER_CUD, \
                0, \
                DNX_CUD2CORE_BITS_PER_CUD, \
                &_lcl_core) \
        ); \
        (_core) = _lcl_core; \
    } while (0)
#define DNX_CUD2CORE_SET_CORE(_unit, _cud, _core) do { \
        uint32 _lcl_core = (_core); \
        DNXC_IF_ERR_EXIT( \
            sw_state_access[(_unit)].dnx.soc.jer2_arad.tm.jer2_arad_multicast.cud2core.bit_range_write( \
                (_unit), \
                (_cud) * DNX_CUD2CORE_BITS_PER_CUD, \
                0, \
                DNX_CUD2CORE_BITS_PER_CUD,\
...
typedef enum
{
  dnxMcdsFreeBuildBlocksAdd_AllMustBeUsed = 0, /* add all entries and verify that they are currently used */
  dnxMcdsFreeBuildBlocksAddAll,                /* add all entries */
  dnxMcdsFreeBuildBlocksAddOnlyFree            /* add only entries marked as free */
} dnx_mcds_free_build_option_t;


/*Each bit in this array represents a TM port. The +1 is to allow memory operations directly in the array */
typedef uint32 dnx_mc_egr_bitmap_t[DNX_MC_NOF_EGRESS_BITMAP_WORDS + 1];
...
typedef uint32 dnx_mc_ingress_dest_t; /* 18 bits */
typedef uint32 dnx_mc_outlif_t;       /* 16 bits in Arad, 18/19 bits in Jericho */
typedef uint8  dnx_mc_local_port_t;   /* 8 bits for normal egress formats */
typedef uint16 dnx_mc_bitmap_id_t;    /* 13 bits range */

typedef uint32 dnx_mc_core_bitmap_t; /* contains one bit per core, specifying if to perform an operation on each active core */
#define DNX_MC_CORE_BITAMAP_NO_CORES ((dnx_mc_core_bitmap_t)0)
#define DNX_MC_CORE_BITAMAP_CORE_0 ((dnx_mc_core_bitmap_t)1)
#define DNX_MC_CORE_BITAMAP_ALL_ACTIVE_CORES(unit) ((DNX_MC_CORE_BITAMAP_CORE_0 << SOC_DNX_CONFIG(unit)->core_mode.nof_active_cores) - 1)
#define DNX_MC_FOREACH_CORE(core_bitmap, core_bitmap_temp, core) \
    for ((core) = 0, (core_bitmap_temp) = (core_bitmap); (core_bitmap_temp); ++core, (core_bitmap_temp) = (core_bitmap_temp) >> 1) \
        if (core_bitmap_temp & DNX_MC_CORE_BITAMAP_CORE_0)

#define DNX_MC_NO_2ND_CUD 0 /* There is no 2nd CUD, this is also the value used by hardware */
#define DNX_MC_2ND_CUD_IS_EEI 0x80000000 /* The type of the 2nd CUD is EEI */

#define DNX_MC_2ND_CUD_TYPE_MCDS2REP(type) (((uint32)(type)) << 31) /* convert the type of the 2nd CUD from group (DNX_MC_GROUP_2ND_CUD_OUTRIF/EEI) to replication (0/DNX_MC_2ND_CUD_IS_EEIa) */
#define DNX_MC_2ND_CUD_TYPE_REP2MCDS(type) (2-(uint8)(((uint32)(type)) >> 31)) /* convert in the opposite direction */

typedef enum /* an Enum specifying if a MC group has replications with a 2nd CUD, and if so what is the type of the 2nd CUD */
{
  dnx_mc_group_2nd_cud_none = 0,   /* The MC group has no replications with a 2nd CUD */
  dnx_mc_group_2nd_cud_outrif = 1, /* The MC group has replications of type OutRif */
  dnx_mc_group_2nd_cud_eei = 2     /* The MC group has replications of type EEI */
} dnx_mc_group_2nd_cud_type_t;
...
typedef struct {
    ...
    uint32 additional_cud; /* replication CUD stored in a special entry and common to the entries following it.  */
                           /* Also encodes the type of this CUD. May be DNX_MC_NO_2ND_CUD. */
} dnx_mc_replication_t; /* represents the information of a multicast replication */
 ...
 */
typedef void (*dnx_egr_mc_write_entry_port_cud_f)(
 ...
 */
typedef void (*dnx_egr_mc_write_entry_port_cud_noptr_f)(
    DNX_SAND_IN    int               unit,
    DNX_SAND_INOUT jer2_arad_mcdb_entry_t *mcdb_entry, /* structure to write to */
    DNX_SAND_IN    dnx_rep_data_t    *rep1,       /* replication 1 */
 ...
 */
typedef void (*dnx_egr_mc_write_entry_cud_f)(
    DNX_SAND_IN    int               unit,
    DNX_SAND_INOUT jer2_arad_mcdb_entry_t *mcdb_entry, /* structure to write to */
    DNX_SAND_IN    dnx_rep_data_t    *rep1,       /* replication 1 */
    DNX_SAND_IN    dnx_rep_data_t    *rep2,       /* replication 2  */
 ...
 * The replications to write are specified by structure pointers, NULL pointers mean disabled replications.
 */
typedef void (*dnx_egr_mc_write_entry_cud_noptr_f)(
    DNX_SAND_IN     int               unit,
    DNX_SAND_INOUT  jer2_arad_mcdb_entry_t *mcdb_entry, /* structure to write to */
    DNX_SAND_IN     dnx_rep_data_t    *rep1,       /* replication 1 */
    DNX_SAND_IN     dnx_rep_data_t    *rep2,       /* replication 2 */
 ...
 */
typedef void (*dnx_egr_mc_write_entry_bm_cud_f)(
    DNX_SAND_IN    int               unit,
    DNX_SAND_INOUT jer2_arad_mcdb_entry_t *mcdb_entry, /* structure to write to */
 ...
 */
typedef uint32 (*dnx_get_replications_from_entry_f)(
    DNX_SAND_IN    int     unit,
 ...
 */
typedef uint32 (*dnx_convert_ingress_replication_hw2api_f)(
    DNX_SAND_IN  int          unit,
 ...
 */
typedef uint32 (*dnx_set_egress_linked_list_f)(
    DNX_SAND_IN  int                           unit,
    ...
    DNX_SAND_IN  uint32                        list_end,            /* The entry that end of the created linked list will point to */
...
typedef struct {
    ...
    jer2_arad_mcdb_entry_t *mcdb;    /* mcdb cache, free list, and other data in the free bits */
    ...
    uint32 nof_egr_ll_groups;   /* The number of egress linked list groups (per core) which is SOC_DNX_CONFIG(unit)->tm.nof_mc_ids - (SOC_DNX_CONFIG(unit)->tm.multicast_egress_bitmap_group_range.mc_id_high + 1) */
    int unit; /* The device unit of the mcds */
    dnx_free_entries_blocks_region_t free_general; /* free lists of entries that are not automatically allocated */
    dnx_free_entries_blocks_region_t ingress_alloc_free; /* free lists of entries that are automatically allocated; ingress and egress separately */
    dnx_free_entries_blocks_region_t egress_alloc_free;

    dnx_egr_mc_write_entry_port_cud_f egr_mc_write_entry_port_cud;
    dnx_egr_mc_write_entry_port_cud_noptr_f egr_mc_write_entry_port_cud_noptr;
    dnx_egr_mc_write_entry_cud_f egr_mc_write_entry_cud;
    dnx_egr_mc_write_entry_cud_noptr_f egr_mc_write_entry_cud_noptr;
    dnx_egr_mc_write_entry_bm_cud_f egr_mc_write_entry_bm_cud;
    dnx_get_replications_from_entry_f get_replications_from_entry;
    dnx_convert_ingress_replication_hw2api_f convert_ingress_replication_hw2api;
    dnx_set_egress_linked_list_f set_egress_linked_list;
    ...
    dnx_rep_data_t reps[DNX_MULT_MAX_REPLICATIONS * SOC_DNX_DEFS_MAX(NOF_CORES)]; /* used to store replication data of a group */

    uint16 nof_reps[SOC_DNX_DEFS_MAX(NOF_CORES)]; /* per core: used to store replication data of a group */
    uint16 nof_ingr_reps;            /* number of ingress replications or egress port replications in non standard egress groups */
    uint16 nof_egr_bitmap_reps[SOC_DNX_DEFS_MAX(NOF_CORES)];      /* number of egress replications of type bitmap + outlif */
    uint16 nof_egr_port_outlif_reps[SOC_DNX_DEFS_MAX(NOF_CORES)]; /* per core: number of egress replications of type port + outlif */
    uint16 nof_egr_outlif_reps[SOC_DNX_DEFS_MAX(NOF_CORES)];      /* per core: number of egress replications of type outlif */
...
} dnx_mcds_base_t; /* base structure shared by Arad and Jericho */


/* values of the info_2nd_cud field of dnx_mcds_base_t specifying if a MC group has replications with a 2nd CUD, and if so what is the type of the 2nd CUD */
#define DNX_MC_GROUP_2ND_CUD_NONE   0 /* The MC group has no replications with a 2nd CUD */
#define DNX_MC_GROUP_2ND_CUD_EEI    1 /* The MC group has replications of type EEI */
#define DNX_MC_GROUP_2ND_CUD_OUTRIF 2 /* The MC group has replications of type OutRif */
 ...
 */
dnx_mcdb_entry_t* dnx_mcds_get_mcdb_entry(
    DNX_SAND_IN  int unit,
    DNX_SAND_IN  uint32 mcdb_index
);


/* function to compare dnx_rep_data_t replications, for sorting usage */
...
uint32 dnx_mcds_build_free_blocks(
    ...
    DNX_SAND_IN    uint32                           end_index,   /* end index of the range to work on, if smaller than start_index hen do nothing */
 ...
 */
dnx_free_entries_blocks_region_t* dnx_mcds_get_region_and_consec_range(dnx_mcds_base_t *mcds, uint32 mcdb_index, uint32 *range_start, uint32 *range_end);
...
uint32 dnx_mcdb_relocate_entries(
    DNX_SAND_IN  int                           unit,
...
uint32 dnx_mcdb_free_linked_list(
    DNX_SAND_IN  int                       unit,
...
uint32 dnx_mult_remove_replication(
    ...
    DNX_SAND_IN  uint32               dest,       /* output array to contain ports/destinations */
    DNX_SAND_IN  soc_if_t             cud,        /* output array to contain encapsulations/CUDs/outlifs */
    DNX_SAND_IN  soc_if_t             cud2,       /* output array to contain second encapsulations/CUDs/outlifs */
...
/*
 * Functions called from outside the dnx multicast code.
 */
 ...
 */
uint32 dnx_mcds_multicast_init(
    DNX_SAND_IN int      unit
);
 ...
 */
uint32 dnx_mcds_multicast_init2(
    DNX_SAND_IN int      unit
);
 ...
 */
uint32 dnx_mcds_multicast_terminate(
    DNX_SAND_IN int unit
);
...
/*
 * Ingress Multicast functions called from outside the dnx multicast code.
 */
...
*********************************************************************/
uint32 dnx_mult_ing_all_groups_close(
    DNX_SAND_IN  int unit
);


/*
 * Egress Multicast functions called from outside the dnx multicast code.
 */
...
*********************************************************************/
uint32 dnx_mult_eg_bitmap_group_range_get(
    DNX_SAND_IN  int                 unit,
...
uint32 dnx_mult_eg_bitmap_group_get(
    ...
    DNX_SAND_OUT DNX_TMC_MULT_EG_VLAN_MEMBERSHIP_GROUP *group     /* output port bitmap */
);
...
*     vlan membership.
*********************************************************************/
uint32 dnx_mult_eg_bitmap_group_close_all_groups(
    DNX_SAND_IN  int                 unit
);
