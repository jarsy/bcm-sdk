...
#ifndef __DNX_TMC_API_STATISTICS_INCLUDED__
/* { */
#define __DNX_TMC_API_STATISTICS_INCLUDED__
 ...
 */
      /* ( (dnx_sand_power_of_2(31) - 1) / (DNX_TMC_DFLT_TICKS_PER_SEC/1000) ) */

/*     Number of programmable counters per Incoming FAP Port   */
#define  DNX_TMC_STAT_PER_IFP_NOF_CNTS 4
...
typedef struct
{
   ...
   */
  uint32 ifp_id[DNX_TMC_STAT_PER_IFP_NOF_CNTS];

} DNX_TMC_STAT_IFP_SELECT_INFO;
...
typedef struct
{
   ...
   *  STAG.
   */
  DNX_TMC_ITM_VSQ_GROUP vsq_grp_id;
  /*
...
} DNX_TMC_STAT_VSQ_SELECT_INFO;

/*
 * This enumerator describe the counter available in the driver.
 * Note, the *order* of the enumerator members is impotent!!
 * This order is used in 'soc_jer2_jer2_jer2_tmcdirect_counters', 'soc_jer2_jer2_jer2_tmcdeferred_counters'
 * and 'soc_jer2_jer2_jer2_tmccounters_info'. If one add/remove enumerator member, one should
 * do the same in 'soc_jer2_jer2_jer2_tmccounters_info'.
 */
typedef enum
{
  /*
  ...
  */
  DNX_TMC_IQM_GLOBAL_RESOURCE_CNT,
  /*
 ...
 DNX_TMC_IQM_ISP_PACKET_CNT,
   ...
   */
 DNX_TMC_IQM_REJECTED_PACKET_LACK_RESOURCE_DBUFFS_CNT,
  /*
   ...
   */
 DNX_TMC_IQM_REJECTED_PACKET_LACK_RESOURCE_DBD_CNT,
  /*
   ...
   */
   DNX_TMC_IQM_REJECTED_BDB_PROTCT_PKT_CNT,
  /*
   ...
   */
   DNX_TMC_IQM_REJECTED_OC_BD_PKT_CNT,
  /*
   ...
   */
   DNX_TMC_IQM_IRR_RJCT_SN_ERR_PKT_CNT,
  /*
   ...
   */
   DNX_TMC_IQM_IRR_RJCT_MC_ERR_PKT_CNT,
  /*
   ...
   */
   DNX_TMC_EGQ_PACEKT_REASSEMBLY_OUTPUT_SOP_CNT,
  /*
  ...
  */
   ...
   */
  DNX_TMC_IQM_REJECTED_CNM_PKT_CNT,
  /*
   ...
   *  Soc_petra-A.
   */
  DNX_TMC_IQM_DRAM_DYNAMIC_REJECT_PKT_CNT,
  /*
   ...
   *  Soc_petra-A.
   */
  DNX_TMC_IQM_DRAM_DYNAMIC_SIZE_CNT,
   /*
 ...
 /*
   ...
   */
  DNX_TMC_IQM_METER_A_CIR_LB_NOF_BYTES_CNT,
  /*
   *  MtrpbCblCnt: Number of bytes assigned by the committed
   *  leaky bucket of the configured index. This register is
   *  clear on read. Not valid for Soc_petra-A.
   */
  DNX_TMC_IQM_METER_B_CIR_LB_NOF_BYTES_CNT,
  /*
   *  MtrpaEblCnt: Number of bytes assigned by the excessed
   *  leaky bucket of the configured index. This register is
   *  clear on read. Not valid for Soc_petra-A.
   */
  DNX_TMC_IQM_METER_A_EIR_LB_NOF_BYTES_CNT,
  /*
   *  MtrpbEblCnt: Number of bytes assigned by the excessed
   *  leaky bucket of the configured index. This register is
   *  clear on read. Not valid for Soc_petra-A.
   */
  DNX_TMC_IQM_METER_B_EIR_LB_NOF_BYTES_CNT,
   ...
   *  for Soc_petra-A.
   */
  DNX_TMC_FCR_CREDIT_CELLS_TX_CNT,
  /*
   ...
   *  for Soc_petra-A.
   */
  DNX_TMC_FCT_FLOW_STATUS_CELLS_TX_CNT,
  /*
   *  CnmPacketsCnt:
   *  Counts the number of CNM packets from the EGQ. Not valid
   *  for Soc_petra-A.
   */
  DNX_TMC_EGQ_CNM_PACKET_CNT,
  /*
   *  PktAgedCnt: Number
   *  of aged packets. Not valid for Soc_petra-A.
   */
  DNX_TMC_EGQ_PACKET_AGED_CNT

} DNX_TMC_STAT_COUNTER_TYPE;
...
typedef struct
{
   ...
   *
   */
  DNX_SAND_64CNT  counters[DNX_TMC_NOF_COUNTER_TYPES];
} DNX_TMC_STAT_ALL_STATISTIC_COUNTERS;
...
typedef enum
{
   ...
   */
  DNX_TMC_STAT_PRINT_LEVEL_ERROR_ONLY=0,
  /*
   *  Counters that present the packet walk-through.
   */
  DNX_TMC_STAT_PRINT_LEVEL_PACKET_WALKTHROUGH=1,
  /*
   *  Represents resources counters and packet walk-through.
   */
  DNX_TMC_STAT_PRINT_LEVEL_ALL=2,
  /*
   *  Total number of counter print levels.
   */
  DNX_TMC_STAT_NOF_PRINT_LEVELS=3
}DNX_TMC_STAT_PRINT_LEVEL;
...
void
  DNX_TMC_STAT_ALL_STATISTIC_COUNTERS_clear(
    DNX_SAND_OUT DNX_TMC_STAT_ALL_STATISTIC_COUNTERS *info
  );

void
  DNX_TMC_STAT_IFP_SELECT_INFO_clear(
    DNX_SAND_OUT DNX_TMC_STAT_IFP_SELECT_INFO *info
  );

void
  DNX_TMC_STAT_VOQ_SELECT_INFO_clear(
    DNX_SAND_OUT DNX_TMC_STAT_VOQ_SELECT_INFO *info
  );

void
  DNX_TMC_STAT_VSQ_SELECT_INFO_clear(
    DNX_SAND_OUT DNX_TMC_STAT_VSQ_SELECT_INFO *info
  );

#if DNX_TMC_DEBUG_IS_LVL1

const char*
  DNX_TMC_STAT_COUNTER_TYPE_to_string(
    DNX_SAND_IN DNX_TMC_STAT_COUNTER_TYPE  counter_type,
    DNX_SAND_IN uint32                 short_format
  );

uint32
  soc_jer2_jer2_jer2_tmcstat_all_counters_get_and_print(
    DNX_SAND_IN  int                            unit,
    DNX_SAND_IN  uint32                            sampling_period
  );

void
  DNX_TMC_STAT_IFP_SELECT_INFO_print(
    DNX_SAND_IN  DNX_TMC_STAT_IFP_SELECT_INFO *info
  );

void
  DNX_TMC_STAT_VOQ_SELECT_INFO_print(
    DNX_SAND_IN  DNX_TMC_STAT_VOQ_SELECT_INFO *info
  );

void
  DNX_TMC_STAT_VSQ_SELECT_INFO_print(
    DNX_SAND_IN  DNX_TMC_STAT_VSQ_SELECT_INFO *info
  );
...
/* } __DNX_TMC_API_STATISTICS_INCLUDED__*/
#endif
