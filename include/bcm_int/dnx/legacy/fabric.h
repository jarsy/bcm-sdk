...
#define BCM_FABRIC_MODID_IS_VALID(unit, modid)  \
        ((modid >= 0) && (modid < SOC_DNX_DEFS_GET(unit, modid_max_valid))) /* True if modid is between 0 and the max 
                                                                              limit of FAP's in the device  */
#define BCM_FABRIC_NUM_OF_LINKS_IS_VALID(unit, num_of_links)  \
        ((num_of_links >= 0) && (num_of_links <= SOC_DNX_DEFS_GET(unit, nof_fabric_links))) /* True if num_of_links is between 0 and the max 
...
#define BCM_FABRIC_MC_ID_IS_VALID(unit, mc_id)  \
        ((mc_id >= 0) && (mc_id <= (SOC_DNX_DEFS_GET(unit, fabric_mesh_mc_id_max_valid))))    /* True if mc_id is between 0 and the max
