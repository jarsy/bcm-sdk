...
#ifndef  INCLUDE_DNX_ALLOC_MNGR_UTILS_H
#define  INCLUDE_DNX_ALLOC_MNGR_UTILS_H

#include <shared/swstate/sw_state_resmgr.h>

#define BCM_DNX_AM_DEFAULT_NOF_POOLS    1
#define BCM_DNX_AM_DEFAULT_POOL_IDX     0
#define BCM_DNX_AM_INVALID_POOL_IDX     (-1)
...
} bcm_dnx_alloc_mngr_utils_info_t;

#define DNX_AM_RES_CHECK_SILENT_MODE(_rv,_flags) \
...
int 
bcm_dnx_am_resource_to_nof_pools(int unit, int res_type, uint8 *nof_pools);

int 
bcm_dnx_am_template_to_nof_pools(int unit, int res_type, uint8 *nof_pools);
...
int 
dnx_am_template_allocate_group(int unit, int pool_idx, int template_id, uint32 flags, const void *data, int nof_additions, int *is_allocated, int *template);
...
#endif /*INCLUDE_DNX_ALLOC_MNGR_UTILS_H*/
