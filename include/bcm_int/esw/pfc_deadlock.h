...
typedef struct _bcm_pfc_hw_resorces_s {
    soc_reg_t timer_count[BCM_COS_COUNT];
    soc_reg_t timer_status[BCM_COS_COUNT];
    soc_reg_t timer_mask[BCM_COS_COUNT];
    soc_reg_t timer_en[BCM_COS_COUNT];
    soc_reg_t chip_config[2];
    soc_reg_t port_config;
    soc_field_t cos_field[BCM_COS_COUNT];
    soc_field_t time_init_val[BCM_COS_COUNT];
...
typedef struct _bcm_pfc_deadlock_control_s {
    ...
    bcm_cos_t pfc_pri2cos[BCM_COS_COUNT]; /* Mapping to reduce run-time
                                           * computation */
    bcm_cos_t pfc_cos2pri[BCM_COS_COUNT]; /* Mapping to reduce run-time
                                           * computation */
    _bcm_pfc_deadlock_config_t cos_config[BCM_COS_COUNT];
