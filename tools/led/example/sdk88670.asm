; $Id: sdk88670.asm,v 1.2 Broadcom SDK $
;
; $Copyright: (c) 2016 Broadcom.
; Broadcom Proprietary and Confidential. All rights reserved.$
; 
;
;This is an example program for programming the LEDs for 88670 (Jericho/QMX) chip.
;This example activates the LEDs on the NEGEV chassis.
;
;Negev chassis configuration:
;       In Negev chassis front panel there are 18 ports, 2 x 6 x 40G + 1 x 6 x 100G.
;       (There are also 4 x 10G ports with 2 LEDs per port, this program does not handle the 4 x 10G ports LEDs)
;       There are 3 LED processors, each LED processor will activate 6 ports LEDs:
;       LED processor 0 - 6 x 100G Falcon (PMH - port macro high)
;       LED processor 1 - 6 x 40G Eagle (PML0 - port macro low 0)
;       LED processor 2 - 6 x 40G Eagle (PML1 - port macro low 1)
;
...
; 
; Negev front panel 18 ports are configured as following:
; 6 port block:        PML0(40G)                                  PMH(100G)                                  PML1(40G)            
; Led proc:                1                                          0                                          2             
;           ---------------------------------------------------------------------------------------------------------------------
; phy_port: |    25   |   37    |   69    |            |   13    |    9    |    1    |            |  113    |   89    |   93    |  
;           ---------------------------------------------------------------------------------------------------------------------
; led index: /6\   \5/ /4\   \3/ /2\   \1/              /6\   \5/ /4\   \3/ /2\   \1/              /6\   \5/ /4\   \3/ /2\   \1/  
;           ---------------------------------------------------------------------------------------------------------------------
; phy_port: |    33   |   29    |   53    |            |   21    |   17    |    5    |            |  129    |   97    |   85    |  
;           ---------------------------------------------------------------------------------------------------------------------
; 
; 
...
PHY_PORT_LED3       EQU    17 ; 17
PHY_PORT_LED4       EQU     9 ;  9
PHY_PORT_LED5       EQU    21 ; 21
PHY_PORT_LED6       EQU    13 ; 13

; led processor 1
; To compile led processor 1 program, comment out led processors 0/2 values and uncomment following 6 values.
;led index                 port index in DATA RAM; phy_port
;PHY_PORT_LED1       EQU    29                   ; 53 (minus 24 for data RAM index)
;PHY_PORT_LED2       EQU    45                   ; 69 (minus 24 for data RAM index)
;PHY_PORT_LED3       EQU     5                   ; 29 (minus 24 for data RAM index)
;PHY_PORT_LED4       EQU    13                   ; 37 (minus 24 for data RAM index)
;PHY_PORT_LED5       EQU     9                   ; 33 (minus 24 for data RAM index)
;PHY_PORT_LED6       EQU     1                   ; 25 (minus 24 for data RAM index)


; led processor 2
; To compile led processor 2 program, comment out 6 led processors 0/1 values and uncomment following 6 values.
;led index                 port index in DATA RAM; phy_port
;PHY_PORT_LED1       EQU     1                   ;  85 (minus 84 for data RAM index)
;PHY_PORT_LED2       EQU     9                   ;  93 (minus 84 for data RAM index)
;PHY_PORT_LED3       EQU    13                   ;  97 (minus 84 for data RAM index)
;PHY_PORT_LED4       EQU     5                   ;  89 (minus 84 for data RAM index)
