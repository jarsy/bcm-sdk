/*
 *         
 * $Id: qtce_diagnostics.c,v 1.1.2.8 Broadcom SDK $
 * 
...
int qtce_phy_diagnostics_get(const phymod_phy_access_t* phy, phymod_phy_diagnostics_t* diag)
{
    ...
    PHYMOD_IF_ERR_RETURN(qmod_pmd_lock_get(&phy->access, &diag->rx_lock));
...
int qtce_phy_pmd_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    int start_lane, num_lane;
    phymod_phy_access_t phy_copy;
    int i = 0;
...
int qtce_phy_pcs_info_dump(const phymod_phy_access_t* phy, void *arg)
{
    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY        (&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN (qmod_diag_disp(&phy_copy.access, (char*)arg));
