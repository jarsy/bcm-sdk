...
int qtce_core_info_get(const phymod_core_access_t* phy, phymod_core_info_t* info)
{
    ...
    if ((serdes_id & 0x3f) == QTCE4_MODEL) {
        ...
        strncpy(info->name, "QTCe4A0", strlen("Tsce4A0")+1); 
    } else {
...
int qtce_phy_firmware_lane_config_set(const phymod_phy_access_t* phy, phymod_firmware_lane_config_t fw_config)
{
    ...
    PHYMOD_USLEEP(500);
    PHYMOD_IF_ERR_RETURN
         (_qtce_phy_firmware_lane_config_set(phy, fw_config));
    /*Hold the per lne soft reset bit*/
    for (i = 0; i < num_lane; i++) {
        phy_copy.access.lane_mask = 1 << (start_lane + i);
        PHYMOD_IF_ERR_RETURN
...
{
    PHYMOD_IF_ERR_RETURN
        (qmod_tx_rx_polarity_set(&phy->access, polarity->tx_polarity, polarity->rx_polarity));
    return PHYMOD_E_NONE;
}
...
{
   PHYMOD_IF_ERR_RETURN
        (qmod_tx_rx_polarity_get(&phy->access, &polarity->tx_polarity, &polarity->rx_polarity));
    return PHYMOD_E_NONE;
}
...
{   
    ...
    switch (rx_control) {
    case phymodRxReset:
        PHYMOD_IF_ERR_RETURN(qmod_rx_lane_control_set(&phy->access, 1));
        break;
...
/*
STATIC
int _qtce_st_hto_interface_config_set(const qmod_device_aux_modes_t *device, int lane_num, int speed_vec, uint32_t *new_pll_div, int16_t *os_mode)
{
    // THIS FUNCTION IS NOT NEEDED FOR QTC 
    return PHYMOD_E_FAIL;
} 
*/
...
{  
    ...
    pm_phy_copy.access.lane_mask = 0x1 << start_lane;
     PHYMOD_IF_ERR_RETURN
        (qtce_phy_firmware_lane_config_get(&pm_phy_copy, &firmware_lane_config)); 
...
int qtce_phy_interface_config_get(const phymod_phy_access_t* phy, uint32_t flags, phymod_ref_clk_t ref_clock, phymod_phy_inf_config_t* config)
{
    ...
    PHYMOD_IF_ERR_RETURN
        (_qtce_speed_id_interface_config_get(phy, speed_id, config, an_control.enable, &firmware_lane_config));
...
int qtce_phy_autoneg_set(const phymod_phy_access_t* phy, const phymod_autoneg_control_t* an)
{
    ...
    /* make sure the firmware config is set to an eenabled */
    PHYMOD_IF_ERR_RETURN
        (qtce_phy_firmware_lane_config_get(&phy_copy, &firmware_lane_config));
    if (an->enable) {
...
int qtce_phy_init(const phymod_phy_access_t* phy, const phymod_phy_init_config_t* init_config)
{
    ...
    for (i = 0; i < num_lane; i++) {
        ...
        tmp_pol.rx_polarity = (init_config->polarity.rx_polarity) >> i & 0x1;
          PHYMOD_IF_ERR_RETURN
            (qtce_phy_polarity_set(&pm_phy_copy, &tmp_pol)); 
    }

    for (i = 0; i < num_lane; i++) {
        pm_phy_copy.access.lane_mask = 0x1 << (i + start_lane);
        PHYMOD_IF_ERR_RETURN
            (qtce_phy_tx_set(&pm_phy_copy, &init_config->tx[i]));
    }
    ...
    PHYMOD_IF_ERR_RETURN
        (qmod_rx_lane_control_set(pm_acc, 1));
    PHYMOD_IF_ERR_RETURN
        (qmod_tx_lane_control_set(pm_acc, QMOD_TX_LANE_RESET_TRAFFIC_ENABLE));         /* TX_LANE_CONTROL */
...
int qtce_phy_loopback_set(const phymod_phy_access_t* phy, phymod_loopback_mode_t loopback, uint32_t enable)
{
    ...
    case phymodLoopbackRemotePCS :
        PHYMOD_IF_ERR_RETURN(qmod_rx_loopback_control(&phy->access, enable, enable, enable)); /* RAVI */
        break;
    default :
        break;
    }             
...
int qtce_phy_rx_pmd_locked_get(const phymod_phy_access_t* phy, uint32_t* rx_seq_done){
    PHYMOD_IF_ERR_RETURN(qmod_pmd_lock_get(&phy->access, rx_seq_done));
    return PHYMOD_E_NONE;
}
...
int qtce_phy_eee_set(const phymod_phy_access_t* phy, uint32 enable)
{
    PHYMOD_IF_ERR_RETURN(qmod_eee_control_set(&phy->access,enable));
    return PHYMOD_E_NONE;
}

int qtce_phy_eee_get(const phymod_phy_access_t* phy, uint32* enable)
{
