/**************************************************************************************
 **************************************************************************************
 *  File Name     :  falcon2_madura_dv_functions.c                                        *
 *  Created On    :  22/05/2014                                                       *
 *  Created By    :  Kiran Divakar                                                    *
 *  Description   :  APIs for Serdes IPs ported over from DV                          *
 *  Revision      :  $Id: falcon2_madura_dv_functions.c 1356 2016-02-12 23:27:16Z cvazquez $ *
 *                                                                                    *
...
/** @file falcon2_madura_dv_functions.c
...
err_code_t falcon2_madura_config_shared_tx_pattern( const phymod_access_t *pa, uint8_t patt_length, const char pattern[]) {
  ...
  for (i=0; i < 15; i++) {
    ...
    switch (i) {
      case 0:  EFUN(wrc_falcon2_madura_patt_gen_seq_14(patt_gen_wr_val));
               break;
      case 1:  EFUN(wrc_falcon2_madura_patt_gen_seq_13(patt_gen_wr_val));
               break;
      case 2:  EFUN(wrc_falcon2_madura_patt_gen_seq_12(patt_gen_wr_val));
               break;
      case 3:  EFUN(wrc_falcon2_madura_patt_gen_seq_11(patt_gen_wr_val));
               break;
      case 4:  EFUN(wrc_falcon2_madura_patt_gen_seq_10(patt_gen_wr_val));
               break;
      case 5:  EFUN(wrc_falcon2_madura_patt_gen_seq_9(patt_gen_wr_val));
               break;
      case 6:  EFUN(wrc_falcon2_madura_patt_gen_seq_8(patt_gen_wr_val));
               break;
      case 7:  EFUN(wrc_falcon2_madura_patt_gen_seq_7(patt_gen_wr_val));
               break;
      case 8:  EFUN(wrc_falcon2_madura_patt_gen_seq_6(patt_gen_wr_val));
               break;
      case 9:  EFUN(wrc_falcon2_madura_patt_gen_seq_5(patt_gen_wr_val));
               break;
      case 10: EFUN(wrc_falcon2_madura_patt_gen_seq_4(patt_gen_wr_val));
               break;
      case 11: EFUN(wrc_falcon2_madura_patt_gen_seq_3(patt_gen_wr_val));
               break;
      case 12: EFUN(wrc_falcon2_madura_patt_gen_seq_2(patt_gen_wr_val));
               break;
      case 13: EFUN(wrc_falcon2_madura_patt_gen_seq_1(patt_gen_wr_val));
               break;
      case 14: EFUN(wrc_falcon2_madura_patt_gen_seq_0(patt_gen_wr_val));
               break;
  ...
  /* EFUN(wr_falcon2_madura_patt_gen_mode_sel(mode_sel)); */
  /* EFUN_PRINTF(("Pattern gen Mode = %d\n",mode));    */

  /* Enable Fixed pattern Generation */
  /* EFUN(wr_falcon2_madura_patt_gen_en(0x1)); */
...
err_code_t falcon2_madura_tx_shared_patt_gen_en( const phymod_access_t *pa, uint8_t enable, uint8_t patt_length) {
  ...
  if (enable) {
      ...
      EFUN(wr_falcon2_madura_patt_gen_start_pos(mode_sel));          /* Start position for pattern */
      EFUN(wr_falcon2_madura_patt_gen_stop_pos(0x0));                /* Stop position for pattern */
    EFUN(wr_falcon2_madura_patt_gen_en(0x1));                        /* Enable Fixed pattern Generation  */
  }
  else {
    EFUN(wr_falcon2_madura_patt_gen_en(0x0));                        /* Disable Fixed pattern Generation  */
  }
  return(ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_rx_lock) {
    ...
    ESTM(*pmd_rx_lock = rd_falcon2_madura_pmd_rx_lock());
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_tx_disable( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    EFUN(wr_falcon2_madura_sdk_tx_disable(0x1));
  }
  else {
    EFUN(wr_falcon2_madura_sdk_tx_disable(0x0));
  }
  return(ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_rx_restart( const phymod_access_t *pa, uint8_t enable) {
  ...
  EFUN(wr_falcon2_madura_rx_restart_pmd_hold(enable));
  return(ERR_CODE_NONE);
}
    ...
    case TX_AFE_DRIVERMODE:
        if(val == DM_NOT_SUPPORTED || val > DM_HALF_AMPLITUDE_HI_IMPED) {
            return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
                ...
                return(wr_falcon2_madura_ams_tx_driver_mode(val));
        }
    default:
        return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
  }
...
err_code_t falcon2_madura_apply_txfir_cfg( const phymod_access_t *pa, int8_t pre, int8_t main, int8_t post1, int8_t post2, int8_t post3) {
  ...
  err_code_t failcode = falcon2_madura_validate_txfir_cfg( pa, pre, main, post1, post2, post3);
...
err_code_t falcon2_madura_config_tx_prbs( const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, uint8_t prbs_inv) {
  ...
  EFUN(wr_falcon2_madura_prbs_gen_mode_sel((uint8_t)prbs_poly_mode));  /* PRBS Generator mode sel */
  EFUN(wr_falcon2_madura_prbs_gen_inv(prbs_inv));                      /* PRBS Invert Enable/Disable */
  /* To enable PRBS Generator */
  /* EFUN(wr_falcon2_madura_prbs_gen_en(0x1)); */
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_get_tx_prbs_config( const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, uint8_t *prbs_inv) {
  ...
  ESTM(val = rd_falcon2_madura_prbs_gen_mode_sel());                   /* PRBS Generator mode sel */
  *prbs_poly_mode = (enum srds_prbs_polynomial_enum)val;
  ESTM(val = rd_falcon2_madura_prbs_gen_inv());                        /* PRBS Invert Enable/Disable */
  *prbs_inv = val;
...
err_code_t falcon2_madura_tx_prbs_en( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    EFUN(wr_falcon2_madura_prbs_gen_en(0x1));                          /* Enable PRBS Generator */
  }
  else {
    EFUN(wr_falcon2_madura_prbs_gen_en(0x0));                          /* Disable PRBS Generator */
  }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_get_tx_prbs_en( const phymod_access_t *pa, uint8_t *enable) {
  ...
  ESTM(*enable = rd_falcon2_madura_prbs_gen_en());
...
err_code_t falcon2_madura_tx_prbs_err_inject( const phymod_access_t *pa, uint8_t enable) {
  ...
  */
  if(enable)
    EFUN(wr_falcon2_madura_prbs_gen_err_ins(0x1));
  EFUN(wr_falcon2_madura_prbs_gen_err_ins(0));
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_config_rx_prbs( const phymod_access_t *pa, enum srds_prbs_polynomial_enum prbs_poly_mode, enum srds_prbs_checker_mode_enum prbs_checker_mode, uint8_t prbs_inv) {
  ...
  EFUN(wr_falcon2_madura_prbs_chk_mode_sel((uint8_t)prbs_poly_mode)); /* PRBS Checker Polynomial mode sel  */
  EFUN(wr_falcon2_madura_prbs_chk_mode((uint8_t)prbs_checker_mode));  /* PRBS Checker mode sel (PRBS LOCK state machine select) */
  EFUN(wr_falcon2_madura_prbs_chk_inv(prbs_inv));                     /* PRBS Invert Enable/Disable */
  /* To enable PRBS Checker */
  /* EFUN(wr_falcon2_madura_prbs_chk_en(0x1)); */
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_get_rx_prbs_config( const phymod_access_t *pa, enum srds_prbs_polynomial_enum *prbs_poly_mode, enum srds_prbs_checker_mode_enum *prbs_checker_mode, uint8_t *prbs_inv) {
  ...
  ESTM(val = rd_falcon2_madura_prbs_chk_mode_sel());                 /* PRBS Checker Polynomial mode sel  */
  *prbs_poly_mode = (enum srds_prbs_polynomial_enum)val;
  ESTM(val = rd_falcon2_madura_prbs_chk_mode());                     /* PRBS Checker mode sel (PRBS LOCK state machine select) */
  *prbs_checker_mode = (enum srds_prbs_checker_mode_enum)val;
  ESTM(val = rd_falcon2_madura_prbs_chk_inv());                      /* PRBS Invert Enable/Disable */
  *prbs_inv = val;
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_rx_prbs_en( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    EFUN(wr_falcon2_madura_prbs_chk_burst_err_cnt_en(0x1));            /* must be enabled before prbs_chk_en */
    EFUN(wr_falcon2_madura_prbs_chk_en(0x1));                          /* Enable PRBS Checker */
  }
  else {
    EFUN(wr_falcon2_madura_prbs_chk_en(0x0));                          /* Disable PRBS Checker */
    EFUN(wr_falcon2_madura_prbs_chk_burst_err_cnt_en(0x0));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_get_rx_prbs_en( const phymod_access_t *pa, uint8_t *enable) {
  ...
  ESTM(*enable = rd_falcon2_madura_prbs_chk_en());
...
err_code_t falcon2_madura_prbs_chk_lock_state( const phymod_access_t *pa, uint8_t *chk_lock) {
    ...
    ESTM(*chk_lock = rd_falcon2_madura_prbs_chk_lock());                  /* PRBS Checker Lock Indication 1 = Locked, 0 = Out of Lock */
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_prbs_err_count_ll( const phymod_access_t *pa, uint32_t *prbs_err_cnt) {
    ...
    EFUN(falcon2_madura_pmd_rdt_reg(pa,TLB_RX_PRBS_CHK_ERR_CNT_MSB_STATUS, &rddata));
    *prbs_err_cnt = ((uint32_t) rddata)<<16;
    ESTM(*prbs_err_cnt = (*prbs_err_cnt | rd_falcon2_madura_prbs_chk_err_cnt_lsb()));
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_prbs_err_count_state( const phymod_access_t *pa, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(falcon2_madura_prbs_err_count_ll( pa, prbs_err_cnt));
...
err_code_t falcon2_madura_display_detailed_prbs_state( const phymod_access_t *pa ) {
    ...
    ESTM_PRINTF(("  %d ",falcon2_madura_get_lane(pa)));
    ESTM(enabled = rd_falcon2_madura_prbs_gen_en());
    if(enabled) {
        ESTM_PRINTF((" PRBS%-2d",srds_return_prbs_order(rd_falcon2_madura_prbs_gen_mode_sel())));
    } else {
        EFUN_PRINTF(("  OFF  "));
    }
       ESTM(enabled = rd_falcon2_madura_prbs_chk_en());
    if(enabled) {
        ESTM_PRINTF(("  PRBS%-2d",srds_return_prbs_order(rd_falcon2_madura_prbs_chk_mode_sel())));
    } else {
        EFUN_PRINTF(("   OFF  "));
    }
    ESTM_PRINTF(("  %d ",rd_falcon2_madura_prbs_chk_lock()));
    EFUN(falcon2_madura_prbs_err_count_state( pa, &err_cnt,&lock_lost));
    EFUN_PRINTF(("  %d  %010d ",lock_lost,err_cnt));
    ESTM_PRINTF(("      %4d ",rd_falcon2_madura_prbs_chk_burst_err_cnt()));
    ESTM_PRINTF(("         %2d ",rd_falcon2_madura_prbs_burst_err_length_status()));
    ESTM_PRINTF(("           %2d ",rd_falcon2_madura_max_prbs_burst_err_length_status()));
    EFUN_PRINTF(("\n"));
...
err_code_t falcon2_madura_dig_lpbk( const phymod_access_t *pa, uint8_t enable) {
    EFUN(wr_falcon2_madura_rx_pi_manual_mode(!enable));

  EFUN(wr_falcon2_madura_dig_lpbk_en(enable));                         /* 0 = diabled, 1 = enabled */
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_dig_lpbk_rptr( const phymod_access_t *pa, uint8_t enable, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        ...
        if (enable) {
            EFUN(falcon2_madura_loop_timing( pa, 0));
            EFUN(falcon2_madura_loop_timing( pa, 1));     /* Program DATA_IN_SIDE side TX_PI for loop-timing mode to lock SYS TX to SYS TX clock freq. */
        } else {
            EFUN(falcon2_madura_loop_timing( pa, 0));
        }
        break;
    ...
    case DIG_LPBK_SIDE:
        ...
        { uint8_t    osr_mode;
          ESTM(osr_mode = rd_falcon2_madura_osr_mode());
          if ((osr_mode == FALCON2_MADURA_OSX16P5) || (osr_mode == FALCON2_MADURA_OSX20P625)) {
              EFUN(wr_falcon2_madura_rx_pi_manual_mode(0x0));
          }
        }
        EFUN(falcon2_madura_tx_rptr_mode_timing( pa, 1));          /* Program DIG_LPBK_SIDE side TX_PI for repeater mode to lock LINE TX to SYS RX clock freq. */
        EFUN(wr_falcon2_madura_dig_lpbk_en(enable));                  /* 0 = diabled, 1 = enabled */
        EFUN(wr_falcon2_madura_ams_tx_sel_ll(00));                /* Switch the tx data mux to ULL traffic and check for data integrity */
        break;
...
err_code_t falcon2_madura_ull_from_dig_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
   switch(mode) {
    case DATA_IN_SIDE:
        EFUN(falcon2_madura_dig_lpbk_rptr( pa, 0,mode));
        EFUN(falcon2_madura_tx_ull_config( pa, 1));
        break;
    case DIG_LPBK_SIDE:
        EFUN(falcon2_madura_dig_lpbk_rptr( pa, 0,mode));
        EFUN(falcon2_madura_rx_ull_config( pa, 1));
        break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
...
err_code_t falcon2_madura_loop_timing( const phymod_access_t *pa, uint8_t enable) {
    ...
    if (enable) {
        EFUN(wr_falcon2_madura_tx_pi_loop_timing_src_sel(0x1)); /* RX phase_sum_val_logic enable */
        ESTM(osr_mode = rd_falcon2_madura_osr_mode());
        if ((osr_mode == FALCON2_MADURA_OSX16P5) || (osr_mode == FALCON2_MADURA_OSX20P625)) {
            EFUN(wr_falcon2_madura_rx_pi_manual_mode(0x1));
        }
        EFUN(wr_falcon2_madura_tx_pi_repeater_mode_en(0x0));
        EFUN(wr_falcon2_madura_tx_pi_en(0x1));                  /* TX_PI enable: 0 = diabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_jitter_filter_en(0x1));    /* Jitter filter enable to lock freq: 0 = diabled, 1 = enabled */
        EFUN(falcon2_madura_delay_us(25));               /* Wait for tclk to lock to CDR */
    } else {
        EFUN(wr_falcon2_madura_tx_pi_jitter_filter_en(0x0));    /* Jitter filter enable to lock freq: 0 = diabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_en(0x0));                  /* TX_PI enable: 0 = diabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_repeater_mode_en(0x1));
        /* EFUN(wr_falcon2_madura_tx_pi_loop_timing_src_sel(0x0)); Through timing is default mode in Repeater cores. So never disabled */
    }
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_rmt_lpbk( const phymod_access_t *pa, uint8_t enable) {
    if (enable) {
        EFUN(falcon2_madura_loop_timing( pa, enable));
        EFUN(wr_falcon2_madura_tx_pi_ext_ctrl_en(0x1));  /* PD path enable: 0 = diabled, 1 = enabled */
        EFUN(wr_falcon2_madura_rmt_lpbk_en(0x1));        /* Remote Loopback Enable: 0 = diabled, 1 = enable  */
        EFUN(falcon2_madura_delay_us(50));        /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
    } else {                              /* Might require longer wait time for smaller values of tx_pi_ext_phase_bwsel_integ */
        EFUN(wr_falcon2_madura_rmt_lpbk_en(0x0));        /* Remote Loopback Enable: 0 = diabled, 1 = enable  */
        EFUN(wr_falcon2_madura_tx_pi_ext_ctrl_en(0x0));  /* PD path enable: 0 = diabled, 1 = enabled */
        EFUN(falcon2_madura_loop_timing( pa, enable));
    }
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_rmt_lpbk_from_ull( const phymod_access_t *pa ) {
    EFUN(falcon2_madura_rmt_lpbk( pa, 0)); /* Reset TX_PI */
    EFUN(falcon2_madura_rmt_lpbk( pa, 1)); /* Enable RMT LPBK */
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_ull_from_rmt_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(falcon2_madura_rx_rptr_mode_timing( pa, 1));  /* Enable jitter filter enable */
        EFUN(_ull_rx_mode_setup( pa, 1));
        break;
    case RMT_LPBK_SIDE:
        EFUN(falcon2_madura_rmt_lpbk( pa, 0));         /* Reset TX_PI */
        EFUN(falcon2_madura_tx_rptr_mode_timing( pa, 1));  /* Enable jitter filter enable */
...
err_code_t falcon2_madura_nl_from_rmt_lpbk( const phymod_access_t *pa, enum srds_rptr_mode_enum mode) {
  switch(mode) {
    case DATA_IN_SIDE:
        EFUN(falcon2_madura_rx_rptr_mode_timing( pa, 1));  /* Enable RX PI transfer */
        break;
    case RMT_LPBK_SIDE:
        EFUN(falcon2_madura_rmt_lpbk( pa, 0));                 /* Reset TX_PI */
        EFUN(falcon2_madura_tx_rptr_mode_timing( pa, 1));      /* Enable jitter filter enable */
        EFUN(wr_falcon2_madura_ams_tx_sel_ll(0x0));              /* Switch the tx data mux to PCS data */
        break;
    default : return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));
        break;
...
err_code_t falcon2_madura_tx_pi_freq_override( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val) {
    if (enable) {
        EFUN(wr_falcon2_madura_tx_pi_en(0x1));                              /* TX_PI enable :            0 = disabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_freq_override_en(0x1));                /* Fixed freq mode enable:   0 = disabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_freq_override_val(freq_override_val)); /* Fixed Freq Override Value (+/-8192) */
    }
    else {
        EFUN(wr_falcon2_madura_tx_pi_freq_override_val(0));                 /* Fixed Freq Override Value to 0 */
        EFUN(wr_falcon2_madura_tx_pi_freq_override_en(0x0));                /* Fixed freq mode enable:   0 = disabled, 1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_en(0x0));                              /* TX_PI enable :            0 = disabled, 1 = enabled */
    }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_tx_pi_jitt_gen( const phymod_access_t *pa, uint8_t enable, int16_t freq_override_val, enum srds_tx_pi_freq_jit_gen_enum jit_type, uint8_t tx_pi_jit_freq_idx, uint8_t tx_pi_jit_amp) {
    ...
    EFUN(falcon2_madura_tx_pi_freq_override( pa, enable, freq_override_val));

    if (enable) {
        EFUN(wr_falcon2_madura_tx_pi_jit_freq_idx(tx_pi_jit_freq_idx));
        EFUN(wr_falcon2_madura_tx_pi_jit_amp(tx_pi_jit_amp));

        if (jit_type == TX_PI_SSC_HIGH_FREQ) {
            EFUN(wr_falcon2_madura_tx_pi_jit_ssc_freq_mode(0x1));        /* SSC_FREQ_MODE:             0 = 6G SSC mode, 1 = 10G SSC mode */
            EFUN(wr_falcon2_madura_tx_pi_ssc_gen_en(0x1));               /* SSC jitter enable:         0 = disabled,    1 = enabled */
        }
        else if (jit_type == TX_PI_SSC_LOW_FREQ) {
            EFUN(wr_falcon2_madura_tx_pi_jit_ssc_freq_mode(0x0));        /* SSC_FREQ_MODE:             0 = 6G SSC mode, 1 = 10G SSC mode */
            EFUN(wr_falcon2_madura_tx_pi_ssc_gen_en(0x1));               /* SSC jitter enable:         0 = disabled,    1 = enabled */
        }
        else if (jit_type == TX_PI_SJ) {
            EFUN(wr_falcon2_madura_tx_pi_sj_gen_en(0x1));                /* Sinusoidal jitter enable:  0 = disabled,    1 = enabled */
        }
    }
    else {
        EFUN(wr_falcon2_madura_tx_pi_ssc_gen_en(0x0));                   /* SSC jitter enable:         0 = disabled,    1 = enabled */
        EFUN(wr_falcon2_madura_tx_pi_sj_gen_en(0x0));                    /* Sinusoidal jitter enable:  0 = disabled,    1 = enabled */
    }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_lane_config_for_iddq( const phymod_access_t *pa ) {
  ...
  EFUN(wr_falcon2_madura_rx_dsc_lock_frc_val(0x0));
  EFUN(wr_falcon2_madura_rx_dsc_lock_frc(0x1));

  /* Switch all the lane clocks to comclk by writing to RX/TX comclk_sel registers */
  EFUN(wr_falcon2_madura_ln_rx_s_comclk_sel(0x1));
    EFUN(wr_falcon2_madura_ln_tx_s_comclk_sel(0x1));

  /* Assert all the AFE pwrdn/reset pins using frc/frc_val to make sure AFE is in lowest possible power mode */
  EFUN(wr_falcon2_madura_afe_tx_pwrdn_frc_val(0x1));
  EFUN(wr_falcon2_madura_afe_tx_pwrdn_frc(0x1));
  EFUN(wr_falcon2_madura_afe_rx_pwrdn_frc_val(0x1));
  EFUN(wr_falcon2_madura_afe_rx_pwrdn_frc(0x1));
  EFUN(wr_falcon2_madura_afe_tx_reset_frc_val(0x1));
  EFUN(wr_falcon2_madura_afe_tx_reset_frc(0x1));
  EFUN(wr_falcon2_madura_afe_rx_reset_frc_val(0x1));
  EFUN(wr_falcon2_madura_afe_rx_reset_frc(0x1));
...
err_code_t falcon2_madura_uc_active_enable(const phymod_access_t *pa, uint8_t enable) {
  EFUN(wrc_falcon2_madura_uc_active(enable));
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_uc_reset( const phymod_access_t *pa, uint8_t enable) {
  if (enable) {
    ...
    EFUN(wrc_falcon2_madura_micro_core_clk_en(0x0));                     /* Disable clock to M0 core */
    EFUN(wrc_falcon2_madura_micro_master_clk_en(0x0));                   /* Disable clock to microcontroller subsystem */
    EFUN(wrc_falcon2_madura_micro_protect_fwcode(0x1));                  /* Gate-off RAM chip select and other signals to avoid ucode corruption */
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD200, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD201, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD202, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD204, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD205, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD206, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD207, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD208, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD209, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD20A, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD20B, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD20C, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD20D, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD20E, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD211, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD212, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD213, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD214, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD215, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD216, 0x0007));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD217, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD218, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD219, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD21A, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD21B, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD220, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD221, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD224, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD226, 0x0000));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD225, 0x8803));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD228, 0x0101));
    EFUN(falcon2_madura_pmd_wr_reg(pa,0xD229, 0x0000));
    ...
    EFUN(wrc_falcon2_madura_micro_ra_init            (0x2));             /* Write command for data RAM initialization */
    EFUN(falcon2_madura_poll_micro_ra_initdone( pa, 250));             /* Poll   status of  data RAM initialization */
    EFUN(wrc_falcon2_madura_micro_ra_init            (0x0));             /* Clear command for data RAM initialization */
    EFUN(wrc_falcon2_madura_micro_core_clk_en  (0x1));                   /* Enable clock to M0 core */
    EFUN(wrc_falcon2_madura_micro_core_rstb    (0x1));
  }
 return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_map_lanes(const phymod_access_t *pa,uint8_t num_lanes, uint8_t const *tx_lane_map, uint8_t const *rx_lane_map) {
    ...
    ESTM(rd_val = rdc_falcon2_madura_uc_active());
    if (rd_val != 0) {
        return (ERR_CODE_UC_ACTIVE);
    }

    /* Verify that the num_lanes parameter is correct. */
    ESTM(rd_val = rdc_falcon2_madura_revid_multiplicity());
    if (rd_val != num_lanes) {
        ...
        for (index1=0; index1<num_lanes; ++index1) {
            ...
            if ((tx_lane_map[index1] != rx_lane_map[index1])
                ...
                EFUN_PRINTF(("Warning:  In core %d, TX lane %d is mapped to %d, while RX lane %d is mapped to %d.\n          Digital and remote loopback will not operate as expected.\n          Further warnings are suppressed.\n",
                             /*EFUN(*/falcon2_madura_get_core(pa)/*)*/, index1, tx_lane_map[index1], index1, rx_lane_map[index1]));
...
err_code_t falcon2_madura_core_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(_falcon2_madura_core_clkgate( pa, 0));
        EFUN(wrc_falcon2_madura_afe_s_pll_pwrdn(0x0));
        EFUN(wrc_falcon2_madura_core_dp_s_rstb(0x1));
        break;
    case PWRDN:
        EFUN(wrc_falcon2_madura_core_dp_s_rstb(0x0));
        EFUN(falcon2_madura_delay_ns(   500)); /* wait >50 comclk cycles  */
        EFUN(wrc_falcon2_madura_afe_s_pll_pwrdn(0x1));
        break;
    case PWRDN_DEEP:
        {   uint8_t lane_orig = falcon2_madura_get_lane(pa);
            uint8_t lane;
            for (lane = 0; lane < SERDES_NUM_LANES; ++lane) {
                EFUN(falcon2_madura_set_lane(pa,  lane));
                EFUN(wr_falcon2_madura_ln_rx_s_pwrdn(  0x1));
                EFUN(wr_falcon2_madura_ln_tx_s_pwrdn(  0x1));
                EFUN(_falcon2_madura_lane_clkgate( pa, 1));
                EFUN(wr_falcon2_madura_ln_dp_s_rstb(   0x0));
            }
            EFUN(falcon2_madura_set_lane(pa, lane_orig));
        }
        EFUN(wrc_falcon2_madura_core_dp_s_rstb(0x0));
        EFUN(falcon2_madura_delay_ns(   500)); /* wait >50 comclk cycles  */
        {   uint8_t pll_orig = falcon2_madura_get_pll(pa);
            uint8_t pll;
            for (pll = 0; pll < SERDES_NUM_PLLS; ++pll) {
                EFUN(falcon2_madura_set_pll(pa,pll));
                EFUN(wrc_falcon2_madura_afe_s_pll_pwrdn(0x1));
            }
            EFUN(falcon2_madura_set_pll(pa,pll_orig));
        }
        EFUN(_falcon2_madura_core_clkgate( pa, 1));
        break;
    default:
...
err_code_t falcon2_madura_lane_pwrdn( const phymod_access_t *pa, enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(wr_falcon2_madura_ln_tx_s_pwrdn(0x0));
        EFUN(wr_falcon2_madura_ln_rx_s_pwrdn(0x0));
        EFUN(_falcon2_madura_lane_clkgate(pa, 0));
        break;
    case PWRDN:
...
err_code_t falcon2_madura_isolate_ctrl_pins( const phymod_access_t *pa, uint8_t enable) {
    ...
    EFUN(falcon2_madura_isolate_lane_ctrl_pins( pa, enable));
    EFUN(falcon2_madura_isolate_core_ctrl_pins( pa, enable));
...
err_code_t falcon2_madura_isolate_lane_ctrl_pins( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    EFUN(wr_falcon2_madura_pmd_ln_tx_h_pwrdn_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_rx_h_pwrdn_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_dp_h_rstb_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_h_rstb_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_tx_disable_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_rx_h_rstb_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_rx_dp_h_rstb_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_tx_dp_h_rstb_pkill(0x1));
    EFUN(wr_falcon2_madura_pmd_ln_tx_h_rstb_pkill(0x1));
  }
  else {
    EFUN(wr_falcon2_madura_pmd_ln_tx_h_pwrdn_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_rx_h_pwrdn_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_dp_h_rstb_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_h_rstb_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_tx_disable_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_rx_h_rstb_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_rx_dp_h_rstb_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_tx_dp_h_rstb_pkill(0x0));
    EFUN(wr_falcon2_madura_pmd_ln_tx_h_rstb_pkill(0x0));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_isolate_core_ctrl_pins( const phymod_access_t *pa, uint8_t enable) {
  ...
  if (enable) {
    EFUN(wrc_falcon2_madura_pmd_core_dp_h_rstb_pkill(0x1));
    EFUN(wrc_falcon2_madura_pmd_mdio_trans_pkill(0x1));
  }
  else {
    EFUN(wrc_falcon2_madura_pmd_core_dp_h_rstb_pkill(0x0));
    EFUN(wrc_falcon2_madura_pmd_mdio_trans_pkill(0x0));
  }
  return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_ucode_mdio_load( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    EFUN(wrc_falcon2_madura_micro_autoinc_wraddr_en(0x1));               /* To auto increment RAM write address */
    EFUN(wrc_falcon2_madura_micro_ra_wrdatasize(0x1));                   /* Select 16bit transfers */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_msw(0x0));                   /* Upper 16bits of start address of Program RAM where the ucode is to be loaded */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_lsw(0x0));                   /* Lower 16bits of start address of Program RAM where the ucode is to be loaded */
    ...
    do {                                                  /* ucode_image loaded 16bits at a time */
        ...
        EFUN(wrc_falcon2_madura_micro_ra_wrdata_lsw(wrdata_lsw));                   /* Program RAM lower 16bits write data */
    }   while (count < ucode_len_padded);                 /* Loop repeated till entire image loaded (upto the 4byte boundary) */

    EFUN(wrc_falcon2_madura_micro_ra_wrdatasize(0x2));                   /* Select 32bit transfers as default */
    EFUN(wrc_falcon2_madura_micro_core_clk_en(0x1));                     /* Enable clock to M0 core */
 /* EFUN(wrc_falcon2_madura_micro_core_rstb(0x1)); */                    /* De-assert reset to micro to start executing microcode */
...
err_code_t falcon2_madura_ucode_load_verify( const phymod_access_t *pa, uint8_t *ucode_image, uint16_t ucode_len) {
    ...
    EFUN(wrc_falcon2_madura_micro_autoinc_rdaddr_en(0x1));               /* To auto increment RAM read address */
    EFUN(wrc_falcon2_madura_micro_ra_rddatasize(0x1));                   /* Select 16bit transfers */
    EFUN(wrc_falcon2_madura_micro_ra_rdaddr_msw(0x0));                   /* Upper 16bits of start address of Program RAM from where to read ucode */
    EFUN(wrc_falcon2_madura_micro_ra_rdaddr_lsw(0x0));                   /* Lower 16bits of start address of Program RAM from where to read ucode */
    ...
    do {                                                  /* ucode_image read 16bits at a time */
        ...
        ESTM(data_lsw = rdc_falcon2_madura_micro_ra_rddata_lsw());
    ...
    EFUN(wrc_falcon2_madura_micro_ra_rddatasize(0x2));                   /* Select 32bit transfers ad default */
...
err_code_t falcon2_madura_wrw_uc_ram( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    EFUN(wrc_falcon2_madura_micro_autoinc_wraddr_en(0));
    EFUN(wrc_falcon2_madura_micro_ra_wrdatasize(0x1));                 /* Select 16bit write datasize */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_msw(0x2000));              /* Upper 16bits of RAM address to be written to */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_lsw(addr));                /* Lower 16bits of RAM address to be written to */
    EFUN(wrc_falcon2_madura_micro_ra_wrdata_lsw(wr_val));              /* uC RAM lower 16bits write data */
    return (ERR_CODE_NONE);
}
...
err_code_t falcon2_madura_wrb_uc_ram( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {
    EFUN(wrc_falcon2_madura_micro_autoinc_wraddr_en(0));
    EFUN(wrc_falcon2_madura_micro_ra_wrdatasize(0x0));                 /* Select 8bit write datasize */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_msw(0x2000));              /* Upper 16bits of RAM address to be written to */
    EFUN(wrc_falcon2_madura_micro_ra_wraddr_lsw(addr));                /* Lower 16bits of RAM address to be written to */
    EFUN(wrc_falcon2_madura_micro_ra_wrdata_lsw(wr_val));              /* uC RAM lower 16bits write data */
    return (ERR_CODE_NONE);
...
/* Micro RAM Word Read */
uint16_t falcon2_madura_rdw_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
   uint16_t rddata;
   if(!err_code_p) {
        return(0);
   }
   *err_code_p = ERR_CODE_NONE;
   EPFUN(wrc_falcon2_madura_micro_autoinc_rdaddr_en(0));
   EPFUN(wrc_falcon2_madura_micro_ra_rddatasize(0x1));                 /* Select 16bit read datasize */
   EPFUN(wrc_falcon2_madura_micro_ra_rdaddr_msw(0x2000));              /* Upper 16bits of RAM address to be read */
   EPFUN(wrc_falcon2_madura_micro_ra_rdaddr_lsw(addr));                /* Lower 16bits of RAM address to be read */
   EPSTM(rddata = rdc_falcon2_madura_micro_ra_rddata_lsw());           /* 16bit read data */
   return (rddata);
}

/* Micro RAM Byte Read */
uint8_t falcon2_madura_rdb_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    uint8_t rddata;
    if(!err_code_p) {
        return(0);
    }
    *err_code_p = ERR_CODE_NONE;
    EPFUN(wrc_falcon2_madura_micro_autoinc_rdaddr_en(0));
    EPFUN(wrc_falcon2_madura_micro_ra_rddatasize(0x0));                 /* Select 8bit read datasize */
    EPFUN(wrc_falcon2_madura_micro_ra_rdaddr_msw(0x2000));              /* Upper 16bits of RAM address to be read */
    EPFUN(wrc_falcon2_madura_micro_ra_rdaddr_lsw(addr));                /* Lower 16bits of RAM address to be read */
    EPSTM(rddata = (uint8_t) rdc_falcon2_madura_micro_ra_rddata_lsw()); /* 16bit read data */
    return (rddata);
}
...
err_code_t falcon2_madura_wrws_uc_ram( const phymod_access_t *pa, uint16_t addr, int16_t wr_val) {
    return (falcon2_madura_wrw_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Byte Signed Write */
err_code_t falcon2_madura_wrbs_uc_ram( const phymod_access_t *pa, uint16_t addr, int8_t wr_val) {
    return (falcon2_madura_wrb_uc_ram( pa, addr, wr_val));
}

/* Micro RAM Word Signed Read */
int16_t falcon2_madura_rdws_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int16_t)falcon2_madura_rdw_uc_ram( pa, err_code_p, addr));
}

/* Micro RAM Byte Signed Read */
int8_t falcon2_madura_rdbs_uc_ram( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
    return ((int8_t)falcon2_madura_rdb_uc_ram( pa, err_code_p, addr));
}
...
err_code_t falcon2_madura_rdblk_uc_ram( const phymod_access_t *pa, uint8_t *mem, uint16_t addr, uint16_t cnt) {
    ...
    while (cnt--) {
        ...
        uint8_t    tmp = falcon2_madura_rdb_uc_ram( pa, &error_code, addr++); /* Value read from the required Data RAM Address */
        if (error_code) {
...
err_code_t falcon2_madura_pll_drv_hv_disable( const phymod_access_t *pa, uint8_t disable) {
  return(wrc_falcon2_madura_ams_pll_drv_hv_disable(disable&0x1));
}
