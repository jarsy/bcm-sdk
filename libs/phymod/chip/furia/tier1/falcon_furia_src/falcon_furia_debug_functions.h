 ...
 *****************************************************************************************
 *                                                                                       *
 *  Revision      :  $Id: falcon_furia_debug_functions.h 1020 2015-04-27 14:20:30Z kirand $ *
 *                                                                                       *
...
struct falcon_furia_usr_ctrl_func_st {
...
struct falcon_furia_usr_ctrl_dfe_func_st {
...
struct falcon_furia_usr_ctrl_disable_functions_st {
  struct falcon_furia_usr_ctrl_func_st field;
  uint16_t word;
};

/** Lane User Control Disable DFE Function Struct */
struct falcon_furia_usr_ctrl_disable_dfe_functions_st {
  struct falcon_furia_usr_ctrl_dfe_func_st field;
  uint8_t  byte;
};

/** Falcon Lane status Struct */
struct falcon_furia_detailed_lane_status_st {
 ...
 */
err_code_t falcon_furia_meas_eye_density_data(const phymod_access_t *pa, const struct falcon_furia_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t *buffer_size);
 ...
 * @return Error Code during data collection (returns ERR_CODE_NONE if no errors)
 */
err_code_t falcon_furia_display_eye_density_data(const phymod_access_t *pa, const struct falcon_furia_eyescan_options_st eyescan_options, int32_t *buffer,uint16_t buffer_size);
...
/*--------------------------------*/
/** Get clk90 offset adjustment value from the uC RAM variable.
...
/** Set clk90 offset adjustment value to the RAM variable.
 * Note: Stop the microcode [use falcon_furia_stop_rx_adaptation()] before using this API
 ...
 */
uint8_t falcon_furia_rdbl_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint16_t addr);
 ...
 */
uint16_t falcon_furia_rdwl_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint16_t addr);
 ...
 */
int8_t falcon_furia_rdbls_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint16_t addr);
 ...
 */
int16_t falcon_furia_rdwls_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint16_t addr);
 ...
 * @return 8bit unsigned value read from uC RAM
 */
uint8_t falcon_furia_rdbc_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint8_t addr);
 ...
 * @return 16bit unsigned value read from uC RAM
 */
uint16_t falcon_furia_rdwc_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint8_t addr);
 ...
 * @return 8bit signed value read from uC RAM
 */
int8_t falcon_furia_rdbcs_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint8_t addr);
 ...
 * @return 16bit signed value read from uC RAM
 */
int16_t falcon_furia_rdwcs_uc_var( const phymod_access_t *pa, err_code_t *err_code, uint8_t addr);
...
/** Reads current pmd lane status and populates the provided structure of type #falcon_furia_detailed_lane_status_st.
...
/** Displays the lane status stored in the input struct of type #falcon_furia_detailed_lane_status_st.
