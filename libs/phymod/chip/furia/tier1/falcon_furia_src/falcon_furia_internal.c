 ...
 *  Revision      :  $Id: falcon_furia_internal.c 1017 2015-04-24 22:07:57Z kirand $ *
 *                                                                                 *
...
/* Setup the Ultra low latency clk and datapath for TX [Return Val = Error Code (0 = PASS)] */
 err_code_t _ull_tx_mode_setup( const phymod_access_t *pa, uint8_t enable) {
    ...
    EFUN(falcon_furia_delay_us(1024));                          /* Wait for phase detector path to settle */
    ...
    EFUN(falcon_furia_poll_st_afe_tx_fifo_resetb_equals_1( pa, 1));  /* Check if FIFO reset is reported to be released within 1ms time interval */
 ...
 uint8_t _falcon_furia_rdb_uc_var( const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr) {
...

    {
        ...
        if (uc_lane_info.is_direct_ram_access_avail) {
            EPFUN(falcon_furia_rdb_ram(pa, &rddata, addr, 1));    /* Direct RAM access */
        }
        else {
...
#ifdef ATE_LOG
  USR_PRINTF(("// ATE_LOG IGNORE FOLLOWING READ ACCESS!\n"));
#endif
    {
      falcon_furia_uc_lane_info_st uc_lane_info;
      USR_MEMSET(&uc_lane_info, 0, sizeof(falcon_furia_uc_lane_info_st));
      ...
      if (uc_lane_info.is_direct_ram_access_avail) {
          EFUN(falcon_furia_rdw_ram(pa,&rddata, addr, 1));                    /* Direct RAM access */
      }
      else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            *err_code_p = ERR_CODE_BAD_PTR_OR_INVALID_INPUT;
...
/* Micro Global RAM Byte Write  */
 err_code_t _falcon_furia_wrb_uc_var( const phymod_access_t *pa, uint16_t addr, uint8_t wr_val) {

    {
        ...
        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrb_ram(pa, addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        }
        else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
...
/* Micro Global RAM Word Write  */
 err_code_t _falcon_furia_wrw_uc_var( const phymod_access_t *pa, uint16_t addr, uint16_t wr_val) {
    ...
    {   falcon_furia_uc_lane_info_st uc_lane_info;
        ...
        EFUN(falcon_furia_get_uc_ln_info(pa, &uc_lane_info));
        if (uc_lane_info.is_direct_ram_access_avail) {
            EFUN(falcon_furia_wrw_ram(pa, addr, 1, &wr_val));
            return (ERR_CODE_NONE);
        } else {
            EFUN_PRINTF(("Global Access not available on cores without direct RAM access\n"));
            return (_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT)); /* Use Serdes FW DSC Command Interface for writing RAM */
 ...
 uint16_t _eye_to_mV(uint8_t var, uint8_t ladder_range)
{
 ...
 err_code_t _falcon_furia_get_osr_mode( const phymod_access_t *pa, falcon_furia_osr_mode_st *imode) {
    falcon_furia_osr_mode_st mode;
     ...
     ENULL_MEMSET(&mode, 0, sizeof(falcon_furia_osr_mode_st));
 ...
 err_code_t _falcon_furia_read_lane_state( const phymod_access_t *pa, falcon_furia_lane_state_st *istate) {
  ...
  ENULL_MEMSET(&state, 0, sizeof(falcon_furia_lane_state_st));

  if(!istate)
          return(_error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT));

  EFUN(_falcon_furia_pmd_lock_status( pa, &state.rx_lock, &state.rx_lock_chg));
  ...
  ENULL_MEMSET(&osr_mode, 0, sizeof(falcon_furia_osr_mode_st));
  ...
  EFUN(_falcon_furia_sigdet_status( pa, &state.sig_det, &state.sig_det_chg));
  ESTM(state.rx_ppm = rd_cdr_integ_reg()/84);
  EFUN(falcon_furia_get_clk90_offset( pa, &state.clk90));
  EFUN(falcon_furia_pmd_rdt_reg(pa,DSC_E_RX_PI_CNT_BIN_LD,&reg_data));
 ...
 err_code_t _falcon_furia_read_core_state( const phymod_access_t *pa, falcon_furia_core_state_st *istate) {
  ...
  ENULL_MEMSET(&state, 0, sizeof(falcon_furia_core_state_st));
  ENULL_MEMSET(&core_cfg, 0, sizeof(struct falcon_furia_uc_core_config_st));
  ...
  {  int16_t                           die_temp = 0;
     EFUN(falcon_furia_read_die_temperature( pa, &die_temp));
  ...
  EFUN(_falcon_furia_pll_lock_status( pa, &state.pll_lock, &state.pll_lock_chg));
 ...
 err_code_t _falcon_furia_display_core_state_no_newline( const phymod_access_t *pa ) {
    ...
    EFUN_PRINTF(("%02d  "              ,  falcon_furia_get_core(pa)));
    ...
    EFUN(_falcon_furia_display_pll_to_divider( pa,   state.pll_div));
 ...
 err_code_t _falcon_furia_pll_lock_status( const phymod_access_t *pa, uint8_t *pll_lock, uint8_t *pll_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(falcon_furia_pmd_rdt_reg(pa,PLL_STATUS_ADDR, &rddata));
 ...
 err_code_t _falcon_furia_pmd_lock_status( const phymod_access_t *pa, uint8_t *pmd_lock, uint8_t *pmd_lock_chg) {
    ...
    uint16_t rddata;
    EFUN(falcon_furia_pmd_rdt_reg(pa,PMD_LOCK_STATUS_ADDR, &rddata));
 ...
 err_code_t _falcon_furia_sigdet_status( const phymod_access_t *pa, uint8_t *sig_det, uint8_t *sig_det_chg) {
    ...
    uint16_t rddata;
    EFUN(falcon_furia_pmd_rdt_reg(pa,SIGDET_STATUS_ADDR, &rddata));
...
#undef BFMASK
 ...
 err_code_t _falcon_furia_display_ber_scan_data( const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    uint8_t range250;
    falcon_furia_osr_mode_st osr_mode;
    struct falcon_furia_uc_core_config_st core_config;

    ENULL_MEMSET(&core_config,0,sizeof(core_config));
    ENULL_MEMSET(&osr_mode,0,sizeof(osr_mode));

    EFUN(falcon_furia_get_uc_core_config( pa, &core_config));
    EFUN(_falcon_furia_get_osr_mode( pa, &osr_mode));
    ...
    for(i=0;i < cnt;i++) {
        ...
        EFUN(falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_WORD, 0, 100));
        ...
        EFUN(falcon_furia_pmd_uc_cmd( pa,  CMD_READ_DIAG_DATA_BYTE, 0, 100));
    ...
    EFUN_PRINTF(("\n"));
    EFUN(falcon_furia_pmd_uc_cmd( pa, CMD_CAPTURE_BER_END,0x00,2000));
