/***********************************************************************************
***********************************************************************************
*  File Name     :  phy8806x_tsc_functions.c                                        *
*  Created On    :  29/04/2013                                                    *
*  Created By    :  Kiran Divakar                                                 *
*  Description   :  APIs for Serdes IPs                                           *
*  Revision      :  $Id: phy8806x_tsc_functions.c 1346 2016-02-06 00:09:35Z kirand $ *
*                                                                                 *
...
#include "phy8806x_tsc_field_access_c.h"
#include "phy8806x_tsc_internal_c.h"
#include "phy8806x_tsc_dv_functions_c.h"



    #include "phy8806x_tsc_pll_config_c.h"
#ifndef UINT32_MAX
...
err_code_t phy8806x_tsc_display_eye_scan (const phymod_access_t *pa){
	...
	EFUN(phy8806x_tsc_display_eye_scan_header (pa, 1));
	/* start horizontal acquisition */
	{   err_code_t err_code = phy8806x_tsc_meas_eye_scan_start (pa, EYE_SCAN_HORIZ);
	    if (err_code)
		    EFUN((phy8806x_tsc_meas_eye_scan_done (pa ), err_code)); }

	/* display stripes */
	for (y = 62; y >= -62; y = y - 2) {
		{   err_code_t err_code = phy8806x_tsc_read_eye_scan_stripe (pa, &stripe[0], &status);
		    if (err_code)
			    EFUN((phy8806x_tsc_meas_eye_scan_done (pa ), err_code)); }
		EFUN(phy8806x_tsc_display_eye_scan_stripe (pa, y, &stripe[0]));
		EFUN_PRINTF(("\n"));
	}
	/* stop acquisition */
	EFUN(phy8806x_tsc_meas_eye_scan_done (pa ));
	EFUN(phy8806x_tsc_display_eye_scan_footer (pa, 1));
...
err_code_t phy8806x_tsc_meas_lowber_eye (const phymod_access_t *pa, const struct phy8806x_tsc_eyescan_options_st eyescan_options, uint32_t *buffer){
	...
	for (y = eyescan_options.vert_max; y >= eyescan_options.vert_min; y = y - eyescan_options.vstep) {
		for (x = eyescan_options.horz_min; x <= eyescan_options.horz_max; x = x + eyescan_options.hstep) {
			/* acquire sample */
			EFUN(phy8806x_tsc_pmd_uc_cmd_with_data (pa, CMD_DIAG_EN, CMD_UC_DIAG_GET_EYE_SAMPLE, ((uint16_t)x) << 8 | (uint8_t)y, 200));
			/* wait for sample complete */
#if 0
			do {
				EFUN(phy8806x_tsc_delay_us(1000));
				ESTM(status = rdv_usr_diag_status());
				EFUN_PRINTF(("status=%04x\n", status));
			} while ((status & 0x8000) == 0);
#else
			EFUN(phy8806x_tsc_poll_diag_done (pa, &status, (((uint32_t)timeout) << 7) * 10 + 20000));
#endif
			EFUN(phy8806x_tsc_prbs_err_count_ll (pa, &errors));
...
err_code_t phy8806x_tsc_meas_eye_scan_start (const phymod_access_t *pa, uint8_t direction){
	...
	if (direction == EYE_SCAN_VERTICAL)
		EFUN(phy8806x_tsc_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_START_VSCAN_EYE, 200));
	else
		EFUN(phy8806x_tsc_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_START_HSCAN_EYE, 200));
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_read_eye_scan_stripe (const phymod_access_t *pa, uint32_t *buffer, uint16_t *status){
	...
	*status = 0;
	for (i = 0; i < 32; i++) {
		{   err_code_t err_code = phy8806x_tsc_poll_diag_eye_data (pa, &val[0], &sts, 200);
		    *status |= sts & 0xF000;
		    if (err_code)
			    return err_code; }
		buffer[i * 2]     = val[0];
...
/*eye_scan_status_t phy8806x_tsc_read_eye_scan_status (pa, void) */
...
err_code_t phy8806x_tsc_meas_eye_scan_done (const phymod_access_t *pa){
	EFUN(phy8806x_tsc_pmd_uc_diag_cmd (pa, CMD_UC_DIAG_DISABLE, 200));
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_start_ber_scan_test (const phymod_access_t *pa, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_CAPTURE_BER_START, ber_scan_mode, 500));
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_read_ber_scan_data (const phymod_access_t *pa, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout){
	...
	*cnt = (sts & 0x00FF) / 3;
	for (i = 0; i < *cnt; i++) {
		...
		EFUN(phy8806x_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, timeout));
		...
		EFUN(phy8806x_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_BYTE, 0, timeout));
...
err_code_t phy8806x_tsc_eye_margin_proj (const phymod_access_t *pa, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control){
	...
	EFUN(phy8806x_tsc_read_ber_scan_data (pa,  &errs[0], &time[0], &cnt, 2000));

	if (verbose > 2) EFUN_PRINTF(("read done cnt=%d\n", cnt));

	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_CAPTURE_BER_END, 0x00, 50));
	...
	EFUN(phy8806x_tsc_display_ber_scan_data (pa, rate, ber_scan_mode, &errs[0], &time[0], (uint8_t)_abs(offset_start)));
...
err_code_t phy8806x_tsc_display_lane_state (const phymod_access_t *pa){
	err_code_t err_code = _phy8806x_tsc_display_lane_state_no_newline (pa );
...
err_code_t phy8806x_tsc_display_core_state_line (const phymod_access_t *pa){
	err_code_t err_code = _phy8806x_tsc_display_core_state_no_newline (pa );
...
err_code_t phy8806x_tsc_display_core_config (const phymod_access_t *pa){
	...
	EFUN_PRINTF((    "***********************************\n\n"));
	{
		...
		struct   phy8806x_tsc_uc_core_config_st pll1_cfg;
		...
		ENULL_MEMSET(&core_cfg, 0, sizeof(core_cfg));
		{
			...
			EFUN(phy8806x_tsc_get_uc_core_config (pa, &pll1_cfg));
...
err_code_t phy8806x_tsc_display_lane_config (const phymod_access_t *pa){
	struct phy8806x_tsc_uc_lane_config_st lane_cfg;
	...
	EFUN_PRINTF((    "*************************************\n\n"                                        ));
	EFUN(                                                         phy8806x_tsc_get_uc_lane_cfg (pa, &lane_cfg)      );
...
err_code_t phy8806x_tsc_display_core_state (const phymod_access_t *pa){
	EFUN(phy8806x_tsc_display_core_state_hdr (pa ));
	EFUN(phy8806x_tsc_display_core_state_line (pa ));
	EFUN(phy8806x_tsc_display_core_state_legend (pa ));
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_display_lane_debug_status (const phymod_access_t *pa){
	/* startup */
	struct phy8806x_tsc_usr_ctrl_disable_functions_st ds;
	struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st dsd;
	/* steady state */
	struct phy8806x_tsc_usr_ctrl_disable_functions_st dss;
	struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st dssd;
	...
	EFUN(phy8806x_tsc_get_usr_ctrl_disable_startup (pa, &ds));
	...
	EFUN(                                                            phy8806x_tsc_get_usr_ctrl_disable_startup_dfe (pa, &dsd) );
	...
	EFUN(                                                           phy8806x_tsc_get_usr_ctrl_disable_steady_state (pa, &dss) );
	...
	EFUN(                                                      phy8806x_tsc_get_usr_ctrl_disable_steady_state_dfe (pa, &dssd) );
...
err_code_t phy8806x_tsc_stop_uc_lane (const phymod_access_t *pa, uint8_t enable){

	if (enable)
		return phy8806x_tsc_pmd_uc_control (pa, CMD_UC_CTRL_STOP_GRACEFULLY, 100);
	else
		return phy8806x_tsc_pmd_uc_control (pa, CMD_UC_CTRL_RESUME, 50);
}


err_code_t phy8806x_tsc_stop_uc_lane_status (const phymod_access_t *pa, uint8_t *uc_lane_stopped){
...
err_code_t phy8806x_tsc_request_stop_rx_adaptation (const phymod_access_t *pa){
	...
	return phy8806x_tsc_pmd_uc_control_return_immediate (pa, CMD_UC_CTRL_STOP_GRACEFULLY);
}
...
err_code_t phy8806x_tsc_ucode_crc_verify (const phymod_access_t *pa, uint16_t ucode_len, uint16_t expected_crc_value){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd_with_data (pa, CMD_CALC_CRC, 0, ucode_len, 200));
...
err_code_t phy8806x_tsc_start_ucode_crc_calc (const phymod_access_t *pa, uint16_t ucode_len){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd_with_data_return_immediate (pa, CMD_CALC_CRC, 0, ucode_len)); /* Invoke Calculate CRC command and return control immediately */
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_check_ucode_crc (const phymod_access_t *pa, uint16_t expected_crc_value, uint32_t timeout_ms){
	...
	err_code = phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 (pa, timeout_ms); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate phy8806x_tsc ready for command */
	if (err_code) {
...
err_code_t phy8806x_tsc_pmd_uc_cmd_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info){
	err_code_t err_code;
	uint16_t cmddata;

	err_code = phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 (pa, 1); /* Poll for uc_dsc_ready_for_cmd = 1 to indicate phy8806x_tsc ready for command */
	if (err_code) {
...
/*EFUN(wr_uc_dsc_ready_for_cmd(0x0)  ); */                     /* Issue command, by clearing "ready for command" field */
	...
	EFUN(phy8806x_tsc_pmd_wr_reg (pa, DSC_A_DSC_UC_CTRL, cmddata));       /* This address is same for Eagle, and all Merlin */

	return ERR_CODE_NONE;
}

err_code_t phy8806x_tsc_pmd_uc_cmd (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint32_t timeout_ms){

	uint8_t uc_dsc_error_found;

	EFUN(phy8806x_tsc_pmd_uc_cmd_return_immediate (pa, cmd, supp_info));          /* Invoke phy8806x_tsc_uc_cmd and return control immediately */

	EFUN(phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 (pa, timeout_ms));       /* Poll for uc_dsc_ready_for_cmd = 1 to indicate phy8806x_tsc ready for command */
...
err_code_t phy8806x_tsc_pmd_uc_cmd_with_data_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data){
	uint16_t cmddata;
	err_code_t err_code;
...
err_code_t phy8806x_tsc_pmd_uc_cmd_with_data (const phymod_access_t *pa, enum srds_pmd_uc_cmd_enum cmd, uint8_t supp_info, uint16_t data, uint32_t timeout_ms){

	uint8_t uc_dsc_error_found;

	EFUN(phy8806x_tsc_pmd_uc_cmd_with_data_return_immediate (pa, cmd, supp_info, data));  /* Invoke phy8806x_tsc_uc_cmd and return control immediately */
...
err_code_t phy8806x_tsc_pmd_uc_control_return_immediate (const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control){
	return phy8806x_tsc_pmd_uc_cmd_return_immediate (pa, CMD_UC_CTRL, (uint8_t)control);
}

err_code_t phy8806x_tsc_pmd_uc_control (const phymod_access_t *pa, enum srds_pmd_uc_ctrl_cmd_enum control, uint32_t timeout_ms){
	return phy8806x_tsc_pmd_uc_cmd (pa, CMD_UC_CTRL, (uint8_t)control, timeout_ms);
}

err_code_t phy8806x_tsc_pmd_uc_diag_cmd (const phymod_access_t *pa, enum srds_pmd_uc_diag_cmd_enum control, uint32_t timeout_ms){
	return phy8806x_tsc_pmd_uc_cmd (pa, CMD_DIAG_EN, (uint8_t)control, timeout_ms);
}
...
uint8_t phy8806x_tsc_rdbl_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint8_t rddata;

	if (!err_code_p)
		return 0;

	EPSTM(rddata = phy8806x_tsc_rdb_uc_ram (pa, err_code_p, (LANE_VAR_RAM_BASE + addr + (phy8806x_tsc_get_lane (pa ) * LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */

	return rddata;
}

/* Micro RAM Lane Byte Signed Read */
int8_t phy8806x_tsc_rdbls_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int8_t)phy8806x_tsc_rdbl_uc_var (pa, err_code_p, addr);
}

/* Micro RAM Lane Word Read */
uint16_t phy8806x_tsc_rdwl_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	uint16_t rddata;
	...
	EPSTM(rddata = phy8806x_tsc_rdw_uc_ram (pa, err_code_p, (LANE_VAR_RAM_BASE + addr + (phy8806x_tsc_get_lane (pa ) * LANE_VAR_RAM_SIZE)))); /* Use Micro register interface for reading RAM */
...
int16_t phy8806x_tsc_rdwls_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint16_t addr){
	return (int16_t)phy8806x_tsc_rdwl_uc_var (pa, err_code_p, addr);
}

/* Micro RAM Lane Byte Write */
err_code_t phy8806x_tsc_wrbl_uc_var (const phymod_access_t *pa, uint16_t addr, uint8_t wr_val){


	if (addr < LANE_VAR_RAM_SIZE)
		return phy8806x_tsc_wrb_uc_ram (pa, (LANE_VAR_RAM_BASE + addr + (phy8806x_tsc_get_lane (pa ) * LANE_VAR_RAM_SIZE)), wr_val); /* Use Micro register interface for writing RAM */
	else
...
err_code_t phy8806x_tsc_wrbls_uc_var (const phymod_access_t *pa, uint16_t addr, int8_t wr_val){
	return phy8806x_tsc_wrbl_uc_var (pa, addr, wr_val);
}

/* Micro RAM Lane Word Write */
err_code_t phy8806x_tsc_wrwl_uc_var (const phymod_access_t *pa, uint16_t addr, uint16_t wr_val){


	if (addr % 2 != 0)                                                            /* Validate even address */
		return _error(ERR_CODE_INVALID_RAM_ADDR);
	if (addr < LANE_VAR_RAM_SIZE)
		return phy8806x_tsc_wrw_uc_ram (pa, (LANE_VAR_RAM_BASE + addr + (phy8806x_tsc_get_lane (pa ) * LANE_VAR_RAM_SIZE)), wr_val); /* Use Micro register interface for writing RAM */
	else
		return _error(ERR_CODE_INVALID_RAM_ADDR);
}

/* Micro RAM Lane Word Signed Write */
err_code_t phy8806x_tsc_wrwls_uc_var (const phymod_access_t *pa, uint16_t addr, int16_t wr_val){
	return phy8806x_tsc_wrwl_uc_var (pa, addr, wr_val);
}
...
uint8_t phy8806x_tsc_rdbc_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr){
	...
	EPSTM(rddata = phy8806x_tsc_rdb_uc_ram (pa, err_code_p, (CORE_VAR_RAM_BASE + addr)));              /* Use Micro register interface for reading RAM */
	return rddata;
}

/* Micro RAM Core Byte Signed Read */
int8_t phy8806x_tsc_rdbcs_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr){
	return (int8_t)phy8806x_tsc_rdbc_uc_var (pa, err_code_p, addr);
}

/* Micro RAM Core Word Read */
uint16_t phy8806x_tsc_rdwc_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr){
	...
	EPSTM(rddata = phy8806x_tsc_rdw_uc_ram (pa, err_code_p, (CORE_VAR_RAM_BASE + addr)));          /* Use Micro register interface for reading RAM */
	return rddata;
}

/* Micro RAM Core Word Signed Read */
int16_t phy8806x_tsc_rdwcs_uc_var (const phymod_access_t *pa, err_code_t *err_code_p, uint8_t addr){
	return (int16_t)phy8806x_tsc_rdwc_uc_var (pa, err_code_p, addr);
}

/* Micro RAM Core Byte Write  */
err_code_t phy8806x_tsc_wrbc_uc_var (const phymod_access_t *pa, uint8_t addr, uint8_t wr_val){


	if (addr < CORE_VAR_RAM_SIZE)
		return phy8806x_tsc_wrb_uc_ram (pa, (CORE_VAR_RAM_BASE + addr), wr_val);                    /* Use Micro register interface for writing RAM */
	else
		return _error(ERR_CODE_INVALID_RAM_ADDR);
}
...
err_code_t phy8806x_tsc_wrbcs_uc_var (const phymod_access_t *pa, uint8_t addr, int8_t wr_val){
	return phy8806x_tsc_wrbc_uc_var (pa, addr, wr_val);
}

/* Micro RAM Core Word Write  */
err_code_t phy8806x_tsc_wrwc_uc_var (const phymod_access_t *pa, uint8_t addr, uint16_t wr_val){
	if (addr % 2 != 0)                                                            /* Validate even address */
		return _error(ERR_CODE_INVALID_RAM_ADDR);
	if (addr < CORE_VAR_RAM_SIZE)
		return phy8806x_tsc_wrw_uc_ram (pa, (CORE_VAR_RAM_BASE + addr), wr_val);                         /* Use Micro register interface for writing RAM */
	else
		return _error(ERR_CODE_INVALID_RAM_ADDR);
}

/* Micro RAM Core Word Signed Write */
err_code_t phy8806x_tsc_wrwcs_uc_var (const phymod_access_t *pa, uint8_t addr, int16_t wr_val){
	return phy8806x_tsc_wrwc_uc_var (pa, addr, wr_val);
}
...
err_code_t phy8806x_tsc_set_uc_core_config (const phymod_access_t *pa, struct phy8806x_tsc_uc_core_config_st struct_val){
	{
		...
		if (reset_state < 7) {
			EFUN_PRINTF(("ERROR: phy8806x_tsc_set_uc_core_config (pa, ..) called without core_dp_s_rstb=0 Lane=%d reset_state=%d\n", phy8806x_tsc_get_lane (pa ), reset_state));
	...
	switch (phy8806x_tsc_get_pll(pa)) {
	case 0:
...
err_code_t phy8806x_tsc_set_usr_ctrl_core_event_log_level (const phymod_access_t *pa, uint8_t core_event_log_level){
...
err_code_t phy8806x_tsc_set_uc_lane_cfg (const phymod_access_t *pa, struct phy8806x_tsc_uc_lane_config_st struct_val){
	...
	if (reset_state < 7) {
		EFUN_PRINTF(("ERROR: phy8806x_tsc_set_uc_lane_cfg (pa, ..) called without ln_dp_s_rstb=0 Lane=%d reset_state=%d\n", phy8806x_tsc_get_lane (pa ), reset_state));
...
err_code_t phy8806x_tsc_set_usr_ctrl_lane_event_log_level (const phymod_access_t *pa, uint8_t lane_event_log_level){
	return wrv_usr_ctrl_lane_event_log_level(lane_event_log_level);
}

err_code_t phy8806x_tsc_set_usr_ctrl_disable_startup (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st set_val){
	EFUN(_update_usr_ctrl_disable_functions_byte (pa, &set_val));
	return wrv_usr_ctrl_disable_startup_functions_word(set_val.word);
}

err_code_t phy8806x_tsc_set_usr_ctrl_disable_startup_dfe (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st set_val){
...
err_code_t phy8806x_tsc_set_usr_ctrl_disable_steady_state (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st set_val){
	EFUN(_update_usr_ctrl_disable_functions_byte (pa, &set_val));
	return wrv_usr_ctrl_disable_steady_state_functions_word(set_val.word);
}

err_code_t phy8806x_tsc_set_usr_ctrl_disable_steady_state_dfe (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st set_val){
	EFUN(_update_usr_ctrl_disable_dfe_functions_byte (pa, &set_val));
...
err_code_t phy8806x_tsc_get_uc_core_config (const phymod_access_t *pa, struct phy8806x_tsc_uc_core_config_st *get_val){
...
err_code_t phy8806x_tsc_get_usr_ctrl_core_event_log_level (const phymod_access_t *pa, uint8_t *core_event_log_level){
...
err_code_t phy8806x_tsc_get_uc_lane_cfg (const phymod_access_t *pa, struct phy8806x_tsc_uc_lane_config_st *get_val){
...
err_code_t phy8806x_tsc_get_usr_ctrl_lane_event_log_level (const phymod_access_t *pa, uint8_t *lane_event_log_level){
...
err_code_t phy8806x_tsc_get_usr_ctrl_disable_startup (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st *get_val){
...
err_code_t phy8806x_tsc_get_usr_ctrl_disable_startup_dfe (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st *get_val){
...
err_code_t phy8806x_tsc_get_usr_ctrl_disable_steady_state (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_functions_st *get_val){
...
err_code_t phy8806x_tsc_get_usr_ctrl_disable_steady_state_dfe (const phymod_access_t *pa, struct phy8806x_tsc_usr_ctrl_disable_dfe_functions_st *get_val){
...
err_code_t phy8806x_tsc_get_clk90_offset_adjust (const phymod_access_t *pa, int8_t *adjust){
	if (!adjust)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
...
err_code_t phy8806x_tsc_set_clk90_offset_adjust (const phymod_access_t *pa, int8_t adjust){
...
err_code_t phy8806x_tsc_get_clk90_offset_override (const phymod_access_t *pa, uint8_t *override){
	if (!override)
		return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
...
err_code_t phy8806x_tsc_set_clk90_offset_override (const phymod_access_t *pa, uint8_t enable, uint8_t override){
...
err_code_t phy8806x_tsc_get_clk90_offset (const phymod_access_t *pa, int8_t *offset){
...
err_code_t phy8806x_tsc_poll_diag_done (const phymod_access_t *pa, uint16_t *status, uint32_t timeout_ms){
	...
	for (loop = 0; loop < 100; loop++) {
		...
		if ((*status & 0x8000) > 0)
			return ERR_CODE_NONE;
		if (loop > 10)
			EFUN(phy8806x_tsc_delay_us(10 * timeout_ms));
	}
...
err_code_t phy8806x_tsc_poll_diag_eye_data (const phymod_access_t *pa, uint32_t *data, uint16_t *status, uint32_t timeout_ms){
	...
	for (loop = 0; loop < 100; loop++) {
		...
		if (((*status & 0x00FF) > 2) || ((*status & 0x8000) > 0)) {
			EFUN(phy8806x_tsc_pmd_uc_cmd (pa,  CMD_READ_DIAG_DATA_WORD, 0, 200));
...
err_code_t phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 (const phymod_access_t *pa, uint32_t timeout_ms){
	...
	for (loop = 0; loop < 100; loop++) {
		uint16_t rddata;
		EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_A_DSC_UC_CTRL, &rddata));
...
#else
	{   uint16_t rddata;
	    EFUN_PRINTF(("// ATE_LOG phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 begin\n"));
	    if (timeout_ms < 100)
		    EFUN(phy8806x_tsc_delay_us(1000 * timeout_ms));
	    else
		    EFUN(phy8806x_tsc_delay_ms(timeout_ms));
	    EFUN(phy8806x_tsc_pmd_rdt_reg (pa, DSC_A_DSC_UC_CTRL, &rddata));
	    if (rddata & 0x0080) {              /* bit 7 is uc_dsc_ready_for_cmd */
		    if (rddata & 0x0040) {      /* bit 6 is uc_dsc_error_found   */
			    ESTM_PRINTF(("ERROR : DSC command returned error (after cmd) cmd = 0x%x, supp_info = 0x%02x !\n", rd_uc_dsc_gp_uc_req(), rd_uc_dsc_supp_info()));
		    ...
		    EFUN_PRINTF(("// ATE_LOG phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 end\n"));
		    return ERR_CODE_NONE;
	    }
	    EFUN_PRINTF(("// ATE_LOG phy8806x_tsc_poll_uc_dsc_ready_for_cmd_equals_1 timeout\n")); }
#endif
...
err_code_t phy8806x_tsc_tx_ull_config (const phymod_access_t *pa, uint8_t enable){
	EFUN(phy8806x_tsc_tx_rptr_mode_timing (pa, enable)); /* Enable jitter filter enable */
...
err_code_t phy8806x_tsc_rx_ull_config (const phymod_access_t *pa, uint8_t enable){
	EFUN(phy8806x_tsc_rx_rptr_mode_timing (pa, enable));
...
err_code_t phy8806x_tsc_tx_rptr_mode_timing (const phymod_access_t *pa, uint8_t enable){
	...
	EFUN(phy8806x_tsc_delay_us(25));                                 /* Wait for TX_PI to settle */
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_init_pram_for_uc_load (const phymod_access_t *pa, uint16_t ucode_len){
	...
	/* Wait 8 pram clocks */
	/* call phy8806x_tsc_finish_pram_load */
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_reg_dump (const phymod_access_t *pa){
	...
	for (addr = 0x0; addr < 0x10; addr++) {
		if (!(addr % 16))
			EFUN_PRINTF(("\n%04x ", addr));
		EFUN(phy8806x_tsc_pmd_rdt_reg (pa, addr, &rddata));
...
err_code_t phy8806x_tsc_uc_core_var_dump (const phymod_access_t *pa){
	uint8_t addr;
	...
	for (addr = 0x0; addr < CORE_VAR_RAM_SIZE; addr++) {
		if (!(addr % 26))
			EFUN_PRINTF(("\n%04x ", addr));
		ESTM_PRINTF(("%02x ", phy8806x_tsc_rdbc_uc_var (pa, __ERR, addr)));
	}
	return ERR_CODE_NONE;
}


err_code_t phy8806x_tsc_uc_lane_var_dump (const phymod_access_t *pa){
	...
	for (addr = 0x0; addr < LANE_VAR_RAM_SIZE; addr++) {
		if (!(addr % 26))
			EFUN_PRINTF(("\n%04x ", addr));
		ESTM_PRINTF(("%02x ", phy8806x_tsc_rdbl_uc_var (pa, __ERR, addr)));
	}
...
err_code_t phy8806x_tsc_display_diag_data (const phymod_access_t *pa, uint16_t diag_level){
	...
	EFUN(phy8806x_tsc_version (pa, &api_version));
	...
	if (diag_level & SRDS_DIAG_LANE) {
		...
		EFUN(phy8806x_tsc_log_full_pmd_state (pa, &lane_st));
		EFUN(phy8806x_tsc_disp_full_pmd_state (pa, &lane_st, 1));
	} else {
		EFUN(phy8806x_tsc_display_lane_state_hdr (pa ));
		EFUN(phy8806x_tsc_display_lane_state (pa ));
	}
	...
	if (diag_level & SRDS_DIAG_EVENT) {
		...
		EFUN(phy8806x_tsc_read_event_log (pa, trace_mem, EVENT_LOG_HEX));
	}
	if (diag_level & SRDS_DIAG_EYE)
		EFUN(phy8806x_tsc_display_eye_scan (pa ));
	if (diag_level & SRDS_DIAG_REG_CORE)
		EFUN(phy8806x_tsc_reg_dump (pa ));
	/* currently REG_CORE and REG_LANE dump same data. */
	/*if(diag_level & SRDS_DIAG_REG_LANE) {*/
	/*    EFUN(phy8806x_tsc_reg_dump (pa ));*/
	/*}*/
	if (diag_level & SRDS_DIAG_UC_CORE)
		EFUN(phy8806x_tsc_uc_core_var_dump (pa ));
	if (diag_level & SRDS_DIAG_UC_LANE)
		EFUN(phy8806x_tsc_uc_lane_var_dump (pa ));
	if (diag_level & SRDS_DIAG_LANE_DEBUG)
		EFUN(phy8806x_tsc_display_lane_debug_status (pa ));
	if (diag_level & SRDS_DIAG_BER_VERT) {
		...
		EFUN(_phy8806x_tsc_display_ber_scan_data (pa, ber_mode, timer_control,  err_threshold));
...
err_code_t phy8806x_tsc_diag_access (const phymod_access_t *pa, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param){
	...
	case SRDS_REG_READ:      {
		...
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				EFUN(phy8806x_tsc_pmd_rdt_reg (pa, i + addr, &tmp));
				ESTM_PRINTF(("%04x ", tmp));
			}
			EFUN_PRINTF(("\n"));
		} else {
			EFUN(phy8806x_tsc_pmd_rdt_reg (pa, addr, &tmp));
	...
	case SRDS_REG_RMW:       {
		...
		EFUN(phy8806x_tsc_pmd_wr_reg (pa, addr, tmp2));
	...
	case SRDS_CORE_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < (uint8_t)data; i++) {
				if (!(i % 26))
					ESTM_PRINTF(("\n%04x ", i + (uint8_t)addr));
				ESTM_PRINTF(("%02x ", phy8806x_tsc_rdbc_uc_var (pa, __ERR, i + (uint8_t)addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_LANE_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 26))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%02x ", phy8806x_tsc_rdbl_uc_var (pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_CORE_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < (uint8_t)data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + (uint8_t)addr));
				ESTM_PRINTF(("%04x ", phy8806x_tsc_rdwc_uc_var (pa, __ERR, i + (uint8_t)addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_LANE_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i = i + addr));
				ESTM_PRINTF(("%04x ", phy8806x_tsc_rdwl_uc_var (pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_CORE_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = phy8806x_tsc_rdbc_uc_var (pa, __ERR, (uint8_t)addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(phy8806x_tsc_wrbc_uc_var (pa, (uint8_t)addr, tmp2));
		EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n", addr, tmp, tmp2));
	} break;
	case SRDS_LANE_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = phy8806x_tsc_rdbl_uc_var (pa, __ERR, addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(phy8806x_tsc_wrbl_uc_var (pa, addr, tmp2));
		EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n", addr, tmp, tmp2));
	} break;
	case SRDS_CORE_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = phy8806x_tsc_rdwc_uc_var (pa, __ERR, (uint8_t)addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(phy8806x_tsc_wrwc_uc_var (pa, (uint8_t)addr, tmp2));
		EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n", addr, tmp, tmp2));
	} break;
	case SRDS_LANE_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = phy8806x_tsc_rdwl_uc_var (pa, __ERR, addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(phy8806x_tsc_wrwl_uc_var (pa, addr, tmp2));
	...
	case SRDS_GLOB_RAM_READ_BYTE: {
		if (data > 1) {
			...
			for (i = 0; i < data; i++) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%02x ", _phy8806x_tsc_rdb_uc_var (pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_GLOB_RAM_RMW_BYTE:  {
		uint8_t tmp, tmp2;
		ESTM(tmp = _phy8806x_tsc_rdb_uc_var (pa, __ERR, addr));
		tmp2 = (tmp & (uint8_t) ~param) | ((uint8_t)param & data);
		EFUN(_phy8806x_tsc_wrb_uc_var (pa, addr, tmp2));
	...
	case SRDS_GLOB_RAM_READ_WORD: {
		if (data > 1) {
			...
			for (i = 0; i < data; i += 2) {
				if (!(i % 16))
					ESTM_PRINTF(("\n%04x ", i + addr));
				ESTM_PRINTF(("%04x ", _phy8806x_tsc_rdw_uc_var (pa, __ERR, i + addr)));
			}
			EFUN_PRINTF(("\n"));
		} else
	...
	case SRDS_GLOB_RAM_RMW_WORD:  {
		uint16_t tmp, tmp2;
		ESTM(tmp = _phy8806x_tsc_rdw_uc_var (pa, __ERR, addr));
		tmp2 = (tmp & ~param) | (param & data);
		EFUN(_phy8806x_tsc_wrw_uc_var (pa, addr, tmp2));
		EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n", addr, tmp, tmp2));
	} break;
	case SRDS_UC_CMD:        {
		uint16_t tmp;
		EFUN(phy8806x_tsc_pmd_uc_cmd_with_data (pa, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
		ESTM(tmp = rd_uc_dsc_data());
		EFUN_PRINTF(("uC Command: cmd=x%02x supp=x%02x data=x%04x returned=x%04x\n", addr, param, data, tmp));
	} break;
	case SRDS_BER_PROJ_DATA: {
		/* display ber projections for all channels */
		EFUN(_phy8806x_tsc_display_ber_scan_data (pa, (uint8_t)addr, (uint8_t)data,  (uint8_t)(param >> 4)));
	} break;
#ifdef TDT_CHARACTERIZATION
	case SRDS_TDT_CHAR:      {
		EFUN(phy8806x_tsc_tdt_char (pa, (enum srds_tdt_pattern_enum)addr, (uint8_t)data,  (uint8_t)param));
	} break;
#endif
	...
	default: EFUN_PRINTF(("Invalid request type phy8806x_tsc_diag_access\n"));
	}
...
err_code_t phy8806x_tsc_read_event_log (const phymod_access_t *pa, uint8_t *trace_mem, enum srds_event_log_display_mode_enum display_mode){
	...
	EFUN(phy8806x_tsc_event_log_stop (pa ));

	EFUN(phy8806x_tsc_event_log_readmem (pa, trace_mem));

	EFUN(phy8806x_tsc_event_log_display (pa, trace_mem, display_mode));

	return ERR_CODE_NONE;
}

err_code_t phy8806x_tsc_event_log_stop (const phymod_access_t *pa){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_START, 10));

	return ERR_CODE_NONE;
}

err_code_t phy8806x_tsc_event_log_readmem (const phymod_access_t *pa, uint8_t *trace_mem){
	...
	do {
		/* Read Next */
		EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_NEXT, 10));
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_EVENT_LOG_READ, CMD_EVENT_LOG_READ_DONE, 10));

	return ERR_CODE_NONE;
}

err_code_t phy8806x_tsc_event_log_display (const phymod_access_t *pa, uint8_t *trace_mem, enum srds_event_log_display_mode_enum display_mode){
		...
		while (addr < trace_mem_size - 8) {                                                     /* last 8-byte reserved for trace memory wraparound */
			...
			if (*(trace_mem + addr) == 0xff) {                                           /* timestamp wraparound event handling */
				...
				if (cc < 10) {
					...
					uc_lane_id_str[1] = '\0';
                /* CID 2009523 Logically DeadCode :  ignore */
				} else if (cc < 100) {
				...
				EFUN(phy8806x_tsc_uc_lane_idx_to_system_id(uc_lane_id_str, cc));
...
err_code_t phy8806x_tsc_display_state (const phymod_access_t *pa){
	EFUN(phy8806x_tsc_display_core_state (pa ));
	EFUN(phy8806x_tsc_display_lane_state_hdr (pa ));
	EFUN(phy8806x_tsc_display_lane_state (pa ));
	EFUN(phy8806x_tsc_display_lane_state_legend (pa ));
	return ERR_CODE_NONE;
}

err_code_t phy8806x_tsc_display_config (const phymod_access_t *pa){
	EFUN(phy8806x_tsc_display_core_config (pa ));
	EFUN(phy8806x_tsc_display_lane_config (pa ));
	return ERR_CODE_NONE;
}
...
err_code_t phy8806x_tsc_read_die_temperature (const phymod_access_t *pa, int16_t *die_temp){
	...
	EFUN(phy8806x_tsc_pmd_uc_cmd (pa, CMD_UC_DBG, CMD_UC_DBG_DIE_TEMP, 100));
...
err_code_t phy8806x_tsc_read_die_temperature_double (const phymod_access_t *pa, USR_DOUBLE *die_temp){
	...
	*die_temp = 410.04 - 0.48705 * ((USR_DOUBLE)die_temp_sensor_reading);
	...
	return ERR_CODE_NONE;
#else
	EFUN_PRINTF((" Function 'phy8806x_tsc_read_die_temperature_double' needs SERDES_API_FLOATING_POINT defined to operate \n"));
	return _error(ERR_CODE_BAD_PTR_OR_INVALID_INPUT);
#endif
}
...
err_code_t phy8806x_tsc_fctts_sn_to_trn (const phymod_access_t *pa, uint16_t xmt_update_page, uint16_t xmt_status_page, uint8_t tts_prbs_sel){
	...
	if (tts_prbs_sel == 0)
		...
		EFUN_PRINTF(("phy8806x_tsc_fctts_sn_to_trn: Invalid TTS PRBS traffic selection."));
	...
	EFUN(phy8806x_tsc_poll_fctts_sn_to_trn_req (pa, 1000));
