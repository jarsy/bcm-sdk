...
err_code_t merlin16_config_shared_tx_pattern(srds_access_t *sa__, uint8_t patt_length, const char pattern[]) {
  ...
  uint8_t    zero_pad_len    = 0; /* suppress warnings, changed by merlin16_INTERNAL_calc_patt_gen_mode_sel() */
  uint16_t   patt_gen_wr_val = 0;
  uint8_t    mode_sel        = 0; /* suppress warnings, changed by merlin16_INTERNAL_calc_patt_gen_mode_sel() */

  EFUN(merlin16_INTERNAL_calc_patt_gen_mode_sel(&mode_sel,&zero_pad_len,patt_length));
  ...
  if ((str_len > 2) && ((USR_STRNCMP(pattern, "0x", 2)) == 0)) {
    /* Hexadecimal Pattern */
    for (i=2; i < str_len; i++) {
      EFUN(merlin16_INTERNAL_compute_bin(pattern[i],bin));
  ...
  for (i=0; i < 15; i++) {
    ...
    for (j=0; j < 4; j++) {
      ...
      bin[4] = '\0';
      EFUN(merlin16_INTERNAL_compute_hex(bin, &hex));
...
err_code_t merlin16_prbs_err_count_state(srds_access_t *sa__, uint32_t *prbs_err_cnt, uint8_t *lock_lost) {
    ...
    EFUN(merlin16_prbs_err_count_ll(sa__, prbs_err_cnt));
...
err_code_t merlin16_display_detailed_prbs_state(srds_access_t *sa__) {
    ...
    ESTM_PRINTF(("  %d ",merlin16_get_lane(sa__)));
    ...
    EFUN(merlin16_prbs_err_count_state(sa__,&err_cnt,&lock_lost));
