...
err_code_t merlin16_stop_rx_adaptation(srds_access_t *sa__, uint8_t enable) {
  ...
  if (enable) {
    return(merlin16_pmd_uc_control(sa__, CMD_UC_CTRL_STOP_GRACEFULLY,100));
  }
  else {
...
err_code_t merlin16_loop_timing(srds_access_t *sa__, uint8_t enable) {
    ...
    if (enable) {
        ...
        USR_DELAY_US(25);               /* Wait for tclk to lock to CDR */
    } 
    else {
...
err_code_t merlin16_rmt_lpbk(srds_access_t *sa__, uint8_t enable) {
    if (enable) {
        ...
        USR_DELAY_US(50);        /* Wait for rclk and tclk phase lock before expecing good data from rmt loopback */
...
err_code_t merlin16_tx_rptr_mode_timing(srds_access_t *sa__, uint8_t enable) {
        ...
        USR_DELAY_US(25);                               /* Wait for TX_PI to settle */
    return (ERR_CODE_NONE);
}
...
err_code_t merlin16_uc_reset(srds_access_t *sa__, uint8_t enable) {
  if (enable) {
    ...
    EFUN(merlin16_pmd_wr_reg(sa__, 0xD225, 0x8401));
...
err_code_t merlin16_init_merlin16_info(srds_access_t *sa__) {
    ...
    ENULL_MEMSET(merlin16_info_ptr, 0, sizeof(merlin16_info_t));
    ...
    EFUN(merlin16_rdblk_uc_prog_ram(sa__, (uint8_t *)info_table, INFO_TABLE_RAM_BASE, INFO_TABLE_END));
    ...
    if (merlin16_info_ptr->lane_var_ram_size != LANE_VAR_RAM_SIZE) {
        return (_error(ERR_CODE_INFO_TABLE_ERROR));
    }
    ...
    if (merlin16_info_ptr->core_var_ram_base != (0x20000000 | CORE_VAR_RAM_BASE)) {
        return (_error(ERR_CODE_INFO_TABLE_ERROR));
    }
    ...
    if (merlin16_info_ptr->lane_var_ram_base != (0x20000000 | LANE_VAR_RAM_BASE)) {
        return (_error(ERR_CODE_INFO_TABLE_ERROR));
    }
...
err_code_t merlin16_configure_pll_refclk_div(srds_access_t *sa__,
    ...
    return merlin16_INTERNAL_configure_pll(sa__, refclk, div, 0, 0);
}

err_code_t merlin16_configure_pll_refclk_vco(srds_access_t *sa__,
                                           enum merlin16_pll_refclk_enum refclk,
                                           uint32_t vco_freq_khz) {
    return merlin16_INTERNAL_configure_pll(sa__, refclk, MERLIN16_PLL_DIV_UNKNOWN, vco_freq_khz, 0);
}

err_code_t merlin16_configure_pll_div_vco(srds_access_t *sa__,
                                        enum merlin16_pll_div_enum div,
                                        uint32_t vco_freq_khz) {
    return merlin16_INTERNAL_configure_pll(sa__, MERLIN16_PLL_REFCLK_UNKNOWN, div, vco_freq_khz, 0);
}
    ...
    for(lane = 0;lane<num_lanes;++lane) {
        ...
        if(enable) {
           ...
           /* Initial TX lane clock phase alignment. Program following registers only for slave lanes, master lane registers should not be programmed (can be kept in default values). */
                ...
                USR_DELAY_US(25);                   /*  wait for the slave lane clocks to lock to the master lane clock */
                ...
                USR_DELAY_US(60);                   /*  wait for the slave lane clocks to lock to the master lane clock withing 1/16th of UI. */
