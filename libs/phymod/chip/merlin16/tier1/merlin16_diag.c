...
err_code_t merlin16_display_eye_scan(srds_access_t *sa__) {
    uint32_t   stripe[64];
    uint16_t   status = 0;
    int8_t     y;

    EFUN(merlin16_display_eye_scan_header(1));

    /* start horizontal acquisition */
    {   err_code_t err_code = merlin16_meas_eye_scan_start(sa__, EYE_SCAN_HORIZ);
        if (err_code) {
            EFUN((merlin16_meas_eye_scan_done(sa__), err_code));
        }
    }
    ...
    for (y = 31;y>=-31;y=y-1)
    {
        {   err_code_t err_code = merlin16_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (err_code) {
                EFUN((merlin16_meas_eye_scan_done(sa__), err_code));
            }
        }
        EFUN(merlin16_display_eye_scan_stripe(sa__, y,&stripe[0]));
        EFUN_PRINTF(("\n"));
    }
    /* stop acquisition */
    EFUN(merlin16_meas_eye_scan_done(sa__));
    EFUN(merlin16_display_eye_scan_footer(1));

    return(ERR_CODE_NONE);
}

err_code_t merlin16_meas_eye_scan_start(srds_access_t *sa__, uint8_t direction) {
    uint8_t lock;
    ...
    if(direction == EYE_SCAN_VERTICAL) {
        EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_VSCAN_EYE,200));
    } else {
        EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_START_HSCAN_EYE,200));
    }
    return(ERR_CODE_NONE);
}

static err_code_t merlin16_poll_diag_data(srds_access_t *sa__, const merlin16_info_t *merlin16_info_ptr, uint16_t *status, uint8_t *diag_rd_ptr, uint8_t byte_count, uint32_t timeout_ms) {
    const uint32_t lane_diag_size = merlin16_info_ptr->diag_mem_ram_size;
    uint8_t loop;
    ...
    *diag_rd_ptr = 0;
    ...
    if (byte_count > (lane_diag_size / 2)) {
        ESTM_PRINTF(("\nERROR : merlin16_poll_diag_data() has excessive byte count of %d.\n", byte_count));
...
typedef struct {
    uint32_t *buffer_ptr;
    } merlin16_read_eye_scan_stripe_callback_arg_t;

static err_code_t merlin16_read_eye_scan_stripe_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_read_eye_scan_stripe_callback_arg_t * const cast_arg = (merlin16_read_eye_scan_stripe_callback_arg_t *)arg;
    *(cast_arg->buffer_ptr++) = merlin16_INTERNAL_float8_to_int32((float8_t)(data & 0xFF));
    if (byte_count > 1) {
        *(cast_arg->buffer_ptr++) = merlin16_INTERNAL_float8_to_int32((float8_t)(data >> 8));
    }
    return (ERR_CODE_NONE);
}

err_code_t merlin16_read_eye_scan_stripe(srds_access_t *sa__, uint32_t *buffer, uint16_t *status) {
    merlin16_info_t const * const merlin16_info_ptr = merlin16_INTERNAL_get_merlin16_info_ptr();
    const uint8_t lane = merlin16_get_lane(sa__);
    const uint8_t stripe_size = 64;

    merlin16_read_eye_scan_stripe_callback_arg_t arg;
    ...
    lane_diag_base = merlin16_info_ptr->diag_mem_ram_base + (lane * merlin16_info_ptr->diag_mem_ram_size);

    EFUN(merlin16_poll_diag_data(sa__, merlin16_info_ptr, status, &diag_rd_ptr, stripe_size, 400));
        
    arg.buffer_ptr = buffer;
    ESTM(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__,
                                              lane_diag_base,
                                              merlin16_info_ptr->diag_mem_ram_size,
                                              diag_rd_ptr,
                                              stripe_size,
                                              &arg,
                                              merlin16_read_eye_scan_stripe_callback));

    diag_rd_ptr = ((uint32_t)diag_rd_ptr + stripe_size) % merlin16_info_ptr->diag_mem_ram_size;
    ESTM(wrv_usr_diag_rd_ptr(diag_rd_ptr));
    ESTM(*status = rdv_usr_diag_status() & 0xFF);
    return(ERR_CODE_NONE);
}

err_code_t merlin16_display_eye_scan_stripe(srds_access_t *sa__, int8_t y,uint32_t *buffer) {
    char line[1024];
    ...
    level = merlin16_INTERNAL_ladder_setting_to_mV(sa__, y,data_thresh);
...
err_code_t merlin16_display_eye_scan_header(int8_t i) {
int8_t x;
    EFUN_PRINTF(("\n"));
...
err_code_t merlin16_display_eye_scan_footer(int8_t i) {
int8_t x;
    for(x=1;x<=i;x++) {
        EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    }
...
err_code_t merlin16_read_eye_scan_status(srds_access_t *sa__, uint16_t *status) {
...
err_code_t merlin16_meas_eye_scan_done(srds_access_t *sa__) {
  EFUN(merlin16_pmd_uc_diag_cmd(sa__, CMD_UC_DIAG_DISABLE,200));
  return(ERR_CODE_NONE);
}


err_code_t merlin16_start_ber_scan_test(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    uint8_t lock,sts;
    ESTM(lock = rd_pmd_rx_lock());
    if(lock == 0) {
    ...
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_START, ber_scan_mode,500));
    return(ERR_CODE_NONE);
}

err_code_t merlin16_read_ber_scan_data(srds_access_t *sa__, uint32_t *errors, uint32_t *timer_values, uint8_t *cnt, uint32_t timeout) {
 ...
 ESTM(sts = rdv_usr_diag_status());
    ...
    *cnt = (sts & 0x00FF)/3;
    for(i=0;i < *cnt;i++) {
        ...
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, timeout));
        ...
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, timeout));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors[i] = merlin16_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values[i] = (merlin16_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
...
err_code_t merlin16_eye_margin_proj(srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    EFUN(merlin16_start_ber_scan_test(sa__, ber_scan_mode, timer_control, max_error_control));
    ...
    if(verbose > 5) {
        ...
        EFUN(merlin16_INTERNAL_poll_diag_done(sa__, &sts,timer_control*2000));
    }
    if(verbose > 2) EFUN_PRINTF(("delay done\n"));

    EFUN(merlin16_read_ber_scan_data(sa__, &errs[0], &time[0], &cnt, 2000));

    if(verbose > 2) EFUN_PRINTF(("read done cnt=%d\n",cnt));

    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,50));
 ...
 /* }                                                                  */
    ...
    EFUN(merlin16_display_ber_scan_data(sa__, rate, ber_scan_mode, &errs[0], &time[0],(uint8_t)SRDS_ABS(offset_start)));
...
err_code_t merlin16_display_ber_scan_data (srds_access_t *sa__, USR_DOUBLE rate, uint8_t ber_scan_mode, uint32_t *total_errs, uint32_t *total_time, uint8_t max_offset) {
    ...
    do {
        if(heye == 1) {
            ...
            if(intrusive) {
                ...
                margins[i] = direction*merlin16_INTERNAL_ladder_setting_to_mV(sa__, offset[i], range250);
            }
#else
            margins[i] = info_out->margins[i];
#endif
...
err_code_t merlin16_display_lane_state_hdr(void) {
...
err_code_t merlin16_display_lane_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**********************************************************************************************\n"));
...
err_code_t merlin16_display_lane_state(srds_access_t *sa__) {
  err_code_t err_code = merlin16_INTERNAL_display_lane_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin16_display_core_state_hdr(void) {
...
err_code_t merlin16_display_core_state_line(srds_access_t *sa__) {
  err_code_t err_code = merlin16_INTERNAL_display_core_state_no_newline(sa__);
  EFUN_PRINTF(("\n"));
  return (err_code);
}

err_code_t merlin16_display_core_state_legend(void) {
  EFUN_PRINTF(("\n"));
  EFUN_PRINTF(("**************************************************************************************************************\n"));
...
err_code_t merlin16_display_core_config(srds_access_t *sa__) {
    EFUN_PRINTF(("\n\n***********************************\n"  ));
...
err_code_t merlin16_display_lane_config(srds_access_t *sa__) {
...
err_code_t merlin16_display_core_state(srds_access_t *sa__) {
    EFUN(merlin16_display_core_state_hdr());
    EFUN(merlin16_display_core_state_line(sa__));
    EFUN(merlin16_display_core_state_legend());
    return ERR_CODE_NONE;
}



err_code_t merlin16_display_lane_debug_status(srds_access_t *sa__) {
    /* startup */
    struct merlin16_usr_ctrl_disable_functions_st     ds;
    struct merlin16_usr_ctrl_disable_dfe_functions_st dsd;
    /* steady state */
    struct merlin16_usr_ctrl_disable_functions_st     dss;
    struct merlin16_usr_ctrl_disable_dfe_functions_st dssd;
    ...
    EFUN(merlin16_get_usr_ctrl_disable_startup(sa__, &ds));
    ...
    EFUN(                                                            merlin16_get_usr_ctrl_disable_startup_dfe(sa__, &dsd));
    ...
    EFUN(                                                           merlin16_get_usr_ctrl_disable_steady_state(sa__, &dss));
    ...
    EFUN(                                                      merlin16_get_usr_ctrl_disable_steady_state_dfe(sa__, &dssd));
...
static err_code_t _display_ber_scan_data(srds_access_t *sa__, uint8_t ber_scan_mode, uint8_t timer_control, uint8_t max_error_control) {
    ...
    for(i=0;i < cnt;i++) {
        /* Read 2 bytes of data */
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_WORD, 0, 100));
        ESTM(dataword = rd_uc_dsc_data());           /* LSB contains 2 -4bit nibbles */
        time_byte = (uint8_t)(dataword>>8);          /* MSB is time byte */
        prbs_multi = (uint8_t)dataword & 0x0F;       /* split nibbles */
        time_multi = (uint8_t)dataword>>4;
        /* Read 1 bytes of data */
        EFUN(merlin16_pmd_uc_cmd(sa__, CMD_READ_DIAG_DATA_BYTE, 0, 100));
        ESTM(prbs_byte = (uint8_t)rd_uc_dsc_data());
        errors = merlin16_INTERNAL_float12_to_uint32(prbs_byte,prbs_multi); /* convert 12bits to uint32 */
        timer_values = (merlin16_INTERNAL_float12_to_uint32(time_byte,time_multi)<<3);
        ...
        if(ber_scan_mode & DIAG_BER_HORZ) {
            ...
            EFUN_PRINTF(("%d %d %d ",direction*merlin16_INTERNAL_ladder_setting_to_mV(sa__, (int8_t)SRDS_ABS(offset_start)-i, range250),errors,timer_values));
        }
    }
    EFUN_PRINTF(("\n"));
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_CAPTURE_BER_END,0x00,2000));
...
err_code_t merlin16_display_diag_data(srds_access_t *sa__, uint16_t  diag_level) {
    ...
    EFUN(merlin16_version(&api_version));
    ...
    {
        EFUN(merlin16_display_lane_state_hdr());
        EFUN(merlin16_display_lane_state(sa__));
    }
    ...
    if(diag_level & SRDS_DIAG_EVENT_SAFE) {
...
#endif
        {
            ...
            EFUN(merlin16_INTERNAL_read_event_log_with_callback(sa__, micro_num, 1, &state, merlin16_INTERNAL_event_log_dump_callback));
            EFUN(merlin16_INTERNAL_event_log_dump_callback(&state, 0, 0));
        }
    }
    if(diag_level & SRDS_DIAG_EVENT) {
        EFUN(merlin16_read_event_log(sa__));
    }
    if(diag_level & SRDS_DIAG_EYE) {
        EFUN(merlin16_display_eye_scan(sa__));
    }
    /* currently REG_CORE and REG_LANE dump same data. */
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        EFUN(merlin16_reg_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_CORE) {
        EFUN(merlin16_uc_core_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_UC_LANE) {
        EFUN(merlin16_uc_lane_var_dump(sa__));
    }
    if(diag_level & SRDS_DIAG_LANE_DEBUG) {
        EFUN(merlin16_display_lane_debug_status(sa__));
    }
...
} merlin16_diag_access_callback_state_t;

static err_code_t merlin16_diag_access_read_byte_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_diag_access_callback_state_t * const state_ptr = (merlin16_diag_access_callback_state_t *)arg;
    while (byte_count > 0) {
...
static err_code_t merlin16_diag_access_read_word_callback(void *arg, uint8_t byte_count, uint16_t data) {
    merlin16_diag_access_callback_state_t * const state_ptr = (merlin16_diag_access_callback_state_t *)arg;
    if (state_ptr->line_byte_counter == 0) {
        USR_PRINTF(("\n%04x  ", state_ptr->addr));
    }
...
/* Required Diagnostic Functions */
err_code_t merlin16_diag_access(srds_access_t *sa__, enum srds_diag_access_enum type, uint16_t addr, uint16_t data, uint16_t param) {
    merlin16_info_t * const merlin16_info_ptr = merlin16_INTERNAL_get_merlin16_info_ptr();
    merlin16_diag_access_callback_state_t state;
    ...
    case SRDS_REG_READ:      {
                             ...
                             if(data > 1) {
                                 ...
                                 for (i = 0; i < data; i++) {
                                     ...
                                     EFUN(merlin16_pmd_rdt_reg(sa__, i+addr,&tmp));
                                     ESTM_PRINTF(("%04x ", tmp));
                                 }
                                 EFUN_PRINTF(("\n"));
                             } else {
                                 EFUN(merlin16_pmd_rdt_reg(sa__, addr,&tmp));
    ...
    case SRDS_REG_RMW:       {
                             ...
                             EFUN(merlin16_pmd_wr_reg(sa__, addr,tmp2));
    ...
    case SRDS_CORE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           data,
                                                                           &state,
                                                                           merlin16_diag_access_read_byte_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
    ...
    case SRDS_LANE_RAM_READ_BYTE: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           (addr + merlin16_info_ptr->lane_var_ram_base
                                                                            + (merlin16_get_lane(sa__) * merlin16_info_ptr->lane_var_ram_size)),
                                                                           data,
                                                                           0,
                                                                           data,
                                                                           &state,
    ...
    case SRDS_CORE_RAM_READ_WORD: {
                             if(data > 1) {
                                 ...
                                 EFUN(merlin16_INTERNAL_rdblk_uc_generic_ram(sa__,
                                                                           ...
                                                                           ((data+1)>>1)<<1, /* Round up to nearest word count */
                                                                           &state,
                                                                           merlin16_diag_access_read_word_callback));
                                 EFUN_PRINTF(("\n"));
                             } else {
    ...
    case SRDS_CORE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_rdbc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin16_wrbc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_rdbl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin16_wrbl_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Lane RAM RMW byte: x%04x = x%02x -> x%02x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_CORE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_rdwc_uc_var(sa__, __ERR, (uint8_t)addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin16_wrwc_uc_var(sa__, (uint8_t)addr,tmp2));
                             EFUN_PRINTF(("Core RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_LANE_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_rdwl_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin16_wrwl_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_BYTE:  {
                             uint8_t tmp, tmp2;
                             ESTM(tmp = merlin16_INTERNAL_rdb_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & (uint8_t)~param) | ((uint8_t)param & data);
                             EFUN(merlin16_INTERNAL_wrb_uc_var(sa__, addr,tmp2));
    ...
    case SRDS_GLOB_RAM_RMW_WORD:  {
                             uint16_t tmp, tmp2;
                             ESTM(tmp = merlin16_INTERNAL_rdw_uc_var(sa__, __ERR,addr));
                             tmp2 = (tmp & ~param) | (param & data);
                             EFUN(merlin16_INTERNAL_wrw_uc_var(sa__, addr,tmp2));
                             EFUN_PRINTF(("Glob RAM RMW word: x%04x = x%04x -> x%04x\n",addr,tmp,tmp2));
                             } break;
    case SRDS_UC_CMD:        {
                             uint16_t tmp;
                             EFUN(merlin16_pmd_uc_cmd_with_data(sa__, (enum srds_pmd_uc_cmd_enum)addr, (uint8_t)param, data, 100));
    ...
    default: EFUN_PRINTF(("Invalid request type merlin16_diag_access\n"));
    }

    return(ERR_CODE_NONE);
}

err_code_t merlin16_display_state (srds_access_t *sa__) {
    EFUN(merlin16_display_core_state(sa__));
    EFUN(merlin16_display_lane_state_hdr());
    EFUN(merlin16_display_lane_state(sa__));
    EFUN(merlin16_display_lane_state_legend());
    return(ERR_CODE_NONE);
}

err_code_t merlin16_display_config (srds_access_t *sa__) {
    EFUN(merlin16_display_core_config(sa__));
    EFUN(merlin16_display_lane_config(sa__));
    return(ERR_CODE_NONE);
}
...
err_code_t merlin16_force_die_temperature (srds_access_t *sa__, int16_t die_temp) {
    if (die_temp>130)
        die_temp = 130;
    if (die_temp<-45)
        die_temp = -45;
...
err_code_t merlin16_display_cl72_status(srds_access_t *sa__) {
  EFUN_PRINTF(("\n\n************************\n"                                                                                                                              ));
