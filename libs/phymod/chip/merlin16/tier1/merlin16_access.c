...
uint8_t merlin16_rdbl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  ...
  EPSTM(rddata = merlin16_rdb_uc_ram(sa__, err_code_p, (lane_var_ram_base+addr+(merlin16_get_lane(sa__)*lane_var_ram_size)))); /* Use Micro register interface for reading RAM */
  return (rddata);
}
...
uint16_t merlin16_rdwl_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint16_t addr) {
  ...
  EPSTM(rddata = merlin16_rdw_uc_ram(sa__, err_code_p, (lane_var_ram_base+addr+(merlin16_get_lane(sa__)*lane_var_ram_size)))); /* Use Micro register interface for reading RAM */
  return (rddata);
}
...
err_code_t merlin16_wrbl_uc_var(srds_access_t *sa__, uint16_t addr, uint8_t wr_val) {
  ...
  return (merlin16_wrb_uc_ram(sa__, (lane_var_ram_base+addr+(merlin16_get_lane(sa__)*lane_var_ram_size)), wr_val));    /* Use Micro register interface for writing RAM */
}
...
err_code_t merlin16_wrwl_uc_var(srds_access_t *sa__, uint16_t addr, uint16_t wr_val) {
  ...
  return (merlin16_wrw_uc_ram(sa__, (lane_var_ram_base+addr+(merlin16_get_lane(sa__)*lane_var_ram_size)), wr_val));    /* Use Micro register interface for writing RAM */
}
...
uint8_t merlin16_rdbc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
  ...
  EPSTM(rddata = merlin16_rdb_uc_ram(sa__, err_code_p, (core_var_ram_base+addr)));    /* Use Micro register interface for reading RAM */
  return (rddata);
}
...
uint16_t merlin16_rdwc_uc_var(srds_access_t *sa__, err_code_t *err_code_p, uint8_t addr) {
    ...
    EPSTM(rddata = merlin16_rdw_uc_ram(sa__, err_code_p, (core_var_ram_base+addr)));    /* Use Micro register interface for reading RAM */
    return (rddata);
}
...
err_code_t merlin16_wrbc_uc_var(srds_access_t *sa__, uint8_t addr, uint8_t wr_val) {
    ...
    return (merlin16_wrb_uc_ram(sa__, (core_var_ram_base+addr), wr_val));               /* Use Micro register interface for writing RAM */
}
...
err_code_t merlin16_wrwc_uc_var(srds_access_t *sa__, uint8_t addr, uint16_t wr_val) {
    ...
    return (merlin16_wrw_uc_ram(sa__, (core_var_ram_base+addr), wr_val));             /* Use Micro register interface for writing RAM */
}
...
err_code_t merlin16_read_die_temperature (srds_access_t *sa__, int16_t *die_temp) {
    ...
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,100));
...
err_code_t merlin16_read_die_temperature_double (srds_access_t *sa__, USR_DOUBLE *die_temp) {
    ...
    EFUN(merlin16_pmd_uc_cmd(sa__, CMD_UC_DBG,CMD_UC_DBG_DIE_TEMP,50));
    ESTM(die_temp_sensor_reading=rd_uc_dsc_data());
