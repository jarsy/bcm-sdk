...
err_code_t merlin16_core_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    switch(mode) {
    case PWR_ON:
        EFUN(merlin16_INTERNAL_core_clkgate(sa__, 0));
        EFUN(wrc_afe_s_pll_pwrdn(0x0));
        EFUN(merlin16_core_dp_reset(sa__, 0x0));
        break;
    case PWRDN:
        EFUN(merlin16_core_dp_reset(sa__, 1));
    ...
    case PWRDN_DEEP:
        ...
        USR_DELAY_NS(500); /* wait >50 comclk cycles  */
                EFUN(wrc_afe_s_pll_pwrdn(0x1));
        EFUN(merlin16_INTERNAL_core_clkgate(sa__, 1));
        break;
    default:
...
err_code_t merlin16_lane_pwrdn(srds_access_t *sa__, enum srds_core_pwrdn_mode_enum mode) {
    ...
    case PWR_ON:
        ...
        EFUN(merlin16_INTERNAL_lane_clkgate(sa__, 0));
        break;
    case PWRDN:
    ...
    case PWRDN_DEEP:
        ...
        EFUN(wr_ln_rx_s_pwrdn(0x1));
        EFUN(wr_ln_tx_s_pwrdn(0x1));
        EFUN(merlin16_INTERNAL_lane_clkgate(sa__, 1));
