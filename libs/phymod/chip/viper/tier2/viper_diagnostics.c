...
int viper_phy_pmd_info_dump(const phymod_phy_access_t* phy, char* arg)
{
    return PHYMOD_E_NONE;
...
int viper_phy_pcs_info_dump(const phymod_phy_access_t* phy, char* arg)
{

    phymod_phy_access_t phy_copy;

    PHYMOD_MEMCPY(&phy_copy, phy, sizeof(phy_copy));
    PHYMOD_IF_ERR_RETURN(viper_diag_disp(&phy_copy.access, (char*)arg));
