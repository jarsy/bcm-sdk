...
static int _sgmiip2x4_prbs_enable_set (const PHYMOD_ST *pa, uint32_t enable)
{
    ...
    /* Set os2/os5 mode */
    /* to fix os mode choice */
    viper_sgmii_mode_get(pa, &sgmii_mode);
    if(!sgmii_mode){
      os_mode = VIPER_MISC8_OSDR_MODE_OSX2;
    }
    SP2_MISC8r_FORCE_OSCDR_MODEf_SET(misc8, os_mode);
    MODIFY_SP2_MISC8r(pa, misc8);
...
static int _sgmiip2x4_global_loopback_set (const PHYMOD_ST *pa, uint8_t enable)
{
    ...
    SP2_XGXSCTLr_MODEf_SET(ctlr, VIPER_XGXS_MODE_COMBO_CORE);
